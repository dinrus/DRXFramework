...
struct SineWaveVoice final : public SynthesiserVoice
{
    ...
    z0 startNote (i32 midiNoteNumber, f32 velocity,
                    SynthesiserSound*, i32 /*currentPitchWheelPosition*/) override
    {
        currentAngle = 0.0;
        level = velocity * 0.15;
        tailOff = 0.0;
    ...
    z0 renderNextBlock (AudioBuffer<f32>& outputBuffer, i32 startSample, i32 numSamples) override
    {
        if (! approximatelyEqual (angleDelta, 0.0))
        {
            if (tailOff > 0.0)
            {
                while (--numSamples >= 0)
                {
                    auto currentSample = (f32) (std::sin (currentAngle) * level * tailOff);
            ...
            else
            {
                while (--numSamples >= 0)
                {
                    auto currentSample = (f32) (std::sin (currentAngle) * level);
...
private:
    f64 currentAngle = 0.0, angleDelta = 0.0, level = 0.0, tailOff = 0.0;
};
...
struct SynthAudioSource final : public AudioSource
{
    ...
    z0 setUsingSineWaveSound()
    {
        synth.clearSounds();
        synth.addSound (new SineWaveSound());
    }

    z0 setUsingSampledSound()
    {
...
private:
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (AudioSynthesiserDemo)
};
