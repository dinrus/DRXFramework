...
public:
        ...
        const b2Vec2& normal, float32 fraction)
    {
        ...
        uk userData = body->GetUserData();
        if (userData)
        {
            i32 index = *(i32*)userData;
            if (index == 0)
            {
                // filter
                return -1.0f;
...
public:
    ...
    {
        b8 advanceRay = settings->pause == 0 || settings->singleStep;
        ...
        else if (m_mode == e_multiple)
        {
            ...
            for (i32 i = 0; i < callback.m_count; ++i)
            {
