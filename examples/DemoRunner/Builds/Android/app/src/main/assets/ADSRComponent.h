...
public:
        ...
        : envelope { *this }
    {
        for (Slider* slider : { &adsrAttack, &adsrDecay, &adsrSustain, &adsrRelease })
        {
            ...
            {
                slider->textFromValueFunction = [slider] (f64 value)
                {
                    Txt text;

                    text << slider->getName();

                    const auto val = (i32) jmap (value, 0.0, 1.0, 0.0, 100.0);
                    text << Txt::formatted (": %d%%", val);
            ...
            else
            {
                slider->textFromValueFunction = [slider] (f64 value)
                {
                    ...
                    text << ": " << ((value < 0.4f) ? Txt::formatted ("%dms", (i32) std::round (value * 1000))
                                                    : Txt::formatted ("%0.2lf Sec", value));
    ...
    ADSR::Parameters getParameters() const
    {
        return
        {
            (f32) adsrAttack.getValue(),
            (f32) adsrDecay.getValue(),
            (f32) adsrSustain.getValue(),
            (f32) adsrRelease.getValue(),
        };
    }
...
private:
    ...
    public:
        ...
        z0 paint (Graphics& g) final
        {
            ...
            g.setColor (Colors::black.withAlpha (0.1f));
            g.fillRect (bounds);

            const auto alpha = 0.4f;

            g.setColor (Color (246, 98, 92).withAlpha (alpha));
            g.fillRect (attackBounds);

            g.setColor (Color (242, 187, 60).withAlpha (alpha));
            g.fillRect (decayBounds);

            g.setColor (Color (109, 234, 166).withAlpha (alpha));
            g.fillRect (sustainBounds);

            g.setColor (Color (131, 61, 183).withAlpha (alpha));
