...
public:
    ...
    z0 Step(Settings* settings)
    {
        ...
        if (m_automated == true)
        {
            i32 actionCount = b2Max(1, e_actorCount >> 2);
    ...
    float32 RayCastCallback(const b2RayCastInput& input, i32 proxyId)
    {
        ...
        b8 hit = actor->aabb.RayCast(&output, input);
...
private:
    ...
    z0 Query()
    {
        ...
        for (i32 i = 0; i < e_actorCount; ++i)
        {
            ...
            b8 overlap = b2TestOverlap(m_queryAABB, m_actors[i].aabb);
            B2_NOT_USED(overlap);
    ...
    z0 RayCast()
    {
        ...
        for (i32 i = 0; i < e_actorCount; ++i)
        {
            ...
            b2RayCastOutput output;
            b8 hit = m_actors[i].aabb.RayCast(&output, input);
            if (hit)
            {
