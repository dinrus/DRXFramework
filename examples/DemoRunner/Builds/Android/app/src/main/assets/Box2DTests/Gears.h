...
public:
    ...
    z0 Step(Settings* settings)
    {
        ...
        m_debugDraw.DrawString(5, m_textLine, "theta1 + %4.2f * theta2 = %4.2f", (f32) ratio, (f32) value);
        m_textLine += 15;

        ratio = m_joint5->GetRatio();
        value = m_joint2->GetJointAngle() + ratio * m_joint3->GetJointTranslation();
        m_debugDraw.DrawString(5, m_textLine, "theta2 + %4.2f * delta = %4.2f", (f32) ratio, (f32) value);
        m_textLine += 15;
    }
