...
struct SidePanelHeader final : public Component
{
        ...
        : owner (o)
    {
        ...
        static u8k settingsIconPathData[]
...
public:
    ...
    {
        return (i32) (selectedCategory.isEmpty() ? DRXDemos::getCategories().size()
                                                 : DRXDemos::getCategory (selectedCategory).demos.size());
    }
    ...
    Txt getNameForRow (i32 rowNumber) override
    {
        ...
        {
            if (isPositiveAndBelow (rowNumber, DRXDemos::getCategories().size()))
                return DRXDemos::getCategories()[(size_t) rowNumber].name;
        }
        else
        {
            auto& category = DRXDemos::getCategory (selectedCategory);
    ...
    z0 returnKeyPressed (i32 row) override                       { selectRow (row); }
    z0 listBoxItemClicked (i32 row, const MouseEvent&) override  { selectRow (row); }

    //==============================================================================
    z0 showCategory (const Txt& categoryName) noexcept
    {
...
private:
    ...
    public:
        ...
        z0 paintButton (Graphics& g, b8, b8) override
        {
    ...
    //==============================================================================
    z0 selectRow (i32 row)
    {
        if (row < 0)
            return;

        if (selectedCategory.isEmpty())
            showCategory (DRXDemos::getCategories()[(size_t) row].name);
        else
...
{
    contentComponent.reset (new DemoContentComponent (*this, [this] (b8 isHeavyweight)
    {
        demosPanel.showOrHide (false);

        if (isHeavyweight)
        {
           #if DRX_MAC && USE_COREGRAPHICS_RENDERING
    ...
    demosPanel.onPanelShowHide = [this] (b8 isShowing)
    {
        if (isShowing)
        {
...
//==============================================================================
z0 MainComponent::paint (Graphics& g)
{
...
z0 MainComponent::homeButtonClicked()
{
...
z0 MainComponent::settingsButtonClicked()
{
    if (contentComponent != nullptr)
        contentComponent->setCurrentTabIndex (2);
}

z0 MainComponent::setRenderingEngine (i32 renderingEngineIndex)
{
    if (renderingEngineIndex != currentRenderingEngineIdx)
        updateRenderingEngine (renderingEngineIndex);
}

z0 MainComponent::parentHierarchyChanged()
{
...
z0 MainComponent::updateRenderingEngine (i32 renderingEngineIndex)
{
