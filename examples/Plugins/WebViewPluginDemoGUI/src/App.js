...
import * as Drx from "juce-framework-frontend";
...
function DrxSlider({ identifier, title }) {
  DrxSlider.propTypes = {
    identifier: PropTypes.string,
    title: PropTypes.string,
  };

  const sliderState = Drx.getSliderState(identifier);
...
function DrxCheckbox({ identifier }) {
  DrxCheckbox.propTypes = {
    identifier: PropTypes.string,
  };

  const checkboxState = Drx.getToggleState(identifier);
...
function DrxComboBox({ identifier }) {
  DrxComboBox.propTypes = {
    identifier: PropTypes.string,
  };

  const comboBoxState = Drx.getComboBoxState(identifier);
...
const sayHello = Drx.getNativeFunction("sayHello");
...
class SpectrumDataReceiver {
  constructor(bufferLength) {
    ...
    this.spectrumDataRegistrationId = window.__JUCE__.backend.addEventListener(
      ...
      () => {
        fetch(Drx.getBackendResourceAddress("spectrumData.json"))
...
function App() {
  const controlParameterIndexUpdater = new Drx.ControlParameterIndexUpdater(
    controlParameterIndexAnnotation
  );
  ...
  return (
    <div>
      <Container>
        <DrxSlider identifier="cutoffSlider" title="Cutoff" />
      </Container>
      ...
      <CardActions style={{ justifyContent: "center" }}>
        <Button
          ...
          sx={{ marginTop: 2 }}
          onClick={() => {
            fetch(Drx.getBackendResourceAddress("data.txt"))
              .then((response) => response.text())
              .then((text) => {
      ...
      <DrxCheckbox identifier="muteToggle" />
      <br></br>
      <DrxComboBox identifier="filterTypeCombo" />
