...
/*******************************************************************************
 ...
 type:                  AudioProcessor
 mainClass:             DrxDemoPluginAudioProcessor
...
public:
                    ...
                    i32 /*currentPitchWheelPosition*/) override
    {
        ...
        angleDelta = cyclesPerSample * MathConstants<f64>::twoPi;
    }
    ...
    z0 renderNextBlock (AudioBuffer<f32>& outputBuffer, i32 startSample, i32 numSamples) override
    {
        if (! approximatelyEqual (angleDelta, 0.0))
        {
            if (tailOff > 0.0)
            {
                while (--numSamples >= 0)
                {
                    auto currentSample = (f32) (sin (currentAngle) * level * tailOff);
            ...
            else
            {
                while (--numSamples >= 0)
                {
                    auto currentSample = (f32) (sin (currentAngle) * level);
...
class DrxDemoPluginAudioProcessor final : public AudioProcessor
{
public:
    //==============================================================================
    DrxDemoPluginAudioProcessor()
        : AudioProcessor (getBusesProperties()),
          state (*this, nullptr, "state",
                 { std::make_unique<AudioParameterFloat> (ParameterID { "gain",  1 }, "Gain",           NormalisableRange<f32> (0.0f, 1.0f), 0.9f),
                   std::make_unique<AudioParameterFloat> (ParameterID { "delay", 1 }, "Delay Feedback", NormalisableRange<f32> (0.0f, 1.0f), 0.5f) })
    {
    ...
    ~DrxDemoPluginAudioProcessor() override = default;
    ...
    {
        return new DrxDemoPluginAudioProcessorEditor (*this);
    }

    //==============================================================================
    const Txt getName() const override                             { return "AudioPluginDemo"; }
    ...
    z0 updateTrackProperties (const TrackProperties& properties) override
    {
        ...
        {
            if (auto* editor = dynamic_cast<DrxDemoPluginAudioProcessorEditor*> (getActiveEditor()))
    ...
    public:
        ...
        z0 set (const AudioPlayHead::PositionInfo& newInfo)
        {
            const drx::SpinLock::ScopedTryLockType lock (mutex);
        ...
        {
            const drx::SpinLock::ScopedLockType lock (mutex);
            return info;
        }
...
private:
    ...
    class DrxDemoPluginAudioProcessorEditor final : public AudioProcessorEditor,
                                                     private Timer,
                                                     private Value::Listener
    {
    public:
        DrxDemoPluginAudioProcessorEditor (DrxDemoPluginAudioProcessor& owner)
        ...
        ~DrxDemoPluginAudioProcessorEditor() override {}
        ...
        z0 hostMIDIControllerIsAvailable (b8 controllerIsAvailable) override
        {
            midiKeyboard.setVisible (! controllerIsAvailable);
        }

        i32 getControlParameterIndex (Component& control) override
        {
        ...
        z0 updateTrackProperties()
        {
            auto trackColor = getProcessor().getTrackProperties().colour;
            auto& lf = getLookAndFeel();

            backgroundColor = (trackColor.has_value() ? trackColor->withAlpha (1.0f).withBrightness (0.266f)
                                                        : lf.findColor (ResizableWindow::backgroundColorId));
            repaint();
        }
    ...
    private:
        ...
        //==============================================================================
        DrxDemoPluginAudioProcessor& getProcessor() const
        {
            return static_cast<DrxDemoPluginAudioProcessor&> (processor);
        }
        ...
        static Txt quarterNotePositionToBarsBeatsString (f64 quarterNotes, AudioPlayHead::TimeSignature sig)
        {
            ...
            auto bar    = ((i32) quarterNotes) / quarterNotesPerBar + 1;
            auto beat   = ((i32) beats) + 1;
            auto ticks  = ((i32) (fmod (beats, 1.0) * 960.0 + 0.5));

            return Txt::formatted ("%d|%d|%03d", bar, beat, ticks);
        }

        // Updates the text in our position label.
        z0 updateTimecodeDisplay (const AudioPlayHead::PositionInfo& pos)
        {
            MemoryOutputStream displayText;

            const auto sig = pos.getTimeSignature().orFallback (AudioPlayHead::TimeSignature{});

            displayText << "[" << SystemStats::getDRXVersion() << "]   "
                        << Txt (pos.getBpm().orFallback (120.0), 2) << " bpm, "
    ...
    z0 process (AudioBuffer<FloatType>& buffer, MidiBuffer& midiMessages, AudioBuffer<FloatType>& delayBuffer)
    {
    ...
    template <typename FloatType>
    z0 applyGain (AudioBuffer<FloatType>& buffer, AudioBuffer<FloatType>& delayBuffer, f32 gainLevel)
    {
    ...
    template <typename FloatType>
    z0 applyDelay (AudioBuffer<FloatType>& buffer, AudioBuffer<FloatType>& delayBuffer, f32 delayLevel)
    {
        auto numSamples = buffer.getNumSamples();
    ...
    AudioBuffer<f32> delayBufferFloat;
    AudioBuffer<f64> delayBufferDouble;
    ...
    z0 updateCurrentTimeInfoFromHost()
    {
        const auto newInfo = [&]
        {
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (DrxDemoPluginAudioProcessor)
};
