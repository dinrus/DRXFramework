 ...
 * dispatcher will be called with opcode=effVendorSpecific, index=effEditDraw, value=parm2, ptr=(uk)(INT_PTR)parm3, opt=message (REAPER_FXEMBED_WM_*)
 *
 ...
 * return 0 if embedding не поддерживается
*/
...
{
  i32 context;        // 0=unknown (v6.23 and earlier), 1=TCP, 2=MCP
  i32 dpi;            // 0=unknown (v6.23 and earlier), otherwise 24.8 fixed point (256=100%)
  i32 mousewheel_amt; // for REAPER_FXEMBED_WM_MOUSEWHEEL, 120 = step, typically
  f64 _res2;

  i32 width, height;
  i32 mouse_x, mouse_y;

  i32 flags; // REAPER_FXEMBED_DRAWINFO_FLAG_PAINT_OPTIONAL etc
  i32 _res3;
...
typedef struct REAPER_FXEMBED_SizeHints { // alias to MINMAXINFO
  i32 preferred_aspect; // 16.16 fixed point (65536 = 1:1, 32768 = 1:2, etc)
  i32 minimum_aspect;   // 16.16 fixed point
...
public:
  ...
  virtual u32 *getBits()=0;
  virtual i32 getWidth()=0;
  virtual i32 getHeight()=0;
  virtual i32 getRowSpan()=0; // includes any off-bitmap data. this is in sizeof(u32) units, not bytes.
  virtual b8 isFlipped() { return false;  }
  virtual b8 resize(i32 w, i32 h)=0;

  virtual uk getDC() { return 0; } // do not use

  virtual INT_PTR Extended(i32 id, uk data) { return 0; }
};
#endif
