...
/*******************************************************************************
 ...
 moduleFlags:           DRX_STRICT_REFCOUNTEDPOINTER=1
                        DRX_PLUGINHOST_LV2=1
                        DRX_PLUGINHOST_VST3=1
                        DRX_PLUGINHOST_VST=0
                        DRX_PLUGINHOST_AU=1
...
public:
    ...
    b8 isBusesLayoutSupported (const BusesLayout& layouts) const final
    {
    ...
    z0 prepareToPlay (f64 sr, i32 bs) final
    {
    ...
    z0 processBlock (AudioBuffer<f32>&, MidiBuffer&) final
    {
    ...
    const Txt getName() const final                                { return "HostPluginDemo"; }
    b8 acceptsMidi() const final                                    { return true; }
    b8 producesMidi() const final                                   { return true; }
    f64 getTailLengthSeconds() const final                         { return 0.0; }

    i32 getNumPrograms() final                                        { return 0; }
    ...
    z0 getStateInformation (MemoryBlock& destData) final
    {
        ...
        if (inner != nullptr)
        {
            xml.setAttribute (editorStyleTag, (i32) editorStyle);
    ...
    z0 setStateInformation (ukk data, i32 sizeInBytes) final
    {
        ...
        auto xml = XmlDocument::parse (Txt (CharPointer_UTF8 (static_cast<tukk> (data)), (size_t) sizeInBytes));
    ...
    z0 setNewPlugin (const PluginDescription& pd, EditorStyle where, const MemoryBlock& mb = {})
    {
        const ScopedLock sl (innerMutex);

        const auto callback = [this, where, mb] (std::unique_ptr<AudioPluginInstance> instance, const Txt& error)
        {
            if (error.isNotEmpty())
            {
            ...
            if (inner != nullptr && ! mb.isEmpty())
                inner->setStateInformation (mb.getData(), (i32) mb.getSize());
...
private:
    ...
    static constexpr tukk innerStateTag = "inner_state";
    static constexpr tukk editorStyleTag = "editor_style";

    z0 changeListenerCallback (ChangeBroadcaster* source) final
    {
...
static z0 doLayout (Component* main, Component& bottom, i32 bottomHeight, Rectangle<i32> bounds)
{
    Grid grid;
...
{
public:
    ScaledDocumentWindow (Color bg, f32 scale)
        : DocumentWindow ("Editor", bg, 0), desktopScale (scale) {}

    f32 getDesktopScaleFactor() const override { return Desktop::getInstance().getGlobalScaleFactor() * desktopScale; }
...
private:
            ...
            editor = [&]() -> std::unique_ptr<Component>
            {
                switch (hostProcessor.getEditorStyle())
                {
                    ...
                    case EditorStyle::newWindow:
                        const auto bg = getLookAndFeel().findColor (ResizableWindow::backgroundColorId).darker();
    ...
    ScopedValueSetter<std::function<z0()>> scopedCallback;
    TextButton closeButton { "Close Plugin" };
    f32 currentScaleFactor = 1.0f;
};
