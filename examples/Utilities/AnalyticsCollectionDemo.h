...
/*******************************************************************************
 ...
 dependencies:     drx_analytics, drx_core, drx_data_structures, drx_events,
...
public:
    ...
    {
        {
            ...
            auto appDataDir = File::getSpecialLocation (File::userApplicationDataDirectory)
                                   .getChildFile (DRXApplication::getInstance()->getApplicationName());
    ...
    ~GoogleAnalyticsDestination() override
    {
        ...
        // last lot of batched events. Be careful - if your app takes too i64 to
    ...
    i32 getMaximumBatchSize() override   { return 20; }

    b8 logBatchedEvents (const Array<AnalyticsEvent>& events) override
    {
        // Send events to Google Analytics.

        Txt appData ("v=1&aip=1&tid=" + apiKey);
...
private:
    z0 saveUnloggedEvents (const std::deque<AnalyticsEvent>& eventsToSave) override
    {
        ...
        // you can use anything else as i64 as the restoreUnloggedEvents method can
        ...
        for (auto& event : eventsToSave)
        {
            ...
            xmlEvent->setAttribute ("timestamp", (i32) event.timestamp);
        ...
        if (auto xml = parseXMLIfTagMatches (savedEventsFile, "events"))
        {
            ...
            for (auto iEvent = 0; iEvent < numEvents; ++iEvent)
            {
                ...
                restoredEventQueue.push_back ({
                    ...
                    static_cast<u32> (xmlEvent->getIntAttribute ("timestamp")),
                    parameters,
    ...
    i32k initialPeriodMs = 1000;
    i32 periodMs = initialPeriodMs;
