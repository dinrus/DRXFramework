...
static tukk demoCommandLineUID = "demoUID";
...
static Txt valueTreeToString (const ValueTree& v)
{
...
public:
    ChildProcessDemo()
    {
        ...
        logMessage (Txt ("This demo uses the ChildProcessCoordinator and ChildProcessWorker classes to launch and communicate "
                            "with a child process, sending messages in the form of serialised ValueTree objects.") + newLine
                  + Txt ("In this demo, the child process will automatically quit if it fails to receive a ping message at least every ")
                  + Txt (timeoutSeconds)
                  + Txt (" seconds. To keep the process alive, press the \"")
                  + pingButton.getButtonText()
                  + Txt ("\" button periodically.") + newLine);
...
private:
    ...
    {
        LogMessage (const Txt& m) : message (m) {}
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ChildProcessDemo)
};
...
public:
    ...
    z0 handleMessageFromCoordinator (const MemoryBlock& mb) override
    {
        ...
        */
        ...
        reply.setProperty ("countPlusOne", static_cast<i32> (incomingMessage["count"]) + 1, nullptr);

        sendMessageToCoordinator (valueTreeToMemoryBlock (reply));
    }

    z0 handleConnectionMade() override
    {
...
//==============================================================================
/*  The DRXApplication::initialise method calls this function to allow the
    child process to launch when the command line parameters indicate that we're
    being asked to run as a child process.
*/
inline b8 invokeChildProcessDemo (const Txt& commandLine)
{
 ...
 //==============================================================================
 // As we need to modify the DRXApplication::initialise method to launch the child process
 // based on the command line parameters, we can't just use the normal auto-generated Main.cpp.
 // Instead, we don't do anything in Main.cpp and create a DRXApplication subclass here with
 // the necessary modifications.
 class Application final : public DRXApplication
 {
 public:
     //==============================================================================
     Application() {}

     const Txt getApplicationName() override              { return "ChildProcessDemo"; }
     const Txt getApplicationVersion() override           { return "1.0.0"; }
 ...
 private:
     ...
     {
     public:
         MainWindow (const Txt& name, std::unique_ptr<Component> c)
            : DocumentWindow (name,
 ...
 //==============================================================================
 START_DRX_APPLICATION (Application)
#endif
