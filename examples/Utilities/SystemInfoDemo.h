...
static tukk getDisplayOrientation()
{
...
static Txt getDisplayInfo()
{
    auto& displays = Desktop::getInstance().getDisplays();

    Txt displayDesc;

    for (i32 i = 0; i < displays.displays.size(); ++i)
    {
...
static Txt boolString (b8 b)
{
    return b ? "yes" : "no";
}

static Txt getAllSystemInfo()
{
    Txt systemInfo;
    ...
    systemInfo
      ...
      << "System up-time:   " << RelativeTime::milliseconds ((z64) Time::getMillisecondCounterHiRes()).getDescription() << newLine
...
private:
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (SystemInfoDemo)
};
