...
struct LiveConstantDemoComponent final : public Component
{
    ...
    {
        g.fillAll (DRX_LIVE_CONSTANT (Color (0xffe5e7a7)));

        g.setColor (DRX_LIVE_CONSTANT (Colors::red.withAlpha (0.2f)));
        auto blockWidth  = DRX_LIVE_CONSTANT (0x120);
        auto blockHeight = DRX_LIVE_CONSTANT (200);
        g.fillRect ((getWidth() - blockWidth) / 2, (getHeight() - blockHeight) / 2, blockWidth, blockHeight);

        auto fontColor = DRX_LIVE_CONSTANT (Color (0xff000a55));
        auto fontSize   = DRX_LIVE_CONSTANT (30.0f);
    ...
    static Txt getDemoText()
    {
        return DRX_LIVE_CONSTANT ("Hello world!");
    }
};
...
public:
    LiveConstantDemo()
    {
        ...
        descriptionLabel.setText ("This demonstrates the DRX_LIVE_CONSTANT macro, which allows you to quickly "
                                  "adjust primitive values at runtime by just wrapping them in a macro.\n\n"
                                  "Editing DRX_LIVE_CONSTANT values is only enabled in debug builds.\n\n"
...
private:
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (LiveConstantDemo)
};
