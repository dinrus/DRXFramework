...
/*******************************************************************************
 ...
 dependencies:     drx_core, drx_data_structures, drx_events, drx_graphics,
                   drx_gui_basics, drx_gui_extra, drx_javascript
...
static constexpr const t8 javascriptSource[] = R"x(/*
...
Demo.print ("Hello World in DRX + Javascript!");
Demo.print ("");
...
public:
    ...
    z0 runScript()
    {
        ...
        if (result.failed())
            ...
            outputDisplay.insertTextAtCaret ("\n(Execution time: " + Txt (elapsedMs, 2) + " milliseconds)");
    }

    z0 consoleOutput (const Txt& message)
    {
    ...
    struct DemoClass final : public DynamicObject
    {
        ...
        DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (DemoClass)
    };
...
private:
    ...
    z0 codeDocumentTextInserted (const Txt&, i32) override    { startTimer (300); }
    z0 codeDocumentTextDeleted (i32, i32) override               { startTimer (300); }
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (JavaScriptDemo)
};
