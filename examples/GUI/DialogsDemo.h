...
public:
    ...
    z0 run() override
    {
        ...
        for (i32 i = 0; i < thingsToDo; ++i)
        {
            ...
            setStatusMessage (Txt (thingsToDo - i) + " things left to do...");
    ...
    z0 threadComplete (b8 userPressedCancel) override
    {
        const Txt messageString (userPressedCancel ? "You pressed cancel!" : "Thread finished ok!");
...
public:
    ...
    DialogsDemo()
    {
        ...
        RuntimePermissions::request (RuntimePermissions::readExternalStorage, [ptr = Component::SafePointer (this)] (b8 granted)
        {
...
private:
    ...
    {
        return [ptr = Component::SafePointer (this)] (i32 result)
        {
            if (ptr != nullptr)
                ptr->messageBox = AlertWindow::showScopedAsync (MessageBoxOptions()
                                                                    .withIconType (MessageBoxIconType::InfoIcon)
                                                                    .withTitle ("Alert Box")
                                                                    .withMessage ("Result code: " + Txt (result))
                                                                    .withButton ("OK"),
                                                                nullptr);
        };
    }
    ...
    auto getAsyncAlertBoxResultChosen()
    {
        return [ptr = Component::SafePointer (this)] (i32 result)
        {
            ...
            ptr->messageBox = AlertWindow::showScopedAsync (MessageBoxOptions()
                                                                .withIconType (MessageBoxIconType::InfoIcon)
                                                                .withTitle ("Alert Box")
                                                                .withMessage ("Result code: " + Txt (result) + newLine
                                                                              + "Option index chosen: " + Txt (optionIndexChosen) + newLine
                                                                              + "Text: " + text)
                                                                .withButton ("OK"),
                                                            nullptr);
        };
    ...
    z0 showWindow (Component& button, DialogType type)
    {
        ...
        else if (type >= loadChooser && type <= saveChooser)
        {
            ...
            else if (type == saveChooser)
            {
                ...
                {
                    fileToSave = fileToSave.getChildFile ("DRX.png");
                    fileToSave.deleteFile();

                    FileOutputStream outStream (fileToSave);

                    if (outStream.openedOk())
                        if (auto inStream = createAssetInputStream ("drx_icon.png"))
        ...
        {
            messageBox = ContentSharer::shareTextScoped ("I love DRX!", [ptr = Component::SafePointer (this)] (b8 success, const Txt& error)
            {
                if (ptr == nullptr)
                    return;

                auto resultString = success ? Txt ("success") : ("failure\n (error: " + error + ")");
        ...
        else if (type == shareFile)
        {
            ...
            if (fileToSave.createDirectory().wasOk())
            {
                ...
                messageBox = ContentSharer::shareFilesScoped (urls, [ptr = Component::SafePointer (this)] (b8 success, const Txt& error)
                {
                    if (ptr == nullptr)
                        return;
        ...
        {
            auto myImage = getImageFromAssets ("drx_icon.png");
            ...
            messageBox = ContentSharer::shareImagesScoped (images, nullptr, [ptr = Component::SafePointer (this)] (b8 success, const Txt& error)
            {
                if (ptr == nullptr)
                    return;

                Txt resultString = success ? Txt ("success")
                                              : ("failure\n (error: " + error + ")");
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (DialogsDemo)
};
