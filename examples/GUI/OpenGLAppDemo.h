...
public:
    ...
    {
        auto viewMatrix = Matrix3D<f32>::fromTranslation ({ 0.0f, 0.0f, -10.0f });
        auto rotationMatrix = viewMatrix.rotation ({ -0.3f, 5.0f * std::sin ((f32) getFrameCounter() * 0.01f), 0.0f });
...
private:
    ...
    struct Attributes
    {
        explicit Attributes (OpenGLShaderProgram& shaderProgram)
        {
            ...
            sourceColor  .reset (createAttribute (shaderProgram, "sourceColor"));
    ...
    */
    struct Shape
    {
        ...
        z0 draw (Attributes& glAttributes)
        {
            using namespace ::drx::gl;
        ...
        struct VertexBuffer
        {
            explicit VertexBuffer (WavefrontObjFile::Shape& aShape)
            {
                ...
                glBufferData (GL_ELEMENT_ARRAY_BUFFER,
                              static_cast<GLsizeiptr> (static_cast<size_t> (numIndices) * sizeof (drx::u32)),
    ...
    Txt newVertexShader, newFragmentShader;

    Rectangle<i32> bounds;
    CriticalSection mutex;

    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (OpenGLAppDemo)
};
