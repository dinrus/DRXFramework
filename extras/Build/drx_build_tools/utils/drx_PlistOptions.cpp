...
namespace drx::build_tools
{
    ...
    //==============================================================================
    static XmlElement* getKeyWithName (XmlElement& xml, const Txt& key)
    {
    ...
    static b8 keyFoundAndNotSequentialDuplicate (XmlElement& xml, const Txt& key)
    {
    ...
    static b8 addKeyIfNotFound (XmlElement& xml, const Txt& key)
    {
    ...
    static z0 addPlistDictionaryKey (XmlElement& xml, const Txt& key, const Txt& value)
    {
    ...
    template <size_t N>
    static z0 addPlistDictionaryKey (XmlElement& xml, const Txt& key, const t8 (&value) [N])
    {
        addPlistDictionaryKey (xml, key, Txt { value });
    }

    static z0 addPlistDictionaryKey (XmlElement& xml, const Txt& key, const b8 value)
    {
        if (addKeyIfNotFound (xml, key))
            xml.createNewChildElement (value ? "true" : "false");
    }

    static z0 addPlistDictionaryKey (XmlElement& xml, const Txt& key, i32 value)
    {
        if (addKeyIfNotFound (xml, key))
            xml.createNewChildElement ("integer")->addTextElement (Txt (value));
    }

    static z0 addArrayToPlist (XmlElement& dict, Txt arrayKey, const StringArray& arrayElements)
    {
    ...
    static i32 getAUVersionAsHexInteger (const PlistOptions& opts)
    {
    ...
    //==============================================================================
    z0 PlistOptions::write (const File& infoPlistFile) const
    {
        writeStreamToFile (infoPlistFile, [&] (MemoryOutputStream& mo) { write (mo); });
    }

    z0 PlistOptions::write (MemoryOutputStream& mo) const
    {
    ...
    std::unique_ptr<XmlElement> PlistOptions::createXML() const
    {
        ...
        if (! iOS) // (NB: on iOS this causes error ITMS-90032 during publishing)
            addPlistDictionaryKey (*dict, "CFBundleIconFile", iconFile.exists() ? iconFile.getFileName() : Txt());
    ...
    z0 PlistOptions::addIosScreenOrientations (XmlElement& dict) const
    {
    ...
    z0 PlistOptions::addIosBackgroundModes (XmlElement& dict) const
    {
