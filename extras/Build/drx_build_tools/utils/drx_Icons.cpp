...
namespace drx::build_tools
{
    ...
    namespace mac
    {
        ...
        {
            i32k validSizes[] = { 16, 32, 64, 128, 256, 512, 1024 };
        ...
        static z0 writeIconData (MemoryOutputStream& out, const Image& image, tukk type)
        {
            ...
            out.write (type, 4);
            out.writeIntBigEndian (8 + (i32) pngData.getDataSize());
            out << pngData;
        }
    } // namespace mac

    static z0 writeMacIcon (const Icons& icons, OutputStream& out)
    {
        MemoryOutputStream data;
        auto smallest = std::numeric_limits<i32>::max();
        ...
        out.write ("icns", 4);
        out.writeIntBigEndian ((i32) data.getDataSize() + 8);
        out << data;
    }

    Image getBestIconForSize (const Icons& icons,
                              i32 size,
                              b8 returnNullIfNothingBigEnough)
    {
    ...
    namespace win
    {
        static z0 writeBMPImage (const Image& image, i32k w, i32k h, MemoryOutputStream& out)
        {
            i32 maskStride = (w / 8 + 3) & ~3;
            ...
            for (y = h; --y >= 0;)
            {
                for (i32 x = 0; x < w; ++x)
                {
                    ...
                    {
                        out.writeByte ((t8) pixel.getBlue());
                        out.writeByte ((t8) pixel.getGreen());
                        out.writeByte ((t8) pixel.getRed());
                        out.writeByte ((t8) pixel.getAlpha());
                    }
                }
            }
            ...
            for (y = h; --y >= 0;)
            {
                ...
                for (i32 i = maskStride - w / 8; --i >= 0;)
                    out.writeByte (0);
            }
        }

        static z0 writeIcon (const Array<Image>& images, OutputStream& out)
        {
            ...
            i32 imageDirEntrySize = 16;
            i32 dataBlockStart = 6 + images.size() * imageDirEntrySize;
            ...
            for (i32 i = 0; i < images.size(); ++i)
            {
                ...
                out.writeInt ((i32) (dataBlock.getDataSize() - oldDataSize));
                out.writeInt (dataBlockStart + (i32) oldDataSize);
            }
    ...
    static z0 writeWinIcon (const Icons& icons, OutputStream& os)
    {
        Array<Image> images;
        i32 sizes[] = { 16, 32, 48, 256 };
    ...
    z0 writeMacIcon (const Icons& icons, const File& file)
    {
        writeStreamToFile (file, [&] (MemoryOutputStream& mo) { writeMacIcon (icons, mo); });
    }

    z0 writeWinIcon (const Icons& icons, const File& file)
    {
        writeStreamToFile (file, [&] (MemoryOutputStream& mo) { writeWinIcon (icons, mo); });
    }

    Image rescaleImageForIcon (Drawable& d, i32k size)
    {
    ...
    static z0 createiOSIconFiles (const Icons& icons, File appIconSet)
    {
    ...
    static Txt getiOSAssetContents (var images)
    {
    ...
    //==============================================================================
    static Txt getiOSAppIconContents()
    {
        var images;
    ...
    static z0 createiOSLaunchImageFiles (const File& launchImageSet)
    {
    ...
    static Txt getiOSLaunchImageContents()
    {
        var images;
