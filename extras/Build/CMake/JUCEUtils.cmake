...
# ==================================================================================================
# DRX Target Support Helper Functions
#
# In this file, functions intended for use by end-users have the prefix `drx_`.
...
define_property(TARGET PROPERTY JUCE_COMPANY_NAME INHERITED
    ...
    FULL_DOCS "This can be found in ProjectInfo::companyName in a generated DrxHeader.h")
...
function(_drx_available_pkgconfig_module_or_else out package alternative_package)
...
if((CMAKE_SYSTEM_NAME STREQUAL "Linux") OR (CMAKE_SYSTEM_NAME MATCHES ".*BSD"))
    _drx_create_pkgconfig_target(JUCE_CURL_LINUX_DEPS libcurl)
    _drx_available_pkgconfig_module_or_else(webkit_package_name webkit2gtk-4.1 webkit2gtk-4.0)

    # All browser related libs are loaded dynamically only if they are available during runtime
    _drx_create_pkgconfig_target(JUCE_BROWSER_LINUX_DEPS NOLINK ${webkit_package_name} gtk+-x11-3.0)
endif()
...
function(_drx_set_default_properties)
...
_drx_set_default_properties()

# ==================================================================================================

function(drx_add_bundle_resources_directory target folder)
    _drx_make_absolute(folder)
...
function(_drx_create_linux_subprocess_helper_target)
    if(TARGET drx_linux_subprocess_helper)
        return()
    endif()

    set(source "${JUCE_CMAKE_UTILS_DIR}/drx_LinuxSubprocessHelper.cpp")

    add_executable(drx_linux_subprocess_helper ${source})
    add_executable(juce::drx_linux_subprocess_helper ALIAS drx_linux_subprocess_helper)
    target_compile_features(drx_linux_subprocess_helper PRIVATE cxx_std_17)
    set(THREADS_PREFER_PTHREAD_FLAG ON)
    find_package(Threads REQUIRED)
    target_link_libraries(drx_linux_subprocess_helper PRIVATE Threads::Threads ${CMAKE_DL_LIBS})
endfunction()

function(_drx_create_embedded_linux_subprocess_target output_target_name target)
    ...
    set(prefix "_drx_directory_prefix")
    ...
    _drx_create_linux_subprocess_helper_target()

    get_target_property(generated_sources_directory ${target} JUCE_GENERATED_SOURCES_DIRECTORY)

    if(generated_sources_directory)
        set(drx_linux_subprocess_helper_binary_dir "${generated_sources_directory}/$<CONFIG>/")
    else()
        set(drx_linux_subprocess_helper_binary_dir "${CMAKE_CURRENT_BINARY_DIR}/drx_LinuxSubprocessHelper/$<CONFIG>/")
    endif()

    set(binary_header_file  "${drx_linux_subprocess_helper_binary_dir}/drx_LinuxSubprocessHelperBinaryData.h")
    set(binary_source_file  "${drx_linux_subprocess_helper_binary_dir}/drx_LinuxSubprocessHelperBinaryData.cpp")
    set(juceaide_input_file "${drx_linux_subprocess_helper_binary_dir}/input_file_list")

    file(GENERATE OUTPUT ${juceaide_input_file} CONTENT "$<TARGET_FILE:drx_linux_subprocess_helper>")
    file(MAKE_DIRECTORY ${drx_linux_subprocess_helper_binary_dir})
    ...
    add_custom_command(
        ...
        COMMAND juce::juceaide binarydata "LinuxSubprocessHelperBinaryData" "${binary_header_file}"
            ${drx_linux_subprocess_helper_binary_dir} "${juceaide_input_file}"
        COMMAND
            ${CMAKE_COMMAND} -E rename "${drx_linux_subprocess_helper_binary_dir}/BinaryData1.cpp" "${binary_source_file}"
        WORKING_DIRECTORY ${CMAKE_CURRENT_LIST_DIR}
        DEPENDS drx_linux_subprocess_helper
        VERBATIM)
    ...
    target_include_directories(${embedded_linux_subprocess_target} INTERFACE ${drx_linux_subprocess_helper_binary_dir})
...
function(drx_link_with_embedded_linux_subprocess target)
    _drx_create_embedded_linux_subprocess_target(embedded_linux_subprocess_target ${target})
...
function(_drx_link_optional_libraries target)
    if((CMAKE_SYSTEM_NAME STREQUAL "Linux") OR (CMAKE_SYSTEM_NAME MATCHES ".*BSD"))
        ...
        if(needs_browser)
            ...
            if(is_plugin)
                drx_link_with_embedded_linux_subprocess(${target})
            endif()
        endif()
    elseif(APPLE)
    ...
    elseif(WIN32)
        ...
        if (needs_webview2)
            ...
            target_link_libraries(${target} PRIVATE juce::drx_webview2)
        endif()
    endif()
endfunction()
...
function(_drx_get_module_definitions target filter out_var)
    set(compile_defs $<TARGET_GENEX_EVAL:${target},$<TARGET_PROPERTY:${target},COMPILE_DEFINITIONS>>)

    if(filter)
        set(${out_var} $<FILTER:${compile_defs},EXCLUDE,DrxPlugin_Build_|JUCE_SHARED_CODE> PARENT_SCOPE)
    else()
        set(${out_var} ${compile_defs} PARENT_SCOPE)
    endif()
endfunction()

function(_drx_append_record output key)
    string(ASCII 30 RS)
    string(ASCII 31 US)
...
function(_drx_append_target_property output key target property)
    ...
    _drx_append_record(${output} ${key} ${prop})
...
function(_drx_write_configure_time_info target)
    _drx_append_target_property(file_content EXECUTABLE_NAME                      ${target} JUCE_PRODUCT_NAME)
    _drx_append_target_property(file_content VERSION                              ${target} JUCE_VERSION)
    _drx_append_target_property(file_content BUILD_VERSION                        ${target} JUCE_BUILD_VERSION)
    _drx_append_target_property(file_content PLIST_TO_MERGE                       ${target} JUCE_PLIST_TO_MERGE)
    _drx_append_target_property(file_content BUNDLE_ID                            ${target} JUCE_BUNDLE_ID)
    _drx_append_target_property(file_content XCODE_EXTRA_PLIST_ENTRIES            ${target} JUCE_XCODE_EXTRA_PLIST_ENTRIES)
    _drx_append_target_property(file_content MICROPHONE_PERMISSION_ENABLED        ${target} JUCE_MICROPHONE_PERMISSION_ENABLED)
    _drx_append_target_property(file_content MICROPHONE_PERMISSION_TEXT           ${target} JUCE_MICROPHONE_PERMISSION_TEXT)
    _drx_append_target_property(file_content CAMERA_PERMISSION_ENABLED            ${target} JUCE_CAMERA_PERMISSION_ENABLED)
    _drx_append_target_property(file_content CAMERA_PERMISSION_TEXT               ${target} JUCE_CAMERA_PERMISSION_TEXT)
    _drx_append_target_property(file_content BLUETOOTH_PERMISSION_ENABLED         ${target} JUCE_BLUETOOTH_PERMISSION_ENABLED)
    _drx_append_target_property(file_content BLUETOOTH_PERMISSION_TEXT            ${target} JUCE_BLUETOOTH_PERMISSION_TEXT)
    _drx_append_target_property(file_content SEND_APPLE_EVENTS_PERMISSION_ENABLED ${target} JUCE_SEND_APPLE_EVENTS_PERMISSION_ENABLED)
    _drx_append_target_property(file_content SEND_APPLE_EVENTS_PERMISSION_TEXT    ${target} JUCE_SEND_APPLE_EVENTS_PERMISSION_TEXT)
    _drx_append_target_property(file_content SHOULD_ADD_STORYBOARD                ${target} JUCE_SHOULD_ADD_STORYBOARD)
    _drx_append_target_property(file_content LAUNCH_STORYBOARD_FILE               ${target} JUCE_LAUNCH_STORYBOARD_FILE)
    _drx_append_target_property(file_content ICON_FILE                            ${target} JUCE_ICON_FILE)
    _drx_append_target_property(file_content PROJECT_NAME                         ${target} JUCE_PRODUCT_NAME)
    _drx_append_target_property(file_content COMPANY_COPYRIGHT                    ${target} JUCE_COMPANY_COPYRIGHT)
    _drx_append_target_property(file_content COMPANY_NAME                         ${target} JUCE_COMPANY_NAME)
    _drx_append_target_property(file_content DOCUMENT_EXTENSIONS                  ${target} JUCE_DOCUMENT_EXTENSIONS)
    _drx_append_target_property(file_content FILE_SHARING_ENABLED                 ${target} JUCE_FILE_SHARING_ENABLED)
    _drx_append_target_property(file_content DOCUMENT_BROWSER_ENABLED             ${target} JUCE_DOCUMENT_BROWSER_ENABLED)
    _drx_append_target_property(file_content STATUS_BAR_HIDDEN                    ${target} JUCE_STATUS_BAR_HIDDEN)
    _drx_append_target_property(file_content REQUIRES_FULL_SCREEN                 ${target} JUCE_REQUIRES_FULL_SCREEN)
    _drx_append_target_property(file_content BACKGROUND_AUDIO_ENABLED             ${target} JUCE_BACKGROUND_AUDIO_ENABLED)
    _drx_append_target_property(file_content BACKGROUND_BLE_ENABLED               ${target} JUCE_BACKGROUND_BLE_ENABLED)
    _drx_append_target_property(file_content PUSH_NOTIFICATIONS_ENABLED           ${target} JUCE_PUSH_NOTIFICATIONS_ENABLED)
    _drx_append_target_property(file_content NETWORK_MULTICAST_ENABLED            ${target} JUCE_NETWORK_MULTICAST_ENABLED)
    _drx_append_target_property(file_content PLUGIN_MANUFACTURER_CODE             ${target} JUCE_PLUGIN_MANUFACTURER_CODE)
    _drx_append_target_property(file_content PLUGIN_CODE                          ${target} JUCE_PLUGIN_CODE)
    _drx_append_target_property(file_content IPHONE_SCREEN_ORIENTATIONS           ${target} JUCE_IPHONE_SCREEN_ORIENTATIONS)
    _drx_append_target_property(file_content IPAD_SCREEN_ORIENTATIONS             ${target} JUCE_IPAD_SCREEN_ORIENTATIONS)
    _drx_append_target_property(file_content PLUGIN_NAME                          ${target} JUCE_PLUGIN_NAME)
    _drx_append_target_property(file_content PLUGIN_MANUFACTURER                  ${target} JUCE_COMPANY_NAME)
    _drx_append_target_property(file_content PLUGIN_DESCRIPTION                   ${target} JUCE_DESCRIPTION)
    _drx_append_target_property(file_content PLUGIN_AU_EXPORT_PREFIX              ${target} JUCE_AU_EXPORT_PREFIX)
    _drx_append_target_property(file_content PLUGIN_AU_MAIN_TYPE                  ${target} JUCE_AU_MAIN_TYPE_CODE)
    _drx_append_target_property(file_content IS_AU_SANDBOX_SAFE                   ${target} JUCE_AU_SANDBOX_SAFE)
    _drx_append_target_property(file_content IS_PLUGIN_SYNTH                      ${target} JUCE_IS_SYNTH)
    _drx_append_target_property(file_content IS_PLUGIN_ARA_EFFECT                 ${target} JUCE_IS_ARA_EFFECT)
    _drx_append_target_property(file_content SUPPRESS_AU_PLIST_RESOURCE_USAGE     ${target} JUCE_SUPPRESS_AU_PLIST_RESOURCE_USAGE)
    _drx_append_target_property(file_content HARDENED_RUNTIME_ENABLED             ${target} JUCE_HARDENED_RUNTIME_ENABLED)
    _drx_append_target_property(file_content APP_SANDBOX_ENABLED                  ${target} JUCE_APP_SANDBOX_ENABLED)
    _drx_append_target_property(file_content APP_SANDBOX_INHERIT                  ${target} JUCE_APP_SANDBOX_INHERIT)
    _drx_append_target_property(file_content HARDENED_RUNTIME_OPTIONS             ${target} JUCE_HARDENED_RUNTIME_OPTIONS)
    _drx_append_target_property(file_content APP_SANDBOX_OPTIONS                  ${target} JUCE_APP_SANDBOX_OPTIONS)
    _drx_append_target_property(file_content APP_SANDBOX_FILE_ACCESS_HOME_RO      ${target} JUCE_APP_SANDBOX_FILE_ACCESS_HOME_RO)
    _drx_append_target_property(file_content APP_SANDBOX_FILE_ACCESS_HOME_RW      ${target} JUCE_APP_SANDBOX_FILE_ACCESS_HOME_RW)
    _drx_append_target_property(file_content APP_SANDBOX_FILE_ACCESS_ABS_RO       ${target} JUCE_APP_SANDBOX_FILE_ACCESS_ABS_RO)
    _drx_append_target_property(file_content APP_SANDBOX_FILE_ACCESS_ABS_RW       ${target} JUCE_APP_SANDBOX_FILE_ACCESS_ABS_RW)
    _drx_append_target_property(file_content APP_SANDBOX_EXCEPTION_IOKIT          ${target} JUCE_APP_SANDBOX_EXCEPTION_IOKIT)
    _drx_append_target_property(file_content APP_GROUPS_ENABLED                   ${target} JUCE_APP_GROUPS_ENABLED)
    _drx_append_target_property(file_content APP_GROUP_IDS                        ${target} JUCE_APP_GROUP_IDS)
    _drx_append_target_property(file_content IS_PLUGIN                            ${target} JUCE_IS_PLUGIN)
    _drx_append_target_property(file_content ICLOUD_PERMISSIONS_ENABLED           ${target} JUCE_ICLOUD_PERMISSIONS_ENABLED)
    _drx_append_target_property(file_content IS_AU_PLUGIN_HOST                    ${target} JUCE_PLUGINHOST_AU)

    if(CMAKE_SYSTEM_NAME STREQUAL "iOS")
        _drx_append_record(file_content IS_IOS 1)
    else()
        _drx_append_record(file_content IS_IOS 0)
    endif()

    get_target_property(drx_library_code ${target} JUCE_GENERATED_SOURCES_DIRECTORY)

    set(info_file "${drx_library_code}/Info.txt")
...
# We use the target preprocessor definitions to work out which DRX modules should go in the DrxHeader.h.
function(_drx_write_generate_time_info target)
    _drx_get_module_definitions(${target} OFF module_defs)
    _drx_append_record(defs MODULE_DEFINITIONS ${module_defs})

    _drx_append_target_property(defs EXECUTABLE_NAME                     ${target} JUCE_PRODUCT_NAME)
    _drx_append_target_property(defs PROJECT_NAME                        ${target} JUCE_PRODUCT_NAME)
    _drx_append_target_property(defs VERSION                             ${target} JUCE_VERSION)
    _drx_append_target_property(defs COMPANY_NAME                        ${target} JUCE_COMPANY_NAME)

    get_target_property(drx_library_code ${target} JUCE_GENERATED_SOURCES_DIRECTORY)

    set(defs_file "${drx_library_code}/$<CONFIG>/Defs.txt")
...
function(drx_add_binary_data target)
    ...
    if(${num_binary_files} LESS 1)
        message(FATAL_ERROR "drx_add_binary_data must be passed at least one file to encode")
    endif()
    ...
    set(drx_binary_data_folder "${CMAKE_CURRENT_BINARY_DIR}/drx_binarydata_${target}/DrxLibraryCode")

    set(binary_file_names)

    foreach(index RANGE 1 ${num_binary_files})
        list(APPEND binary_file_names "${drx_binary_data_folder}/BinaryData${index}.cpp")
    endforeach()

    file(MAKE_DIRECTORY ${drx_binary_data_folder})
    ...
    list(APPEND binary_file_names "${drx_binary_data_folder}/${JUCE_ARG_HEADER_NAME}")

    set(newline_delimited_input)

    foreach(name IN LISTS JUCE_ARG_SOURCES)
        _drx_make_absolute(name)
        set(newline_delimited_input "${newline_delimited_input}${name}\n")
    endforeach()

    set(input_file_list "${drx_binary_data_folder}/input_file_list")
    ...
    add_custom_command(OUTPUT ${binary_file_names}
        COMMAND juce::juceaide binarydata "${JUCE_ARG_NAMESPACE}" "${JUCE_ARG_HEADER_NAME}"
            ${drx_binary_data_folder} "${input_file_list}"
        WORKING_DIRECTORY ${CMAKE_CURRENT_LIST_DIR}
        DEPENDS "${input_file_list}" ${JUCE_ARG_SOURCES}
        VERBATIM)

    target_sources(${target} PRIVATE "${binary_file_names}")
    target_include_directories(${target} INTERFACE ${drx_binary_data_folder})
...
function(_drx_version_code version_in out_var)
...
function(_drx_to_char_literal str out_var help_text)
...
function(drx_generate_drx_header target)
    get_target_property(drx_library_code ${target} JUCE_GENERATED_SOURCES_DIRECTORY)

    if(NOT drx_library_code)
        message(FATAL_ERROR "Target ${target} does not have a generated sources directory. Ensure it was created with a drx_add_* function")
    endif()

    set(drx_header ${drx_library_code}/DrxHeader.h)
    target_sources(${target} PRIVATE ${drx_header})

    set(defs_file $<GENEX_EVAL:$<TARGET_PROPERTY:${target},JUCE_DEFS_FILE>>)

    set(extra_args)

    add_custom_command(OUTPUT "${drx_header}"
        COMMAND juce::juceaide header "${defs_file}" "${drx_header}" ${extra_args}
        DEPENDS "${defs_file}"
        VERBATIM)
endfunction()
...
function(_drx_execute_juceaide)
...
function(_drx_set_output_name target name)
...
function(_drx_check_icon_files_exist icon_files)
...
function(_drx_generate_icon source_target dest_target)
    get_target_property(drx_library_code ${source_target} JUCE_GENERATED_SOURCES_DIRECTORY)
    get_target_property(drx_property_icon_big ${source_target} JUCE_ICON_BIG)
    get_target_property(drx_property_icon_small ${source_target} JUCE_ICON_SMALL)

    set(icon_args)

    if(drx_property_icon_big)
        list(APPEND icon_args "${drx_property_icon_big}")
    endif()

    if(drx_property_icon_small)
        list(APPEND icon_args "${drx_property_icon_small}")
    endif()
    ...
    if(CMAKE_SYSTEM_NAME STREQUAL "Darwin")
        ...
        _drx_check_icon_files_exist("${icon_args}")

        set(generated_icon "${drx_library_code}/Icon.icns")
        # To get compiled properly, we need the icon before the plist is generated!
        _drx_execute_juceaide(macicon "${generated_icon}" ${icon_args})
    ...
    elseif(CMAKE_SYSTEM_NAME STREQUAL "Windows")
        ...
        set(generated_icon "${drx_library_code}/icon.ico")
        _drx_execute_juceaide(winicon "${generated_icon}" ${icon_args})
    ...
    if(generated_icon)
        _drx_check_icon_files_exist("${generated_icon}")
...
function(_drx_add_xcode_entitlements source_target dest_target)
    if(NOT APPLE)
        return()
    endif()

    get_target_property(drx_kind_string ${dest_target} JUCE_TARGET_KIND_STRING)
    get_target_property(input_info_file ${source_target} JUCE_INFO_FILE)

    get_target_property(drx_library_code ${source_target} JUCE_GENERATED_SOURCES_DIRECTORY)
    set(entitlements_file "${drx_library_code}/${dest_target}.entitlements")

    _drx_execute_juceaide(entitlements "${drx_kind_string}" "${input_info_file}" "${entitlements_file}")
    set_target_properties(${dest_target} PROPERTIES
...
function(_drx_configure_bundle source_target dest_target)
    _drx_generate_icon(${source_target} ${dest_target})
    _drx_write_configure_time_info(${source_target})
    ...
    set(this_output_info_dir "${drx_library_code}/${dest_target}")
    set(this_output_pkginfo "${this_output_info_dir}/PkgInfo")
    set(this_output_plist "${this_output_info_dir}/Info.plist")

    get_target_property(drx_kind_string ${dest_target} JUCE_TARGET_KIND_STRING)

    _drx_execute_juceaide(plist "${drx_kind_string}" "${input_info_file}" "${this_output_plist}")
    set_target_properties(${dest_target} PROPERTIES
        BUNDLE TRUE
        MACOSX_BUNDLE_INFO_PLIST "${this_output_plist}")

    add_custom_command(OUTPUT "${this_output_pkginfo}"
        COMMAND juce::juceaide pkginfo "${drx_kind_string}" "${this_output_pkginfo}"
        VERBATIM)
    ...
    _drx_add_xcode_entitlements(${source_target} ${dest_target})
    ...
    if(drx_kind_string STREQUAL "AUv3 AppExtension")
...
function(_drx_add_resources_rc source_target dest_target)
    ...
    if(NOT TARGET ${source_target}_rc_lib)
        ...
        set(resource_rc_file "${drx_library_code}/${source_target}_resources.rc")
...
function(_drx_configure_app_bundle source_target dest_target)
    set_target_properties(${dest_target} PROPERTIES
...
function(_drx_create_windows_package source_target dest_target extension default_icon arch_string)
...
function(_drx_add_unity_plugin_prefix_if_necessary name out_var)
...
function(_drx_add_unity_script_file shared_target out_var)
    ...
    get_target_property(drx_library_code ${shared_target} JUCE_GENERATED_SOURCES_DIRECTORY)
    _drx_add_unity_plugin_prefix_if_necessary("${plugin_name}" script_prefix)
    set(script_out "${drx_library_code}/${script_prefix}_UnityScript.cs")
...
function(_drx_copy_dir target from to)
...
function(_drx_set_copy_properties shared_code target from to_property)
...
function(_drx_adhoc_sign target)
...
function(drx_enable_copy_plugin_step shared_code_target)
    ...
    foreach(target IN LISTS active_targets)
        ...
        if(dest)
            _drx_copy_dir("${target}" "${source}" "$<GENEX_EVAL:${dest}>")
        else()
...
function(_drx_add_lv2_manifest_helper_target)
    if(TARGET drx_lv2_helper OR (CMAKE_SYSTEM_NAME STREQUAL "iOS") OR (CMAKE_SYSTEM_NAME STREQUAL "Android"))
        return()
    endif()

    get_target_property(module_path juce::drx_audio_plugin_client INTERFACE_JUCE_MODULE_PATH)
    set(source "${module_path}/drx_audio_plugin_client/LV2/drx_LV2ManifestHelper.cpp")
    add_executable(drx_lv2_helper "${source}")
    add_executable(juce::drx_lv2_helper ALIAS drx_lv2_helper)
    target_compile_features(drx_lv2_helper PRIVATE cxx_std_17)
    set_target_properties(drx_lv2_helper PROPERTIES BUILD_WITH_INSTALL_RPATH ON)
    set(THREADS_PREFER_PTHREAD_FLAG ON)
    find_package(Threads REQUIRED)
    target_link_libraries(drx_lv2_helper PRIVATE Threads::Threads ${CMAKE_DL_LIBS})
endfunction()

# ==================================================================================================

function(_drx_add_vst3_manifest_helper_target)
    if(TARGET drx_vst3_helper
    ...
    get_target_property(module_path juce::drx_audio_processors INTERFACE_JUCE_MODULE_PATH)
    set(vst3_dir "${module_path}/drx_audio_processors/format_types/VST3_SDK")
    ...
    set(source "${module_path}/drx_audio_plugin_client/VST3/drx_VST3ManifestHelper.${extension}")

    add_executable(drx_vst3_helper "${source}")
    add_executable(juce::drx_vst3_helper ALIAS drx_vst3_helper)

    target_include_directories(drx_vst3_helper PRIVATE "${vst3_dir}" "${module_path}")

    add_library(drx_interface_definitions INTERFACE)
    _drx_add_standard_defs(drx_interface_definitions)
    target_link_libraries(drx_vst3_helper PRIVATE drx_interface_definitions)
    target_compile_features(drx_vst3_helper PRIVATE cxx_std_17)

    if(CMAKE_SYSTEM_NAME STREQUAL "Darwin")
        _drx_link_frameworks(drx_vst3_helper PRIVATE Cocoa)
        target_compile_options(drx_vst3_helper PRIVATE -fobjc-arc)
    endif()

    set_target_properties(drx_vst3_helper PROPERTIES BUILD_WITH_INSTALL_RPATH ON)
    set(THREADS_PREFER_PTHREAD_FLAG ON)
    find_package(Threads REQUIRED)
    target_link_libraries(drx_vst3_helper PRIVATE Threads::Threads ${CMAKE_DL_LIBS} drx_recommended_config_flags)

    if(CMAKE_CXX_COMPILER_ID STREQUAL "GNU" AND CMAKE_CXX_COMPILER_VERSION VERSION_LESS 9)
        target_link_libraries(drx_vst3_helper PRIVATE stdc++fs)
    endif()
endfunction()

function(drx_enable_vst3_manifest_step shared_code_target)
    ...
    if(manifest_step_added)
        message(WARNING "VST3 manifest generation has already been enabled for target ${shared_code_target}. "
            "You may need to set VST3_AUTO_MANIFEST FALSE in drx_add_plugin, and/or check that you're "
            "not calling drx_enable_vst3_manifest_step multiple times.")
        return()
    endif()
    ...
    if(copy_step_added)
        message(FATAL_ERROR "VST3 manifest generation would run after plugin copy step, so it has been disabled. "
            "If you're manually calling drx_enable_vst3_manifest_step, then you probably need to call "
            "drx_enable_copy_plugin_step too.")
    endif()
    ...
    _drx_add_vst3_manifest_helper_target()
...
function(_drx_disable_system_includes target)
...
function(drx_set_aax_sdk_path path)
    if(TARGET drx_aax_sdk)
        message(FATAL_ERROR "drx_set_aax_sdk_path should only be called once")
    endif()

    _drx_make_absolute(path)
    ...
    if((CMAKE_SYSTEM_NAME STREQUAL "Darwin") OR (CMAKE_SYSTEM_NAME STREQUAL "Windows"))
        add_library(drx_aax_sdk INTERFACE IMPORTED GLOBAL)
    else()
        return()
    endif()

    _drx_disable_system_includes(drx_aax_sdk)
    target_include_directories(drx_aax_sdk INTERFACE
        "${path}"
        "${path}/Interfaces"
        "${path}/Interfaces/ACF")
    set_target_properties(drx_aax_sdk PROPERTIES INTERFACE_JUCE_AAX_DEFAULT_ICON "${path}/Utilities/PlugIn.ico")
endfunction()

function(_drx_init_bundled_aax_sdk)
    if(TARGET drx_aax_sdk)
        return()
    endif()

    get_target_property(module_path juce::drx_audio_plugin_client INTERFACE_JUCE_MODULE_PATH)
    drx_set_aax_sdk_path("${module_path}/drx_audio_plugin_client/AAX/SDK")
endfunction()

# ==================================================================================================

function(_drx_set_plugin_target_properties shared_code_target kind)
    ...
    elseif(kind STREQUAL "VST3")
        ...
        if(CMAKE_SYSTEM_NAME STREQUAL "Windows")
            ...
            _drx_create_windows_package(${shared_code_target} ${target_name} vst3 "" "${windows_arch}-win")
        endif()
        ...
        _drx_set_copy_properties(${shared_code_target} ${target_name} "${output_path}" JUCE_VST3_COPY_DIR)

        _drx_adhoc_sign(${target_name})
        ...
        if(vst3_auto_manifest)
            drx_enable_vst3_manifest_step(${shared_code_target})
        endif()
    elseif(kind STREQUAL "VST")
        ...
        _drx_set_copy_properties(${shared_code_target} ${target_name} "${output_path}" JUCE_VST_COPY_DIR)
    elseif(kind STREQUAL "AU")
        ...
        set(output_path "$<TARGET_BUNDLE_DIR:${target_name}>")
        _drx_set_copy_properties(${shared_code_target} ${target_name} "${output_path}" JUCE_AU_COPY_DIR)
    ...
    elseif(kind STREQUAL "AAX")
        ...
        _drx_init_bundled_aax_sdk()
        get_target_property(default_icon drx_aax_sdk INTERFACE_JUCE_AAX_DEFAULT_ICON)
        ...
        if(CMAKE_SYSTEM_NAME STREQUAL "Windows")
            ...
            _drx_create_windows_package(${shared_code_target} ${target_name} aaxplugin "${default_icon}" "${windows_arch}")
        endif()

        set(output_path "${products_folder}/${product_name}.aaxplugin")
        _drx_set_copy_properties(${shared_code_target} ${target_name} "${output_path}" JUCE_AAX_COPY_DIR)
    elseif(kind STREQUAL "Unity")
        ...
        _drx_add_unity_script_file(${shared_code_target} script_file)
        ...
        if(CMAKE_SYSTEM_NAME STREQUAL "Darwin")
            set(output_path "$<TARGET_BUNDLE_DIR:${target_name}>")
            _drx_set_copy_properties(${shared_code_target} ${target_name} "${output_path}" JUCE_UNITY_COPY_DIR)
        else()
            ...
            _drx_set_copy_properties(${shared_code_target}
                ${target_name}
    ...
    elseif(kind STREQUAL "LV2")
        ...
        if(NOT JUCE_LV2URI MATCHES "https?://.*|urn:.*")
            message(WARNING
                ...
                "Check the LV2URI argument to drx_add_plugin.")
        endif()

        set(source_header "${JUCE_CMAKE_UTILS_DIR}/DrxLV2Defines.h.in")
        get_target_property(drx_library_code "${shared_code_target}" JUCE_GENERATED_SOURCES_DIRECTORY)
        configure_file("${source_header}" "${drx_library_code}/DrxLV2Defines.h")
        ...
        _drx_add_lv2_manifest_helper_target()

        add_custom_command(TARGET ${target_name} POST_BUILD
            COMMAND drx_lv2_helper "$<TARGET_FILE:${target_name}>"
            VERBATIM)

        _drx_set_copy_properties(${shared_code_target} ${target_name} "${output_path}" JUCE_LV2_COPY_DIR)
    endif()
endfunction()

# Place plugin wrapper targets alongside the shared code target in IDEs
function(_drx_set_plugin_folder_property shared_target wrapper_target)
...
function(_drx_get_plugin_kind_name kind out_var)
    if(kind STREQUAL "AU")
...
function(_drx_link_plugin_wrapper shared_code_target kind)
    set(target_name ${shared_code_target}_${kind})
    ...
    target_link_libraries(${target_name} PRIVATE
        ${shared_code_target}
        juce::drx_audio_plugin_client_${kind})

    _drx_set_output_name(${target_name} $<TARGET_PROPERTY:${shared_code_target},JUCE_PRODUCT_NAME>)

    _drx_set_plugin_folder_property("${shared_code_target}" "${target_name}")

    _drx_get_plugin_kind_name(${kind} drx_kind_string)
    set_target_properties(${target_name} PROPERTIES
        ...
        JUCE_TARGET_KIND_STRING "${drx_kind_string}")
    ...
    if(NOT kind STREQUAL "LV2")
        _drx_configure_bundle(${shared_code_target} ${target_name})
    else()
        _drx_write_configure_time_info(${shared_code_target})
    endif()

    _drx_set_plugin_target_properties(${shared_code_target} ${kind})
    _drx_add_resources_rc(${shared_code_target} ${target_name})
endfunction()

# ==================================================================================================

function(_drx_get_vst3_category_string target out_var)
...
function(_drx_configure_plugin_targets target)
    _drx_set_output_name(${target} $<TARGET_PROPERTY:${target},JUCE_PRODUCT_NAME>_SharedCode)

    target_link_libraries(${target} PRIVATE juce::drx_audio_plugin_client)

    get_target_property(enabled_formats ${target} JUCE_FORMATS)

    set(active_formats)
    _drx_get_platform_plugin_kinds(plugin_kinds)
    ...
    if((VST IN_LIST active_formats) AND (NOT TARGET drx_vst2_sdk))
        message(FATAL_ERROR "Use drx_set_vst2_sdk_path to set up the VST sdk before adding VST targets")
    endif()

    _drx_add_standard_defs(${target})
    _drx_add_plugin_definitions(${target} PRIVATE ${active_formats})

    # The plugin wrappers need to know what other modules are available, especially
    # drx_audio_utils and drx_gui_basics. We achieve this by searching for
    # JUCE_MODULE_AVAILABLE_ private compile definitions, and reexporting them in
    # the interface compile definitions.
    _drx_get_module_definitions(${target} ON enabled_modules)
    ...
    _drx_version_code(${project_version_string} project_version_hex)
    ...
    _drx_to_char_literal(${project_manufacturer_code} project_manufacturer_code "plugin manufacturer")
    _drx_to_char_literal(${project_plugin_code} project_plugin_code "plugin")

    _drx_get_vst3_category_string(${target} vst3_category_string)

    target_compile_definitions(${target} PUBLIC
        JUCE_STANDALONE_APPLICATION=DrxPlugin_Build_Standalone
        DrxPlugin_IsSynth=$<BOOL:$<TARGET_PROPERTY:${target},JUCE_IS_SYNTH>>
        DrxPlugin_ManufacturerCode=0x${project_manufacturer_code}
        DrxPlugin_Manufacturer="$<TARGET_PROPERTY:${target},JUCE_COMPANY_NAME>"
        DrxPlugin_ManufacturerWebsite="$<TARGET_PROPERTY:${target},JUCE_COMPANY_WEBSITE>"
        DrxPlugin_ManufacturerEmail="$<TARGET_PROPERTY:${target},JUCE_COMPANY_EMAIL>"
        DrxPlugin_PluginCode=0x${project_plugin_code}
        DrxPlugin_ProducesMidiOutput=$<BOOL:$<TARGET_PROPERTY:${target},JUCE_NEEDS_MIDI_OUTPUT>>
        DrxPlugin_IsMidiEffect=$<BOOL:$<TARGET_PROPERTY:${target},JUCE_IS_MIDI_EFFECT>>
        DrxPlugin_WantsMidiInput=$<BOOL:$<TARGET_PROPERTY:${target},JUCE_NEEDS_MIDI_INPUT>>
        DrxPlugin_EditorRequiresKeyboardFocus=$<BOOL:$<TARGET_PROPERTY:${target},JUCE_EDITOR_WANTS_KEYBOARD_FOCUS>>
        DrxPlugin_Name="$<TARGET_PROPERTY:${target},JUCE_PLUGIN_NAME>"
        DrxPlugin_Desc="$<TARGET_PROPERTY:${target},JUCE_DESCRIPTION>"
        DrxPlugin_Version=${project_version_string}
        DrxPlugin_VersionString="${project_version_string}"
        DrxPlugin_VersionCode=${project_version_hex}
        DrxPlugin_VSTUniqueID=DrxPlugin_PluginCode
        DrxPlugin_VSTCategory=$<TARGET_PROPERTY:${target},JUCE_VST2_CATEGORY>
        DrxPlugin_Vst3Category="${vst3_category_string}"
        DrxPlugin_AUMainType=$<TARGET_PROPERTY:${target},JUCE_AU_MAIN_TYPE_CODE>
        DrxPlugin_AUSubType=DrxPlugin_PluginCode
        DrxPlugin_AUExportPrefix=$<TARGET_PROPERTY:${target},JUCE_AU_EXPORT_PREFIX>
        DrxPlugin_AUExportPrefixQuoted="$<TARGET_PROPERTY:${target},JUCE_AU_EXPORT_PREFIX>"
        DrxPlugin_AUManufacturerCode=DrxPlugin_ManufacturerCode
        DrxPlugin_CFBundleIdentifier=$<TARGET_PROPERTY:${target},JUCE_BUNDLE_ID>
        DrxPlugin_AAXIdentifier=$<TARGET_PROPERTY:${target},JUCE_AAX_IDENTIFIER>
        DrxPlugin_AAXManufacturerCode=DrxPlugin_ManufacturerCode
        DrxPlugin_AAXProductId=DrxPlugin_PluginCode
        DrxPlugin_AAXCategory=$<TARGET_PROPERTY:${target},JUCE_AAX_CATEGORY>
        DrxPlugin_AAXDisableBypass=$<BOOL:$<TARGET_PROPERTY:${target},JUCE_DISABLE_AAX_BYPASS>>
        DrxPlugin_AAXDisableMultiMono=$<BOOL:$<TARGET_PROPERTY:${target},JUCE_DISABLE_AAX_MULTI_MONO>>
        DrxPlugin_VSTNumMidiInputs=$<TARGET_PROPERTY:${target},JUCE_VST_NUM_MIDI_INS>
        DrxPlugin_VSTNumMidiOutputs=$<TARGET_PROPERTY:${target},JUCE_VST_NUM_MIDI_OUTS>
        DrxPlugin_Enable_ARA=$<BOOL:$<TARGET_PROPERTY:${target},JUCE_IS_ARA_EFFECT>>
        DrxPlugin_ARAFactoryID=$<TARGET_PROPERTY:${target},JUCE_ARA_FACTORY_ID>
        DrxPlugin_ARADocumentArchiveID=$<TARGET_PROPERTY:${target},JUCE_ARA_DOCUMENT_ARCHIVE_ID>
        DrxPlugin_ARACompatibleArchiveIDs=$<TARGET_PROPERTY:${target},JUCE_ARA_COMPATIBLE_ARCHIVE_IDS>
        DrxPlugin_ARAContentTypes=$<TARGET_PROPERTY:${target},JUCE_ARA_ANALYSIS_TYPES>
        DrxPlugin_ARATransformationFlags=$<TARGET_PROPERTY:${target},JUCE_ARA_TRANSFORMATION_FLAGS>)
    ...
    _drx_set_plugin_folder_property("${target}" "${target}_All")

    foreach(kind IN LISTS active_formats)
        _drx_link_plugin_wrapper(${target} ${kind})
    ...
    if(TARGET ${target}_Standalone)
        _drx_configure_app_bundle(${target} ${target}_Standalone)
    endif()

    if(TARGET ${target}_AAX)
        _drx_init_bundled_aax_sdk()
        target_link_libraries(${target}_AAX PRIVATE drx_aax_sdk)
    endif()
    ...
    if(wants_copy)
        drx_enable_copy_plugin_step("${target}")
    endif()
endfunction()
...
function(_drx_aax_categories_to_int categories_list result)
...
function(_drx_set_generic_property_if_not_set target property)
...
function(_drx_set_property_if_not_set target property)
    _drx_set_generic_property_if_not_set(${target} JUCE_${property} ${ARGN})
endfunction()

function(_drx_make_valid_4cc out_var)
...
function(_drx_set_fallback_properties target)
    _drx_set_property_if_not_set(${target} PRODUCT_NAME ${target})

    get_target_property(output_name ${target} JUCE_PRODUCT_NAME)
    _drx_set_property_if_not_set(${target} DESCRIPTION "${output_name}")
    _drx_set_property_if_not_set(${target} PLUGIN_NAME "${output_name}")

    get_target_property(real_company_name ${target} JUCE_COMPANY_NAME)
    _drx_set_property_if_not_set(${target} BUNDLE_ID "com.${real_company_name}.${target}")
    ...
    _drx_set_property_if_not_set(${target} VERSION ${PROJECT_VERSION})
    ...
    _drx_set_property_if_not_set(${target} BUILD_VERSION "${final_version}")
    ...
    _drx_set_property_if_not_set(${target} IPHONE_SCREEN_ORIENTATIONS
        UIInterfaceOrientationPortrait UIInterfaceOrientationLandscapeLeft
        UIInterfaceOrientationLandscapeRight)

    _drx_set_property_if_not_set(${target} IPAD_SCREEN_ORIENTATIONS
        UIInterfaceOrientationPortrait UIInterfaceOrientationLandscapeLeft
        UIInterfaceOrientationLandscapeRight)

    _drx_set_property_if_not_set(${target}
        LAUNCH_STORYBOARD_FILE "${JUCE_CMAKE_UTILS_DIR}/LaunchScreen.storyboard")

    _drx_set_property_if_not_set(${target} PLUGIN_MANUFACTURER_CODE "Manu")

    # The plugin code will change on each run, unless you specify one manually!
    _drx_make_valid_4cc(random_code)
    _drx_set_property_if_not_set(${target} PLUGIN_CODE ${random_code})

    _drx_set_property_if_not_set(${target} IS_SYNTH FALSE)
    _drx_set_property_if_not_set(${target} NEEDS_MIDI_INPUT FALSE)
    _drx_set_property_if_not_set(${target} NEEDS_MIDI_OUTPUT FALSE)
    _drx_set_property_if_not_set(${target} IS_MIDI_EFFECT FALSE)
    _drx_set_property_if_not_set(${target} EDITOR_WANTS_KEYBOARD_FOCUS FALSE)
    _drx_set_property_if_not_set(${target} DISABLE_AAX_BYPASS FALSE)
    _drx_set_property_if_not_set(${target} DISABLE_AAX_MULTI_MONO FALSE)

    _drx_set_property_if_not_set(${target} PLUGINHOST_AU FALSE)

    get_target_property(bundle_id ${target} JUCE_BUNDLE_ID)
    _drx_set_property_if_not_set(${target} AAX_IDENTIFIER ${bundle_id})

    _drx_set_property_if_not_set(${target} VST_NUM_MIDI_INS 16)
    _drx_set_property_if_not_set(${target} VST_NUM_MIDI_OUTS 16)

    _drx_set_property_if_not_set(${target} AU_SANDBOX_SAFE FALSE)

    _drx_set_property_if_not_set(${target} SUPPRESS_AU_PLIST_RESOURCE_USAGE FALSE)

    _drx_set_property_if_not_set(${target} HARDENED_RUNTIME_ENABLED FALSE)
    _drx_set_property_if_not_set(${target} APP_SANDBOX_ENABLED FALSE)
    _drx_set_property_if_not_set(${target} APP_SANDBOX_INHERIT FALSE)

    _drx_set_property_if_not_set(${target} VST3_AUTO_MANIFEST TRUE)

    get_target_property(is_synth ${target} JUCE_IS_SYNTH)

    # VST3_CATEGORIES
    if(is_synth)
        _drx_set_property_if_not_set(${target} VST3_CATEGORIES Instrument Synth)
    else()
        _drx_set_property_if_not_set(${target} VST3_CATEGORIES Fx)
    endif()

    # VST2_CATEGORY
    if(is_synth)
        _drx_set_property_if_not_set(${target} VST2_CATEGORY kPlugCategSynth)
    else()
        _drx_set_property_if_not_set(${target} VST2_CATEGORY kPlugCategEffect)
    endif()
    ...
    # AU MAIN TYPE
    if(is_midi_effect)
        _drx_set_property_if_not_set(${target} AU_MAIN_TYPE kAudioUnitType_MIDIProcessor)
    elseif(is_synth)
        _drx_set_property_if_not_set(${target} AU_MAIN_TYPE kAudioUnitType_MusicDevice)
    elseif(needs_midi_input)
        _drx_set_property_if_not_set(${target} AU_MAIN_TYPE kAudioUnitType_MusicEffect)
    else()
        _drx_set_property_if_not_set(${target} AU_MAIN_TYPE kAudioUnitType_Effect)
    endif()

    _drx_set_property_if_not_set(${target} TARGETED_DEVICE_FAMILY "1,2")
    ...
    _drx_set_property_if_not_set(${target} AU_EXPORT_PREFIX ${au_prefix})
    ...
    _drx_set_property_if_not_set(${target} AAX_CATEGORY ${default_aax_category})

    get_target_property(actual_aax_category ${target} JUCE_AAX_CATEGORY)
    _drx_aax_categories_to_int("${actual_aax_category}" aax_category_int)
    ...
    _drx_set_property_if_not_set(${target} LV2URI "${company_website}/plugins/${plugin_name_sanitised}")
    ...
    _drx_set_property_if_not_set(${target} ARA_ANALYSIS_TYPES 0)
    ...
    _drx_set_property_if_not_set(${target} ARA_TRANSFORMATION_FLAGS ${default_ara_transformation_flags})
    ...
    _drx_set_property_if_not_set(${target} IS_ARA_EFFECT FALSE)
    get_target_property(final_bundle_id ${target} JUCE_BUNDLE_ID)
    _drx_set_property_if_not_set(${target} ARA_FACTORY_ID "\"${final_bundle_id}.arafactory.${final_version}\"")
    _drx_set_property_if_not_set(${target} ARA_DOCUMENT_ARCHIVE_ID "\"${final_bundle_id}.aradocumentarchive.1\"")
    _drx_set_property_if_not_set(${target} ARA_COMPATIBLE_ARCHIVE_IDS "\"\"")
endfunction()

# ==================================================================================================

function(_drx_initialise_target target)
    set(one_value_args
        VERSION
        BUILD_VERSION
        PRODUCT_NAME
    ...
    set(drx_library_code "${base_folder}/DrxLibraryCode")

    set_target_properties(${target} PROPERTIES JUCE_GENERATED_SOURCES_DIRECTORY "${drx_library_code}")
    ...
    if(JUCE_ARG_ICON_BIG)
        _drx_make_absolute_and_check(JUCE_ARG_ICON_BIG)
    endif()

    if(JUCE_ARG_ICON_SMALL)
        _drx_make_absolute_and_check(JUCE_ARG_ICON_SMALL)
    endif()
    ...
    foreach(arg_string IN LISTS one_value_args multi_value_args)
        _drx_set_property_if_not_set(${target} ${arg_string} "${JUCE_ARG_${arg_string}}")
    endforeach()

    _drx_set_fallback_properties(${target})

    target_include_directories(${target} PRIVATE
        $<TARGET_PROPERTY:${target},JUCE_GENERATED_SOURCES_DIRECTORY>)
    target_link_libraries(${target} PUBLIC $<$<TARGET_EXISTS:drx_vst2_sdk>:drx_vst2_sdk>)
    ...
    if(is_pluginhost_au)
        ...
        if(CMAKE_SYSTEM_NAME STREQUAL "Darwin" OR CMAKE_SYSTEM_NAME STREQUAL "iOS")
            _drx_link_frameworks("${target}" PRIVATE CoreAudioKit)
        endif()

        if(CMAKE_SYSTEM_NAME STREQUAL "Darwin")
            _drx_link_frameworks("${target}" PRIVATE AudioUnit)
        endif()
    endif()

    _drx_write_generate_time_info(${target})
    _drx_link_optional_libraries(${target})
    _drx_fixup_module_source_groups()
endfunction()

# ==================================================================================================

function(drx_add_console_app target)
    ...
    # if we wanted to run on iOS, we'd use drx_gui_app instead.
    ...
    _drx_initialise_target(${target} ${ARGN})

    if(NOT JUCE_ARG__NO_RESOURCERC)
        set_target_properties(${target} PROPERTIES JUCE_TARGET_KIND_STRING "ConsoleApp")
        _drx_write_configure_time_info(${target})
        _drx_add_resources_rc(${target} ${target})
        _drx_add_xcode_entitlements(${target} ${target})
    endif()
endfunction()

function(drx_add_gui_app target)
    if(CMAKE_SYSTEM_NAME STREQUAL "Android")
    ...
    target_compile_definitions(${target} PRIVATE JUCE_STANDALONE_APPLICATION=1)
    _drx_initialise_target(${target} ${ARGN})
    _drx_set_output_name(${target} $<TARGET_PROPERTY:${target},JUCE_PRODUCT_NAME>)
    set_target_properties(${target} PROPERTIES JUCE_TARGET_KIND_STRING "App")
    _drx_configure_bundle(${target} ${target})
    _drx_configure_app_bundle(${target} ${target})
    _drx_add_resources_rc(${target} ${target})
endfunction()

function(drx_add_plugin target)
    add_library(${target} STATIC)
    set_target_properties(${target} PROPERTIES JUCE_IS_PLUGIN TRUE)
    _drx_initialise_target(${target} ${ARGN})
    _drx_configure_plugin_targets(${target})
endfunction()

# ==================================================================================================

function(_drx_target_args_from_plugin_characteristics out_var)
    set(pairs
...
function(_drx_get_pip_targets pip out_var)
    set(test_targets "${pip}")

    _drx_get_all_plugin_kinds(plugin_kinds)
...
function(drx_add_pip header)
    _drx_make_absolute(header)

    _drx_extract_metadata_block(JUCE_PIP_METADATA "${header}" metadata_dict)

    _drx_get_metadata("${metadata_dict}" name JUCE_PIP_NAME)
    ...
    _drx_get_metadata("${metadata_dict}" type pip_kind)

    if(NOT pip_kind)
        message(FATAL_ERROR "Error in '${header}': PIP headers must declare a `type` field")
    endif()

    _drx_get_metadata("${metadata_dict}" pluginCharacteristics pip_charateristics)

    _drx_target_args_from_plugin_characteristics(extra_target_args ${pip_charateristics})

    list(APPEND extra_target_args
        NEEDS_CURL TRUE
        NEEDS_WEB_BROWSER TRUE)

    _drx_get_metadata("${metadata_dict}" moduleFlags pip_moduleflags)
    ...
    elseif(pip_kind STREQUAL "Component")
        ...
        drx_add_gui_app(${JUCE_PIP_NAME} ${extra_target_args})
    elseif(pip_kind STREQUAL "Console")
        set(source_main "${JUCE_CMAKE_UTILS_DIR}/PIPConsole.cpp.in")
        drx_add_console_app(${JUCE_PIP_NAME} ${extra_target_args})
    else()
    ...
    # Generate Main.cpp
    _drx_get_metadata("${metadata_dict}" mainClass JUCE_PIP_MAIN_CLASS)
    get_target_property(drx_library_code ${JUCE_PIP_NAME} JUCE_GENERATED_SOURCES_DIRECTORY)
    set(pip_main "${drx_library_code}/Main.cpp")

    set(JUCE_PIP_HEADER "${header}")
    configure_file(${source_main} ${pip_main})
    target_sources(${JUCE_PIP_NAME} PRIVATE ${pip_main})

    _drx_get_metadata("${metadata_dict}" dependencies pip_dependencies)

    drx_generate_drx_header(${JUCE_PIP_NAME})
    ...
    _drx_get_pip_targets(${JUCE_PIP_NAME} pip_targets)

    # This keeps the PIPs slightly better organised in the DRX megaproject
    ...
    if(DEFINED JUCE_SOURCE_DIR)
        ...
        if((CMAKE_SYSTEM_NAME STREQUAL "iOS") AND (header MATCHES "^${JUCE_SOURCE_DIR}"))
            foreach(target_name IN LISTS pip_targets)
                if(TARGET "${target_name}")
                    drx_add_bundle_resources_directory("${target_name}" "${JUCE_SOURCE_DIR}/examples/Assets")
                endif()
            endforeach()
        endif()
...
function(drx_set_vst2_sdk_path path)
    if(TARGET drx_vst2_sdk)
        message(FATAL_ERROR "drx_set_vst2_sdk_path should only be called once")
    endif()
    ...
    add_library(drx_vst2_sdk INTERFACE IMPORTED GLOBAL)

    _drx_disable_system_includes(drx_vst2_sdk)
    # This is a bit of a hack, but we really need the VST2 paths to always follow the VST3 paths.
    target_include_directories(drx_vst2_sdk INTERFACE
        $<TARGET_PROPERTY:juce::drx_vst3_headers,INTERFACE_INCLUDE_DIRECTORIES>
        "${path}")
endfunction()

function(drx_set_vst3_sdk_path path)
    if(TARGET drx_vst3_sdk)
        message(FATAL_ERROR "drx_set_vst3_sdk_path should only be called once")
    endif()
    ...
    add_library(drx_vst3_sdk INTERFACE IMPORTED GLOBAL)

    _drx_disable_system_includes(drx_vst3_sdk)
    target_include_directories(drx_vst3_sdk INTERFACE "${path}")
endfunction()

function(drx_set_ara_sdk_path path)
    if(TARGET drx_ara_sdk)
        message(FATAL_ERROR "drx_set_ara_sdk_path should only be called once")
    endif()
    ...
    add_library(drx_ara_sdk INTERFACE IMPORTED GLOBAL)

    _drx_disable_system_includes(drx_ara_sdk)
    target_include_directories(drx_ara_sdk INTERFACE "${path}")
endfunction()

# ==================================================================================================

function(drx_disable_default_flags)
    set(langs C CXX)
