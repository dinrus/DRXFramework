...
# ==================================================================================================
# DRX Modules Support Helper Functions
#
...
if(NOT CMAKE_C_COMPILE_OBJECT)
    message(FATAL_ERROR "A C compiler is required to build DRX. Add 'C' to your project's LANGUAGES.")
endif()
...
function(_drx_find_target_architecture result)
    set(test_file "${JUCE_CMAKE_UTILS_DIR}/drx_runtime_arch_detection.cpp")
...
if((CMAKE_SYSTEM_NAME STREQUAL "Windows")
   ...
   OR (CMAKE_SYSTEM_NAME MATCHES ".*BSD"))
    ...
    if(NOT DEFINED JUCE_TARGET_ARCHITECTURE)
        _drx_find_target_architecture(target_arch)
...
function(_drx_add_interface_library target)
...
function(_drx_add_standard_defs drx_target)
    _drx_get_debug_config_genex(debug_config)
    target_compile_definitions(${drx_target} INTERFACE
...
macro(_drx_make_absolute_and_check path)
...
# the fields from a DRX module header to the target as INTERFACE_ properties.
# We can extract properties later using `_drx_get_metadata`.
...
function(_drx_extract_metadata_block delim_str file_with_block out_dict)
...
function(_drx_get_metadata target key out_var)
...
# ==================================================================================================
function(_drx_should_build_module_source filename output_var)
...
function(_drx_module_sources module_path output_path built_sources other_sources)
    ...
    foreach(filename IN LISTS module_cpp)
        _drx_should_build_module_source("${filename}" should_build_file)
...
function(_drx_get_all_plugin_kinds out)
    set(${out} AU AUv3 AAX LV2 Standalone Unity VST VST3 PARENT_SCOPE)
endfunction()

function(_drx_get_platform_plugin_kinds out)
...
function(_drx_add_plugin_definitions target visibility)
    _drx_get_all_plugin_kinds(options)
    ...
    foreach(opt IN LISTS options)
        ...
        target_compile_definitions(${target} ${visibility} "DrxPlugin_Build_${opt}=${flag_value}")
    endforeach()
endfunction()
...
# `find_library`.
function(_drx_link_frameworks target visibility)
    set(options WEAK)
    ...
    foreach(framework IN LISTS JUCE_LINK_FRAMEWORKS_UNPARSED_ARGUMENTS)
        if(CMAKE_SYSTEM_NAME STREQUAL "Darwin")
            if(JUCE_LINK_FRAMEWORKS_WEAK)
                ...
                find_library("drx_found_${framework}" "${framework}" REQUIRED)
                set(framework_flags "${drx_found_${framework}}")
            endif()
...
function(_drx_add_plugin_wrapper_target format path out_path)
    _drx_module_sources("${path}" "${out_path}" out_var headers)
    set(target_name drx_audio_plugin_client_${format})

    _drx_add_interface_library("${target_name}" ${out_var})
    _drx_add_plugin_definitions("${target_name}" INTERFACE ${format})
    _drx_add_standard_defs("${target_name}")

    target_compile_features("${target_name}" INTERFACE cxx_std_17)
    add_library("juce::${target_name}" ALIAS "${target_name}")

    if(format STREQUAL "AUv3")
        _drx_link_frameworks("${target_name}" INTERFACE AVFoundation)

        if(CMAKE_SYSTEM_NAME STREQUAL "Darwin")
            _drx_link_frameworks("${target_name}" INTERFACE AudioUnit)
        endif()
    elseif(format STREQUAL "AU")
        target_include_directories("${target_name}" INTERFACE "${out_path}/drx_audio_plugin_client/AU")
        _drx_link_frameworks("${target_name}" INTERFACE AudioUnit CoreAudioKit)
    endif()
endfunction()

# ==================================================================================================

function(_drx_link_libs_from_metadata module_name dict key)
    _drx_get_metadata("${dict}" "${key}" libs)
...
function(_drx_create_pkgconfig_target name)
    set(options NOLINK)
    cmake_parse_arguments(JUCE_ARG "${options}" "" "" ${ARGN})
    ...
    install(TARGETS pkgconfig_${name} EXPORT DRX)
...
function(_drx_add_library_path target path)
    if(EXISTS "${path}")
        target_link_directories(${target} INTERFACE ${path})
    endif()
endfunction()

function(_drx_add_module_staticlib_paths module_target module_path)
    if(CMAKE_SYSTEM_NAME STREQUAL "Darwin")
        _drx_add_library_path(${module_target} "${module_path}/libs/MacOSX")
    elseif(CMAKE_SYSTEM_NAME STREQUAL "iOS")
        _drx_add_library_path(${module_target} "${module_path}/libs/iOS")
    elseif((CMAKE_SYSTEM_NAME STREQUAL "Linux") OR (CMAKE_SYSTEM_NAME MATCHES ".*BSD"))
        _drx_add_library_path(${module_target} "${module_path}/libs/Linux/${JUCE_TARGET_ARCHITECTURE}")
    ...
    elseif(CMAKE_SYSTEM_NAME STREQUAL "Android")
        _drx_add_library_path(${module_target} "${module_path}/libs/Android/${CMAKE_ANDROID_ARCH_ABI}")
    endif()
endfunction()

# ==================================================================================================

function(_drx_remove_empty_list_elements arg)
    list(FILTER ${arg} EXCLUDE REGEX "^$")
    set(${arg} ${${arg}} PARENT_SCOPE)
endfunction()

function(drx_add_module module_path)
    set(one_value_args INSTALL_PATH ALIAS_NAMESPACE)
    cmake_parse_arguments(JUCE_ARG "" "${one_value_args}" "" ${ARGN})

    _drx_make_absolute(module_path)
    ...
    _drx_module_sources("${module_path}" "${base_path}" globbed_sources headers)

    set(all_module_sources)

    if(${module_name} STREQUAL "drx_audio_plugin_client")
        list(REMOVE_ITEM headers
            "${module_path}/LV2/drx_LV2ManifestHelper.cpp"
            "${module_path}/VST3/drx_VST3ManifestHelper.mm"
            "${module_path}/VST3/drx_VST3ManifestHelper.cpp")

        _drx_get_platform_plugin_kinds(plugin_kinds)

        foreach(kind IN LISTS plugin_kinds)
            _drx_add_plugin_wrapper_target(${kind} "${module_path}" "${base_path}")
        endforeach()
    ...
    _drx_add_interface_library(${module_name} ${all_module_sources})

    set_property(GLOBAL APPEND PROPERTY _drx_module_names ${module_name})
    ...
    if(${module_name} STREQUAL "drx_core")
        _drx_add_standard_defs(${module_name})

        target_link_libraries(drx_core INTERFACE juce::drx_atomic_wrapper)

        if(CMAKE_SYSTEM_NAME MATCHES ".*BSD")
            target_link_libraries(drx_core INTERFACE execinfo)
        elseif(CMAKE_SYSTEM_NAME STREQUAL "Android")
            target_sources(drx_core INTERFACE "${ANDROID_NDK}/sources/android/cpufeatures/cpu-features.c")
            target_include_directories(drx_core INTERFACE "${ANDROID_NDK}/sources/android/cpufeatures")
            target_link_libraries(drx_core INTERFACE android log)
        endif()
    endif()

    if(${module_name} STREQUAL "drx_audio_processors")
        add_library(drx_vst3_headers INTERFACE)

        target_compile_definitions(drx_vst3_headers INTERFACE "$<$<TARGET_EXISTS:drx_vst3_sdk>:JUCE_CUSTOM_VST3_SDK=1>")

        target_include_directories(drx_vst3_headers INTERFACE
            "$<$<TARGET_EXISTS:drx_vst3_sdk>:$<TARGET_PROPERTY:drx_vst3_sdk,INTERFACE_INCLUDE_DIRECTORIES>>"
            "$<$<NOT:$<TARGET_EXISTS:drx_vst3_sdk>>:${base_path}/drx_audio_processors/format_types/VST3_SDK>")

        target_link_libraries(drx_audio_processors INTERFACE drx_vst3_headers)

        add_library(drx_lilv_headers INTERFACE)
        set(lv2_base_path "${base_path}/drx_audio_processors/format_types/LV2_SDK")
        target_include_directories(drx_lilv_headers INTERFACE
            "${lv2_base_path}"
            "${lv2_base_path}/lv2"
            "${lv2_base_path}/serd"
            "${lv2_base_path}/sord"
        ...
        target_link_libraries(drx_audio_processors INTERFACE drx_lilv_headers)

        add_library(drx_ara_headers INTERFACE)

        target_include_directories(drx_ara_headers INTERFACE
            "$<$<TARGET_EXISTS:drx_ara_sdk>:$<TARGET_PROPERTY:drx_ara_sdk,INTERFACE_INCLUDE_DIRECTORIES>>")

        target_link_libraries(drx_audio_processors INTERFACE drx_ara_headers)

        if(JUCE_ARG_ALIAS_NAMESPACE)
            add_library(${JUCE_ARG_ALIAS_NAMESPACE}::drx_vst3_headers ALIAS drx_vst3_headers)
            add_library(${JUCE_ARG_ALIAS_NAMESPACE}::drx_lilv_headers ALIAS drx_lilv_headers)
            add_library(${JUCE_ARG_ALIAS_NAMESPACE}::drx_ara_headers ALIAS drx_ara_headers)
        endif()
    endif()
    ...
    if((${module_name} STREQUAL "drx_audio_devices") AND (CMAKE_SYSTEM_NAME STREQUAL "Android"))
        add_subdirectory("${module_path}/native/oboe")
        target_link_libraries(${module_name} INTERFACE oboe)
    endif()

    if((${module_name} STREQUAL "drx_opengl") AND (CMAKE_SYSTEM_NAME STREQUAL "Android"))
    ...
    _drx_extract_metadata_block(JUCE_MODULE_DECLARATION "${module_path}/${module_header_name}" metadata_dict)

    _drx_get_metadata("${metadata_dict}" minimumCppStandard module_cpp_standard)
    ...
    if(CMAKE_SYSTEM_NAME STREQUAL "Darwin")
        _drx_get_metadata("${metadata_dict}" OSXFrameworks module_osxframeworks)

        _drx_remove_empty_list_elements(module_osxframeworks)
        foreach(module_framework IN LISTS module_osxframeworks)
            _drx_link_frameworks("${module_name}" INTERFACE "${module_framework}")
        endforeach()

        _drx_get_metadata("${metadata_dict}" WeakOSXFrameworks module_weakosxframeworks)

        _drx_remove_empty_list_elements(module_weakosxframeworks)
        foreach(module_framework IN LISTS module_weakosxframeworks)
            _drx_link_frameworks("${module_name}" INTERFACE WEAK "${module_framework}")
        endforeach()

        _drx_link_libs_from_metadata("${module_name}" "${metadata_dict}" OSXLibs)
    elseif(CMAKE_SYSTEM_NAME STREQUAL "iOS")
        _drx_get_metadata("${metadata_dict}" iOSFrameworks module_iosframeworks)

        _drx_remove_empty_list_elements(module_iosframeworks)
        foreach(module_framework IN LISTS module_iosframeworks)
            _drx_link_frameworks("${module_name}" INTERFACE "${module_framework}")
        endforeach()

        _drx_get_metadata("${metadata_dict}" WeakiOSFrameworks module_weakiosframeworks)

        _drx_remove_empty_list_elements(module_weakiosframeworks)
        foreach(module_framework IN LISTS module_weakiosframeworks)
            _drx_link_frameworks("${module_name}" INTERFACE WEAK "${module_framework}")
        endforeach()

        _drx_link_libs_from_metadata("${module_name}" "${metadata_dict}" iOSLibs)
    elseif((CMAKE_SYSTEM_NAME STREQUAL "Linux") OR (CMAKE_SYSTEM_NAME MATCHES ".*BSD"))
        _drx_get_metadata("${metadata_dict}" linuxPackages module_linuxpackages)

        if(module_linuxpackages)
            _drx_create_pkgconfig_target(${module_name}_LINUX_DEPS ${module_linuxpackages})
            target_link_libraries(${module_name} INTERFACE juce::pkgconfig_${module_name}_LINUX_DEPS)
        endif()

        _drx_link_libs_from_metadata("${module_name}" "${metadata_dict}" linuxLibs)
    elseif(CMAKE_SYSTEM_NAME STREQUAL "Windows")
        if((CMAKE_CXX_COMPILER_ID STREQUAL "MSVC") OR (CMAKE_CXX_COMPILER_FRONTEND_VARIANT STREQUAL "MSVC"))
            if(module_name MATCHES "drx_gui_basics|drx_audio_processors|drx_core|drx_graphics")
                target_compile_options(${module_name} INTERFACE /bigobj)
            endif()

            _drx_link_libs_from_metadata("${module_name}" "${metadata_dict}" windowsLibs)
        endif()
    endif()

    _drx_get_metadata("${metadata_dict}" dependencies module_dependencies)
    target_link_libraries(${module_name} INTERFACE ${module_dependencies})

    _drx_get_metadata("${metadata_dict}" searchpaths module_searchpaths)
    ...
    _drx_add_module_staticlib_paths("${module_name}" "${module_path}")
...
# source files that should not be built. This is called automatically by the drx_add_* functions.
function(_drx_fixup_module_source_groups)
    if(JUCE_ENABLE_MODULE_SOURCE_GROUPS)
        get_property(all_modules GLOBAL PROPERTY _drx_module_names)
        ...
        foreach(module_name IN LISTS all_modules)
            ...
            source_group(TREE ${path} PREFIX "DRX Modules" FILES ${header_files} ${source_files})
