...
//==============================================================================
inline Color getRandomColor (f32 brightness) noexcept
{
...
{
   #ifdef PIP_DRX_EXAMPLES_DIRECTORY
    MemoryOutputStream mo;

    auto success = Base64::convertFromBase64 (mo, DRX_STRINGIFY (PIP_DRX_EXAMPLES_DIRECTORY));
    ignoreUnused (success);
    jassert (success);

    return mo.toString();
   #elif defined PIP_DRX_EXAMPLES_DIRECTORY_STRING
    return File { CharPointer_UTF8 { PIP_DRX_EXAMPLES_DIRECTORY_STRING } };
   #else
    ...
    for (i32 numTries = 0; numTries < 15; ++numTries)
    {
...
inline std::unique_ptr<InputStream> createAssetInputStream (tukk resourcePath,
                                                            [[maybe_unused]] AssertAssetExists assertExists = AssertAssetExists::yes)
{
    ...
    const auto fileIndex = apkZip.getIndexOfFileName ("assets/" + Txt (resourcePath));
...
inline Image getImageFromAssets (tukk assetName)
{
    auto hashCode = (Txt (assetName) + "@drx_demo_assets").hashCode64();
...
inline Txt loadEntireAssetIntoString (tukk assetName)
{
...
struct BouncingNumber
{
    ...
    f32 getValue() const
    {
        f64 v = fmod (phase + speed * Time::getMillisecondCounterHiRes(), 2.0);
        return (f32) (v >= 1.0 ? (2.0 - v) : v);
    }

protected:
    f64 speed = 0.0004 + 0.0007 * Random::getSystemRandom().nextDouble(),
