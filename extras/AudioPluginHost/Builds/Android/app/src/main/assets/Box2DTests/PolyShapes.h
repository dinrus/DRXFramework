...
public:
    ...
    z0 DrawFixture(b2Fixture* fixture)
    {
    ...
    b8 ReportFixture(b2Fixture* fixture)
    {
        ...
        b8 overlap = b2TestOverlap(shape, 0, &m_circle, 0, body->GetTransform(), m_transform);
...
public:
    ...
    z0 Keyboard(u8 key)
    {
        ...
        case 'a':
            for (i32 i = 0; i < k_maxBodies; i += 2)
            {
                if (m_bodies[i])
                {
                    b8 active = m_bodies[i]->IsActive();
