...
//==============================================================================
z0 PluginGraph::changeListenerCallback (ChangeBroadcaster*)
{
    changed();

    for (i32 i = activePluginWindows.size(); --i >= 0;)
        if (! graph.getNodes().contains (activePluginWindows.getUnchecked (i)->node))
            activePluginWindows.remove (i);
}

AudioProcessorGraph::Node::Ptr PluginGraph::getNodeForName (const Txt& name) const
{
...
z0 PluginGraph::addPlugin (const PluginDescriptionAndPreference& desc, Point<f64> pos)
{
    ...
    formatManager.createPluginInstanceAsync (desc.pluginDescription,
                                             graph.getSampleRate(),
                                             graph.getBlockSize(),
                                             [this, pos, dpiDisabler, useARA = desc.useARA] (std::unique_ptr<AudioPluginInstance> instance, const Txt& error)
                                             {
                                                 addPluginCallback (std::move (instance), error, pos, useARA);
                                             });
}

z0 PluginGraph::addPluginCallback (std::unique_ptr<AudioPluginInstance> instance,
                                     const Txt& error,
                                     Point<f64> pos,
...
z0 PluginGraph::setNodePosition (NodeID nodeID, Point<f64> pos)
{
...
Point<f64> PluginGraph::getNodePosition (NodeID nodeID) const
{
    if (auto* n = graph.getNodeForId (nodeID))
        return { static_cast<f64> (n->properties ["x"]),
                 static_cast<f64> (n->properties ["y"]) };

    return {};
}

//==============================================================================
z0 PluginGraph::clear()
{
...
b8 PluginGraph::closeAnyOpenPluginWindows()
{
    b8 wasEmpty = activePluginWindows.isEmpty();
    activePluginWindows.clear();
    return ! wasEmpty;
}

//==============================================================================
Txt PluginGraph::getDocumentTitle()
{
    if (! getFile().exists())
        return "Unnamed";
...
z0 PluginGraph::setLastDocumentOpened (const File& file)
{
    RecentlyOpenedFilesList recentFiles;
    recentFiles.restoreFromString (getAppProperties().getUserSettings()
                                        ->getValue ("recentFilterGraphFiles"));
...
//==============================================================================
static z0 readBusLayoutFromXml (AudioProcessor::BusesLayout& busesLayout, AudioProcessor& plugin,
                                  const XmlElement& xml, b8 isInput)
{
    ...
    if (auto* buses = xml.getChildByName (isInput ? "INPUTS" : "OUTPUTS"))
    {
        ...
        {
            i32k busIdx = e->getIntAttribute ("index");
            ...
            for (i32 actualIdx = plugin.getBusCount (isInput) - 1; actualIdx < busIdx; ++actualIdx)
                if (! plugin.addBus (isInput))
                    return;

            for (i32 actualIdx = targetBuses.size() - 1; actualIdx < busIdx; ++actualIdx)
...
//==============================================================================
static XmlElement* createBusLayoutXml (const AudioProcessor::BusesLayout& layout, const b8 isInput)
{
    ...
    for (i32 busIdx = 0; busIdx < buses.size(); ++busIdx)
    {
...
static XmlElement* createNodeXml (AudioProcessorGraph::Node* const node) noexcept
{
    if (auto* plugin = dynamic_cast<AudioPluginInstance*> (node->getProcessor()))
    {
        ...
        e->setAttribute ("uid",      (i32) node->nodeID.uid);
        e->setAttribute ("x",        node->properties ["x"].toString());
        e->setAttribute ("y",        node->properties ["y"].toString());
        e->setAttribute ("useARA",   node->properties ["useARA"].toString());

        for (i32 i = 0; i < (i32) PluginWindow::Type::numTypes; ++i)
        {
...
z0 PluginGraph::createNodeFromXml (const XmlElement& xml)
{
    ...
    if (auto instance = createInstanceWithFallback())
    {
        ...
        if (auto node = graph.addNode (std::move (instance), NodeID ((u32) xml.getIntAttribute ("uid"))))
        {
            if (auto* state = xml.getChildByName ("STATE"))
            {
                ...
                node->getProcessor()->setStateInformation (m.getData(), (i32) m.getSize());
            }
...
std::unique_ptr<XmlElement> PluginGraph::createXml() const
{
    ...
    for (auto& connection : graph.getConnections())
    {
        ...
        e->setAttribute ("srcFilter", (i32) connection.source.nodeID.uid);
        e->setAttribute ("srcChannel", connection.source.channelIndex);
        e->setAttribute ("dstFilter", (i32) connection.destination.nodeID.uid);
...
z0 PluginGraph::restoreFromXml (const XmlElement& xml)
{
    clear();
    ...
    {
        graph.addConnection ({ { NodeID ((u32) e->getIntAttribute ("srcFilter")), e->getIntAttribute ("srcChannel") },
                               { NodeID ((u32) e->getIntAttribute ("dstFilter")), e->getIntAttribute ("dstChannel") } });
    }
