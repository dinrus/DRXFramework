...
constexpr tukk scanModeKey = "pluginScanMode";
...
private:
    z0 handleMessageFromWorker (const MemoryBlock& mb) override
    {
    ...
    b8 connectionLost = false;
    b8 gotResult = false;

    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (Superprocess)
};
...
public:
    ...
    b8 findPluginTypesFor (AudioPluginFormat& format,
...
private:
    ...
    */
    b8 addPluginDescriptions (const Txt& formatName,
    ...
    std::atomic<b8> scanInProcess { true };

    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (CustomPluginScanner)
};
...
private:
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (CustomPluginListComponent)
};
...
private:
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (PluginListWindow)
};

//==============================================================================
MainHostWindow::MainHostWindow()
    : DocumentWindow (DRXApplication::getInstance()->getApplicationName(),
                      LookAndFeel::getDefaultLookAndFeel().findColor (ResizableWindow::backgroundColorId),
...
z0 MainHostWindow::closeButtonPressed()
{
...
z0 MainHostWindow::tryToQuitApplication()
{
...
z0 MainHostWindow::changeListenerCallback (ChangeBroadcaster* changed)
{
    ...
    {
        auto title = DRXApplication::getInstance()->getApplicationName();
...
PopupMenu MainHostWindow::getMenuForIndex (i32 topLevelMenuIndex, const Txt& /*menuName*/)
{
    PopupMenu menu;
...
z0 MainHostWindow::menuItemSelected (i32 menuItemID, i32 /*topLevelMenuIndex*/)
{
    if (menuItemID == 250)
    {
        if (graphHolder != nullptr)
    ...
    else if (menuItemID >= 200 && menuItemID < 210)
    {
        ...
        getAppProperties().getUserSettings()->setValue ("pluginSortMethod", (i32) pluginSortMethod);
...
z0 MainHostWindow::menuBarActivated (b8 isActivated)
{
    if (isActivated && graphHolder != nullptr)
        Component::unfocusAllComponents();
}

z0 MainHostWindow::createPlugin (const PluginDescriptionAndPreference& desc, Point<i32> pos)
{
    if (graphHolder != nullptr)
...
static constexpr i32 menuIDBase = 0x324503f4;

static z0 addToMenu (const KnownPluginList::PluginTree& tree,
                       PopupMenu& m,
...
z0 MainHostWindow::addPluginsToMenu (PopupMenu& m)
{
    if (graphHolder != nullptr)
    {
        i32 i = 0;
...
std::optional<PluginDescriptionAndPreference> MainHostWindow::getChosenType (i32k menuID) const
{
...
z0 MainHostWindow::getAllCommands (Array<CommandID>& commands)
{
...
z0 MainHostWindow::getCommandInfo (const CommandID commandID, ApplicationCommandInfo& result)
{
    const Txt category ("General");
...
b8 MainHostWindow::perform (const InvocationInfo& info)
{
...
z0 MainHostWindow::showAudioSettings()
{
...
b8 MainHostWindow::isInterestedInFileDrag (const StringArray&)
{
    return true;
}

z0 MainHostWindow::fileDragEnter (const StringArray&, i32, i32)
{
}

z0 MainHostWindow::fileDragMove (const StringArray&, i32, i32)
{
}

z0 MainHostWindow::fileDragExit (const StringArray&)
{
}

z0 MainHostWindow::filesDropped (const StringArray& files, i32 x, i32 y)
{
    if (graphHolder != nullptr)
    {
       #if ! (DRX_ANDROID || DRX_IOS)
       ...
       #endif
        {
            ...
            auto pos = graphHolder->getLocalPoint (this, Point<i32> (x, y));

            for (i32 i = 0; i < jmin (5, typesFound.size()); ++i)
...
b8 MainHostWindow::isDoublePrecisionProcessingEnabled()
{
    if (auto* props = getAppProperties().getUserSettings())
...
b8 MainHostWindow::isAutoScalePluginWindowsEnabled()
{
    if (auto* props = getAppProperties().getUserSettings())
...
z0 MainHostWindow::updatePrecisionMenuItem (ApplicationCommandInfo& info)
{
    info.setInfo ("Double Floating-Point Precision Rendering", {}, "General", 0);
    info.setTicked (isDoublePrecisionProcessingEnabled());
}

z0 MainHostWindow::updateAutoScaleMenuItem (ApplicationCommandInfo& info)
{
