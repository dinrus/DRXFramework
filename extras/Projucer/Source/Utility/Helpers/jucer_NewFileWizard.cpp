...
namespace
{
    static Txt fillInBasicTemplateFields (const File& file, const Project::Item& item, tukk templateName)
    {
        i32 dataSize;

        if (auto* data = BinaryData::getNamedResource (templateName, dataSize))
        {
            auto fileTemplate = Txt::fromUTF8 (data, dataSize);
    ...
    static b8 fillInNewCppFileTemplate (const File& file, const Project::Item& item, tukk templateName)
    {
        return build_tools::overwriteFileWithNewDataIfDifferent (file, fillInBasicTemplateFields (file, item, templateName));
    }

    i32k menuBaseID = 0x12d83f0;
}
...
{
public:
    Txt getName() override  { return "CPP File"; }

    z0 createNewFile (Project&, Project::Item parent) override
    {
    ...
    static b8 create (NewFileWizard::Type& wizard, Project::Item parent, const File& newFile, tukk templateName)
    {
...
{
public:
    Txt getName() override  { return "Header File"; }
...
{
public:
    Txt getName() override  { return "CPP & Header File"; }
...
{
public:
    Txt getName() override  { return "Component class (split between a CPP & header)"; }
    ...
    static b8 create (NewFileWizard::Type& wizard, const Txt& className, Project::Item parent,
                        const File& newFile, tukk templateName)
    {
        auto content = fillInBasicTemplateFields (newFile, parent, templateName)
                           .replace ("%%component_class%%", className)
                           .replace ("%%include_juce%%", CodeHelpers::createIncludePathIncludeStatement (Project::getDrxSourceHFilename()));
...
private:
    virtual z0 createFiles (Project::Item parent, const Txt& className, const File& newFile)
    {
    ...
    static Txt getClassNameFieldName()  { return "Class Name"; }

    z0 createNewFileInternal (Project::Item parent)
    {
        ...
        asyncAlertWindow->addTextEditor (getClassNameFieldName(), Txt(), Txt(), false);
        ...
        auto resultCallback = [safeThis = WeakReference<NewComponentFileWizard> { this }, parent] (i32 result)
        {
            if (safeThis == nullptr)
                return;
            ...
            const Txt className (aw.getTextEditorContents (getClassNameFieldName()).trim());
    ...
    DRX_DECLARE_WEAK_REFERENCEABLE (NewComponentFileWizard)
};
...
{
public:
    Txt getName() override  { return "Component class (in a single source file)"; }

    z0 createFiles (Project::Item parent, const Txt& className, const File& newFile) override
    {
...
//==============================================================================
z0 NewFileWizard::Type::showFailedToWriteMessage (const File& file)
{
...
z0 NewFileWizard::Type::askUserToChooseNewFile (const Txt& suggestedFilename, const Txt& wildcard,
...
z0 NewFileWizard::addWizardsToMenu (PopupMenu& m) const
{
    for (i32 i = 0; i < wizards.size(); ++i)
        m.addItem (menuBaseID + i, "Add New " + wizards.getUnchecked (i)->getName() + "...");
}

b8 NewFileWizard::runWizardFromMenu (i32 chosenMenuItemID, Project& project, const Project::Item& projectGroupToAddTo) const
{
...
z0 NewFileWizard::registerWizard (Type* newWizard)
{
