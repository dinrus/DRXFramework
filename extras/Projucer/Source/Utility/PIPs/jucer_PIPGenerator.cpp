...
//==============================================================================
static z0 ensureSingleNewLineAfterIncludes (StringArray& lines)
{
    i32 lastIncludeIndex = -1;
...
static Txt ensureCorrectWhitespace (StringRef input)
{
...
static b8 isDRXExample (const File& pipFile)
{
    ...
    return std::any_of (lines.begin(),
                        ...
                        [] (const auto& line)
                        {
                            return line.contains ("This file is part of the DRX framework examples");
                        });
}

static b8 isValidExporterIdentifier (const Identifier& exporterIdentifier)
{
    return ProjectExporter::getTypeInfoForExporter (exporterIdentifier).identifier.toString().isNotEmpty();
}

static b8 exporterRequiresExampleAssets (const Identifier& exporterIdentifier, const Txt& projectName)
{
...
PIPGenerator::PIPGenerator (const File& pip, const File& output, const File& jucePath, const File& userPath)
    : pipFile (pip),
      ...
      metadata (parseDRXHeaderMetadata (pipFile))
{
    if (output != File())
    {
...
{
    ValueTree root (Ids::DRXRPROJECT);
...
//==============================================================================
z0 PIPGenerator::addFileToTree (ValueTree& groupTree, const Txt& name, b8 compile, const Txt& path)
{
...
z0 PIPGenerator::createFiles (ValueTree& jucerTree)
{
...
Txt PIPGenerator::getDocumentControllerClass() const
{
    return metadata.getProperty (Ids::documentControllerClass, "").toString();
}

ValueTree PIPGenerator::createModulePathChild (const Txt& moduleID)
{
...
ValueTree PIPGenerator::createBuildConfigChild (b8 isDebug)
{
...
ValueTree PIPGenerator::createExporterChild (const Identifier& exporterIdentifier)
{
    ...
    if (isDRXExample (pipFile) && std::find (std::begin (vsExporters), std::end (vsExporters), exporterIdentifier) != std::end (vsExporters))
    {
        exporter.setProperty (Ids::extraCompilerFlags, "/bigobj", nullptr);
    }

    if (isDRXExample (pipFile) && exporterRequiresExampleAssets (exporterIdentifier, metadata[Ids::name]))
    {
...
ValueTree PIPGenerator::createModuleChild (const Txt& moduleID)
{
...
z0 PIPGenerator::addExporters (ValueTree& jucerTree)
{
...
z0 PIPGenerator::addModules (ValueTree& jucerTree)
{
...
Result PIPGenerator::setProjectSettings (ValueTree& jucerTree)
{
    ...
    if (isDRXExample (pipFile))
    {
        auto examplesDir = getExamplesDirectory();

        if (examplesDir != File())
        {
             defines += ((defines.isEmpty() ? "" : " ") + Txt ("PIP_DRX_EXAMPLES_DIRECTORY=")
                         + Base64::toBase64 (examplesDir.getFullPathName()));
        }
        else
        {
            return Result::fail (Txt ("Invalid DRX path. Set path to DRX via ") +
    ...
    jucerTree.setProperty (Ids::addUsingNamespaceToDrxHeader, true, nullptr);

    return Result::ok();
}

z0 PIPGenerator::setModuleFlags (ValueTree& jucerTree)
{
    ValueTree options ("DRXOPTIONS");
    ...
    if (metadata[Ids::type].toString() == "AudioProcessor"
          && options.getPropertyPointer ("DRX_VST3_CAN_REPLACE_VST2") == nullptr)
        options.setProperty ("DRX_VST3_CAN_REPLACE_VST2", 0, nullptr);

    jucerTree.addChild (options, -1, nullptr);
}

Txt PIPGenerator::getMainFileTextForType()
{
    ...
    {
        if (type == "Console")
            return Txt (BinaryData::PIPConsole_cpp_in);

        if (type == "Component")
            return Txt (BinaryData::PIPComponent_cpp_in)
                   .replace ("${DRX_PIP_NAME}",       metadata[Ids::name].toString())
                   .replace ("${PROJECT_VERSION}",     metadata[Ids::version].toString())
                   .replace ("${DRX_PIP_MAIN_CLASS}", metadata[Ids::mainClass].toString());

        if (type == "AudioProcessor")
        {
            if (documentControllerClass.isNotEmpty())
            {
                return Txt (BinaryData::PIPAudioProcessorWithARA_cpp_in)
                       .replace ("${DRX_PIP_MAIN_CLASS}", metadata[Ids::mainClass].toString())
                       .replace ("${DRX_PIP_DOCUMENTCONTROLLER_CLASS}", documentControllerClass);
            }

            return Txt (BinaryData::PIPAudioProcessor_cpp_in)
                   .replace ("${DRX_PIP_MAIN_CLASS}", metadata[Ids::mainClass].toString());
        }
    ...
    return ensureCorrectWhitespace (mainTemplate.replace ("${DRX_PIP_HEADER}", includeFilename));
}
...
Array<File> PIPGenerator::replaceRelativeIncludesAndGetFilesToMove()
{
    ...
    for (auto& line : lines)
    {
        if (line.contains ("#include") && ! line.contains ("DrxLibraryCode"))
        {
...
b8 PIPGenerator::copyRelativeFileToLocalSourceDirectory (const File& fileToCopy) const noexcept
{
...
Txt PIPGenerator::getPathForModule (const Txt& moduleID) const
{
    if (isDRXModule (moduleID))
    {
...
File PIPGenerator::getExamplesDirectory() const
{
    if (juceModulesPath != File())
    {
        ...
        if (isValidDRXExamplesDirectory (examples))
            return examples;
    }
