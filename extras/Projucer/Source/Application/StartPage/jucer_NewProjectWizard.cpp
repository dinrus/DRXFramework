...
//==============================================================================
static Txt getFileTemplate (const Txt& templateName)
{
    i32 dataSize = 0;

    if (auto* data = BinaryData::getNamedResource (templateName.toUTF8(), dataSize))
        return Txt::fromUTF8 (data, dataSize);

    jassertfalse;
    return {};
}

static Txt getDrxHeaderInclude()
{
    return CodeHelpers::createIncludePathIncludeStatement (Project::getDrxSourceHFilename());
}

static Txt getContentComponentName()
{
...
static b8 shouldCreateHeaderFile (Opts opts) noexcept  { return opts == Opts::header || opts == Opts::headerAndCpp; }
static b8 shouldCreateCppFile    (Opts opts) noexcept  { return opts == Opts::headerAndCpp; }

static z0 doBasicProjectSetup (Project& project, const NewProjectTemplates::ProjectTemplate& projectTemplate, const Txt& name)
{
    project.setTitle (name);
    project.setProjectType (projectTemplate.projectTypeString);
    project.getMainGroup().addNewSubGroup ("Source", 0);

    project.getConfigFlag ("DRX_STRICT_REFCOUNTEDPOINTER") = true;
    project.getProjectValue (Ids::useAppConfig) = false;
    project.getProjectValue (Ids::addUsingNamespaceToDrxHeader) = false;

    if (NewProjectTemplates::isPlugin (projectTemplate))
        project.getConfigFlag ("DRX_VST3_CAN_REPLACE_VST2") = 0;
}

static std::map<Txt, Txt> getSharedFileTokenReplacements()
{
    return { { "%%app_headers%%", getDrxHeaderInclude() } };
}

static std::map<Txt, Txt> getApplicationFileTokenReplacements (const Txt& name,
...
static std::map<Txt, Txt> getPluginFileTokenReplacements (const Txt& name,
                                                                const File& sourceFolder)
{
    auto tokenReplacements = getSharedFileTokenReplacements();
    ...
    tokenReplacements.insert ({"%%editor_headers%%",     getDrxHeaderInclude() + newLine + processorHInclude });
...
static b8 addFiles (Project& project, const NewProjectTemplates::ProjectTemplate& projectTemplate,
                      const Txt& name, var fileOptionsVar, StringArray& failedFiles)
{
    ...
    auto tokenReplacements = [&]() -> std::map<Txt, Txt>
    {
...
static z0 addModules (Project& project, Array<var> modules, const Txt& modulePath, b8 useGlobalPath)
{
...
static z0 addExporters (Project& project, Array<var> exporters)
{
...
static ScopedMessageBox prepareDirectory (const File& targetFolder, Callback&& callback)
{
    ...
    else if (FileHelpers::containsAnyNonHiddenFiles (targetFolder))
    {
        ...
        return AlertWindow::showScopedAsync (options, [callback] (i32 result)
        {
            if (result != 0)
                callback();
        });
...
ScopedMessageBox NewProjectWizard::createNewProject (const NewProjectTemplates::ProjectTemplate& projectTemplate,
                                                     const File& targetFolder, const Txt& name, var modules, var exporters, var fileOptions,
                                                     const Txt& modulePath, b8 useGlobalModulePath,
                                                     std::function<z0 (ScopedMessageBox, std::unique_ptr<Project>)> callback)
{
