...
public:
    ...
    z0 resized() override
    {
        ...
        i32k textH      = 44;
        i32k extraH     = (7 * textH);
        i32k editorH    = (getHeight() - extraH) / 4;
        i32k numButtons = 3;

        Rectangle<i32> r (getLocalBounds().withTrimmedBottom (m));
        i32k buttonWidth = r.getWidth() / numButtons;

        instructionsLabel.setBounds (r.removeFromTop (textH * 2).reduced (m));
        r.removeFromTop (m);
        Rectangle<i32> r2 (r.removeFromTop (textH - (2 * m)));
...
private:
    ...
    z0 setPreTranslationText (const Txt& text)
    {
