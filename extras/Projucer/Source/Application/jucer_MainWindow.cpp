...
private:
    ...
    z0 handleComponentMovedOrResized()                { triggerAsyncUpdate(); }

    z0 componentMovedOrResized (b8, b8) override  { handleComponentMovedOrResized(); }
    using ComponentMovementWatcher::componentMovedOrResized;

    z0 handleAsyncUpdate() override                   { resized(); }
    ...
    z0 refreshBackgroundImage()
    {
        ...
        componentImage = mainWindow.createComponentSnapshot (mainWindow.getLocalBounds())
                                   .rescaled (roundToInt ((f32) parentBounds.getWidth() / 1.75f),
                                              roundToInt ((f32) parentBounds.getHeight() / 1.75f));
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (BlurOverlayWithComponent)
};
...
z0 MainWindow::createProjectContentCompIfNeeded()
{
...
z0 MainWindow::updateTitleBarIcon()
{
    if (auto* peer = getPeer())
    {
        if (currentProject != nullptr)
        {
            ...
            peer->setIcon (ImageCache::getFromMemory (BinaryData::drx_icon_png, BinaryData::drx_icon_pngSize));
        }
        else
        {
...
z0 MainWindow::closeButtonPressed()
{
    ProjucerApplication::getApp().mainWindowList.closeWindow (this);
}

z0 MainWindow::closeCurrentProject (OpenDocumentManager::SaveIfNeeded askUserToSave, std::function<z0 (b8)> callback)
{
    if (currentProject == nullptr)
    {
    ...
    ProjucerApplication::getApp().openDocumentManager
        .closeAllDocumentsUsingProjectAsync (*currentProject,
                                             askUserToSave,
                                             [parent = SafePointer<MainWindow> { this }, askUserToSave, callback] (b8 closedSuccessfully)
    {
        if (parent == nullptr)
            return;
...
z0 MainWindow::moveProject (File newProjectFileToOpen, OpenInIDE openInIDE)
{
    closeCurrentProject (OpenDocumentManager::SaveIfNeeded::no,
                         [parent = SafePointer<MainWindow> { this }, newProjectFileToOpen, openInIDE] (b8)
    {
        if (parent == nullptr)
            return;

        parent->openFile (newProjectFileToOpen, [parent, openInIDE] (b8 openedSuccessfully)
        {
...
z0 MainWindow::setProject (std::unique_ptr<Project> newProject)
{
...
z0 MainWindow::restoreWindowPosition()
{
    Txt windowState;
...
b8 MainWindow::canOpenFile (const File& file) const
{
...
z0 MainWindow::openFile (const File& file, std::function<z0 (b8)> callback)
{
    ...
    if (file.exists())
    {
        ...
        if (isPIPFile (file))
        {
            openPIP (file, [parent, createCompAndShowEditor, callback] (b8 openedSuccessfully)
            {
                if (parent == nullptr)
                    return;
...
z0 MainWindow::openPIP (const File& pipFile, std::function<z0 (b8)> callback)
{
    ...
    openFile (generator->getJucerFile(), [parent = SafePointer<MainWindow> { this }, generator, callback] (b8 openedSuccessfully)
    {
        if (parent == nullptr)
            return;
...
z0 MainWindow::setupTemporaryPIPProject (PIPGenerator& generator)
{
...
b8 MainWindow::isInterestedInFileDrag (const StringArray& filenames)
{
...
static z0 filesDroppedRecursive (Component::SafePointer<MainWindow> parent, StringArray filenames)
{
    ...
    parent->openFile (f, [parent, filenames] (b8 openedSuccessfully)
    {
...
z0 MainWindow::filesDropped (const StringArray& filenames, i32 /*mouseX*/, i32 /*mouseY*/)
{
    filesDroppedRecursive (this, filenames);
}

b8 MainWindow::shouldDropFilesWhenDraggedExternally (const DragAndDropTarget::SourceDetails& sourceDetails,
                                                       StringArray& files, b8& canMoveFiles)
{
    if (auto* tv = dynamic_cast<TreeView*> (sourceDetails.sourceComponent.get()))
    {
        Array<JucerTreeViewBase*> selected;

        for (i32 i = tv->getNumSelectedItems(); --i >= 0;)
            if (auto* b = dynamic_cast<JucerTreeViewBase*> (tv->getSelectedItem (i)))
                selected.add (b);

        if (! selected.isEmpty())
        {
            for (i32 i = selected.size(); --i >= 0;)
            {
...
z0 MainWindow::activeWindowStatusChanged()
{
...
z0 MainWindow::initialiseProjectWindow()
{
    setResizable (true, false);
    updateTitleBarIcon();
}

z0 MainWindow::showStartPage()
{
    jassert (currentProject == nullptr);
...
z0 MainWindow::getAllCommands (Array <CommandID>& commands)
{
    const CommandID ids[] =
    {
...
z0 MainWindow::getCommandInfo (const CommandID commandID, ApplicationCommandInfo& result)
{
    switch (commandID)
    {
...
b8 MainWindow::perform (const InvocationInfo& info)
{
...
z0 MainWindow::valueChanged (Value& value)
{
...
z0 MainWindow::changeListenerCallback (ChangeBroadcaster* source)
{
...
z0 MainWindowList::forceCloseAllWindows()
{
    windows.clear();
}

static z0 askAllWindowsToCloseRecursive (WeakReference<MainWindowList> parent, std::function<z0 (b8)> callback)
{
    ...
    parent->windows[0]->closeCurrentProject (OpenDocumentManager::SaveIfNeeded::yes, [parent, callback] (b8 closedSuccessfully)
    {
        if (parent == nullptr)
            return;
...
z0 MainWindowList::askAllWindowsToClose (std::function<z0 (b8)> callback)
{
    saveCurrentlyOpenProjectList();
    askAllWindowsToCloseRecursive (this, std::move (callback));
}

z0 MainWindowList::createWindowIfNoneAreOpen()
{
    if (windows.isEmpty())
        createNewMainWindow()->showStartPage();
}

z0 MainWindowList::closeWindow (MainWindow* w)
{
    ...
    {
        DRXApplicationBase::getInstance()->systemRequestedQuit();
    }
    else
   #endif
    {
        w->closeCurrentProject (OpenDocumentManager::SaveIfNeeded::yes,
                                [parent = WeakReference<MainWindowList> { this }, w] (b8 closedSuccessfully)
                                {
                                    if (parent == nullptr)
                                        return;
...
z0 MainWindowList::goToSiblingWindow (MainWindow* w, i32 delta)
{
...
z0 MainWindowList::openDocument (OpenDocumentManager::Document* doc, b8 grabFocus)
{
...
z0 MainWindowList::openFile (const File& file, std::function<z0 (b8)> callback, b8 openInBackground)
{
    if (! file.exists())
    {
        NullCheckedInvocation::invoke (callback, false);
        ...
        || isPIPFile (file))
    {
        ...
        w->openFile (file, [parent, previousFrontWindow, w, openInBackground, callback] (b8 openedSuccessfully)
        {
            if (parent == nullptr)
                return;
    ...
    getFrontmostWindow()->openFile (file, [parent, callback] (b8 openedSuccessfully)
    {
        if (parent != nullptr)
            NullCheckedInvocation::invoke (callback, openedSuccessfully);
    });
...
MainWindow* MainWindowList::getFrontmostWindow (b8 createIfNotFound)
{
    if (windows.isEmpty())
    {
        if (createIfNotFound)
...
z0 MainWindowList::checkWindowBounds (MainWindow& windowToCheck)
{
    ...
    auto ensureWindowIsFullyOnscreen = [&]
    {
        ...
        Point<i32> constrainedTopLeft (constrainedX, constrainedY);
...
z0 MainWindowList::saveCurrentlyOpenProjectList()
{
    Array<File> projects;
    auto& desktop = Desktop::getInstance();

    for (i32 i = 0; i < desktop.getNumComponents(); ++i)
    {
        if (auto* mw = dynamic_cast<MainWindow*> (desktop.getComponent (i)))
...
z0 MainWindowList::reopenLastProjects()
{
    const ScopedValueSetter<b8> setter (isInReopenLastProjects, true);
...
z0 MainWindowList::sendLookAndFeelChange()
{
    for (auto* w : windows)
