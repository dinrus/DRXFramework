...
{
public:
    DownloadAndInstallThread  (const VersionInfo::Asset& a, const File& t, std::function<z0 (Result)>&& cb)
...
private:
    ...
    Result install (const MemoryBlock& data)
    {
        ...
        if (zip.getNumEntries() == 0)
            return Result::fail ("The downloaded file was not a valid DRX file!");
        ...
        if (! unzipTarget.folder.getChildFile ("DRX").moveFileTo (targetFolder))
    ...
    {
        constexpr u32 executableFlag = (1 << 22);
    ...
    File targetFolder;
    std::function<z0 (Result)> completionCallback;
};
...
z0 LatestVersionCheckerAndUpdater::checkForNewVersion (b8 background)
{
    if (! isThreadRunning())
    {
...
//==============================================================================
z0 LatestVersionCheckerAndUpdater::run()
{
    ...
    if (info == nullptr)
    {
        if (! backgroundCheck)
        {
            auto options = MessageBoxOptions::makeOptionsOk (MessageBoxIconType::WarningIcon,
                                                             ...
                                                             "Failed to communicate with the DRX update server.\n"
                                                             "Please try again in a few minutes.\n\n"
                                                             "If this problem persists you can download the latest version of DRX from drx.com");
    ...
    if (! info->isNewerVersionThanCurrent())
    {
        if (! backgroundCheck)
        {
            auto options = MessageBoxOptions::makeOptionsOk (MessageBoxIconType::InfoIcon,
                                                             ...
                                                             "Your DRX version is up to date.");
            messageBox = AlertWindow::showScopedAsync (options, nullptr);
        }
        return;
    }
    ...
    Txt requiredFilename ("drx-" + info->versionString + "-" + osString + ".zip");
...
{
public:
    UpdateDialog (const Txt& newVersion, const Txt& releaseNotes)
    {
        titleLabel.setText ("DRX version " + newVersion, dontSendNotification);
        titleLabel.setFont (FontOptions { 15.0f, Font::bold });
        titleLabel.setJustificationType (Justification::centred);
        addAndMakeVisible (titleLabel);

        contentLabel.setText ("A new version of DRX is available - would you like to download it?", dontSendNotification);
        ...
        juceIcon = Drawable::createFromImageData (BinaryData::drx_icon_png,
    ...
    static std::unique_ptr<DialogWindow> launchDialog (const Txt& newVersionString,
                                                       const Txt& releaseNotes)
    {
        DialogWindow::LaunchOptions options;

        options.dialogTitle = "Download DRX version " + newVersionString + "?";
...
private:
    ...
    {
        cancelButton.setColor (TextButton::buttonColorId, findColor (secondaryButtonBackgroundColorId));
    ...
    z0 setParentWindow (DialogWindow* parent)
    {
        parentWindow = parent;
    }

    z0 exitModalStateWithResult (i32 result)
    {
    ...
    Rectangle<i32> juceIconBounds { 10, 10, 64, 64 };

    DialogWindow* parentWindow = nullptr;
};

z0 LatestVersionCheckerAndUpdater::askUserForLocationToDownload (const VersionInfo::Asset& asset)
{
                          ...
                          [weakThis = WeakReference<LatestVersionCheckerAndUpdater> { this }, asset] (const FileChooser& fc)
    {
        ...
        // By default we will install into 'targetFolder/DRX', but we should install into
        // 'targetFolder' if that is an existing DRX directory.
        b8 willOverwriteDrxFolder = [&targetFolder]
        {
            if (isDRXFolder (targetFolder))
                return true;

            targetFolder = targetFolder.getChildFile ("DRX");

            return isDRXFolder (targetFolder);
        }();

        auto targetFolderPath = targetFolder.getFullPathName();

        const auto onResult = [weakThis, asset, targetFolder] (i32 result)
        {
        ...
        if (willOverwriteDrxFolder)
        {
            if (targetFolder.getChildFile (".git").isDirectory())
            {
                auto options = MessageBoxOptions::makeOptionsOk (MessageBoxIconType::WarningIcon,
                                                                 "Downloading New DRX Version",
                                                                 targetFolderPath + "\n\n"
                                                                 "is a GIT repository!\n\n"
            ...
            auto options = MessageBoxOptions::makeOptionsOkCancel (MessageBoxIconType::WarningIcon,
                                                                   "Overwrite Existing DRX Folder?",
                                                                   "Do you want to replace the folder\n\n" + targetFolderPath + "\n\n"
                                                                   "with the latest version from drx.com?\n\n"
...
z0 LatestVersionCheckerAndUpdater::askUserAboutNewVersion (const Txt& newVersionString,
                                                             const Txt& releaseNotes,
...
z0 LatestVersionCheckerAndUpdater::showDialogWindow (const Txt& newVersionString,
                                                       const Txt& releaseNotes,
                                                       const VersionInfo::Asset& asset)
{
    ...
    if (auto* mm = ModalComponentManager::getInstance())
    {
        mm->attachCallback (dialogWindow.get(),
                            ModalCallbackFunction::create ([this, asset] (i32 result)
                                                           {
                                                               if (result == 1)
...
z0 LatestVersionCheckerAndUpdater::addNotificationToOpenProjects (const VersionInfo::Asset& asset)
{
...
//==============================================================================
static z0 restartProcess (const File& targetFolder)
{
...
z0 LatestVersionCheckerAndUpdater::downloadAndInstall (const VersionInfo::Asset& asset, const File& targetFolder)
{
