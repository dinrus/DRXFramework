...
//==============================================================================
static auto createIcon (ukk iconData, size_t iconDataSize)
{
...
static ProjectExporter::ExporterTypeInfo createExporterTypeInfo (ukk iconData, size_t iconDataSize)
{
...
b8 ProjectExporter::canProjectBeLaunched (Project* project)
{
    if (project != nullptr)
    {
...
Txt ProjectExporter::getUniqueName() const
{
...
b8 ProjectExporter::shouldFileBeCompiledByDefault (const File& file) const
{
    return file.hasFileExtension (cOrCppFileExtensions)
        || file.hasFileExtension (asmFileExtensions);
}

z0 ProjectExporter::updateCompilerFlagValues()
{
...
//==============================================================================
z0 ProjectExporter::createPropertyEditors (PropertyListBuilder& props)
{
...
z0 ProjectExporter::createIconProperties (PropertyListBuilder& props)
{
...
//==============================================================================
z0 ProjectExporter::addSettingsForProjectType (const build_tools::ProjectType& type)
{
...
z0 ProjectExporter::addExtraIncludePathsIfPluginOrHost()
{
         ...
         || ((shouldBuildTargetType (Target::VST3PlugIn) && project.shouldBuildVST3()) || project.isVST3PluginHost()))
    {
        ...
        if (! project.isConfigFlagEnabled ("DRX_CUSTOM_VST3_SDK"))
            addToExtraSearchPaths (getInternalVST3SDKPath(), 0);
    }

    const auto lv2BasePath = getModuleFolderRelativeToProject ("drx_audio_processors").getChildFile ("format_types")
                                                                                       .getChildFile ("LV2_SDK");

    if ((shouldBuildTargetType (Target::LV2PlugIn) && project.shouldBuildLV2()) || project.isLV2PluginHost())
    {
        const std::vector<tukk> paths[] { { "" },
                                                 { "lv2" },
                                                 { "serd" },
                                                 { "sord" },
                                                 { "sord", "src" },
        ...
        {
            const auto appendComponent = [] (const build_tools::RelativePath& f, tukk component)
            {
...
z0 ProjectExporter::addARAPathsIfPluginOrHost()
{
    if (project.shouldEnableARA() || project.isARAPluginHost())
        addARAFoldersToPath();
}

z0 ProjectExporter::addCommonAudioPluginSettings()
{
    if (shouldBuildTargetType (build_tools::ProjectType::Target::AAXPlugIn))
        addAAXFoldersToPath();
}

z0 ProjectExporter::addLegacyVSTFolderToPathIfSpecified()
{
...
{
    return getModuleFolderRelativeToProject ("drx_audio_processors")
                           .getChildFile ("format_types")
                           .getChildFile ("VST3_SDK");
}

z0 ProjectExporter::addAAXFoldersToPath()
{
...
z0 ProjectExporter::addARAFoldersToPath()
{
...
z0 ProjectExporter::addTargetSpecificPreprocessorDefs (StringPairArray& defs, const build_tools::ProjectType::Target::Type targetType) const
{
    using Target = build_tools::ProjectType::Target::Type;
    const std::pair<tukk, Target> targetFlags[] { { "DrxPlugin_Build_VST",        Target::VSTPlugIn },
                                                         { "DrxPlugin_Build_VST3",       Target::VST3PlugIn },
                                                         { "DrxPlugin_Build_AU",         Target::AudioUnitPlugIn },
                                                         { "DrxPlugin_Build_AUv3",       Target::AudioUnitv3PlugIn },
                                                         { "DrxPlugin_Build_AAX",        Target::AAXPlugIn },
                                                         { "DrxPlugin_Build_Standalone", Target::StandalonePlugIn },
                                                         { "DrxPlugin_Build_Unity",      Target::UnityPlugIn },
                                                         { "DrxPlugin_Build_LV2",        Target::LV2PlugIn } };
    ...
    {
        defs.set ("DrxPlugin_Enable_ARA", "1");
    }

    linuxSubprocessHelperProperties.setCompileDefinitionIfNecessary (defs);
}

z0 ProjectExporter::addDefaultPreprocessorDefs (StringPairArray& defs) const
{
    defs.set (getExporterIdentifierMacro(), "1");
    defs.set ("DRX_APP_VERSION", project.getVersionString());
    defs.set ("DRX_APP_VERSION_HEX", project.getVersionAsHex());
}

Txt ProjectExporter::replacePreprocessorTokens (const ProjectExporter::BuildConfiguration& config,
                                                   const Txt& sourceString) const
{
    return build_tools::replacePreprocessorDefs (getAllPreprocessorDefs (config, build_tools::ProjectType::Target::unspecified),
                                                 sourceString);
}

Txt ProjectExporter::getCompilerFlagsForFileCompilerFlagScheme (StringRef schemeName) const
{
...
Txt ProjectExporter::getCompilerFlagsForProjectItem (const Project::Item& item) const
{
    return getCompilerFlagsForFileCompilerFlagScheme (item.getCompilerFlagSchemeString());
}

z0 ProjectExporter::copyMainGroupFromProject()
{
...
Project::Item& ProjectExporter::getModulesGroup()
{
    if (modulesGroup == nullptr)
    {
        ...
        itemGroups.add (Project::Item::createGroup (project, "DRX Modules", "__modulesgroup__", true));
...
//==============================================================================
static b8 isWebBrowserComponentEnabled (const Project& project)
{
    static Txt guiExtrasModule ("drx_gui_extra");

    return (project.getEnabledModules().isModuleEnabled (guiExtrasModule)
            && project.isConfigFlagEnabled ("DRX_WEB_BROWSER", true));
}

static b8 isCurlEnabled (Project& project)
{
    static Txt juceCoreModule ("drx_core");

    return (project.getEnabledModules().isModuleEnabled (juceCoreModule)
            && project.isConfigFlagEnabled ("DRX_USE_CURL", true));
}

static b8 isLoadCurlSymbolsLazilyEnabled (Project& project)
{
    static Txt juceCoreModule ("drx_core");

    return (project.getEnabledModules().isModuleEnabled (juceCoreModule)
            && project.isConfigFlagEnabled ("DRX_LOAD_CURL_SYMBOLS_LAZILY", false));
}
...
z0 ProjectExporter::addProjectPathToBuildPathList (StringArray& pathList,
...
z0 ProjectExporter::addToModuleLibPaths (const build_tools::RelativePath& pathFromProjectFolder)
{
    addProjectPathToBuildPathList (moduleLibSearchPaths, pathFromProjectFolder);
}

z0 ProjectExporter::addToExtraSearchPaths (const build_tools::RelativePath& pathFromProjectFolder, i32 index)
{
...
static var getStoredPathForModule (const Txt& id, const ProjectExporter& exp)
{
    return getAppSettings().getStoredPath (isDRXModule (id) ? Ids::defaultDrxModulePath : Ids::defaultUserModulePath,
                                           exp.getTargetOSForExporter()).get();
}

ValueTreePropertyWithDefault ProjectExporter::getPathForModuleValue (const Txt& moduleID)
{
...
Txt ProjectExporter::getPathForModuleString (const Txt& moduleID) const
{
...
z0 ProjectExporter::removePathForModule (const Txt& moduleID)
{
...
build_tools::RelativePath ProjectExporter::getModuleFolderRelativeToProject (const Txt& moduleID) const
{
...
Txt ProjectExporter::getLegacyModulePath() const
{
    return getSettingString ("juceFolder");
}

build_tools::RelativePath ProjectExporter::getLegacyModulePath (const Txt& moduleID) const
{
    if (project.getEnabledModules().shouldCopyModuleFilesLocally (moduleID))
    ...
    auto oldDrxPath = getLegacyModulePath();

    if (oldDrxPath.isEmpty())
        return build_tools::RelativePath();

    build_tools::RelativePath p (oldDrxPath, build_tools::RelativePath::projectFolder);
...
z0 ProjectExporter::updateOldModulePaths()
{
    auto oldPath = getLegacyModulePath();

    if (oldPath.isNotEmpty())
    {
        for (i32 i = project.getEnabledModules().getNumModules(); --i >= 0;)
        {
...
static b8 areSameExporters (const ProjectExporter& p1, const ProjectExporter& p2)
{
    return p1.getExporterIdentifier() == p2.getExporterIdentifier();
}

static b8 areCompatibleExporters (const ProjectExporter& p1, const ProjectExporter& p2)
{
...
z0 ProjectExporter::createDefaultModulePaths()
{
    ...
    for (const auto& modID : project.getEnabledModules().getAllModules())
        getPathForModuleValue (modID) = (exporterToCopy != nullptr ? exporterToCopy->getPathForModuleString (modID) : "../../drx");
}
...
i32 ProjectExporter::getNumConfigurations() const
{
    return getConfigurations().getNumChildren();
}

ProjectExporter::BuildConfiguration::Ptr ProjectExporter::getConfiguration (i32 index) const
{
    return createBuildConfig (getConfigurations().getChild (index));
}

z0 ProjectExporter::addNewConfigurationFromExisting (const BuildConfiguration& configToCopy)
{
...
z0 ProjectExporter::addNewConfiguration (b8 isDebugConfig)
{
    auto configs = getConfigurations();
...
z0 ProjectExporter::BuildConfiguration::removeFromExporter()
{
    ValueTree configs (config.getParent());
    configs.removeChild (config, project.getUndoManagerFor (configs));
}

z0 ProjectExporter::createDefaultConfigs()
{
...
b8 ProjectExporter::ConfigIterator::next()
{
...
b8 ProjectExporter::ConstConfigIterator::next()
{
    if (++index >= exporter.getNumConfigurations())
        return false;
...
Txt ProjectExporter::BuildConfiguration::getGCCOptimisationFlag() const
{
...
z0 ProjectExporter::BuildConfiguration::addGCCOptimisationProperty (PropertyListBuilder& props)
{
...
z0 ProjectExporter::BuildConfiguration::addRecommendedLinuxCompilerWarningsProperty (PropertyListBuilder& props)
{
...
z0 ProjectExporter::BuildConfiguration::addRecommendedLLVMCompilerWarningsProperty (PropertyListBuilder& props)
{
    recommendedWarningsValue.setDefault ("");
...
z0 ProjectExporter::BuildConfiguration::createPropertyEditors (PropertyListBuilder& props)
{
...
Txt ProjectExporter::BuildConfiguration::getPrecompiledHeaderFileContent() const
{
...
Txt ProjectExporter::getExternalLibraryFlags (const BuildConfiguration& config) const
{
...
b8 LinuxSubprocessHelperProperties::shouldUseLinuxSubprocessHelper() const
{
...
z0 LinuxSubprocessHelperProperties::deployLinuxSubprocessHelperSourceFilesIfNecessary() const
{
    if (shouldUseLinuxSubprocessHelper())
    {
        ...
        const std::pair<File, tukk> sources[]
        {
            { owner.resolveRelativePath (getSimpleBinaryBuilderSource()),   BinaryData::drx_SimpleBinaryBuilder_cpp   },
            { owner.resolveRelativePath (getLinuxSubprocessHelperSource()), BinaryData::drx_LinuxSubprocessHelper_cpp }
        };
...
build_tools::RelativePath LinuxSubprocessHelperProperties::getLinuxSubprocessHelperSource() const
{
    return build_tools::RelativePath { "make_helpers", build_tools::RelativePath::buildTargetFolder }
        .getChildFile ("drx_LinuxSubprocessHelper.cpp");
}

z0 LinuxSubprocessHelperProperties::setCompileDefinitionIfNecessary (StringPairArray& defs) const
{
    if (shouldUseLinuxSubprocessHelper())
...
{
    return build_tools::RelativePath { "make_helpers", build_tools::RelativePath::buildTargetFolder }
                            .getChildFile ("drx_SimpleBinaryBuilder.cpp");
}

build_tools::RelativePath LinuxSubprocessHelperProperties::getLinuxSubprocessHelperBinaryDataSource() const
{
    return build_tools::RelativePath ("pre_build", drx::build_tools::RelativePath::buildTargetFolder)
                            .getChildFile ("drx_LinuxSubprocessHelperBinaryData.cpp");
}

z0 LinuxSubprocessHelperProperties::addToExtraSearchPathsIfNecessary() const
{
    if (shouldUseLinuxSubprocessHelper())
    {
...
std::optional<Txt> LinuxSubprocessHelperProperties::getParentDirectoryRelativeToBuildTargetFolder (build_tools::RelativePath rp)
{
    jassert (rp.getRoot() == drx::build_tools::RelativePath::buildTargetFolder);
...
Txt LinuxSubprocessHelperProperties::makeSnakeCase (const Txt& s)
{
    Txt result;
    result.preallocateBytes (128);

    b8 previousCharacterUnderscore = false;
...
Txt LinuxSubprocessHelperProperties::getBinaryNameFromSource (const build_tools::RelativePath& rp)
{
