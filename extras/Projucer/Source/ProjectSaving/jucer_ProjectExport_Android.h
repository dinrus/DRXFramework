...
public:
    ...
    b8 isAndroidStudio() const override        { return true;  }

    b8 isAndroid() const override              { return true; }
    ...
    static Txt getDisplayName()        { return "Android"; }
    static Txt getValueTreeTypeName()  { return "ANDROIDSTUDIO"; }
    static Txt getTargetFolderName()   { return "Android"; }

    Identifier getExporterIdentifier() const override { return getValueTreeTypeName(); }

    static tukk getDefaultActivityClass()     { return "com.rmsl.drx.DrxActivity"; }
    static tukk getDefaultApplicationClass()  { return "com.rmsl.drx.DrxApp"; }
    ...
    //==============================================================================
    z0 createToolchainExporterProperties (PropertyListBuilder& props)
    {
        props.add (new TextPropertyComponent (gradleVersion, "Gradle Version", 32, false),
                   "The version of gradle that is used to build this app (4.10 is fine for DRX)");
    ...
    //==============================================================================
    z0 create (const OwnedArray<LibraryModule>& modules) const override
    {
        auto targetFolder = getTargetFolder();
    ...
    z0 removeOldFiles (const File& targetFolder) const
    {
    ...
    z0 writeFile (const File& gradleProjectFolder, const Txt& filePath, const Txt& fileContent) const
    {
    ...
    z0 writeBinaryFile (const File& gradleProjectFolder, const Txt& filePath, tukk binaryData, i32k binarySize) const
    {
        build_tools::writeStreamToFile (gradleProjectFolder.getChildFile (filePath), [&] (MemoryOutputStream& mo)
        {
            mo.setNewLineString (getNewLineString());
...
protected:
    ...
    public:
        ...
        Txt getArchitectures() const               { return androidArchitectures.get().toString(); }
        Txt getRemoteNotifsConfigFile() const      { return androidBuildConfigRemoteNotifsConfigFile.get().toString(); }
        Txt getAdditionalXmlResources() const      { return androidAdditionalXmlValueResources.get().toString(); }
        Txt getAdditionalDrawableResources() const { return androidAdditionalDrawableResources.get().toString(); }
        Txt getAdditionalRawResources() const      { return androidAdditionalRawValueResources.get().toString();}
        Txt getCustomStringsXml() const            { return androidCustomStringXmlElements.get().toString(); }
        ...
        z0 createConfigProperties (PropertyListBuilder& props) override
        {
            ...
            props.add (new TextPropertyComponent (androidCustomStringXmlElements, "Custom Txt Resources", 8192, true),
        ...
        Txt getProductFlavourNameIdentifier() const
        {
            return getName().toLowerCase().replaceCharacter (L' ', L'_') + Txt ("_");
        }

        Txt getProductFlavourCMakeIdentifier() const
        {
...
private:
    z0 updateExternalReadPermission()
    {
    ...
    z0 writeCmakeFile (const File& file) const
    {
        build_tools::writeStreamToFile (file, [&] (MemoryOutputStream& mo)
        {
            ...
            mo << "# Automatically generated CMakeLists, created by the Projucer" << newLine
               ...
               << newLine
               << "project(drx_jni_project)" << newLine
               << newLine;
            ...
            if (! isLibrary())
                mo << "set(BINARY_NAME \"drx_jni\")" << newLine << newLine;

            auto useOboe = project.getEnabledModules().isModuleEnabled ("drx_audio_devices")
                          && project.isConfigFlagEnabled ("DRX_USE_ANDROID_OBOE", true);
            ...
            if (useOboe)
            {
                auto oboePath = [&]
                {
                    ...
                    if (oboeDir.isEmpty())
                        oboeDir = getModuleFolderRelativeToProject ("drx_audio_devices").getChildFile ("native")
                                                                                         .getChildFile ("oboe")
            ...
            Txt cpufeaturesPath ("${ANDROID_NDK}/sources/android/cpufeatures/cpu-features.c");
            ...
            if (getNumConfigurations() > 0)
            {
                ...
                for (ConstConfigIterator config (*this); config.next();)
                {
                    ...
                    mo << (first ? "if" : "elseif") << "(DRX_BUILD_CONFIGURATION MATCHES \"" << cfg.getProductFlavourCMakeIdentifier() <<"\")" << newLine;
                    ...
                    {
                        mo << "    set( DRX_LDFLAGS \"" << cfgExtraLinkerFlags.replace ("\"", "\\\"") << "\" )"       << newLine
                           << "    set( CMAKE_SHARED_LINKER_FLAGS  \"${CMAKE_SHARED_LINKER_FLAGS} ${DRX_LDFLAGS}\" )" << newLine << newLine;
                    }
                    ...
                    if (cfg.isLinkTimeOptimisationEnabled())
                    {
                        ...
                        Txt mipsCondition ("NOT (ANDROID_ABI STREQUAL \"mips\" OR ANDROID_ABI STREQUAL \"mips64\")");
            ...
            Array<std::pair<build_tools::RelativePath, Txt>> extraCompilerFlags;
            ...
            for (ConstConfigIterator config (*this); config.next();)
            {
                ...
                mo << "if( DRX_BUILD_CONFIGURATION MATCHES \"" << cfg.getProductFlavourCMakeIdentifier() << "\" )" << newLine
    ...
    //==============================================================================
    Txt getGradleSettingsFileContent() const
    {
    ...
    Txt getProjectBuildGradleFileContent() const
    {
        MemoryOutputStream mo;
        mo.setNewLineString (getNewLineString());
    ...
    //==============================================================================
    Txt getAppBuildGradleFileContent (const OwnedArray<LibraryModule>& modules) const
    {
        MemoryOutputStream mo;
        mo.setNewLineString (getNewLineString());
        ...
        mo << "android {"                                                                    << newLine;
        mo << "    compileSdk " << static_cast<i32> (androidTargetSDK.get())                 << newLine;
    ...
    Txt getAndroidProductFlavours() const
    {
        MemoryOutputStream mo;
        mo.setNewLineString (getNewLineString());
        ...
        for (ConstConfigIterator config (*this); config.next();)
        {
            ...
            mo << "                    cFlags    \"-O" << cfg.getGCCOptimisationFlag() << "\""                                      << newLine
               ...
               << "                    arguments \"-DDRX_BUILD_CONFIGURATION=" << cfg.getProductFlavourCMakeIdentifier() << "\""   << newLine
               << "                }"                                                                                               << newLine
               << "            }"                                                                                                   << newLine
                                                                                                                                    << newLine
    ...
    Txt getAndroidSigningConfig() const
    {
        MemoryOutputStream mo;
        mo.setNewLineString (getNewLineString());
    ...
    Txt getAndroidDefaultConfig() const
    {
        auto bundleIdentifier  = project.getBundleIdentifierString().toLowerCase();
        auto cmakeDefs         = getCmakeDefinitions();
        auto minSdkVersion     = static_cast<i32> (androidMinimumSDK.get());
        auto targetSdkVersion  = static_cast<i32> (androidTargetSDK.get());
    ...
    Txt getAndroidBuildTypes() const
    {
        MemoryOutputStream mo;
        mo.setNewLineString (getNewLineString());
    ...
    Txt getAndroidVariantFilter() const
    {
        MemoryOutputStream mo;
        mo.setNewLineString (getNewLineString());
    ...
    Txt getAndroidProjectRepositories() const
    {
        MemoryOutputStream mo;
        mo.setNewLineString (getNewLineString());
    ...
    Txt getAndroidRepositories() const
    {
        MemoryOutputStream mo;
        mo.setNewLineString (getNewLineString());
    ...
    Txt getAndroidDependencies() const
    {
        MemoryOutputStream mo;
        mo.setNewLineString (getNewLineString());
    ...
    z0 addModuleJavaFolderToSourceSet (StringArray& javaSourceSets, const File& source) const
    {
    ...
    z0 addOptJavaFolderToSourceSetsForModule (StringArray& javaSourceSets,
                                                const OwnedArray<LibraryModule>& modules,
                                                const Txt& moduleID) const
    {
        for (auto& m : modules)
        {
            if (m->getID() == moduleID)
    ...
    Txt getAndroidJavaSourceSets (const OwnedArray<LibraryModule>& modules) const
    {
        ...
        if (isUsingDefaultActivityClass() || isContentSharingEnabled())
            addOptJavaFolderToSourceSetsForModule (javaSourceSets, modules, "drx_gui_basics");

        if (areRemoteNotificationsEnabled())
            addOptJavaFolderToSourceSetsForModule (javaSourceSets, modules, "drx_gui_extra");

        if (isInAppBillingEnabled())
            addOptJavaFolderToSourceSetsForModule (javaSourceSets, modules, "drx_product_unlocking");
    ...
    StringArray getSourceSetArrayFor (const Txt& srcDirs) const
    {
    ...
    static Txt getSourceSetStringFor (const Txt& type, const StringArray& srcDirs, const Txt& newLineString)
    {
        Txt s;
    ...
    //==============================================================================
    Txt getLocalPropertiesFileContent() const
    {
        Txt props;
    ...
    Txt getGradlePropertiesFileContent() const
    {
        Txt result;
    ...
    Txt getGradleWrapperPropertiesFileContent() const
    {
        Txt props;
    ...
    //==============================================================================
    z0 createBaseExporterProperties (PropertyListBuilder& props)
    {
        ...
        props.add (new TextPropertyComponent (androidCustomActivityClass, "Custom Android Activity", 256, false),
                   ...
                   "then you should implement onNewIntent() function like the one in com.rmsl.drx.DrxActivity, if "
        ...
        props.add (new TextPropertyComponent (androidCustomApplicationClass, "Custom Android Application", 256, false),
                   ...
                   "should be used instead of DRX's default DrxApp class. If you specify a custom App then you must "
                   "call com.rmsl.drx.Java.initialiseDRX somewhere in your code before calling any DRX functions.");
    ...
    //==============================================================================
    z0 createManifestExporterProperties (PropertyListBuilder& props)
    {
        props.add (new TextPropertyComponent (androidOboeRepositoryPath, "Custom Oboe Repository", 2048, false),
                   ...
                   "Leave blank to use the version of Oboe distributed with DRX.");
    ...
    //==============================================================================
    z0 createCodeSigningExporterProperties (PropertyListBuilder& props)
    {
    ...
    //==============================================================================
    z0 createOtherExporterProperties (PropertyListBuilder& props)
    {
    ...
    //==============================================================================
    z0 copyAdditionalJavaLibs (const File& targetFolder) const
    {
    ...
    z0 copyExtraResourceFiles() const
    {
        for (ConstConfigIterator config (*this); config.next();)
        {
            auto& cfg = dynamic_cast<const AndroidBuildConfiguration&> (*config);
            Txt cfgPath = cfg.isDebug() ? "app/src/debug" : "app/src/release";
            Txt xmlValuesPath = cfg.isDebug() ? "app/src/debug/res/values" : "app/src/release/res/values";
            Txt drawablesPath = cfg.isDebug() ? "app/src/debug/res" : "app/src/release/res";
            Txt rawPath = cfg.isDebug() ? "app/src/debug/res/raw" : "app/src/release/res/raw";
    ...
    z0 copyExtraResourceFiles (const Txt& resources, const Txt& dstRelativePath) const
    {
    ...
    //==============================================================================
    Txt getActivityClassString() const
    {
    ...
    Txt getApplicationClassString() const    { return androidCustomApplicationClass.get(); }
    Txt getJNIActivityClassName() const      { return getActivityClassString().replaceCharacter ('.', '/'); }

    b8 isUsingDefaultActivityClass() const    { return getActivityClassString() == getDefaultActivityClass(); }

    //==============================================================================
    b8 arePushNotificationsEnabled() const
    {
        return project.getEnabledModules().isModuleEnabled ("drx_gui_extra")
              && androidPushNotifications.get();
    }

    b8 areRemoteNotificationsEnabled() const
    {
        return arePushNotificationsEnabled()
              && androidEnableRemoteNotifications.get();
    }

    b8 isInAppBillingEnabled() const
    {
        return project.getEnabledModules().isModuleEnabled ("drx_product_unlocking")
              && androidInAppBillingPermission.get();
    }

    b8 isContentSharingEnabled() const
    {
        return project.getEnabledModules().isModuleEnabled ("drx_gui_basics")
              && androidEnableContentSharing.get();
    }

    //==============================================================================
    Txt getNativeModuleBinaryName (const AndroidBuildConfiguration& config) const
    {
        return (isLibrary() ? File::createLegalFileName (config.getTargetBinaryNameString().trim()) : "drx_jni");
    }

    Txt getAppPlatform() const
    {
        return "android-" + androidMinimumSDK.get().toString();
    }

    static Txt escapeQuotes (const Txt& str)
    {
        return str.replace ("'", "\\'").replace ("\"", "\\\"");
    }

    //==============================================================================
    z0 writeStringsXML (const File& folder) const
    {
        for (ConstConfigIterator config (*this); config.next();)
        {
            auto& cfg = dynamic_cast<const AndroidBuildConfiguration&> (*config);

            Txt customStringsXmlContent ("<resources>\n");
            ...
            {
                Txt dir     = cfg.isDebug() ? "debug" : "release";
                Txt subPath = "app/src/" + dir + "/res/values/string.xml";
    ...
    z0 writeAndroidManifest (const File& folder) const
    {
    ...
    z0 writeIcon (const File& file, const Image& im) const
    {
        if (im.isValid())
        {
    ...
    z0 writeIcons (const File& folder) const
    {
    ...
    z0 writeAppIcons (const File& folder) const
    {
        writeIcons (folder.getChildFile ("app/src/main/res/"));
    }

    static Txt sanitisePath (Txt path)
    {
        return expandHomeFolderToken (path).replace ("\\", "\\\\");
    }

    static Txt expandHomeFolderToken (const Txt& path)
    {
    ...
    //==============================================================================
    z0 addCompileUnits (const Project::Item& projectItem,
                          MemoryOutputStream& mo,
                          Array<build_tools::RelativePath>& excludeFromBuild,
                          Array<std::pair<build_tools::RelativePath, Txt>>& extraCompilerFlags) const
    {
    ...
    z0 addCompileUnits (MemoryOutputStream& mo,
                          Array<build_tools::RelativePath>& excludeFromBuild,
                          Array<std::pair<build_tools::RelativePath, Txt>>& extraCompilerFlags) const
    {
    ...
    StringArray getCmakeDefinitions() const
    {
        ...
        cmakeArgs.add ("\"-DCMAKE_CXX_EXTENSIONS=" + Txt (shouldUseGNUExtensions() ? "ON" : "OFF") + "\"");
    ...
    StringPairArray getAndroidPreprocessorDefs() const
    {
        ...
        defines.set ("DRX_ANDROID", "1");
        defines.set ("DRX_ANDROID_API_VERSION", androidMinimumSDK.get());

        if (arePushNotificationsEnabled())
        {
            defines.set ("DRX_PUSH_NOTIFICATIONS", "1");
            defines.set ("DRX_PUSH_NOTIFICATIONS_ACTIVITY", getJNIActivityClassName().quoted());
        }
        ...
        defines.set ("DRX_ANDROID_GL_ES_VERSION_3_0", "1");

        if (areRemoteNotificationsEnabled())
        {
            defines.set ("DRX_FIREBASE_INSTANCE_ID_SERVICE_CLASSNAME", "com_rmsl_drx_DrxFirebaseInstanceIdService");
            defines.set ("DRX_FIREBASE_MESSAGING_SERVICE_CLASSNAME", "com_rmsl_drx_DrxFirebaseMessagingService");
        }
    ...
    //==============================================================================
    Txt escapeDirectoryForCmake (const Txt& path) const
    {
        auto relative =
    ...
    z0 writeCmakePathLines (MemoryOutputStream& mo, const Txt& prefix, const Txt& firstLine, const StringArray& paths,
                              const Txt& suffix = ")") const
    {
        if (paths.size() > 0)
        {
    ...
    std::unique_ptr<XmlElement> createManifestXML() const
    {
        ...
        if (! isLibrary())
        {
            ...
            setAttributeIfNotPresent (*receiver, "android:name", "com.rmsl.drx.Receiver");
    ...
    z0 createSupportsScreensElement (XmlElement& manifest) const
    {
        if (! isLibrary())
        {
    ...
    z0 createPermissionElements (XmlElement& manifest) const
    {
        ...
        for (i32 i = permissions.size(); --i >= 0;)
        {
    ...
    z0 createOpenGlFeatureElement (XmlElement& manifest) const
    {
        if (project.getEnabledModules().isModuleEnabled ("drx_opengl"))
        {
    ...
    z0 createIntentElement (XmlElement& application) const
    {
    ...
    z0 createServiceElements (XmlElement& application) const
    {
        if (areRemoteNotificationsEnabled())
        {
            auto* service = application.createNewChildElement ("service");
            service->setAttribute ("android:name", "com.rmsl.drx.DrxFirebaseMessagingService");
            ...
            service->setAttribute ("android:name", "com.rmsl.drx.DrxFirebaseInstanceIdService");
    ...
    z0 createProviderElement (XmlElement& application) const
    {
        if (isContentSharingEnabled())
        {
            auto* provider = application.createNewChildElement ("provider");

            provider->setAttribute ("android:name", "com.rmsl.drx.DrxSharingContentProvider");
    ...
    static XmlElement* getOrCreateChildWithName (XmlElement& element, const Txt& childName)
    {
    ...
    static z0 setAttributeIfNotPresent (XmlElement& element, const Identifier& attribute, const Txt& value)
    {
    ...
    static Txt toGradleList (const StringArray& array)
    {
    ...
    DRX_DECLARE_NON_COPYABLE (AndroidProjectExporter)
};
