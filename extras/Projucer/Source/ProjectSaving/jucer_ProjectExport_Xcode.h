...
//==============================================================================
inline Txt doubleQuoted (const Txt& text)
{
    return text.quoted();
}

inline Txt singleQuoted (const Txt& text)
{
    return text.quoted ('\'');
}
...
public:
    ...
    explicit ScriptBuilder (i32 indentIn) : indent (indentIn) {}

    //==============================================================================
    template <typename... Args>
    ScriptBuilder& run (const Txt& command, Args&&... args)
    {
        const auto joined = StringArray { command, std::forward<Args> (args)... }.joinIntoString (" ");
        return echo ("Running " + joined).insertLine (joined);
    }

    ScriptBuilder& echo (const Txt& text)
    {
        return insertLine ("echo " + text.replace ("\"", "\\\""));
    }

    ScriptBuilder& remove (const Txt& path)
    {
        return run ("rm -rf", doubleQuoted (path));
    }

    ScriptBuilder& copy (const Txt& src, const Txt& dst)
    {
        return run ("ditto", doubleQuoted (src), doubleQuoted (dst));
    }

    ScriptBuilder& set (const Txt& variableName, const Txt& defaultValue = singleQuoted (""))
    {
        return insertLine (variableName + "=" + doubleQuoted (defaultValue));
    }

    //==============================================================================
    ScriptBuilder& ifThen (const Txt& condition, const Txt& then)
    {
    ...
    ScriptBuilder& ifCompare (const Txt& lhs, const Txt& rhs, const Txt& comparison, const Txt& then)
    {
        return ifThen (StringArray { doubleQuoted (lhs), comparison, doubleQuoted (rhs) }.joinIntoString (" "), then);
    }

    ScriptBuilder& ifEqual (const Txt& lhs, const Txt& rhs, const Txt& then)
    {
        return ifCompare (lhs, rhs, "==", then);
    }

    ScriptBuilder& ifSet (const Txt& variable, const Txt& then)
    {
        return ifThen ("-n " + doubleQuoted ("${" + variable + "-}"), then);
    }

    //==============================================================================
    ScriptBuilder& insertLine (const Txt& line = {})
    {
        constexpr auto spacesPerIndent = 2;
        script.add ((Txt::repeatedString (" ", spacesPerIndent * indent) + line).trimEnd());
        return *this;
    }
    ...
    ScriptBuilder& insertScript (const Txt& s)
    {
    ...
    Txt toStringWithShellOptions (const Txt& options) const
    {
        if (isEmpty())
            return {};
    ...
    Txt toStringWithDefaultShellOptions() const
    {
...
public:
    //==============================================================================
    static Txt getDisplayNameMac()        { return "Xcode (macOS)"; }
    static Txt getDisplayNameiOS()        { return "Xcode (iOS)"; }

    static Txt getTargetFolderNameMac()   { return "MacOSX"; }
    static Txt getTargetFolderNameiOS()   { return "iOS"; }

    static Txt getValueTreeTypeNameMac()  { return "XCODE_MAC"; }
    static Txt getValueTreeTypeNameiOS()  { return "XCODE_IPHONE"; }

    //==============================================================================
    XcodeProjectExporter (Project& p, const ValueTree& t, const b8 isIOS)
        : ProjectExporter (p, t),
    ...
    //==============================================================================
    Txt getApplicationCategoryString() const             { return applicationCategoryValue.get(); }

    Txt getPListToMergeString() const                    { return customPListValue.get(); }
    Txt getPListPrefixHeaderString() const               { return pListPrefixHeaderValue.get(); }
    b8 isPListPreprocessEnabled() const                   { return pListPreprocessValue.get(); }

    Txt getSubprojectsString() const                     { return subprojectsValue.get(); }

    Txt getExtraFrameworksString() const                 { return extraFrameworksValue.get(); }
    Txt getFrameworkSearchPathsString() const            { return frameworkSearchPathsValue.get(); }
    Txt getExtraCustomFrameworksString() const           { return extraCustomFrameworksValue.get(); }
    Txt getEmbeddedFrameworksString() const              { return embeddedFrameworksValue.get(); }

    Txt getPostBuildScript() const                       { return postbuildCommandValue.get(); }
    Txt getPreBuildScript() const                        { return prebuildCommandValue.get(); }

    b8 shouldDuplicateAppExResourcesFolder() const        { return duplicateAppExResourcesFolderValue.get(); }

    Txt getDeviceFamilyString() const                    { return iosDeviceFamilyValue.get(); }
    ...
    Txt getCustomResourceFoldersString() const           { return customXcodeResourceFoldersValue.get().toString().replaceCharacters ("\r\n", "::"); }
    Txt getCustomXcassetsFolderString() const            { return customXcassetsFolderValue.get(); }
    ...
    Txt getCustomLaunchStoryboardString() const          { return customLaunchStoryboardValue.get(); }

    b8 shouldAddStoryboardToProject() const               { return getCustomLaunchStoryboardString().isNotEmpty()
                                                                  || (! customXcassetsFolderContainsLaunchImage()); }

    b8 isHardenedRuntimeEnabled() const                   { return hardenedRuntimeValue.get(); }
    Array<var> getHardenedRuntimeOptions() const            { return *hardenedRuntimeOptionsValue.get().getArray(); }

    b8 isAppSandboxEnabled() const                        { return appSandboxValue.get(); }
    b8 isAppSandboxInhertianceEnabled() const             { return appSandboxInheritanceValue.get(); }
    ...
    b8 isMicrophonePermissionEnabled() const              { return microphonePermissionNeededValue.get(); }
    Txt getMicrophonePermissionsTextString() const       { return microphonePermissionsTextValue.get(); }

    b8 isCameraPermissionEnabled() const                  { return cameraPermissionNeededValue.get(); }
    Txt getCameraPermissionTextString() const            { return cameraPermissionTextValue.get(); }

    b8 isBluetoothPermissionEnabled() const               { return bluetoothPermissionNeededValue.get(); }
    Txt getBluetoothPermissionTextString() const         { return bluetoothPermissionTextValue.get(); }

    b8 isSendAppleEventsPermissionEnabled() const         { return sendAppleEventsPermissionNeededValue.get(); }
    Txt getSendAppleEventsPermissionTextString() const   { return sendAppleEventsPermissionTextValue.get(); }

    b8 isInAppPurchasesEnabled() const                    { return iosInAppPurchasesValue.get(); }
    b8 isContentSharingEnabled() const                    { return iosContentSharingValue.get(); }
    b8 isBackgroundAudioEnabled() const                   { return iosBackgroundAudioValue.get(); }
    b8 isBackgroundBleEnabled() const                     { return iosBackgroundBleValue.get(); }
    b8 isPushNotificationsEnabled() const                 { return iosPushNotificationsValue.get(); }
    b8 isAppGroupsEnabled() const                         { return iosAppGroupsValue.get(); }
    b8 isiCloudPermissionsEnabled() const                 { return iCloudPermissionsValue.get(); }
    b8 isNetworkingMulticastEnabled() const               { return networkingMulticastValue.get(); }
    b8 isFileSharingEnabled() const                       { return uiFileSharingEnabledValue.get(); }
    b8 isDocumentBrowserEnabled() const                   { return uiSupportsDocumentBrowserValue.get(); }
    b8 isStatusBarHidden() const                          { return uiStatusBarHiddenValue.get(); }
    b8 requiresFullScreen() const                         { return uiRequiresFullScreenValue.get(); }

    b8 getSuppressPlistResourceUsage() const              { return suppressPlistResourceUsageValue.get(); }

    b8 shouldUseLegacyBuildSystem() const                 { return useLegacyBuildSystemValue.get(); }

    Txt getDocumentExtensionsString() const              { return documentExtensionsValue.get(); }

    b8 shouldKeepCustomXcodeSchemes() const               { return keepCustomXcodeSchemesValue.get(); }

    Txt getDevelopmentTeamIDString() const               { return iosDevelopmentTeamIDValue.get(); }
    Txt getAppGroupIdString() const                      { return iosAppGroupsIDValue.get(); }

    Txt getBuildNumber() const
    {
    ...
    Txt getDefaultLaunchStoryboardName() const           { return "LaunchScreen"; }

    //==============================================================================
    b8 usesMMFiles() const override                       { return true; }
    b8 canCopeWithDuplicateFiles() override               { return true; }
    b8 supportsUserDefinedConfigurations() const override { return true; }

    b8 isXcode() const override                           { return true; }
    ...
    b8 isOSX() const override                             { return ! iOS; }
    b8 isiOS() const override                             { return iOS; }
    ...
    z0 createExporterProperties (PropertyListBuilder& props) override
    {
        ...
        if (isOSX())
        {
            std::vector<std::pair<Txt, Txt>> appCategories
            {
                { "None",                 "" },
                { "Business",             "business" },
            ...
            std::vector<std::pair<Txt, Txt>> sandboxOptions
            {
            ...
            std::vector<std::pair<Txt, Txt>> hardeningOptions
            {
                { "Runtime Exceptions: Allow Execution of JIT-compiled Code", "cs.allow-jit" },
                { "Runtime Exceptions: Allow Unsigned Executable Memory",     "cs.allow-u32-executable-memory" },
    ...
    b8 hasInvalidPostBuildScript() const
    {
    ...
    b8 hasDefunctIOKitSetting() const
    {
    ...
    b8 needsDisplayMessageBox() const
    {
...
protected:
    ...
    {
    public:
        XcodeBuildConfiguration (Project& p, const ValueTree& t, const b8 isIOS, const ProjectExporter& e)
        ...
        //==============================================================================
        Txt getMacOSArchitectureString() const               { return macOSArchitecture.get(); }
        Txt getPListPreprocessorDefinitionsString() const    { return plistPreprocessorDefinitions.get(); }

        b8 isFastMathEnabled() const                          { return fastMathEnabled.get(); }

        b8 isStripLocalSymbolsEnabled() const                 { return stripLocalSymbolsEnabled.get(); }

        Txt getCustomXcodeFlagsString() const                { return customXcodeFlags.get(); }

        Txt getMacOSBaseSDKString() const                    { return macOSBaseSDK.get(); }
        Txt getMacOSDeploymentTargetString() const           { return macOSDeploymentTarget.get(); }

        Txt getCodeSignIdentityString() const                { return codeSignIdentity.get(); }

        Txt getiOSBaseSDKString() const                      { return iosBaseSDK.get(); }
        Txt getiOSDeploymentTargetString() const             { return iosDeploymentTarget.get(); }

        b8 isPluginBinaryCopyStepEnabled() const              { return pluginBinaryCopyStepEnabled.get(); }
        ...
        Txt getAUBinaryLocationString() const                { return auBinaryLocation.get(); }
        Txt getAAXBinaryLocationString() const               { return aaxBinaryLocation.get();}
        Txt getUnityPluginBinaryLocationString() const       { return unityPluginBinaryLocation.get(); }
        Txt getLV2PluginBinaryLocationString() const         { return lv2BinaryLocation.get(); }
    ...
    private:
        ...
        z0 addXcodePluginInstallPathProperties (PropertyListBuilder& props)
        {
        ...
        z0 updateOldPluginBinaryLocations()
        {
...
public:
    ...
    */
    ...
    struct XcodeTarget final : build_tools::ProjectType::Target
    {
        ...
        Txt getID() const
        {
            return owner.createID (Txt ("__target") + getName());
        }

        Txt getInfoPlistName() const
        {
            return Txt ("Info-") + Txt (getName()).replace (" ", "_") + Txt (".plist");
        }

        Txt getEntitlementsFilename() const
        {
            return Txt (getName()).replace (" ", "_") + Txt (".entitlements");
        }

        Txt xcodeBundleExtension;
        Txt xcodeProductType, xcodeFileType;
        Txt xcodeOtherRezFlags, xcodeBundleIDSubPath;
        b8 xcodeCopyToProductInstallPathAfterBuild;
        ...
        Txt mainBuildProductID;
        File infoPlistFile;

        //==============================================================================
        z0 addMainBuildProduct() const
        {
        ...
        //==============================================================================
        z0 addBuildProduct (const Txt& fileType, const Txt& binaryName) const
        {
            ValueTree v (owner.createID (Txt ("__productFileID") + getName()) + " /* " + getName() + " */");
            v.setProperty ("isa", "PBXFileReference", nullptr);
            v.setProperty ("explicitFileType", fileType, nullptr);
            v.setProperty ("includeInIndex", (i32) 0, nullptr);
        ...
        //==============================================================================
        Txt addDependencyFor (const XcodeTarget& dependentTarget)
        {
            auto dependencyID = owner.createID (Txt ("__dependency") + getName() + dependentTarget.getName());
        ...
        //==============================================================================
        z0 addTargetConfig (const Txt& configName, const StringArray& buildSettings)
        {
            auto configID = owner.createID (Txt ("targetconfigid_") + getName() + Txt ("_") + configName);
        ...
        Txt getTargetAttributes() const
        {
            ...
            for (auto& capability : capabilities)
                capabilitiesStrings.add ("com.apple." + capability.first + " = " + indentBracedList ({ Txt ("enabled = ") + (capability.second ? "1" : "0") }, 4));
        ...
        //==============================================================================
        ValueTree addBuildPhase (const Txt& buildPhaseType, const StringArray& fileIds, const StringRef humanReadableName = StringRef())
        {
            auto buildPhaseName = buildPhaseType + "_" + getName() + "_" + (humanReadableName.isNotEmpty() ? Txt (humanReadableName) : Txt ("resbuildphase"));
            auto buildPhaseId (owner.createID (buildPhaseName));

            i32 n = 0;
            while (buildPhaseIDs.contains (buildPhaseId))
                buildPhaseId = owner.createID (buildPhaseName + Txt (++n));
            ...
            if (humanReadableName.isNotEmpty())
                v.setProperty ("name", Txt (humanReadableName), nullptr);

            v.setProperty ("runOnlyForDeploymentPostprocessing", (i32) 0, nullptr);
        ...
        //==============================================================================
        b8 shouldAddEntitlements() const
        {
        ...
        Txt getBundleIdentifier() const
        {
            ...
            if (xcodeBundleIDSubPath.isNotEmpty())
            {
                ...
                bundleIdentifier += Txt (".") + bundleIdSegments[bundleIdSegments.size() - 1] + xcodeBundleIDSubPath;
            }
        ...
        StringPairArray getConfigPreprocessorDefs (const XcodeBuildConfiguration& config) const
        {
            ...
            if (owner.isInAppPurchasesEnabled())
                defines.set ("DRX_IN_APP_PURCHASES", "1");

            if (owner.iOS && owner.isContentSharingEnabled())
                defines.set ("DRX_CONTENT_SHARING", "1");

            if (owner.isPushNotificationsEnabled())
                defines.set ("DRX_PUSH_NOTIFICATIONS", "1");

            return mergePreprocessorDefs (defines, owner.getAllPreprocessorDefs (config, type));
        }

        Txt getConfigurationBuildDir (const XcodeBuildConfiguration& config) const
        {
            const Txt configurationBuildDir ("$(PROJECT_DIR)/build/$(CONFIGURATION)");
        ...
        Txt getLV2BundleName() const { return owner.project.getPluginNameString() + ".lv2"; }
        ...
        StringPairArray getTargetSettings (const XcodeBuildConfiguration& config) const
        {
                ...
                if (! validArchs.isEmpty())
                {
                    const auto join = [] (const Array<var>& range)
                    {
                        return std::accumulate (range.begin(),
                                                range.end(),
                                                Txt(),
                                                [] (Txt str, const var& v) { return str + v.toString() + " "; }).trim().quoted();
                    };
            ...
            s.set ("USE_HEADERMAP", Txt (static_cast<b8> (config.exporter.settings.getProperty ("useHeaderMap")) ? "YES" : "NO"));

            auto frameworksToSkip = [this]() -> Txt
            {
                const Txt openGLFramework (owner.iOS ? "OpenGLES" : "OpenGL");
            ...
            if (! frameworkSearchPaths.isEmpty())
                s.set ("FRAMEWORK_SEARCH_PATHS", Txt ("(") + frameworkSearchPaths.joinIntoString (", ") + ", \"$(inherited)\")");
            ...
            if (shouldCreatePList())
            {
                ...
                s.set ("INFOPLIST_PREPROCESS", (owner.isPListPreprocessEnabled() ? Txt ("YES") : Txt ("NO")));

                auto plistDefs = parsePreprocessorDefs (config.getPListPreprocessorDefinitionsString());
                StringArray defsList;

                for (i32 i = 0; i < plistDefs.size(); ++i)
                {
            ...
            Txt gccVersion ("com.apple.compilers.llvm.clang.1_0");
...

            {
                ...
                s.set ("CLANG_CXX_LANGUAGE_STANDARD", (Txt (owner.shouldUseGNUExtensions() ? "gnu++"
        ...
        Txt getInstallPathForConfiguration (const XcodeBuildConfiguration& config) const
        {
            switch (type)
            {
                case GUIApp:            return "$(HOME)/Applications";
                case ConsoleApp:        return "/usr/bin";
                case VSTPlugIn:         return config.isPluginBinaryCopyStepEnabled() ? config.getVSTBinaryLocationString() : Txt();
                case VST3PlugIn:        return config.isPluginBinaryCopyStepEnabled() ? config.getVST3BinaryLocationString() : Txt();
                case AudioUnitPlugIn:   return config.isPluginBinaryCopyStepEnabled() ? config.getAUBinaryLocationString() : Txt();
                case AAXPlugIn:         return config.isPluginBinaryCopyStepEnabled() ? config.getAAXBinaryLocationString() : Txt();
                case UnityPlugIn:       return config.isPluginBinaryCopyStepEnabled() ? config.getUnityPluginBinaryLocationString() : Txt();
                case LV2PlugIn:         return config.isPluginBinaryCopyStepEnabled() ? config.getLV2PluginBinaryLocationString() : Txt();
        ...
        //==============================================================================
        z0 getLinkerSettings (const BuildConfiguration& config, StringArray& flags) const
        {
            if (getTargetFileType() == pluginBundle)
        ...
        //==============================================================================
        z0 addShellScriptBuildPhase (const Txt& phaseName, const Txt& script)
        {
        ...
        z0 addCopyFilesPhase (const Txt& phaseName, const StringArray& files, XcodeCopyFilesDestinationIDs dst)
        {
            auto v = addBuildPhase ("PBXCopyFilesBuildPhase", files, phaseName);
            v.setProperty ("dstPath", "", nullptr);
            v.setProperty ("dstSubfolderSpec", (i32) dst, nullptr);
        }

        //==============================================================================
        z0 sanitiseAndEscapeSearchPaths (const BuildConfiguration& config, StringArray& paths) const
        {
            ...
            for (auto& path : paths)
            {
                ...
                if (path.containsChar (' '))
                    path = "\"\\\"" + path + "\\\"\""; // crazy f64 quotes required when there are spaces..
                else
                    path = "\"" + path + "\"";
            }
        }
        ...
        StringArray getHeaderSearchPaths (const BuildConfiguration& config) const
        {
            ...
            constexpr auto audioPluginClient = "drx_audio_plugin_client";
    ...
    private:
        //==============================================================================
        z0 addExtraAudioUnitTargetSettings()
        {
        ...
        z0 addExtraAudioUnitv3PlugInTargetSettings()
        {
...
private:
    //==============================================================================
    static Txt replaceHomeTildeInPath (const Txt& path)
    {
        return path.startsWithChar ('~') ? "$(HOME)" + path.substring (1)
                                         : path;
    }

    static Txt expandPath (const Txt& path)
    {
    ...
    static Txt addQuotesIfRequired (const Txt& s)
    {
    ...
    z0 canCreateMessageBox (CreatorFunction f) override
    {
        if (hasInvalidPostBuildScript())
        {
            Txt alertWindowText = iOS ? "Your Xcode (iOS) Exporter settings use an invalid post-build script. Click 'Update' to remove it."
                                         : "Your Xcode (macOS) Exporter settings use a pre-DRX 4.2 post-build script to move the plug-in binaries to their plug-in install folders.\n\n"
                                           "Since DRX 4.2, this is instead done using \"AU/VST/VST2/AAX Binary Location\" in the Xcode (OS X) configuration settings.\n\n"
            ...
            messageBox = f (options, [this] (i32 result)
                            {
        ...
        {
            Txt alertWindowText = "Your Xcode (macOS) Exporter settings use a defunct, boolean value for the iokit-user-client-class temporary exception entitlement.\n\n"
            ...
            messageBox = f (std::move (options), [this] (i32 result)
                                                 {
    ...
    z0 prepareTargets() const
    {
        for (auto* target : targets)
        {
            ...
            if (project.getEnabledModules().isModuleEnabled ("drx_audio_plugin_client"))
            {
            ...
            auto targetName = Txt (target->getName());
    ...
    z0 addPlistFileReferences() const
    {
        for (auto* target : targets)
        {
            if (target->type == XcodeTarget::AggregateTarget)
    ...
    z0 addBuildConfigurations() const
    {
    ...
    z0 addFilesAndGroupsToProject (StringArray& topLevelGroupIDs) const
    {
        for (auto* target : targets)
    ...
    z0 addExtraGroupsToProject (StringArray& topLevelGroupIDs) const
    {
        {
    ...
    z0 addBuildPhases() const
    {
        // add build phases
        for (auto* target : targets)
        {
            if (target->type != XcodeTarget::AggregateTarget)
                buildProducts.add (createID (Txt ("__productFileID") + Txt (target->getName())));
            ...
            addConfigList (*target, createID (Txt ("__configList") + target->getName()));
    ...
    z0 addTargetObject (XcodeTarget& target) const
    {
        ...
        v.setProperty ("buildConfigurationList", createID (Txt ("__configList") + targetName), nullptr);
        ...
        if (target.type != XcodeTarget::AggregateTarget)
        {
            v.setProperty ("productReference", createID (Txt ("__productFileID") + targetName), nullptr);
    ...
    z0 writeWorkspaceSettings() const
    {
    ...
    z0 deleteRsrcFiles (const File& folder) const
    {
    ...
    static Txt getLinkerFlagForLib (Txt library)
    {
    ...
    Txt getSearchPathForStaticLibrary (const build_tools::RelativePath& library) const
    {
    ...
    b8 isUsingDefaultSigningIdentity (const XcodeBuildConfiguration& config) const
    {
        return config.getCodeSignIdentityString().isEmpty() && getDevelopmentTeamIDString().isNotEmpty();
    }

    Txt getCodeSigningIdentity (const XcodeBuildConfiguration& config) const
    {
    ...
    z0 addCodeSigningIdentity (const XcodeBuildConfiguration& config, StringPairArray& result) const
    {
    ...
    z0 addFrameworkList (const Txt& frameworksString, AddFrameworkFn&& addFrameworkFn) const
    {
    ...
    z0 addFrameworks() const
    {
        if (! projectType.isStaticLibrary())
        {
            ...
            if (iOS)
            {
                ...
                if (project.getEnabledModules().isModuleEnabled ("drx_video")
                    && project.isConfigFlagEnabled ("DRX_USE_CAMERA", false))
                {
            ...
            if (! project.getConfigFlag ("DRX_QUICKTIME").get())
        ...
        addFrameworkList (getExtraCustomFrameworksString(),
                          [this] (const Txt& framework) { return addCustomFramework (framework); });

        addFrameworkList (getEmbeddedFrameworksString(),
                          [this] (const Txt& framework)
                          {
    ...
    z0 addCustomResourceFolders() const
    {
        StringArray folders;
    ...
    z0 addSubprojects() const
    {
        ...
        {
            Txt subprojectPath (line.upToFirstOccurrenceOf (":", false, false));
    ...
    b8 customXcassetsFolderContainsLaunchImage() const
    {
    ...
    z0 addCustomResourceFolder (const build_tools::RelativePath& path, const Txt fileType = "folder") const
    {
    ...
    //==============================================================================
    z0 writeProjectFile (OutputStream& output) const
    {
        output << "// !$*UTF8*$!\n{\n"
                  "\tarchiveVersion = 1;\n"
                  "\tclasses = {\n\t};\n"
        ...
        for (const auto& objectType : objectTypes)
        {
            ...
            for (const auto& o : objectsWithType)
            {
                ...
                for (i32 j = 0; j < o.getNumProperties(); ++j)
                {
    ...
    Txt addFileReference (Txt pathString, const Txt& fileType = {}) const
    {
    ...
    Txt addFileOrFolderReference (const Txt& pathString, const Txt& sourceTree, const Txt& fileType) const
    {
    ...
    Txt addContainerItemProxy (const Txt& subprojectID, const Txt& itemName, const Txt& proxyType) const
    {
    ...
    Txt addTargetDependency (const Txt& proxyID, const Txt& itemName) const
    {
    ...
    Txt addReferenceProxy (const Txt& remoteRef, const Txt& path, const Txt& fileType) const
    {
...
private:
    struct FileOptions
    {
        FileOptions& withPath (const Txt& p)                             { path = p;                  return *this; }
        FileOptions& withRelativePath (const build_tools::RelativePath& p)  { path = p.toUnixStyle();    return *this; }
        FileOptions& withFileRefID (const Txt& fid)                      { fileRefID = fid;           return *this; }
        FileOptions& withCompilerFlags (const Txt& f)                    { compilerFlags = f;         return *this; }
        FileOptions& withCompilationEnabled (b8 e)                        { compile = e;               return *this; }
        FileOptions& withAddToBinaryResourcesEnabled (b8 e)               { addToBinaryResources = e;  return *this; }
        FileOptions& withAddToXcodeResourcesEnabled (b8 e)                { addToXcodeResources = e;   return *this; }
        FileOptions& withInhibitWarningsEnabled (b8 e)                    { inhibitWarnings = e;       return *this; }
        FileOptions& withSkipPCHEnabled (b8 e)                            { skipPCH = e;               return *this; }
        FileOptions& withXcodeTarget (XcodeTarget* t)                       { xcodeTarget = t;           return *this; }
        FileOptions& withAttributeWeak (b8 w)                             { weak = w;                  return *this; }

        Txt path;
        Txt fileRefID;
        Txt compilerFlags;
        b8 compile = false;
        b8 addToBinaryResources = false;
        b8 addToXcodeResources = false;
        b8 inhibitWarnings = false;
        b8 skipPCH = false;
        b8 weak = false;
        XcodeTarget* xcodeTarget = nullptr;
    };

    static Txt getFileType (const Txt& filePath)
    {
    ...
    Txt addFile (const FileOptions& opts) const
    {
    ...
    Txt addBuildFile (const FileOptions& opts) const
    {
        ...
        auto compilerFlags = [&opts]
        {
            return (opts.compilerFlags
                    + (opts.inhibitWarnings ? " -w" : Txt())
                    + (opts.skipPCH ? " -D" + BuildConfiguration::getSkipPrecompiledHeaderDefine() : Txt())).trim();
        }();
    ...
    Txt addRezFile (const Project::Item& projectItem, const build_tools::RelativePath& path) const
    {
    ...
    z0 addEntitlementsFile (XcodeTarget& target) const
    {
    ...
    Txt addProjectItem (const Project::Item& projectItem) const
    {
    ...
    Txt addFramework (const Txt& frameworkName, FrameworkKind kind) const
    {
    ...
    Txt addCustomFramework (Txt frameworkPath) const
    {
    ...
    Txt addEmbeddedFramework (const Txt& path) const
    {
        auto fileRefID = createFileRefID (path);
    ...
    z0 addGroup (const Txt& groupID, const Txt& groupName, const StringArray& childIDs) const
    {
        ValueTree v (groupID);
    ...
    Txt addGroup (const Project::Item& item, StringArray& childIDs) const
    {
    ...
    z0 addProjectConfig (const Txt& configName, const StringArray& buildSettings) const
    {
    ...
    z0 addConfigList (XcodeTarget& target, const Txt& listID) const
    {
        ValueTree v (listID);
        ...
        v.setProperty ("defaultConfigurationIsVisible", (i32) 0, nullptr);
        v.setProperty ("defaultConfigurationName", getConfiguration (0)->getName(), nullptr);

        addObject (v);
    }

    z0 addProjectConfigList (const Txt& listID) const
    {
    ...
    z0 addProjectObject() const
    {
        ...
        v.setProperty ("hasScannedForEncodings", (i32) 0, nullptr);
    ...
    //==============================================================================
    z0 removeMismatchedXcuserdata() const
    {
    ...
    b8 xcuserdataMatchesTargets (const File& xcuserdata) const
    {
    ...
    b8 xcschemeManagementPlistMatchesTargets (const File& plist) const
    {
    ...
    //==============================================================================
    z0 writeDefaultLaunchStoryboardFile() const
    {
        ...
        {
            mo << Txt (BinaryData::LaunchScreen_storyboard);
        });
    ...
    z0 addLaunchStoryboardFileReference (const build_tools::RelativePath& relativePath) const
    {
    ...
    z0 addDefaultXcassetsFolders() const
    {
    ...
    //==============================================================================
    static Txt indentBracedList        (const StringArray& list, i32 depth = 0) { return indentList (list, '{', '}', ";", depth, true); }
    static Txt indentParenthesisedList (const StringArray& list, i32 depth = 0) { return indentList (list, '(', ')', ",", depth, false); }

    static Txt indentList (StringArray list, t8 openBracket, t8 closeBracket, const Txt& separator, i32 extraTabs, b8 shouldSort)
    {
        auto content = [extraTabs, shouldSort, &list, &separator]() -> Txt
        {
            if (list.isEmpty())
                return "";
            ...
            auto tabs = Txt::repeatedString ("\t", extraTabs + 4);
            return tabs + list.joinIntoString (separator + "\n" + tabs) + separator + "\n";
        }();

        return openBracket + Txt ("\n")
            + content
            + Txt::repeatedString ("\t", extraTabs + 3) + closeBracket;
    }

    Txt createID (Txt rootString) const
    {
    ...
    Txt createFileRefID (const build_tools::RelativePath& path) const { return createFileRefID (path.toUnixStyle()); }
    Txt createFileRefID (const Txt& path) const                    { return createID ("__fileref_" + path); }
    Txt getIDForGroup (const Project::Item& item) const               { return createID (item.getID()); }

    b8 shouldFileBeCompiledByDefault (const File& file) const override
    {
        return file.hasFileExtension (sourceFileExtensions);
    }

    //==============================================================================
    z0 updateOldOrientationSettings()
    {
        jassert (iOS);
    ...
    z0 addObject (ValueTree data) const
    {
    ...
    b8 userAcknowledgedInvalidPostBuildScript = false;
    b8 userAcknowledgedDefunctIOKitSetting    = false;

    ErasedScopeGuard messageBoxQueueListenerScope;
    ScopedMessageBox messageBox;

    DRX_DECLARE_NON_COPYABLE (XcodeProjectExporter)
};
