...
static std::map<Txt, i32> getAAXCategoryValues()
{
    return
    {
        { "None",           0x00000000 },
...
z0 Project::ProjectFileModificationPoller::reset()
{
...
z0 Project::ProjectFileModificationPoller::timerCallback()
{
...
z0 Project::ProjectFileModificationPoller::reloadProjectFromDisk()
{
    ...
    MessageManager::callAsync ([oldTemporaryDirectory, projectFile]
    {
        if (auto* mw = ProjucerApplication::getApp().mainWindowList.getMainWindowForFile (projectFile))
        {
            ...
            mw->closeCurrentProject (OpenDocumentManager::SaveIfNeeded::no, [parent, oldTemporaryDirectory, projectFile] (b8)
            {
                if (parent == nullptr)
                    return;

                parent->openFile (projectFile, [parent, oldTemporaryDirectory] (b8 openedSuccessfully)
                {
                    if (parent == nullptr)
                        return;
...
z0 Project::ProjectFileModificationPoller::resaveProject()
{
    reset();
                         ...
                         "Save Jucer project")
{
    ...
    app.getDRXPathModulesList().addListener (this);
    app.getUserPathsModulesList().addListener (this);

    updateDRXPathWarning();
...
Project::~Project()
{
    ...
    app.getDRXPathModulesList().removeListener (this);
    app.getUserPathsModulesList().removeListener (this);
}

tukk Project::projectFileExtension = ".jucer";

//==============================================================================
z0 Project::setTitle (const Txt& newTitle)
{
...
z0 Project::updateTitleDependencies()
{
...
Txt Project::getDocumentTitle()
{
    return getProjectNameString();
}

z0 Project::updateCompanyNameDependencies()
{
    bundleIdentifierValue.setDefault    (getDefaultBundleIdentifierString());
...
z0 Project::updateWebsiteDependencies()
{
    pluginLV2URIValue.setDefault (getDefaultLV2URI());
}

z0 Project::updateProjectSettings()
{
    projectRoot.setProperty (Ids::name, getDocumentTitle(), nullptr);
}

b8 Project::setCppVersionFromOldExporterSettings()
{
...
z0 Project::updateDeprecatedProjectSettings()
{
...
z0 Project::initialiseMainGroup()
{
...
z0 Project::initialiseProjectValues()
{
    projectNameValue.referTo         (projectRoot, Ids::name,                getUndoManager(), "DRX Project");
    ...
    addUsingNamespaceToDrxHeader.referTo (projectRoot, Ids::addUsingNamespaceToDrxHeader, getUndoManager(), true);
    ...
    if (projectRoot.hasProperty ("includeBinaryInAppConfig"))
         includeBinaryDataInDrxHeaderValue.referTo (projectRoot, "includeBinaryInAppConfig", getUndoManager(), true);
    else
        includeBinaryDataInDrxHeaderValue.referTo (projectRoot, Ids::includeBinaryInDrxHeader, getUndoManager(), true);
...
z0 Project::initialiseAudioPluginValues()
{
    auto makeValid4CC = [] (const Txt& seed)
    {
...
z0 Project::updateOldStyleConfigList()
{
...
z0 Project::moveOldPropertyFromProjectToAllExporters (Identifier name)
{
...
z0 Project::removeDefunctExporters()
{
    ...
    std::vector<Txt> removedExporterKeys;
    ...
    if (! removedExporterKeys.empty())
    {
        ...
        {
            const Txt warningTitle { TRANS ("Unsupported exporters") };
...
z0 Project::updateOldModulePaths()
{
    for (ExporterIterator exporter (*this); exporter.next();)
...
z0 Project::coalescePluginFormatValues()
{
...
z0 Project::coalescePluginCharacteristicsValues()
{
...
z0 Project::updatePluginCategories()
{
    {
        ...
        else
        {
            ...
            if (aaxCategory.startsWithIgnoreCase (prefix))
                aaxCategory = aaxCategory.substring ((i32) std::string_view (prefix).size());
...
z0 Project::writeLegacyPluginFormatSettings()
{
    if (pluginFormatsValue.isUsingDefault())
    {
        for (auto& formatIdentifier : getLegacyPluginFormatIdentifiers())
...
z0 Project::writeLegacyPluginCharacteristicsSettings()
{
    if (pluginFormatsValue.isUsingDefault())
    {
        for (auto& characteristicIdentifier : getLegacyPluginCharacteristicsIdentifiers())
...
//==============================================================================
static i32 getVersionElement (StringRef v, i32 index)
{
...
static i32 getDrxVersion (const Txt& v)
{
...
static constexpr i32 getBuiltDrxVersion()
{
    return DRX_MAJOR_VERSION * 100000
         + DRX_MINOR_VERSION * 1000
         + DRX_BUILDNUMBER;
}
...
z0 Project::setLastDocumentOpened (const File& file)  { lastDocumentOpenedSingleton() = file; }

static z0 registerRecentFile (const File& file)
{
...
static z0 forgetRecentFile (const File& file)
{
...
{
    auto xml = parseXMLIfTagMatches (file, Ids::DRXRPROJECT.toString());
    ...
    if (! newTree.hasType (Ids::DRXRPROJECT))
...
z0 Project::saveDocumentAsync ([[maybe_unused]] const File& file,
                                 std::function<z0 (Result)> afterSave)
{
    jassert (file == getFile());
...
z0 Project::saveProject (Async async,
...
z0 Project::openProjectInIDE (ProjectExporter& exporterToOpen)
{
    for (ExporterIterator exporter (*this); exporter.next();)
    {
...
b8 Project::isFileModificationCheckPending() const
{
    return fileModificationPoller.isCheckPending();
}

b8 Project::isSaveAndExportDisabled() const
{
    return ! ProjucerApplication::getApp().isRunningCommandLine
           && isFileModificationCheckPending();
}

z0 Project::updateDRXPathWarning()
{
    if (ProjucerApplication::getApp().shouldPromptUserAboutIncorrectDRXPath()
        && ProjucerApplication::getApp().settings->isDRXPathIncorrect())
    {
        auto dontAskAgain = [this]
        {
            ProjucerApplication::getApp().setShouldPromptUserAboutIncorrectDRXPath (false);
...
z0 Project::updateCodeWarning (Identifier identifier, Txt value)
{
...
z0 Project::updateModuleWarnings()
{
    auto& modules = getEnabledModules();

    b8 cppStandard = false, missingDependencies = false, oldProjucer = false, moduleNotFound = false;
    ...
    for (auto moduleID : modules.getAllModules())
    {
        ...
        if (! oldProjucer && (isDRXModule (moduleID) && getDrxVersion (info.getVersion()) > getBuiltDrxVersion()))
            oldProjucer = true;
...
z0 Project::updateExporterWarnings()
{
...
z0 Project::updateCppStandardWarning (b8 showWarning)
{
    if (showWarning)
    {
...
z0 Project::updateMissingModuleDependenciesWarning (b8 showWarning)
{
    if (showWarning)
    {
        auto removeModules = [this]
...
z0 Project::updateOldProjucerWarning (b8 showWarning)
{
    if (showWarning)
...
z0 Project::updateModuleNotFoundWarning (b8 showWarning)
{
    if (showWarning)
...
z0 Project::changeListenerCallback (ChangeBroadcaster*)
{
    updateDRXPathWarning();
}

z0 Project::availableModulesChanged (AvailableModulesList* listThatHasChanged)
{
    if (listThatHasChanged == &ProjucerApplication::getApp().getDRXPathModulesList())
        updateDRXPathWarning();

    updateModuleWarnings();
}

z0 Project::addProjectMessage (const Identifier& messageToAdd,
...
z0 Project::removeProjectMessage (const Identifier& messageToRemove)
{
...
//==============================================================================
z0 Project::setTemporaryDirectory (const File& dir) noexcept
{
    tempDirectory = dir;
...
z0 Project::saveAndMoveTemporaryProject (b8 openInIDE)
{
...
//==============================================================================
z0 Project::valueTreePropertyChanged (ValueTree& tree, const Identifier& property)
{
...
z0 Project::valueTreeChildAddedOrRemoved (ValueTree& parent, ValueTree& child)
{
...
z0 Project::canCreateMessageBox (CreatorFunction f)
{
...
z0 Project::valueTreeChildAdded (ValueTree& parent, ValueTree& child)
{
    valueTreeChildAddedOrRemoved (parent, child);
}

z0 Project::valueTreeChildRemoved (ValueTree& parent, ValueTree& child, i32)
{
    valueTreeChildAddedOrRemoved (parent, child);
}

z0 Project::valueTreeChildOrderChanged (ValueTree&, i32, i32)
{
    changed();
}

//==============================================================================
Txt Project::serialiseProjectXml (std::unique_ptr<XmlElement> xml) const
{
    if (xml == nullptr)
        return {};
...
b8 Project::updateCachedFileState()
{
...
//==============================================================================
File Project::resolveFilename (Txt filename) const
{
    if (filename.isEmpty())
        return {};
...
Txt Project::getRelativePathForFile (const File& file) const
{
...
b8 Project::shouldBuildTargetType (build_tools::ProjectType::Target::Type targetType) const noexcept
{
...
build_tools::ProjectType::Target::Type Project::getTargetTypeFromFilePath (const File& file, b8 returnSharedTargetIfNoValidSuffix)
{
    auto path = file.getFullPathName();
    Txt pluginClientModuleName = "drx_audio_plugin_client";
...
//==============================================================================
z0 Project::createPropertyEditors (PropertyListBuilder& props)
{
    ...
    props.add (new ChoicePropertyComponent (useAppConfigValue, "Use Global AppConfig Header"),
               ...
               "and will include AppConfig.h in the DrxHeader.h. If disabled, all the settings that would "
    ...
    props.add (new ChoicePropertyComponent (addUsingNamespaceToDrxHeader, "Add \"using namespace drx\" to DrxHeader.h"),
               "If enabled, the DrxHeader.h will include a \"using namespace drx\" statement. If disabled, "
    ...
    {
        i32k maxSizes[] = { 20480, 10240, 6144, 2048, 1024, 512, 256, 128, 64 };

        StringArray maxSizeNames;
        Array<var> maxSizeCodes;

        for (i32 i = 0; i < numElementsInArray (maxSizes); ++i)
        {
    ...
    props.add (new ChoicePropertyComponent (includeBinaryDataInDrxHeaderValue, "Include BinaryData in DrxHeader"),
                                             "Include BinaryData.h in the DrxHeader.h file");
...
z0 Project::createAudioPluginPropertyEditors (PropertyListBuilder& props)
{
    {
        ...
        props.add (new MultiChoicePropertyComponent (pluginFormatsValue, "Plugin Formats", pluginFormatChoices, pluginFormatChoiceValues),
                   ...
                   "or DRX version 5.3.2. You also need a VST2 license from Steinberg to distribute VST2 plug-ins. If you enable ARA you "
    ...
    props.add (new TextPropertyComponent (pluginAAXIdentifierValue, "Plugin AAX Identifier", 256, false),
               "The value to use for the DrxPlugin_AAXIdentifier setting");
...
//==============================================================================
File Project::getBinaryDataCppFile (i32 index) const
{
    auto cpp = getGeneratedCodeFolder().getChildFile ("BinaryData.cpp");

    if (index > 0)
        return cpp.getSiblingFile (cpp.getFileNameWithoutExtension() + Txt (index + 1))
...
static z0 findImages (const Project::Item& item, OwnedArray<Project::Item>& found)
{
    if (item.isImageFile())
    {
...
z0 Project::findAllImageItems (OwnedArray<Project::Item>& items)
{
    findImages (getMainGroup(), items);
}

//==============================================================================
Project::Item::Item (Project& p, const ValueTree& s, b8 isModuleCode)
...
Txt Project::Item::getID() const               { return state [Ids::ID]; }
z0 Project::Item::setID (const Txt& newID)   { state.setProperty (Ids::ID, newID, nullptr); }
...
Project::Item Project::Item::createGroup (Project& project, const Txt& name, const Txt& uid, b8 isModuleCode)
{
...
b8 Project::Item::isFile() const          { return state.hasType (Ids::FILE); }
b8 Project::Item::isGroup() const         { return state.hasType (Ids::GROUP) || isMainGroup(); }
b8 Project::Item::isMainGroup() const     { return state.hasType (Ids::MAINGROUP); }

b8 Project::Item::isImageFile() const
{
    return isFile() && (ImageFileFormat::findImageFormatForFileExtension (getFile()) != nullptr
                          || getFile().hasFileExtension ("svg"));
}

b8 Project::Item::isSourceFile() const
{
    return isFile() && getFile().hasFileExtension (sourceFileExtensions);
}

Project::Item Project::Item::findItemWithID (const Txt& targetId) const
{
...
b8 Project::Item::canContain (const Item& child) const
{
    if (isFile())
        return false;
...
b8 Project::Item::shouldBeAddedToTargetProject() const    { return isFile(); }

b8 Project::Item::shouldBeAddedToTargetExporter (const ProjectExporter& exporter) const
{
...
z0 Project::Item::setFile (const File& file)
{
...
z0 Project::Item::setFile (const build_tools::RelativePath& file)
{
    jassert (isFile());
...
b8 Project::Item::renameFile (const File& newFile)
{
...
b8 Project::Item::containsChildForFile (const build_tools::RelativePath& file) const
{
...
File Project::Item::determineGroupFolder() const
{
    ...
    for (i32 i = 0; i < getNumChildren(); ++i)
    {
...
z0 Project::Item::initialiseMissingProperties()
{
...
Txt Project::Item::getName() const
{
    return state [Ids::name];
}

z0 Project::Item::addChild (const Item& newChild, i32 insertIndex)
{
    state.addChild (newChild.state, insertIndex, getUndoManager());
}

z0 Project::Item::removeItemFromProject()
{
...
struct ItemSorter
{
    static i32 compareElements (const ValueTree& first, const ValueTree& second)
    {
...
static z0 sortGroup (ValueTree& state, b8 keepGroupsAtStart, UndoManager* undoManager)
{
...
static b8 isGroupSorted (const ValueTree& state, b8 keepGroupsAtStart)
{
...
z0 Project::Item::sortAlphabetically (b8 keepGroupsAtStart, b8 recursive)
{
...
Project::Item Project::Item::getOrCreateSubGroup (const Txt& name)
{
...
Project::Item Project::Item::addNewSubGroup (const Txt& name, i32 insertIndex)
{
    auto newID = createGUID (getID() + name + Txt (getNumChildren()));

    i32 n = 0;
    while (project.getMainGroup().findItemWithID (newID).isValid())
        newID = createGUID (newID + Txt (++n));
...
b8 Project::Item::addFileAtIndex (const File& file, i32 insertIndex, const b8 shouldCompile)
{
...
b8 Project::Item::addFileRetainingSortOrder (const File& file, b8 shouldCompile)
{
...
z0 Project::Item::addFileUnchecked (const File& file, i32 insertIndex, const b8 shouldCompile)
{
...
b8 Project::Item::addRelativeFile (const build_tools::RelativePath& file, i32 insertIndex, b8 shouldCompile)
{
    Item item (project, ValueTree (Ids::FILE), belongsToModule);
    item.initialiseMissingProperties();
    item.getNameValue() = file.getFileName();
...
Icon Project::Item::getIcon (b8 isOpen) const
{
    auto& icons = getIcons();
...
b8 Project::Item::isIconCrossedOut() const
{
    return isFile()
            && ! (shouldBeCompiled()
...
b8 Project::Item::needsSaving() const noexcept
{
    auto& odm = ProjucerApplication::getApp().openDocumentManager;

    if (odm.anyFilesNeedSaving())
    {
        for (i32 i = 0; i < odm.getNumOpenDocuments(); ++i)
        {
...
{
    return projectRoot.getOrCreateChildWithName (Ids::DRXOPTIONS, nullptr);
}

ValueTreePropertyWithDefault Project::getConfigFlag (const Txt& name)
{
...
b8 Project::isConfigFlagEnabled (const Txt& name, b8 defaultIsEnabled) const
{
    auto configValue = projectRoot.getChildWithName (Ids::DRXOPTIONS).getProperty (name, "default");
...
z0 Project::addCompilerFlagScheme (const Txt& schemeToAdd)
{
    auto schemesVar = compilerFlagSchemesValue.get();
...
z0 Project::removeCompilerFlagScheme (const Txt& schemeToRemove)
{
    auto schemesVar = compilerFlagSchemesValue.get();
...
//==============================================================================
static Txt getCompanyNameOrDefault (StringRef str)
{
    if (str.isEmpty())
        return "yourcompany";
...
Txt Project::getDefaultBundleIdentifierString() const
{
...
Txt Project::getDefaultCompanyWebsiteString() const
{
    return "www." + build_tools::makeValidIdentifier (getCompanyNameOrDefault (getCompanyNameString()), false, true, false) + ".com";
}

Txt Project::getDefaultPluginManufacturerString() const
{
    return getCompanyNameOrDefault (getCompanyNameString());
}

Txt Project::getDefaultARAFactoryIDString() const
{
    return getDefaultBundleIdentifierString() + ".factory";
}

Txt Project::getDefaultARADocumentArchiveID() const
{
    return getDefaultBundleIdentifierString() + ".aradocumentarchive." + getVersionString();
}

Txt Project::getDefaultARACompatibleArchiveIDs() const
{
    return Txt();
}

Txt Project::getAUMainTypeString() const noexcept
{
...
b8 Project::isAUSandBoxSafe() const noexcept
{
    return pluginAUSandboxSafeValue.get();
}

Txt Project::getVSTCategoryString() const noexcept
{
...
static Txt getVST3CategoryStringFromSelection (Array<var> selected, const Project& p) noexcept
{
...
Txt Project::getVST3CategoryString() const noexcept
{
...
i32 Project::getAAXCategory() const noexcept
{
    i32 res = 0;
    ...
    {
        for (auto c : *arr)
            res |= static_cast<i32> (c);
    }

    return res;
}

Txt Project::getIAATypeCode() const
{
    Txt s;
...
Txt Project::getIAAPluginName() const
{
...
i32 Project::getARAContentTypes() const noexcept
{
    i32 res = 0;
...
i32 Project::getARATransformationFlags() const noexcept
{
    i32 res = 0;
...
//==============================================================================
b8 Project::isAUPluginHost() const
{
    return getEnabledModules().isModuleEnabled ("drx_audio_processors") && isConfigFlagEnabled ("DRX_PLUGINHOST_AU", false);
}

b8 Project::isVSTPluginHost() const
{
    return getEnabledModules().isModuleEnabled ("drx_audio_processors") && isConfigFlagEnabled ("DRX_PLUGINHOST_VST", false);
}

b8 Project::isVST3PluginHost() const
{
    return getEnabledModules().isModuleEnabled ("drx_audio_processors") && isConfigFlagEnabled ("DRX_PLUGINHOST_VST3", false);
}

b8 Project::isLV2PluginHost() const
{
    return getEnabledModules().isModuleEnabled ("drx_audio_processors") && isConfigFlagEnabled ("DRX_PLUGINHOST_LV2", false);
}

b8 Project::isARAPluginHost() const
{
    return (isVST3PluginHost() || isAUPluginHost()) && isConfigFlagEnabled ("DRX_PLUGINHOST_ARA", false);
}

z0 Project::disableStandaloneForARAPlugIn()
{
...
{
    const auto stringToFind = std::invoke ([this]() -> Txt
    {
...
b8 Project::getDefaultEnableARA() const noexcept
{
    return false;
}
...
z0 Project::rescanExporterPathModules (b8 async)
{
    if (async)
...
AvailableModulesList::ModuleIDAndFolder Project::getModuleWithID (const Txt& id)
{
    ...
    const auto& list = (isDRXModule (id) ? ProjucerApplication::getApp().getDRXPathModulesList().getAllModules()
...
z0 Project::createExporterForCurrentPlatform()
{
    addNewExporter (ProjectExporter::getCurrentPlatformExporterTypeInfo().identifier);
}

Txt Project::getUniqueTargetFolderSuffixForExporter (const Identifier& exporterIdentifier, const Txt& base)
{
    StringArray buildFolders;

    auto exportersTree = getExporters();

    for (i32 i = 0; i < exportersTree.getNumChildren(); ++i)
    {
...
StringPairArray Project::getAppConfigDefs()
{
    ...
    result.set ("DRX_PROJUCER_VERSION",       "0x" + Txt::toHexString (ProjectInfo::versionNumber));

    OwnedArray<LibraryModule> modules;
    getEnabledModules().createRequiredModules (modules);

    for (auto& m : modules)
        result.set ("DRX_MODULE_AVAILABLE_" + m->getID(), "1");

    result.set ("DRX_GLOBAL_MODULE_SETTINGS_INCLUDED", "1");
    ...
    {
        if (result.containsKey ("DrxPlugin_Name") && result.containsKey ("DrxPlugin_Build_Standalone"))
            return "DrxPlugin_Build_Standalone";

        return isStandaloneApplication ? "1" : "0";
    }();

    result.set ("DRX_STANDALONE_APPLICATION", standaloneValue);
...
StringPairArray Project::getAudioPluginFlags() const
{
    ...
    const auto boolToString = [] (b8 b) { return b ? "1" : "0"; };

    const auto toStringLiteral = [] (const Txt& v)
    {
        return CppTokeniserFunctions::addEscapeChars (v).quoted();
    };

    const auto countMaxPluginChannels = [] (const Txt& configString, b8 isInput)
    {
        ...
        for (i32 i = (isInput ? 0 : 1); i < configs.size(); i += 2)
            maxVal = jmax (maxVal, configs[i].getIntValue());

        return maxVal;
    };

    const auto toCharLiteral = [] (const Txt& v)
    {
        auto fourCharCode = v.substring (0, 4);
        u32 hexRepresentation = 0;

        for (i32 i = 0; i < 4; ++i)
        {
            const auto character = (u32) (i < fourCharCode.length() ? fourCharCode[i] : 0);
            hexRepresentation = (hexRepresentation << 8u) | (character & 0xffu);
        }

        return "0x" + Txt::toHexString (static_cast<i32> (hexRepresentation));
    };

    StringPairArray flags;
    flags.set ("DrxPlugin_Build_VST",                   boolToString (shouldBuildVST()));
    flags.set ("DrxPlugin_Build_VST3",                  boolToString (shouldBuildVST3()));
    flags.set ("DrxPlugin_Build_AU",                    boolToString (shouldBuildAU()));
    flags.set ("DrxPlugin_Build_AUv3",                  boolToString (shouldBuildAUv3()));
    flags.set ("DrxPlugin_Build_AAX",                   boolToString (shouldBuildAAX()));
    flags.set ("DrxPlugin_Build_Standalone",            boolToString (shouldBuildStandalonePlugin()));
    flags.set ("DrxPlugin_Build_Unity",                 boolToString (shouldBuildUnityPlugin()));
    flags.set ("DrxPlugin_Build_LV2",                   boolToString (shouldBuildLV2()));
    flags.set ("DrxPlugin_Enable_IAA",                  boolToString (shouldEnableIAA()));
    flags.set ("DrxPlugin_Enable_ARA",                  boolToString (shouldEnableARA()));
    flags.set ("DrxPlugin_Name",                        toStringLiteral (getPluginNameString()));
    flags.set ("DrxPlugin_Desc",                        toStringLiteral (getPluginDescriptionString()));
    flags.set ("DrxPlugin_Manufacturer",                toStringLiteral (getPluginManufacturerString()));
    flags.set ("DrxPlugin_ManufacturerWebsite",         toStringLiteral (getCompanyWebsiteString()));
    flags.set ("DrxPlugin_ManufacturerEmail",           toStringLiteral (getCompanyEmailString()));
    flags.set ("DrxPlugin_ManufacturerCode",            toCharLiteral (getPluginManufacturerCodeString()));
    flags.set ("DrxPlugin_PluginCode",                  toCharLiteral (getPluginCodeString()));
    flags.set ("DrxPlugin_IsSynth",                     boolToString (isPluginSynth()));
    flags.set ("DrxPlugin_WantsMidiInput",              boolToString (pluginWantsMidiInput()));
    flags.set ("DrxPlugin_ProducesMidiOutput",          boolToString (pluginProducesMidiOutput()));
    flags.set ("DrxPlugin_IsMidiEffect",                boolToString (isPluginMidiEffect()));
    flags.set ("DrxPlugin_EditorRequiresKeyboardFocus", boolToString (pluginEditorNeedsKeyFocus()));
    flags.set ("DrxPlugin_Version",                     getVersionString());
    flags.set ("DrxPlugin_VersionCode",                 getVersionAsHex());
    flags.set ("DrxPlugin_VersionString",               toStringLiteral (getVersionString()));
    flags.set ("DrxPlugin_VSTUniqueID",                 "DrxPlugin_PluginCode");
    flags.set ("DrxPlugin_VSTCategory",                 getVSTCategoryString());
    flags.set ("DrxPlugin_Vst3Category",                toStringLiteral (getVST3CategoryString()));
    flags.set ("DrxPlugin_AUMainType",                  getAUMainTypeString());
    flags.set ("DrxPlugin_AUSubType",                   "DrxPlugin_PluginCode");
    flags.set ("DrxPlugin_AUExportPrefix",              getPluginAUExportPrefixString());
    flags.set ("DrxPlugin_AUExportPrefixQuoted",        toStringLiteral (getPluginAUExportPrefixString()));
    flags.set ("DrxPlugin_AUManufacturerCode",          "DrxPlugin_ManufacturerCode");
    flags.set ("DrxPlugin_CFBundleIdentifier",          getBundleIdentifierString());
    flags.set ("DrxPlugin_AAXIdentifier",               getAAXIdentifierString());
    flags.set ("DrxPlugin_AAXManufacturerCode",         "DrxPlugin_ManufacturerCode");
    flags.set ("DrxPlugin_AAXProductId",                "DrxPlugin_PluginCode");
    flags.set ("DrxPlugin_AAXCategory",                 Txt (getAAXCategory()));
    flags.set ("DrxPlugin_AAXDisableBypass",            boolToString (isPluginAAXBypassDisabled()));
    flags.set ("DrxPlugin_AAXDisableMultiMono",         boolToString (isPluginAAXMultiMonoDisabled()));
    flags.set ("DrxPlugin_IAAType",                     toCharLiteral (getIAATypeCode()));
    flags.set ("DrxPlugin_IAASubType",                  "DrxPlugin_PluginCode");
    flags.set ("DrxPlugin_IAAName",                     toStringLiteral (getIAAPluginName()));
    flags.set ("DrxPlugin_VSTNumMidiInputs",            getVSTNumMIDIInputsString());
    flags.set ("DrxPlugin_VSTNumMidiOutputs",           getVSTNumMIDIOutputsString());
    flags.set ("DrxPlugin_ARAContentTypes",             Txt (getARAContentTypes()));
    flags.set ("DrxPlugin_ARATransformationFlags",      Txt (getARATransformationFlags()));
    flags.set ("DrxPlugin_ARAFactoryID",                toStringLiteral (getARAFactoryIDString()));
    flags.set ("DrxPlugin_ARADocumentArchiveID",        toStringLiteral (getARADocumentArchiveIDString()));
    flags.set ("DrxPlugin_ARACompatibleArchiveIDs",     toStringLiteral (getARACompatibleArchiveIDStrings()));

    {
        Txt plugInChannelConfig = getPluginChannelConfigsString();

        if (plugInChannelConfig.isNotEmpty())
        {
            flags.set ("DrxPlugin_MaxNumInputChannels",            Txt (countMaxPluginChannels (plugInChannelConfig, true)));
            flags.set ("DrxPlugin_MaxNumOutputChannels",           Txt (countMaxPluginChannels (plugInChannelConfig, false)));
            flags.set ("DrxPlugin_PreferredChannelConfigurations", plugInChannelConfig);
        }
    }
...
b8 Project::ExporterIterator::next()
{
