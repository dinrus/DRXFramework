...
{
public:
    ModuleItem (Project& p, const Txt& modID)
    ...
    Txt getUniqueName() const override     { return "module_" + moduleID; }
    Txt getDisplayName() const override    { return moduleID; }
    Txt getRenamingName() const override   { return getDisplayName(); }
    z0 setName (const Txt&) override     {}
    b8 isMissing() const override           { return missingDependencies; }
    b8 hasWarnings() const override         { return cppStandardHigherThanProject; }
    ...
    Icon getIcon() const override
    {
        ...
        if (! isSelected())
        {
            if (moduleInfo.isValid() && moduleInfo.getVendor() == "drx")
            {
...
private:
    ...
    b8 missingDependencies = false;
    b8 cppStandardHigherThanProject = false;
    ...
    {
    public:
        ModuleSettingsPanel (Project& p, const Txt& modID, TreeView* tree)
        ...
        z0 refresh()
        {
            ...
            for (Project::ExporterIterator exporter (project); exporter.next();)
            {
                ...
                const auto fallbackPath = getAppSettings().getStoredPath (isDRXModule (moduleID) ? Ids::defaultDrxModulePath
            ...
            props.add (new BooleanPropertyComponent (useGlobalPathValue,
                       ...
                       Txt ("If this is enabled, then the locally-stored global path (set in the ") + menuItemString + " menu item) "
            ...
            props.add (new BooleanPropertyComponent (modules.shouldCopyModuleFilesLocallyValue (moduleID),
                       ...
                       "If this is enabled, then a local copy of the entire module will be made inside your project (in the auto-generated DrxLibraryFiles folder), "
        ...
        Txt getModuleID() const noexcept    { return moduleID; }

    private:
        z0 valueTreePropertyChanged (ValueTree&, const Identifier& property) override
        {
            if (property == Ids::defaultDrxModulePath || property == Ids::defaultUserModulePath)
                refresh();
        }
        ...
        {
        public:
            ModuleInfoComponent (Project& p, const Txt& modID)
        ...
        private:
            ...
            DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ModuleInfoComponent)
        };
        ...
        {
        public:
            MissingDependenciesComponent (Project& p, const Txt& modID)
            ...
            z0 paint (Graphics& g) override
            {
                Txt text ("This module has missing dependencies!\n\n"
        ...
        private:
            ...
            DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (MissingDependenciesComponent)
        };
        ...
        struct CppStandardWarningComponent final : public PropertyComponent
        {
            ...
            z0 paint (Graphics& g) override
            {
                auto text = Txt ("This module has a higher C++ language standard requirement than your project!\n\n"
            ...
            DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (CppStandardWarningComponent)
        };
    };

    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ModuleItem)
};
...
public:
          ...
          modulesListTree (project.getEnabledModules().getState())
    {
        ...
        ProjucerApplication::getApp().getDRXPathModulesList().addListener (this);
    ...
    {
        ProjucerApplication::getApp().getDRXPathModulesList().removeListener (this);
    ...
    b8 isModulesList() const override     { return true; }
    b8 canBeSelected() const override     { return true; }
    b8 mightContainSubItems() override    { return true; }
    Txt getUniqueName() const override   { return "modules"; }
    Txt getRenamingName() const override { return getDisplayName(); }
    Txt getDisplayName() const override  { return "Modules"; }
    z0 setName (const Txt&) override   {}
    ...
    {
        for (i32 i = 0; i < project.getEnabledModules().getNumModules(); ++i)
    ...
    z0 showPopupMenu (Point<i32> p) override
    {
        ...
        addModulesSubMenu ("Global DRX modules path",
                           ProjucerApplication::getApp().getDRXPathModulesList().getAllModules(),
...
private:
    ...
    z0 removeDuplicateModules()
    {
        auto jucePathModulesList = ProjucerApplication::getApp().getDRXPathModulesList().getAllModules();
    ...
    z0 setSelectedItem (const Txt& itemID)
    {
        for (i32 i = 0; i < getNumSubItems(); ++i)
        {
            if (auto* item = getSubItem (i))
    ...
    //==============================================================================
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (EnabledModulesItem)
};
