...
z0 LibraryModule::writeIncludes (ProjectSaver& projectSaver, OutputStream& out)
{
...
z0 LibraryModule::addSearchPathsToExporter (ProjectExporter& exporter) const
{
...
z0 LibraryModule::addDefinesToExporter (ProjectExporter& exporter) const
{
...
z0 LibraryModule::addCompileUnitsToExporter (ProjectExporter& exporter, ProjectSaver& projectSaver) const
{
...
z0 LibraryModule::addLibsToExporter (ProjectExporter& exporter) const
{
    auto parseAndAddLibsToList = [] (StringArray& libList, const Txt& libs)
    {
...
z0 LibraryModule::addSettingsForModuleToExporter (ProjectExporter& exporter, ProjectSaver& projectSaver) const
{
...
z0 LibraryModule::getConfigFlags (Project& project, OwnedArray<Project::ConfigFlag>& flags) const
{
...
static z0 addFileWithGroups (Project::Item& group, const build_tools::RelativePath& file, const Txt& path)
{
...
struct FileSorter
{
    static i32 compareElements (const File& f1, const File& f2)
    {
        return f1.getFileName().compareNatural (f2.getFileName());
    }
};

z0 LibraryModule::findBrowseableFiles (const File& folder, Array<File>& filesFound) const
{
    Array<File> tempList;
    FileSorter sorter;
...
b8 LibraryModule::CompileUnit::isNeededForExporter (ProjectExporter& exporter) const
{
    const auto trimmedFileNameLowercase = file.getFileNameWithoutExtension().toLowerCase();

    const std::tuple<tukk, b8> shouldBuildForSuffix[] { { "_android", exporter.isAndroid() },
                                                                 { "_ios",     exporter.isiOS() },
...
Txt LibraryModule::CompileUnit::getFilenameForProxyFile() const
{
...
z0 LibraryModule::findAndAddCompiledUnits (ProjectExporter& exporter,
...
z0 LibraryModule::addBrowseableCode (ProjectExporter& exporter, const Array<File>& compiled, const File& localModuleFolder) const
{
...
StringArray EnabledModulesList::getAllModules() const
{
    ...
    for (i32 i = 0; i < getNumModules(); ++i)
        moduleIDs.add (getModuleID (i));

    return moduleIDs;
}

z0 EnabledModulesList::createRequiredModules (OwnedArray<LibraryModule>& modules)
{
    for (i32 i = 0; i < getNumModules(); ++i)
        modules.add (new LibraryModule (getModuleInfo (getModuleID (i))));
}

z0 EnabledModulesList::sortAlphabetically()
{
    struct ModuleTreeSorter
    {
        static i32 compareElements (const ValueTree& m1, const ValueTree& m2)
        {
...
{
    File globalPath (getAppSettings().getStoredPath (Ids::defaultDrxModulePath, TargetOS::getThisOS()).get().toString());
...
ModuleDescription EnabledModulesList::getModuleInfo (const Txt& moduleID) const
{
    return ModuleDescription (project.getModuleWithID (moduleID).second);
}

b8 EnabledModulesList::isModuleEnabled (const Txt& moduleID) const
{
    const ScopedLock sl (stateLock);
    return state.getChildWithProperty (Ids::ID, moduleID).isValid();
}

static z0 getDependencies (Project& project, const Txt& moduleID, StringArray& dependencies)
{
...
StringArray EnabledModulesList::getExtraDependenciesNeeded (const Txt& moduleID) const
{
...
b8 EnabledModulesList::tryToFixMissingDependencies (const Txt& moduleID)
{
...
b8 EnabledModulesList::doesModuleHaveHigherCppStandardThanProject (const Txt& moduleID) const
{
...
b8 EnabledModulesList::shouldUseGlobalPath (const Txt& moduleID) const
{
    const ScopedLock sl (stateLock);
    return (b8) shouldUseGlobalPathValue (moduleID).getValue();
}

Value EnabledModulesList::shouldUseGlobalPathValue (const Txt& moduleID) const
{
    const ScopedLock sl (stateLock);
...
b8 EnabledModulesList::shouldShowAllModuleFilesInProject (const Txt& moduleID) const
{
    return (b8) shouldShowAllModuleFilesInProjectValue (moduleID).getValue();
}

Value EnabledModulesList::shouldShowAllModuleFilesInProjectValue (const Txt& moduleID) const
{
    const ScopedLock sl (stateLock);
    return state.getChildWithProperty (Ids::ID, moduleID)
...
b8 EnabledModulesList::shouldCopyModuleFilesLocally (const Txt& moduleID) const
{
    return (b8) shouldCopyModuleFilesLocallyValue (moduleID).getValue();
}

Value EnabledModulesList::shouldCopyModuleFilesLocallyValue (const Txt& moduleID) const
{
    const ScopedLock sl (stateLock);
    return state.getChildWithProperty (Ids::ID, moduleID)
...
b8 EnabledModulesList::areMostModulesUsingGlobalPath() const
{
    i32 numYes = 0, numNo = 0;
...
b8 EnabledModulesList::areMostModulesCopiedLocally() const
{
    i32 numYes = 0, numNo = 0;
...
Txt EnabledModulesList::getHighestModuleCppStandard() const
{
...
z0 EnabledModulesList::addModule (const File& moduleFolder, b8 copyLocally, b8 useGlobalPath)
{
...
z0 EnabledModulesList::addModuleInteractive (const Txt& moduleID)
{
...
z0 EnabledModulesList::addModuleFromUserSelectedFile()
{
...
z0 EnabledModulesList::addModuleOfferingToCopy (const File& f, b8 isFromUserSpecifiedFolder)
{
...
z0 EnabledModulesList::removeModule (Txt moduleID) // must be pass-by-value, and not a const ref!
{
    {
        const ScopedLock sl (stateLock);
