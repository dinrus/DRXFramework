...
struct FileHelpers
{
    ...
    static b8 endsWith (const std::string& s, t8 c)
    {
        if (s.length() == 0)
            return false;
    ...
    static b8 deleteFile (const std::string& path)
    {
        if (! exists (path))
            return true;
    ...
    static b8 isDirectory (const std::string& path)
    {
        ...
        struct DRX_STAT info;

        return    ! path.empty()
               && DRX_STAT (path.c_str(), &info) == 0
    ...
    static b8 createDirectory (const std::string& path)
    {
        if (isDirectory (path))
            return true;
...
{
    static b8 isQuoteCharacter (t8 c)
    {
        return c == '"' || c == '\'';
    }
    ...
    static z0 ltrim (std::string& s)
    {
        s.erase (s.begin(), std::find_if (s.begin(), s.end(), [] (i32 c) { return ! std::isspace (c); }));
    }

    static z0 rtrim (std::string& s)
    {
        s.erase (std::find_if (s.rbegin(), s.rend(), [] (i32 c) { return ! std::isspace (c); }).base(), s.end());
    }
    ...
    static std::string replaced (const std::string& str, t8 charToReplace, t8 replaceWith)
    {
        auto result = str;
...
//==============================================================================
static b8 addFile (const std::string& filePath,
                     ...
                     b8 verbose)
{
    ...
    std::vector<t8> buffer ((size_t) fileStream.tellg());
    fileStream.seekg (0);
    ...
    headerStream << "    extern tukk  " << variableName << ";" << std::endl
                 << "    i32k           " << variableName << "Size = "
                 << buffer.size() << ";" << std::endl;

    cppStream << "static u8k temp0[] = {";

    auto* data = (u8k*) buffer.data();
    ...
    cppStream << (i32) data[buffer.size() - 1] << ",0,0};" << std::endl;
    cppStream << "tukk " << binaryNamespace << "::" << variableName
              << " = (tukk) temp0" << ";" << std::endl << std::endl;
...
public:
    ...
    static std::optional<Arguments> create (i32 argc, tuk argv[])
    {
...
private:
    Arguments (std::vector<std::string> args, b8 verboseIn)
