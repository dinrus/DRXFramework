...
def remove_drx_namespaces(source):
...
def add_doxygen_group(path, group_name):
    ...
    if re.match(r"^drx_.*\.(h|dox)", filename):
        with open(path, "r") as f:
            content = f.read()
        with open(path, "w") as f:
            f.write("\r\n/** @weakgroup " + group_name + "\r\n *  @{\r\n */\r\n")
            f.write(remove_drx_namespaces(content))
            f.write("\r\n/** @}*/\r\n")
...
if __name__ == "__main__":
    ...
    # Get the list of DRX modules to include.
    if args.subdirs:
        drx_modules = args.subdirs.split(",")
    else:
        drx_modules = []
        for item in sorted(os.listdir(args.source_dir)):
            if os.path.isdir(os.path.join(args.source_dir, item)):
                drx_modules.append(item)

    # Copy the DRX modules to the temporary directory, and process the source
    # files.
    module_definitions = []
    for module_name in drx_modules:
    ...
    with open(os.path.join(args.dest_dir, "drx_modules.dox"), "w") as f:
        f.write("\r\n\r\n".join(module_definitions))

    # Copy markdown docs
    for name in ["DRX Module Format.md", "CMake API.md"]:
