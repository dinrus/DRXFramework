...
The simplest way to include DRX in your project is to add DRX as a
subdirectory of your project, and to include the line `add_subdirectory(DRX)`
in your project CMakeLists.txt. This will make the DRX targets and helper
functions available for use by your custom targets.

### Using `find_package`

To install DRX globally on your system, you'll need to tell CMake where to
place the installed files.

    # Go to DRX directory
    cd /path/to/clone/DRX
    # Configure build with library components only
    cmake -B cmake-build-install -DCMAKE_INSTALL_PREFIX=/path/to/DRX/install
    # Run the installation
    cmake --build cmake-build-install --target install

In your project which consumes DRX, make sure the project CMakeLists.txt contains the line
`find_package(DRX CONFIG REQUIRED)`. This will make the DRX modules and CMake helper functions
available for use in the rest of your build. Then, run the build like so:

    # Go to project directory
    cd /path/to/my/project
    # Configure build, passing the DRX install path you used earlier
    cmake -B cmake-build -DCMAKE_PREFIX_PATH=/path/to/DRX/install
    # Build the project
    cmake --build cmake-build

### Example projects

In the DRX/examples/CMake directory, you'll find example projects for a GUI app, a console app,
and an audio plugin. You can simply copy one of these subdirectories out of the DRX repo, add DRX
as a submodule, and uncomment the call to `add_subdirectory` where indicated in the CMakeLists.txt.
Alternatively, if you've installed DRX using a package manager or the CMake install target, you can
...
you might invoke CMake like this, from the new directory you created.
    ...
    cmake -Bbuild (-GgeneratorName) (-DDRX_BUILD_EXTRAS=ON) (-DDRX_BUILD_EXAMPLES=ON)
...
to see a full list of generators on your platform). If you included DRX as a subdirectory, you can
...
Note that the static library produced by `drx_add_binary_data` automatically sets this property.
...
These flags can be enabled or disabled to change the behaviour of parts of the DRX build.
...
- Supplying an option in the form `-DNAME_OF_OPTION=ON/OFF` to the initial CMake configuration call,
  or
- Calling `set(NAME_OF_OPTION ON/OFF)` before including DRX in your project via `add_subdirectory`
  or `find_package`.
...
if you've included DRX in your own project.
...
included DRX in your own project.

#### `DRX_ENABLE_MODULE_SOURCE_GROUPS`
...
that the call to `drx_add_module` which adds a specific module happens *before* calling
`drx_add_*` to add any dependent targets.

The modules will be placed in a group named "DRX Modules" within the group for each target,
alongside the "Source Files" and "Header Files" groups.

Note: Source groups will only work when all DRX-dependent targets are created using the
`drx_add_*` functions. The standard `add_executable` and `add_library` commands are likely to
result in broken builds when source groups are enabled!

#### `DRX_COPY_PLUGIN_AFTER_BUILD`
...
Only brings in targets for the built-in DRX modules, and the `drx_add_module*` CMake functions.
This is meant for highly custom use-cases where the `drx_add_gui_app` and `drx_add_plugin`
...
`DRX_ENABLE_MODULE_SOURCE_GROUPS` will have no effect.

#### `DRX_WEBVIEW2_PACKAGE_LOCATION`

You can ask DRX to link the WebView2 library statically to your target on Windows, by specifying 
the `NEEDS_WEBVIEW2` option when creating your target. In this case DRX will search for the 
...
#### `drx_add_<target>`
    ...
    drx_add_gui_app(<target> [KEY value]...)
    drx_add_console_app(<target> [KEY value]...)
    drx_add_plugin(<target> [KEY value]...)

`drx_add_gui_app` and `drx_add_console_app` add an executable target with name `<target>`.
`drx_add_plugin` adds a 'shared code' static library target with name `<target>`, along with extra
...
Each of these arguments adds a property to the resulting target in the form `DRX_paramName`, where
`paramName` is one of the parameter keys below. For example, after a call to
`drx_add_gui_app(my_target PRODUCT_NAME "Target")`, the target `my_target` will have a property
named `DRX_PRODUCT_NAME` with the value `"Target"`. After creating a target with one of these
commands, properties beginning with `DRX_` can be _queried_, but changing their values might not
have any effect (or might even break things in unexpected ways!), so always pass DRX target
...
- Copyright text which will be added to the app/plugin's Info.plist. The value of this argument
  will be inherited from the `DRX_COMPANY_COPYRIGHT` property, so if you want to use the same
  `COMPANY_COPYRIGHT` for several targets in a build tree, you can call
  `set_directory_properties(PROPERTIES DRX_COMPANY_COPYRIGHT ...)` after including DRX but
...
- The name of this target's author. Will be added to the app/plugin's Info.plist, and may be used
  ...
  will be inherited from the `DRX_COMPANY_NAME` property, so if you want to use the same
  `COMPANY_NAME` for several targets in a build tree, you can call
  `set_directory_properties(PROPERTIES DRX_COMPANY_NAME ...)` after including DRX but before
...
- The address of a website related to this target in some way. The value of this argument will be
  inherited from the `DRX_COMPANY_WEBSITE` property, so if you want to use the same
  `COMPANY_WEBSITE` for several targets in a build tree, you can call
  `set_directory_properties(PROPERTIES DRX_COMPANY_WEBSITE ...)` after including DRX but before
...
- An email address for this target's author. The value of this argument will be inherited from the
  `DRX_COMPANY_EMAIL` property, so if you want to use the same `COMPANY_EMAIL` for several
  targets in a build tree, you can call `set_directory_properties(PROPERTIES DRX_COMPANY_EMAIL
  ...)` after including DRX but before adding the targets, and then omit the `COMPANY_EMAIL`
...
`NEEDS_CURL`
- On Linux, DRX may or may not need to link to Curl depending on the compile definitions that are
  set on a DRX target. By default, we don't link Curl because you might not need it, but if you
  get linker or include errors that reference Curl, just set this argument to `TRUE`.

`NEEDS_WEB_BROWSER`
- On Linux, DRX may or may not need to link to Webkit depending on the compile definitions that
  are set on a DRX target. By default, we don't link Webkit because you might not need it, but
  if you get linker or include errors that reference Webkit, just set this argument to `TRUE`.

`NEEDS_WEBVIEW2`
- On Windows, DRX may or may not need to link to WebView2 depending on the compile definitions that
  are set on a DRX target. By default, we don't link WebView2 because you might not need it, but
  if you get linker or include errors that reference WebView2, just set this argument to `TRUE`.

`NEEDS_STORE_KIT`
- On macOS, DRX may or may not need to link to StoreKit depending on the compile definitions that
  are set on a DRX target. By default, we don't link StoreKit because you might not need it, but
...
- For plugin targets, specifies the plugin targets to build. Should be provided as a
  ...
  without first calling `drx_set_aax_sdk_path` or `drx_set_vst2_sdk_path` respectively.
...
- May be either TRUE or FALSE (defaults to FALSE). If TRUE, will add the preprocessor definition
  `DRX_PLUGINHOST_AU=1` to the new target, and will link the macOS frameworks necessary for
  hosting plugins. Using this parameter should be preferred over using
  `target_compile_definitions` to manually set the `DRX_PLUGINHOST_AU` preprocessor definition.
...
- May be either TRUE or FALSE (defaults to FALSE). If TRUE, will override the value of the
  ...
  option exists to maintain compatibility with a previous, buggy version of DRX's CMake support
...
- Whether or not to install the plugin to the current system after building. False by default.
  ...
  you can set the property `DRX_COPY_PLUGIN_AFTER_BUILD` on the directory before adding the
...
- The location to which VST2 (legacy) plugins will be copied after building if
  ...
  in a subdirectory to a non-default location, you can set the `DRX_VST_COPY_DIR` property on
...
- The location to which VST3 plugins will be copied after building if `COPY_PLUGIN_AFTER_BUILD`
  ...
  non-default location, you can set the `DRX_VST3_COPY_DIR` property on the directory before
...
- The location to which AAX plugins will be copied after building if `COPY_PLUGIN_AFTER_BUILD`
  ...
  non-default location, you can set the `DRX_AAX_COPY_DIR` property on the directory before
  adding the plugin targets, rather than setting this argument on each individual target.
...
- The location to which AU plugins will be copied after building if `COPY_PLUGIN_AFTER_BUILD`
  ...
  non-default location, you can set the `DRX_AU_COPY_DIR` property on the directory before
  adding the plugin targets, rather than setting this argument on each individual target.
...
- The location to which Unity plugins will be copied after building if `COPY_PLUGIN_AFTER_BUILD`
  ...
  non-default location, you can set the `DRX_UNITY_COPY_DIR` property on the directory before
  adding the plugin targets, rather than setting this argument on each individual target.
...
- May be either TRUE or FALSE (defaults to TRUE). When TRUE, a POST_BUILD step will be added to the
  ...
  `drx_enable_vst3_manifest_step` function. It is strongly recommended to generate a manifest for
...
#### `drx_add_bundle_resources_directory`

    drx_add_bundle_resources_directory(<target> <folder>)
...
#### `drx_generate_drx_header`

    drx_generate_drx_header(<target>)

Introspects the DRX modules that have been linked to `<target>` and generates a `DrxHeader.h`
which contains `#include` statements for each of the module headers. This header also contains an
optional `using namespace drx` statement, and an optional `ProjectInfo` block, each of which can be
disabled by setting the compile definitions `DONT_SET_USING_DRX_NAMESPACE` and
`DRX_DONT_DECLARE_PROJECTINFO` respectively. The resulting header can be included with `#include
<DrxHeader.h>`. In plain CMake projects which don't require Projucer compatibility, the use of
DrxHeader.h is optional. Instead, module headers can be included directly in source files that
require them.

#### `drx_enable_copy_plugin_step`

    drx_enable_copy_plugin_step(<target>)

As an alternative to the DRX_COPY_PLUGIN_AFTER_BUILD property, you may call this function to
manually enable post-build copy on a plugin. The argument to this function should be a target
previously created with `drx_add_plugin`.

DRX_COPY_PLUGIN_AFTER_BUILD will cause plugins to be installed immediately after building. This is
not always appropriate, if extra build steps (such as signing or modifying the plugin bundle) must
be executed before the install. In such cases, you should leave DRX_COPY_PLUGIN_AFTER_BUILD
disabled, use `add_custom_command(TARGET POST_BUILD)` to add your own post-build steps, and then
finally call `drx_enable_copy_plugin_step`.

If your custom build steps need to use the location of the plugin artefact, you can extract this
by querying the property `DRX_PLUGIN_ARTEFACT_FILE` on a plugin target (*not* the shared code
target!).

#### `drx_enable_vst3_manifest_step`

    drx_enable_vst3_manifest_step(<target>)

You may call this function to manually enable VST3 manifest generation on a plugin. The argument to
this function should be a target previously created with `drx_add_plugin`.
...
steps, and then finally call `drx_enable_vst3_manifest_step`.

#### `drx_set_<kind>_sdk_path`

    drx_set_aax_sdk_path(<absolute path>)
    drx_set_vst2_sdk_path(<absolute path>)
    drx_set_vst3_sdk_path(<absolute path>)
    drx_set_ara_sdk_path(<absolute path>)
...
#### `drx_add_module`
    ...
    drx_add_module(<path to module>)
    drx_add_modules(<names of module>...)

`drx_add_module` adds a library target for the DRX module located at the provided path. `<path>`
must be the path to a module directory (e.g. /Users/me/DRX/modules/drx_core). This will add an
...
`company::my_module`. ``` drx_add_module(my_module ALIAS_NAMESPACE company)` ```

`drx_add_modules` is a convenience function that can be used to add multiple DRX modules at once.
...
plugin formats as possible. To build VST2 targets, call `drx_set_vst2_sdk_path` *before* calling
`drx_add_pip`.

This is mainly provided to build the built-in example projects in the DRX repo, and for building
quick proof-of-concept demo apps with minimal set-up. For any use-case more complex than a
proof-of-concept, you should prefer the `drx_add_gui_app`, `drx_add_plugin`, or
`drx_add_console_app` functions, which provide more fine-grained control over the properties of
your target.

#### `drx_disable_default_flags`
...
#### `drx_link_with_embedded_linux_subprocess`
...
by the `drx_gui_extra` module and only when its `DRX_WEB_BROWSER` capability is enabled. This
...
This function is automatically called if necessary for all targets created by one of the DRX target
creation functions i.e. `drx_add_gui_app`, `drx_add_console_app` and `drx_add_gui_app`. You don't
need to call this function manually in these cases.

### Targets

#### `drx::drx_recommended_warning_flags`

    target_link_libraries(myTarget PUBLIC drx::drx_recommended_warning_flags)

This is a target which can be linked to other targets using `target_link_libraries`, in order to
enable the recommended DRX warnings when building them.
...
#### `drx::drx_recommended_config_flags`

    target_link_libraries(myTarget PUBLIC drx::drx_recommended_config_flags)

This is a target which can be linked to other targets using `target_link_libraries`, in order to
enable the recommended DRX optimisation and debug flags.
...
#### `drx::drx_recommended_lto_flags`

    target_link_libraries(myTarget PUBLIC drx::drx_recommended_lto_flags)

This is a target which can be linked to other targets using `target_link_libraries`, in order to
enable the recommended DRX link time optimisation settings.
