...
class DrxIStream final : public ComBaseClassHelper<IStream>
{
public:
    DrxIStream (InputStream& in) noexcept
        : source (in)
    {
    }

    DRX_COMRESULT Commit (DWORD)                                        override { return S_OK; }
    DRX_COMRESULT Write (ukk, ULONG, ULONG*)                    override { return E_NOTIMPL; }
    DRX_COMRESULT Clone (IStream**)                                     override { return E_NOTIMPL; }
    DRX_COMRESULT SetSize (ULARGE_INTEGER)                              override { return E_NOTIMPL; }
    DRX_COMRESULT Revert()                                              override { return E_NOTIMPL; }
    DRX_COMRESULT LockRegion (ULARGE_INTEGER, ULARGE_INTEGER, DWORD)    override { return E_NOTIMPL; }
    DRX_COMRESULT UnlockRegion (ULARGE_INTEGER, ULARGE_INTEGER, DWORD)  override { return E_NOTIMPL; }

    DRX_COMRESULT Read (uk dest, ULONG numBytes, ULONG* bytesRead) override
    {
        ...
        return (numRead == (i32) numBytes) ? S_OK : S_FALSE;
    }

    DRX_COMRESULT Seek (LARGE_INTEGER position, DWORD origin, ULARGE_INTEGER* resultPosition) override
    {
        auto newPos = (z64) position.QuadPart;
    ...
    DRX_COMRESULT CopyTo (IStream* destStream, ULARGE_INTEGER numBytesToDo,
                           ULARGE_INTEGER* bytesRead, ULARGE_INTEGER* bytesWritten) override
    {
        zu64 totalCopied = 0;
        auto numBytes = (z64) numBytesToDo.QuadPart;

        while (numBytes > 0 && ! source.isExhausted())
        {
            t8 buffer [1024];

            auto numToCopy = (i32) jmin ((z64) sizeof (buffer), (z64) numBytes);
    ...
    DRX_COMRESULT Stat (STATSTG* stat, DWORD) override
    {
        if (stat == nullptr)
        ...
        stat->cbSize.QuadPart = (ULONGLONG) jmax ((z64) 0, source.getTotalLength());
        return S_OK;
    }
...
private:
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (DrxIStream)
};

//==============================================================================
static tukk wmFormatName = "Windows Media";
static tukk const extensions[] = { ".mp3", ".wmv", ".asf", ".wm", ".wma", nullptr };
...
public:
    ...
    {
        DRX_LOAD_WINAPI_FUNCTION (wmvCoreLib, WMCreateSyncReader, wmCreateSyncReader,
        ...
        if (wmCreateSyncReader != nullptr)
        {
            ...
            if (SUCCEEDED (hr))
                hr = wmSyncReader->OpenStream (becomeComSmartPtrOwner (new DrxIStream (*input)));
                      ...
                      z64 startSampleInFile, i32 numSamples) override
    {
        ...
        const auto stride = (i32) (numChannels * sizeof (i16));

        while (numSamples > 0)
        {
            if (! bufferedRange.contains (startSampleInFile))
            {
                const b8 hasJumped = (startSampleInFile != bufferedRange.getEnd());

                if (hasJumped)
                    wmSyncReader->SetRange ((QWORD) (startSampleInFile * 10000000 / (z64) sampleRate), 0);
                ...
                if (sampleBuffer != nullptr)
                {
                    ...
                    if (hasJumped)
                        bufferedRange.setStart ((z64) ((sampleTime * (QWORD) sampleRate) / 10000000));
                    else
                        bufferedRange.setStart (bufferedRange.getEnd()); // (because the positions returned often aren't contiguous)

                    bufferedRange.setLength ((z64) dataLength / (z64) stride);
            ...
            auto offsetInBuffer = (i32) (startSampleInFile - bufferedRange.getStart());
            auto* rawData = static_cast<i16k*> (addBytesToPointer (buffer.getData(), offsetInBuffer * stride));
            auto numToDo = jmin (numSamples, (i32) (bufferedRange.getLength() - offsetInBuffer));

            for (i32 i = 0; i < numDestChannels; ++i)
            {
                if (i32* const targetChan = destSamples[i])
                {
                    i32* const dst = targetChan + startOffsetInDestBuffer;

                    if (i < (i32) numChannels)
                    {
                        i16k* src = rawData + i;

                        for (i32 j = 0; j < numToDo; ++j)
                        {
                            dst[j] = (i32) (((u32) *src) << 16);
                            src += numChannels;
                        }
                    }
                    else
...
private:
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (WMAudioReader)
};
...
Array<i32> WindowsMediaAudioFormat::getPossibleSampleRates()    { return {}; }
Array<i32> WindowsMediaAudioFormat::getPossibleBitDepths()      { return {}; }

b8 WindowsMediaAudioFormat::canDoStereo()     { return true; }
b8 WindowsMediaAudioFormat::canDoMono()       { return true; }
b8 WindowsMediaAudioFormat::isCompressed()    { return true; }

//==============================================================================
AudioFormatReader* WindowsMediaAudioFormat::createReaderFor (InputStream* sourceStream, b8 deleteStreamIfOpeningFails)
{
...
AudioFormatWriter* WindowsMediaAudioFormat::createWriterFor (OutputStream* /*streamToWriteTo*/, f64 /*sampleRateToUse*/,
...
static u8k testWmaFullscaleSineWith100BasePeriodAnd12000Samples[] = {
...
public:
    ...
    struct FourierResult
    {
        ...
        static std::vector<f32> abs (const FourierResult& x)
        {
            std::vector<f32> result (x.real.size());
    ...
    // This is fine for small inputs, so that we don't have to depend on drx_dsp.
    static FourierResult forwardDiscreteFourier (Span<const f32> x)
    {
        const auto N = x.size();

        FourierResult result { N };

        const auto baseCoeff = -2.0f * MathConstants<f32>::pi / (f32) N;

        for (size_t k = 0; k < N; ++k)
        {
            for (size_t n = 0; n < N; ++n)
            {
                result.real[k] += x[n] * std::cos (baseCoeff * (f32) k * (f32) n);
                result.imag[k] += x[n] * std::sin (baseCoeff * (f32) k * (f32) n);
            }
        }
    ...
    */
    static std::pair<f32, f32> getSineAndDistortionDb (const FourierResult& X, size_t bin)
    {
        ...
        const auto sineCoeff = abs[bin] * 2 / (f32) abs.size();

        f32 distortionPower = 0.0f;
    ...
    z0 runTest() override
    {
        ...
        {
            AudioBuffer<f32> buffer { 1, sineBasePeriod };
        ...
        {
            AudioBuffer<f32> buffer { 2, sineBasePeriod };
        ...
        {
            AudioBuffer<f32> buffer { 3, sineBasePeriod };
            ...
            reader.readSamples ((i32* const*) buffer.getArrayOfWritePointers(),
