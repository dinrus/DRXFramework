...
typedef struct {
  i32 res_type;
  i32 limit_type; /* 0 lowpass limited, 1 point stereo limited */
  i32 grouping;
...
typedef struct {
  i32 att[P_NOISECURVES];
  f32 boost;
  f32 decay;
} att3;
typedef struct { i32 data[P_NOISECURVES]; } adj3;

typedef struct {
  i32   pre[PACKETBLOBS];
  i32   post[PACKETBLOBS];
  f32 kHz[PACKETBLOBS];
  f32 lowpasskHz[PACKETBLOBS];
} adj_stereo;
...
typedef struct {
  i32 data[P_NOISECURVES][17];
} noise3;

typedef struct {
  i32      mappings;
  const f64  *rate_mapping;
  const f64  *quality_mapping;
  i32      coupling_restriction;
  i64     samplerate_min_restriction;
  i64     samplerate_max_restriction;


  i32k     *blocksize_short;
  i32k     *blocksize_long;

  const att3    *psy_tone_masteratt;
  i32k     *psy_tone_0dB;
  i32k     *psy_tone_dBsuppress;
  ...
  i32k         *psy_noise_dBsuppress;

  const compandblock  *psy_noise_compand;
  const f64        *psy_noise_compand_short_mapping;
  const f64        *psy_noise_compand_long_mapping;

  i32k      *psy_noise_normal_start[2];
  i32k      *psy_noise_normal_partition[2];
  const f64   *psy_noise_normal_thresh;

  i32k      *psy_ath_float;
  i32k      *psy_ath_abs;

  const f64   *psy_lowpass;

  const vorbis_info_psy_global *global_params;
  const f64     *global_mapping;
  const adj_stereo *stereo_modes;

  const static_codebook *const *const *const floor_books;
  const vorbis_info_floor1 *floor_params;
  i32k floor_mappings;
  i32k **floor_mapping_list;
...
static z0 vorbis_encode_floor_setup(vorbis_info *vi,i32 s,
...
static z0 vorbis_encode_global_psych_setup(vorbis_info *vi,f64 s,
...
static z0 vorbis_encode_global_stereo(vorbis_info *vi,
                                        const highlevel_encode_setup *const hi,
                                        const adj_stereo *p){
  f32 s=hi->stereo_point_setting;
  i32 i,is=s;
  f64 ds=s-is;
  codec_setup_info *ci=(codec_setup_info*)vi->codec_setup;
  vorbis_info_psy_global *g=&ci->psy_g_param;
  ...
  if(p){
    ...
    if(hi->managed){
      ...
      for(i=0;i<PACKETBLOBS;i++){
        f32 kHz=p[is].kHz[i]*(1.-ds)+p[is+1].kHz[i]*ds;
      ...
      f32 kHz=p[is].kHz[PACKETBLOBS/2]*(1.-ds)+p[is+1].kHz[PACKETBLOBS/2]*ds;
      for(i=0;i<PACKETBLOBS;i++){
        g->coupling_pointlimit[0][i]=kHz*1000./vi->rate*ci->blocksizes[0];
        g->coupling_pointlimit[1][i]=kHz*1000./vi->rate*ci->blocksizes[1];
        g->coupling_pkHz[i]=kHz;
...
static z0 vorbis_encode_psyset_setup(vorbis_info *vi,f64 s,
                                       i32k *nn_start,
                                       i32k *nn_partition,
                                       const f64 *nn_thresh,
                                       i32 block){
...
static z0 vorbis_encode_tonemask_setup(vorbis_info *vi,f64 s,i32 block,
                                         const att3 *att,
                                         i32k  *max,
...
static z0 vorbis_encode_compand_setup(vorbis_info *vi,f64 s,i32 block,
...
static z0 vorbis_encode_peak_setup(vorbis_info *vi,f64 s,i32 block,
                                    i32k *suppress){
  i32 is=s;
  f64 ds=s-is;
  codec_setup_info *ci=(codec_setup_info*)vi->codec_setup;
...
static z0 vorbis_encode_noisebias_setup(vorbis_info *vi,f64 s,i32 block,
                                         i32k *suppress,
                                         const noise3 *in,
  ...
  for(j=0;j<P_NOISECURVES;j++){
    f32 min=p->noiseoff[j][0]+6; /* the lowest it can go */
    for(i=0;i<P_BANDS;i++){
...
static z0 vorbis_encode_ath_setup(vorbis_info *vi,i32 block){
  codec_setup_info *ci=(codec_setup_info*)vi->codec_setup;
  vorbis_info_psy *p=ci->psy_param[block];
...
static i32 book_dup_or_new(codec_setup_info *ci,const static_codebook *book){
  i32 i;
...
static z0 vorbis_encode_blocksize_setup(vorbis_info *vi,f64 s,
                                         i32k *shortb,i32k *longb){

  codec_setup_info *ci=(codec_setup_info*)vi->codec_setup;
  i32 is=s;

  i32 blockshort=shortb[is];
  i32 blocklong=longb[is];
  ci->blocksizes[0]=blockshort;
  ci->blocksizes[1]=blocklong;

}

static z0 vorbis_encode_residue_setup(vorbis_info *vi,
                                        i32 number, i32 block,
                                        const vorbis_residue_template *res){
  ...
  /* fill in all the books */
  {
    ...
    if(ci->hi.managed){
      ...
      for(i=0;i<r->partitions;i++){
        for(k=0;k<4;k++){
          if(res->books_base_managed->books[i][k]){
            i32 bookid=book_dup_or_new(ci,res->books_base_managed->books[i][k]);
      ...
      for(i=0;i<r->partitions;i++){
        for(k=0;k<4;k++){
          if(res->books_base->books[i][k]){
            i32 bookid=book_dup_or_new(ci,res->books_base->books[i][k]);
            r->booklist[booklist++]=bookid;
  ...
  /* lowpass setup/pointlimit */
  {
    ...
    if(ci->residue_type[number]==2){
      ...
      r->end=(i32)((freq/nyq*blocksize*ch)/r->grouping+.9)* /* round up only if we're well past */
        r->grouping;
      ...
      r->end=(i32)((freq/nyq*blocksize)/r->grouping+.9)* /* round up only if we're well past */
        r->grouping;
      /* the blocksize and grouping may disagree at the end */
...
static z0 vorbis_encode_map_n_res_setup(vorbis_info *vi,f64 s,
                                          const vorbis_mapping_template *maps){

  codec_setup_info *ci=(codec_setup_info*)vi->codec_setup;
  i32 i,j,is=s,modes=2;
...
static f64 setting_to_approx_bitrate(vorbis_info *vi){
  codec_setup_info *ci=(codec_setup_info*)vi->codec_setup;
  highlevel_encode_setup *hi=&ci->hi;
  ve_setup_data_template *setup=(ve_setup_data_template *)hi->setup;
  i32 is=hi->base_setting;
  f64 ds=hi->base_setting-is;
  i32 ch=vi->channels;
  const f64 *r=setup->rate_mapping;
...
static ukk get_setup_template(i64 ch,i64 srate,
                                      f64 req,i32 q_or_bitrate,
                                      f64 *base_setting){
  i32 i=0,j;
  ...
  while(setup_list[i]){
    if(setup_list[i]->coupling_restriction==-1 ||
      ...
      if(srate>=setup_list[i]->samplerate_min_restriction &&
        ...
        i32 mappings=setup_list[i]->mappings;
        const f64 *map=(q_or_bitrate?
...
i32 vorbis_encode_setup_init(vorbis_info *vi){
  i32 i,i0=0,singleblock=0;
  codec_setup_info *ci=(codec_setup_info*)vi->codec_setup;
  ...
  /* again, bound this to avoid the app shooting itself i32 he foot
     too badly */
  ...
  /* choose block sizes from configured sizes as well as paying
     ...
     short and i64 blocks are the same length, we set long_block_p
  ...
  /* floor setup; choose proper floor params.  Allocated on the floor
     stack in order; if we alloc only a single i64 floor, it's 0 */
  ...
  if(hi->bitrate_av)
    vi->bitrate_window=(f64)hi->bitrate_reservoir/hi->bitrate_av;
  else
    vi->bitrate_window=0.;
...
static z0 vorbis_encode_setup_setting(vorbis_info *vi,
                                       i64  channels,
                                       i64  rate){
  i32 i,is;
  codec_setup_info *ci=(codec_setup_info*)vi->codec_setup;
...
i32 vorbis_encode_setup_vbr(vorbis_info *vi,
                            i64  channels,
                            i64  rate,
                            f32 quality){
  codec_setup_info *ci;
...
i32 vorbis_encode_init_vbr(vorbis_info *vi,
                           i64 channels,
                           i64 rate,

                           f32 base_quality /* 0. to 1. */
                           ){
  i32 ret=0;
...
i32 vorbis_encode_setup_managed(vorbis_info *vi,
                                i64 channels,
                                i64 rate,
...
i32 vorbis_encode_init(vorbis_info *vi,
                       i64 channels,
                       i64 rate,
  ...
  i32 ret=vorbis_encode_setup_managed(vi,channels,rate,
                                      max_bitrate,
                                      nominal_bitrate,
                                      min_bitrate);
  if(ret){
...
i32 vorbis_encode_ctl(vorbis_info *vi,i32 number,uk arg){
  if(vi){
    codec_setup_info *ci=(codec_setup_info*)vi->codec_setup;
    highlevel_encode_setup *hi=&ci->hi;
    i32 setp=(number&0xf); /* a read request has a low nibble of 0 */
    ...
    case OV_ECTL_RATEMANAGE_GET:
      {
        ...
        ai->bitrate_hard_window=ai->bitrate_av_window=
          (f64)hi->bitrate_reservoir/vi->rate;
