...
static i64 _get_data(OggVorbis_File *vf){
  errno=0;
  if(!(vf->callbacks.read_func))return(-1);
  if(vf->datasource){
    t8 *buffer=ogg_sync_buffer(&vf->oy,READSIZE);
    i64 bytes=(vf->callbacks.read_func)(buffer,1,READSIZE,vf->datasource);
...
static i32 _seek_helper(OggVorbis_File *vf,ogg_int64_t offset){
  if(vf->datasource){
...
static z0 _add_serialno(ogg_page *og,i64 **serialno_list, i32 *n){
  i64 s = ogg_page_serialno(og);
  (*n)++;

  if(*serialno_list){
    *serialno_list = (i64*)_ogg_realloc(*serialno_list, sizeof(**serialno_list)*(*n));
  }else{
    *serialno_list = (i64*)_ogg_malloc(sizeof(**serialno_list));
  }
...
static i32 _lookup_serialno(i64 s, i64 *serialno_list, i32 n){
  if(serialno_list){
    while(n--){
...
static i32 _lookup_page_serialno(ogg_page *og, i64 *serialno_list, i32 n){
  i64 s = ogg_page_serialno(og);
...
static ogg_int64_t _get_prev_page_serial(OggVorbis_File *vf, ogg_int64_t begin,
                                         i64 *serial_list, i32 serial_n,
                                         i32 *serialno, ogg_int64_t *granpos){
  ogg_page og;
  ogg_int64_t end=begin;
  ogg_int64_t ret;
...
static i32 _fetch_headers(OggVorbis_File *vf,vorbis_info *vi,vorbis_comment *vc,
                          i64 **serialno_list, i32 *serialno_n,
                          ogg_page *og_ptr){
  ogg_page og;
  ogg_packet op;
  i32 i,ret;
  ...
  while(1){
    ...
    while(i<2){ /* get a page loop */
      ...
      while(i<2){ /* get a packet loop */
        ...
        i32 result=ogg_stream_packetout(&vf->os,&op);
        if(result==0)break;
        if(result==-1){
          ret=OV_EBADHEADER;
          goto bail_header;
  ...
  while(1){
    ...
    while((result=ogg_stream_packetout(&vf->os,&op))){
      if(result>0){ /* ignore holes */
        i64 thisblock=vorbis_packet_blocksize(vi,&op);
        if(thisblock>=0){
          if(lastblock!=-1)
...
static i32 _bisect_forward_serialno(OggVorbis_File *vf,
                                    ogg_int64_t begin,
  ...
  */
  ...
  if(_lookup_serialno(endserial,currentno_list,currentnos)){
    ...
    vf->serialnos=(i64*)_ogg_malloc(vf->links*sizeof(*vf->serialnos));
    ...
    i64 *next_serialno_list=NULL;
    i32 next_serialnos=0;
    vorbis_info vi;
    vorbis_comment vc;
    i32 testserial = serialno+1;
...
static i32 _make_decode_ready(OggVorbis_File *vf){
...
static i32 _open_seekable2(OggVorbis_File *vf){
  ogg_int64_t dataoffset=vf->dataoffsets[0],end,endgran=-1;
  i32 endserial=vf->os.serialno;
  i32 serialno=vf->os.serialno;
...
static z0 _decode_clear(OggVorbis_File *vf){
...
static i32 _fetch_and_process_packet(OggVorbis_File *vf,
                                     ogg_packet *op_in,
                                     i32 readp,
                                     i32 spanp){
  ogg_page og;
  ...
  while(1){
    ...
    if(vf->ready_state==INITSET){
      i32 hs=vorbis_synthesis_halfrate_p(vf->vi);

      while(1) {
              ogg_packet op;
              ogg_packet *op_ptr=(op_in?op_in:&op);
        i32 result=ogg_stream_packetout(&vf->os,op_ptr);
        ...
        if(result>0){
          ...
          if(!vorbis_synthesis(&vf->vb,op_ptr)){ /* lazy check for lazy
            ...
            {
              i32 oldsamples=vorbis_synthesis_pcmout(&vf->vd,NULL);
            ...
            if(granulepos!=-1 && !op_ptr->e_o_s){
              i32 link=(vf->seekable?vf->current_link:0);
              i32 i,samples;
    ...
    */
    ...
    if(vf->ready_state!=INITSET){
      ...
      if(vf->ready_state<STREAMSET){
        if(vf->seekable){
          i64 serialno = ogg_page_serialno(&og);
          ...
          i32 ret=_fetch_headers(vf,vf->vi,vf->vc,NULL,NULL,&og);
          if(ret)return(ret);
...
/* if, eg, 64 bit stdio is configured by default, this will build with
   fseek64 */
static i32 _fseek64_wrap(FILE *f,ogg_int64_t off,i32 whence){
  if(f==NULL)return(-1);
  return fseek(f,off,whence);
}

static i32 _ov_open1(uk f,OggVorbis_File *vf,const t8 *initial,
                     i64 ibytes, ov_callbacks callbacks){
  i32 offsettest=((f && callbacks.seek_func)?callbacks.seek_func(f,0,SEEK_CUR):-1);
  i64 *serialno_list=NULL;
  i32 serialno_list_size=0;
  i32 ret;
  ...
  /* perhaps some data was previously read into a buffer for testing
     ...
     non-seekable stream) */
  if(initial){
    t8 *buffer=ogg_sync_buffer(&vf->oy,ibytes);
  ...
  if((ret=_fetch_headers(vf,vf->vi,vf->vc,&serialno_list,&serialno_list_size,NULL))<0){
    ...
    vf->serialnos=(i64*)_ogg_calloc(serialno_list_size+2,sizeof(*vf->serialnos));
...
static i32 _ov_open2(OggVorbis_File *vf){
  if(vf->ready_state != PARTOPEN) return OV_EINVAL;
  vf->ready_state=OPENED;
  if(vf->seekable){
    i32 ret=_open_seekable2(vf);
    if(ret){
      vf->datasource=NULL;
      ov_clear(vf);
    }
...
i32 ov_open_callbacks(uk f,OggVorbis_File *vf,
    const t8 *initial,i64 ibytes,ov_callbacks callbacks){
  i32 ret=_ov_open1(f,vf,initial,ibytes,callbacks);
  if(ret)return ret;
  return _ov_open2(vf);
}

i32 ov_open(FILE *f,OggVorbis_File *vf,const t8 *initial,i64 ibytes){
  ov_callbacks callbacks = {
    (size_t (*)(uk , size_t, size_t, uk ))  fread,
    (i32 (*)(uk , ogg_int64_t, i32))              _fseek64_wrap,
    (i32 (*)(uk ))                             fclose,
    (i64 (*)(uk ))                            ftell
  };

  return ov_open_callbacks((uk )f, vf, initial, ibytes, callbacks);
}

i32 ov_fopen(const t8 *path,OggVorbis_File *vf){
  i32 ret;
...
i32 ov_halfrate(OggVorbis_File *vf,i32 flag){
  i32 i;
...
i32 ov_halfrate_p(OggVorbis_File *vf){
  if(vf->vi==NULL)return OV_EINVAL;
...
i32 ov_test_callbacks(uk f,OggVorbis_File *vf,
    const t8 *initial,i64 ibytes,ov_callbacks callbacks)
{
  return _ov_open1(f,vf,initial,ibytes,callbacks);
}

i32 ov_test(FILE *f,OggVorbis_File *vf,const t8 *initial,i64 ibytes){
  ov_callbacks callbacks = {
    (size_t (*)(uk , size_t, size_t, uk ))  fread,
    (i32 (*)(uk , ogg_int64_t, i32))              _fseek64_wrap,
    (i32 (*)(uk ))                             fclose,
  ...
  return ov_test_callbacks((uk )f, vf, initial, ibytes, callbacks);
}

i32 ov_test_open(OggVorbis_File *vf){
  if(vf->ready_state!=PARTOPEN)return(OV_EINVAL);
  return _ov_open2(vf);
}

/* How many logical bitstreams in this physical bitstream? */
i64 ov_streams(OggVorbis_File *vf){
  return vf->links;
}

/* Is the FILE * associated with vf seekable? */
i64 ov_seekable(OggVorbis_File *vf){
  return vf->seekable;
}
...
i64 ov_bitrate(OggVorbis_File *vf,i32 i){
...
*/
i64 ov_bitrate_instant(OggVorbis_File *vf){
  i32 link=(vf->seekable?vf->current_link:0);
  i64 ret;
  if(vf->ready_state<OPENED)return(OV_EINVAL);
...
/* Guess */
i64 ov_serialnumber(OggVorbis_File *vf,i32 i){
...
*/
ogg_int64_t ov_raw_total(OggVorbis_File *vf,i32 i){
  if(vf->ready_state<OPENED)return(OV_EINVAL);
...
*/
ogg_int64_t ov_pcm_total(OggVorbis_File *vf,i32 i){
  if(vf->ready_state<OPENED)return(OV_EINVAL);
  if(!vf->seekable || i>=vf->links)return(OV_EINVAL);
  if(i<0){
    ogg_int64_t acc=0;
...
/* returns: total seconds of content if i==-1
            ...
            length) or only partially open
*/
f64 ov_time_total(OggVorbis_File *vf,i32 i){
  if(vf->ready_state<OPENED)return(OV_EINVAL);
  if(!vf->seekable || i>=vf->links)return(OV_EINVAL);
  if(i<0){
    f64 acc=0;
    ...
    return((f64)(vf->pcmlengths[i*2+1])/vf->vi[i].rate);
  }
}
...
i32 ov_raw_seek(OggVorbis_File *vf,ogg_int64_t pos){
...

  {
    ...
    while(1){
      if(vf->ready_state>=STREAMSET){
        ...
        i32 result=ogg_stream_packetout(&work_os,&op);
...
i32 ov_pcm_seek_page(OggVorbis_File *vf,ogg_int64_t pos){
  i32 link=-1;
...

  {
    ...
    while(begin<end){
      ...
      if(end-begin<CHUNKSIZE){
        ...
        bisect=begin +
          (ogg_int64_t)((f64)(target-begintime)*(end-begin)/(endtime-begintime))
          - CHUNKSIZE;
...
i32 ov_pcm_seek(OggVorbis_File *vf,ogg_int64_t pos){
  i32 thisblock,lastblock=0;
  i32 ret=ov_pcm_seek_page(vf,pos);
  if(ret<0)return(ret);
  ...
  while(1){
    ...
    i32 ret=ogg_stream_packetpeek(&vf->os,&op);
    if(ret>0){
      thisblock=vorbis_packet_blocksize(vf->vi+vf->current_link,&op);
      if(thisblock<0){
        ogg_stream_packetout(&vf->os,NULL);
     ...
     logical bitstream boundary with abandon is OK. */
  {
    ...
    while(vf->pcm_offset<((pos>>hs)<<hs)){
      ...
      i64 samples=vorbis_synthesis_pcmout(&vf->vd,NULL);
...
/* seek to a playback time relative to the decompressed pcm stream
   returns zero on success, nonzero on failure */
i32 ov_time_seek(OggVorbis_File *vf,f64 seconds){
  ...
  for(link=0;link<vf->links;link++){
    f64 addsec = ov_time_total(vf,link);
...
/* page-granularity version of ov_time_seek
   returns zero on success, nonzero on failure */
i32 ov_time_seek_page(OggVorbis_File *vf,f64 seconds){
  /* translate time to PCM position and call ov_pcm_seek */
...
f64 ov_time_tell(OggVorbis_File *vf){
  i32 link=0;
  ogg_int64_t pcm_total=0;
  f64 time_total=0.f;
  ...
  return((f64)time_total+(f64)(vf->pcm_offset-pcm_total)/vf->vi[link].rate);
}
...
vorbis_info *ov_info(OggVorbis_File *vf,i32 link){
  if(vf->seekable){
    if(link<0)
      if(vf->ready_state>=STREAMSET)
...
vorbis_comment *ov_comment(OggVorbis_File *vf,i32 link){
  if(vf->seekable){
    if(link<0)
      if(vf->ready_state>=STREAMSET)
...
static i32 host_is_big_endian() {
  ogg_int32_t pattern = 0xfeedface; /* deadbeef */
  u8 *bytewise = (u8 *)&pattern;
...
i64 ov_read_filter(OggVorbis_File *vf,t8 *buffer,i32 length,
  ...
  i32 i,j;
  i32 host_endian = host_is_big_endian();
  i32 hs;
  ...
  while(1){
    ...
    {
      i32 ret=_fetch_and_process_packet(vf,NULL,1,1);
      if(ret==OV_EOF)
        return(0);
      if(ret<=0)
        return(ret);
  ...
  if(samples>0){
    ...
    i64 channels=ov_info(vf,-1)->channels;
    i64 bytespersample=word * channels;
...
i64 ov_read(OggVorbis_File *vf,t8 *buffer,i32 length,
             i32 bigendianp,i32 word,i32 sgned,i32 *bitstream){
  return ov_read_filter(vf, buffer, length, bigendianp, word, sgned, bitstream, NULL, NULL);
}

/* input values: pcm_channels) a f32 vector per channel of output
...
i64 ov_read_float(OggVorbis_File *vf,f32 ***pcm_channels,i32 length,
                   i32 *bitstream){
  ...
  while(1){
    if(vf->ready_state==INITSET){
      f32 **pcm;
      i64 samples=vorbis_synthesis_pcmout(&vf->vd,&pcm);
      if(samples){
        i32 hs=vorbis_synthesis_halfrate_p(vf->vi);
...
extern const f32 *vorbis_window(vorbis_dsp_state *v,i32 W);

static z0 _ov_splice(f32 **pcm,f32 **lappcm,
                       i32 n1, i32 n2,
                       i32 ch1, i32 ch2,
...
static i32 _ov_initset(OggVorbis_File *vf){
  while(1){
    if(vf->ready_state==INITSET)break;
    /* suck in another packet */
    {
      i32 ret=_fetch_and_process_packet(vf,NULL,1,0);
...
static i32 _ov_initprime(OggVorbis_File *vf){
...
static z0 _ov_getlap(OggVorbis_File *vf,vorbis_info *vi,vorbis_dsp_state *vd,
                       f32 **lappcm,i32 lapsize){
  i32 lapcount=0,i;
  f32 **pcm;

  /* try first to decode the lapping data */
  while(lapcount<lapsize){
    i32 samples=vorbis_synthesis_pcmout(vd,&pcm);
    if(samples){
    ...
    /* suck in another packet */
      i32 ret=_fetch_and_process_packet(vf,NULL,1,0); /* do *not* span */
      if(ret==OV_EOF)break;
    }
  }
  if(lapcount<lapsize){
    ...
    i32 samples=vorbis_synthesis_lapout(&vf->vd,&pcm);
    if(samples==0){
      for(i=0;i<vi->channels;i++)
...
i32 ov_crosslap(OggVorbis_File *vf1, OggVorbis_File *vf2){
  vorbis_info *vi1,*vi2;
  f32 **lappcm;
  f32 **pcm;
  const f32 *w1,*w2;
  i32 n1,n2,i,ret,hs1,hs2;
  ...
  lappcm=(f32**) alloca(sizeof(*lappcm)*vi1->channels);
  ...
  for(i=0;i<vi1->channels;i++)
    lappcm[i]=(f32*) alloca(sizeof(**lappcm)*n1);
...
static i32 _ov_64_seek_lap(OggVorbis_File *vf,ogg_int64_t pos,
                           i32 (*localseek)(OggVorbis_File *,ogg_int64_t)){
  vorbis_info *vi;
  f32 **lappcm;
  f32 **pcm;
  const f32 *w1,*w2;
  i32 n1,n2,ch1,ch2,hs;
  i32 i,ret;
  ...
  lappcm=(f32**) alloca(sizeof(*lappcm)*ch1);
  for(i=0;i<ch1;i++)
    lappcm[i]=(f32*) alloca(sizeof(**lappcm)*n1);
...
i32 ov_raw_seek_lap(OggVorbis_File *vf,ogg_int64_t pos){
  return _ov_64_seek_lap(vf,pos,ov_raw_seek);
}

i32 ov_pcm_seek_lap(OggVorbis_File *vf,ogg_int64_t pos){
  return _ov_64_seek_lap(vf,pos,ov_pcm_seek);
}

i32 ov_pcm_seek_page_lap(OggVorbis_File *vf,ogg_int64_t pos){
  return _ov_64_seek_lap(vf,pos,ov_pcm_seek_page);
}

static i32 _ov_d_seek_lap(OggVorbis_File *vf,f64 pos,
                           i32 (*localseek)(OggVorbis_File *,f64)){
  vorbis_info *vi;
  f32 **lappcm;
  f32 **pcm;
  const f32 *w1,*w2;
...
i32 ov_time_seek_lap(OggVorbis_File *vf,f64 pos){
  return _ov_d_seek_lap(vf,pos,ov_time_seek);
}

i32 ov_time_seek_page_lap(OggVorbis_File *vf,f64 pos){
