...
static z0 mapping0_free_info(vorbis_info_mapping *i){
...
static z0 mapping0_pack(vorbis_info *vi,vorbis_info_mapping *vm,
                          oggpack_buffer *opb){
  i32 i;
...
static vorbis_info_mapping *mapping0_unpack(vorbis_info *vi,oggpack_buffer *opb){
  ...
  if(b){
    ...
    for(i=0;i<info->coupling_steps;i++){
      ...
      i32 testM=info->coupling_mag[i]=
        oggpack_read(opb,ov_ilog(vi->channels-1));
      i32 testA=info->coupling_ang[i]=
        oggpack_read(opb,ov_ilog(vi->channels-1));
...
static i32 mapping0_forward(vorbis_block *vb){
  ...
  i32    *nonzero    = (i32*)alloca(sizeof(*nonzero)*vi->channels);
  f32  **gmdct     = (f32**)_vorbis_block_alloc(vb,vi->channels*sizeof(*gmdct));
  i32    **iwork      = (i32**)_vorbis_block_alloc(vb,vi->channels*sizeof(*iwork));
  i32 ***floor_posts = (i32***)_vorbis_block_alloc(vb,vi->channels*sizeof(*floor_posts));

  f32 global_ampmax=vbi->ampmax;
  f32 *local_ampmax=(f32*)alloca(sizeof(*local_ampmax)*vi->channels);
  i32 blocktype=vbi->blocktype;
  ...
  for(i=0;i<vi->channels;i++){
    ...
    iwork[i]=(i32*)_vorbis_block_alloc(vb,n/2*sizeof(**iwork));
    gmdct[i]=(f32*)_vorbis_block_alloc(vb,n/2*sizeof(**gmdct));
    ...
    for(j=1;j<n-1;j+=2){
      f32 temp=pcm[j]*pcm[j]+pcm[j+1]*pcm[j+1];
  ...
  {
    f32   *noise        = (f32*)_vorbis_block_alloc(vb,n/2*sizeof(*noise));
    f32   *tone         = (f32*)_vorbis_block_alloc(vb,n/2*sizeof(*tone));

    for(i=0;i<vi->channels;i++){
      /* the encoder setup assumes that all the modes used by any
         specific bitrate tweaking use the same floor */

      i32 submap=info->chmuxlist[i];

      /* the following makes things clearer to *me* anyway */
      f32 *mdct    =gmdct[i];
      f32 *logfft  =vb->pcm[i];

      f32 *logmdct =logfft+n/2;
      f32 *logmask =logfft;

      vb->mode=modenumber;

      floor_posts[i]=(i32**)_vorbis_block_alloc(vb,PACKETBLOBS*sizeof(**floor_posts));
  ...
  */
  ...
  {
    i32 **couple_bundle=(i32**)alloca(sizeof(*couple_bundle)*vi->channels);
    i32 *zerobundle=(i32*)alloca(sizeof(*zerobundle)*vi->channels);
    ...
    for(k=(vorbis_bitrate_managed(vb)?0:PACKETBLOBS/2);
      ...
      for(i=0;i<info->submaps;i++){
        i32 ch_in_bundle=0;
        i64 **classifications;
        i32 resnum=info->residuesubmap[i];
...
static i32 mapping0_inverse(vorbis_block *vb,vorbis_info_mapping *l){
  vorbis_dsp_state     *vd=vb->vd;
  vorbis_info          *vi=vd->vi;
  codec_setup_info     *ci=(codec_setup_info*)vi->codec_setup;
  ...
  i32                   i,j;
  i64                  n=vb->pcmend=ci->blocksizes[vb->W];

  f32 **pcmbundle=(f32**) alloca(sizeof(*pcmbundle)*vi->channels);
  i32    *zerobundle=(i32*) alloca(sizeof(*zerobundle)*vi->channels);

  i32   *nonzero  =(i32*) alloca(sizeof(*nonzero)*vi->channels);
  uk *floormemo=(uk*) alloca(sizeof(*floormemo)*vi->channels);
  ...
  for(i=info->coupling_steps-1;i>=0;i--){
    f32 *pcmM=vb->pcm[info->coupling_mag[i]];
    f32 *pcmA=vb->pcm[info->coupling_ang[i]];
