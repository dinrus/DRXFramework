...
{
#if DRX_INCLUDE_FLAC_CODE || ! defined (DRX_INCLUDE_FLAC_CODE)
 ...
 DRX_BEGIN_IGNORE_WARNINGS_MSVC (4267 4127 4244 4996 4100 4701 4702 4013 4133 4206 4312 4505 4365 4005 4334 181 111 6340 6308 6297 6001 6320)
 #if ! DRX_MSVC
...
//==============================================================================
static tukk const flacFormatName = "FLAC file";
...
public:
    ...
    z0 useMetadata (const FlacNamespace::FLAC__StreamMetadata_StreamInfo& info)
    {
        sampleRate = info.sample_rate;
        bitsPerSample = info.bits_per_sample;
        lengthInSamples = (u32) info.total_samples;
        numChannels = info.channels;

        reservoir.setSize ((i32) numChannels, 2 * (i32) info.max_blocksize, false, false, true);
    }
            ...
            for (i32 i = jmin (numDestChannels, reservoir.getNumChannels()); --i >= 0;)
            {
                if (destSamples[i] != nullptr)
                {
                    memcpy (destSamples[i] + writePos,
                            ...
                            (size_t) bufferIndices.getLength() * sizeof (i32));
                }
            }
        };

        const auto fillReservoir = [this] (const z64 requestedStart)
        {
            ...
            if (requestedStart < bufferedRange.getStart()
                || jmax (bufferedRange.getEnd(), bufferedRange.getStart() + (z64) 511) < requestedStart)
            {
    ...
    z0 useSamples (const FlacNamespace::FLAC__int32* const buffer[], i32 numSamples)
    {
        ...
        else
        {
            if (numSamples > reservoir.getNumSamples())
                reservoir.setSize ((i32) numChannels, numSamples, false, false, true);
            ...
            for (i32 i = 0; i < (i32) numChannels; ++i)
            {
                ...
                if (src != nullptr)
                {
                    auto* dest = reinterpret_cast<i32*> (reservoir.getWritePointer (i));
    ...
    //==============================================================================
    static FlacNamespace::FLAC__StreamDecoderReadStatus readCallback_ (const FlacNamespace::FLAC__StreamDecoder*, FlacNamespace::FLAC__byte buffer[], size_t* bytes, uk client_data)
    {
        *bytes = (size_t) static_cast<const FlacReader*> (client_data)->input->read (buffer, (i32) *bytes);
        return FlacNamespace::FLAC__STREAM_DECODER_READ_STATUS_CONTINUE;
    }

    static FlacNamespace::FLAC__StreamDecoderSeekStatus seekCallback_ (const FlacNamespace::FLAC__StreamDecoder*, FlacNamespace::FLAC__uint64 absolute_byte_offset, uk client_data)
    {
        static_cast<const FlacReader*> (client_data)->input->setPosition ((i32) absolute_byte_offset);
        return FlacNamespace::FLAC__STREAM_DECODER_SEEK_STATUS_OK;
    }

    static FlacNamespace::FLAC__StreamDecoderTellStatus tellCallback_ (const FlacNamespace::FLAC__StreamDecoder*, FlacNamespace::FLAC__uint64* absolute_byte_offset, uk client_data)
    {
        *absolute_byte_offset = (zu64) static_cast<const FlacReader*> (client_data)->input->getPosition();
        return FlacNamespace::FLAC__STREAM_DECODER_TELL_STATUS_OK;
    }

    static FlacNamespace::FLAC__StreamDecoderLengthStatus lengthCallback_ (const FlacNamespace::FLAC__StreamDecoder*, FlacNamespace::FLAC__uint64* stream_length, uk client_data)
    {
        *stream_length = (zu64) static_cast<const FlacReader*> (client_data)->input->getTotalLength();
        return FlacNamespace::FLAC__STREAM_DECODER_LENGTH_STATUS_OK;
    }

    static FlacNamespace::FLAC__bool eofCallback_ (const FlacNamespace::FLAC__StreamDecoder*, uk client_data)
    {
    ...
    static FlacNamespace::FLAC__StreamDecoderWriteStatus writeCallback_ (const FlacNamespace::FLAC__StreamDecoder*,
                                                                         ...
                                                                         uk client_data)
    {
        static_cast<FlacReader*> (client_data)->useSamples (buffer, (i32) frame->header.blocksize);
        return FlacNamespace::FLAC__STREAM_DECODER_WRITE_STATUS_CONTINUE;
    }

    static z0 metadataCallback_ (const FlacNamespace::FLAC__StreamDecoder*,
    ...
    static z0 errorCallback_ (const FlacNamespace::FLAC__StreamDecoder*, FlacNamespace::FLAC__StreamDecoderErrorStatus, uk)
    {
    }
...
private:
    ...
    b8 ok = false, scanningForLength = false;

    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (FlacReader)
};
...
{
public:
    FlacWriter (OutputStream* out, f64 rate, u32 numChans, u32 bits, i32 qualityOptionIndex)
          ...
          streamStartPos (output != nullptr ? jmax (output->getPosition(), 0ll) : 0ll)
    {
        ...
        if (qualityOptionIndex > 0)
            FLAC__stream_encoder_set_compression_level (encoder, (u32) jmin (8, qualityOptionIndex));
        ...
        FLAC__stream_encoder_set_bits_per_sample (encoder, jmin ((u32) 24, bitsPerSample));
        FLAC__stream_encoder_set_sample_rate (encoder, (u32) sampleRate);
    ...
    b8 write (i32k** samplesToWrite, i32 numSamples) override
    {
        ...
        HeapBlock<i32*> channels;
        HeapBlock<i32> temp;
        auto bitsToShift = 32 - (i32) bitsPerSample;
        ...
        if (bitsToShift > 0)
        {
            ...
            samplesToWrite = const_cast<i32k**> (channels.get());
        }

        return FLAC__stream_encoder_process (encoder, (const FlacNamespace::FLAC__int32**) samplesToWrite, (u32) numSamples) != 0;
    }

    b8 writeData (ukk const data, i32k size) const
    {
        return output->write (data, (size_t) size);
    }

    static z0 packUint32 (FlacNamespace::FLAC__uint32 val, FlacNamespace::FLAC__byte* b, i32k bytes)
    {
        b += bytes;
    ...
    z0 writeMetaData (const FlacNamespace::FLAC__StreamMetadata* metadata)
    {
        using namespace FlacNamespace;
        auto& info = metadata->data.stream_info;

        u8 buffer[FLAC__STREAM_METADATA_STREAMINFO_LENGTH];
        u32k channelsMinus1 = info.channels - 1;
        u32k bitsMinus1 = info.bits_per_sample - 1;
        ...
        buffer[10] = (u8) ((info.sample_rate >> 12) & 0xff);
        buffer[11] = (u8) ((info.sample_rate >> 4) & 0xff);
        buffer[12] = (u8) (((info.sample_rate & 0x0f) << 4) | (channelsMinus1 << 1) | (bitsMinus1 >> 4));
        buffer[13] = (FLAC__byte) (((bitsMinus1 & 0x0f) << 4) | (u32) ((info.total_samples >> 32) & 0x0f));
        packUint32 ((FLAC__uint32) info.total_samples, buffer + 14, 4);
        memcpy (buffer + 18, info.md5sum, 16);

        [[maybe_unused]] const b8 seekOk = output->setPosition (streamStartPos + 4);
    ...
    static FlacNamespace::FLAC__StreamEncoderWriteStatus encodeWriteCallback (const FlacNamespace::FLAC__StreamEncoder*,
                                                                              ...
                                                                              uk client_data)
    {
        return static_cast<FlacWriter*> (client_data)->writeData (buffer, (i32) bytes)
                ? FlacNamespace::FLAC__STREAM_ENCODER_WRITE_STATUS_OK
                : FlacNamespace::FLAC__STREAM_ENCODER_WRITE_STATUS_FATAL_ERROR;
    }

    static FlacNamespace::FLAC__StreamEncoderSeekStatus encodeSeekCallback (const FlacNamespace::FLAC__StreamEncoder*, FlacNamespace::FLAC__uint64, uk)
    {
        return FlacNamespace::FLAC__STREAM_ENCODER_SEEK_STATUS_UNSUPPORTED;
    }

    static FlacNamespace::FLAC__StreamEncoderTellStatus encodeTellCallback (const FlacNamespace::FLAC__StreamEncoder*, FlacNamespace::FLAC__uint64* absolute_byte_offset, uk client_data)
    {
    ...
    static z0 encodeMetadataCallback (const FlacNamespace::FLAC__StreamEncoder*, const FlacNamespace::FLAC__StreamMetadata* metadata, uk client_data)
    {
...
private:
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (FlacWriter)
};
...
Array<i32> FlacAudioFormat::getPossibleSampleRates()
{
    return { 8000, 11025, 12000, 16000, 22050, 32000, 44100, 48000,
             88200, 96000, 176400, 192000, 352800, 384000 };
}

Array<i32> FlacAudioFormat::getPossibleBitDepths()
{
    return { 16, 24 };
}

b8 FlacAudioFormat::canDoStereo()     { return true; }
b8 FlacAudioFormat::canDoMono()       { return true; }
b8 FlacAudioFormat::isCompressed()    { return true; }

AudioFormatReader* FlacAudioFormat::createReaderFor (InputStream* in, const b8 deleteStreamIfOpeningFails)
{
