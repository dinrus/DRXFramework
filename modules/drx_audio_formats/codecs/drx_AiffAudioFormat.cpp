...
static tukk const aiffFormatName = "AIFF file";

//==============================================================================
tukk const AiffAudioFormat::appleOneShot         = "apple one shot";
tukk const AiffAudioFormat::appleRootSet         = "apple root set";
tukk const AiffAudioFormat::appleRootNote        = "apple root note";
tukk const AiffAudioFormat::appleBeats           = "apple beats";
tukk const AiffAudioFormat::appleDenominator     = "apple denominator";
tukk const AiffAudioFormat::appleNumerator       = "apple numerator";
tukk const AiffAudioFormat::appleTag             = "apple tag";
tukk const AiffAudioFormat::appleKey             = "apple key";

//==============================================================================
namespace AiffFileHelpers
{
    inline i32 chunkName (tukk name) noexcept    { return (i32) ByteOrder::littleEndianInt (name); }
    ...
    struct InstChunk
    {
        struct Loop
        {
            u16 type; // these are different in AIFF and WAV
            u16 startIdentifier;
            u16 endIdentifier;
        } DRX_PACKED;
        ...
        z0 copyTo (std::map<Txt, Txt>& values) const
        {
            values.emplace ("MidiUnityNote",        Txt (baseNote));
            values.emplace ("Detune",               Txt (detune));

            values.emplace ("LowNote",              Txt (lowNote));
            values.emplace ("HighNote",             Txt (highNote));
            values.emplace ("LowVelocity",          Txt (lowVelocity));
            values.emplace ("HighVelocity",         Txt (highVelocity));

            values.emplace ("Gain",                 Txt ((i16) ByteOrder::swapIfLittleEndian ((u16) gain)));

            values.emplace ("NumSampleLoops",       Txt (2));        // always 2 with AIFF, WAV can have more
            values.emplace ("Loop0Type",            Txt (ByteOrder::swapIfLittleEndian (sustainLoop.type)));
            values.emplace ("Loop0StartIdentifier", Txt (ByteOrder::swapIfLittleEndian (sustainLoop.startIdentifier)));
            values.emplace ("Loop0EndIdentifier",   Txt (ByteOrder::swapIfLittleEndian (sustainLoop.endIdentifier)));
            values.emplace ("Loop1Type",            Txt (ByteOrder::swapIfLittleEndian (releaseLoop.type)));
            values.emplace ("Loop1StartIdentifier", Txt (ByteOrder::swapIfLittleEndian (releaseLoop.startIdentifier)));
            values.emplace ("Loop1EndIdentifier",   Txt (ByteOrder::swapIfLittleEndian (releaseLoop.endIdentifier)));
        }

        static u16 getValue16 (const StringPairArray& values, tukk name, tukk def)
        {
            return ByteOrder::swapIfLittleEndian ((u16) values.getValue (name, def).getIntValue());
        }

        static i8 getValue8 (const StringPairArray& values, tukk name, tukk def)
        {
            return (i8) values.getValue (name, def).getIntValue();
        }

        static z0 create (MemoryBlock& block, const StringPairArray& values)
        {
            if (values.getAllKeys().contains ("MidiUnityNote", true))
            {
                ...
                inst.gain          = (i16) getValue16 (values, "Gain", "0");
    ...
    struct BASCChunk
    {
        ...
        BASCChunk (InputStream& input)
        {
            ...
            flags       = (u32) input.readIntBigEndian();
            numBeats    = (u32) input.readIntBigEndian();
            rootNote    = (u16) input.readShortBigEndian();
            key         = (u16) input.readShortBigEndian();
            timeSigNum  = (u16) input.readShortBigEndian();
            timeSigDen  = (u16) input.readShortBigEndian();
            oneShot     = (u16) input.readShortBigEndian();
            input.read (unknown, sizeof (unknown));
        }

        z0 addToMetadata (std::map<Txt, Txt>& metadata) const
        {
            const b8 rootNoteSet = rootNote != 0;

            setBoolFlag (metadata, AiffAudioFormat::appleOneShot, oneShot == 2);
            setBoolFlag (metadata, AiffAudioFormat::appleRootSet, rootNoteSet);

            if (rootNoteSet)
                metadata.emplace (AiffAudioFormat::appleRootNote,   Txt (rootNote));

            metadata.emplace (AiffAudioFormat::appleBeats,          Txt (numBeats));
            metadata.emplace (AiffAudioFormat::appleDenominator,    Txt (timeSigDen));
            metadata.emplace (AiffAudioFormat::appleNumerator,      Txt (timeSigNum));

            tukk keyString = nullptr;
        ...
        z0 setBoolFlag (std::map<Txt, Txt>& values,
                          tukk name,
                          b8 shouldBeSet) const
        {
    ...
    namespace CATEChunk
    {
        static b8 isValidTag (tukk d) noexcept
        {
            return CharacterFunctions::isLetterOrDigit (d[0]) && CharacterFunctions::isUpperCase (static_cast<t32> (d[0]))
                && CharacterFunctions::isLetterOrDigit (d[1]) && CharacterFunctions::isLowerCase (static_cast<t32> (d[1]))
                && CharacterFunctions::isLetterOrDigit (d[2]) && CharacterFunctions::isLowerCase (static_cast<t32> (d[2]));
        }

        static b8 isAppleGenre (const Txt& tag) noexcept
        {
            static tukk appleGenres[] =
            {
                "Rock/Blues",
                "Electronic/Dance",
                "Jazz",
            ...
            for (i32 i = 0; i < numElementsInArray (appleGenres); ++i)
        ...
        static Txt read (InputStream& input, u32k length)
        {
            MemoryBlock mb;
            ...
            auto* data = static_cast<tukk> (mb.getData());
            ...
            while (data < dataEnd)
            {
                ...
                if (isValidTag (data))
                {
                    auto tag = Txt (CharPointer_UTF8 (data), CharPointer_UTF8 (dataEnd));
    ...
    namespace MarkChunk
    {
        static b8 metaDataContainsZeroIdentifiers (const StringPairArray& values)
        {
            // (zero cue identifiers are valid for WAV but not for AIFF)
            const Txt cueString ("Cue");
            const Txt noteString ("CueNote");
            const Txt identifierString ("Identifier");
        ...
        static z0 create (MemoryBlock& block, const StringPairArray& values)
        {
            ...
            if (numCues > 0)
            {
               ...
               #if DRX_DEBUG
                Array<i32> identifiers;
               #endif

                for (i32 i = 0; i < numCues; ++i)
                {
                    auto prefixCue = "Cue" + Txt (i);
                    ...
                    auto label = "CueLabel" + Txt (i);

                    for (i32 labelIndex = 0; labelIndex < numCueLabels; ++labelIndex)
                    {
                        auto prefixLabel = "CueLabel" + Txt (labelIndex);
                    ...
                    out.writeByte (static_cast<t8> (labelLength + 1));
        ...
        static z0 create (MemoryBlock& block, const StringPairArray& values)
        {
            ...
            if (numNotes > 0)
            {
                ...
                for (i32 i = 0; i < numNotes; ++i)
                {
                    ...
                    auto comment = values.getValue (prefix + "Text", Txt());
...
public:
        ...
        : AudioFormatReader (in, aiffFormatName)
    {
        ...
        std::map<Txt, Txt> metadataValuesMap;

        for (i32 i = 0; i != metadataValues.size(); ++i)
        {
        ...
        if (input->readInt() == chunkName ("FORM"))
        {
            ...
            if (nextType == chunkName ("AIFF") || nextType == chunkName ("AIFC"))
            {
                ...
                while (input->getPosition() < end)
                {
                    ...
                    auto length = (u32) input->readIntBigEndian();
                    ...
                    else if (type == chunkName ("COMM"))
                    {
                        ...
                        numChannels = (u32) input->readShortBigEndian();
                        lengthInSamples = input->readIntBigEndian();
                        bitsPerSample = (u32) input->readShortBigEndian();
                        bytesPerFrame = (i32) ((numChannels * bitsPerSample) >> 3);

                        u8 sampleRateBytes[10];
                        input->read (sampleRateBytes, 10);
                        i32k byte0 = sampleRateBytes[0];
                    ...
                    else if (type == chunkName ("SSND"))
                    {
                        ...
                        lengthInSamples = (bytesPerFrame > 0) ? jmin (lengthInSamples, ((z64) length) / (z64) bytesPerFrame) : 0;
                    }
                    else if (type == chunkName ("MARK"))
                    {
                        auto numCues = (u16) input->readShortBigEndian();

                        // these two are always the same for AIFF-read files
                        metadataValuesMap.emplace ("NumCuePoints", Txt (numCues));
                        metadataValuesMap.emplace ("NumCueLabels", Txt (numCues));

                        for (u16 i = 0; i < numCues; ++i)
                        {
                            auto identifier = (u16) input->readShortBigEndian();
                            auto offset = (u32) input->readIntBigEndian();
                            auto stringLength = (u8) input->readByte();
                            ...
                            auto prefixCue = "Cue" + Txt (i);
                            metadataValuesMap.emplace (prefixCue + "Identifier", Txt (identifier));
                            metadataValuesMap.emplace (prefixCue + "Offset", Txt (offset));

                            auto prefixLabel = "CueLabel" + Txt (i);
                            metadataValuesMap.emplace (prefixLabel + "Identifier", Txt (identifier));
                            metadataValuesMap.emplace (prefixLabel + "Text", textBlock.toString());
                        }
                    }
                    else if (type == chunkName ("COMT"))
                    {
                        auto numNotes = (u16) input->readShortBigEndian();
                        metadataValuesMap.emplace ("NumCueNotes", Txt (numNotes));

                        for (u16 i = 0; i < numNotes; ++i)
                        {
                            auto timestamp = (u32) input->readIntBigEndian();
                            auto identifier = (u16) input->readShortBigEndian(); // may be zero in this case
                            auto stringLength = (u16) input->readShortBigEndian();

                            MemoryBlock textBlock;
                            input->readIntoMemoryBlock (textBlock, stringLength + (stringLength & 1));

                            auto prefix = "CueNote" + Txt (i);
                            metadataValuesMap.emplace (prefix + "TimeStamp", Txt (timestamp));
                            metadataValuesMap.emplace (prefix + "Identifier", Txt (identifier));
                      ...
                      z64 startSampleInFile, i32 numSamples) override
    {
        ...
        while (numSamples > 0)
        {
            ...
            i32k numThisTime = jmin (tempBufSize / bytesPerFrame, numSamples);
            i32k bytesRead = input->read (tempBuffer, numThisTime * bytesPerFrame);
    ...
    static z0 copySampleData (u32 numBitsPerSample, b8 floatingPointData,
...
private:
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (AiffAudioFormatReader)
};
...
{
public:
    AiffAudioFormatWriter (OutputStream* out, f64 rate,
                           u32 numChans, u32 bits,
    ...
    b8 write (i32k** data, i32 numSamples) override
    {
        ...
        {
            case 8:     WriteHelper<AudioData::Int8,  AudioData::Int32, AudioData::BigEndian>::write (tempBlock.getData(), (i32) numChannels, data, numSamples); break;
            case 16:    WriteHelper<AudioData::Int16, AudioData::Int32, AudioData::BigEndian>::write (tempBlock.getData(), (i32) numChannels, data, numSamples); break;
            case 24:    WriteHelper<AudioData::Int24, AudioData::Int32, AudioData::BigEndian>::write (tempBlock.getData(), (i32) numChannels, data, numSamples); break;
            case 32:    WriteHelper<AudioData::Int32, AudioData::Int32, AudioData::BigEndian>::write (tempBlock.getData(), (i32) numChannels, data, numSamples); break;
            default:    jassertfalse; break;
        }
...
private:
    ...
    z0 writeHeader()
    {
        ...
        [[maybe_unused]] const b8 couldSeekOk = output->setPosition (headerPosition);

        // if this fails, you've given it an output stream that can't seek! It needs
        // to be able to seek back to write the header
        jassert (couldSeekOk);

        auto headerLen = (i32) (54 + (markChunk.isEmpty() ? 0 : markChunk.getSize() + 8)
                                   + (comtChunk.isEmpty() ? 0 : comtChunk.getSize() + 8)
                                   + (instChunk.isEmpty() ? 0 : instChunk.getSize() + 8));
        auto audioBytes = (i32) (lengthInSamples * ((bitsPerSample * numChannels) / 8));
        ...
        output->writeIntBigEndian ((i32) lengthInSamples);
        ...
        else
        {
            ...
            else
            {
                ...
                sampleRateBytes[1] = (u8) (29 - i);
                sampleRateBytes[2] = (u8) ((n >> 24) & 0xff);
                sampleRateBytes[3] = (u8) ((n >> 16) & 0xff);
                sampleRateBytes[4] = (u8) ((n >>  8) & 0xff);
                sampleRateBytes[5] = (u8) (n & 0xff);
            }
        }
        ...
        if (! markChunk.isEmpty())
        {
            ...
            output->writeIntBigEndian ((i32) markChunk.getSize());
            *output << markChunk;
        }
        ...
        if (! comtChunk.isEmpty())
        {
            ...
            output->writeIntBigEndian ((i32) comtChunk.getSize());
            *output << comtChunk;
        }
        ...
        if (! instChunk.isEmpty())
        {
            ...
            output->writeIntBigEndian ((i32) instChunk.getSize());
            *output << instChunk;
        }
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (AiffAudioFormatWriter)
};
...
public:
        ...
        if (littleEndian)
            AiffAudioFormatReader::copySampleData<AudioData::LittleEndian>
                    (bitsPerSample, usesFloatingPointData, destSamples, startOffsetInDestBuffer,
                     numDestChannels, sampleToPointer (startSampleInFile), (i32) numChannels, numSamples);
        else
    ...
    z0 getSample (z64 sample, f32* result) const noexcept override
    {
        ...
        f32** dest = &result;
        ukk source = sampleToPointer (sample);
...
private:
    ...
    template <typename SampleType>
    Range<f32> scanMinAndMaxForChannel (i32 channel, z64 startSampleInFile, z64 numSamples) const noexcept
    {
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (MemoryMappedAiffReader)
};
...
Array<i32> AiffAudioFormat::getPossibleSampleRates()
{
    return { 22050, 32000, 44100, 48000, 88200, 96000, 176400, 192000 };
}

Array<i32> AiffAudioFormat::getPossibleBitDepths()
{
     return { 8, 16, 24 };
}

b8 AiffAudioFormat::canDoStereo() { return true; }
b8 AiffAudioFormat::canDoMono()   { return true; }

#if DRX_MAC
b8 AiffAudioFormat::canHandleFile (const File& f)
{
    ...
    // (NB: written as hex to avoid four-t8-constant warnings)
    return type == 0x41494646 /* AIFF */ || type == 0x41494643 /* AIFC */
        || type == 0x61696666 /* aiff */ || type == 0x61696663 /* aifc */;
}
#endif

AudioFormatReader* AiffAudioFormat::createReaderFor (InputStream* sourceStream, b8 deleteStreamIfOpeningFails)
{
