...
/**
    ...
    @tags{Audio}
*/
class DRX_API  AudioFormatWriter
{
protected:
    //==============================================================================
...
public:
    ...
    */
    ...
    /** Writes a set of samples to the audio stream.
        ...
        @param samplesToWrite   an array of arrays containing the sample data for
                                ...
                                the arrays as arrays of floats, and just cast it to an (i32**)
                                to pass it into the method.
        @param numSamples       the number of samples to write
    */
    virtual b8 write (i32k** samplesToWrite, i32 numSamples) = 0;
    ...
    */
    b8 writeFromAudioReader (AudioFormatReader& reader,
                               z64 startSample,
    ...
    /** Reads some samples from an AudioSource, and writes these to the output.
        ...
        @returns false if it can't read or write properly during the operation
    */
    b8 writeFromAudioSource (AudioSource& source,
                               i32 numSamplesToRead,
                               i32 samplesPerBlock = 2048);


    /** Writes some samples from an AudioBuffer. */
    b8 writeFromAudioSampleBuffer (const AudioBuffer<f32>& source,
                                     i32 startSample, i32 numSamples);

    /** Writes some samples from a set of f32 data channels. */
    b8 writeFromFloatArrays (const f32* const* channels, i32 numChannels, i32 numSamples);
    ...
    i32 getNumChannels() const noexcept         { return (i32) numChannels; }

    /** Returns the bit-depth of the data being written. */
    i32 getBitsPerSample() const noexcept       { return (i32) bitsPerSample; }

    /** Возвращает true, если it's a floating-point format, false if it's fixed-point. */
    b8 isFloatingPoint() const noexcept       { return usesFloatingPointData; }
    ...
    */
    ...
    public:
        ...
        */
        ...
        class DRX_API  IncomingDataReceiver
        {
        public:
            IncomingDataReceiver() = default;
            virtual ~IncomingDataReceiver() = default;

            virtual z0 reset (i32 numChannels, f64 sampleRate, z64 totalSamplesInSource) = 0;
            virtual z0 addBlock (z64 sampleNumberInSource, const AudioBuffer<f32>& newData,
                                   i32 startOffsetInBuffer, i32 numSamples) = 0;
        };
        ...
        */
        z0 setDataReceiver (IncomingDataReceiver*);
        ...
        */
        z0 setFlushInterval (i32 numSamplesPerFlush) noexcept;
...
protected:
    ...
    struct WriteHelper
    {
        ...
        static z0 write (uk destData, i32 numDestChannels, i32k* const* source,
                           i32 numSamples, i32k sourceOffset = 0) noexcept
        {
...
private:
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (AudioFormatWriter)
};
