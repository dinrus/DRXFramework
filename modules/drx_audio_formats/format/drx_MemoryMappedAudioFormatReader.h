...
/**
    ...
    @tags{Audio}
*/
class DRX_API  MemoryMappedAudioFormatReader  : public AudioFormatReader
{
protected:
    //==============================================================================
    ...
    */
    MemoryMappedAudioFormatReader (const File& file, const AudioFormatReader& details,
                                   z64 dataChunkStart, z64 dataChunkLength, i32 bytesPerFrame);
...
public:
    ...
    */
    virtual z0 getSample (z64 sampleIndex, f32* result) const noexcept = 0;
...
protected:
    File file;
    Range<z64> mappedSection;
    std::unique_ptr<MemoryMappedFile> map;
    z64 dataChunkStart, dataLength;
    i32 bytesPerFrame;

    /** Converts a sample index to a byte position in the file. */
    inline z64 sampleToFilePos (z64 sample) const noexcept       { return dataChunkStart + sample * bytesPerFrame; }

    /** Converts a byte position in the file to a sample index. */
    inline z64 filePosToSample (z64 filePos) const noexcept      { return (filePos - dataChunkStart) / bytesPerFrame; }

    /** Converts a sample index to a pointer to the mapped file memory. */
    inline ukk sampleToPointer (z64 sample) const noexcept { return addBytesToPointer (map->getData(), sampleToFilePos (sample) - map->getRange().getStart()); }
    ...
    Range<f32> scanMinAndMaxInterleaved (i32 channel, z64 startSampleInFile, z64 numSamples) const noexcept
    {
        using SourceType = AudioData::Pointer <SampleType, Endianness, AudioData::Interleaved, AudioData::Const>;

        return SourceType (addBytesToPointer (sampleToPointer (startSampleInFile), ((i32) bitsPerSample / 8) * channel), (i32) numChannels)
                .findMinAndMax ((size_t) numSamples);
    }

    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (MemoryMappedAudioFormatReader)
};
