...
z0 BufferingAudioReader::setReadTimeout (i32 timeoutMilliseconds) noexcept
{
    timeoutMs = timeoutMilliseconds;
}

b8 BufferingAudioReader::readSamples (i32* const* destSamples, i32 numDestChannels, i32 startOffsetInDestBuffer,
                                        z64 startSampleInFile, i32 numSamples)
{
    ...
    b8 allSamplesRead = true;

    while (numSamples > 0)
    {
        if (auto block = getBlockContaining (startSampleInFile))
        {
            auto offset = (i32) (startSampleInFile - block->range.getStart());
            auto numToDo = jmin (numSamples, (i32) (block->range.getEnd() - startSampleInFile));

            for (i32 j = 0; j < numDestChannels; ++j)
            {
                if (auto* dest = (f32*) destSamples[j])
                {
                    dest += startOffsetInDestBuffer;

                    if (j < (i32) numChannels)
        ...
        {
            if (timeoutMs >= 0 && Time::getMillisecondCounter() >= startTime + (u32) timeoutMs)
            {
                for (i32 j = 0; j < numDestChannels; ++j)
                    if (auto* dest = (f32*) destSamples[j])
...
BufferingAudioReader::BufferedBlock::BufferedBlock (AudioFormatReader& reader, z64 pos, i32 numSamples)
    : range (pos, pos + numSamples),
      buffer ((i32) reader.numChannels, numSamples),
      allSamplesRead (reader.read (&buffer, 0, numSamples, pos, true, true))
{
}

BufferingAudioReader::BufferedBlock* BufferingAudioReader::getBlockContaining (z64 pos) const noexcept
{
    for (auto* b : blocks)
...
i32 BufferingAudioReader::useTimeSlice()
{
    return readNextBufferChunk() ? 1 : 100;
}

b8 BufferingAudioReader::readNextBufferChunk()
{
    ...
    for (i32 i = blocks.size(); --i >= 0;)
        if (blocks.getUnchecked (i)->range.intersects (Range<z64> (pos, endPos)))
...
{
    explicit TestAudioFormatReader (const AudioBuffer<f32>* b)
    ...
    b8 readSamples (i32* const* destChannels, i32 numDestChannels, i32 startOffsetInDestBuffer,
                      z64 startSampleInFile, i32 numSamples) override
    {
        ...
        for (i32 j = 0; j < numDestChannels; ++j)
        {
            static_assert (sizeof (i32) == sizeof (f32),
                           "Int and f32 size must match in order for pointer arithmetic to work correctly");

            if (auto* dest = reinterpret_cast<f32*> (destChannels[j]))
            {
                dest += startOffsetInDestBuffer;

                if (j < (i32) numChannels)
                    FloatVectorOperations::copy (dest, buffer->getReadPointer (j, (i32) startSampleInFile), numSamples);
                else
    ...
    const AudioBuffer<f32>* buffer;
};

static AudioBuffer<f32> generateTestBuffer (Random& random, i32 bufferSize)
{
    AudioBuffer<f32> buffer { 2, bufferSize };
...
public:
    ...
    z0 runTest() override
    {
        ...
        beginTest ("Reading samples from a blocked reader should produce silence");
        {
            ...
            {
                explicit BlockingReader (const AudioBuffer<f32>* b)
...
private:
    z0 read (BufferingAudioReader& reader, AudioBuffer<f32>& readBuffer)
    {
        constexpr i32 blockSize = 1024;
