...
public:
     ...
     */
    explicit LatencyTuner(AudioStream &stream, i32 maximumBufferSize);
     ...
     * @param bufferSize
     */
    z0 setMinimumBufferSize(i32 bufferSize) {
        mMinimumBufferSize = bufferSize;
    }

    i32 getMinimumBufferSize() const {
     ...
     * @param sizeIncrement
     */
    z0 setBufferSizeIncrement(i32 sizeIncrement) {
        mBufferSizeIncrement = sizeIncrement;
    }

    i32 getBufferSizeIncrement() const {
...
private:
    ...
    static constexpr i32 kIdleCount = 8;
    static constexpr i32 kDefaultNumBursts = 2;

    AudioStream           &mStream;
    State                 mState = State::Idle;
    i32               mMaxBufferSize = 0;
    i32               mPreviousXRuns = 0;
    i32               mIdleCountDown = 0;
    i32               mMinimumBufferSize;
    i32               mBufferSizeIncrement;
    std::atomic<i32>  mLatencyTriggerRequests{0}; // TODO user atomic requester from AAudio
    std::atomic<i32>  mLatencyTriggerResponses{0};
};
