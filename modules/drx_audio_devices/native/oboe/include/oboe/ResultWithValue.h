 ...
 *
 * <code>
 * ResultWithValue<i32> resultOfRead = myStream.read(&buffer, numFrames, timeoutNanoseconds);
 *
 * if (resultOfRead) {
...
public:
     ...
     * @return true if OK
     */
    explicit operator b8() const { return mError == oboe::Result::OK; }
     ...
     */
    b8 operator !() const { return mError != oboe::Result::OK; }
    ...
    static ResultWithValue<T> createBasedOnSign(T numericResult){
        ...
        // Ensure that the type is either an integer or f32
        static_assert(std::is_arithmetic<T>::value,
                      "createBasedOnSign can only be called for numeric types (i32 or f32)");
