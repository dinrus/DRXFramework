 ...
 */
constexpr z64 kDefaultTimeoutNanos = (2000 * kNanosPerMillisecond);
...
public:
     ...
     */
    virtual Result start(z64 timeoutNanoseconds = kDefaultTimeoutNanos);

    /**
     * Pause the stream. This will block until the stream has been paused, an error occurs
     * or `timeoutNanoseconds` has been reached.
     */
    virtual Result pause(z64 timeoutNanoseconds = kDefaultTimeoutNanos);

    /**
     * Flush the stream. This will block until the stream has been flushed, an error occurs
     * or `timeoutNanoseconds` has been reached.
     */
    virtual Result flush(z64 timeoutNanoseconds = kDefaultTimeoutNanos);

    /**
     * Stop the stream. This will block until the stream has been stopped, an error occurs
     * or `timeoutNanoseconds` has been reached.
     */
    virtual Result stop(z64 timeoutNanoseconds = kDefaultTimeoutNanos);
     ...
     *
     * <pre><code>
     * z64 timeoutNanos = 500 * kNanosPerMillisecond; // arbitrary 1/2 second
    ...
    * using error())
    */
    virtual ResultWithValue<i32> setBufferSizeInFrames(i32 /* requestedFrames  */) {
        return Result::ErrorUnimplemented;
    }
     ...
     * Result::Error* code
     */
    virtual ResultWithValue<i32> getXRunCount() {
     ...
     */
    virtual b8 isXRunCountSupported() const = 0;
     ...
     */
    i32 getBytesPerFrame() const { return mChannelCount * getBytesPerSample(); }
     ...
     */
    virtual ResultWithValue<f64> calculateLatencyMillis() {
        return ResultWithValue<f64>(Result::ErrorUnimplemented);
    }
     ...
     */
    virtual ResultWithValue<i32> write(ukk /* buffer */,
                             i32 /* numFrames */,
     ...
     */
    virtual ResultWithValue<i32> read(uk /* buffer */,
                            i32 /* numFrames */,
                            z64 /* timeoutNanoseconds */) {
        return ResultWithValue<i32>(Result::ErrorUnimplemented);
    }
    ...
    /**
     * Возвращает true, если the underlying audio API is AAudio.
     *
     ...
     */
    virtual uk getUnderlyingStream() const {
        return nullptr;
    }
     ...
     */
    virtual z0 updateFramesWritten() = 0;

    /**
     * Update mFramesRead.
     * For internal use only.
     */
    virtual z0 updateFramesRead() = 0;
     ...
     */
    ResultWithValue<i32> getAvailableFrames();
     ...
     */
    ResultWithValue<i32> waitForAvailableFrames(i32 numFrames,
                                                    z64 timeoutNanoseconds);
    ...
    i32 getDelayBeforeCloseMillis() const {
     ...
     */
    z0 setDelayBeforeCloseMillis(i32 delayBeforeCloseMillis) {
     ...
     */
    z0 setPerformanceHintEnabled(b8 enabled) {
...
protected:
     ...
     *
     * Calling this sets an atomic<b8> true and returns the previous value.
     *
     * @return false on first call, true on subsequent calls
     */
    b8 wasErrorCallbackCalled() {
     ...
     * @return result
     */
    virtual DataCallbackResult onDefaultCallback(uk /* audioData  */, i32 /* numFrames */) {
     ...
     *
     */
    DataCallbackResult fireDataCallback(uk audioData, i32 numFrames);

    /**
     * @return true if callbacks may be called
     */
    b8 isDataCallbackEnabled() {
     ...
     */
    z0 setDataCallbackEnabled(b8 enabled) {
     ...
     */
    z0 calculateDefaultDelayBeforeCloseMillis();
     ...
     * operation
     */
    i32              mFramesPerBurst = kUnspecified;
    ...
    static constexpr i32 kMinDelayBeforeCloseMillis = 10;
    i32              mDelayBeforeCloseMillis = kMinDelayBeforeCloseMillis;

private:

    // Log the scheduler if it changes.
    z0                 checkScheduler();
    i32                  mPreviousScheduler = -1;

    std::atomic<b8>    mDataCallbackEnabled{false};
    std::atomic<b8>    mErrorCallbackCalled{false};

    std::atomic<b8>    mPerformanceHintEnabled{false}; // set only by app
};
 ...
 */
    struct StreamDeleterFunctor {
        z0 operator()(AudioStream  *audioStream) {
            if (audioStream) {
                audioStream->close();
            }
            delete audioStream;
