...
static const t8 *getSafeDlerror() {
    static const t8 *defaultMessage = "not found?";
    t8 *errorMessage = dlerror();
...
b8 EngineOpenSLES::linkOpenSLES() {
...
SLInterfaceID EngineOpenSLES::getIidPointer(const t8 *symbolName) {
...
z0 EngineOpenSLES::close() {
    std::lock_guard<std::mutex> lock(mLock);
    close_l();
}

// This must be called under mLock
z0 EngineOpenSLES::close_l() {
    if (--mOpenCount == 0) {
