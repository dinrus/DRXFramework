...
SLuint32 AudioInputStreamOpenSLES::channelCountToChannelMask(i32 channelCount) const {
...
Result AudioInputStreamOpenSLES::open() {
    ...
    if (getSdkVersion() < __ANDROID_API_M__ && mFormat == AudioFormat::Float){
        // TODO: Allow floating point format on API <23 using f32->i16 converter
...
z0 AudioInputStreamOpenSLES::updateFramesWritten() {
    if (usingFIFO()) {
