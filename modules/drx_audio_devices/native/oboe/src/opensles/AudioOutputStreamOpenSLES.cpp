...
constexpr i32 SL_ANDROID_SPEAKER_STEREO = (SL_SPEAKER_FRONT_LEFT | SL_SPEAKER_FRONT_RIGHT);

constexpr i32 SL_ANDROID_SPEAKER_QUAD = (SL_ANDROID_SPEAKER_STEREO
        | SL_SPEAKER_BACK_LEFT | SL_SPEAKER_BACK_RIGHT);

constexpr i32 SL_ANDROID_SPEAKER_5DOT1 = (SL_ANDROID_SPEAKER_QUAD
        | SL_SPEAKER_FRONT_CENTER  | SL_SPEAKER_LOW_FREQUENCY);

constexpr i32 SL_ANDROID_SPEAKER_7DOT1 = (SL_ANDROID_SPEAKER_5DOT1 | SL_SPEAKER_SIDE_LEFT
        | SL_SPEAKER_SIDE_RIGHT);

SLuint32 AudioOutputStreamOpenSLES::channelCountToChannelMask(i32 channelCount) const {
...
Result AudioOutputStreamOpenSLES::open() {
    ...
    if (getSdkVersion() < __ANDROID_API_L__ && mFormat == AudioFormat::Float){
        // TODO: Allow floating point format on API <21 using f32->i16 converter
...
Result AudioOutputStreamOpenSLES::requestStart() {
    ...
    if (getBufferDepth(mSimpleBufferQueueInterface) == 0) {
        ...
        b8 shouldStopStream = processBufferCallback(mSimpleBufferQueueInterface);
        if (shouldStopStream) {
...
Result AudioOutputStreamOpenSLES::requestPause_l() {
    ...
    if (result == Result::OK) {
        ...
        z64 framesWritten = getFramesWritten();
        if (framesWritten >= 0) {
...
z0 AudioOutputStreamOpenSLES::setFramesRead(z64 framesRead) {
    z64 millisWritten = framesRead * kMillisPerSecond / getSampleRate();
    mPositionMillis.set(millisWritten);
}

z0 AudioOutputStreamOpenSLES::updateFramesRead() {
    if (usingFIFO()) {
