...
public:
    ...
    i32 open(i32 bytesPerFixedBlock) override;
     ...
     */
    i32 read(u8 *buffer, i32 numBytes);

private:
    i32 readFromStorage(u8 *buffer, i32 numBytes);

    i32               mValid = 0;            // Number of valid bytes in mStorage.
};
