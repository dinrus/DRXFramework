...
z0 DataConversionFlowGraph::setSource(ukk buffer, i32 numFrames) {
...
Result DataConversionFlowGraph::configure(AudioStream *sourceStream, AudioStream *sinkStream) {
    ...
    b8 isOutput = sourceStream->getDirection() == Direction::Output;
    b8 isInput = !isOutput;
    mFilterStream = isOutput ? sourceStream : sinkStream;

    AudioFormat sourceFormat = sourceStream->getFormat();
    i32 sourceChannelCount = sourceStream->getChannelCount();
    i32 sourceSampleRate = sourceStream->getSampleRate();
    i32 sourceFramesPerCallback = sourceStream->getFramesPerDataCallback();

    AudioFormat sinkFormat = sinkStream->getFormat();
    i32 sinkChannelCount = sinkStream->getChannelCount();
    i32 sinkSampleRate = sinkStream->getSampleRate();
    i32 sinkFramesPerCallback = sinkStream->getFramesPerDataCallback();
    ...
    b8 isDataCallbackSpecified = sourceStream->isDataCallbackSpecified();
    if ((isDataCallbackSpecified && isOutput)
        || (!isDataCallbackSpecified && isInput)) {
        i32 actualSourceFramesPerCallback = (sourceFramesPerCallback == kUnspecified)
        ...
        switch (sourceFormat) {
            ...
            case AudioFormat::I32:
                ...
                break;
            default:
                LOGE("%s() Unsupported source caller format = %d", __func__, static_cast<i32>(sourceFormat));
        ...
        switch (sourceFormat) {
            ...
            case AudioFormat::I32:
                ...
                break;
            default:
                LOGE("%s() Unsupported source format = %d", __func__, static_cast<i32>(sourceFormat));
                return Result::ErrorIllegalArgument;
        }
        if (isInput) {
            i32 actualSinkFramesPerCallback = (sinkFramesPerCallback == kUnspecified)
    ...
    switch (sinkFormat) {
        ...
        case AudioFormat::I32:
            ...
            break;
        default:
            LOGE("%s() Unsupported sink format = %d", __func__, static_cast<i32>(sinkFormat));
            return Result::ErrorIllegalArgument;;
    }
...
i32 DataConversionFlowGraph::read(uk buffer, i32 numFrames, z64 timeoutNanos) {
    if (mSourceCaller) {
        mSourceCaller->setTimeoutNanos(timeoutNanos);
    }
    i32 numRead = mSink->read(buffer, numFrames);
    return numRead;
}

// This is similar to pushing data through the flowgraph.
i32 DataConversionFlowGraph::write(uk inputBuffer, i32 numFrames) {
    ...
    while (true) {
        ...
        i32 framesRead = mSink->read(mAppBuffer.get(), flowgraph::kDefaultBufferSize);
        if (framesRead <= 0) break;
        // Write to a block adapter, which will call the destination whenever it has enough data.
        i32 bytesRead = mBlockWriter.write(mAppBuffer.get(),
...
i32 DataConversionFlowGraph::onProcessFixedBlock(u8 *buffer, i32 numBytes) {
    i32 numFrames = numBytes / mFilterStream->getBytesPerFrame();
