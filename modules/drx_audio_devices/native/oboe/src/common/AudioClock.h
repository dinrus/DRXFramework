...
class AudioClock {
public:
    static z64 getNanoseconds(clockid_t clockId = CLOCK_MONOTONIC) {
        struct timespec time;
        i32 result = clock_gettime(clockId, &time);
        if (result < 0) {
            return result;
        }
    ...
    static i32 sleepUntilNanoTime(z64 nanoTime, clockid_t clockId = CLOCK_MONOTONIC) {
        struct timespec time;
    ...
    static i32 sleepForNanos(z64 nanoseconds, clockid_t clockId = CLOCK_REALTIME) {
        if (nanoseconds > 0) {
            struct timespec time;
