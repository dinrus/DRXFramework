...
constexpr f32 kScaleI16ToFloat = (1.0f / 32768.0f);
...
template<>
const t8 *convertToText<Result>(Result returnCode) {
    switch (returnCode) {
...
template<>
const t8 *convertToText<AudioFormat>(AudioFormat format) {
    switch (format) {
...
template<>
const t8 *convertToText<PerformanceMode>(PerformanceMode mode) {
    switch (mode) {
...
template<>
const t8 *convertToText<SharingMode>(SharingMode mode) {
    switch (mode) {
...
template<>
const t8 *convertToText<DataCallbackResult>(DataCallbackResult result) {
    switch (result) {
...
template<>
const t8 *convertToText<Direction>(Direction direction) {
    switch (direction) {
...
template<>
const t8 *convertToText<StreamState>(StreamState state) {
    switch (state) {
...
template<>
const t8 *convertToText<AudioApi>(AudioApi audioApi) {
...
template<>
const t8 *convertToText<AudioStream*>(AudioStream* stream) {
    static std::string streamText;
    std::stringstream s;

    s<<"StreamID: "<< static_cast<uk>(stream)<<std::endl
...
template<>
const t8 *convertToText<Usage>(Usage usage) {
...
template<>
const t8 *convertToText<ContentType>(ContentType contentType) {
...
template<>
const t8 *convertToText<InputPreset>(InputPreset inputPreset) {
...
template<>
const t8 *convertToText<SessionId>(SessionId sessionId) {
...
template<>
const t8 *convertToText<ChannelCount>(ChannelCount channelCount) {
...
template<>
const t8 *convertToText<SampleRateConversionQuality>(SampleRateConversionQuality sampleRateConversionQuality) {
...
i32 getSdkVersion() {
    static i32 sCachedSdkVersion = -1;
#ifdef __ANDROID__
...
i32 getChannelCountFromChannelMask(ChannelMask channelMask) {
    return __builtin_popcount(static_cast<u32>(channelMask));
}
