...
public:
    SourceI16Caller(i32 channelCount, i32 framesPerCallback)
    : AudioSourceCaller(channelCount, framesPerCallback, sizeof(i16)) {
        mConversionBuffer = std::make_unique<i16[]>(static_cast<size_t>(channelCount)
...
private:
    std::unique_ptr<i16[]>  mConversionBuffer;
};
