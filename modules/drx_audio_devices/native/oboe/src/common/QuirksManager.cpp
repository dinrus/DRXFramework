...
i32 QuirksManager::DeviceQuirks::clipBufferSize(AudioStream &stream,
            i32 requestedSize) {
    if (!OboeGlobals::areWorkaroundsEnabled()) {
        return requestedSize;
    }
    i32 bottomMargin = kDefaultBottomMarginInBursts;
    i32 topMargin = kDefaultTopMarginInBursts;
    ...
    i32 burst = stream.getFramesPerBurst();
    i32 minSize = bottomMargin * burst;
    i32 adjustedSize = requestedSize;
    if (adjustedSize < minSize ) {
        adjustedSize = minSize;
    } else {
        i32 maxSize = stream.getBufferCapacityInFrames() - (topMargin * burst);
...
b8 QuirksManager::DeviceQuirks::isAAudioMMapPossible(const AudioStreamBuilder &builder) const {
    b8 isSampleRateCompatible =
...
b8 QuirksManager::DeviceQuirks::shouldConvertFloatToI16ForOutputStreams() {
...
public:
    ...
    i32 getExclusiveBottomMarginInBursts() const override {
        return kBottomMargin;
    }

    i32 getExclusiveTopMarginInBursts() const override {
        return kTopMargin;
    }

    // See Oboe issues #824 and #1247 for more information.
    b8 isMonoMMapActuallyStereo() const override {
        return isExynos9810 || isExynos850; // TODO We can make this version specific if it gets fixed.
    }

    b8 isAAudioMMapPossible(const AudioStreamBuilder &builder) const override {
    ...
    b8 isMMapSafe(const AudioStreamBuilder &builder) override {
        const b8 isInput = builder.getDirection() == Direction::Input;
        // This detects b/159066712 , S20 LSI has corrupt low latency audio recording
        // and turns off MMAP.
        // See also https://github.com/google/oboe/issues/892
        b8 isRecordingCorrupted = isInput
            && isExynos990
...
private:
    ...
    static constexpr i32 kBottomMargin = 2;
    static constexpr i32 kTopMargin = 1;
    b8 isExynos9810 = false;
    b8 isExynos990 = false;
    b8 isExynos850 = false;
    i32 mBuildChangelist = 0;
};
...
private:
    b8 isSM8150 = false;
    static constexpr i32 kBottomMargin = 1;
};
...
QuirksManager::QuirksManager() {
    ...
    if (productManufacturer == "samsung") {
        ...
        b8 isExynos = (arch.rfind("exynos", 0) == 0); // starts with?
        if (isExynos) {
...
b8 QuirksManager::isConversionNeeded(
        const AudioStreamBuilder &builder,
        AudioStreamBuilder &childBuilder) {
    b8 conversionNeeded = false;
    const b8 isLowLatency = builder.getPerformanceMode() == PerformanceMode::LowLatency;
    const b8 isInput = builder.getDirection() == Direction::Input;
    const b8 isFloat = builder.getFormat() == AudioFormat::Float;
    const b8 isIEC61937 = builder.getFormat() == AudioFormat::IEC61937;
    ...
    // Add quirk for f32 output when needed.
    if (OboeGlobals::areWorkaroundsEnabled()
            && isFloat
            && !isInput
            && builder.isFormatConversionAllowed()
        ...
        conversionNeeded = true;
        LOGI("QuirksManager::%s() f32 was requested but not supported on pre-L devices "
             "and some devices like Vivo devices may have issues on L devices, "
             "creating an underlying I16 stream and using format conversion to provide a f32 "
             "stream", __func__);
    }
...
b8 QuirksManager::isMMapSafe(AudioStreamBuilder &builder) {
