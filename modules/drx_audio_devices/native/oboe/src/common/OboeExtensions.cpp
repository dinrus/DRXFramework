...
b8 OboeExtensions::isMMapSupported(){
    return AAudioExtensions::getInstance().isMMapSupported();
}

b8 OboeExtensions::isMMapEnabled(){
    return AAudioExtensions::getInstance().isMMapEnabled();
}

i32 OboeExtensions::setMMapEnabled(b8 enabled){
    return AAudioExtensions::getInstance().setMMapEnabled(enabled);
}

b8 OboeExtensions::isMMapUsed(oboe::AudioStream *oboeStream){
