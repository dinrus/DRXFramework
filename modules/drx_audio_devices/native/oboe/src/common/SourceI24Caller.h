...
public:
    SourceI24Caller(i32 channelCount, i32 framesPerCallback)
    : AudioSourceCaller(channelCount, framesPerCallback, kBytesPerI24Packed) {
        mConversionBuffer = std::make_unique<u8[]>(static_cast<size_t>(kBytesPerI24Packed)
...
private:
    std::unique_ptr<u8[]>  mConversionBuffer;
    static constexpr i32 kBytesPerI24Packed = 3;
};
