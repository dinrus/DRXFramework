...
LatencyTuner::LatencyTuner(oboe::AudioStream &stream, i32 maximumBufferSize)
        : mStream(stream)
        , mMaxBufferSize(maximumBufferSize) {
    i32 burstSize = stream.getFramesPerBurst();
...
Result LatencyTuner::tune() {
    ...
    i32 numRequests = mLatencyTriggerRequests.load();
    ...
    if (mState == State::Active) {
        ...
        if (xRunCountResult == Result::OK) {
            if ((xRunCountResult.value() - mPreviousXRuns) > 0) {
                ...
                i32 oldBufferSize = mStream.getBufferSizeInFrames();
                i32 requestedBufferSize = oldBufferSize + getBufferSizeIncrement();
...
z0 LatencyTuner::requestReset() {
    if (mState != State::Unsupported) {
        mLatencyTriggerRequests++;
    }
}

z0 LatencyTuner::reset() {
    mState = State::Idle;
...
b8 LatencyTuner::isAtMaximumBufferSize() {
