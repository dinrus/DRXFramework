...
i32 AudioSourceCaller::onProcessFixedBlock(u8 *buffer, i32 numBytes) {
    AudioStreamDataCallback *callback = mStream->getDataCallback();
    i32 result = 0;
    i32 numFrames = numBytes / mStream->getBytesPerFrame();
    if (callback != nullptr) {
        ...
        if (!readResult) return (i32) readResult.error();
