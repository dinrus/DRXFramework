...
b8 oboe::OboeGlobals::mWorkaroundsEnabled = true;
 ...
 * are not available
 */
i32 DefaultStreamValues::SampleRate = 48000; // Common rate for mobile audio and video
i32 DefaultStreamValues::FramesPerBurst = 192; // 4 msec at 48000 Hz
i32 DefaultStreamValues::ChannelCount = 2; // Stereo

constexpr i32 kBufferSizeInBurstsForLowLatencyStreams = 2;
...
b8 AudioStreamBuilder::isAAudioSupported() {
    return AudioStreamAAudio::isSupported() && OBOE_ENABLE_AAUDIO;
}

b8 AudioStreamBuilder::isAAudioRecommended() {
...
b8 AudioStreamBuilder::isCompatible(AudioStreamBase &other) {
...
Result AudioStreamBuilder::openStreamInternal(AudioStream **streamPP) {
    ...
    *streamPP = nullptr;
    ...
    b8 conversionNeeded = QuirksManager::getInstance().isConversionNeeded(*this, childBuilder);
    ...
    b8 wasMMapOriginallyEnabled = AAudioExtensions::getInstance().isMMapEnabled();
    b8 wasMMapTemporarilyDisabled = false;
    if (wasMMapOriginallyEnabled) {
        b8 isMMapSafe = QuirksManager::getInstance().isMMapSafe(childBuilder);
        if (!isMMapSafe) {
