...
typedef i32 aaudio_stream_state_t;
typedef i32 aaudio_direction_t;
typedef i32 aaudio_format_t;
typedef i32 aaudio_data_callback_result_t;
typedef i32 aaudio_result_t;
typedef i32 aaudio_sharing_mode_t;
typedef i32 aaudio_performance_mode_t;
...
typedef z0 (*AAudioStream_errorCallback)(
        AAudioStream *stream,
        uk userData,
        aaudio_result_t error);

// These were defined in P
typedef i32 aaudio_usage_t;
typedef i32 aaudio_content_type_t;
typedef i32 aaudio_input_preset_t;
typedef i32 aaudio_session_id_t;
...
#if __NDK_MAJOR__ < 24
// Defined in SC_V2
typedef u32 aaudio_channel_mask_t;
typedef i32 aaudio_spatialization_behavior_t;
#endif
...
class AAudioLoader {
  public:
    ...
    typedef i32  (*signature_I_PPB)(AAudioStreamBuilder **builder);

    typedef const t8 * (*signature_CPH_I)(i32);

    typedef i32 (*signature_I_PBPPS)(AAudioStreamBuilder *,
                                      AAudioStream **stream);  // AAudioStreamBuilder_open()

    typedef i32 (*signature_I_PB)(AAudioStreamBuilder *);  // AAudioStreamBuilder_delete()
    // AAudioStreamBuilder_setSampleRate()
    typedef z0    (*signature_V_PBI)(AAudioStreamBuilder *, i32);

    // AAudioStreamBuilder_setChannelMask()
    typedef z0    (*signature_V_PBU)(AAudioStreamBuilder *, u32);

    typedef z0    (*signature_V_PBCPH)(AAudioStreamBuilder *, const t8 *);

    // AAudioStreamBuilder_setPrivacySensitive
    typedef z0    (*signature_V_PBO)(AAudioStreamBuilder *, b8);

    typedef i32 (*signature_I_PS)(AAudioStream *);  // AAudioStream_getSampleRate()
    typedef z64 (*signature_L_PS)(AAudioStream *);  // AAudioStream_getFramesRead()
    // AAudioStream_setBufferSizeInFrames()
    typedef i32 (*signature_I_PSI)(AAudioStream *, i32);

    typedef z0    (*signature_V_PBPDPV)(AAudioStreamBuilder *,
                                          AAudioStream_dataCallback,
                                          uk );

    typedef z0    (*signature_V_PBPEPV)(AAudioStreamBuilder *,
                                          AAudioStream_errorCallback,
                                          uk );

    typedef aaudio_format_t (*signature_F_PS)(AAudioStream *stream);

    typedef i32 (*signature_I_PSPVIL)(AAudioStream *, uk , i32, z64);
    typedef i32 (*signature_I_PSCPVIL)(AAudioStream *, ukk , i32, z64);

    typedef i32 (*signature_I_PSTPTL)(AAudioStream *,
                                          aaudio_stream_state_t,
                                          aaudio_stream_state_t *,
                                          z64);

    typedef i32 (*signature_I_PSKPLPL)(AAudioStream *, clockid_t, z64 *, z64 *);

    typedef b8    (*signature_O_PS)(AAudioStream *);

    typedef u32 (*signature_U_PS)(AAudioStream *);
    ...
    uk getLibHandle() const { return mLibHandle; }
  ...
  private:
    ...
    signature_I_PPB     load_I_PPB(const t8 *name);
    signature_CPH_I     load_CPH_I(const t8 *name);
    signature_V_PBI     load_V_PBI(const t8 *name);
    signature_V_PBCPH   load_V_PBCPH(const t8 *name);
    signature_V_PBPDPV  load_V_PBPDPV(const t8 *name);
    signature_V_PBPEPV  load_V_PBPEPV(const t8 *name);
    signature_I_PB      load_I_PB(const t8 *name);
    signature_I_PBPPS   load_I_PBPPS(const t8 *name);
    signature_I_PS      load_I_PS(const t8 *name);
    signature_L_PS      load_L_PS(const t8 *name);
    signature_F_PS      load_F_PS(const t8 *name);
    signature_O_PS      load_O_PS(const t8 *name);
    signature_I_PSI     load_I_PSI(const t8 *name);
    signature_I_PSPVIL  load_I_PSPVIL(const t8 *name);
    signature_I_PSCPVIL load_I_PSCPVIL(const t8 *name);
    signature_I_PSTPTL  load_I_PSTPTL(const t8 *name);
    signature_I_PSKPLPL load_I_PSKPLPL(const t8 *name);
    signature_V_PBU     load_V_PBU(const t8 *name);
    signature_U_PS      load_U_PS(const t8 *name);
    signature_V_PBO     load_V_PBO(const t8 *name);

    uk mLibHandle = nullptr;
};
