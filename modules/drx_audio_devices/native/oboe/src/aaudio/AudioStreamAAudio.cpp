...
static z0 oboe_aaudio_error_thread_proc_common(AudioStreamAAudio *oboeStream,
                                          Result error) {
#if 0
    ...
    b8 isErrorHandled = errorCallback->onError(oboeStream, error);
...
static z0 oboe_aaudio_error_thread_proc(AudioStreamAAudio *oboeStream,
                                          Result error) {
...
static z0 oboe_aaudio_error_thread_proc_shared(std::shared_ptr<AudioStream> sharedStream,
                                          Result error) {
    LOGD("%s(,%d) - entering >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>", __func__, error);
...
b8 AudioStreamAAudio::isSupported() {
    mLibLoader = AAudioLoader::getInstance();
    i32 openResult = mLibLoader->open();
    return openResult == 0;
}
...
z0 AudioStreamAAudio::internalErrorCallback(
        AAudioStream *stream,
        uk userData,
...
z0 AudioStreamAAudio::beginPerformanceHintInCallback() {
    if (isPerformanceHintEnabled()) {
        if (!mAdpfOpenAttempted) {
            z64 targetDurationNanos = (mFramesPerBurst * 1e9) / getSampleRate();
            // This has to be called from the callback thread so we get the right TID.
            i32 adpfResult = mAdpfWrapper.open(gettid(), targetDurationNanos);
            if (adpfResult < 0) {
...
z0 AudioStreamAAudio::endPerformanceHintInCallback(i32 numFrames) {
    if (mAdpfWrapper.isOpen()) {
        // Scale the measured duration based on numFrames so it is normalized to a full burst.
        f64 durationScaler = static_cast<f64>(mFramesPerBurst) / numFrames;
...
z0 AudioStreamAAudio::logUnsupportedAttributes() {
    ...
    if (sdkVersion < __ANDROID_API_P__) {
        if (mUsage != Usage::Media) {
            LOGW("Usage [AudioStreamBuilder::setUsage()] "
                 "не поддерживается on AAudio streams running on pre-Android P versions.");
        }
...
Result AudioStreamAAudio::open() {
    ...
    i32 capacity = mBufferCapacityInFrames;
    constexpr i32 kCapacityRequiredForFastLegacyTrack = 4096; // matches value in AudioFinger
    if (OboeGlobals::areWorkaroundsEnabled()
        ...
        LOGD("AudioStreamAAudio.open() capacity changed from %d to %d for lower latency",
             static_cast<i32>(mBufferCapacityInFrames), capacity);
    }
    ...
    if (mLibLoader->stream_isPrivacySensitive != nullptr && mDirection == oboe::Direction::Input) {
        b8 isPrivacySensitive = mLibLoader->stream_isPrivacySensitive(mAAudioStream);
    ...
    LOGD("AudioStreamAAudio.open() format=%d, sampleRate=%d, capacity = %d",
            static_cast<i32>(mFormat), static_cast<i32>(mSampleRate),
            static_cast<i32>(mBufferCapacityInFrames));

    calculateDefaultDelayBeforeCloseMillis();

error2:
    mLibLoader->builder_delete(aaudioBuilder);
    if (static_cast<i32>(result) > 0) {
...
static z0 oboe_stop_thread_proc(AudioStream *oboeStream) {
    if (oboeStream != nullptr) {
        oboeStream->requestStop();
    }
}

z0 AudioStreamAAudio::launchStopThread() {
...
DataCallbackResult AudioStreamAAudio::callOnAudioReady(AAudioStream * /*stream*/,
    ...
    if (result == DataCallbackResult::Continue) {
        ...
        if (result == DataCallbackResult::Stop) {
            ...
            LOGE("Oboe callback returned unexpected value. Error: %d", static_cast<i32>(result));
        }
...
ResultWithValue<i32>   AudioStreamAAudio::write(ukk buffer,
                                     i32 numFrames,
                                     z64 timeoutNanoseconds) {
    std::shared_lock<std::shared_mutex> lock(mAAudioStreamLock);
    AAudioStream *stream = mAAudioStream.load();
    if (stream != nullptr) {
        i32 result = mLibLoader->stream_write(mAAudioStream, buffer,
                                                  numFrames, timeoutNanoseconds);
        return ResultWithValue<i32>::createBasedOnSign(result);
    } else {
        return ResultWithValue<i32>(Result::ErrorClosed);
    }
}

ResultWithValue<i32>   AudioStreamAAudio::read(uk buffer,
                                 i32 numFrames,
                                 z64 timeoutNanoseconds) {
    std::shared_lock<std::shared_mutex> lock(mAAudioStreamLock);
    AAudioStream *stream = mAAudioStream.load();
    if (stream != nullptr) {
        i32 result = mLibLoader->stream_read(mAAudioStream, buffer,
                                                 numFrames, timeoutNanoseconds);
        return ResultWithValue<i32>::createBasedOnSign(result);
    } else {
        return ResultWithValue<i32>(Result::ErrorClosed);
...
ResultWithValue<i32> AudioStreamAAudio::setBufferSizeInFrames(i32 requestedFrames) {
    i32 adjustedFrames = requestedFrames;
    ...
    AAudioStream *stream = mAAudioStream.load();
    if (stream != nullptr) {
        i32 newBufferSize = mLibLoader->stream_setBufferSize(mAAudioStream, adjustedFrames);
        // Cache the result if it's valid
        if (newBufferSize > 0) mBufferSizeInFrames = newBufferSize;
        return ResultWithValue<i32>::createBasedOnSign(newBufferSize);
    } else {
        return ResultWithValue<i32>(Result::ErrorClosed);
    }
}
...
i32 AudioStreamAAudio::getBufferSizeInFrames() {
    std::shared_lock<std::shared_mutex> lock(mAAudioStreamLock);
    AAudioStream *stream = mAAudioStream.load();
    if (stream != nullptr) {
...
z0 AudioStreamAAudio::updateFramesRead() {
    std::shared_lock<std::shared_mutex> lock(mAAudioStreamLock);
    AAudioStream *stream = mAAudioStream.load();
...
z0 AudioStreamAAudio::updateFramesWritten() {
    std::shared_lock<std::shared_mutex> lock(mAAudioStreamLock);
    AAudioStream *stream = mAAudioStream.load();
    if (stream != nullptr) {
...
ResultWithValue<i32> AudioStreamAAudio::getXRunCount() {
    std::shared_lock<std::shared_mutex> lock(mAAudioStreamLock);
    AAudioStream *stream = mAAudioStream.load();
    if (stream != nullptr) {
        return ResultWithValue<i32>::createBasedOnSign(mLibLoader->stream_getXRunCount(stream));
    } else {
...
ResultWithValue<f64> AudioStreamAAudio::calculateLatencyMillis() {
    // Get the time that a known audio frame was presented.
    z64 hardwareFrameIndex;
    z64 hardwareFrameHardwareTime;
    auto result = getTimestamp(CLOCK_MONOTONIC,
                               &hardwareFrameIndex,
                               &hardwareFrameHardwareTime);
    if (result != oboe::Result::OK) {
        return ResultWithValue<f64>(static_cast<Result>(result));
    }

    // Get counter closest to the app.
    b8 isOutput = (getDirection() == oboe::Direction::Output);
    z64 appFrameIndex = isOutput ? getFramesWritten() : getFramesRead();
    ...
    z64 frameTimeDelta = (frameIndexDelta * oboe::kNanosPerSecond) / getSampleRate();
    ...
    f64 latencyNanos = static_cast<f64>(isOutput
                          ? (appFrameHardwareTime - appFrameAppTime) // hardware is later
                          : (appFrameAppTime - appFrameHardwareTime)); // hardware is earlier
    f64 latencyMillis = latencyNanos / kNanosPerMillisecond;

    return ResultWithValue<f64>(latencyMillis);
}

b8 AudioStreamAAudio::isMMapUsed() {
    std::shared_lock<std::shared_mutex> lock(mAAudioStreamLock);
    AAudioStream *stream = mAAudioStream.load();
    if (stream != nullptr) {
