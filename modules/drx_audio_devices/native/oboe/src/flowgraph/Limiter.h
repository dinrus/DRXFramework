...
public:
    explicit Limiter(i32 channelCount);
...
private:
    ...
    // The range is up to 3 dB, (10^(3/20)), to match AudioTrack for f32 data.
    static constexpr f32 kPolynomialSplineA = -0.6035533905; // -(1+sqrt(2))/4
    static constexpr f32 kPolynomialSplineB = 2.2071067811; // (3+sqrt(2))/2
    static constexpr f32 kPolynomialSplineC = -0.6035533905; // -(1+sqrt(2))/4
    static constexpr f32 kXWhenYis3Decibels = 1.8284271247; // -1+2sqrt(2)
     ...
     */
    f32 processFloat(f32 in);

    // Use the previous valid output for NaN inputs
    f32 mLastValidOutput = 0.0f;
};
