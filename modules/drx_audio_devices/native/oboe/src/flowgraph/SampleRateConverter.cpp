...
SampleRateConverter::SampleRateConverter(i32 channelCount,
...
b8 SampleRateConverter::isInputAvailable() {
...
const f32 *SampleRateConverter::getNextInputFrame() {
    const f32 *inputBuffer = input.getBuffer();
    return &inputBuffer[mInputCursor++ * input.getSamplesPerFrame()];
}

i32 SampleRateConverter::onProcess(i32 numFrames) {
    ...
    while (framesLeft > 0) {
        ...
        if(mResampler.isWriteNeeded()) {
            if (isInputAvailable()) {
                const f32 *frame = getNextInputFrame();
