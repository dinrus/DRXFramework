 ...
 */
static i32 clamp32FromFloat(f32 f)
{
    static const f32 scale = (f32)(1UL << 31);
    static const f32 limpos = 1.;
    static const f32 limneg = -1.;
    ...
    f *= scale;
    /* integer conversion is through truncation (though i32 to f32 is not).
 ...
 * depending on hardware and future implementation of this function.
 */
static i32 clamp24FromFloat(f32 f)
{
    static const f32 scale = 1 << 23;
    return (i32) lroundf(fmaxf(fminf(f * scale, scale - 1.f), -scale));
}
