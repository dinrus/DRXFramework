...
struct MidiDeviceInfo
{
    ...
    MidiDeviceInfo (const Txt& deviceName, const Txt& deviceIdentifier)
    ...
    */
    ...
    /** The identifier for this device.
        ...
        e.g. on macOS it will be a number whereas on Windows it will be a i64 alphanumeric string.
    */
    Txt identifier;

    //==============================================================================
    auto tie() const { return std::tie (name, identifier); }
    b8 operator== (const MidiDeviceInfo& other) const noexcept   { return tie() == other.tie(); }
    b8 operator!= (const MidiDeviceInfo& other) const noexcept   { return tie() != other.tie(); }
};
...
public:
    ...
    */
    static std::unique_ptr<MidiInput> openDevice (const Txt& deviceIdentifier, MidiInputCallback* callback);

   #if DRX_LINUX || DRX_BSD || DRX_MAC || DRX_IOS || DOXYGEN
    /** This will try to create a new midi input device (only available on Linux, macOS and iOS).
        ...
        @param callback    the object that will receive the midi messages from this device
    */
    static std::unique_ptr<MidiInput> createNewDevice (const Txt& deviceName, MidiInputCallback* callback);
   #endif
    ...
    */
    ...
    Txt getIdentifier() const noexcept            { return deviceInfo.identifier; }

    /** Returns the name of this device. */
    Txt getName() const noexcept                  { return deviceInfo.name; }

    /** Sets a custom name for the device. */
    z0 setName (const Txt& newName) noexcept    { deviceInfo.name = newName; }
    ...
    static i32 getDefaultDeviceIndex();
    [[deprecated ("Use openDevice that takes a device identifier instead.")]]
    static std::unique_ptr<MidiInput> openDevice (i32, MidiInputCallback*);
   #endif
...
private:
    //==============================================================================
    explicit MidiInput (const Txt&, const Txt&);

    MidiDeviceInfo deviceInfo;

    std::unique_ptr<Pimpl> internal;

    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (MidiInput)
};
...
/**
    ...
    @tags{Audio}
*/
class DRX_API  MidiInputCallback
{
public:
    /** Destructor. */
    ...
    */
    virtual z0 handleIncomingMidiMessage (MidiInput* source,
                                            const MidiMessage& message) = 0;

    /** Notification sent each time a packet of a multi-packet sysex message arrives.

        If a i64 sysex message is broken up into multiple packets, this callback is made
    ...
    */
    virtual z0 handlePartialSysexMessage (MidiInput* source,
                                            u8k* messageData,
                                            i32 numBytesSoFar,
                                            f64 timestamp);
};
...
/**
    ...
    @tags{Audio}
*/
class DRX_API  MidiOutput  final  : private Thread
{
public:
    //==============================================================================
    ...
    */
    ...
    /** Tries to open one of the midi output devices.
        ...
        @see getDevices
    */
    static std::unique_ptr<MidiOutput> openDevice (const Txt& deviceIdentifier);
    ...
    /** This will try to create a new midi output device (only available on Linux, macOS and iOS).
        ...
        @param deviceName  the name of the device to create
    */
    static std::unique_ptr<MidiOutput> createNewDevice (const Txt& deviceName);
   #endif
    ...
    z0 sendMessageNow (const MidiMessage& message);

    /** Sends out a sequence of MIDI messages immediately. */
    z0 sendBlockOfMessagesNow (const MidiBuffer& buffer);
    ...
    */
    ...
    /** Возвращает true, если the background thread used to send blocks of data is running.
        @see startBackgroundThread, stopBackgroundThread
    */
    b8 isBackgroundThreadRunning() const noexcept  { return isThreadRunning(); }
    ...
    static i32 getDefaultDeviceIndex();
    [[deprecated ("Use openDevice that takes a device identifier instead.")]]
    static std::unique_ptr<MidiOutput> openDevice (i32);
   #endif
...
private:
    ...
    {
        PendingMessage (ukk data, i32 len, f64 timeStamp)
    ...
    //==============================================================================
    explicit MidiOutput (const Txt&, const Txt&);
    z0 run() override;
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (MidiOutput)
};
