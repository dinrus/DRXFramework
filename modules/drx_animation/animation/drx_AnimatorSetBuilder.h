...
/**
    ...
    @tags{Animations}
*/
class DRX_API  AnimatorSetBuilder
{
public:
    ...
    */
    AnimatorSetBuilder followedBy (f64 delayMs);

    /** Adds an empty Animator to the execution graph that will start executing after the Animator
        provided last to this builder object, completes upon its first update, and executes the
        provided callback.
    */
    AnimatorSetBuilder followedBy (std::function<z0()> cb);
    ...
    */
    AnimatorSetBuilder withTimeTransform (std::function<f64 (f64)> transform);
...
private:
    ...
    z0 add (std::optional<Animator> parent, Animator child);
