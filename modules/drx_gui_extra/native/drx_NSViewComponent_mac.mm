...
public:
    ...
    z0 componentMovedOrResized (Component& comp, b8 wasMoved, b8 wasResized) override
    {
...
private:
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (NSViewAttachment)
};
...
z0 NSViewComponent::setView (uk view)
{
    if (view != getView())
    {
        auto old = attachment;
...
z0 NSViewComponent::resizeViewToFit()
{
    if (attachment != nullptr)
        static_cast<NSViewAttachment*> (attachment.get())->componentMovedOrResized (true, true);
}

z0 NSViewComponent::paint (Graphics&) {}

z0 NSViewComponent::alphaChanged()
{
    if (attachment != nullptr)
        (static_cast<NSViewAttachment*> (attachment.get()))->updateAlpha();
}

ReferenceCountedObject* NSViewComponent::attachViewToComponent (Component& comp, uk view)
{
