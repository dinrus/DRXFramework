...
namespace
{
    ...
    b8 createAppleRemoteInterface (io_object_t iod, uk* device)
    {
    ...
    z0 appleRemoteQueueCallback (uk const target, const IOReturn result, uk, uk)
    {
...
b8 AppleRemoteDevice::start (const b8 inExclusiveMode)
{
    if (queue != nullptr)
        return true;
...
b8 AppleRemoteDevice::isActive() const
{
    return queue != nullptr;
}

b8 AppleRemoteDevice::open (const b8 openInExclusiveMode)
{
    Array<i32> cookies;
    ...
    for (i32 i = 0; i < CFArrayGetCount (elements.object); ++i)
    {
...
z0 AppleRemoteDevice::handleCallbackInternal()
{
    i32 totalValues = 0;
    ...
    while (numCookies < numElementsInArray (cookies))
    {
        ...
        {
            totalValues += e.value;
            cookies [numCookies++] = (t8) (pointer_sized_int) e.elementCookie;
        }
    }

    cookies [numCookies++] = 0;

    static const t8 buttonPatterns[] =
    {
    ...
    while (i < numElementsInArray (buttonPatterns))
    {
        ...
        i += (i32) strlen (buttonPatterns + i) + 1;
        ++buttonNum;
    }
}
