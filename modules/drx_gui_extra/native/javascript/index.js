...
/**
 * Returns a function object that calls a function registered on the DRX backend and forwards all
 * parameters to it.
 *
...
function getNativeFunction(name) {
  if (!window.__JUCE__.initialisationData.__drx__functions.includes(name))
    console.warn(
  ...
  const f = function () {
    ...
    window.__JUCE__.backend.emitEvent("__drx__invoke", {
      name: name,
...
class SliderState {
  constructor(name) {
    if (!window.__JUCE__.initialisationData.__drx__sliders.includes(name))
      console.warn(
    ...
    this.name = name;
    this.identifier = "__drx__slider" + this.name;
    this.scaledValue = 0;
    this.properties = {
      start: 0,
      end: 1,
...
for (const sliderName of window.__JUCE__.initialisationData.__drx__sliders)
...
class ToggleState {
  constructor(name) {
    if (!window.__JUCE__.initialisationData.__drx__toggles.includes(name))
      console.warn(
    ...
    this.name = name;
    this.identifier = "__drx__toggle" + this.name;
    this.value = false;
    this.properties = {
      name: "",
      parameterIndex: -1,
...
for (const name of window.__JUCE__.initialisationData.__drx__toggles)
...
class ComboBoxState {
  constructor(name) {
    if (!window.__JUCE__.initialisationData.__drx__comboBoxes.includes(name))
      console.warn(
    ...
    this.name = name;
    this.identifier = "__drx__comboBox" + this.name;
    this.value = 0.0;
    this.properties = {
      name: "",
      parameterIndex: -1,
...
for (const name of window.__JUCE__.initialisationData.__drx__comboBoxes)
...
function getBackendResourceAddress(path) {
  const platform =
    window.__JUCE__.initialisationData.__drx__platform.length > 0
      ? window.__JUCE__.initialisationData.__drx__platform[0]
      : "";
  ...
  console.warn(
    "getBackendResourceAddress() called, but no DRX native backend is detected."
  );
  return path;
}
...
class ControlParameterIndexUpdater {
  ...
  handleMouseMove(event) {
    ...
    window.__JUCE__.backend.emitEvent(
      "__drx__controlParameterIndexChanged",
