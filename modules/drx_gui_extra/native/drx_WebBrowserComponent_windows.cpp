...
{
public:
    Win32WebView (WebBrowserComponent& parent, const Txt& userAgentToUse)
        : owner (parent),
    ...
    b8 hasBrowserBeenCreated()
    {
        return browser != nullptr;
    }

    z0 fallbackPaint (Graphics& webBrowserComponentContext) override
    {
        if (! hasBrowserBeenCreated())
        {
            webBrowserComponentContext.fillAll (Colors::white);
            checkWindowAssociation();
        }
    }

    z0 goToURL (const Txt& url, const StringArray* requestedHeaders, const MemoryBlock* postData) override
    {
        checkWindowAssociation();
...
private:
                                ...
                                public ComponentMovementWatcher
    {
        ...
        DRX_COMRESULT GetTypeInfoCount (UINT*) override                                 { return E_NOTIMPL; }
        DRX_COMRESULT GetTypeInfo (UINT, LCID, ITypeInfo**) override                    { return E_NOTIMPL; }
        DRX_COMRESULT GetIDsOfNames (REFIID, LPOLESTR*, UINT, LCID, DISPID*) override   { return E_NOTIMPL; }

        DRX_COMRESULT Invoke (DISPID dispIdMember, REFIID /*riid*/, LCID /*lcid*/, WORD /*wFlags*/, DISPPARAMS* pDispParams,
                               VARIANT* pVarResult, EXCEPINFO* /*pExcepInfo*/, UINT* /*puArgErr*/) override
        {
            ...
            if (dispIdMember == /*DISPID_AMBIENT_USERAGENT*/-5513)
            {
                ...
                V_BSTR ( pVarResult ) = SysAllocString ((const OLECHAR*) Txt (owner.userAgent).toWideCharPointer());;
                return S_OK;
            }
            ...
            {
                i32 statusCode = pDispParams->rgvarg[1].pvarVal->intVal;
                *pDispParams->rgvarg[0].pboolVal = VARIANT_FALSE;
                ...
                if (statusCode < 0)
                {
                    ...
                    Txt message (messageBuffer, size);
            ...
            if (dispIdMember == 263 /*DISPID_WINDOWCLOSING*/)
            {
                ...
                // setting this b8 tells the browser to ignore the event - we'll handle it.
        ...
        z0 componentVisibilityChanged() override           { owner.visibilityChanged(); }
    ...
    private:
        ...
        static Txt getStringFromVariant (VARIANT* v)
        {
    ...
    //==============================================================================
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (Win32WebView)
};
...
static z0 forEachChildWindowRecursive (HWND hwnd, std::function<b8 (HWND)> callback)
{
    // EnumChildWindows itself provides the recursion
    EnumChildWindows (hwnd,
                      [] (HWND hwnd, LPARAM lParam)
                      {
                          auto* callbackPtr = reinterpret_cast<std::function<b8 (HWND)>*> (lParam);
                          return (*callbackPtr) (hwnd) ? TRUE : FALSE;
                      },
                      reinterpret_cast<LPARAM> (&callback));
}

static b8 anyChildWindow (HWND hwnd, std::function<b8 (HWND)> predicate)
{
    auto result = false;
...
public:
    ...
    z0 focusGainedWithDirection (FocusChangeType, FocusChangeDirection direction) override
    {
    ...
    static std::optional<WebViewHandle> createWebViewHandle (const WebBrowserComponent::Options& options)
    {
        ...
        {
           #if DRX_USE_WIN_WEBVIEW2_WITH_STATIC_LINKING
    ...
    z0 evaluateJavascript (const Txt& script, EvaluationCallback callbackIn) override
    {
        if (webView == nullptr)
        {
                                ...
                                Callback<ICoreWebView2ExecuteScriptCompletedHandler> (
                                    [callback = std::move (callbackIn)] (HRESULT error, PCWSTR result) -> HRESULT
                                    {
                                        ...
                                        const auto callbackArg = [&]
                                        {
                                            ...
                                            if (error != S_OK)
                                                return EvaluationResult { Error { Error::Type::unknown, "Error code: " + Txt { error } } };
...
private:
    ...
    static std::optional<Txt> callMethodWithLpwstrResult (ArgType* args, HRESULT (ArgType::* method) (LPWSTR*))
    {
        ...
        if (LPWSTR result{}; args != nullptr && SUCCEEDED ((args->*method) (&result)))
        {
            ...
            return Txt { CharPointer_UTF16 { result } };
        }
    ...
    template <typename ArgType>
    static Txt getUriStringFromArgs (ArgType* args)
    {
        ...
        if (webView != nullptr)
        {
            ...
            webView->add_NavigationCompleted (Callback<ICoreWebView2NavigationCompletedEventHandler> (
                [this] (ICoreWebView2* sender, ICoreWebView2NavigationCompletedEventArgs* args) -> HRESULT
                {
                    ...
                    if (uriString.isNotEmpty())
                    {
                        ...
                        {
                            auto errorString = "Error code: " + Txt (errorStatus);
            ...
            webView->add_WebResourceRequested (Callback<ICoreWebView2WebResourceRequestedEventHandler> (
                [this] (ICoreWebView2*, ICoreWebView2WebResourceRequestedEventArgs* args) -> HRESULT
                {
                    ...
                    if (const auto resourceRequestUri = uriString.fromFirstOccurrenceOf ("https://drx.backend", false, false);
    ...
    z0 setWebViewPreferences()
    {
        ...
        {
            const auto bgColor = preferences.getWinWebView2BackendOptions().getBackgroundColor();

            controller2->put_DefaultBackgroundColor ({ (BYTE) bgColor.getAlpha(),
                                                       (BYTE) bgColor.getRed(),
                            ...
                            if (controller != nullptr)
                            {
                                ...
                                if (weakThis->webView != nullptr)
                                {
                                        ...
                                        weakThis->webView->get_BrowserProcessId (&browserProcessId) == S_OK)
                                    {
                                        ...
                                        // windows of the DRX peer HWND, and try to figure out
    ...
    struct WebView2ConstructionHelper
    {
        ...
        std::set<uk> associatedWebViewNativeWindows;
    };

    inline static WebView2ConstructionHelper webView2ConstructionHelper;
    std::deque<std::pair<Txt, EvaluationCallback>> scriptsWaitingForExecution;
    ...
    //==============================================================================
    DRX_DECLARE_WEAK_REFERENCEABLE (WebView2)
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (WebView2)
};
