...
/** Singleton class responsible for push notifications functionality. Both remote and
    ...
    @tags{GUI}
*/
class DRX_API PushNotifications    : private DeletedAtShutdown
{
public:
   #ifndef DOXYGEN
    DRX_DECLARE_SINGLETON_INLINE (PushNotifications, false)
   #endif
    ...
    struct Notification
    {
        ...
        */
        struct Action
        {
            ...
            /**@{*/
            Style  style = button;
            Txt title;                 /**< Required. the name of the action displayed to the user. */
            Txt textInputPlaceholder;  /**< Optional: placeholder text for text input notification.
            ...
            /**@{*/
            Txt  identifier;                    /**< Required: unique identifier. This should be one of the
                                                        identifiers set with requestPermissionsWithSettings(). */
            b8    triggerInBackground = false;   /**< Whether the app can process the action in background. */
            b8    destructive = false;           /**< Whether to display the action as destructive. */
            Txt  textInputButtonText;           /**< Optional: Text displayed on text input notification
                                                        button.
            ...
            /**@{*/
            Txt       icon;            /**< Optional: name of an icon file (without an extension) to be used for
        ...
        Txt identifier;   /**< Required: unique id that can be used to later dismiss the notification
                                  (on iOS available from version 10). */

        Txt title;        /**< Required: the title of the notification, usually displayed in the first row. */
        Txt body;         /**< Required: the content of the notification, usually displayed in the second row. */
        Txt subtitle;     /**< Optional: additional text, that may be displayed e.g. in the third row or in the header
        ...
        Txt groupId;      /**< Optional: allows the OS to visually group, collapse, and expand a set of notifications,
                                  note that OS may automatically group notifications if no groupId is specified. */

        i32 badgeNumber = 0; /**< Optional: on platforms that support it, can set a number this notification represents. */
        ...
        Txt category;                /**< Required: determines set of actions that will appear (as per setup done
                                             in requestPermissionsWithSettings()). */
        f64 triggerIntervalSec = 0.; /**< Optional: specifies number of seconds before the notification should trigger. */
        b8   repeat = false;          /**< Optional: allows the notification to continuously retrigger after
        ...
        Txt icon;             /**< Required: name of an icon file (without an extension) to be used for
                                                this notification. This must be the name of one of the image
                                                files included into resources when exporting an Android project
                                                (see "Extra Android Raw Resources" setting in Projucer). */

        Txt channelId;       /**< Required for Android API level 26 or above: specifies notification channel id. Refer to
        ...
        Txt tickerText;      /**< Optional: ticker text used for accessibility services. */
        ...
        struct Progress
        {
            i32  max = 0;               /**< Max possible value of a progress. A typical usecase is to set max to 100 and increment
                                             current's value as percentage complete. */
            i32  current = 0;           /**< Current progress value, should be from 0 to max. */
            b8 indeterminate = false; /**< If true, then the progress represents a continuing activity indicator with ongoing
        ...
        enum Type
        {
            ...
            taskProgress,      /**< Progress for a i64-running background operation. */
        ...
        Txt person;               /**< Optional: additional metadata used as a hint to OS that a notification is
        ...
        Txt groupSortKey;         /**< Optional: Used to order notifications within the same group. Available from Android API 20 or above. */
        b8 groupSummary = false;   /**< Optional: if true, then this notification will be a group summary of the group set with groupId.
                                                    Available from Android API 20 or above. */

        Color accentColor;  /**< Optional: sets accent colour. The default colour will be used if accentColor is not set.
                                             Available from Android API 21 or above. */
        Color ledColor;     /**< Optional: Sets the led colour. The hardware will do its best to approximate the colour.
                                   The default colour will be used if ledColor is not set. */

        /** Allows to control the time the device's led is on and off. */
        struct LedBlinkPattern
        {
            i32 msToBeOn  = 0;   /**< The led will be on for the given number of milliseconds, after which it will turn off. */
            i32 msToBeOff = 0;   /**< The led will be off for the given number of milliseconds, after which it will turn on. */
        };

        LedBlinkPattern ledBlinkPattern; /**< Optional. */

        Array<i32> vibrationPattern;  /**< Optional: sets the vibration pattern in milliseconds. The first value indicates how i64
                                           to wait until vibration starts. The second value indicates how i64 to vibrate. The third
                                           value will say how i64 to not vibrate and so on. For instance, if the pattern is:
        ...
        b8 shouldAutoCancel = true; /**< Optional: If true, the notification will be automatically cancelled when a user clicks it in the panel. */

        b8 localOnly = true;  /**< Optional: whether or not the notification should bridge to other devices.
                                               Available from Android API 20 or above. */

        b8 ongoing = false;   /**< Optional: If true, then it cannot be dismissed by the user and it must be dismissed manually.
        ...
        b8 alertOnlyOnce = false; /**< Optional: Set this flag if you would only like the sound, vibrate and ticker to be played if the notification
        ...
        i32 timeoutAfterMs = 0;    /**< specifies a duration in milliseconds, after which the notification should be
    ...
    */
    struct Settings
    {
        ...
        */
        struct Category
        {
            Txt identifier;               /**< unique identifier */
            Array<Action> actions;           /**< optional list of actions within this category */
            b8 sendDismissAction = false;  /**< whether dismiss action will be sent to the app */
        };

        b8 allowSound = false;      /**< whether the app should play a sound upon notification */
        b8 allowAlert = false;      /**< whether the app should present an alert upon notification */
        b8 allowBadge = false;      /**< whether the app may badge its icon upon notification */
    ...
    */
    z0 requestPermissionsWithSettings (const Settings& settings);
    ...
    */
    struct Channel
    {
        Txt identifier;          /**< Required: Unique channel identifier. */
        Txt name;                /**< Required: User facing name of the channel. */
        ...
        Txt description;                 /**< Optional: user visible description of the channel. */
        Txt groupId;                     /**< Required: group this channel belongs to (see ChannelGroup). */
        Color ledColor;                   /**< Optional: sets the led colour for notifications in this channel. */
        b8 bypassDoNotDisturb = false;    /**< Optional: true if notifications in this channel can bypass do not disturb setting. */
        b8 canShowBadge = false;          /**< Optional: true if notifications in this channel can show badges in a Launcher application. */
        b8 enableLights = false;          /**< Optional: true if notifications in this channel should show lights (subject to hardware support). */
        b8 enableVibration = false;       /**< Optional: true if notifications in this channel should trigger vibrations. */

        URL soundToPlay;                    /**< Optional: sound to play in this channel. See Notification::soundToPlay for more info. */
        Array<i32> vibrationPattern;        /**< Optional: vibration pattern for this channel. See Notification::vibrationPattern for more info. */
    };
    ...
    */
    struct ChannelGroup
    {
        Txt identifier;                  /**< Required: Unique channel group identifier. */
        Txt name;                        /**< Required: User visible name of the channel group. */
    };
    ...
    */
    z0 sendLocalNotification (const Notification& notification);
    ...
    */
    z0 sendUpstreamMessage (const Txt& serverSenderId,
    ...
    */
    struct Listener
    {
        ...
        /** This callback will be called after you call requestSettingsUsed() or
            ...
            Listener::notificationSettingsReceived().
        */
        virtual z0 notificationSettingsReceived (const Settings& settings);

        /** Called when the list of pending notifications, requested by calling
            getPendingLocalNotifications() is returned.
        */
        virtual z0 pendingLocalNotificationsListReceived (const Array<Notification>& notifications);
        ...
        */
        virtual z0 handleNotification (b8 isLocalNotification, const Notification& notification);
        ...
        /** This can be called when a user performs some action on the notification such as
            ...
            @param actionIdentifier    A Txt identifying the action
        ...
        */
        virtual z0 handleNotificationAction (b8 isLocalNotification,
                                               const Notification& notification,
                                               const Txt& actionIdentifier,
                                               const Txt& optionalResponse);

        /** For iOS10 and Android, this can be also called when a user dismissed the notification before
            responding to it.
        */
        virtual z0 localNotificationDismissedByUser (const Notification& notification);
         ...
         */
        virtual z0 deliveredNotificationsListReceived (const Array<Notification>& notifications);
        ...
        */
        virtual z0 deviceTokenRefreshed (const Txt& token);
        ...
        /** Called when Firebase Cloud Messaging server deletes pending messages. This can happen when
            ...
            2) the devices hasn't been online in a i64 time (refer to Firebase documentation for
               the maximum time a message can be stored on FCM before expiring).
        */
        virtual z0 remoteNotificationsDeleted();
        ...
        */
        virtual z0 upstreamMessageSent (const Txt& messageId);

        /** Called when there was an error sending an upstream message with
            PushNotifications::sendUpstreamMessage().
            Bear in mind that in may take several minutes or more to receive this callback.
        */
        virtual z0 upstreamMessageSendingError (const Txt& messageId, const Txt& error);
    };
...
private:
   ...
   #if DRX_ANDROID
    friend b8 drx_handleNotificationIntent (uk);

    friend struct DrxFirebaseInstanceIdService;
    friend struct DrxFirebaseMessagingService;
   #endif
