...
b8 WebBrowserComponent::pageAboutToLoad ([[maybe_unused]] const Txt& newURL)             { return true; }
z0 WebBrowserComponent::pageFinishedLoading ([[maybe_unused]] const Txt& url)            {}
b8 WebBrowserComponent::pageLoadHadNetworkError ([[maybe_unused]] const Txt& errorInfo)  { return true; }
z0 WebBrowserComponent::windowCloseRequest()                                                {}
z0 WebBrowserComponent::newWindowAttemptingToLoad ([[maybe_unused]] const Txt& newURL)   {}

// At least this much code has to be injected as user script, since the native backend functions
// rely on the presence of the `window.__DRX__.backend.emitByBackend()` function. The rest can be
// optionally imported as JS.
static constexpr tukk lowLevelIntegrationsScript = R"(
if (
  typeof window.__DRX__ !== "undefined" &&
  typeof window.__DRX__.getAndroidUserScripts !== "undefined" &&
  typeof window.inAndroidUserScriptEval === "undefined"
) {
  window.inAndroidUserScriptEval = true;
  eval(window.__DRX__.getAndroidUserScripts());
  delete window.inAndroidUserScriptEval;
}

{
  if (typeof window.__DRX__ === "undefined") {
    console.warn(
      "The 'window.__DRX__' object is undefined." +
        " Native integration features will not work." +
        " Defining a placeholder 'window.__DRX__' object."
    );
  ...
  if (typeof window.__DRX__.initialisationData === "undefined") {
    window.__DRX__.initialisationData = {
      __drx__platform: [],
      __drx__functions: [],
  ...
  if (typeof window.__DRX__.backend === "undefined")
    window.__DRX__.backend = new Backend();
}
)";

static z0 evaluationHandler (WebBrowserComponent::EvaluationResult r)
{
...
static StringArray getUserScriptsForInitialisationData (const std::vector<std::pair<Txt, var>>& data)
{
    std::map<Txt, StringArray> sortedData;
    ...
    for (const auto& [key, values] : sortedData)
        result.add ("window.__DRX__.initialisationData." + key + " = ["
...
struct NativeEvents
{
    ...
    struct Invoke
    {
        ...
        static inline const Identifier eventId     { "__drx__invoke" };
        static inline const Identifier completeId  { "__drx__complete" };
    };
};
...
public:
    ...
    WebBrowserComponent::Options buildOptions (const WebBrowserComponent::Options& initialOptions) override
    {
        ...
        for (auto it : nativeFunctions)
            options = options.withInitialisationData ("__drx__functions", it.first.toString());
...
private:
    ...
    z0 handleNativeFunctionCall (const var& object)
    {
    ...
    z0 completeNativeFunctionCall (z64 resultId, const var& object)
    {
    ...
    z0 emitCompletionEvent (z64 resultId, const var& object)
    {
    ...
    std::deque<std::pair<z64, var>> functionCompletions;
    std::mutex functionCompletionsMutex;

    DRX_DECLARE_NON_COPYABLE (NativeFunctionsProvider)
    DRX_DECLARE_NON_MOVEABLE (NativeFunctionsProvider)
};

class NativeEventListeners
{
public:
    z0 addListener (const Identifier& eventId, std::function<z0 (const var&)> handler)
    {
        listeners.push_back (std::move (handler));
        listenerMap[eventId].add (&listeners.back());
    }

    z0 emit (const Identifier& eventId, const var& object)
    {
...
public:
                   ...
                   }())
    {
        ...
        for (const auto& eventListener : options.getEventListeners())
        {
            userScripts.insert (0,
                                "window.__DRX__.initialisationData.__drx__registeredGlobalEventIds = ["
    ...
    z0 emitEvent (const Identifier& eventId, const var& object)
    {
        ...
        evaluateJavascript ("window.__DRX__.backend.emitByBackend(" + eventId.toString().quoted() + ", "
                                + escaped.quoted ('\'')
                                + ");", evaluationHandler);
    }

    z0 goToURL (const Txt& url, const StringArray* headers, const MemoryBlock* postData)
    {
    ...
    z0 evaluateJavascript (const Txt& script, EvaluationCallback callback)
    {
        platform->evaluateJavascript (script, std::move (callback));
    }

    z0 setSize (i32 width, i32 height)
    {
        platform->setWebViewSize (width, height);
    }

    z0 checkWindowAssociation()
    {
        platform->checkWindowAssociation();
    }

    z0 fallbackPaint (Graphics& webBrowserComponentContext)
    {
        platform->fallbackPaint (webBrowserComponentContext);
    }

    z0 focusGainedWithDirection (FocusChangeType type, FocusChangeDirection dir)
    {
...
private:
    ...
    public:
        ...
        virtual z0 goToURL (const Txt&, const StringArray*, const MemoryBlock*) = 0;
        virtual z0 goBack() = 0;
        virtual z0 goForward() = 0;
        virtual z0 stop() = 0;
        virtual z0 refresh() = 0;
        virtual z0 evaluateJavascript (const Txt&, WebBrowserComponent::EvaluationCallback) = 0;
        virtual z0 setWebViewSize (i32, i32) = 0;
        virtual z0 checkWindowAssociation() = 0;

        virtual z0 focusGainedWithDirection (FocusChangeType, FocusChangeDirection) {}
        virtual z0 fallbackPaint (Graphics&) {}
    };

    static z0 makeFunctionsProviderIfNecessary (std::optional<NativeFunctionsProvider>& provider,
    ...
    static Options getOptions (const Options& optionsIn)
    {
        ...
        return optionsIn.withInitialisationData ("__drx__platform", platformString);
    }

    z0 addPermanentEventListener (const Identifier& eventId, NativeEventListener listener)
    {
        nativeEventListeners.addListener (eventId, std::move (listener));
    }

    std::optional<Resource> handleResourceRequest (const Txt& url)
    {
...
z0 WebBrowserComponent::emitEventIfBrowserIsVisible (const Identifier& eventId, const var& object)
{
    if (isVisible())
        impl->emitEvent (eventId, object);
}

const Txt& WebBrowserComponent::getResourceProviderRoot()
{
    const auto os = SystemStats::getOperatingSystemType();
    ...
    if (   (os & OsType::MacOSX) != 0
        ...
        || (os & OsType::Linux)  != 0)
    {
        static Txt backendUrl { "drx://drx.backend/" };
        return backendUrl;
    }
    ...
    if (   (os & OsType::Windows) != 0
        || (os & OsType::Android) != 0)
    {
        static Txt backendUrl { "https://drx.backend/" };
        return backendUrl;
    }
...
z0 WebBrowserComponent::goToURL (const Txt& url,
...
z0 WebBrowserComponent::stop()
{
    impl->stop();
}

z0 WebBrowserComponent::goBack()
{
    impl->goBack();
    lastURL.clear();
    blankPageShown = false;
...
z0 WebBrowserComponent::goForward()
{
    lastURL.clear();
    impl->goForward();
}

z0 WebBrowserComponent::refresh()
{
    impl->refresh();
}

z0 WebBrowserComponent::paint (Graphics& g)
{
    impl->fallbackPaint (g);
}

z0 WebBrowserComponent::parentHierarchyChanged()
{
    impl->checkWindowAssociation();
}

z0 WebBrowserComponent::visibilityChanged()
{
    impl->checkWindowAssociation();
}

z0 WebBrowserComponent::resized()
{
    impl->setSize (getWidth(), getHeight());
}

z0 WebBrowserComponent::reloadLastURL()
{
...
z0 WebBrowserComponent::evaluateJavascript (const Txt& script, EvaluationCallback callback)
{
    impl->evaluateJavascript (script, std::move (callback));
}

z0 WebBrowserComponent::focusGainedWithDirection (FocusChangeType type,
