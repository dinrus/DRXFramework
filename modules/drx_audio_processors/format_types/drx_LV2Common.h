...
public:
    ...
    z0 setBuffer (uk buf, size_t size)
    {
        lv2_atom_forge_set_buffer (&forge, static_cast<u8*> (buf), size);
    }

          LV2_Atom_Forge* get()       { return &forge; }
    const LV2_Atom_Forge* get() const { return &forge; }

    z0 writeChunk (u32 size)
    {
...
struct ScopedFrame
{
    ...
    DRX_DECLARE_NON_COPYABLE (ScopedFrame)
    DRX_DECLARE_NON_MOVEABLE (ScopedFrame)
    DRX_LEAK_DETECTOR (ScopedFrame)
};
...
struct NumericAtomParser
{
    ...
    static Optional<Target> tryParse (const LV2_Atom&, ukk)
    {
        return {};
    }

    template <typename Target, typename Head, typename... Tail>
    static Optional<Target> tryParse (const LV2_Atom& atom, ukk data, Tag<Head> head, Tag<Tail>... tail)
    {
    ...
    Optional<Target> parseNumericAtom (const LV2_Atom* atom, ukk data) const
    {
        if (atom == nullptr)
            return {};
        ...
        return tryParse<Target> (*atom,
                                 data,
                                 Tag<i32> { mLV2_ATOM__Bool },
                                 Tag<i32> { mLV2_ATOM__Int },
                                 Tag<z64> { mLV2_ATOM__Long },
                                 Tag<f32>   { mLV2_ATOM__Float },
                                 Tag<f64>  { mLV2_ATOM__Double });
    }
    ...
    LV2_URID map (tukk str) const { return mapFeature.map (mapFeature.handle, str); }
   ...
   #undef X
    ...
    DRX_LEAK_DETECTOR (NumericAtomParser)
};

//==============================================================================
struct PatchSetHelper
{
    PatchSetHelper (LV2_URID_Map mapFeatureIn, tukk pluginUri)
        : parser (mapFeatureIn),
          pluginUrid (parser.map (pluginUri))
    {}

    b8 isPlugin (const LV2_Atom* subject) const
    {
        if (subject == nullptr)
            return true;
    ...
    z0 processPatchSet (const LV2_Atom_Object* object, Callback&& callback)
    {
    ...
    template <typename Callback>
    z0 processPatchSet (const LV2_Atom_Event* event, Callback&& callback)
    {
    ...
    template <typename Callback>
    z0 setPluginProperty (const LV2_Atom* property, const LV2_Atom* value, Callback&& callback)
    {
        if (property == nullptr)
        {
        ...
        const auto parseResult = parser.parseNumericAtom<f32> (value);
   ...
   #undef X
    ...
    DRX_LEAK_DETECTOR (PatchSetHelper)
};
...
struct SequenceWithSize
{
    ...
    DRX_LEAK_DETECTOR (SequenceWithSize)
};
...
struct SequenceIteratorTraits
{
    ...
    static b8 isEnd (const Container& s, Iter it) noexcept
    {
        return lv2_atom_sequence_is_end (s.body, static_cast<u32> (s.size), it);
    }
...
const std::map<Txt, AudioChannelSet::ChannelType> channelDesignationMap
{
...
/*  Useful for converting a `uk` to another type (X11 Window, function pointer etc.) without
    invoking UB.
*/
...
struct SinglePortInfo
{
    ...
    b8 operator== (const SinglePortInfo& other) const { return tie() == other.tie(); }
    b8 operator<  (const SinglePortInfo& other) const { return index < other.index; }
};
...
struct ParsedGroup
{
    ...
    b8 operator== (const ParsedGroup& other) const { return tie() == other.tie(); }
    ...
    static AudioChannelSet getEquivalentSet (const std::set<SinglePortInfo>& info)
    {
        ...
        if (std::any_of (info.begin(), info.end(), hasUnknownKind))
            return AudioChannelSet::discreteChannels ((i32) info.size());
    ...
    b8 isCompatible (const AudioChannelSet& requestedBus) const
    {
...
public:
    ...
    i32 getChannelForPort (u32 port) const
    {
...
private:
    ...
    static std::map<u32, i32> getPortLayout (const Array<AudioChannelSet>& layout,
                                                  const std::vector<ParsedGroup>& parsedGroup)
    {
        if ((i32) parsedGroup.size() != layout.size())
        {
        ...
        std::map<u32, i32> result;
        i32 channelOffsetOfBus = 0;
        ...
        if ((i32) result.size() != channelOffsetOfBus)
        {
            jassertfalse;
            return {};
        }
    ...
    std::map<u32, i32> ports;

    DRX_LEAK_DETECTOR (PortToAudioBufferMap)
};
...
static inline std::vector<ParsedGroup> findStableBusOrder (const Txt& mainGroupUid,
                                                           const std::map<Txt, std::set<SinglePortInfo>>& groupedPorts,
                                                           const std::set<SinglePortInfo>& ungroupedPorts)
{
    ...
    const auto pushGroup = [&] (const std::pair<Txt, std::set<SinglePortInfo>>& info)
    {
...
*/
static inline b8 isNameStartChar (t32 input)
{
...
static inline b8 isNameChar (t32 input)
{
...
static inline Txt sanitiseStringAsTtlName (const Txt& input)
{
    if (input.isEmpty())
        return {};

    std::vector<t32> sanitised;
    sanitised.reserve (static_cast<size_t> (input.length()));

    sanitised.push_back (isNameStartChar (input[0]) ? input[0] : '_');

    std::for_each (std::begin (input) + 1, std::end (input), [&] (t32 x)
    {
        sanitised.push_back (isNameChar (x) ? x : '_');
    });

    return Txt (CharPointer_UTF32 { sanitised.data() }, sanitised.size());
}
