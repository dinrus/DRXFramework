...
#if (DRX_PLUGINHOST_ARA && (DRX_PLUGINHOST_VST3 || DRX_PLUGINHOST_AU) && (DRX_MAC || DRX_WINDOWS || DRX_LINUX)) || DOXYGEN
...
private:
    struct Deleter
    {
        z0 operator() (Ptr ptr) const noexcept
        {
...
/** Helper class for the host side implementation of the %ARA %AudioSource model object.
    ...
    @tags{ARA}
*/
class DRX_API AudioSource : public ManagedARAHandle<AudioSource, ARA::ARAAudioSourceRef>
{
public:
    ...
    */
    z0 update (const ARA::ARAAudioSourceProperties& props);

    /** Changes the plugin's access to the %AudioSource samples through the DocumentController.

        Places the DocumentController in editable state.
    */
    z0 enableAudioSourceSamplesAccess (b8);
...
/** Helper class for the host side implementation of the %ARA %AudioModification model object.
    ...
    @tags{ARA}
*/
class DRX_API AudioModification : public ManagedARAHandle<AudioModification, ARA::ARAAudioModificationRef>
{
public:
    /** Returns an %ARA versioned struct with the `structSize` correctly set for the currently
        used SDK version.
    ...
    */
    ...
    /** Updates the state of the corresponding %ARA model object.
        ...
        field has already been correctly set.
    */
    z0 update (const ARA::ARAAudioModificationProperties& props);
...
public:
    ...
    */
    z0 addListener (DeletionListener& x);
...
/** Helper class for the host side implementation of the %ARA %MusicalContext model object.
    ...
    @tags{ARA}
*/
class DRX_API MusicalContext : public ManagedARAHandle<MusicalContext, ARA::ARAMusicalContextRef>
{
public:
    /** Returns an %ARA versioned struct with the `structSize` correctly set for the currently
        used SDK version.
    ...
    */
    ...
    /** Updates the state of the corresponding %ARA model object.
        ...
        field has already been correctly set.
    */
    z0 update (const ARA::ARAMusicalContextProperties& props);
...
/** Helper class for the host side implementation of the %ARA %RegionSequence model object.
    ...
    @tags{ARA}
*/
class DRX_API RegionSequence : public ManagedARAHandle<RegionSequence, ARA::ARARegionSequenceRef>
{
public:
    /** Returns an %ARA versioned struct with the `structSize` correctly set for the currently
        used SDK version.
    ...
    */
    ...
    /** Updates the state of the corresponding %ARA model object.
        ...
        field has already been correctly set.
    */
    z0 update (const ARA::ARARegionSequenceProperties& props);
...
public:
     ...
     */
    z0 add    (PlaybackRegion& region) { registry->add    (region); }
    ...
    /** Removes a PlaybackRegion from the corresponding ::PlaybackRendererInterface or ::EditorRendererInterface.
        ...
        after AudioProcessor::releaseResources().
     */
    z0 remove (PlaybackRegion& region) { registry->remove (region); }

    /** Возвращает true, если the underlying %ARA plugin extension instance fulfills the corresponding role. */
    b8 isValid() { return registry->isValid(); }
...
private:
    ...
    public:
        ...
        b8 isValid() { return rendererRef != nullptr && rendererInterface != nullptr; }

        z0 add (PlaybackRegion& region)
        {
            if (isValid())
        ...
        z0 remove (PlaybackRegion& region)
        {
            doRemoveListener (region.getDeletionListener());
        }

    private:
        z0 doRemoveListener (DeletionListener& listener) noexcept
        {
            listener.removeListener (*this);
            removeListener (listener);
        }

        z0 removeListener (DeletionListener& listener) noexcept override
        {
...
/** Wrapper class for `ARA::ARAPlugInExtensionInstance*`.
    ...
    ARAHostDocumentController must remain valid as i64 as the plugin extension is in use.

    @tags{ARA}
*/
class DRX_API PlugInExtensionInstance final
{
public:
    ...
    */
    b8 isValid() const noexcept { return instance != nullptr; }
...
/** Wrapper class for `ARA::Host::DocumentController`.
    ...
    @tags{ARA}
*/
class DRX_API ARAHostDocumentController final
{
public:
    /** Factory function.
...
/** Calls the provided callback with an ARAFactoryWrapper object obtained from the provided
    ...
    ARA extensions.
*/
z0 DRX_API createARAFactoryAsync (AudioPluginInstance& instance, std::function<z0 (ARAFactoryWrapper)> cb);
