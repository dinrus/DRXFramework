...
struct SpeakerMappings  : private AudioChannelSet // (inheritance only to give easier access to items in the namespace)
{
    ...
    struct Mapping
    {
        ...
        b8 matches (const Array<ChannelType>& chans) const noexcept
        {
            auto n = static_cast<i32> (sizeof (channels) / sizeof (ChannelType));
    ...
    static AudioChannelSet vstArrangementTypeToChannelSet (i32 arr, i32 fallbackNumChannels)
    {
    ...
    static i32 channelSetToVstArrangementType (AudioChannelSet channels)
    {
    ...
    static z0 channelSetToVstArrangement (const AudioChannelSet& channels, Vst2::VstSpeakerArrangement& result)
    {
        result.type = channelSetToVstArrangementType (channels);
        result.numChannels = channels.size();

        for (i32 i = 0; i < result.numChannels; ++i)
        {
    ...
    public:
        ...
        VstSpeakerConfigurationHolder (const AudioChannelSet& channels)
        {
            ...
            for (i32 i = 0; i < dst.numChannels; ++i)
            {
    ...
    private:
        DRX_LEAK_DETECTOR (VstSpeakerConfigurationHolder)

        HeapBlock<Vst2::VstSpeakerArrangement> storage;

        Vst2::VstSpeakerArrangement* allocate (i32 numChannels)
        {
    ...
    static i32 getSpeakerType (AudioChannelSet::ChannelType type) noexcept
    {
        static const std::map<AudioChannelSet::ChannelType, i32> speakerTypeMap =
        {
