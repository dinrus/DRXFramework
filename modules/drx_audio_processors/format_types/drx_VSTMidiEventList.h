...
#ifdef DRX_VSTINTERFACE_H_INCLUDED
...
class VSTMidiEventList
{
    ...
    static auto& getEvent (Events& events, i32 index)
    {
    ...
    Vst2::VstEvent* const& getEvent (i32 index) const { return getEvent (*events, index); }
    Vst2::VstEvent*      & getEvent (i32 index)       { return getEvent (*events, index); }
...
public:
    ...
    z0 addEvent (ukk const midiData, i32 numBytes, i32 frameOffset)
    {
        ensureSize (numEventsUsed + 1);

        uk const ptr = getEvent (numEventsUsed);
        ...
        else
        {
            ...
            se->sysexDump = new t8 [(size_t) numBytes];
    ...
    // or plugin.
    static z0 addEventsToMidiBuffer (const Vst2::VstEvents* events, MidiBuffer& dest)
    {
        for (i32 i = 0; i < events->numEvents; ++i)
        {
            ...
            if (e != nullptr)
            {
                ...
                {
                    dest.addEvent ((const drx::u8*) static_cast<const Vst2::VstMidiEvent*> (ptr)->midiData,
                                   4, e->deltaFrames);
                }
                else if (e->type == Vst2::kVstSysExType)
                {
                    const auto* se = static_cast<const Vst2::VstMidiSysexEvent*> (ptr);
                    dest.addEvent ((const drx::u8*) se->sysexDump,
                                   (i32) se->dumpBytes,
                                   e->deltaFrames);
                }
            }
    ...
    //==============================================================================
    z0 ensureSize (i32 numEventsNeeded)
    {
        if (numEventsNeeded > numEventsAllocated)
        {
            ...
            for (i32 i = numEventsAllocated; i < numEventsNeeded; ++i)
    ...
    z0 freeEvents()
    {
        if (events != nullptr)
        {
            for (i32 i = numEventsAllocated; --i >= 0;)
...
private:
    i32 numEventsUsed, numEventsAllocated;
    ...
    static z0 freeVSTEvent (Vst2::VstEvent* e)
    {
        if (e->type == Vst2::kVstSysExType)
        {
            delete[] (reinterpret_cast<Vst2::VstMidiSysexEvent*> (e)->sysexDump);
...
#endif // DRX_VSTINTERFACE_H_INCLUDED
