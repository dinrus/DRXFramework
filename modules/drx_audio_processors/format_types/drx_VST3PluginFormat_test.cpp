...
public:
    ...
    z0 runTest() override
    {
        beginTest ("ChannelMapping for a stereo bus performs no remapping");
        {
            ...
            expect (map.getDrxChannelForVst3Channel (0) == 0); // L -> left
            expect (map.getDrxChannelForVst3Channel (1) == 1); // R -> right
        }
        ...
        beginTest ("ChannelMapping for a k91_6 bus remaps the channels appropriately");
        {
            ...
            expect (map.getDrxChannelForVst3Channel (0)  == 0);  // L   -> left
            expect (map.getDrxChannelForVst3Channel (1)  == 1);  // R   -> right
            expect (map.getDrxChannelForVst3Channel (2)  == 2);  // C   -> centre
            expect (map.getDrxChannelForVst3Channel (3)  == 3);  // Lfe -> LFE
            expect (map.getDrxChannelForVst3Channel (4)  == 4);  // Ls  -> leftSurround
            expect (map.getDrxChannelForVst3Channel (5)  == 5);  // Rs  -> rightSurround
            expect (map.getDrxChannelForVst3Channel (6)  == 6);  // Lc  -> leftCentre
            expect (map.getDrxChannelForVst3Channel (7)  == 7);  // Rc  -> rightCentre
            expect (map.getDrxChannelForVst3Channel (8)  == 8);  // Sl  -> leftSurroundSide
            expect (map.getDrxChannelForVst3Channel (9)  == 9);  // Sr  -> rightSurroundSide
            expect (map.getDrxChannelForVst3Channel (10) == 10); // Tfl -> topFrontLeft
            expect (map.getDrxChannelForVst3Channel (11) == 11); // Tfr -> topFrontRight
            expect (map.getDrxChannelForVst3Channel (12) == 12); // Trl -> topRearLeft
            expect (map.getDrxChannelForVst3Channel (13) == 13); // Trr -> topRearRight
            expect (map.getDrxChannelForVst3Channel (14) == 14); // Tsl -> topSideLeft
            expect (map.getDrxChannelForVst3Channel (15) == 15); // Tsr -> topSideRight
        }
        ...
        beginTest ("ChannelMapping for a k91_6_W bus remaps the channels appropriately");
        {
            ...
            expect (map.getDrxChannelForVst3Channel (2)  == 2);  // C   -> centre
            expect (map.getDrxChannelForVst3Channel (3)  == 3);  // Lfe -> LFE
            expect (map.getDrxChannelForVst3Channel (4)  == 10); // Ls  -> leftSurroundRear
            expect (map.getDrxChannelForVst3Channel (5)  == 11); // Rs  -> rightSurroundRear
            expect (map.getDrxChannelForVst3Channel (6)  == 4);  // Sl  -> leftSurroundSide
            expect (map.getDrxChannelForVst3Channel (7)  == 5);  // Sr  -> rightSurroundSide
            expect (map.getDrxChannelForVst3Channel (8)  == 6);  // Tfl -> topFrontLeft
            expect (map.getDrxChannelForVst3Channel (9)  == 7);  // Tfr -> topFrontRight
            expect (map.getDrxChannelForVst3Channel (10) == 8);  // Trl -> topRearLeft
            expect (map.getDrxChannelForVst3Channel (11) == 9);  // Trr -> topRearRight
            expect (map.getDrxChannelForVst3Channel (12) == 14); // Tsl -> topSideLeft
            expect (map.getDrxChannelForVst3Channel (13) == 15); // Tsr -> topSideRight
            expect (map.getDrxChannelForVst3Channel (14) == 12); // Lw  -> wideLeft
            expect (map.getDrxChannelForVst3Channel (15) == 13); // Lw  -> wideRight
        }
        ...
        {
            ClientBufferMapperData<f32> remapper;
            ...
            for (const auto& config : { Config { stereoBus, stereoBus }, Config { emptyBuses, stereoBus }, Config { stereoBus, emptyBuses } })
            {
                ...
                {
                    const ClientRemappedBuffer<f32> scopedBuffer { remapper, &config.ins, &config.outs, data };
        ...
        beginTest ("A layout with more output channels than input channels leaves unused inputs untouched");
        {
            ...
            {
                ClientRemappedBuffer<f32> scopedBuffer { remapper, &config.ins, &config.outs, data };
                auto& remapped = scopedBuffer.buffer;
                ...
                // Write some data to the buffer in DRX layout
                for (auto i = 0; i < remapped.getNumChannels(); ++i)
                {
                    auto* ptr = remapped.getWritePointer (i);
                    std::fill (ptr, ptr + remapped.getNumSamples(), (f32) i);
                }
            }
            ...
            expect (channelStartsWithValue (data.outputs[1], 4, 8.0f));  // DRX surround side -> VST3 surround side
            expect (channelStartsWithValue (data.outputs[1], 5, 9.0f));
            expect (channelStartsWithValue (data.outputs[1], 6, 6.0f));  // DRX surround rear -> VST3 surround rear
        ...
        beginTest ("Inactive buses are ignored");
        {
            ...
            // The host doesn't need to provide trailing buses that are inactive, as i64 as the
        ...
        beginTest ("HostBufferMapper reorders channels correctly");
        {
...

            {
                ...
                AudioBuffer<f32> hostBuffer (16, blockSize);
                const auto* clientBuffers = mapper.getVst3LayoutForDrxBuffer (hostBuffer);
...

            {
                mapper.prepare ({ ChannelMapping { AudioChannelSet::mono() },
                                  ...
                                  ChannelMapping { AudioChannelSet::mono(), false } });
                AudioBuffer<f64> hostBuffer (2, blockSize);
                const auto* clientBuffers = mapper.getVst3LayoutForDrxBuffer (hostBuffer);
...
private:
    ...
    struct Config
    {
        ...
        i32 getNumChannels() const { return countUsedClientChannels (ins, outs); }
    };

    struct TestBuffers
    {
        explicit TestBuffers (i32 samples) : numSamples (samples) {}

        z0 init()
        {
            for (const auto [index, channel] : enumerate (buffers, 1))
                std::fill (channel.begin(), channel.end(), (f32) index);
        }

        b8 allMatch (i32 channel, f32 value) const
        {
            const auto& buf = buffers[(size_t) channel];
            return std::all_of (buf.begin(), buf.end(), [&] (auto x) { return exactlyEqual (x, value); });
        }

        b8 isClear (i32 channel) const
        {
        ...
        f32* addChannel()
        {
            ...
            return buffers.back().data();
              ...
              f32* get (i32 channel)       { return buffers[(size_t) channel].data(); }
        const f32* get (i32 channel) const { return buffers[(size_t) channel].data(); }

        std::vector<std::vector<f32>> buffers;
        i32 numSamples = 0;
    };

    static b8 channelStartsWithValue (Steinberg::Vst::AudioBusBuffers& bus, size_t index, f32 value)
    {
        return exactlyEqual (bus.channelBuffers32[index][0], value);
    }

    static b8 allMatch (const AudioBuffer<f32>& buf, i32 index, f32 value)
    {
    ...
    struct MultiBusBuffers
    {
        ...
        std::vector<std::vector<f32*>> pointerStorage;

        MultiBusBuffers withBus (TestBuffers& storage, i32 numChannels) &&
        {
            MultiBusBuffers result { std::move (buffers), std::move (pointerStorage) };

            std::vector<f32*> pointers;

            for (auto i = 0; i < numChannels; ++i)
                pointers.push_back (storage.addChannel());

            Steinberg::Vst::AudioBusBuffers buffer;
            buffer.numChannels = (Steinberg::i32) pointers.size();
    ...
    static Steinberg::Vst::ProcessData makeProcessData (i32 blockSize, MultiBusBuffers& ins, MultiBusBuffers& outs)
    {
        Steinberg::Vst::ProcessData result;
        result.numSamples = blockSize;
        result.inputs = ins.buffers.data();
        result.numInputs = (Steinberg::i32) ins.buffers.size();
        result.outputs = outs.buffers.data();
        result.numOutputs = (Steinberg::i32) outs.buffers.size();
        return result;
    }
};
