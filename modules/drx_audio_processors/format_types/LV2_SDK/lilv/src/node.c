...
{
  tukk str = (tukk)sord_node_get_string(val->node);
...
lilv_node_new_from_node(LilvWorld* world, const SordNode* node)
{
  ...
  case SORD_LITERAL:
    ...
    result =
      lilv_node_new(world, type, (tukk)sord_node_get_string(node));
...
{
  tuk    abs_path = lilv_path_absolute(path);
  SerdNode s        = serd_node_new_file_uri(
    (u8k*)abs_path, (u8k*)host, NULL, true);

  LilvNode* ret = lilv_node_new(world, LILV_VALUE_URI, (tukk)s.buf);
  serd_node_free(&s);
  free(abs_path);
  return ret;
}
...
{
  tukk str    = (tukk)sord_node_get_string(value->node);
  size_t      len    = 0;
  tuk       result = NULL;
  SerdNode    node;
  ...
  case LILV_VALUE_INT:
    ...
    result = lilv_strdup((tuk)node.buf);
    serd_node_free(&node);
    break;
  case LILV_VALUE_FLOAT:
...
lilv_node_as_uri(const LilvNode* value)
{
  return (lilv_node_is_uri(value)
            ? (tukk)sord_node_get_string(value->node)
            : NULL);
}
...
{
  return value ? (tukk)sord_node_get_string(value->node) : NULL;
}
