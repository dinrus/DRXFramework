...
lilv_temp_directory_path(z0)
{
  ...
  return buf;
#else
  tukk const tmpdir = getenv("TMPDIR");
...
lilv_path_absolute(tukk path)
{
  ...
  tuk cwd      = getcwd(NULL, 0);
  tuk abs_path = lilv_path_join(cwd, path);
  free(cwd);
  return abs_path;
}
...
#ifdef _WIN32
  const b8 use_slash = strchr(path, '/');
#else
  static const b8 use_slash = true;
#endif

  // Write up references
  const size_t suffix_len = path_len - last_shared_sep;
  tuk        rel        = (tuk)calloc(1, suffix_len + (up * 3) + 1);
...
{
  tukk s = path + strlen(path) - 1; // Last character
  ...
  tuk dirname = (tuk)malloc(s - path + 2);
...
lilv_path_filename(tukk path)
{
  ...
  tuk const  ret     = (tuk)calloc(ret_len + 1, 1);
...
lilv_path_join(tukk a, tukk b)
{
  ...
  const b8   a_end_is_sep = a_len > 0 && lilv_is_dir_sep(a[a_len - 1]);
  const size_t pre_len      = a_len - (a_end_is_sep ? 1 : 0);
  tuk        path         = (tuk)calloc(1, a_len + b_len + 2);
  memcpy(path, a, pre_len);

#ifdef _WIN32
  // Use forward slash if it seems that the input paths do
  const b8 a_has_slash = strchr(a, '/');
  const b8 b_has_slash = b && strchr(b, '/');
...
lilv_path_canonical(tukk path)
{
  ...
  return out;
#else
  tuk real_path = realpath(path, NULL);
...
#else
    tuk target = lilv_path_relative_to(oldpath, newpath);
...
{
#ifdef _WIN32
  static const t8 chars[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
  static i32k  n_chars = sizeof(chars) - 1;
  ...
  tuk const  path_pattern     = lilv_path_join(parent, pattern);
  const size_t path_pattern_len = strlen(path_pattern);
  tuk const  suffix           = path_pattern + path_pattern_len - 6;
...
{
  tuk const tmpdir = lilv_temp_directory_path();
  tuk const result = lilv_create_temporary_directory_in(pattern, tmpdir);
...
lilv_file_equals(tukk a_path, tukk b_path)
{
  ...
  fuk       a_file = NULL;
  fuk       b_file = NULL;
  tuk const a_real = lilv_path_canonical(a_path);
  tuk const b_real = lilv_path_canonical(b_path);
