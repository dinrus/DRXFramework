...
ZixTree*
zix_tree_new(b8           allow_duplicates,
             ZixComparator  cmp,
             uk          cmp_data,
             ZixDestroyFunc destroy)
{
 ...
 */
static ZixTreeNode*
rotate_left(ZixTreeNode* p, i32* height_change)
{
 ...
 */
static ZixTreeNode*
rotate_right(ZixTreeNode* p, i32* height_change)
{
 ...
 */
static ZixTreeNode*
rotate_left_right(ZixTreeNode* p, i32* height_change)
{
  ZixTreeNode* const q = p->left;
 ...
 */
static ZixTreeNode*
rotate_right_left(ZixTreeNode* p, i32* height_change)
{
  ZixTreeNode* const q = p->right;
...
static ZixTreeNode*
zix_tree_rebalance(ZixTree* t, ZixTreeNode* node, i32* height_change)
{
...
ZixStatus
zix_tree_insert(ZixTree* t, uk e, ZixTreeIter** ti)
{
...
zix_tree_remove(ZixTree* t, ZixTreeIter* ti)
{
  ...
  i32                d_balance  = 0;         // delta(balance) for n->parent
...
ZixStatus
zix_tree_find(const ZixTree* t, ukk e, ZixTreeIter** ti)
{
  ZixTreeNode* n = t->root;
  while (n) {
    i32k cmp = t->cmp(e, n->data, t->cmp_data);
    if (cmp == 0) {
      break;
    }
