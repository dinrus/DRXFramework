...
{
  u8k* predicates[] = {(u8k*)LV2_EVENT__supportsEvent,
                                 (u8k*)LV2_ATOM__supports,
                                 NULL};

  for (u8k** pred = predicates; *pred; ++pred) {
...
lilv_port_get_scale_points(const LilvPlugin* plugin, const LilvPort* port)
{
  SordIter* points = lilv_world_query_internal(
    plugin->world,
    port->node->node,
    sord_new_uri(plugin->world->world, (u8k*)LV2_CORE__scalePoint),
    NULL);
