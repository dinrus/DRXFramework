...
#define NEW_URI(uri) sord_new_uri(world->world, (u8k*)(uri))
  ...
  world->uris.xsd_double          = NEW_URI(LILV_NS_XSD "f64");
...
z0
lilv_world_set_option(LilvWorld* world, tukk uri, const LilvNode* value)
{
...
static SerdNode
lilv_new_uri_relative_to_base(u8k* uri_str,
                              u8k* base_uri_str)
{
  SerdURI base_uri;
                      ...
                      const SordNode* bundle)
{
  ...
  if (plugin) {
    ...
    tukk     plugin_uri_str = lilv_node_as_uri(plugin_uri);
                             ...
                             const LilvNode* manifest)
{
  ...
  for (; !sord_iter_end(iter); sord_iter_next(iter)) {
    ...
    u8k*  lib_uri  = sord_node_get_string(binary);
    tuk           lib_path = lilv_file_uri_parse((tukk)lib_uri, 0);
    if (!lib_path) {
    ...
    // Open library
    dlerror();
    uk lib = dlopen(lib_path, RTLD_LAZY);
    if (!lib) {
      LILV_ERRORF(
    ...
    typedef i32 (*OpenFunc)(LV2_Dyn_Manifest_Handle*,
    ...
    typedef i32 (*GetSubjectsFunc)(LV2_Dyn_Manifest_Handle, fuk);
    ...
    serd_reader_read_file_handle(reader, fd, (u8k*)"(dyn-manifest)");
    serd_reader_free(reader);
    serd_env_free(env);
...
{
  typedef i32 (*CloseFunc)(LV2_Dyn_Manifest_Handle);
...
lilv_world_get_manifest_uri(LilvWorld* world, const LilvNode* bundle_uri)
{
  SerdNode manifest_uri = lilv_new_uri_relative_to_base(
    (u8k*)"manifest.ttl", sord_node_get_string(bundle_uri->node));
  LilvNode* manifest = lilv_new_uri(world, (tukk)manifest_uri.buf);
                  ...
                  const LilvNode* plugin_uri)
{
  ...
  serd_reader_read_file(reader,
                        (u8k*)lilv_node_as_string(manifest_uri));
  ...
  FOREACH_MATCH (f) {
    ...
    u8k*  file_str = sord_node_get_string(file);
...
get_version(LilvWorld* world, SordModel* model, const LilvNode* subject)
{
  ...
  if (minor_node && micro_node) {
    version.minor = atoi((tukk)sord_node_get_string(minor_node));
    version.micro = atoi((tukk)sord_node_get_string(micro_node));
  }
...
lilv_world_load_bundle(LilvWorld* world, const LilvNode* bundle_uri)
{
  ...
  FOREACH_MATCH (plug_results) {
    ...
    i32k cmp = lilv_version_cmp(&this_version, &last_version);
    if (cmp > 0) {
      zix_tree_insert(
...
static z0
load_dir_entry(tukk dir, tukk name, uk data)
{
  LilvWorld* world = (LilvWorld*)data;
  tuk      path  = lilv_strjoin(dir, "/", name, "/", NULL);
  SerdNode   suri  = serd_node_new_file_uri((u8k*)path, 0, 0, true);
  LilvNode*  node  = lilv_new_uri(world, (tukk)suri.buf);
...
static z0
lilv_world_load_directory(LilvWorld* world, tukk dir_path)
{
  tuk path = lilv_expand(dir_path);
  if (path) {
...
static tukk
first_path_sep(tukk path)
{
  for (tukk p = path; *p != '\0'; ++p) {
 ...
 */
static z0
lilv_world_load_path(LilvWorld* world, tukk lv2_path)
{
  while (lv2_path[0] != '\0') {
    tukk const sep = first_path_sep(lv2_path);
    if (sep) {
      const size_t dir_len = sep - lv2_path;
      tuk const  dir     = (tuk)malloc(dir_len + 1);
  ...
  */
  ...
  FOREACH_MATCH (classes) {
    ...
    LilvPluginClass* pclass = lilv_plugin_class_new(
      world, parent, class_node, (tukk)sord_node_get_string(label));
    if (pclass) {
...
{
  tukk lv2_path = world->opt.lv2_path;
  if (!lv2_path) {
...
lilv_world_load_file(LilvWorld* world, SerdReader* reader, const LilvNode* uri)
{
  ...
  if (strncmp((tukk)uri_str, "file:", 5)) {
    return SERD_FAILURE; // Not a local file
  }

  if (strcmp((tukk)uri_str + uri_len - 4, ".ttl")) {
...
lilv_world_get_symbol(LilvWorld* world, const LilvNode* subject)
{
  ...
  SerdURI uri;
  serd_uri_parse((u8k*)lilv_node_as_uri(subject), &uri);
  tukk str = "_";
  if (uri.fragment.buf) {
    str = (tukk)uri.fragment.buf + 1;
  } else if (uri.query.buf) {
    str = (tukk)uri.query.buf;
  } else if (uri.path.buf) {
    tukk last_slash = strrchr((tukk)uri.path.buf, '/');
    str = last_slash ? (last_slash + 1) : (tukk)uri.path.buf;
  }

  // Replace invalid characters
  const size_t len = strlen(str);
  tuk const  sym = (tuk)calloc(1, len + 1);
