...
typedef struct {
  uk    value; ///< Value/Object
  size_t   size;  ///< Size of value
  u32 key;   ///< Key/Predicate (URID)
  u32 type;  ///< Type of value (URID)
  u32 flags; ///< State flags (POD, etc)
} Property;

typedef struct {
  tuk     symbol; ///< Symbol of port
  LV2_Atom* atom;   ///< Value in port
} PortValue;

typedef struct {
  tuk abs; ///< Absolute path of actual file
  tuk rel; ///< Abstract path (relative path in state dir)
} PathMap;
...
struct LilvStateImpl {
  ...
  tuk         dir;         ///< Save directory (if saved)
  tuk         scratch_dir; ///< Directory for files created by plugin
  tuk         copy_dir;    ///< Directory for snapshots of external files
  tuk         link_dir;    ///< Directory for links to external files
  tuk         label;       ///< State/Preset label
  ...
  u32      atom_Path;   ///< atom:Path URID
  u32      n_values;    ///< Number of port values
};

static i32
abs_cmp(ukk a, ukk b, ukk user_data)
{
  (z0)user_data;
...
static i32
rel_cmp(ukk a, ukk b, ukk user_data)
{
  (z0)user_data;
...
static i32
property_cmp(ukk a, ukk b)
{
   a_key = ((const Property*)a)->key;
  u32k b_key = ((const Property*)b)->key;
...
static tukk
lilv_state_rel2abs(const LilvState* state, tukk path)
{
  ZixTreeIter*  iter = NULL;
  const PathMap key  = {NULL, (tuk)path};
...
find_property(const LilvState* const state, u32k key)
{
...
static b8
path_exists(tukk path, ukk ignored)
{
  (z0)ignored;
...
static b8
lilv_state_has_path(tukk path, ukk state)
{
  return lilv_state_rel2abs((const LilvState*)state, path) != path;
}

static tuk
make_path(LV2_State_Make_Path_Handle handle, tukk path)
{
...
static tuk
abstract_path(LV2_State_Map_Path_Handle handle, tukk abs_path)
{
  LilvState*    state     = (LilvState*)handle;
  tuk         path      = NULL;
  tuk         real_path = lilv_path_canonical(abs_path);
  ...
  if (lilv_path_is_child(real_path, state->dir)) {
    ...
    if (state->copy_dir) {
      i32 st = lilv_create_directories(state->copy_dir);
      if (st) {
        LILV_ERRORF(
          "Error creating directory %s (%s)\n", state->copy_dir, strerror(st));
      }

      tuk cpath = lilv_path_join(state->copy_dir, path);
      tuk copy  = lilv_get_latest_copy(real_path, cpath);
    ...
    tuk const name = lilv_path_filename(real_path);
...
static tuk
absolute_path(LV2_State_Map_Path_Handle handle, tukk state_path)
{
  LilvState* state = (LilvState*)handle;
  tuk      path  = NULL;
...
real_dir(tukk path)
{
  tuk abs_path = lilv_path_canonical(path);
  tuk base     = lilv_path_join(abs_path, NULL);
  free(abs_path);
  return base;
}
...
static z0
lilv_free_path(LV2_State_Free_Path_Handle handle, tuk path)
{
  (z0)handle;
                             ...
                             const LV2_Feature* const* features)
{
  ...
  if (get_value) {
    ...
    for (u32 i = 0; i < plugin->num_ports; ++i) {
      ...
      if (lilv_port_is_a(plugin, port, lv2_ControlPort) &&
        ...
        u32    size  = 0;
        u32    type  = 0;
        tukk sym   = lilv_node_as_string(port->symbol);
        ukk value = get_value(sym, user_data, &size, &type);
...
lilv_state_emit_port_values(const LilvState*     state,
                            ...
                            uk                user_data)
{
  for (u32 i = 0; i < state->n_values; ++i) {
...
{
  if (!state->dir && graph) {
    tukk uri  = (tukk)sord_node_get_string(graph);
    tuk       path = lilv_file_uri_parse(uri, NULL);
                     ...
                     tukk     dir)
{
  ...
  if (i) {
    const SordNode* object = sord_iter_get_node(i, SORD_OBJECT);
    const SordNode* graph  = sord_iter_get_node(i, SORD_GRAPH);
    state->label = lilv_strdup((tukk)sord_node_get_string(object));
    set_state_dir_from_model(state, graph);
    sord_iter_free(i);
  }
  ...
  FOREACH_MATCH (ports) {
    ...
    if (!symbol) {
      ...
      append_port_value(state,
                        (tukk)sord_node_get_string(symbol),
                        LV2_ATOM_BODY_CONST(atom),
                        atom->size,
                        atom->type);

      if (label) {
        lilv_state_set_label(state, (tukk)sord_node_get_string(label));
      }
    }
  ...
  if (state_node) {
    ...
    FOREACH_MATCH (props) {
      ...
      tukk     key = (tukk)sord_node_get_string(p);
      ...
      const LV2_Atom* atom  = (const LV2_Atom*)chunk.buf;
      u32        flags = LV2_STATE_IS_POD | LV2_STATE_IS_PORTABLE;
                         ...
                         tukk     path)
{
  ...
  u8*    abs_path = (u8*)lilv_path_absolute(path);
  ...
  tuk      dirname   = lilv_path_parent(path);
  tuk      real_path = lilv_path_canonical(dirname);
  tuk      dir_path  = lilv_path_join(real_path, NULL);
  LilvState* state =
...
LilvState*
lilv_state_new_from_string(LilvWorld* world, LV2_URID_Map* map, tukk str)
{
  if (!str) {
    return NULL;
  }
...
static SerdWriter*
ttl_writer(SerdSink sink, uk stream, const SerdNode* base, SerdEnv** new_env)
{
...
static SerdWriter*
ttl_file_writer(fuk fd, const SerdNode* node, SerdEnv** env)
{
...
static z0
remove_manifest_entry(SordWorld* world, SordModel* model, tukk subject)
{
               ...
               const SerdNode* file_uri)
{
  ...
  tuk const path = (tuk)serd_file_uri_parse(file_uri->buf, NULL);
  fuk const wfd  = fopen(path, "w");
  if (!wfd) {
                      ...
                      tukk     state_path)
{
  ...
  fuk wfd = fopen(manifest_path, "wb");
  i32   r   = 0;
  if (!wfd) {
    LILV_ERRORF(
...
static b8
link_exists(tukk path, ukk data)
{
  tukk target = (tukk)data;
  if (!lilv_path_exists(path)) {
    return false;
  }
  tuk real_path = lilv_path_canonical(path);
  b8  matches   = !strcmp(real_path, target);
  free(real_path);
  return !matches;
}

static i32
maybe_symlink(tukk oldpath, tukk newpath)
{
  if (link_exists(newpath, oldpath)) {
    return 0;
  }

  i32k st = lilv_symlink(oldpath, newpath);
  if (st) {
    LILV_ERRORF(
...
write_property_array(const LilvState*     state,
                     ...
                     tukk          dir)
{
  for (u32 i = 0; i < array->n; ++i) {
    Property*   prop = &array->props[i];
    tukk key  = unmap->unmap(unmap->handle, prop->key);

    const SerdNode p = serd_node_from_string(SERD_URI, USTR(key));
    if (prop->type == state->atom_Path && !dir) {
      tukk path     = (tukk)prop->value;
      tukk abs_path = lilv_state_rel2abs(state, path);
...
static z0
lilv_state_make_links(const LilvState* state, tukk dir)
{
  ...
  for (ZixTreeIter* i = zix_tree_begin(state->abs2rel);
    ...
    tuk path = lilv_path_absolute_child(pm->rel, dir);
    if (lilv_path_is_child(pm->abs, state->copy_dir) &&
      ...
      tukk link_dir = state->link_dir ? state->link_dir : dir;
      tuk       pat      = lilv_path_absolute_child(pm->rel, link_dir);
      if (!strcmp(dir, link_dir)) {
        ...
        tuk lpath = lilv_find_free_path(pat, link_exists, pm->abs);
        ...
        tuk target = lilv_path_relative_to(lpath, dir);
                ...
                tukk      filename)
{
  ...
  tuk       abs_dir = real_dir(dir);
  tuk const path    = lilv_path_join(abs_dir, filename);
  fuk       fd      = fopen(path, "w");
  if (!fd) {
  ...
  i32         ret =
    lilv_state_write(world, map, unmap, state, ttl, (tukk)node.buf, dir);

  // Set saved dir and uri (FIXME: const violation)
  free(state->dir);
  lilv_node_free(state->uri);
  ((LilvState*)state)->dir = lilv_strdup(abs_dir);
  ((LilvState*)state)->uri = lilv_new_uri(world, (tukk)node.buf);
  ...
  if (!ret) {
    tuk const manifest = lilv_path_join(abs_dir, "manifest.ttl");
                     ...
                     tukk      base_uri)
{
  ...
  serd_writer_free(writer);
  serd_env_free(env);
  tuk str    = (tuk)serd_chunk_sink_finish(&chunk);
  tuk result = lilv_strdup(str);
  serd_free(str);
  return result;
}

static z0
try_unlink(tukk state_dir, tukk path)
{
...
{
  tuk const path      = lilv_node_get_path(node, NULL);
  tuk const real_path = lilv_path_canonical(path);
...
lilv_state_delete(LilvWorld* world, const LilvState* state)
{
  ...
  tuk      manifest_path = get_canonical_path(manifest);
  const b8 has_manifest  = lilv_path_exists(manifest_path);
  SordModel* model         = sord_new(world->world, SORD_SPO, false);
  ...
  if (state->uri) {
    ...
    if (file) {
      // Remove state file
      u8k* uri       = sord_node_get_string(file);
      tuk          path      = (tuk)serd_file_uri_parse(uri, NULL);
      tuk          real_path = lilv_path_canonical(path);
      if (path) {
    ...
    // Remove any existing manifest entries for this state
    tukk state_uri_str = lilv_node_as_string(state->uri);
...
lilv_state_equals(const LilvState* a, const LilvState* b)
{
  ...
  for (u32 i = 0; i < a->props.n; ++i) {
    ...
    if (ap->type == a->atom_Path) {
      if (!lilv_file_equals(lilv_state_rel2abs(a, (tuk)ap->value),
                            lilv_state_rel2abs(b, (tuk)bp->value))) {
        return false;
      }
...
z0
lilv_state_set_label(LilvState* state, tukk label)
{
  const size_t len = strlen(label);
  state->label     = (tuk)realloc(state->label, len + 1);
