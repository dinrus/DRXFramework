...
#define NS_DOAP (u8k*)"http://usefulinc.com/ns/doap#"
...
lilv_plugin_load(LilvPlugin* plugin)
{
  ...
  if (plugin->dynmanifest) {
    typedef i32 (*GetDataFunc)(
      LV2_Dyn_Manifest_Handle handle, FILE * fp, tukk uri);
  ...
  if (!plugin->ports) {
    ...
    FOREACH_MATCH (ports) {
      ...
      if (!lilv_node_is_string(symbol) ||
          !is_symbol((tukk)sord_node_get_string(symbol->node))) {
      ...
      u32  this_index = lilv_node_as_int(index);
                                   ...
                                   ...)
{
  ...
  u32 count = lilv_plugin_get_num_ports_of_class_va(plugin, class_1, args);
...
lilv_plugin_has_latency(const LilvPlugin* plugin)
{
  ...
  FOREACH_MATCH (ports) {
    ...
    const b8 latent = !sord_iter_end(prop) || !sord_iter_end(des);
    sord_iter_free(prop);
    sord_iter_free(des);
    if (latent) {
      ret = true;
                                 ...
                                 const SordNode*   port_property)
{
  ...
  for (u32 i = 0; i < plugin->num_ports; ++i) {
    ...
    const b8 found = !sord_iter_end(iter);
    sord_iter_free(iter);
...
lilv_plugin_get_project(const LilvPlugin* plugin)
{
  ...
  SordNode* lv2_project =
    sord_new_uri(plugin->world->world, (u8k*)LV2_CORE__project);
...
static LilvNode*
lilv_plugin_get_author_property(const LilvPlugin* plugin, u8k* uri)
{
...
lilv_plugin_get_uis(const LilvPlugin* plugin)
{
  ...
  SordNode* ui_ui_node =
    sord_new_uri(plugin->world->world, (u8k*)LV2_UI__ui);
  SordNode* ui_binary_node =
    sord_new_uri(plugin->world->world, (u8k*)LV2_UI__binary);
...
static z0
maybe_write_prefixes(SerdWriter* writer, SerdEnv* env, fuk file)
{
                              ...
                              fuk             plugin_file)
{
  ...
  u32k  num_ports = lilv_plugin_get_num_ports(plugin);
                                 ...
                                 tukk       plugin_file_path)
{
  ...
  const SerdNode file_node =
    serd_node_from_string(SERD_URI, (u8k*)plugin_file_path);
  serd_writer_write_statement(
    writer,
    0,
    NULL,
