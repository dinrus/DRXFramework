              ...
              const LV2_Feature* const* features)
{
  ZixTreeIter*  i   = NULL;
  const LilvLib key = {
    world, (LilvNode*)uri, (tuk)bundle_path, NULL, NULL, NULL, 0};
  ...
  tukk const lib_uri = lilv_node_as_uri(uri);
  tuk const       lib_path =
    (tuk)serd_file_uri_parse((u8k*)lib_uri, NULL);
  if (!lib_path) {
    return NULL;
  }

  dlerror();
  uk lib = dlopen(lib_path, RTLD_NOW);
  if (!lib) {
