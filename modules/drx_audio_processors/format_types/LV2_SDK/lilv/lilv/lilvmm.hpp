...
#define LILV_WRAP0_VOID(prefix, name) \
  inline z0 name() { lilv_##prefix##_##name(me); }
...
#define LILV_WRAP1_VOID(prefix, name, T1, a1) \
  inline z0 name(T1 a1) { lilv_##prefix##_##name(me, a1); }
...
#define LILV_WRAP2_VOID(prefix, name, T1, a1, T2, a2) \
  inline z0 name(T1 a1, T2 a2) { lilv_##prefix##_##name(me, a1, a2); }
...
struct Node {
  ...
  inline b8 equals(const Node& other) const
  {
    return lilv_node_equals(me, other.me);
  }

  inline b8 operator==(const Node& other) const { return equals(other); }

  LILV_WRAP_CONVERSION(LilvNode);

  LILV_WRAP0(tuk, node, get_turtle_token);
  LILV_WRAP0(b8, node, is_uri);
  LILV_WRAP0(tukk, node, as_uri);
  LILV_WRAP0(b8, node, is_blank);
  LILV_WRAP0(tukk, node, as_blank);
  LILV_WRAP0(b8, node, is_literal);
  LILV_WRAP0(b8, node, is_string);
  LILV_WRAP0(tukk, node, as_string);
  LILV_WRAP0(b8, node, is_float);
  LILV_WRAP0(f32, node, as_float);
  LILV_WRAP0(b8, node, is_int);
  LILV_WRAP0(i32, node, as_int);
  LILV_WRAP0(b8, node, is_bool);
  LILV_WRAP0(b8, node, as_bool);
...
struct Nodes {
  ...
  LILV_WRAP1(b8, nodes, contains, const Node&, node);
...
struct UI {
  ...
  /*LILV_WRAP3(b8, ui, is_supported,
             LilvUISupportedFunc, supported_func,
             const LilvNode*,     container_type,
             const LilvNode**,    ui_type);*/
  LILV_WRAP1(b8, ui, is_a, const LilvNode*, class_uri);
...
#define LILV_PORT_WRAP1(RT, name, T1, a1) \
  ...
  LILV_PORT_WRAP1(b8, has_property, LilvNode*, property_uri);
  LILV_PORT_WRAP1(b8, supports_event, LilvNode*, event_uri);
  LILV_PORT_WRAP0(const LilvNode*, get_symbol);
  LILV_PORT_WRAP0(LilvNode*, get_name);
  LILV_PORT_WRAP0(const LilvNodes*, get_classes);
  LILV_PORT_WRAP1(b8, is_a, LilvNode*, port_class);
...
struct Plugin {
  ...
  LILV_WRAP1(b8, plugin, has_feature, const Node&, feature_uri);
  ...
  LILV_WRAP0(u32, plugin, get_num_ports);
  LILV_WRAP0(b8, plugin, has_latency);
  LILV_WRAP0(u32, plugin, get_latency_port_index);
  LILV_WRAP0(Node, plugin, get_author_name);
  LILV_WRAP0(Node, plugin, get_author_email);
  LILV_WRAP0(Node, plugin, get_author_homepage);
  LILV_WRAP0(b8, plugin, is_replaced);
  ...
  inline Port get_port_by_index(u32 index) const
  {
  ...
  inline z0 get_port_ranges_float(f32* min_values,
                                    f32* max_values,
                                    f32* def_values) const
  {
    return lilv_plugin_get_port_ranges_float(
      me, min_values, max_values, def_values);
  }

  inline u32 get_num_ports_of_class(LilvNode* class_1, ...) const
  {
    va_list args;
...
struct Instance {
  ...
  LILV_DEPRECATED
  inline Instance(Plugin plugin, f64 sample_rate)
  {
  ...
  LILV_WRAP1_VOID(instance, run, u32, sample_count);
  LILV_WRAP0_VOID(instance, deactivate);

  inline ukk get_extension_data(tukk uri) const
  {
...
struct World {
  ...
  inline LilvNode* new_uri(tukk uri) const
  {
    return lilv_new_uri(me, uri);
  }

  inline LilvNode* new_string(tukk str) const
  {
    return lilv_new_string(me, str);
  }

  inline LilvNode* new_int(i32 val) const { return lilv_new_int(me, val); }

  inline LilvNode* new_float(f32 val) const
  {
    return lilv_new_float(me, val);
  }

  inline LilvNode* new_bool(b8 val) const { return lilv_new_bool(me, val); }
  ...
  LILV_WRAP2_VOID(world, set_option, tukk, uri, LilvNode*, value);
  ...
  LILV_WRAP1(i32, world, load_resource, const LilvNode*, resource);
