...
typedef z0 LilvIter;          /**< Collection iterator */
typedef z0 LilvPluginClasses; /**< A set of #LilvPluginClass. */
typedef z0 LilvPlugins;       /**< A set of #LilvPlugin. */
typedef z0 LilvScalePoints;   /**< A set of #LilvScalePoint. */
typedef z0 LilvUIs;           /**< A set of #LilvUI. */
typedef z0 LilvNodes;         /**< A set of #LilvNode. */
...
LILV_API
LilvNode*
lilv_new_uri(LilvWorld* world, tukk uri);
...
LILV_API
LilvNode*
lilv_new_file_uri(LilvWorld* world, tukk host, tukk path);
...
LILV_API
LilvNode*
lilv_new_string(LilvWorld* world, tukk str);
...
LILV_API
LilvNode*
lilv_new_int(LilvWorld* world, i32 val);
...
LILV_API
LilvNode*
lilv_new_float(LilvWorld* world, f32 val);
...
LILV_API
LilvNode*
lilv_new_bool(LilvWorld* world, b8 val);
...
/**
   ...
   Возвращает true, если `value` is a string or numeric value.
*/
LILV_API
b8
...
/**
   ...
   Возвращает true, если `value` is a string value (and not numeric).
*/
LILV_API
b8
...
LILV_API
tuk
lilv_node_get_path(const LilvNode* value, tuk* hostname);
...
/**
   ...
   Hosts MUST NOT attach any i64-term significance to bundle paths
...
/**
   ...
   minimum, maximum or default value, or the port's type is not f32, the
   corresponding array element will be set to NAN.

   This is a convenience method for the common case of getting the range of
   all f32 ports on a plugin, and may be significantly faster than
...
LILV_API
const LilvPort*
lilv_plugin_get_port_by_index(const LilvPlugin* plugin, u32 index);
...
*/
typedef ukk (*LilvGetPortValueFunc)(tukk port_symbol,
                                            uk       user_data,
                                            u32*   size,
                                            u32*   type);
...
*/
typedef z0 (*LilvSetPortValueFunc)(tukk port_symbol,
                                     uk       user_data,
                                     ukk value,
                                     u32    size,
                                     u32    type);
...
*/
static inline z0
lilv_instance_run(LilvInstance* instance, u32 sample_count)
{
...
*/
static inline ukk
lilv_instance_get_extension_data(const LilvInstance* instance, tukk uri)
{
...
*/
typedef u32 (*LilvUISupportedFunc)(tukk container_type_uri,
                                        tukk ui_type_uri);
