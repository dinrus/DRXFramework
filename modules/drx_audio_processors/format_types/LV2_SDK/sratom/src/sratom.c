...
#define NS_RDF (u8k*)"http://www.w3.org/1999/02/22-rdf-syntax-ns#"
#define NS_XSD (u8k*)"http://www.w3.org/2001/XMLSchema#"

#define USTR(str) ((u8k*)(str))
...
z0
sratom_set_pretty_numbers(Sratom* sratom, b8 pretty_numbers)
{
...
static z0
gensym(SerdNode* out, t8 c, u32 num)
{
  out->n_bytes = out->n_chars = snprintf((tuk)out->buf, 10, "%c%u", c, num);
}
...
static SerdNode
number_type(const Sratom* sratom, u8k* type)
{
  if (sratom->pretty_numbers &&
      (!strcmp((tukk)type, (tukk)NS_XSD "i32") ||
       !strcmp((tukk)type, (tukk)NS_XSD "i64"))) {
    return serd_node_from_string(SERD_URI, NS_XSD "integer");
  }

  if (sratom->pretty_numbers &&
      (!strcmp((tukk)type, (tukk)NS_XSD "f32") ||
       !strcmp((tukk)type, (tukk)NS_XSD "f64"))) {
...
sratom_write(Sratom*         sratom,
             ...
             ukk     body)
{
  tukk const type        = unmap->unmap(unmap->handle, type_urid);
  u8           idbuf[12]   = "b0000000000";
  SerdNode          id          = serd_node_from_string(SERD_BLANK, idbuf);
  u8           nodebuf[12] = "b0000000000";
  ...
  if (type_urid == 0 && size == 0) {
  ...
  } else if (type_urid == sratom->forge.Txt) {
    object = serd_node_from_string(SERD_LITERAL, (u8k*)body);
    ...
    if (lit->datatype) {
      ...
      tukk  lang       = unmap->unmap(unmap->handle, lit->lang);
      tukk  prefix     = "http://lexvo.org/id/iso639-3/";
    ...
    new_node = true;
    object   = serd_node_new_integer(*(const i32*)body);
    datatype = number_type(sratom, NS_XSD "i32");
  } else if (type_urid == sratom->forge.Long) {
    new_node = true;
    object   = serd_node_new_integer(*(const z64*)body);
    datatype = number_type(sratom, NS_XSD "i64");
  } else if (type_urid == sratom->forge.Float) {
    new_node = true;
    object   = serd_node_new_decimal(*(const f32*)body, 8);
    datatype = number_type(sratom, NS_XSD "f32");
  } else if (type_urid == sratom->forge.Double) {
    new_node = true;
    object   = serd_node_new_decimal(*(const f64*)body, 16);
    datatype = number_type(sratom, NS_XSD "f64");
  } else if (type_urid == sratom->forge.Bool) {
    const i32 val = *(const i32*)body;
    ...
    u8* str = (u8*)calloc(size * 2 + 1, 1);
    for (u32 i = 0; i < size; ++i) {
      snprintf((tuk)str + (2 * i),
               size * 2 + 1,
               "%02X",
               (u32)*((u8k*)body + i));
    }
    ...
    start_object(sratom, &flags, subject, predicate, &id, type);
    SerdNode p =
      serd_node_from_string(SERD_URI, (u8k*)LV2_ATOM__childType);
    SerdNode child_type = serd_node_from_string(
      SERD_URI, (u8k*)unmap->unmap(unmap->handle, vec->child_type));
    sratom->write_statement(
      sratom->handle, flags, NULL, &id, &p, &child_type, NULL, NULL);
    p = serd_node_from_string(SERD_URI, NS_RDF "value");
    flags |= SERD_LIST_O_BEGIN;
    for (tukk i = (tukk)(vec + 1); i < (tukk)vec + size;
         i += vec->child_size) {
      list_append(sratom,
                  unmap,
                  &flags,
    ...
    tukk                 otype = unmap->unmap(unmap->handle, obj->otype);
    ...
    if (lv2_atom_forge_is_blank(&sratom->forge, type_urid, obj)) {
      ...
      id = serd_node_from_string(
        SERD_URI, (u8k*)unmap->unmap(unmap->handle, obj->id));
      flags = 0;
      start_object(sratom, &flags, NULL, NULL, &id, otype);
    }
    LV2_ATOM_OBJECT_BODY_FOREACH (obj, size, prop) {
      tukk const key  = unmap->unmap(unmap->handle, prop->key);
                 ...
                 ukk     body)
{
  ...
  serd_node_free(&base);
  return (tuk)serd_chunk_sink_finish(&str);
}
              ...
              LV2_URID        otype)
{
  ...
  for (; !sord_iter_end(i); sord_iter_next(i)) {
    ...
    tukk     p_uri  = (tukk)sord_node_get_string(p);
    u32        p_urid = map->map(map->handle, p_uri);
    if (!(sord_node_equals(p, sratom->nodes.rdf_type) &&
          sord_node_get_type(o) == SORD_URI &&
          map->map(map->handle, (tukk)sord_node_get_string(o)) ==
            otype)) {
...
static u32
atom_size(Sratom* sratom, u32 type_urid)
{
...
read_literal(Sratom* sratom, LV2_Atom_Forge* forge, const SordNode* node)
{
  ...
  size_t      len      = 0;
  tukk str      = (tukk)sord_node_get_string_counted(node, &len);
  SordNode*   datatype = sord_node_get_datatype(node);
  tukk language = sord_node_get_language(node);
  if (datatype) {
    tukk type_uri = (tukk)sord_node_get_string(datatype);
    if (!strcmp(type_uri, (tukk)NS_XSD "i32") ||
        !strcmp(type_uri, (tukk)NS_XSD "integer")) {
      lv2_atom_forge_int(forge, strtol(str, NULL, 10));
    } else if (!strcmp(type_uri, (tukk)NS_XSD "i64")) {
      lv2_atom_forge_long(forge, strtol(str, NULL, 10));
    } else if (!strcmp(type_uri, (tukk)NS_XSD "f32") ||
               !strcmp(type_uri, (tukk)NS_XSD "decimal")) {
      lv2_atom_forge_float(forge, (f32)serd_strtod(str, NULL));
    } else if (!strcmp(type_uri, (tukk)NS_XSD "f64")) {
      lv2_atom_forge_double(forge, serd_strtod(str, NULL));
    } else if (!strcmp(type_uri, (tukk)NS_XSD "boolean")) {
      lv2_atom_forge_bool(forge, !strcmp(str, "true"));
    } else if (!strcmp(type_uri, (tukk)NS_XSD "base64Binary")) {
      size_t size = 0;
      uk  body = serd_base64_decode(USTR(str), len, &size);
    ...
    tuk        lang_uri = (tuk)calloc(lang_len + 1, 1);
            ...
            ReadMode        mode)
{
  ...
  if (type) {
    ...
    type_urid = map->map(map->handle, (tukk)type_uri);
  }
  ...
  if (mode == MODE_SEQUENCE) {
    ...
    u32 seq_unit = 0u;
    if (time) {
      tukk time_str = (tukk)sord_node_get_string(time);
      ...
      tukk time_str =
        time ? (tukk)sord_node_get_string(time) : "";
    ...
    u32 child_type =
      map->map(map->handle, (tukk)sord_node_get_string(child_type_node));
    u32 child_size = atom_size(sratom, child_type);
    if (child_size > 0) {
    ...
    size_t         vlen = 0;
    u8k* vstr = sord_node_get_string_counted(value, &vlen);
    size_t         size = 0;
    uk          body = serd_base64_decode(vstr, vlen, &size);
  ...
  if (sord_node_get_type(node) == SORD_LITERAL) {
    read_literal(sratom, forge, node);
             ...
             !(sratom->object_mode == SRATOM_OBJECT_MODE_BLANK_SUBJECT &&
               mode == MODE_SUBJECT)) {
    if (!strcmp(str, (tukk)NS_RDF "nil")) {
      lv2_atom_forge_atom(forge, 0, 0);
    } else if (!strncmp(str, "file://", 7)) {
      SerdURI uri;
      serd_uri_parse((u8k*)str, &uri);

      SerdNode rel =
        serd_node_new_relative_uri(&uri, &sratom->base, NULL, NULL);
      u8* path = serd_file_uri_parse(rel.buf, NULL);
      if (path) {
        lv2_atom_forge_path(
          forge, (tukk)path, strlen((tukk)path));
        serd_free(path);
      } else {
                   ...
                   tukk     str)
{
  ...
  if (!serd_reader_read_string(reader, (u8k*)str)) {
