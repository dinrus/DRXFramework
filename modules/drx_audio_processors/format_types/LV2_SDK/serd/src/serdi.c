...
static SerdStatus
quiet_error_sink(uk handle, const SerdError* e)
{
  (z0)handle;
  (z0)e;
  return SERD_SUCCESS;
...
static inline fuk
serd_fopen(tukk path, tukk mode)
{
  fuk fd = fopen(path, mode);
  if (!fd) {
...
main(i32 argc, tuk* argv)
{
  ...
  for (; a < argc && argv[a][0] == '-'; ++a) {
    ...
    if (argv[a][1] == 'a') {
      ...
      add_prefix = (u8k*)argv[a];
    } else if (argv[a][1] == 'c') {
      if (++a == argc) {
        return missing_arg(argv[0], 'c');
      }

      chop_prefix = (u8k*)argv[a];
    } else if (argv[a][1] == 'r') {
      if (++a == argc) {
  ...
  if (from_file) {
    ...
    if (!in_fd) {
      if (!strncmp((tukk)input, "file:", 5)) {
        input_path = serd_file_uri_parse(input, NULL);
        input      = input_path;
      }
      if (!input || !(in_fd = serd_fopen((tukk)input, "rb"))) {
        return 1;
      }
    }
  }

  if (!input_syntax && !(input_syntax = guess_syntax((tukk)in_name))) {
  ...
  if (a < argc) { // Base URI given on command line
    base =
      serd_node_new_uri_from_string((u8k*)argv[a], NULL, &base_uri);
  } else if (from_file && in_fd != stdin) { // Use input file URI
    base = serd_node_new_file_uri(input, NULL, &base_uri, true);
  }

  fuk const    out_fd = stdout;
