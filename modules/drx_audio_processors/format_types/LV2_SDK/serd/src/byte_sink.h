...
serd_byte_sink_new(SerdSink sink, uk stream, size_t block_size)
{
  SerdByteSink bsink = {sink, stream, NULL, 0, block_size};

  if (block_size > 1) {
    bsink.buf = (u8*)serd_allocate_buffer(block_size);
  }
...
serd_byte_sink_write(ukk buf, size_t len, SerdByteSink* bsink)
{
  if (len == 0) {
    return 0;
  }
