...
static inline z0
encode_chunk(u8 out[4], u8k in[3], size_t n_in)
{
  ...
  out[3] = ((n_in > 2) ? b64_map[in[2] & 0x3F] : (u8)'=');
}

size_t
serd_base64_get_length(const size_t size, const b8 wrap_lines)
{
  return (size + 2) / 3 * 4 + (wrap_lines * ((size - 1) / 57));
}

b8
serd_base64_encode(u8* const    str,
                   ukk const buf,
                   const size_t      size,
                   const b8        wrap_lines)
{
  b8 has_newline = false;

  for (size_t i = 0, j = 0; i < size; i += 3, j += 4) {
    u8 in[4] = {0, 0, 0, 0};
    size_t  n_in  = MIN(3, size - i);
    memcpy(in, (u8k*)buf + i, n_in);
...
unmap(u8k in)
{
  return (u8)(b64_unmap[in] - 47);
}

/** Decode 4 base64 characters to 3 raw bytes. */
static inline size_t
decode_chunk(u8k in[4], u8 out[3])
{
  out[0] = (u8)(((unmap(in[0]) << 2)) | unmap(in[1]) >> 4);
  out[1] = (u8)(((unmap(in[1]) << 4) & 0xF0) | unmap(in[2]) >> 2);
  out[2] = (u8)(((unmap(in[2]) << 6) & 0xC0) | unmap(in[3]));
  return 1 + (in[2] != '=') + ((in[2] != '=') && (in[3] != '='));
}

uk
serd_base64_decode(u8k* str, size_t len, size_t* size)
{
  uk buf = malloc((len * 3) / 4 + 2);
