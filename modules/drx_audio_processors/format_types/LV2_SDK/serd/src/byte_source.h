...
typedef struct {
  ...
  uk               stream;      ///< Stream (e.g. FILE)
  size_t              page_size;   ///< Number of bytes to read at a time
  size_t              buf_size;    ///< Number of bytes in file_buf
  Cursor              cur;         ///< Cursor for error reporting
  u8*            file_buf;    ///< Buffer iff reading pages from a file
  u8k*      read_buf;    ///< Pointer to file_buf or read_byte
  size_t              read_head;   ///< Offset into read_buf
  u8             read_byte;   ///< 1-byte 'buffer' used when not paging
  b8                from_stream; ///< True iff reading from `stream`
  b8                prepared;    ///< True iff prepared for reading
  b8                eof;         ///< True iff end of file reached
} SerdByteSource;

SerdStatus
serd_byte_source_open_file(SerdByteSource* source, fuk file, b8 bulk);

SerdStatus
serd_byte_source_open_string(SerdByteSource* source, u8k* utf8);
...
static inline SERD_PURE_FUNC u8
