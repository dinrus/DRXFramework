...
serd_env_find(const SerdEnv* env, u8k* name, size_t name_len)
{
  for (size_t i = 0; i < env->n_prefixes; ++i) {
...
{
  u8k* const colon =
    (u8k*)memchr(curie->buf, ':', curie->n_bytes + 1);
...
serd_env_expand_node(const SerdEnv* env, const SerdNode* node)
{
  ...
  case SERD_CURIE: {
    ...
    snprintf((tuk)buf, len + 1, "%s%s", prefix.buf, suffix.buf);
...
z0
serd_env_foreach(const SerdEnv* env, SerdPrefixSink func, uk handle)
{
  for (size_t i = 0; i < env->n_prefixes; ++i) {
