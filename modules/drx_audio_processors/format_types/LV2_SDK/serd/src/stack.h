...
#define SERD_STACK_BOTTOM sizeof(uk)
...
serd_stack_push(SerdStack* stack, size_t n_bytes)
{
  ...
  if (stack->buf_size < new_size) {
    ...
    stack->buf = (u8*)realloc(stack->buf, stack->buf_size);
  }

  u8* const ret = (stack->buf + stack->size);
...
serd_stack_pop_aligned(SerdStack* stack, size_t n_bytes)
{
  ...
  u8k pad = stack->buf[stack->size - 1];
