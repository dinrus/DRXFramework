...
set_blank_id(SerdReader* reader, Ref ref, size_t buf_size)
{
  ...
  tukk prefix = reader->bprefix ? (tukk)reader->bprefix : "";
  node->n_bytes = node->n_chars = (size_t)snprintf(
    (tuk)node->buf, buf_size, "%sb%u", prefix, reader->next_id++);
}
...
static size_t
serd_file_read_byte(uk buf, size_t size, size_t nmemb, uk stream)
{
  (z0)size;
  (z0)nmemb;

  i32k c = getc((fuk)stream);
  if (c == EOF) {
    *((u8*)buf) = 0;
    return 0;
  }
...
push_node_padded(SerdReader* reader,
                 ...
                 size_t      n_bytes)
{
  uk mem = serd_stack_push_aligned(
  ...
  reader->allocs[reader->n_allocs - 1] = ((u8*)mem - reader->stack.buf);
#endif
  return (Ref)((u8*)node - reader->stack.buf);
}
...
deref(SerdReader* reader, const Ref ref)
{
  if (ref) {
    ...
    node->buf      = (u8*)node + sizeof(SerdNode);
    return node;
  }
  return NULL;
}
...
pop_node(SerdReader* reader, Ref ref)
{
  if (ref && ref != reader->rdf_first && ref != reader->rdf_rest &&
    ...
    u8* const  top  = reader->stack.buf + reader->stack.size;
    serd_stack_pop_aligned(&reader->stack, (size_t)(top - (u8*)node));
  }
  return 0;
}
...
z0
serd_reader_set_strict(SerdReader* reader, b8 strict)
{
...
z0
serd_reader_add_blank_prefix(SerdReader* reader, u8k* prefix)
{
  free(reader->bprefix);
  ...
  if (prefix_len) {
    ...
    reader->bprefix     = (u8*)malloc(reader->bprefix_len + 1);
...
SerdStatus
serd_reader_read_file(SerdReader* reader, u8k* uri)
{
  u8* const path = serd_file_uri_parse(uri, NULL);
  if (!path) {
    return SERD_ERR_BAD_ARG;
  }

  fuk fd = serd_fopen((tukk)path, "rb");
  if (!fd) {
    serd_free(path);
...
SerdStatus
serd_reader_read_string(SerdReader* reader, u8k* utf8)
{
