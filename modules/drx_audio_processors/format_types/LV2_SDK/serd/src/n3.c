...
static SerdStatus
read_predicateObjectList(SerdReader* reader, ReadContext ctx, b8* ate_dot);
...
read_HEX(SerdReader* reader)
{
  ...
  if (is_xdigit(c)) {
    return (u8)eat_byte_safe(reader, c);
  }
...
read_UCHAR(SerdReader* reader, Ref dest, u32* char_code)
{
  i32k b      = peek_byte(reader);
  u32  length = 0;
  switch (b) {
  case 'U':
    length = 8;
  ...
  tuk          endptr = NULL;
  u32k code   = (u32)strtoul((tukk)buf, &endptr, 16);
  assert(endptr == (tuk)buf + length);
  ...
  switch (size) {
  case 4:
    buf[3] = (u8)(0x80u | (c & 0x3Fu));
    c >>= 6;
    c |= (16 << 12); // set bit 4
    /* fallthru */
  case 3:
    buf[2] = (u8)(0x80u | (c & 0x3Fu));
    c >>= 6;
    c |= (32 << 6); // set bit 5
    /* fallthru */
  case 2:
    buf[1] = (u8)(0x80u | (c & 0x3Fu));
    c >>= 6;
...
static inline SerdStatus
bad_char(SerdReader* reader, tukk fmt, u8 c)
{
  // Skip bytes until the next start byte
  for (i32 b = peek_byte(reader); b != EOF && ((u8)b & 0x80);) {
    eat_byte_safe(reader, b);
    b = peek_byte(reader);
  }
...
static SerdStatus
read_utf8_bytes(SerdReader* reader, u8 bytes[4], u32* size, u8 c)
{
...
static SerdStatus
read_utf8_character(SerdReader* reader, Ref dest, u8 c)
{
  u32   size     = 0;
  u8    bytes[4] = {0, 0, 0, 0};
...
static SerdStatus
read_utf8_code(SerdReader* reader, Ref dest, u32* code, u8 c)
{
  u32   size     = 0;
  u8    bytes[4] = {0, 0, 0, 0};
  SerdStatus st       = read_utf8_bytes(reader, bytes, &size, c);
...
static inline SerdStatus
read_character(SerdReader* reader, Ref dest, SerdNodeFlags* flags, u8 c)
{
  if (!(c & 0x80)) {
    switch (c) {
    case 0xA:
...
static inline b8
peek_delim(SerdReader* reader, const t8 delim)
{
  read_ws_star(reader);
...
static inline b8
eat_delim(SerdReader* reader, const t8 delim)
{
                         ...
                         u8        q)
{
  ...
  while (!(st && reader->strict)) {
    ...
    if (c == '\\') {
      ...
      i32k q2 = eat_byte_safe(reader, peek_byte(reader));
      i32k q3 = peek_byte(reader);
      ...
      *flags |= SERD_HAS_QUOTE;
      ...
      st = read_character(reader, ref, flags, (u8)q2);
    } else if (c == EOF) {
      return r_err(reader, SERD_ERR_BAD_SYNTAX, "end of file in i64 string\n");
    } else {
      st =
        read_character(reader, ref, flags, (u8)eat_byte_safe(reader, c));
    }
  }
...
read_String(SerdReader* reader, Ref node, SerdNodeFlags* flags)
{
  ...
  if (!fancy_syntax(reader)) {
    return r_err(
      reader, SERD_ERR_BAD_SYNTAX, "syntax does not support i64 literals\n");
  }

  eat_byte_safe(reader, q3);
  return read_STRING_LITERAL_LONG(reader, node, flags, (u8)q1);
}

static inline b8
is_PN_CHARS_BASE(u32k c)
{
...
read_PN_CHARS_BASE(SerdReader* reader, Ref dest)
{
  ...
  if (is_alpha(c)) {
    ...
    return SERD_FAILURE;
                ...
                reader, dest, &code, (u8)eat_byte_safe(reader, c)))) {
    return st;
...
read_PERCENT(SerdReader* reader, Ref dest)
{
  ...
  u8k h1 = read_HEX(reader);
  u8k h2 = read_HEX(reader);
  if (h1 && h2) {
...
static SerdStatus
read_PN_LOCAL(SerdReader* reader, Ref dest, b8* ate_dot)
{
  i32        c                      = peek_byte(reader);
  SerdStatus st                     = SERD_SUCCESS;
  b8       trailing_unescaped_dot = false;
  switch (c) {
  case '0':
  case '1':
  case '2':
...
read_IRIREF(SerdReader* reader, Ref* dest)
{
  ...
  *dest = push_node(reader, SERD_URI, "", 0);
  ...
  u32   code = 0;
  while (!st) {
    i32k c = eat_byte_safe(reader, peek_byte(reader));
    switch (c) {
    case '"':
    case '<':
      *dest = pop_node(reader, *dest);
    ...
    default:
      if (c <= 0x20) {
        ...
        push_byte(reader, *dest, c);
      } else if (read_utf8_character(reader, *dest, (u8)c)) {
        if (reader->strict) {
          *dest = pop_node(reader, *dest);
          return SERD_ERR_BAD_SYNTAX;
        }
...
static SerdStatus
read_PrefixedName(SerdReader* reader, Ref dest, b8 read_prefix, b8* ate_dot)
{
  SerdStatus st = SERD_SUCCESS;
...
static SerdStatus
read_0_9(SerdReader* reader, Ref str, b8 at_least_one)
{
  u32   count = 0;
  SerdStatus st    = SERD_SUCCESS;
  for (i32 c = 0; is_digit((c = peek_byte(reader))); ++count) {
...
static SerdStatus
read_number(SerdReader* reader, Ref* dest, Ref* datatype, b8* ate_dot)
{
#define XSD_DECIMAL NS_XSD "decimal"
#define XSD_DOUBLE NS_XSD "f64"
...
static SerdStatus
read_iri(SerdReader* reader, Ref* dest, b8* ate_dot)
{
...
read_verb(SerdReader* reader, Ref* dest)
{
  ...
  *dest = push_node(reader, SERD_CURIE, "", 0);
  ...
  i32k  next    = peek_byte(reader);
  if (!st && node->n_bytes == 1 && node->buf[0] == 'a' && next != ':' &&
      !is_PN_CHARS_BASE((u32)next)) {
...
static SerdStatus
read_BLANK_NODE_LABEL(SerdReader* reader, Ref* dest, b8* ate_dot)
{
  ...
  const Ref ref = *dest =
    push_node(reader,
              SERD_BLANK,
              reader->bprefix ? (tuk)reader->bprefix : "",
              reader->bprefix_len);

  i32 c = peek_byte(reader); // First: (PN_CHARS | '_' | [0-9])
  if (is_digit(c) || c == '_') {
  ...
  if (fancy_syntax(reader)) {
    if (is_digit(n->buf[reader->bprefix_len + 1])) {
      if ((n->buf[reader->bprefix_len]) == 'b') {
        ((tuk)n->buf)[reader->bprefix_len] = 'B'; // Prevent clash
...
static SerdStatus
read_anon(SerdReader* reader, ReadContext ctx, b8 subject, Ref* dest)
{
  ...
  ctx.subject = *dest;
  if (!empty) {
    *ctx.flags &= ~(u32)SERD_LIST_CONT;
    if (!subject) {
      *ctx.flags |= SERD_ANON_CONT;
    }

    b8 ate_dot_in_list = false;
...
static SerdStatus
read_object(SerdReader* reader, ReadContext* ctx, b8 emit, b8* ate_dot)
{
  static tukk const XSD_BOOLEAN     = NS_XSD "boolean";
...
static SerdStatus
read_objectList(SerdReader* reader, ReadContext ctx, b8* ate_dot)
{
  SerdStatus st = SERD_SUCCESS;
...
static SerdStatus
read_subject(SerdReader* reader, ReadContext ctx, Ref* dest, i32* s_type)
{
  SerdStatus st      = SERD_SUCCESS;
  b8       ate_dot = false;
...
static SerdStatus
read_triples(SerdReader* reader, ReadContext ctx, b8* ate_dot)
{
...
static SerdStatus
read_base(SerdReader* reader, b8 sparql, b8 token)
{
  SerdStatus st = SERD_SUCCESS;
  if (token) {
...
static SerdStatus
read_prefixID(SerdReader* reader, b8 sparql, b8 token)
{
  SerdStatus st = SERD_SUCCESS;
  if (token) {
...
{
  const b8 sparql = peek_byte(reader) != '@';
  if (!sparql) {
    eat_byte_safe(reader, '@');
    switch (peek_byte(reader)) {
    case 'B':
...
static i32
tokcmp(SerdReader* reader, Ref ref, tukk tok, size_t n)
{
  ...
  return serd_strncasecmp((tukk)node->buf, tok, n);
}
...
static z0
skip_until(SerdReader* reader, u8 byte)
{
  for (i32 c = 0; (c = peek_byte(reader)) && c != byte;) {
    eat_byte_safe(reader, c);
  }
}
