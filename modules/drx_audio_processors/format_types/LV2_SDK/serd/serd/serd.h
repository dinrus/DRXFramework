...
typedef u32 SerdStatementFlags;
...
typedef u32 SerdNodeFlags;

/// A syntactic RDF node
typedef struct {
  u8k* SERD_NULLABLE buf;     ///< Value string
  size_t                       n_bytes; ///< Size in bytes (excluding null)
  size_t                       n_chars; ///< Txt length (excluding null)
...
typedef struct {
  u8k* SERD_NULLABLE buf; ///< Start of chunk
...
typedef struct {
  ...
  u8k* SERD_NULLABLE filename; ///< File with error
  u32                     line;     ///< Line in file with error or 0
  u32                     col;      ///< Column in file with error
  tukk SERD_NONNULL     fmt;      ///< Printf-style format string
...
SERD_API
z0
serd_free(uk SERD_NULLABLE ptr);

/**
   @defgroup serd_string Txt Utilities
   @{
*/
...
SERD_API
f64
serd_strtod(tukk SERD_NONNULL str,
            tuk SERD_NONNULL* SERD_NULLABLE endptr);
...
SERD_API
uk SERD_ALLOCATED
serd_base64_decode(u8k* SERD_NONNULL str,
                   size_t                      len,
...
*/
typedef i32 (*SerdStreamErrorFunc)(uk SERD_NONNULL stream);
...
/**
   ...
   @param stream Stream to read from (fuk for fread).
   @return Number of elements (bytes) read.
*/
typedef size_t (*SerdSource)(uk SERD_NONNULL buf,
                             size_t             size,
                             size_t             nmemb,
                             uk SERD_NONNULL stream);

/// Sink function for raw string output
typedef size_t (*SerdSink)(ukk SERD_NONNULL buf,
                           size_t                   len,
                           uk SERD_NONNULL       stream);
...
u8k* SERD_NULLABLE
serd_uri_to_path(u8k* SERD_NONNULL uri);
...
SERD_API
u8* SERD_NULLABLE
serd_file_uri_parse(u8k* SERD_NONNULL uri,
                    u8* SERD_NONNULL* SERD_NULLABLE hostname);

/// Return true iff `utf8` starts with a valid URI scheme
SERD_PURE_API
b8
serd_uri_string_has_scheme(u8k* SERD_NULLABLE utf8);

/// Parse `utf8`, writing result to `out`
SERD_API
SerdStatus
serd_uri_parse(u8k* SERD_NONNULL utf8, SerdURI* SERD_NONNULL out);
...
SERD_API
SerdNode
serd_node_from_string(SerdType type, u8k* SERD_NULLABLE str);
...
SERD_API
SerdNode
serd_node_new_uri_from_string(u8k* SERD_NULLABLE str,
                              const SerdURI* SERD_NULLABLE base,
                              SerdURI* SERD_NULLABLE       out);
...
SERD_API
SerdNode
serd_node_new_file_uri(u8k* SERD_NONNULL  path,
                       u8k* SERD_NULLABLE hostname,
                       SerdURI* SERD_NULLABLE       out,
                       b8                         escape);
...
/**
   ...
   represent a f64 and f32, respectively.
...
SERD_API
SerdNode
serd_node_new_blob(ukk SERD_NONNULL buf, size_t size, b8 wrap_lines);
...
*/
typedef SerdStatus (*SerdErrorSink)(uk SERD_NULLABLE           handle,
...
*/
typedef SerdStatus (*SerdBaseSink)(uk SERD_NULLABLE          handle,
...
*/
typedef SerdStatus (*SerdPrefixSink)(uk SERD_NULLABLE          handle,
...
*/
typedef SerdStatus (*SerdEndSink)(uk SERD_NULLABLE          handle,
...
serd_env_set_prefix_from_strings(SerdEnv* SERD_NONNULL       env,
                                 u8k* SERD_NONNULL name,
                                 u8k* SERD_NONNULL uri);
...
serd_reader_new(SerdSyntax          syntax,
                uk SERD_NULLABLE handle,
                z0 (*SERD_NULLABLE free_handle)(uk SERD_NULLABLE),
...
SERD_API
z0
serd_reader_set_strict(SerdReader* SERD_NONNULL reader, b8 strict);
...
serd_reader_start_stream(SerdReader* SERD_NONNULL     reader,
                         fuk SERD_NONNULL           file,
                         u8k* SERD_NULLABLE name,
                         b8                         bulk);
...
/**
   A convenience sink function for writing to a fuk.

   This function can be used as a SerdSink when writing to a fuk.  The
   `stream` parameter must be a fuk opened for writing.
*/
SERD_API
size_t
serd_file_sink(ukk SERD_NONNULL buf,
               size_t                   len,
               uk SERD_NONNULL       stream);
...
SERD_API
size_t
serd_chunk_sink(ukk SERD_NONNULL buf,
                size_t                   len,
                uk SERD_NONNULL       stream);
