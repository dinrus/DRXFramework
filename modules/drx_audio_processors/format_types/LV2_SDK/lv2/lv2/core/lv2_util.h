...
*/
static inline uk
lv2_features_data(const LV2_Feature* const* features, tukk const uri)
{
  if (features) {
...
/**
   ...
   The arguments should be a series of tukk uri, uk* data, b8
   ...
   tukk missing = lv2_features_query(
        features,
...
lv2_features_query(const LV2_Feature* const* features, ...)
{
  ...
  tukk uri = NULL;
  while ((uri = va_arg(args, tukk))) {
    uk* data     = va_arg(args, uk*);
    b8   required = va_arg(args, i32);
