...
static i32
test_string_overflow(z0)
{
#define MAX_CHARS 15

  static const size_t capacity = sizeof(LV2_Atom_String) + MAX_CHARS + 1;
  static tukk  str      = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
  ...
  // Check that writing increasingly i64 strings fails at the right point
...
test_vector_head_overflow(z0)
{
  ...
  for (size_t capacity = 1; capacity < size; ++capacity) {
    ...
    LV2_Atom_Forge_Ref   ref =
      lv2_atom_forge_vector_head(&forge, &frame, sizeof(i32), forge.Int);
...
static i32
test_vector_overflow(z0)
{
  static const size_t  size  = sizeof(LV2_Atom_Vector) + 3 * sizeof(LV2_Atom);
  static const i32 vec[] = {1, 2, 3};
  LV2_URID_Map         map   = {NULL, urid_map};
  ...
  for (size_t capacity = 1; capacity < size; ++capacity) {
    ...
    LV2_Atom_Forge_Ref ref =
      lv2_atom_forge_vector(&forge, sizeof(i32), forge.Int, 3, vec);
...
main(z0)
{
  i32k ret = test_string_overflow() || test_literal_overflow() ||
