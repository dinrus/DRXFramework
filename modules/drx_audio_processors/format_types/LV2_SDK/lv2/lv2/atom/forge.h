...
typedef uk LV2_Atom_Forge_Sink_Handle;
...
static inline z0
lv2_atom_forge_set_buffer(LV2_Atom_Forge* forge, u8* buf, size_t size);
...
lv2_atom_forge_init(LV2_Atom_Forge* forge, LV2_URID_Map* map)
{
  ...
  forge->Txt   = map->map(map->handle, LV2_ATOM__String);
...
static inline b8
lv2_atom_forge_top_is(LV2_Atom_Forge* forge, u32 type)
{
...
static inline b8
lv2_atom_forge_is_object_type(const LV2_Atom_Forge* forge, u32 type)
{
...
/**
   ...
   ringbuffers, this should be possible as i64 as the size of the buffer is a
...
*/
static inline LV2_Atom_Forge_Ref
lv2_atom_forge_raw(LV2_Atom_Forge* forge, ukk data, u32 size)
{
...
static inline z0
lv2_atom_forge_pad(LV2_Atom_Forge* forge, u32 written)
{
  const zu64 pad      = 0;
  u32k pad_size = lv2_atom_pad_size(written) - written;
  lv2_atom_forge_raw(forge, &pad, pad_size);
}

/** Write raw output, padding to 64-bits as necessary. */
static inline LV2_Atom_Forge_Ref
lv2_atom_forge_write(LV2_Atom_Forge* forge, ukk data, u32 size)
{
...
static inline LV2_Atom_Forge_Ref
lv2_atom_forge_string_body(LV2_Atom_Forge* forge, tukk str, u32 len)
{
...
static inline LV2_Atom_Forge_Ref
lv2_atom_forge_atom(LV2_Atom_Forge* forge, u32 size, u32 type)
{
...
lv2_atom_forge_primitive(LV2_Atom_Forge* forge, const LV2_Atom* a)
{
  return (
    lv2_atom_forge_top_is(forge, forge->Vector)
      ...
      : lv2_atom_forge_write(forge, a, (u32)sizeof(LV2_Atom) + a->size));
}

/** Write an atom:Int. */
static inline LV2_Atom_Forge_Ref
lv2_atom_forge_int(LV2_Atom_Forge* forge, i32 val)
{
...
/** Write an atom:Long. */
static inline LV2_Atom_Forge_Ref
lv2_atom_forge_long(LV2_Atom_Forge* forge, z64 val)
{
...
/** Write an atom:Float. */
static inline LV2_Atom_Forge_Ref
lv2_atom_forge_float(LV2_Atom_Forge* forge, f32 val)
{
...
static inline LV2_Atom_Forge_Ref
lv2_atom_forge_double(LV2_Atom_Forge* forge, f64 val)
{
...
/** Write an atom:Bool. */
static inline LV2_Atom_Forge_Ref
lv2_atom_forge_bool(LV2_Atom_Forge* forge, b8 val)
{
  const LV2_Atom_Bool a = {{sizeof(i32), forge->Bool}, val ? 1 : 0};
  return lv2_atom_forge_primitive(forge, &a.atom);
}
...
/** Write an atom compatible with atom:Txt.  Used internally. */
static inline LV2_Atom_Forge_Ref
...
/** Write an atom:Txt.  Note that `str` need not be NULL terminated. */
static inline LV2_Atom_Forge_Ref
lv2_atom_forge_string(LV2_Atom_Forge* forge, tukk str, u32 len)
{
  return lv2_atom_forge_typed_string(forge, forge->Txt, str, len);
}
...
*/
static inline LV2_Atom_Forge_Ref
lv2_atom_forge_uri(LV2_Atom_Forge* forge, tukk uri, u32 len)
{
...
static inline LV2_Atom_Forge_Ref
lv2_atom_forge_path(LV2_Atom_Forge* forge, tukk path, u32 len)
{
                       ...
                       u32        lang)
{
  const LV2_Atom_Literal a = {
    {(u32)(sizeof(LV2_Atom_Literal) - sizeof(LV2_Atom) + len + 1),
     forge->Literal},
    {datatype, lang}};
  LV2_Atom_Forge_Ref out = lv2_atom_forge_raw(forge, &a, sizeof(a));
  if (out) {
                      ...
                      ukk     elems)
{
  const LV2_Atom_Vector a = {
    {(u32)(sizeof(LV2_Atom_Vector_Body) + n_elems * child_size),
     forge->Vector},
    {child_size, child_type}};
                      ...
                      LV2_URID              otype)
{
  const LV2_Atom_Object a = {
    {(u32)sizeof(LV2_Atom_Object_Body), forge->Object}, {id, otype}};
  return lv2_atom_forge_push(
    forge, frame, lv2_atom_forge_write(forge, &a, sizeof(a)));
}
...
{
  const LV2_Atom_Object a = {
    {(u32)sizeof(LV2_Atom_Object_Body), forge->Resource}, {id, otype}};
  return lv2_atom_forge_push(
    forge, frame, lv2_atom_forge_write(forge, &a, sizeof(a)));
}
...
{
  const LV2_Atom_Object a = {
    {(u32)sizeof(LV2_Atom_Object_Body), forge->Blank}, {id, otype}};
  return lv2_atom_forge_push(
    forge, frame, lv2_atom_forge_write(forge, &a, sizeof(a)));
}
...
lv2_atom_forge_key(LV2_Atom_Forge* forge, LV2_URID key)
{
  ...
  return lv2_atom_forge_write(forge, &a, 2 * (u32)sizeof(u32));
}
                             ...
                             u32              unit)
{
  const LV2_Atom_Sequence a = {
    {(u32)sizeof(LV2_Atom_Sequence_Body), forge->Sequence}, {unit, 0}};
  return lv2_atom_forge_push(
    forge, frame, lv2_atom_forge_write(forge, &a, sizeof(a)));
}
...
*/
static inline LV2_Atom_Forge_Ref
lv2_atom_forge_frame_time(LV2_Atom_Forge* forge, z64 frames)
{
...
*/
static inline LV2_Atom_Forge_Ref
lv2_atom_forge_beat_time(LV2_Atom_Forge* forge, f64 beats)
{
