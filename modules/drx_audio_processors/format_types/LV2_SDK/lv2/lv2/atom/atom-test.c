...
#define NUM_PROPS 15
  ...
  tuk pbody = (tuk)LV2_ATOM_BODY(path);
  if (strcmp(pbody, pstr)) {
    return test_fail("%s != \"%s\"\n", pbody, pstr);
  }

  // eg_uri = (URI)"http://example.org/value"
  tukk    ustr     = "http://example.org/value";
  u32k ustr_len = (u32)strlen(ustr);
  ...
  tuk ubody = (tuk)LV2_ATOM_BODY(uri);
  ...
  tuk sbody = (tuk)LV2_ATOM_BODY(string);
  ...
  tuk lbody = (tuk)LV2_ATOM_CONTENTS(LV2_Atom_Literal, literal);
  ...
  LV2_Atom_Vector* vector  = (LV2_Atom_Vector*)lv2_atom_forge_deref(
    &forge,
    lv2_atom_forge_vector(&forge, sizeof(i32), forge.Int, 4, elems));
  uk vec_body = LV2_ATOM_CONTENTS(LV2_Atom_Vector, vector);
  ...
  LV2_Atom_Vector*     vector2 = (LV2_Atom_Vector*)lv2_atom_forge_deref(
    &forge,
    lv2_atom_forge_vector_head(&forge, &vec_frame, sizeof(i32), forge.Int));
  for (u32 e = 0; e < sizeof(elems) / sizeof(i32); ++e) {
  ...
  // Test equality
  LV2_Atom_Int itwo = {{forge.Int, sizeof(i32)}, 2};
  ...
  LV2_ATOM_SEQUENCE_FOREACH (seq, ev) {
    ...
    } else if (((LV2_Atom_Int*)&ev->body)->body != (i32)n_events + 1) {
  ...
  i32 n_matches = lv2_atom_object_query((LV2_Atom_Object*)obj, q);
  for (i32 n = 0; n < 2; ++n) {
