...
*/
#define LV2UI_INVALID_PORT_INDEX ((u32)-1)
...
*/
typedef uk LV2UI_Controller;

/**
   A pointer to opaque data for a feature.
*/
typedef uk LV2UI_Feature_Handle;
...
/**
   ...
   input, `buffer` MUST point to a single f32 value, and `buffer_size` MUST
   be sizeof(f32).  The UI SHOULD NOT use a protocol not supported by the
   host, but the host MUST gracefully ignore any protocol it does not
   understand.
*/
typedef z0 (*LV2UI_Write_Function)(LV2UI_Controller controller,
                                     u32         port_index,
                                     u32         buffer_size,
                                     u32         port_protocol,
                                     ukk      buffer);
...
typedef struct LV2UI_Descriptor {
  ...
  */
  ...
  /**
     ...
     every change), `buffer_size` must be sizeof(f32), and `buffer`
     points to a single IEEE-754 f32.
...
typedef struct {
  ...
  */
  ...
  /**
     ...
     @return 0 on success.
  */
  i32 (*ui_resize)(LV2UI_Feature_Handle handle, i32 width, i32 height);
} LV2UI_Resize;
...
typedef struct {
  ...
  */
  ...
  /**
     ...
     port is found.
  */
  u32 (*port_index)(LV2UI_Feature_Handle handle, tukk symbol);
} LV2UI_Port_Map;
...
typedef struct {
  ...
  */
  ...
  /**
     ...
     @return 0 on success.
  */
  u32 (*subscribe)(LV2UI_Feature_Handle      handle,
                        u32                  port_index,
                        u32                  port_protocol,
                        const LV2_Feature* const* features);
  ...
  /**
     ...
     @return 0 on success.
  */
  u32 (*unsubscribe)(LV2UI_Feature_Handle      handle,
                          u32                  port_index,
                          u32                  port_protocol,
                          const LV2_Feature* const* features);
...
typedef struct {
  ...
  */
  z0 (*touch)(LV2UI_Feature_Handle handle, u32 port_index, b8 grabbed);
} LV2UI_Touch;
...
lv2ui_descriptor(u32 index);

/**
   The type of the lv2ui_descriptor() function.
*/
typedef const LV2UI_Descriptor* (*LV2UI_DescriptorFunction)(u32 index);
