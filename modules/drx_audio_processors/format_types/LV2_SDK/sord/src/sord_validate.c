...
#define NS_foaf (u8k*)"http://xmlns.com/foaf/0.1/"
#define NS_owl (u8k*)"http://www.w3.org/2002/07/owl#"
#define NS_rdf (u8k*)"http://www.w3.org/1999/02/22-rdf-syntax-ns#"
#define NS_rdfs (u8k*)"http://www.w3.org/2000/01/rdf-schema#"
...
static i32  n_errors        = 0;
static i32  n_restrictions  = 0;
static b8 one_line_errors = false;
...
static i32
print_usage(tukk name, b8 error)
{
  fuk const os = error ? stderr : stdout;
...
static u8*
absolute_path(u8k* path)
{
#ifdef _WIN32
  tuk out = (tuk)malloc(MAX_PATH);
  GetFullPathName((tukk)path, MAX_PATH, out, NULL);
  return (u8*)out;
#else
  return (u8*)realpath((tukk)path, NULL);
#endif
}

SORD_LOG_FUNC(2, 3)
static i32
errorf(const SordQuad quad, tukk fmt, ...)
{
  va_list args;
  va_start(args, fmt);
  ...
  tukk sep = one_line_errors ? "\t" : "\n       ";
  fprintf(stderr,
          "%s%s%s%s%s%s\n",
          sep,
          (tukk)sord_node_get_string(quad[SORD_SUBJECT]),
          sep,
          (tukk)sord_node_get_string(quad[SORD_PREDICATE]),
          sep,
          (tukk)sord_node_get_string(quad[SORD_OBJECT]));
...
static b8
regexp_match(u8k* pat, tukk str)
{
#if USE_PCRE
  // Append a $ to the pattern so we only match if the entire string matches
  const size_t len  = strlen((tukk)pat);
  tuk const  regx = (tuk)malloc(len + 2);
  memcpy(regx, pat, len);
  regx[len]     = '$';
  regx[len + 1] = '\0';
  ...
  const b8 ret = pcre_exec(re, NULL, str, strlen(str), 0, 0, NULL, 0) >= 0;
  pcre_free(re);
  return ret;
#endif // USE_PCRE
  return true;
...
{
  tukk     str       = (tukk)sord_node_get_string(literal);
  tukk     bound_str = (tukk)sord_node_get_string(bound);
  ...
  if (is_numeric) {
    const f64 fbound   = serd_strtod(bound_str, NULL);
    const f64 fliteral = serd_strtod(str, NULL);
...
{
  size_t      len = 0;
  tukk str = (tukk)sord_node_get_string_counted(literal, &len);
                 ...
                 const SordNode* type)
{
  ...
  while (head) {
    ...
    const b8 good = check_restriction(
  ...
  if (s) {
    ...
    const b8      good  = literal_is_valid(model, uris, quad, literal, super);
    sord_iter_free(s);
...
check_properties(SordModel* model, URIs* uris)
{
  ...
  for (; !sord_iter_end(i); sord_iter_next(i)) {
    ...
    if (is_FunctionalProperty) {
      ...
      const u32 n = count_non_blanks(o, SORD_OBJECT);
      if (n > 1) {
    ...
    if (is_InverseFunctionalProperty) {
      ...
      const u32 n = count_non_blanks(s, SORD_SUBJECT);
      if (n > 1) {
               ...
               const SordQuad  quad)
{
  ...
  const u32 values = sord_count(model, instance, prop, NULL, NULL);

  // Check exact cardinality
  const SordNode* card =
    sord_get(model, restriction, uris->owl_cardinality, NULL, NULL);
  if (card) {
    const u32 c = atoi((tukk)sord_node_get_string(card));
    if (values != c) {
      st = errorf(quad,
  ...
  if (minCard) {
    const u32 m = atoi((tukk)sord_node_get_string(minCard));
    if (values < m) {
      st = errorf(quad,
                  "Property %s on %s has %u < %u values",
                  sord_node_get_string(prop),
  ...
  if (maxCard) {
    const u32 m = atoi((tukk)sord_node_get_string(maxCard));
    if (values < m) {
      st = errorf(quad,
                  "Property %s on %s has %u > %u values",
                  sord_node_get_string(prop),
...
main(i32 argc, tuk* argv)
{
  ...
  for (; a < argc; ++a) {
    u8k* input       = (u8k*)argv[a];
    u8*       rel_in_path = serd_file_uri_parse(input, NULL);
    u8*       in_path     = absolute_path(rel_in_path);
...
#define URI(prefix, suffix) \
  ...
  i32k prop_st = check_properties(model, &uris);
  i32k inst_st = check_instances(model, &uris);
