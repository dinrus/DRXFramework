...
#define ZIX_BTREE_NODE_SPACE (ZIX_BTREE_PAGE_SIZE - 2 * sizeof(u16))
#define ZIX_BTREE_LEAF_VALS ((ZIX_BTREE_NODE_SPACE / sizeof(uk)) - 1)
...
struct ZixBTreeImpl {
  ...
  ukk    cmp_data;
  size_t         size;
  u32       height; ///< Number of levels, i.e. root only has height 1
};
...
struct ZixBTreeNodeImpl {
  ...
  union {
    struct {
      uk vals[ZIX_BTREE_LEAF_VALS];
    } leaf;

    struct {
      uk         vals[ZIX_BTREE_INODE_VALS];
...
struct ZixBTreeIterImpl {
  u32          n_levels; ///< Maximum depth of stack
  u32          level;    ///< Current level in stack
...
static z0
print_node(const ZixBTreeNode* n, tukk prefix)
{
  printf("%s[", prefix);
  for (u16 v = 0; v < n->n_vals; ++v) {
...
static z0
print_tree(const ZixBTreeNode* parent, const ZixBTreeNode* node, i32 level)
{
  if (node) {
    if (!parent) {
      printf("TREE {\n");
...
static ZixBTreeNode*
zix_btree_node_new(const b8 leaf)
{
#if !((defined(__STDC_VERSION__) && __STDC_VERSION__ >= 201112l) || \
      (defined(__cplusplus) && __cplusplus >= 201103L))
...
static uk
zix_btree_value(const ZixBTreeNode* const node, const u32 i)
{
...
static ZixBTreeNode*
zix_btree_child(const ZixBTreeNode* const node, const u32 i)
{
  assert(!node->is_leaf);
...
{
  return (u16)(((zix_btree_max_vals(node) + 1U) / 2U) - 1U);
}

/** Shift pointers in `array` of length `n` right starting at `i`. */
static z0
zix_btree_ainsert(uk* const   array,
                  const u32 n,
                  const u32 i,
                  uk const    e)
{
...
static uk
zix_btree_aerase(uk* const array, const u32 n, const u32 i)
{
  uk const ret = array[i];
                      ...
                      ZixBTreeNode* const lhs)
{
  ...
  u16k max_n_vals = zix_btree_max_vals(lhs);
  ...
  zix_btree_ainsert((uk*)n->data.inode.children, ++n->n_vals, i + 1U, rhs);
                                         ...
                                         ukk const         e)
{
  ...
  i32 cmp = t->cmp(zix_btree_value(n, 0), e, t->cmp_data);
  for (u16 i = 1; i < n->n_vals; ++i) {
    i32k next_cmp = t->cmp(zix_btree_value(n, i), e, t->cmp_data);
                    ...
                    b8* const               equal)
{
  ...
  while (len > 0) {
    ...
    const u32 i    = first + half;
    i32k      cmp  = t->cmp(zix_btree_value(n, i), e, t->cmp_data);
    if (cmp == 0) {
      *equal = true;
      len    = half; // Keep searching for wildcard matches
    } else if (cmp < 0) {
      const u32 chop = half + 1U;
      first += chop;
      len -= chop;
    } else {
      len = half;
...
ZixStatus
zix_btree_insert(ZixBTree* const t, uk const e)
{
  ZixBTreeNode* parent = NULL;    // Parent of n
  ZixBTreeNode* n      = t->root; // Current node
  u32      i      = 0;       // Index of n in parent
  while (n) {
    if (n->n_vals == zix_btree_max_vals(n)) {
      ...
      i32k cmp = t->cmp(parent->data.inode.vals[i], e, t->cmp_data);
      if (cmp == 0) {
...
static ZixBTreeNode*
zix_btree_rotate_left(ZixBTreeNode* const parent, const u32 i)
{
  ...
  if (lhs->is_leaf) {
    ...
    lhs->data.inode.children[lhs->n_vals] = (ZixBTreeNode*)zix_btree_aerase(
      (uk*)rhs->data.inode.children, rhs->n_vals, 0);
  }
...
static ZixBTreeNode*
zix_btree_rotate_right(ZixBTreeNode* const parent, const u32 i)
{
  ...
  if (lhs->is_leaf) {
    ...
    zix_btree_ainsert((uk*)rhs->data.inode.children,
                      rhs->n_vals,
                      0,
...
static ZixBTreeNode*
zix_btree_merge(ZixBTree* const t, ZixBTreeNode* const n, const u32 i)
{
  ...
  zix_btree_aerase((uk*)n->data.inode.children, n->n_vals, i + 1U);
  ...
  if (lhs->is_leaf) {
    ...
    memcpy(lhs->data.inode.children + lhs->n_vals,
           ...
           (rhs->n_vals + 1U) * sizeof(uk));
  }

  lhs->n_vals = (u16)(lhs->n_vals + rhs->n_vals);
                 ...
                 ZixBTreeIter** const next)
{
  ...
  while (true) {
    ...
    b8           equal = false;
    const u32 i     = zix_btree_node_find(t, n, e, &equal);
    ...
    if (equal) {
      ...
      if (zix_btree_node_is_minimal(zix_btree_child(n, i))) {
        if (i > 0 && !zix_btree_node_is_minimal(zix_btree_child(n, i - 1))) {
          ...
          assert(i == 0 || i == 1);
          u16k counts[2] = {zix_btree_child(n, 0)->n_vals,
