...
#define TUP_FMT_ELEM(e) ((e) ? sord_node_get_string(e) : (u8k*)"*")
...
#ifdef SORD_DEBUG_SEARCH
/** Txt name of each ordering (array indexed by SordOrder) */
static tukk const order_names[NUM_ORDERS] = {"spo",
                                                    "sop",
                                                    "ops",
                                                    "osp",
                                                    "pso",
...
*/
static i32k orderings[NUM_ORDERS][TUP_LEN] = {
  {0, 1, 2, 3}, // SPO
  {0, 2, 1, 3}, // SOP
  {2, 1, 0, 3}, // OPS
  {2, 0, 1, 3}, // OSP
...
struct SordIterImpl {
  ...
  i32              n_prefix;    ///< Prefix for RANGE and FILTER_RANGE
  b8             end;         ///< True iff reached end
  b8             skip_graphs; ///< Iteration should ignore graphs
};

static u32
sord_node_hash(ukk n)
{
  const SordNode* node = (const SordNode*)n;
  u32        hash = zix_digest_start();
...
static b8
sord_node_hash_equal(ukk a, ukk b)
{
...
SORD_LOG_FUNC(3, 4)
static z0
error(SordWorld* world, SerdStatus st, tukk fmt, ...)
{
  va_list args;
  va_start(args, fmt);
...
{
  i32k cmp = strcmp((tukk)sord_node_get_string(a),
                         (tukk)sord_node_get_string(b));
  if (cmp != 0) {
    return cmp;
  }

  const b8 a_has_lang     = a->meta.lit.lang[0];
  const b8 b_has_lang     = b->meta.lit.lang[0];
  const b8 a_has_datatype = a->meta.lit.datatype;
  const b8 b_has_datatype = b->meta.lit.datatype;
  const b8 a_has_meta     = a_has_lang || a_has_datatype;
  const b8 b_has_meta     = b_has_lang || b_has_datatype;
  ...
  return strcmp((tukk)a->meta.lit.datatype->node.buf,
                (tukk)b->meta.lit.datatype->node.buf);
}
...
sord_node_compare(const SordNode* a, const SordNode* b)
{
  ...
  case SERD_URI:
  case SERD_BLANK:
    return strcmp((tukk)a->node.buf, (tukk)b->node.buf);
  case SERD_LITERAL:
...
*/
static i32
sord_quad_compare(ukk x_ptr, ukk y_ptr, ukk user_data)
{
  i32k* const             ordering = (i32k*)user_data;
  const SordNode* const* const x        = (const SordNode* const*)x_ptr;
  const SordNode* const* const y        = (const SordNode* const*)y_ptr;

  for (i32 i = 0; i < TUP_LEN; ++i) {
    i32k idx = ordering[i];
    i32k cmp = sord_node_compare(x[idx], y[idx]);
    if (cmp) {
      return cmp;
    }
  }
...
sord_iter_seek_match_range(SordIter* iter)
{
  ...
  do {
    ...
    for (i32 i = 0; i < iter->n_prefix; ++i) {
      i32k idx = orderings[iter->order][i];
...
*/
static inline b8
sord_has_index(SordModel* model, SordOrder* order, i32* n_prefix, b8 graphs)
{
  if (graphs) {
...
sord_best_index(SordModel*     sord,
                ...
                i32*           n_prefix)
{
  const b8 graph_search = (pat[TUP_G] != 0);

  const u32 sig = (pat[0] ? 1 : 0) * 0x100 + (pat[1] ? 1 : 0) * 0x010 +
                       (pat[2] ? 1 : 0) * 0x001;
...
SordModel*
sord_new(SordWorld* world, u32 indices, b8 graphs)
{
  ...
  for (u32 i = 0; i < (NUM_ORDERS / 2); ++i) {
    i32k* const ordering   = orderings[i];
    i32k* const g_ordering = orderings[i + (NUM_ORDERS / 2)];

    if (indices & (1 << i)) {
      model->indices[i] =
        zix_btree_new(sord_quad_compare, (uk)ordering, NULL);
      if (graphs) {
        model->indices[i + (NUM_ORDERS / 2)] =
          zix_btree_new(sord_quad_compare, (uk)g_ordering, NULL);
      } else {
  ...
  if (!model->indices[DEFAULT_ORDER]) {
    model->indices[DEFAULT_ORDER] =
      zix_btree_new(sord_quad_compare, (uk)orderings[DEFAULT_ORDER], NULL);
  }
  if (graphs && !model->indices[DEFAULT_GRAPH_ORDER]) {
    model->indices[DEFAULT_GRAPH_ORDER] = zix_btree_new(
      sord_quad_compare, (uk)orderings[DEFAULT_GRAPH_ORDER], NULL);
  }
...
sord_find(SordModel* model, const SordQuad pat)
{
  ...
  if (mode == FILTER_ALL) {
    ...
    i32k* const ordering   = orderings[index_order];
    for (i32 i = 0; i < n_prefix; ++i) {
...
static u8*
sord_strndup(u8k* str, size_t len)
{
  u8* dup = (u8*)malloc(len + 1);
...
static SordNode*
sord_insert_node(SordWorld* world, const SordNode* key, b8 copy)
{
  SordNode* node = NULL;
  ZixStatus st   = zix_hash_insert(world->nodes, key, (uk*)&node);
  switch (st) {
...
SordNode*
sord_new_uri(SordWorld* world, u8k* uri)
{
...
SordNode*
sord_new_blank(SordWorld* world, u8k* str)
{
...
sord_remove(SordModel* model, const SordQuad tup)
{
  ...
  for (u32 i = 0; i < NUM_ORDERS; ++i) {
    if (model->indices[i] && (i < GSPO || tup[3])) {
      if (zix_btree_remove(model->indices[i], tup, (uk*)&quad, NULL)) {
        assert(i == 0); // Assuming index coherency
