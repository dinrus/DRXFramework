...
static i32k      DIGITS        = 3;
static const u32 n_objects_per = 2;

static i32 n_expected_errors = 0;

typedef struct {
  SordQuad query;
  i32      expected_num_results;
} QueryTest;

#define USTR(s) ((u8k*)(s))

static SordNode*
uri(SordWorld* world, i32 num)
{
  if (num == 0) {
    return 0;
  }
  ...
  t8  str[]   = "eg:000";
  tuk uri_num = str + 3; // First `0'
  snprintf(uri_num, DIGITS + 1, "%0*d", DIGITS, num);
  return sord_new_uri(world, (u8k*)str);
}
...
generate(SordWorld* world, SordModel* sord, size_t n_quads, SordNode* graph)
{
  ...
  for (size_t i = 0; i < n_quads; ++i) {
    i32 num = (i * n_objects_per) + 1;

    SordNode* ids[2 + n_objects_per];
    for (u32 j = 0; j < 2 + n_objects_per; ++j) {
      ids[j] = uri(world, num++);
    }

    for (u32 j = 0; j < n_objects_per; ++j) {
...
#define NUM_PATTERNS 18
  ...
  QueryTest patterns[NUM_PATTERNS] = {
    {{0, 0, 0}, (i32)(n_quads * n_objects_per) + 12},
    {{uri(world, 1), 0, 0}, 2},
  ...
  for (; !sord_iter_end(iter); sord_iter_next(iter)) {
    ...
    zu64 count = sord_count(sord, id[0], 0, 0, 0);
...
static SerdStatus
unexpected_error(uk handle, const SerdError* error)
{
...
static SerdStatus
expected_error(uk handle, const SerdError* error)
{
...
static i32
finished(SordWorld* world, SordModel* sord, i32 status)
{
  sord_free(sord);
...
main(i32 argc, tuk* argv)
{
  ...
  u8k ni_hao[] = {0xE4, 0xBD, 0xA0, 0xE5, 0xA5, 0xBD, 0};
  SordNode*     chello   = sord_new_literal(world, NULL, ni_hao, "cmn");

  // Test literal length
  size_t         n_bytes;
  size_t         n_chars;
  u8k* str = sord_node_get_string_counted(lit_id2, &n_bytes);
  if (strcmp((tukk)str, "hello")) {
  ...
  } else if (strcmp((tukk)str, (tukk)ni_hao)) {
  ...
  if (strcmp((tukk)sord_node_get_string(reluri),
  ...
  if (strcmp((tukk)sord_node_get_string(reluri2),
             "http://drobilla.net/")) {
    fprintf(stderr,
            "Fail: Bad relative URI constructed: <%s>\n",
            sord_node_get_string(reluri));
  ...
  static tukk const index_names[6] = {
  ...
  static tukk const graph_index_names[6] = {
  ...
  // Test f64 remove (silent success)
  sord_remove(sord, tup);
