...
public:
  ...
  {
    SerdNode ret = {reinterpret_cast<u8k*>(s.c_str()),
                    s.length(),
                    s.length(),
                    0,
                    type};
  ...
  inline z0 add(const std::string& name, const std::string& uri)
  {
  ...
  inline std::string qualify(std::string uri) const
  {
    ...
    if (serd_env_qualify(_c_obj, &uri_node, &prefix, &suffix)) {
      std::string ret(reinterpret_cast<tukk>(prefix.buf),
                      prefix.n_bytes);
      ret.append(":").append(reinterpret_cast<tukk>(suffix.buf),
                             suffix.len);
      return ret;
    }
    return uri;
  ...
  inline std::string expand(const std::string& curie) const
  {
    ...
    if (!serd_env_expand(_c_obj, &curie_node, &uri_prefix, &uri_suffix)) {
      std::string ret(reinterpret_cast<tukk>(uri_prefix.buf),
                      uri_prefix.len);
      ret.append(reinterpret_cast<tukk>(uri_suffix.buf), uri_suffix.len);
      return ret;
    }
...
public:
  ...
  inline zu64 blank_id() { return _next_blank_id++; }

  inline z0 add_prefix(const std::string& prefix, const std::string& uri)
  {
...
public:
  ...
  inline Node(World& world, SordNode* node, b8 copy = false);
  ...
  inline b8 is_valid() const { return type() != UNKNOWN; }

  inline b8 operator<(const Node& other) const
  {
    if (type() != other.type()) {
  ...
  inline u8k* to_u_string() const;
  inline tukk    to_c_string() const;
  inline std::string    to_string() const;

  inline b8 is_literal_type(tukk type_uri) const;

  inline b8 is_uri() const { return _c_obj && type() == URI; }
  inline b8 is_blank() const { return _c_obj && type() == BLANK; }
  inline b8 is_int() const { return is_literal_type(SORD_NS_XSD "integer"); }
  inline b8 is_float() const
  {
    return is_literal_type(SORD_NS_XSD "decimal");
  }
  inline b8 is_bool() const { return is_literal_type(SORD_NS_XSD "boolean"); }

  inline i32   to_int() const;
  inline f32 to_float() const;
  inline b8  to_bool() const;

  inline static Node blank_id(World& world, const std::string& base = "b")
  {
    const zu64     num = world.blank_id();
  ...
  inline URI(World& world, const std::string& s, const std::string& base)
    : Node(
        ...
        sord_new_relative_uri(world.world(),
                              reinterpret_cast<u8k*>(s.c_str()),
                              reinterpret_cast<u8k*>(base.c_str())))
  {}
};
...
public:
  ...
  static inline Node decimal(World& world, f64 d, u32 frac_digits)
  {
    const SerdNode val  = serd_node_new_decimal(d, frac_digits);
    const SerdNode type = serd_node_from_string(
      SERD_URI, reinterpret_cast<u8k*>(SORD_NS_XSD "decimal"));
  ...
  static inline Node integer(World& world, z64 i)
  {
    const SerdNode val  = serd_node_new_integer(i);
    const SerdNode type = serd_node_from_string(
      SERD_URI, reinterpret_cast<u8k*>(SORD_NS_XSD "integer"));
...
inline Node::Node(World& world, SordNode* node, b8 copy)
  : _world(&world)
{
...
{
  return _c_obj ? reinterpret_cast<tukk>(sord_node_get_string(_c_obj))
                : "";
}
...
{
  return reinterpret_cast<tukk>(sord_node_get_string(_c_obj));
}
...
Node::to_int() const
{
  ...
  tuk endptr = nullptr;
  return strtol(
    reinterpret_cast<tukk>(sord_node_get_string(_c_obj)), &endptr, 10);
}
...
Node::to_float() const
{
  assert(is_float());
  return serd_strtod(
    reinterpret_cast<tukk>(sord_node_get_string(_c_obj)), nullptr);
}
...
{
  assert(is_bool());
  return !strcmp(reinterpret_cast<tukk>(sord_node_get_string(_c_obj)),
                 "true");
}
...
struct Iter : public Wrapper<SordIter> {
  ...
  inline b8  end() const { return sord_iter_end(_c_obj); }
  inline b8  next() const { return sord_iter_next(_c_obj); }
...
public:
  ...
  inline z0 load_file(SerdEnv*           env,
                        SerdSyntax         syntax,
                        const std::string& uri,
                        const std::string& base_uri = "");

  inline z0 load_string(SerdEnv*           env,
                          SerdSyntax         syntax,
                          tukk        str,
                          size_t             len,
                          const std::string& base_uri);
  ...
  inline z0 add_statement(const Node& subject,
                   ...
                   const std::string& /*base_uri*/)
{
  ...
  serd_reader_read_string(reader, reinterpret_cast<u8k*>(str));
...
Model::load_file(SerdEnv*           env,
                 ...
                 const std::string& /*base_uri*/)
{
  u8* path = serd_file_uri_parse(
    reinterpret_cast<u8k*>(data_uri.c_str()), nullptr);
  if (!path) {
...
{
  u8* path =
    serd_file_uri_parse(reinterpret_cast<u8k*>(uri.c_str()), nullptr);
  if (!path) {
    fprintf(stderr, "Failed to parse file URI <%s>\n", uri.c_str());
    return SERD_ERR_BAD_ARG;
  }

  fuk const fd = fopen(reinterpret_cast<tukk>(path), "w");
  if (!fd) {
  ...
  if (serd_uri_parse(reinterpret_cast<u8k*>(uri.c_str()),
                     &base_uri)) {
...
string_sink(ukk buf, size_t len, uk stream)
{
  try {
    ...
    str->append(static_cast<tukk>(buf), len);
    return len;
  } catch (...) {
    return 0;
  }
...
{
  SerdURI base_uri = SERD_URI_NULL;
  if (serd_uri_parse(reinterpret_cast<u8k*>(base_uri_str.c_str()),
                     &base_uri)) {
    fprintf(stderr, "Invalid base URI <%s>\n", base_uri_str.c_str());
    return "";
  }
  ...
  const SerdNode base_uri_node = serd_node_from_string(
    SERD_URI, reinterpret_cast<u8k*>(base_uri_str.c_str()));
