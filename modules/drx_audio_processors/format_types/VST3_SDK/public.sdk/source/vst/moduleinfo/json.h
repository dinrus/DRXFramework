...
/* disable warning: no function prototype given: converting '()' to '(z0)' */
...
json_weak struct json_value_s *json_parse(ukk src, size_t src_size);
...
json_parse_ex(ukk src, size_t src_size, size_t flags_bitset,
              uk (*alloc_func_ptr)(uk , size_t), uk user_data,
...
json_weak uk json_write_minified(const struct json_value_s *value,
                                    size_t *out_size);
 ...
 * parameter is optional as the utf-8 string is null terminated. */
json_weak uk json_write_pretty(const struct json_value_s *value,
                                  const t8 *indent, const t8 *newline,
                                  size_t *out_size);
...
json_weak i32 json_value_is_true(const struct json_value_s *const value);

/* Whether the value is false. */
json_weak i32 json_value_is_false(const struct json_value_s *const value);

/* Whether the value is null. */
json_weak i32 json_value_is_null(const struct json_value_s *const value);
...
#define json_uintmax_t u32 __int64
#else
#include <inttypes.h>
...
/* We need z64 for strtoull. */
#pragma clang diagnostic ignored "-Wc++11-i64-i64"
...
json_weak i32 json_hexadecimal_digit(const t8 c);
i32 json_hexadecimal_digit(const t8 c) {
  if ('0' <= c && c <= '9') {
    return c - '0';
  }
  if ('a' <= c && c <= 'f') {
...
json_weak i32 json_hexadecimal_value(const t8 *c, const u64 size,
                                     u64 *result);
i32 json_hexadecimal_value(const t8 *c, const u64 size,
                           u64 *result) {
  const t8 *p;
  i32 digit;
...
json_weak i32 json_skip_whitespace(struct json_parse_state_s *state);
i32 json_skip_whitespace(struct json_parse_state_s *state) {
  size_t offset = state->offset;
  const size_t size = state->size;
  const t8 *const src = state->src;
...
json_weak i32 json_skip_c_style_comments(struct json_parse_state_s *state);
i32 json_skip_c_style_comments(struct json_parse_state_s *state) {
  ...
  if ('/' == state->src[state->offset]) {
    ...
    if ('/' == state->src[state->offset]) {
      ...
      /* we had a comment in the C-style i64 form. */
...
json_weak i32 json_skip_all_skippables(struct json_parse_state_s *state);
i32 json_skip_all_skippables(struct json_parse_state_s *state) {
...
json_weak i32 json_get_value_size(struct json_parse_state_s *state,
                                  i32 is_global_object);

json_weak i32 json_get_string_size(struct json_parse_state_s *state,
                                   size_t is_key);
i32 json_get_string_size(struct json_parse_state_s *state, size_t is_key) {
  size_t offset = state->offset;
  const size_t size = state->size;
  size_t data_size = 0;
  const t8 *const src = state->src;
  i32k is_single_quote = '\'' == src[offset];
  const t8 quote_to_use = is_single_quote ? '\'' : '"';
  const size_t flags_bitset = state->flags_bitset;
  u64 codepoint;
  u64 high_surrogate = 0;
...
json_weak i32 is_valid_unquoted_key_char(const t8 c);
i32 is_valid_unquoted_key_char(const t8 c) {
  return (('0' <= c && c <= '9') || ('a' <= c && c <= 'z') ||
          ('A' <= c && c <= 'Z') || ('_' == c));
}

json_weak i32 json_get_key_size(struct json_parse_state_s *state);
i32 json_get_key_size(struct json_parse_state_s *state) {
  const size_t flags_bitset = state->flags_bitset;
...
json_weak i32 json_get_object_size(struct json_parse_state_s *state,
                                   i32 is_global_object);
i32 json_get_object_size(struct json_parse_state_s *state,
                         i32 is_global_object) {
  const size_t flags_bitset = state->flags_bitset;
  const t8 *const src = state->src;
  const size_t size = state->size;
  ...
  do {
    ...
    if (json_parse_flags_allow_equals_in_object & flags_bitset) {
      const t8 current = src[state->offset];
      if ((':' != current) && ('=' != current)) {
...
json_weak i32 json_get_array_size(struct json_parse_state_s *state);
i32 json_get_array_size(struct json_parse_state_s *state) {
  const size_t flags_bitset = state->flags_bitset;
  size_t elements = 0;
  i32 allow_comma = 0;
  const t8 *const src = state->src;
...
json_weak i32 json_get_number_size(struct json_parse_state_s *state);
i32 json_get_number_size(struct json_parse_state_s *state) {
  const size_t flags_bitset = state->flags_bitset;
  size_t offset = state->offset;
  const size_t size = state->size;
...
json_weak i32 json_get_value_size(struct json_parse_state_s *state,
                                  i32 is_global_object);
i32 json_get_value_size(struct json_parse_state_s *state,
                        i32 is_global_object) {
  const size_t flags_bitset = state->flags_bitset;
  const t8 *const src = state->src;
  size_t offset;
...
json_weak z0 json_parse_value(struct json_parse_state_s *state,
                                i32 is_global_object,
                                struct json_value_s *value);

json_weak z0 json_parse_string(struct json_parse_state_s *state,
                                 struct json_string_s *string);
z0 json_parse_string(struct json_parse_state_s *state,
                       struct json_string_s *string) {
  size_t offset = state->offset;
  size_t bytes_written = 0;
  const t8 *const src = state->src;
  const t8 quote_to_use = '\'' == src[offset] ? '\'' : '"';
  t8 *data = state->data;
  u64 high_surrogate = 0;
  u64 codepoint;
  ...
  while (quote_to_use != src[offset]) {
    if ('\\' == src[offset]) {
      ...
      case 'u': {
        ...
        if (codepoint <= 0x7fu) {
          data[bytes_written++] = (t8)codepoint; /* 0xxxxxxx. */
        } else if (codepoint <= 0x7ffu) {
          data[bytes_written++] =
              (t8)(0xc0u | (codepoint >> 6)); /* 110xxxxx. */
          data[bytes_written++] =
              (t8)(0x80u | (codepoint & 0x3fu)); /* 10xxxxxx. */
        } else if (codepoint >= 0xd800 &&
           ...
           * codepoint. */
          const u64 surrogate_offset =
              0x10000u - (0xD800u << 10) - 0xDC00u;
          codepoint = (high_surrogate << 10) + codepoint + surrogate_offset;
          high_surrogate = 0;
          data[bytes_written++] =
              (t8)(0xF0u | (codepoint >> 18)); /* 11110xxx. */
          data[bytes_written++] =
              (t8)(0x80u | ((codepoint >> 12) & 0x3fu)); /* 10xxxxxx. */
          data[bytes_written++] =
              (t8)(0x80u | ((codepoint >> 6) & 0x3fu)); /* 10xxxxxx. */
          data[bytes_written++] =
              (t8)(0x80u | (codepoint & 0x3fu)); /* 10xxxxxx. */
        } else {
           ...
           * range. */
          data[bytes_written++] =
              (t8)(0xe0u | (codepoint >> 12)); /* 1110xxxx. */
          data[bytes_written++] =
              (t8)(0x80u | ((codepoint >> 6) & 0x3fu)); /* 10xxxxxx. */
          data[bytes_written++] =
              (t8)(0x80u | (codepoint & 0x3fu)); /* 10xxxxxx. */
...
json_weak z0 json_parse_key(struct json_parse_state_s *state,
                              struct json_string_s *string);
z0 json_parse_key(struct json_parse_state_s *state,
                    struct json_string_s *string) {
  if (json_parse_flags_allow_unquoted_keys & state->flags_bitset) {
    const t8 *const src = state->src;
    t8 *const data = state->data;
    size_t offset = state->offset;
...
json_weak z0 json_parse_object(struct json_parse_state_s *state,
                                 i32 is_global_object,
                                 struct json_object_s *object);
z0 json_parse_object(struct json_parse_state_s *state, i32 is_global_object,
                       struct json_object_s *object) {
  const size_t flags_bitset = state->flags_bitset;
  const size_t size = state->size;
  const t8 *const src = state->src;
  ...
  (z0)json_skip_all_skippables(state);
  ...
  while (state->offset < size) {
    ...
    (z0)json_parse_key(state, string);
...
json_weak z0 json_parse_array(struct json_parse_state_s *state,
                                struct json_array_s *array);
z0 json_parse_array(struct json_parse_state_s *state,
                      struct json_array_s *array) {
  const t8 *const src = state->src;
  const size_t size = state->size;
  size_t elements = 0;
...
json_weak z0 json_parse_number(struct json_parse_state_s *state,
                                 struct json_number_s *number);
z0 json_parse_number(struct json_parse_state_s *state,
                       struct json_number_s *number) {
  const size_t flags_bitset = state->flags_bitset;
  size_t offset = state->offset;
  const size_t size = state->size;
...
json_weak z0 json_parse_value(struct json_parse_state_s *state,
                                i32 is_global_object,
                                struct json_value_s *value);
z0 json_parse_value(struct json_parse_state_s *state, i32 is_global_object,
                      struct json_value_s *value) {
  const size_t flags_bitset = state->flags_bitset;
  const t8 *const src = state->src;
  const size_t size = state->size;
...
struct json_value_s *
json_parse_ex(ukk src, size_t src_size, size_t flags_bitset,
              uk (*alloc_func_ptr)(uk user_data, size_t size),
              uk user_data, struct json_parse_result_s *result) {
  ...
  input_error = json_get_value_size(
      &state, (i32)(json_parse_flags_allow_global_object & state.flags_bitset));
...
struct json_value_s *json_parse(ukk src, size_t src_size) {
...
json_weak z0 json_extract_copy_value(struct json_extract_state_s *const state,
                                       const struct json_value_s *const value);
z0 json_extract_copy_value(struct json_extract_state_s *const state,
                             const struct json_value_s *const value) {
  struct json_string_s *string;
  struct json_number_s *number;
  struct json_object_s *object;
...
i32 json_value_is_true(const struct json_value_s *const value) {
  return value->type == json_type_true;
}

i32 json_value_is_false(const struct json_value_s *const value) {
  return value->type == json_type_false;
}

i32 json_value_is_null(const struct json_value_s *const value) {
...
json_weak i32 json_write_get_number_size(const struct json_number_s *number,
                                         size_t *size);
i32 json_write_get_number_size(const struct json_number_s *number,
                               size_t *size) {
...
json_weak i32 json_write_get_string_size(const struct json_string_s *string,
                                         size_t *size);
i32 json_write_get_string_size(const struct json_string_s *string,
                               size_t *size) {
  size_t i;
...
json_write_minified_get_array_size(const struct json_array_s *array,
                                   size_t *size);
i32 json_write_minified_get_array_size(const struct json_array_s *array,
                                       size_t *size) {
  struct json_array_element_s *element;
...
json_write_minified_get_object_size(const struct json_object_s *object,
                                    size_t *size);
i32 json_write_minified_get_object_size(const struct json_object_s *object,
                                        size_t *size) {
  struct json_object_element_s *element;
...
json_write_minified_get_value_size(const struct json_value_s *value,
                                   size_t *size);
i32 json_write_minified_get_value_size(const struct json_value_s *value,
                                       size_t *size) {
  switch (value->type) {
  default:
...
json_weak t8 *json_write_minified_value(const struct json_value_s *value,
                                          t8 *data);

json_weak t8 *json_write_number(const struct json_number_s *number,
                                  t8 *data);
t8 *json_write_number(const struct json_number_s *number, t8 *data) {
  ...
  if (number->number_size >= 2) {
    ...
    case 'X':
      ...
      do {
        *(data + i - 1) = '0' + (t8)(parsed_number % 10);
        parsed_number /= 10;
        i--;
      } while (0 != parsed_number);
...
json_weak t8 *json_write_string(const struct json_string_s *string,
                                  t8 *data);
t8 *json_write_string(const struct json_string_s *string, t8 *data) {
  size_t i;
...
json_weak t8 *json_write_minified_array(const struct json_array_s *array,
                                          t8 *data);
t8 *json_write_minified_array(const struct json_array_s *array, t8 *data) {
  struct json_array_element_s *element = json_null;
...
json_weak t8 *json_write_minified_object(const struct json_object_s *object,
                                           t8 *data);
t8 *json_write_minified_object(const struct json_object_s *object,
                                 t8 *data) {
  struct json_object_element_s *element = json_null;
...
json_weak t8 *json_write_minified_value(const struct json_value_s *value,
                                          t8 *data);
t8 *json_write_minified_value(const struct json_value_s *value, t8 *data) {
  switch (value->type) {
  default:
    /* unknown value type found! */
    return json_null;
...
uk json_write_minified(const struct json_value_s *value, size_t *out_size) {
  size_t size = 0;
  t8 *data = json_null;
  t8 *data_end = json_null;
...
json_weak i32 json_write_pretty_get_value_size(const struct json_value_s *value,
                                               size_t depth, size_t indent_size,
                                               size_t newline_size,
                                               size_t *size);

json_weak i32 json_write_pretty_get_array_size(const struct json_array_s *array,
                                               size_t depth, size_t indent_size,
                                               size_t newline_size,
                                               size_t *size);
i32 json_write_pretty_get_array_size(const struct json_array_s *array,
                                     size_t depth, size_t indent_size,
...
json_write_pretty_get_object_size(const struct json_object_s *object,
                                  size_t depth, size_t indent_size,
                                  size_t newline_size, size_t *size);
i32 json_write_pretty_get_object_size(const struct json_object_s *object,
                                      size_t depth, size_t indent_size,
                                      size_t newline_size, size_t *size) {
  struct json_object_element_s *element;
...
json_weak i32 json_write_pretty_get_value_size(const struct json_value_s *value,
                                               ...
                                               size_t newline_size,
                                               size_t *size);
i32 json_write_pretty_get_value_size(const struct json_value_s *value,
                                     size_t depth, size_t indent_size,
                                     size_t newline_size, size_t *size) {
  switch (value->type) {
  default:
...
json_weak t8 *json_write_pretty_value(const struct json_value_s *value,
                                        size_t depth, const t8 *indent,
                                        const t8 *newline, t8 *data);

json_weak t8 *json_write_pretty_array(const struct json_array_s *array,
                                        size_t depth, const t8 *indent,
                                        const t8 *newline, t8 *data);
t8 *json_write_pretty_array(const struct json_array_s *array, size_t depth,
                              const t8 *indent, const t8 *newline,
                              t8 *data) {
  size_t k, m;
  struct json_array_element_s *element;
...
json_weak t8 *json_write_pretty_object(const struct json_object_s *object,
                                         size_t depth, const t8 *indent,
                                         const t8 *newline, t8 *data);
t8 *json_write_pretty_object(const struct json_object_s *object, size_t depth,
                               const t8 *indent, const t8 *newline,
                               t8 *data) {
  size_t k, m;
  struct json_object_element_s *element;
...
json_weak t8 *json_write_pretty_value(const struct json_value_s *value,
                                        size_t depth, const t8 *indent,
                                        const t8 *newline, t8 *data);
t8 *json_write_pretty_value(const struct json_value_s *value, size_t depth,
                              const t8 *indent, const t8 *newline,
                              t8 *data) {
  switch (value->type) {
  default:
...
uk json_write_pretty(const struct json_value_s *value, const t8 *indent,
                        const t8 *newline, size_t *out_size) {
  size_t size = 0;
