...
public:
	 ...
	 */
	virtual tresult PLUGIN_API getScreenSize (ViewRect* size, f32* scale) = 0;
	 ...
	 *	@return kResultTrue on success
	 */
	virtual tresult PLUGIN_API sendRemoteControlEvent (u32 event) = 0;

	/** ask for the host icon.
	 *	@param icon pointer to a CGImageRef
	 *	@return kResultTrue on success
	 */
	virtual tresult PLUGIN_API getHostIcon (uk* icon) = 0;
...
public:
	...
	*/
	virtual z0 PLUGIN_API onInterAppAudioConnectionStateChange (TBool newState) = 0;
