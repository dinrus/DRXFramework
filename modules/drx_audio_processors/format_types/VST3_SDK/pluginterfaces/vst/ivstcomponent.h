...
const i32 kDefaultFactoryFlags = PFactoryInfo::kUnicode; 
...
struct BusInfo
{
	...
	i32 channelCount;		///< number of channels (if used then need to be recheck after \ref
	...
	u32 flags;			///< flags - a combination of \ref BusFlags
	enum BusFlags
	{
...
struct RoutingInfo
{
	...
	i32 busIndex;			///< bus index
	i32 channel;			///< channel (-1 for all channels)
};
...
//------------------------------------------------------------------------
	...
	virtual i32 PLUGIN_API getBusCount (MediaType type, BusDirection dir) = 0;

	/** Called after the plug-in is initialized. See \ref MediaTypes, BusDirections */
	virtual tresult PLUGIN_API getBusInfo (MediaType type, BusDirection dir, i32 index, BusInfo& bus /*out*/) = 0;
	...
	virtual tresult PLUGIN_API activateBus (MediaType type, BusDirection dir, i32 index,
	                                        TBool state) = 0;
