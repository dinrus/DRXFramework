...
typedef u32 NoteExpressionTypeID;
/** Note Expression Value */
typedef f64 NoteExpressionValue;
/*@}*/
...
*/
enum NoteExpressionTypeIDs : u32
{
...
struct NoteExpressionValueDescription
{
	...
	i32 stepCount;					///< number of discrete steps (0: continuous, 1: toggle, discrete value otherwise - see \ref vst3ParameterIntro)
};
...
struct NoteExpressionValueEvent
{
	...
	i32 noteId;					///< associated note identifier to apply the change
...
struct NoteExpressionTextEvent
{
	...
	u32 textLen;					///< the number of characters (TChar) between the beginning of text and the terminating
...
struct NoteExpressionTypeInfo
{
	...
	i32 unitId;							///< id of unit this NoteExpression belongs to (see \ref vst3Units), in order to sort the note expression, it is possible to use unitId like for parameters. -1 means no unit used.
	...
	i32 flags;							///< NoteExpressionTypeFlags (see below)
...
public:
	...
	virtual i32 PLUGIN_API getNoteExpressionCount (i32 busIndex, i16 channel) = 0;

	/** Returns note change type info. */
	virtual tresult PLUGIN_API getNoteExpressionInfo (i32 busIndex, i16 channel, i32 noteExpressionIndex, NoteExpressionTypeInfo& info /*out*/) = 0;

	/** Gets a user readable representation of the normalized note change value. */
	virtual tresult PLUGIN_API getNoteExpressionStringByValue (i32 busIndex, i16 channel, NoteExpressionTypeID id, NoteExpressionValue valueNormalized /*in*/, String128 string /*out*/) = 0;

	/** Converts the user readable representation to the normalized note change value. */
	virtual tresult PLUGIN_API getNoteExpressionValueByString (i32 busIndex, i16 channel, NoteExpressionTypeID id, const TChar* string /*in*/, NoteExpressionValue& valueNormalized /*out*/) = 0;
...
struct KeyswitchInfo
{
	...
	i32 keyswitchMin;			///< associated main key switch min (value between [0, 127])
	i32 keyswitchMax;			///< associated main key switch max (value between [0, 127])
	i32 keyRemapped;			/** optional remapped key switch (default -1), the plug-in could provide one remapped
								key for a key switch (allowing better location on the keyboard of the key switches) */

	i32 unitId;				///< id of unit this key switch belongs to (see \ref vst3Units), -1 means no unit used.

	i32 flags;				///< not yet used (set to 0)
};
...
public:
	...
	virtual i32 PLUGIN_API getKeyswitchCount (i32 busIndex, i16 channel) = 0;

	/** Returns key switch info. */
	virtual tresult PLUGIN_API getKeyswitchInfo (i32 busIndex, i16 channel, i32 keySwitchIndex, KeyswitchInfo& info /*out*/) = 0;
