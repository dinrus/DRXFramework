...
struct NoteOnEvent
{
	i16 channel;		///< channel index in event bus
	i16 pitch;		///< range [0, 127] = [C-2, G8] with A3=440Hz (12-TET: twelve-tone equal temperament)
	f32 tuning;		///< 1.f = +1 cent, -1.f = -1 cent
	f32 velocity;		///< range [0.0, 1.0]
	i32 length;		///< in sample frames (optional, Note Off has to follow in any case!)
	i32 noteId;		///< note identifier (if not available then -1)
};
...
{
	i16 channel;		///< channel index in event bus
	i16 pitch;		///< range [0, 127] = [C-2, G8] with A3=440Hz (12-TET)
	f32 velocity;		///< range [0.0, 1.0]
	i32 noteId;		///< associated noteOn identifier (if not available then -1)
	f32 tuning;		///< 1.f = +1 cent, -1.f = -1 cent
};
...
struct DataEvent
{
	u32 size;		///< size in bytes of the data block bytes
	u32 type;		///< type of this data block (see \ref DataTypes)
	u8k* bytes;	///< pointer to the data block
...
{
	i16 channel;		///< channel index in event bus
	i16 pitch;		///< range [0, 127] = [C-2, G8] with A3=440Hz
	f32 pressure;		///< range [0.0, 1.0]
	i32 noteId;		///< event should be applied to the noteId (if not -1)
};
...
struct ChordEvent
{
	i16 root;			///< range [0, 127] = [C-2, G8] with A3=440Hz
	i16 bassNote;		///< range [0, 127] = [C-2, G8] with A3=440Hz
	i16 mask;			///< root is bit 0
	u16 textLen;		///< the number of characters (TChar) between the beginning of text and the terminating
...
struct ScaleEvent
{
	i16 root;			///< range [0, 127] = root Note/Transpose Factor
	i16 mask;			///< Bit 0 =  C,  Bit 1 = C#, ... (0x5ab5 = Major Scale)
	u16 textLen;		///< the number of characters (TChar) between the beginning of text and the terminating
						///< null character (without including the terminating null character itself)
...
{
	u8 controlNumber;///< see enum ControllerNumbers [0, 255]
	i8 channel;		///< channel index in event bus [0, 15]
	i8 value;			///< value of Controller [0, 127]
	i8 value2;		///< [0, 127] used for pitch bend (kPitchBend) and polyPressure (kCtrlPolyPressure)
};
...
struct Event
{
	i32 busIndex;				///< event bus index
	i32 sampleOffset;			///< sample frames related to the current block start sample position
	TQuarterNotes ppqPosition;	///< position in project
	u16 flags;				///< combination of \ref EventFlags
	...
	u16 type;				///< a value from \ref EventTypes
	union
	{
...
//------------------------------------------------------------------------
	...
	virtual i32 PLUGIN_API getEventCount () = 0;

	/** Gets parameter by index. */
	virtual tresult PLUGIN_API getEvent (i32 index, Event& e /*out*/) = 0;
