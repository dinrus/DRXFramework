...
blocks will be empty as i64 as the curve slope is 0.
...
\code{.cpp}
//------------------------------------------------------------------------
f64 x1 = -1; // position of last point related to current buffer
f64 y1 = currentParameterValue; // last transmitted value
...
f64 offset = y1 - (slope * x1);

f64 curveValue = (slope * bufferTime) + offset; // bufferTime is any position in buffer
\endcode
...
//------------------------------------------------------------------------
	...
	virtual i32 PLUGIN_API getPointCount () = 0;

	/** Gets the value and offset at a given index. */
	virtual tresult PLUGIN_API getPoint (i32 index, i32& sampleOffset /*out*/, ParamValue& value /*out*/) = 0;

	/** Adds a new value at the end of the queue, its index is returned. */
	virtual tresult PLUGIN_API addPoint (i32 sampleOffset, ParamValue value, i32& index /*out*/) = 0;
...
//------------------------------------------------------------------------
	...
	virtual i32 PLUGIN_API getParameterCount () = 0;

	/** Returns the queue at a given index. */
	virtual IParamValueQueue* PLUGIN_API getParameterData (i32 index) = 0;

	/** Adds a new parameter queue with a given ID at the end of the list,
	returns it and its index in the parameter changes list. */
	virtual IParamValueQueue* PLUGIN_API addParameterData (const ParamID& id, i32& index /*out*/) = 0;
