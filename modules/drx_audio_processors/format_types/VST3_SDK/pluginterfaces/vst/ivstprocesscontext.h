...
//------------------------------------------------------------------------
	...
	u32 framesPerSecond;		///< frame rate
	u32 flags;				///< flags #FrameRateFlags
//------------------------------------------------------------------------
};
...
{
//------------------------------------------------------------------------
	u8 keyNote;		///< key note in chord
	u8 rootNote;		///< lowest note in chord
...
//------------------------------------------------------------------------
	...
	u32 state;					///< a combination of the values from \ref StatesAndFlags

	f64 sampleRate;				///< current sample rate					(always valid)
	TSamples projectTimeSamples;	///< project time in samples				(always valid)

	z64 systemTime;				///< system time in nanoseconds					(optional)
	...
	f64 tempo;					///< tempo in BPM (Beats Per Minute)			(optional)
	i32 timeSigNumerator;			///< time signature numerator (e.g. 3 for 3/4)	(optional)
	i32 timeSigDenominator;		///< time signature denominator (e.g. 4 for 3/4) (optional)

	Chord chord;					///< musical info								(optional)

	i32 smpteOffsetSubframes;		///< SMPTE (sync) offset in subframes (1/80 of frame) (optional)
	FrameRate frameRate;			///< frame rate									(optional)

	i32 samplesToNextClock;		///< MIDI Clock Resolution (24 Per Quarter Note), can be negative (nearest) (optional)
//------------------------------------------------------------------------
};
