...
z0 popupContextMenu (IComponentHandler* componentHandler, IPlugView* view, const ParamID* paramID, UCoord x, UCoord y)
{
...
public:
	...
	virtual tresult PLUGIN_API executeMenuItem (i32 tag) = 0;
	//------------------------------------------------------------------------
	static const FUID iid;
};
...
struct IContextMenuItem
{
	...
	i32 tag;										///< Identifier tag of the item
	i32 flags;									///< Flags of the item
...
public:
	...
	virtual i32 PLUGIN_API getItemCount () = 0;

	/** Gets a menu item and its target (target could be not assigned). */
	virtual tresult PLUGIN_API getItem (i32 index, Item& item /*out*/, IContextMenuTarget** target /*out*/) = 0;
