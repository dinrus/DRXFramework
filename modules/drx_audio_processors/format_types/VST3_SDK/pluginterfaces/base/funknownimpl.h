...
struct ITest : public U::Unknown
{
    ...
    virtual b8 equal (i32 a, i32 b) const = 0;
};

struct TestImpl : public U::Extends<FooImpl, U::Directly<ITest>>
{
    b8 equal (i32 a, i32 b) const override { return a == b; }
};
@endcode
...
{
    AlternativeFooImpl (i32 dummy = 0) : dummy {dummy} {}
    z0 bar () override {}
...
struct Destroyer
{
	...
	static z0 destroy (UnknownT* ptr)
	{
		if (!!ptr)
			ptr->release ();
	}
 ...
 */
template <u32 t1, u32 t2, u32 t3, u32 t4>
struct UID
{
	enum : i8
	{
		l1_1 = static_cast<i8> ((t1 & 0xFF000000) >> 24),
		l1_2 = static_cast<i8> ((t1 & 0x00FF0000) >> 16),
		l1_3 = static_cast<i8> ((t1 & 0x0000FF00) >> 8),
		l1_4 = static_cast<i8> ((t1 & 0x000000FF)),
		l2_1 = static_cast<i8> ((t2 & 0xFF000000) >> 24),
		l2_2 = static_cast<i8> ((t2 & 0x00FF0000) >> 16),
		l2_3 = static_cast<i8> ((t2 & 0x0000FF00) >> 8),
		l2_4 = static_cast<i8> ((t2 & 0x000000FF)),
		l3_1 = static_cast<i8> ((t3 & 0xFF000000) >> 24),
		l3_2 = static_cast<i8> ((t3 & 0x00FF0000) >> 16),
		l3_3 = static_cast<i8> ((t3 & 0x0000FF00) >> 8),
		l3_4 = static_cast<i8> ((t3 & 0x000000FF)),
		l4_1 = static_cast<i8> ((t4 & 0xFF000000) >> 24),
		l4_2 = static_cast<i8> ((t4 & 0x00FF0000) >> 16),
		l4_3 = static_cast<i8> ((t4 & 0x0000FF00) >> 8),
		l4_4 = static_cast<i8> ((t4 & 0x000000FF))
	};
...
{
	I* out = nullptr;
	return u && u->queryInterface (getTUID<I> (), reinterpret_cast<uk*> (&out)) == kResultOk ?
	           owned (out) :
	           nullptr;
}
...
//------------------------------------------------------------------------
	...
	u32 PLUGIN_API addRef () { return ++refCount; }

	u32 PLUGIN_API release ()
	{
		if (--refCount == 0)
		{
			destroyInstance ();
...
//------------------------------------------------------------------------
	...
	u32 PLUGIN_API addRef () { return 1000; }
	u32 PLUGIN_API release () { return 1000; }
...
{
//------------------------------------------------------------------------
	tresult PLUGIN_API queryInterface (const TUID /*iid*/, uk* obj)
	{
		*obj = nullptr;
		return kNoInterface;
	}
...
//------------------------------------------------------------------------
	...
	tresult PLUGIN_API queryInterface (const TUID tuid, uk* obj) override
	{
		if (!obj)
	...
	u32 PLUGIN_API addRef () override { return BaseClass::addRef (); }
	u32 PLUGIN_API release () override { return BaseClass::release (); }
...
private:
	...
	inline constexpr b8 match (const TUID tuid) const noexcept
	{
		return reinterpret_cast<const zu64*> (tuid)[0] ==
		           reinterpret_cast<const zu64*> (getTUID<Interface> ())[0] &&
		       reinterpret_cast<const zu64*> (tuid)[1] ==
		           reinterpret_cast<const zu64*> (getTUID<Interface> ())[1];
	}

	template <i32 = 0>
	inline constexpr b8 queryInterfaceImpl (const TUID, uk&) const noexcept
	{
		return false;
	}

	template <typename Interface, typename... RemainingInterfaces>
	inline b8 queryInterfaceImpl (const TUID tuid, uk& obj) noexcept
	{
	...
	template <i32 = 0>
	inline constexpr uk getInterface (const TUID) const noexcept
	{
		return nullptr;
	}

	template <typename Interface, typename... RemainingInterfaces>
	inline uk getInterface (const TUID tuid) noexcept
	{
