...
//------------------------------------------------------------------------
	...
	virtual tresult PLUGIN_API read (uk buffer, i32 numBytes, i32* numBytesRead = nullptr) = 0;
	...
	virtual tresult PLUGIN_API write (uk buffer, i32 numBytes, i32* numBytesWritten = nullptr) = 0;
	...
	virtual tresult PLUGIN_API seek (z64 pos, i32 mode, z64* result = nullptr) = 0;
	
	/** Gets current stream read-write position. 
	\param pos : is assigned the current position if function succeeds */
	virtual tresult PLUGIN_API tell (z64* pos) = 0;
//------------------------------------------------------------------------
...
//------------------------------------------------------------------------
	...
	virtual tresult PLUGIN_API getStreamSize (z64& size) = 0;
	/** Set the steam size. File streams can only be resized if they are write enabled. */
	virtual tresult PLUGIN_API setStreamSize (z64 size) = 0;
