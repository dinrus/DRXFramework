...
//------------------------------------------------------------------------
	...
	UString (char16* buffer, i32 size) : thisBuffer (buffer), thisSize (size) {}

	/** returns buffer size */
	i32 getSize () const { return thisSize; }
	...
	UString& assign (const char16* src, i32 srcSize = -1);

	/** Append UTF-16 buffer (srcSize is in code unit (count of char16)). */
	UString& append (const char16* src, i32 srcSize = -1);

	/** Copy to UTF-16 buffer (dstSize is in code unit (count of char16)). */
	const UString& copyTo (char16* dst, i32 dstSize) const;

	/** Copy from ASCII string (srcSize is in code unit (count of char16)). */
	UString& fromAscii (tukk src, i32 srcSize = -1);
	UString& assign (tukk src, i32 srcSize = -1) { return fromAscii (src, srcSize); }

	/** Copy to ASCII string. */
	const UString& toAscii (tuk dst, i32 dstSize) const;

	/** Scan integer from string. */
	b8 scanInt (z64& value) const;

	/** Print integer to string. */
	b8 printInt (z64 value);

	/** Scan f32 from string. */
	b8 scanFloat (f64& value) const;

	/** Print f32 to string. */
	b8 printFloat (f64 value, i32 precision = 4);
//------------------------------------------------------------------------
protected:
	char16* thisBuffer;
	i32 thisSize; ///< size in code unit (not in byte!)
};

//------------------------------------------------------------------------
/** UTF-16 string with fixed buffer size.
 */
template <i32 maxSize>
...
//------------------------------------------------------------------------
	...
	UStringBuffer (const char16* src, i32 srcSize = -1) : UString (data, maxSize)
	{
		data[0] = 0;
		if (src)
			assign (src, srcSize);
	...
	UStringBuffer (tukk src, i32 srcSize = -1) : UString (data, maxSize)
	{
		data[0] = 0;
		if (src)
