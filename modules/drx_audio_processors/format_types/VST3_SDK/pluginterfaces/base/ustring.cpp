...
z0 StringCopy (TDstChar* dst, i32 dstSize, const TSrcChar* src, i32 srcSize = -1)
{
	i32 count = dstSize;
...
i32 StringLength (const TSrcChar* src, i32 srcSize = -1)
{
	if (srcSize == 0)
		return 0;
	i32 length = 0;
...
// UString
//------------------------------------------------------------------------
i32 UString::getLength () const
{
	return StringLength<char16> (thisBuffer, thisSize);
}

//------------------------------------------------------------------------
UString& UString::assign (const char16* src, i32 srcSize)
{
...
//------------------------------------------------------------------------
UString& UString::append (const char16* src, i32 srcSize)
{
...
//------------------------------------------------------------------------
const UString& UString::copyTo (char16* dst, i32 dstSize) const
{
...
//------------------------------------------------------------------------
UString& UString::fromAscii (tukk src, i32 srcSize)
{
	StringCopy<char16, t8> (thisBuffer, thisSize, src, srcSize);
	return *this;
}

//------------------------------------------------------------------------
const UString& UString::toAscii (tuk dst, i32 dstSize) const
{
	StringCopy<t8, char16> (dst, dstSize, thisBuffer, thisSize);
	return *this;
}

//------------------------------------------------------------------------
b8 UString::scanFloat (f64& value) const
{
#if SMTG_OS_WINDOWS
...
//------------------------------------------------------------------------
b8 UString::printFloat (f64 value, i32 precision)
{
#if SMTG_OS_WINDOWS
	...
	return result;
#elif SMTG_OS_LINUX
	auto utf8Buffer = reinterpret_cast<tuk> (thisBuffer);
...
//------------------------------------------------------------------------
b8 UString::scanInt (z64& value) const
{
#if SMTG_OS_WINDOWS
