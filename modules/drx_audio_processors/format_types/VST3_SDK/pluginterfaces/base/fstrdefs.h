...
//----------------------------------------------------------------------------
template <class T>
inline SMTG_CONSTEXPR14 i32 _tstrlen (const T* wcs)
{
	const T* eos = wcs;
	...
	return (i32) (eos - wcs - 1);
}

inline SMTG_CONSTEXPR14 i32 tstrlen (const tchar* str) {return _tstrlen (str);}
inline SMTG_CONSTEXPR14 i32 strlen8 (const char8* str) {return _tstrlen (str);}
inline SMTG_CONSTEXPR14 i32 strlen16 (const char16* str) {return _tstrlen (str);}

//----------------------------------------------------------------------------
template <class T>
inline SMTG_CONSTEXPR14 i32 _tstrcmp (const T* src, const T* dst)
{
	while (*src == *dst && *dst)
	{
		src++;
...
inline SMTG_CONSTEXPR14 i32 tstrcmp (const tchar* src, const tchar* dst) {return _tstrcmp (src, dst);}
inline SMTG_CONSTEXPR14 i32 strcmp8 (const char8* src, const char8* dst) {return _tstrcmp (src, dst);}
inline SMTG_CONSTEXPR14 i32 strcmp16 (const char16* src, const char16* dst) {return _tstrcmp (src, dst);}

template <typename T>
inline SMTG_CONSTEXPR14 i32 strcmpT (const T* first, const T* last);

template <>
inline SMTG_CONSTEXPR14 i32 strcmpT<char8> (const char8* first, const char8* last) { return _tstrcmp (first, last); }

template <>
inline SMTG_CONSTEXPR14 i32 strcmpT<char16> (const char16* first, const char16* last) { return _tstrcmp (first, last); }

//----------------------------------------------------------------------------
template <class T>
inline SMTG_CONSTEXPR14 i32 _tstrncmp (const T* first, const T* last, u32 count)
{
	if (count == 0)
		return 0;
	...
	if (*last == 0)
		return 1;
	return (i32) (*first - *last);
}

inline SMTG_CONSTEXPR14 i32 tstrncmp (const tchar* first, const tchar* last, u32 count) {return _tstrncmp (first, last, count);}
inline SMTG_CONSTEXPR14 i32 strncmp8 (const char8* first, const char8* last, u32 count) {return _tstrncmp (first, last, count);}
inline SMTG_CONSTEXPR14 i32 strncmp16 (const char16* first, const char16* last, u32 count) {return _tstrncmp (first, last, count);}

template <typename T>
inline SMTG_CONSTEXPR14 i32 strncmpT (const T* first, const T* last, u32 count);

template <>
inline SMTG_CONSTEXPR14 i32 strncmpT<char8> (const char8* first, const char8* last, u32 count) { return _tstrncmp (first, last, count); }

template <>
inline SMTG_CONSTEXPR14 i32 strncmpT<char16> (const char16* first, const char16* last, u32 count) {return _tstrncmp (first, last, count); }
...
//----------------------------------------------------------------------------
template <class T>
inline SMTG_CONSTEXPR14 T* _tstrncpy (T* dest, const T* source, u32 count)
{
	T* start = dest;
...
inline SMTG_CONSTEXPR14 tchar* tstrncpy (tchar* dest, const tchar* source, u32 count) {return _tstrncpy (dest, source, count);}
inline SMTG_CONSTEXPR14 char8* strncpy8 (char8* dest, const char8* source, u32 count) {return _tstrncpy (dest, source, count);}
inline SMTG_CONSTEXPR14 char16* strncpy16 (char16* dest, const char16* source, u32 count) {return _tstrncpy (dest, source, count);}
...
//----------------------------------------------------------------------------
inline SMTG_CONSTEXPR14 z0 str8ToStr16 (char16* dst, const char8* src, i32 n = -1)
{
	i32 i = 0;
	for (;;)
	{
...
//------------------------------------------------------------------------
inline SMTG_CONSTEXPR14 b8 FIDStringsEqual (FIDString id1, FIDString id2)
{
	return (id1 && id2) ? (strcmp8 (id1, id2) == 0) : false;
}

static SMTG_CONSTEXPR u32k kPrintfBufferSize = 4096;
