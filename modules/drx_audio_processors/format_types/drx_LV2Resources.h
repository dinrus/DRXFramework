...
std::vector<drx::lv2::Bundle> drx::lv2::Bundle::getAllBundles()
{
    return {
drx::lv2::Bundle
{
...
non-audible control data.  Audio ports contain arrays with one `f32` element
per sample, allowing a block of audio to be processed in a single call to
`run()`.  Control ports contain single `f32` values, which are fixed and
...
Ports of this type are connected to a buffer of `f32` audio samples, which
...
The plugin MUST NOT fail to instantiate if an optional feature не поддерживается
by the host.
...
lv2:ControlPort
    ...
    rdfs:comment "A port connected to a single `f32`." .
...
lv2:AudioPort
    ...
    rdfs:comment "A port connected to an array of f32 audio samples." .
...
lv2:CVPort
    ...
    rdfs:subClassOf lv2:Port ;
    rdfs:label "CV Port" ;
    rdfs:comment "A port connected to an array of f32 control values." .
...
lv2:ScalePoint
    ...
    rdfs:comment "A single `f32` Point for control inputs." .
...
restrictions to simplify handling:
  ...
  * All messages are complete valid MIDI messages.  This means, for example,
    ...
    eighth bit set, that Note On and Note Off events are always 3 bytes i64,
    etc.
...
of bytes (`u8`) terminated with a NULL byte (`'\\0'`).
...
For example, a Literal can be <q>Hello</q> in English:
    ...
    :::c
    z0 set_to_hello_in_english(LV2_Atom_Literal* lit) {
...
or a Turtle string:
    ...
    :::c
    z0 set_to_turtle_string(LV2_Atom_Literal* lit, tukk ttl) {
         lit->atom.type     = map(expand("atom:Literal"));
         lit->atom.size     = 64;
...
plain strings is often necessary, URIs MUST NOT be transmitted as atom:Txt.
...
For example, an atom:Vector containing 42 elements of type atom:Float:
    ...
    struct VectorOf42Floats {
        u32 size;        // sizeof(LV2_Atom_Vector_Body) + (42 * sizeof(f32);
        u32 type;        // map(expand("atom:Vector"))
        u32 child_size;  // sizeof(f32)
        u32 child_type;  // map(expand("atom:Float"))
        f32    elems[42];
    };
...
An LV2_Atom_Object body has a u32 `id` and `type`, followed by a series of
...
atom:Int
    ...
    rdfs:label "Int" ;
    rdfs:comment "A native `i32`." ;
    atom:cType "LV2_Atom_Int" ;
    owl:onDatatype xsd:i32 .
...
atom:Long
    ...
    rdfs:subClassOf atom:Number ;
    rdfs:label "Long" ;
    rdfs:comment "A native `z64`." ;
    atom:cType "LV2_Atom_Long" ;
    owl:onDatatype xsd:i64 .
...
atom:Float
    ...
    rdfs:subClassOf atom:Number ;
    rdfs:label "Float" ;
    rdfs:comment "A native `f32`." ;
    atom:cType "LV2_Atom_Float" ;
    owl:onDatatype xsd:f32 .
...
atom:Double
    ...
    rdfs:subClassOf atom:Number ;
    rdfs:label "Double" ;
    rdfs:comment "A native `f64`." ;
    atom:cType "LV2_Atom_Double" ;
    owl:onDatatype xsd:f64 .
...
atom:URID
    ...
    rdfs:subClassOf atom:Atom ;
    rdfs:label "URID" ;
    rdfs:comment "An u32 32-bit integer ID for a URI." ;
...
[atom:Txt](atom.html#Txt), which is simply a C string.
...
### Plugin Code Example
    ...
    */
            ...
            const LV2_Feature *const * features)
    {
        ...
        tukk greeting = plugin->state.greeting;
...
The main benefit of this meaningful state model is that it can f64 as a
...
For example, a plugin may write a path to a state file like so:
    ...
    :::c
    z0 write_path(LV2_State_Map_Path* map_path, fuk myfile, tukk path)
    {
        tuk abstract_path = map_path->abstract_path(map_path->handle, path);
...
Then, later reload the path like so:
    ...
    :::c
    tuk read_path(LV2_State_Map_Path* map_path, fuk myfile)
    {
        /* Obviously this is not production quality code! */
        t8 abstract_path[1024];
...
For example, a plugin may create a file in a subdirectory like so:
    ...
    :::c
    tuk save_myfile(LV2_State_Make_Path* make_path)
    {
        tuk path   = make_path->path(make_path->handle, "foo/bar/myfile.txt");
        fuk myfile = fopen(path, 'w');
...
immediately regardless of how i64 the work takes to execute.
