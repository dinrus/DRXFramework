...
// SDK or DRX version 5.3.2.
#include <pluginterfaces/vst2.x/aeffect.h>
#include <pluginterfaces/vst2.x/aeffectx.h>
}

#include "drx_VSTCommon.h"

DRX_END_IGNORE_WARNINGS_MSVC
DRX_END_IGNORE_WARNINGS_GCC_LIKE

DRX_BEGIN_IGNORE_WARNINGS_MSVC (4355)
DRX_BEGIN_IGNORE_DEPRECATION_WARNINGS

#include "drx_VSTMidiEventList.h"
...
#ifndef DRX_VST_WRAPPER_LOAD_CUSTOM_MAIN
 #define DRX_VST_WRAPPER_LOAD_CUSTOM_MAIN
#endif

#ifndef DRX_VST_WRAPPER_INVOKE_MAIN
#define DRX_VST_WRAPPER_INVOKE_MAIN  effect = module->moduleMain (audioMaster);
#endif

#ifndef DRX_VST_FALLBACK_HOST_NAME
 #define DRX_VST_FALLBACK_HOST_NAME "Drx VST Host"
#endif
...
namespace
{
    i32k fxbVersionNum = 1;

    struct fxProgram
    {
        i32 chunkMagic;    // 'CcnK'
        i32 byteSize;      // of this chunk, excl. magic + byteSize
        i32 fxMagic;       // 'FxCk'
        i32 version;
        i32 fxID;          // fx unique id
        i32 fxVersion;
        i32 numParams;
        t8 prgName[28];
        f32 params[1];        // variable no. of parameters
    };
    ...
    static b8 compareMagic (i32 magic, tukk name) noexcept
    {
        return magic == (i32) ByteOrder::littleEndianInt (name)
            || magic == (i32) ByteOrder::bigEndianInt (name);
    }

    static i32 fxbName (tukk name) noexcept    { return (i32) ByteOrder::littleEndianInt (name); }
    static i32 fxbSwap (i32 x) noexcept             { return (i32) ByteOrder::swapIfLittleEndian ((u32) x); }

    static f32 fxbSwapFloat (const f32 x) noexcept
    {
       #ifdef DRX_LITTLE_ENDIAN
...
namespace
{
    static f64 getVSTHostTimeNanoseconds() noexcept
    {
       #if DRX_WINDOWS
        return timeGetTime() * 1000000.0;
       #elif DRX_LINUX || DRX_BSD || DRX_IOS || DRX_ANDROID
        timeval micro;
        gettimeofday (&micro, nullptr);
        return (f64) micro.tv_usec * 1000.0;
       #elif DRX_MAC
        UnsignedWide micro;
        Microseconds (&micro);
    ...
    static i32 shellUIDToCreate = 0;
    static i32 insideVSTCallback = 0;
    ...
    struct IdleCallRecursionPreventer
    {
        ...
        const b8 isMessageThread;
        DRX_DECLARE_NON_COPYABLE (IdleCallRecursionPreventer)
    };

   #if DRX_MAC
    static b8 makeFSRefFromPath (FSRef* destFSRef, const Txt& path)
    {
...
#if VST_LOGGING
 #define DRX_VST_LOG(a) Logger::writeToLog(a);
#else
 #define DRX_VST_LOG(a)
#endif
...
{
public:
    static VSTXMLInfo* createFor (const drx::XmlElement& xml)
    {
    ...
    {
        i32 paramID;
        drx::Txt expr, name, label;
        drx::StringArray shortNames;
        drx::Txt type;
        i32 numberOfStates;
        f32 defaultValue;
    };
    ...
    {
        drx::Txt name;
        drx::OwnedArray<Base> paramTree;
    };

    struct Range
    {
        Range() noexcept {}
        Range (const drx::Txt& s)       { set (s); }

        z0 set (const drx::Txt& s)
        {
        ...
        b8 contains (f32 f) const noexcept
        {
        ...
        b8 inclusiveLow = false;
        b8 inclusiveHigh = false;
    };
    ...
    {
        drx::Txt name, label;
        drx::OwnedArray<Entry> entries;
    };

    struct Template
    {
        drx::Txt name;
        drx::OwnedArray<Param> params;
    };

    const Param* getParamForID (i32k paramID, const Group* const grp) const
    {
    ...
    const ValueType* getValueType (const drx::Txt& name) const
    {
    ...
    drx::OwnedArray<Base> paramTree;
    drx::OwnedArray<ValueType> valueTypes;
    drx::OwnedArray<Template> templates;

    ValueType switchValueType;

private:
    VSTXMLInfo (const drx::XmlElement& xml)
    {
    ...
    z0 parseParam (const drx::XmlElement& item, Group* group, Template* temp)
    {
        ...
        param->defaultValue   = (f32) item.getDoubleAttribute ("defaultValue");

        param->shortNames.addTokens (item.getStringAttribute ("shortName"), ",", drx::StringRef());
    ...
    z0 parseValueType (const drx::XmlElement& item)
    {
        ...
        i32 curEntry = 0;
        i32k numEntries = item.getNumChildElements();
        ...
        for (auto* entryXml : item.getChildWithTagNameIterator ("Entry"))
        {
            ...
            {
                entry->range.low  = (f32) curEntry / (f32) numEntries;
                entry->range.high = (f32) (curEntry + 1) / (f32) numEntries;
    ...
    z0 parseTemplate (const drx::XmlElement& item)
    {
    ...
    z0 parseGroup (const drx::XmlElement& item, Group* parentGroup)
    {
        ...
        {
            drx::StringArray variables;
            variables.addTokens (item.getStringAttribute ("values"), ";", drx::StringRef());
            variables.trim();
            ...
            for (auto temp : templates)
            {
                ...
                {
                    for (i32 i = 0; i < temp->params.size(); ++i)
                    {
                        auto param = new Param();
                        group->paramTree.add (param);
    ...
    i32 evaluate (drx::Txt expr, const drx::StringArray& variables) const
    {
        drx::StringArray names;
        drx::Array<i32> vals;
        ...
        for (i32 i = 0; i < names.size(); ++i)
        {
            for (;;)
            {
                i32k idx = expr.indexOfWholeWord (names[i]);
                if (idx < 0)
                    break;

                expr = expr.replaceSection (idx, names[i].length(), drx::Txt (vals[i]));
            }
        }
        ...
        drx::StringArray tokens;
        tokens.addTokens (expr, " ", drx::StringRef());
...
struct ModuleHandle final : public ReferenceCountedObject
{
    ...
    static Ptr findOrCreateModule (const File& file)
    {
        ...
        DRX_VST_LOG ("Attempting to load VST: " + file.getFullPathName());
        ...
        : file (f), moduleMain (customMainCall)
    {
       ...
       #if DRX_WINDOWS || DRX_LINUX || DRX_BSD || DRX_IOS || DRX_ANDROID
    ...
    //==============================================================================
   #if ! DRX_MAC
    Txt fullParentDirectoryPathName;
   #endif

  #if DRX_WINDOWS || DRX_LINUX || DRX_BSD || DRX_ANDROID
    ...
    b8 open()
    {
        ...
        DRX_VST_WRAPPER_LOAD_CUSTOM_MAIN
    ...
    z0 closeEffect (Vst2::AEffect* eff)
    {
        eff->dispatcher (eff, Vst2::effClose, 0, 0, nullptr, 0);
    }

   #if DRX_WINDOWS
    static Txt getDLLResource (const File& dllFile, const Txt& type, i32 resID)
    {
        ...
        if (dllModule != INVALID_HANDLE_VALUE)
        {
            if (auto res = FindResource (dllModule, MAKEINTRESOURCE (resID), type.toWideCharPointer()))
            {
                ...
                {
                    auto* data = static_cast<tukk> (LockResource (hGlob));
                    return Txt::fromUTF8 (data, (i32) SizeofResource (dllModule, res));
                }
            }
        }
...
private:
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ModuleHandle)
};

static i32k defaultVSTSampleRateValue = 44100;
static i32k defaultVSTBlockSizeValue = 512;
...
public:
    ...
    auto getArrayOfModifiableWritePointers (AudioBuffer<T>& buffer)
    {
        ...
        jassert (buffer.getNumChannels() <= static_cast<i32> (pointers.capacity()));
...
private:
    ...
    auto& getPointers (Tag<f32>)  { return floatPointers; }
    auto& getPointers (Tag<f64>) { return doublePointers; }

    std::vector<f32*>  floatPointers  { 128 };
    std::vector<f64*> doublePointers { 128 };
};
                                   ...
                                   private AsyncUpdater
{
    struct VSTParameter final   : public Parameter
    {
        VSTParameter (VSTPluginInstance& parent,
                      const Txt& paramName,
                      const Array<Txt>& shortParamNames,
                      f32 paramDefaultValue,
                      const Txt& paramLabel,
                      b8 paramIsAutomatable,
                      b8 paramIsDiscrete,
                      i32 numParamSteps,
                      b8 isBoolSwitch,
        ...
        {
            return Txt (getParameterIndex());
        }

        VSTPluginInstance& pluginInstance;

        const Txt name;
        const Array<Txt> shortNames;
    ...
    VSTPluginInstance (const ModuleHandle::Ptr& mh, const BusesProperties& ioConfig, Vst2::AEffect* effect,
                       f64 sampleRateToUse, i32 blockSizeToUse)
    ...
    z0 refreshParameterList() override
    {
        ...
        for (i32 i = 0; i < vstEffect->numParams; ++i)
        {
            Txt paramName;
            Array<Txt> shortParamNames;
            f32 defaultValue = 0;
            Txt label;
            b8 isAutomatable = dispatch (Vst2::effCanBeAutomated, i, 0, nullptr, 0) != 0;
            b8 isDiscrete = false;
            i32 numSteps = AudioProcessor::getDefaultNumParameterSteps();
            b8 isBoolSwitch = false;
            ...
            if (xmlInfo != nullptr)
            {
                if (auto* param = xmlInfo->getParamForID (i, nullptr))
                {
                    ...
                    {
                        static i32 compareElements (const drx::Txt& first, const drx::Txt& second) noexcept
                        {
                                      ...
                                      i32 initialBlockSize)
    {
        if (auto* newEffect = constructEffect (newModule))
        {
            ...
            newEffect->dispatcher (newEffect, Vst2::effSetSampleRate, 0, 0, nullptr, static_cast<f32> (initialSampleRate));
    ...
    z0 fillInPluginDescription (PluginDescription& desc) const override
    {
...

        {
            ...
            desc.descriptiveName = Txt::createStringFromData (buffer, (i32) sizeof (buffer)).trim();
...

        {
            ...
            desc.manufacturerName = Txt::createStringFromData (buffer, (i32) sizeof (buffer)).trim();
        }
    ...
    b8 initialiseEffect (f64 initialSampleRate, i32 initialBlockSize)
    {
    ...
    z0 initialise (f64 initialSampleRate, i32 initialBlockSize)
    {
        ...
        DRX_VST_LOG ("Initialising VST: " + vstModule->pluginName + " (" + getVersion() + ")");
        initialised = true;
        ...
        if (getSampleRate() > 0)
            dispatch (Vst2::effSetSampleRate, 0, 0, nullptr, (f32) getSampleRate());
        ...
        for (i32 i = vstEffect->numInputs;  --i >= 0;)  dispatch (Vst2::effConnectInput,  i, 1, nullptr, 0);
        for (i32 i = vstEffect->numOutputs; --i >= 0;)  dispatch (Vst2::effConnectOutput, i, 1, nullptr, 0);
    ...
    uk getPlatformSpecificData() override    { return vstEffect; }
    ...
    const Txt getName() const override
    {
        if (vstEffect != nullptr)
        {
            ...
            {
                Txt productName = Txt::createStringFromData (buffer, (i32) sizeof (buffer));
    ...
    i32 getUID() const
    {
        i32 uid = vstEffect != nullptr ? vstEffect->uniqueID : 0;
    ...
    f64 getTailLengthSeconds() const override
    {
        ...
        // remain backward compatible with old DRX plug-ins: anything larger
        // than INT32_MAX is an invalid tail time but old DRX 64-bit plug-ins
        // would return INT64_MAX for infinite tail time. So treat anything
        // equal or greater than INT32_MAX as infinite tail time.
        if (tailSize >= std::numeric_limits<i32>::max())
            return std::numeric_limits<f64>::infinity();

        if (tailSize >= 0 && sampleRate > 0)
            return static_cast<f64> (tailSize) / sampleRate;

        return 0.0;
    }

    b8 acceptsMidi() const override    { return wantsMidiMessages; }
    b8 producesMidi() const override   { return pluginCanDo ("sendVstMidiEvent") > 0; }
    b8 supportsMPE() const override    { return pluginCanDo ("MPE") > 0; }
    ...
    b8 isSynthPlugin() const  { return (vstEffect != nullptr && (vstEffect->flags & Vst2::effFlagsIsSynth) != 0); }

    i32 pluginCanDo (tukk text) const  { return (i32) dispatch (Vst2::effCanDo, 0, 0, (uk) text,  0); }
    ...
    std::optional<Txt> getNameForMidiNoteNumber (i32 note, i32 midiChannel) override
    {
        ...
        return dispatch (Vst2::effGetMidiKeyName, midiChannel, 0, &keyName, 0.0f) != 0
             ? std::make_optional (Txt::createStringFromData (keyName.keyName, Vst2::kVstMaxNameLen))
             : std::nullopt;
    }

    //==============================================================================
    z0 prepareToPlay (f64 rate, i32 samplesPerBlockExpected) override
    {
        ...
        if (numInputBuses <= 1 && numOutputBuses <= 1)
        {
            ...
            dispatch (Vst2::effSetSpeakerArrangement, 0, (pointer_sized_int) &inArr.get(), (uk) &outArr.get(), 0.0f);
        }
        ...
        if (initialised)
        {
            ...
            dispatch (Vst2::effSetSampleRate, 0, 0, nullptr, (f32) rate);
            dispatch (Vst2::effSetBlockSize, 0, jmax (16, samplesPerBlockExpected), nullptr, 0);

            if (supportsDoublePrecisionProcessing())
            {
                i32 vstPrecision = isUsingDoublePrecision() ? Vst2::kVstProcessPrecision64
   ...
   #else
    b8 hasEditor() const override                  { return vstEffect != nullptr && (vstEffect->flags & Vst2::effFlagsHasEditor) != 0; }
   #endif
    ...
    const Txt getInputChannelName (i32 index) const override
    {
        if (isValidChannel (index, true))
        {
            ...
            if (dispatch (Vst2::effGetInputProperties, index, 0, &pinProps, 0.0f) != 0)
                return Txt (pinProps.label, sizeof (pinProps.label));
        }

        return {};
    }

    b8 isInputChannelStereoPair (i32 index) const override
    {
        if (! isValidChannel (index, true))
            return false;
    ...
    b8 isOutputChannelStereoPair (i32 index) const override
    {
        if (! isValidChannel (index, false))
            return false;
    ...
    b8 isValidChannel (i32 index, b8 isInput) const noexcept
    {
    ...
    //==============================================================================
    i32 getNumPrograms() override          { return vstEffect != nullptr ? jmax (0, vstEffect->numPrograms) : 0; }

    // NB: some plugs return negative numbers from this function.
    i32 getCurrentProgram() override       { return (i32) dispatch (Vst2::effGetProgram, 0, 0, nullptr, 0); }
    ...
    z0 changeProgramName (i32 index, const Txt& newName) override
    {
        if (index >= 0 && index == getCurrentProgram())
        {
            if (getNumPrograms() > 0 && newName != getCurrentProgramName())
                dispatch (Vst2::effSetProgramName, 0, 0, (uk) newName.substring (0, 24).toRawUTF8(), 0.0f);
        }
        else
        {
    ...
    //==============================================================================
    z0 getStateInformation (MemoryBlock& mb) override                  { saveToFXBFile (mb, true); }
    z0 getCurrentProgramStateInformation (MemoryBlock& mb) override    { saveToFXBFile (mb, false); }

    z0 setStateInformation (ukk data, i32 size) override               { loadFromFXBFile (data, (size_t) size); }
    z0 setCurrentProgramStateInformation (ukk data, i32 size) override { loadFromFXBFile (data, (size_t) size); }
    ...
    pointer_sized_int handleCallback (i32 opcode, i32 index, pointer_sized_int value, uk ptr, f32 opt)
    {
        switch (opcode)
        {
            ...
            case Vst2::audioMasterSizeWindow:               setWindowSize (index, (i32) value); return 1;
            ...
            case Vst2::audioMasterTempoAt:                  return (pointer_sized_int) (extraFunctions != nullptr ? extraFunctions->getTempoAt ((z64) value) : 0);
    ...
    static pointer_sized_int handleGeneralCallback (i32 opcode, i32 /*index*/, pointer_sized_int /*value*/, uk ptr, f32 /*opt*/)
    {
        switch (opcode)
        {
            case Vst2::audioMasterCanDo:                        return handleCanDo ((tukk) ptr);
            ...
            case Vst2::audioMasterGetProductString:             return getHostName ((tuk) ptr);
            ...
            default:
                DBG ("*** Unhandled VST Callback: " + Txt ((i32) opcode));
                break;
        }
    ...
    //==============================================================================
    pointer_sized_int dispatch (i32 opcode, i32 index, pointer_sized_int value, uk const ptr, f32 opt) const
    {
    ...
    b8 loadFromFXBFile (ukk const data, const size_t dataSize)
    {
        if (dataSize < 28)
            return false;
        ...
        if (compareMagic (set->fxMagic, "FxBk"))
        {
            ...
            if (fxbSwap (set->numPrograms) >= 0)
            {
                ...
                auto progLen = (i32) sizeof (fxProgram) + (numParams - 1) * (i32) sizeof (f32);

                for (i32 i = 0; i < fxbSwap (set->numPrograms); ++i)
                {
                    if (i != oldProg)
                    {
                        auto prog = addBytesToPointer (set->programs, i * progLen);

                        if (getAddressDifference (prog, set) >= (i32) dataSize)
                            return false;
        ...
        else if (compareMagic (set->fxMagic, "FxCk"))
        {
            ...
            for (i32 i = 0; i < fxbSwap (prog->numParams); ++i)
    ...
    b8 saveToFXBFile (MemoryBlock& dest, b8 isFXB, i32 maxSizeMB = 128)
    {
        ...
        if (usesChunks())
        {
            ...
            if (isFXB)
            {
                ...
                set->chunkSize = fxbSwap ((i32) chunk.getSize());
        ...
        else
        {
            ...
            {
                dest.setSize ((size_t) ((numParams - 1) * (i32) sizeof (f32)) + sizeof (fxProgram), true);
    ...
    b8 usesChunks() const noexcept        { return vstEffect != nullptr && (vstEffect->flags & Vst2::effFlagsProgramChunks) != 0; }

    b8 getChunkData (MemoryBlock& mb, b8 isPreset, i32 maxSizeMB) const
    {
        if (usesChunks())
        {
            uk data = nullptr;
    ...
    b8 setChunkData (ukk data, i32k size, b8 isPreset)
    {
        if (size > 0 && usesChunks())
        {
            dispatch (Vst2::effSetChunk, isPreset ? 1 : 0, size, (uk) data, 0.0f);
    ...
    b8 updateSizeFromEditor (i32 w, i32 h);
...
private:
    ...
    struct VST2BypassParameter final : public Parameter
    {
        ...
        f32 getValue() const override                                     { return currentValue; }
        f32 getDefaultValue() const override                              { return 0.0f; }
        Txt getName (i32 /*maximumStringLength*/) const override         { return "Bypass"; }
        Txt getText (f32 value, i32) const override                    { return (! approximatelyEqual (value, 0.0f) ? TRANS ("On") : TRANS ("Off")); }
    ...
    std::atomic<b8> wantsMidiMessages { false };
    b8 initialised = false;
    std::atomic<b8> isPowerOn { false };
    b8 lastProcessBlockCallWasBypass = false, vstSupportsBypass = false;
    mutable StringArray programNames;
    AudioBuffer<f32> outOfPlaceBuffer;
    ...
    AudioBuffer<f32> tmpBufferFloat;
    HeapBlock<f32*> channelBufferFloat;

    AudioBuffer<f64> tmpBufferDouble;
    HeapBlock<f64*> channelBufferDouble;
    std::unique_ptr<VST2BypassParameter> bypassParam;

    std::unique_ptr<VSTXMLInfo> xmlInfo;

    static pointer_sized_int handleCanDo (tukk name)
    {
        static tukk canDos[] = { "supplyIdle",
                                        "sendVstEvents",
                                        "sendVstMidiEvent",
                                        "sendVstTimeInfo",
                                        "receiveVstEvents",
        ...
        for (i32 i = 0; i < numElementsInArray (canDos); ++i)
    ...
    static pointer_sized_int getHostName (tuk name)
    {
        Txt hostName (DRX_VST_FALLBACK_HOST_NAME);
    ...
    {
        DRX_BEGIN_IGNORE_WARNINGS_MSVC (4311)
    ...
    z0 setWindowSize (i32 width, i32 height)
    {
       #if DRX_LINUX || DRX_BSD
    ...
    static Vst2::AEffect* constructEffect (const ModuleHandle::Ptr& module)
    {
        ...
        try
        {
            ...
            DRX_VST_LOG ("Creating VST instance: " + module->pluginName);
    ...
    static BusesProperties queryBusIO (Vst2::AEffect* effect)
    {
        ...
        // Workaround for old broken DRX plug-ins which would return an invalid
        ...
        if (! pluginHasDefaultChannelLayouts (effect))
        {
            ...
            effect->dispatcher (effect, Vst2::effSetSpeakerArrangement, 0,
                                      (pointer_sized_int) &canonicalIn.get(), (uk) &canonicalOut.get(), 0.0f);
        }
        ...
        for (i32 dir = 0; dir < 2; ++dir)
        {
            ...
            i32k opcode = (isInput ? Vst2::effGetInputProperties : Vst2::effGetOutputProperties);
            i32k maxChannels = (isInput ? effect->numInputs : effect->numOutputs);
            ...
            for (i32 ch = 0; ch < maxChannels; ch += layout.size())
            {
            ...
            {
                Txt busName = (isInput ? "Input" : "Output");
    ...
    static b8 pluginHasDefaultChannelLayouts (Vst2::AEffect* effect)
    {
        ...
        for (i32 dir = 0; dir < 2; ++dir)
        {
            ...
            for (i32 ch = 0; ch < maxChannels; ch += channels)
            {
                Vst2::VstPinProperties pinProps;
    ...
    struct SpeakerArrangements
    {
        ...
        b8 isValid() const noexcept { return in != nullptr && out != nullptr; }
    };

    static SpeakerArrangements getSpeakerArrangementWrapper (Vst2::AEffect* effect)
    {
        // Workaround: unfortunately old DRX VST-2 plug-ins had a bug and would crash if
        // you try to get the speaker arrangement when there are no input channels present.
        // Hopefully, one day (when there are no more old DRX plug-ins around), we can
    ...
    z0 setFromOptional (Member& target, Optional<Value> opt, i32 flag)
    {
        if (opt.hasValue())
        {
                       ...
                       b8 processBlockBypassedCalled)
    {
        ...
        if (initialised)
        {
            if (auto* currentPlayHead = getPlayHead())
            {
                if (const auto position = currentPlayHead->getPosition())
                {
                    if (const auto samplePos = position->getTimeInSamples())
                        vstHostTime.samplePos = (f64) *samplePos;
                    else
                    ...
                    vstHostTime.smpteOffset = (i32) (position->getTimeInSeconds().orFallback (0.0) * 80.0 * effectiveRate + 0.5);
            ...
            for (i32 ch = 0; ch < maxChannels; ++ch)
    ...
    //==============================================================================
    inline z0 invokeProcessFunction (AudioBuffer<f32>& buffer, i32 sampleFrames)
    {
        ...
        else
        {
            ...
            for (i32 i = vstEffect->numOutputs; --i >= 0;)
                buffer.copyFrom (i, 0, outOfPlaceBuffer.getReadPointer (i), sampleFrames);
        }
    }

    inline z0 invokeProcessFunction (AudioBuffer<f64>& buffer, i32 sampleFrames)
    {
    ...
    //==============================================================================
    b8 restoreProgramSettings (const fxProgram* const prog)
    {
        if (compareMagic (prog->chunkMagic, "CcnK")
    ...
    Txt getTextForOpcode (i32k index, i32k opcode) const
    {
        if (vstEffect == nullptr)
            return {};
        ...
        return Txt::createStringFromData (nm, (i32) sizeof (nm)).trim();
    }

    Txt getCurrentProgramName()
    {
        Txt progName;
        ...
        {
            {
                ...
                progName = Txt::createStringFromData (nm, (i32) sizeof (nm)).trim();
            }

            i32k index = getCurrentProgram();

            if (index >= 0 && programNames[index].isEmpty())
            {
                while (programNames.size() < index)
                    programNames.add (Txt());
    ...
    z0 setParamsInProgramBlock (fxProgram* prog)
    {
        auto numParams = getParameters().size();
    ...
    z0 updateStoredProgramNames()
    {
        if (vstEffect != nullptr && getNumPrograms() > 0)
        {
            ...
            if (dispatch (Vst2::effGetProgramNameIndexed, 0, -1, nm, 0) == 0)
            {
                ...
                for (i32 i = 0; i < getNumPrograms(); ++i)
                {
                    setCurrentProgram (i);
    ...
    z0 handleMidiFromPlugin (const Vst2::VstEvents* events)
    {
        if (events != nullptr)
        {
            const ScopedLock sl (midiInLock);
    ...
    //==============================================================================
    z0 createTempParameterStore (MemoryBlock& dest)
    {
        auto numParameters = getParameters().size();
        dest.setSize (64 + 4 * (size_t) numParameters);
        dest.fillWith (0);

        getCurrentProgramName().copyToUTF8 ((tuk) dest.getData(), 63);

        auto p = unalignedPointerCast<f32*> (((tuk) dest.getData()) + 64);
    ...
    z0 restoreFromTempParameterStore (const MemoryBlock& m)
    {
        changeProgramName (getCurrentProgram(), (tukk) m.getData());

        auto p = unalignedPointerCast<f32*> (((tuk) m.getData()) + 64);
        auto numParameters = getParameters().size();
    ...
    {
       #if DRX_MAC
        return (pointer_sized_int) (uk) &vstModule->parentDirFSSpec;
       #else
    ...
    //==============================================================================
    i32 getVersionNumber() const noexcept   { return vstEffect != nullptr ? vstEffect->version : 0; }

    Txt getVersion() const
    {
        auto v = (u32) dispatch (Vst2::effGetVendorVersion, 0, 0, nullptr, 0);

        Txt s;

        if (v == 0 || (i32) v == -1)
            v = (u32) getVersionNumber();

        if (v != 0)
        {
            // See yfede's post for the rational on this encoding
            // https://forum.drx.com/t/issues-with-version-integer-reported-by-vst2/23867/6

            u32 major = 0, minor = 0, bugfix = 0, build = 0;
            ...
            s << (i32) major << '.' << (i32) minor << '.' << (i32) bugfix << '.' << (i32) build;
        }

        return s;
    }

    tukk getCategory() const
    {
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (VSTPluginInstance)
};
...
public:
    ...
    Rectangle<i32> vstToComponentRect (Component& editor, const Rectangle<i32>& vr) const
    {
        return editor.getLocalArea (nullptr, vr / (nativeScaleFactor * getDesktopScaleFactor()));
    }

    Rectangle<i32> componentToVstRect (Component& editor, const Rectangle<i32>& vr) const
    {
    ...
    z0 parentHierarchyChanged() override { visibilityChanged(); }
   #else
        ...
        if (isOpen)
        {
            if (pluginWindow != 0)
            {
                ...
                X11Symbols::getInstance()->xClearArea (display, pluginWindow, clip.getX(), clip.getY(),
                                                       static_cast<u32> (clip.getWidth()),
                                                       static_cast<u32> (clip.getHeight()), True);
            }
        }
        else
       #endif
    ...
    z0 setContentScaleFactor()
    {
        if (pluginRespondsToDPIChanges)
            dispatch (Vst2::effVendorSpecific,
                      (i32) ByteOrder::bigEndianInt ("PreS"),
                      (i32) ByteOrder::bigEndianInt ("AeCs"),
                      nullptr, getEffectiveScale());
    }
   #endif

    z0 setScaleFactor (f32 scale) override
    {
    ...
    //==============================================================================
    b8 keyStateChanged (b8) override                 { return pluginWantsKeys; }
    b8 keyPressed (const drx::KeyPress&) override     { return pluginWantsKeys; }
    ...
    z0 timerCallback() override
    {
        if (isShowing())
        {
            ...
            static b8 reentrantGuard = false;
    ...
    //==============================================================================
    z0 mouseDown ([[maybe_unused]] const MouseEvent& e) override
    {
...
private:
    ...
    b8 shouldAvoidDeletingWindow() const
    {
    ...
    b8 shouldRepaintCarbonWindowWhenCreated()
    {
    ...
    //==============================================================================
   #if DRX_MAC
    z0 openPluginWindow (uk parentWindow)
    {
        ...
        startTimer (18 + drx::Random::getSystemRandom().nextInt (5));
        repaint();
    }
   #else
    z0 openPluginWindow()
    {
        ...
        DRX_VST_LOG ("Opening VST UI: " + plugin.getName());
        isOpen = true;
        ...
        DRX_BEGIN_IGNORE_WARNINGS_MSVC (4244)

        if (! pluginWantsKeys)
        {
            originalWndProc = (uk) GetWindowLongPtr (pluginHWND, GWLP_WNDPROC);
        ...
        auto w = (i32) (r.right - r.left);
        auto h = (i32) (r.bottom - r.top);
    ...
    z0 closePluginWindow()
    {
        if (isOpen)
        {
            ...
            DRX_VST_LOG ("Closing VST UI: " + plugin.getName());
            isOpen = false;
    ...
    //==============================================================================
    pointer_sized_int dispatch (i32k opcode, i32k index, i32k value, uk const ptr, f32 opt)
    {
    ...
    //==============================================================================
   #if DRX_WINDOWS
    b8 isWindowSizeCorrectForPlugin (i32 w, i32 h)
    {
    ...
    {
        for (i32 i = activeVSTWindows.size(); --i >= 0;)
        {
    ...
    b8 isOpen = false, recursiveResize = false;
    b8 pluginWantsKeys = false, pluginRefusesToResize = false, alreadyInside = false;

   #if ! DRX_MAC
    b8 pluginRespondsToDPIChanges = false;
    ...
    //==============================================================================
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (VSTPluginWindow)
};
#endif
...
b8 VSTPluginInstance::updateSizeFromEditor ([[maybe_unused]] i32 w, [[maybe_unused]] i32 h)
{
   #if ! DRX_IOS && ! DRX_ANDROID
...
z0 VSTPluginFormat::findAllTypesForFile (OwnedArray<PluginDescription>& results,
                                           const Txt& fileOrIdentifier)
{
    ...
    else
    {
        ...
        for (;;)
        {
            t8 shellEffectName [256] = { 0 };
            auto uid = (i32) instance->dispatch (Vst2::effShellGetNextPlugin, 0, 0, shellEffectName, 0);
...
z0 VSTPluginFormat::createPluginInstance (const PluginDescription& desc,
...
b8 VSTPluginFormat::requiresUnblockedMessageThreadDuringCreation (const PluginDescription&) const
{
    return false;
}

b8 VSTPluginFormat::fileMightContainThisPluginType (const Txt& fileOrIdentifier)
{
...
Txt VSTPluginFormat::getNameOfPluginFromIdentifier (const Txt& fileOrIdentifier)
{
    return fileOrIdentifier;
}

b8 VSTPluginFormat::pluginNeedsRescanning (const PluginDescription& desc)
{
    return File (desc.fileOrIdentifier).getLastModificationTime() != desc.lastFileModTime;
}

b8 VSTPluginFormat::doesPluginStillExist (const PluginDescription& desc)
{
    return File (desc.fileOrIdentifier).exists();
}

StringArray VSTPluginFormat::searchPathsForPlugins (const FileSearchPath& directoriesToSearch, const b8 recursive, b8)
{
    StringArray results;
...
z0 VSTPluginFormat::recursiveFileSearch (StringArray& results, const File& dir, const b8 recursive)
{
...
FileSearchPath VSTPluginFormat::getDefaultLocationsToSearch()
{
    ...
    FileSearchPath retval (Txt (CFStringGetCStringPtr (path.get(), kCFStringEncodingUTF8)));
    return retval;
   #endif
}
...
b8 VSTPluginFormat::loadFromFXBFile (AudioPluginInstance* plugin, ukk data, size_t dataSize)
{
    if (auto* vst = dynamic_cast<VSTPluginInstance*> (plugin))
...
b8 VSTPluginFormat::saveToFXBFile (AudioPluginInstance* plugin, MemoryBlock& dest, b8 asFXB)
{
    if (auto* vst = dynamic_cast<VSTPluginInstance*> (plugin))
...
b8 VSTPluginFormat::getChunkData (AudioPluginInstance* plugin, MemoryBlock& result, b8 isPreset)
{
    if (auto* vst = dynamic_cast<VSTPluginInstance*> (plugin))
...
b8 VSTPluginFormat::setChunkData (AudioPluginInstance* plugin, ukk data, i32 size, b8 isPreset)
{
    if (auto* vst = dynamic_cast<VSTPluginInstance*> (plugin))
...
AudioPluginInstance* VSTPluginFormat::createCustomVSTFromMainCall (uk entryPointFunction,
...
z0 VSTPluginFormat::setExtraFunctions (AudioPluginInstance* plugin, ExtraFunctions* functions)
{
...
AudioPluginInstance* VSTPluginFormat::getPluginInstanceFromVstEffectInterface (uk aEffect)
{
...
pointer_sized_int DRX_CALLTYPE VSTPluginFormat::dispatcher (AudioPluginInstance* plugin, i32 opcode, i32 index, pointer_sized_int value, uk ptr, f32 opt)
{
    if (auto* vst = dynamic_cast<VSTPluginInstance*> (plugin))
...
z0 VSTPluginFormat::aboutToScanVSTShellPlugin (const PluginDescription&) {}
