...
/**	Callback interface to access context information from the host. Implemented by the host
	...
	z0 PLUGIN_API MyEditController::notifyContextInfoChange ()
	{
		i32 channelIndex = 0;
...
{	
	...
	virtual Steinberg::tresult PLUGIN_API getContextInfoValue (Steinberg::i32& value, Steinberg::FIDString id) = 0;

	/** Get context information by identifier. */
	virtual Steinberg::tresult PLUGIN_API getContextInfoString (Steinberg::Vst::TChar* string, Steinberg::i32 maxCharCount, Steinberg::FIDString id) = 0;
...
struct IContextInfoProvider2: IContextInfoProvider
{
	...
	virtual Steinberg::tresult PLUGIN_API getContextInfoValue (f64& value, Steinberg::FIDString id) = 0;
		
	/** Set context information by identifier (floating-point). */
	virtual Steinberg::tresult PLUGIN_API setContextInfoValue (Steinberg::FIDString id, f64 value) = 0;
	
	/** Set context information by identifier (integer). */
	virtual Steinberg::tresult PLUGIN_API setContextInfoValue (Steinberg::FIDString id, Steinberg::i32 value) = 0;
...
struct IContextInfoHandler: Steinberg::FUnknown
{
	...
	virtual z0 PLUGIN_API notifyContextInfoChange () = 0;
...
struct IContextInfoHandler2: Steinberg::FUnknown
{
	...
	virtual z0 PLUGIN_API notifyContextInfoChange (Steinberg::FIDString id) = 0;
...
namespace ContextInfo
{
	...
	const Steinberg::FIDString kType = "type";						///< (R) channel type (i32, see ChannelType enumeration)
	const Steinberg::FIDString kMain = "main";						///< (R) channel is main output (i32, 0: false, 1: true)
	const Steinberg::FIDString kIndex = "index";					///< (R) channel index (i32, starts at zero)
	const Steinberg::FIDString kColor = "color";					///< (R/W) channel color (i32: RGBA)
	const Steinberg::FIDString kVisibility = "visibility";			///< (R) channel visibility (i32, 0: false, 1: true)
	const Steinberg::FIDString kSelected = "selected";				///< (R/W) selection state, channel is selected exlusively and scrolled into view on write (i32, 0: false, 1: true)
	const Steinberg::FIDString kMultiSelect = "multiselect";		///< (W) select channel without unselecting others (i32, 0: false, 1: true)
	const Steinberg::FIDString kFocused = "focused";				///< (R) focus for user input when multiple channels are selected (i32, 0: false, 1: true)

	const Steinberg::FIDString kRegionName = "regionName";			///< (R) name of region/event for region/event-based effects (string)
	const Steinberg::FIDString kRegionSelected = "regionSelected";	///< (R) selection state of region/event for region/event-based effects (i32, 0: false, 1: true)

	// per instance (requires IContextInfoHandler2 on plug-in side)
	const Steinberg::FIDString kVolume = "volume";					///< (R/W) volume factor [f32, 0. = -oo dB, 1. = 0dB, etc.], also available as string
	const Steinberg::FIDString kMaxVolume = "maxVolume";			///< (R) maximum volume factor [f32, 1. = 0dB], also available as string
	const Steinberg::FIDString kPan = "pan";						///< (R/W) stereo panning [f32, < 0.5 = (L), 0.5 = (C), > 0.5 = (R)], also available as string
	const Steinberg::FIDString kMute = "mute";						///< (R/W) mute (i32, 0: false, 1: true)
	const Steinberg::FIDString kSolo = "solo";						///< (R/W) solo (i32, 0: false, 1: true)	
	const Steinberg::FIDString kSendCount = "sendcount";			///< (R) send count [i32]
