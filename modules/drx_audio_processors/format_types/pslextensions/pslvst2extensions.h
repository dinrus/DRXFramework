...
namespace PlugCanDos
{
	...
	static tukk canDoViewResize = "supportsViewResize";

	/** Check if view can be embedded by the host. */
	static tukk canDoViewEmbedding = "supportsViewEmbedding";
	
	/** Check if view scaling for high-DPI is supported by the plug-in. */
	static tukk canDoViewDpiScaling = "supportsViewDpiScaling";

	/** Check if gain reduction reporting is supported by the plug-in. */
	static tukk canDoGainReductionInfo = "supportsGainReductionInfo";

	/** Check if slave effects are supported by plug-in. */
	static tukk canDoSlaveEffects = "supportsSlaveEffects";
}
...
enum Opcodes
{
	...
	/** Get current gain reduction for display. The ptrArg is a f32* to be set to the dB value.
		For more details, please check the documentation of Presonus::IGainReductionInfo. */
	kEffGetGainReductionValueInDb = 'GRdB',

	/** Add slave effect. The ptrArg is a pointer to the slave AEffect, the 'opt' f32 transmits the mode (see enum SlaveMode).
