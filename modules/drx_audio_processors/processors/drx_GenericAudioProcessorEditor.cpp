...
public:
    ...
    virtual z0 handleNewParameterValue() = 0;
...
private:
    ...
    //==============================================================================
    z0 audioProcessorParameterChanged (AudioProcessor*, i32 index, f32) override
    {
    ...
    Atomic<i32> parameterValueHasChanged { 0 };
    const b8 isLegacyParam;

    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ParameterListener)
};
...
public:
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ParameterComponent)
};
...
public:
    ...
    z0 handleNewParameterValue() override
    {
...
private:
    ...
    b8 isParameterOn() const    { return getParameter().getValue() >= 0.5f; }

    ToggleButton button;

    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (BooleanParameterComponent)
};
...
private:
    z0 rightButtonChanged()
    {
        ...
        if (isParameterOn() != buttonState)
        {
            ...
            else
            {
                ...
                // value using those strings, rather than a f32, because VSTs can
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (SwitchParameterComponent)
};
...
private:
    z0 handleNewParameterValue() override
    {
        ...
        if (index < 0)
        {
            // The parameter is producing some unexpected text, so we'll do
            // some linear interpolation.
            index = roundToInt (getParameter().getValue() * (f32) (parameterValues.size() - 1));
        }
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ChoiceParameterComponent)
};
...
private:
    ...
    z0 sliderValueChanged()
    {
        ...
        if (! approximatelyEqual (getParameter().getValue(), newVal))
        {
            ...
            getParameter().setValueNotifyingHost ((f32) slider.getValue());
            updateTextDisplay();
    ...
    z0 sliderStartedDragging()
    {
        isDragging = true;
        getParameter().beginChangeGesture();
    }

    z0 sliderStoppedDragging()
    {
        isDragging = false;
        getParameter().endChangeGesture();
    }
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (SliderParameterComponent)
};
...
private:
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ParameterDisplayComponent)
};
...
struct ParamControlItem final : public TreeViewItem
{
    ...
    i32 getItemHeight() const override { return 40; }
...
struct ParameterGroupItem final : public TreeViewItem
{
    ...
    b8 mightContainSubItems() override { return getNumSubItems() > 0; }
...
struct GenericAudioProcessorEditor::Pimpl
{
    ...
    static i32 getNumIndents (const TreeViewItem& item)
    {
        i32 maxInner = 0;
...
z0 GenericAudioProcessorEditor::paint (Graphics& g)
{
    g.fillAll (getLookAndFeel().findColor (ResizableWindow::backgroundColorId));
}

z0 GenericAudioProcessorEditor::resized()
{
