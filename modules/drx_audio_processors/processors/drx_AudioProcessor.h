...
/**
    ...
    @tags{Audio}
*/
class DRX_API  AudioProcessor : private AAXClientExtensions
{
protected:
...
public:
    ...
    virtual const Txt getName() const = 0;
    ...
    */
    virtual z0 prepareToPlay (f64 sampleRate,
                                i32 maximumExpectedSamplesPerBlock) = 0;
    ...
    */
    ...
    /** Renders the next block.
        ...
        @see getBusBuffer
    */
    virtual z0 processBlock (AudioBuffer<f32>& buffer,
    ...
    /** Renders the next block.
        ...
        @see getBusBuffer
    */
    virtual z0 processBlock (AudioBuffer<f64>& buffer,
    ...
    */
    virtual z0 processBlockBypassed (AudioBuffer<f32>& buffer,
                                       MidiBuffer& midiMessages);
    ...
    /** Renders the next block when the processor is being bypassed.
        ...
        and dry (bypassed) signals.
    */
    virtual z0 processBlockBypassed (AudioBuffer<f64>& buffer,
                                       MidiBuffer& midiMessages);
    ...
    */
    ...
    private:
        ...
        static auto& getBuses (This& t, b8 isInput) { return isInput ? t.inputBuses : t.outputBuses; }
    ...
    public:
        ...
        auto& getBuses (b8 isInput) const { return getBuses (*this, isInput); }
        auto& getBuses (b8 isInput)       { return getBuses (*this, isInput); }

        /** Get the number of channels of a particular bus */
        i32 getNumChannels (b8 isInput, i32 busIndex) const noexcept
        {
        ...
        AudioChannelSet& getChannelSet (b8 isInput, i32 busIndex) noexcept
        {
            return getBuses (isInput).getReference (busIndex);
        }

        /** Get the channel set of a particular bus */
        AudioChannelSet getChannelSet (b8 isInput, i32 busIndex) const noexcept
        {
        ...
        i32 getMainInputChannels()  const noexcept                  { return getNumChannels (true, 0); }

        /** Get the number of output channels on the main bus. */
        i32 getMainOutputChannels() const noexcept                  { return getNumChannels (false, 0); }

        b8 operator== (const BusesLayout& other) const noexcept   { return inputBuses == other.inputBuses && outputBuses == other.outputBuses; }
        b8 operator!= (const BusesLayout& other) const noexcept   { return inputBuses != other.inputBuses || outputBuses != other.outputBuses; }
    };
    ...
    {
    public:
        /** Возвращает true, если this bus is an input bus. */
        b8 isInput() const noexcept;

        /** Returns the index of this bus. */
        i32 getBusIndex() const noexcept;

        /** Возвращает true, если the current bus is the main input or output bus. */
        b8 isMain() const noexcept                                    { return getBusIndex() == 0; }
        ...
        */
        ...
        /** Checks if a particular layout is supported.
            ...
            @see AudioChannelSet
        */
        b8 isLayoutSupported (const AudioChannelSet& set, BusesLayout* currentLayout = nullptr) const;

        /** Checks if this bus can support a given number of channels. */
        b8 isNumberOfChannelsSupported (i32 channels) const;

        /** Returns a ChannelSet that the bus supports with a given number of channels. */
        AudioChannelSet supportedLayoutWithChannels (i32 channels) const;

        /** Returns the maximum number of channels that this bus can support.
            @param limit The maximum value to return.
        */
        i32 getMaxSupportedChannels (i32 limit = AudioChannelSet::maxChannelsOfNamedLayout) const;
        ...
        */
        ...
        //==============================================================================
        /** Возвращает true, если the current bus is enabled. */
        b8 isEnabled() const noexcept                             { return ! layout.isDisabled(); }

        /** Enable or disable this bus. This will return false if the AudioProcessor
            does not support disabling this bus. */
        b8 enable (b8 shouldEnable = true);

        /** Returns if this bus is enabled by default. */
        b8 isEnabledByDefault() const noexcept                    { return enabledByDefault; }
        ...
        */
        i32 getChannelIndexInProcessBlockBuffer (i32 channelIndex) const noexcept;
    ...
    private:
        ...
        Bus (AudioProcessor&, const Txt&, const AudioChannelSet&, b8);
        ...
        z0 updateChannelCount() noexcept;

        AudioProcessor& owner;
        Txt name;
        AudioChannelSet layout, dfltLayout, lastLayout;
        b8 enabledByDefault;
        i32 cachedChannelCount;

        DRX_DECLARE_NON_COPYABLE (Bus)
    };

    //==============================================================================
    /** Returns the number of buses on the input or output side */
    i32 getBusCount (b8 isInput) const noexcept                   { return (isInput ? inputBuses : outputBuses).size(); }

    /** Returns the audio bus with a given index and direction.
        If busIndex is invalid then this method will return a nullptr.
    */
    Bus* getBus (b8 isInput, i32 busIndex) noexcept               { return getBusImpl (*this, isInput, busIndex); }

    /** Returns the audio bus with a given index and direction.
        If busIndex is invalid then this method will return a nullptr.
    */
    const Bus* getBus (b8 isInput, i32 busIndex) const noexcept   { return getBusImpl (*this, isInput, busIndex); }
    ...
    /**  Callback to query if a bus can currently be added.
         ...
         @see addBus
    */
    virtual b8 canAddBus (b8 isInput) const;
    ...
    /**  Callback to query if the last bus can currently be removed.
         ...
         The default implementation will always return false.
    */
    virtual b8 canRemoveBus (b8 isInput) const;
    ...
    */
    b8 removeBus (b8 isInput);

    //==============================================================================
    /** Set the channel layouts of this audio processor.

        If the layout не поддерживается by this audio processor then
        ...
        processor supports.
    */
    b8 setBusesLayout (const BusesLayout&);
    ...
    */
    b8 setBusesLayoutWithoutEnabling (const BusesLayout&);
    ...
    */
    AudioChannelSet getChannelLayoutOfBus (b8 isInput, i32 busIndex) const noexcept;
    ...
    */
    b8 setChannelLayoutOfBus (b8 isInput, i32 busIndex, const AudioChannelSet& layout);
    ...
    */
    inline i32 getChannelCountOfBus (b8 isInput, i32 busIndex) const noexcept
    {
    ...
    /** Returns the position of a bus's channels within the processBlock buffer.
        ...
        maps onto a specific bus's channel.
     */
    i32 getChannelIndexInProcessBlockBuffer (b8 isInput, i32 busIndex, i32 channelIndex) const noexcept;
    ...
    */
    i32 getOffsetInBusBufferForAbsoluteChannelIndex (b8 isInput, i32 absoluteChannelIndex, i32& busIndex) const noexcept;
     ...
     */
    template <typename FloatType>
    AudioBuffer<FloatType> getBusBuffer (AudioBuffer<FloatType>& processBlockBuffer, b8 isInput, i32 busIndex) const
    {
    ...
    //==============================================================================
    /** Возвращает true, если the Audio processor is likely to support a given layout.
        This can be called regardless if the processor is currently running.
    */
    b8 checkBusesLayoutSupported (const BusesLayout&) const;

    //==============================================================================
    /** Возвращает true, если the Audio processor supports f64 precision floating point processing.
        The default implementation will always return false.
        If you return true here then you must override the f64 precision versions
    ...
    */
    virtual b8 supportsDoublePrecisionProcessing() const;
    ...
    */
    ...
    /** Возвращает true, если the current precision is set to doublePrecision. */
    b8 isUsingDoublePrecision() const noexcept                        { return processingPrecision == doublePrecision; }

    /** Changes the processing precision of the receiver. A client of the AudioProcessor
        calls this function to indicate which version of processBlock (single or f64
        ...
        Note that setting the processing precision to f64 floating point precision
        on a receiver which does not support f64 precision processing (i.e.
    ...
    */
    z0 setProcessingPrecision (ProcessingPrecision newPrecision) noexcept;
    ...
    */
    ...
    /** Returns the total number of input channels.
        ...
        or aux buses.
     */
    i32 getTotalNumInputChannels()  const noexcept              { return cachedTotalIns; }
    ...
    /** Returns the total number of output channels.
        ...
        or aux buses.
     */
    i32 getTotalNumOutputChannels() const noexcept              { return cachedTotalOuts; }

    /** Returns the number of input channels on the main bus. */
    inline i32 getMainBusNumInputChannels()   const noexcept    { return getChannelCountOfBus (true,  0); }

    /** Returns the number of output channels on the main bus. */
    inline i32 getMainBusNumOutputChannels()  const noexcept    { return getChannelCountOfBus (false, 0); }

    //==============================================================================
    /** Возвращает true, если the channel layout map contains a certain layout.

        You can use this method to help you implement the checkBusesLayoutSupported
        method. For example

        @code
        b8 checkBusesLayoutSupported (const BusesLayout& layouts) override
        {
            return containsLayout (layouts, {{1,1},{2,2}});
        }
        @endcode
    */
    static b8 containsLayout (const BusesLayout& layouts, const std::initializer_list<const short[2]>& channelLayoutList)
    {
        return containsLayout (layouts, layoutListToArray (channelLayoutList));
    }

    template <size_t numLayouts>
    static b8 containsLayout (const BusesLayout& layouts, const short (&channelLayoutList) [numLayouts][2])
    {
        return containsLayout (layouts, layoutListToArray (channelLayoutList));
    }
    ...
    */
    f64 getSampleRate() const noexcept                       { return currentSampleRate; }
    ...
    */
    i32 getLatencySamples() const noexcept                      { return latencySamples; }
    ...
    */
    z0 setLatencySamples (i32 newLatency);

    /** Returns the length of the processor's tail, in seconds. */
    virtual f64 getTailLengthSeconds() const = 0;

    /** Возвращает true, если the processor wants MIDI messages.
    ...
    */
    virtual b8 acceptsMidi() const = 0;

    /** Возвращает true, если the processor produces MIDI messages.

        This must return the same value every time it is called.
        This may be called by the audio thread, so this should be fast.
        Ideally, just return a constant.
    */
    virtual b8 producesMidi() const = 0;

    /** Возвращает true, если the processor supports MPE.

        This must return the same value every time it is called.
        This may be called by the audio thread, so this should be fast.
        Ideally, just return a constant.
    */
    virtual b8 supportsMPE() const                            { return false; }

    /** Возвращает true, если this is a MIDI effect plug-in and does no audio processing.

        This must return the same value every time it is called.
        This may be called by the audio thread, so this should be fast.
        Ideally, just return a constant.
    */
    virtual b8 isMidiEffect() const                           { return false; }
    ...
    /** This returns a critical section that will automatically be locked while the host
        ...
        too i64, because that could cause stuttering playback. If you need to do something
        that'll take a i64 time and need the processing to stop while it happens, use the
    ...
    */
    ...
    /** Enables and disables the processing callback.
        ...
        @see getCallbackLock
    */
    z0 suspendProcessing (b8 shouldBeSuspended);

    /** Возвращает true, если processing is currently suspended.
        @see suspendProcessing
    */
    b8 isSuspended() const noexcept                                   { return suspended; }
    ...
    */
    ...
    //==============================================================================
    /** Возвращает true, если the processor is being run in an offline mode for rendering.
        ...
        @see setNonRealtime()
    */
    b8 isNonRealtime() const noexcept                                 { return nonRealtime; }
    ...
    */
    virtual z0 setNonRealtime (b8 isNonRealtime) noexcept;
    ...
    */
    static i32 getDefaultNumParameterSteps() noexcept;
    ...
    */
    z0 updateHostDisplay (const ChangeDetails& details = ChangeDetails::getDefaultFlags());
    ...
    */
    z0 addParameter (AudioProcessorParameter*);
    ...
    /** Adds a group of parameters to the AudioProcessor.
        ...
        @see addParameter
     */
    z0 addParameterGroup (std::unique_ptr<AudioProcessorParameterGroup>);
    ...
    */
    z0 setParameterTree (AudioProcessorParameterGroup&& newTree);
    ...
    */
    virtual z0 refreshParameterList();
    ...
    */
    virtual i32 getNumPrograms() = 0;

    /** Returns the number of the currently active program. */
    virtual i32 getCurrentProgram() = 0;

    /** Called by the host to change the current program. */
    virtual z0 setCurrentProgram (i32 index) = 0;

    /** Must return the name of a given program. */
    virtual const Txt getProgramName (i32 index) = 0;

    /** Called by the host to rename a program. */
    virtual z0 changeProgramName (i32 index, const Txt& newName) = 0;
    ...
    */
    virtual z0 getStateInformation (drx::MemoryBlock& destData) = 0;
    ...
    */
    virtual z0 getCurrentProgramStateInformation (drx::MemoryBlock& destData);
    ...
    */
    virtual z0 setStateInformation (ukk data, i32 sizeInBytes) = 0;
    ...
    */
    virtual z0 setCurrentProgramStateInformation (ukk data, i32 sizeInBytes);

    /** This method is called when the total number of input or output channels is changed. */
    virtual z0 numChannelsChanged();

    /** This method is called when the number of buses is changed. */
    virtual z0 numBusesChanged();

    /** This method is called when the layout of the audio processor changes. */
    virtual z0 processorLayoutsChanged();

    //==============================================================================
    /** Adds a listener that will be called when an aspect of this processor changes. */
    virtual z0 addListener (AudioProcessorListener* newListener);

    /** Removes a previously added listener. */
    virtual z0 removeListener (AudioProcessorListener* listenerToRemove);
    ...
    */
    virtual z0 setPlayHead (AudioPlayHead* newPlayHead);
    ...
    z0 setPlayConfigDetails (i32 numIns, i32 numOuts, f64 sampleRate, i32 blockSize);
    ...
    /** This is called by the processor to specify its details before being played. You
        ...
        @see setBusesLayout
    */
    z0 setRateAndBufferSizeDetails (f64 sampleRate, i32 blockSize) noexcept;
    ...
    */
    virtual z0 audioWorkgroupContextChanged ([[maybe_unused]] const AudioWorkgroup& workgroup) {}
    ...
    */
    ...
    struct CurveData
    {
        ...
        std::function<f32 (f32)> curve;   // a function which represents your curve (such as an eq)
        Range<f32> xRange, yRange;          // the data range of your curve
    ...
    z0 editorBeingDeleted (AudioProcessorEditor*) noexcept;
    ...
    */
    ...
    static tukk getWrapperTypeDescription (AudioProcessor::WrapperType) noexcept;
    ...
    {
        std::optional<Txt> name;     // The name of the track - this will be empty if the track name is not known
        std::optional<Color> colour;   // The colour of the track - this will be empty if the colour is not known
    ...
    */
    virtual z0 updateTrackProperties (const TrackProperties& properties);
    ...
    */
    virtual std::optional<Txt> getNameForMidiNoteNumber (i32 note, i32 midiChannel);
    ...
    /** Helper function that just converts an xml element into a binary blob.
        ...
        from a binary blob.
    */
    static z0 copyXmlToBinary (const XmlElement& xml,
                                 drx::MemoryBlock& destData);
    ...
    */
    static std::unique_ptr<XmlElement> getXmlFromBinary (ukk data, i32 sizeInBytes);

    /** @internal */
    static z0 DRX_CALLTYPE setTypeOfNextNewPlugin (WrapperType);
...
protected:
    /** Callback to query if the AudioProcessor supports a specific layout.
        ...
        implementation will accept any layout. DRX does basic sanity checks so that
        the provided layouts parameter will have the same number of buses as your
        AudioProcessor.

        @see checkBusesLayoutSupported
    */
    virtual b8 isBusesLayoutSupported (const BusesLayout&) const          { return true; }
    ...
    */
    virtual b8 canApplyBusesLayout (const BusesLayout& layouts) const     { return isBusesLayoutSupported (layouts); }
    ...
    /** This method will be called when a new bus layout needs to be applied.
        ...
        implementation.
    */
    virtual b8 applyBusLayouts (const BusesLayout& layouts);
    ...
    struct BusesProperties
    {
        ...
        z0 addBus (b8 isInput, const Txt& name, const AudioChannelSet& defaultLayout, b8 isActivatedByDefault = true);

        [[nodiscard]] BusesProperties withInput  (const Txt& name, const AudioChannelSet& defaultLayout, b8 isActivatedByDefault = true) const;
        [[nodiscard]] BusesProperties withOutput (const Txt& name, const AudioChannelSet& defaultLayout, b8 isActivatedByDefault = true) const;
    };
    ...
    */
    virtual b8 canApplyBusCountChange (b8 isInput, b8 isAddingBuses,
    ...
    /** @internal */
    z0 sendParamChangeMessageToListeners (i32 parameterIndex, f32 newValue);

public:
   #ifndef DOXYGEN
    // These methods are all deprecated in favour of using AudioProcessorParameter
    // and AudioProcessorParameterGroup
    [[deprecated]] virtual i32 getNumParameters();
    [[deprecated]] virtual const Txt getParameterName (i32 parameterIndex);
    [[deprecated]] virtual Txt getParameterID (i32 index);
    [[deprecated]] virtual f32 getParameter (i32 parameterIndex);
    [[deprecated]] virtual Txt getParameterName (i32 parameterIndex, i32 maximumStringLength);
    [[deprecated]] virtual const Txt getParameterText (i32 parameterIndex);
    [[deprecated]] virtual Txt getParameterText (i32 parameterIndex, i32 maximumStringLength);
    [[deprecated]] virtual i32 getParameterNumSteps (i32 parameterIndex);
    [[deprecated]] virtual b8 isParameterDiscrete (i32 parameterIndex) const;
    [[deprecated]] virtual f32 getParameterDefaultValue (i32 parameterIndex);
    [[deprecated]] virtual Txt getParameterLabel (i32 index) const;
    [[deprecated]] virtual b8 isParameterOrientationInverted (i32 index) const;
    [[deprecated]] virtual z0 setParameter (i32 parameterIndex, f32 newValue);
    [[deprecated]] virtual b8 isParameterAutomatable (i32 parameterIndex) const;
    [[deprecated]] virtual b8 isMetaParameter (i32 parameterIndex) const;
    [[deprecated]] virtual AudioProcessorParameter::Category getParameterCategory (i32 parameterIndex) const;
    [[deprecated]] z0 beginParameterChangeGesture (i32 parameterIndex);
    [[deprecated]] z0 endParameterChangeGesture (i32 parameterIndex);
    [[deprecated]] z0 setParameterNotifyingHost (i32 parameterIndex, f32 newValue);
    ...
    [[deprecated]] i32 getNumInputChannels() const noexcept                   { return getTotalNumInputChannels(); }
    [[deprecated]] i32 getNumOutputChannels() const noexcept                  { return getTotalNumOutputChannels(); }
    [[deprecated]] const Txt getInputSpeakerArrangement() const noexcept   { return cachedInputSpeakerArrString; }
    [[deprecated]] const Txt getOutputSpeakerArrangement() const noexcept  { return cachedOutputSpeakerArrString; }
    [[deprecated]] virtual const Txt getInputChannelName  (i32 channelIndex) const;
    [[deprecated]] virtual const Txt getOutputChannelName (i32 channelIndex) const;
    [[deprecated]] virtual b8 isInputChannelStereoPair  (i32 index) const;
    [[deprecated]] virtual b8 isOutputChannelStereoPair (i32 index) const;
   #endif
...
private:
    ...
    struct InOutChannelPair
    {
        ...
        InOutChannelPair (i16 inCh, i16 outCh) noexcept    : inChannels (inCh), outChannels (outCh) {}
        InOutChannelPair (i16k (&config)[2]) noexcept   : inChannels (config[0]), outChannels (config[1]) {}

        b8 operator== (const InOutChannelPair& other) const noexcept
        {
            return other.inChannels == inChannels && other.outChannels == outChannels;
        }

        i16 inChannels = 0, outChannels = 0;
    };
    ...
    static Array<InOutChannelPair> layoutListToArray (const short (&configuration) [numLayouts][2])
    {
        ...
        for (size_t i = 0; i < numLayouts; ++i)
            layouts.add (InOutChannelPair (configuration[(i32) i]));
    ...
    template <typename This>
    static auto getBusImpl (This& t, b8 isInput, i32 busIndex) -> decltype (t.getBus (isInput, busIndex))
    {
    ...
    static b8 containsLayout (const BusesLayout&, const Array<InOutChannelPair>&);

    //==============================================================================
    z0 createBus (b8 isInput, const BusProperties&);

    //==============================================================================
    Array<AudioProcessorListener*> listeners;
    Component::SafePointer<AudioProcessorEditor> activeEditor;
    f64 currentSampleRate = 0;
    i32 blockSize = 0, latencySamples = 0;
    b8 suspended = false;
    std::atomic<b8> nonRealtime { false };
    ...
    Txt cachedInputSpeakerArrString, cachedOutputSpeakerArrString;
    i32 cachedTotalIns = 0, cachedTotalOuts = 0;

    AudioProcessorParameterGroup parameterTree;
    Array<AudioProcessorParameter*> flatParameterList;

    AudioProcessorParameter* getParamChecked (i32) const;

  #if DRX_DEBUG
   #if ! DRX_DISABLE_AUDIOPROCESSOR_BEGIN_END_GESTURE_CHECKING
    BigInteger changingParams;
   #endif

    b8 textRecursionCheck = false;
    std::unordered_set<Txt> paramIDs, groupIDs;
   #if ! DRX_DISABLE_CAUTIOUS_PARAMETER_ID_CHECKING
    std::unordered_set<Txt> trimmedParamIDs;
   #endif
  #endif

    z0 checkForDuplicateTrimmedParamID (AudioProcessorParameter*);
    z0 validateParameter (AudioProcessorParameter*);
    z0 checkForDuplicateParamID (AudioProcessorParameter*);
    z0 checkForDuplicateGroupIDs (const AudioProcessorParameterGroup&);

    AudioProcessorListener* getListenerLocked (i32) const noexcept;
    z0 updateSpeakerFormatStrings();
    z0 audioIOChanged (b8 busNumberChanged, b8 channelNumChanged);
    z0 getNextBestLayout (const BusesLayout&, BusesLayout&) const;

    template <typename floatType>
    z0 processBypassed (AudioBuffer<floatType>&, MidiBuffer&);
    ...
    virtual b8 silenceInProducesSilenceOut() const  { return false; }

    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (AudioProcessor)
};
