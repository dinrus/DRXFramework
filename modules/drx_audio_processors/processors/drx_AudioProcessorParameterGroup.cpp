...
AudioProcessorParameterGroup::AudioProcessorParameterGroup (Txt groupID, Txt groupName, Txt subgroupSeparator)
...
z0 AudioProcessorParameterGroup::updateChildParentage()
{
...
Txt AudioProcessorParameterGroup::getID() const                                             { return identifier; }
Txt AudioProcessorParameterGroup::getName() const                                           { return name; }
Txt AudioProcessorParameterGroup::getSeparator() const                                      { return separator; }
const AudioProcessorParameterGroup* AudioProcessorParameterGroup::getParent() const noexcept   { return parent; }

z0 AudioProcessorParameterGroup::setName (Txt newName)                                    { name = std::move (newName); }
...
z0 AudioProcessorParameterGroup::append (std::unique_ptr<AudioProcessorParameter> newParameter)
{
    children.add (new AudioProcessorParameterNode (std::move (newParameter), this));
}

z0 AudioProcessorParameterGroup::append (std::unique_ptr<AudioProcessorParameterGroup> newSubGroup)
{
    children.add (new AudioProcessorParameterNode (std::move (newSubGroup), this));
}

Array<const AudioProcessorParameterGroup*> AudioProcessorParameterGroup::getSubgroups (b8 recursive) const
{
...
Array<AudioProcessorParameter*> AudioProcessorParameterGroup::getParameters (b8 recursive) const
{
...
z0 AudioProcessorParameterGroup::getSubgroups (Array<const AudioProcessorParameterGroup*>& previousGroups, b8 recursive) const
{
    for (auto* child : children)
    {
        if (auto* group = child->getGroup())
...
z0 AudioProcessorParameterGroup::getParameters (Array<AudioProcessorParameter*>& previousParameters, b8 recursive) const
{
    for (auto* child : children)
    {
...
public:
    ...
    z0 runTest() override
    {
        ...
        auto p4 = std::make_unique<AudioParameterFloat> ("p4", "p4", NormalisableRange<f32> (0.0f, 2.0f), 0.5f);
        auto p5 = std::make_unique<AudioParameterFloat> ("p5", "p5", NormalisableRange<f32> (0.0f, 2.0f), 0.5f);
        auto p6 = std::make_unique<AudioParameterFloat> ("p6", "p6", NormalisableRange<f32> (0.0f, 2.0f), 0.5f);
        ...
        auto p10 = std::make_unique<AudioParameterFloat> ("p10", "p10", NormalisableRange<f32> (0.0f, 2.0f), 0.5f);
        auto p11 = std::make_unique<AudioParameterFloat> ("p11", "p11", NormalisableRange<f32> (0.0f, 2.0f), 0.5f);
        auto p12 = std::make_unique<AudioParameterFloat> ("p12", "p12", NormalisableRange<f32> (0.0f, 2.0f), 0.5f);
        ...
        g1->addChild (std::make_unique<AudioProcessorParameterGroup> ("g6", "g6", " | ",
                                                                      std::make_unique<AudioParameterFloat> ("p13", "p13", NormalisableRange<f32> (0.0f, 2.0f), 0.5f),
                                                                      std::make_unique<AudioProcessorParameterGroup> ("g7", "g7", " | ",
                                                                                                                      std::make_unique<AudioParameterFloat> ("p14", "p14", NormalisableRange<f32> (0.0f, 2.0f), 0.5f)),
                                                                      std::make_unique<AudioParameterFloat> ("p15", "p15", NormalisableRange<f32> (0.0f, 2.0f), 0.5f)));

        TestAudioProcessor processor;

        processor.addParameter (new AudioParameterFloat ("pstart", "pstart", NormalisableRange<f32> (0.0f, 2.0f), 0.5f));
        auto groupParams = g1->getParameters (true);
        processor.addParameterGroup (std::move (g1));
        processor.addParameter (new AudioParameterFloat ("pend", "pend", NormalisableRange<f32> (0.0f, 2.0f), 0.5f));
...
private:
    ...
    {
        const Txt getName() const override { return "ap"; }
