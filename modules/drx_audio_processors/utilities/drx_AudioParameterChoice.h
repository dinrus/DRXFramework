...
/** Properties of an AudioParameterChoice.
    ...
    @tags{Audio}
*/
class AudioParameterChoiceAttributes : public RangedAudioParameterAttributes<AudioParameterChoiceAttributes, i32> {};
...
/**
    ...
    @tags{Audio}
*/
class DRX_API  AudioParameterChoice  : public RangedAudioParameter
{
public:
    ...
    */
    ...
    AudioParameterChoice (const ParameterID& parameterID,
                          ...
                          std::function<Txt (i32 index, i32 maximumStringLength)> stringFromIndex = nullptr,
                          std::function<i32 (const Txt& text)> indexFromString = nullptr)
    ...
    i32 getIndex() const noexcept                   { return roundToInt (value.load()); }

    /** Returns the current index of the selected item. */
    operator i32() const noexcept                   { return getIndex(); }

    /** Returns the name of the currently selected item. */
    Txt getCurrentChoiceName() const noexcept    { return choices[getIndex()]; }

    /** Returns the name of the currently selected item. */
    operator Txt() const noexcept                { return getCurrentChoiceName(); }

    /** Changes the selected item to a new index. */
    AudioParameterChoice& operator= (i32 newValue);
...
private:
    ...
    std::function<Txt (i32, i32)> stringFromIndexFunction;
    std::function<i32 (const Txt&)> indexFromStringFunction;

    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (AudioParameterChoice)
};
