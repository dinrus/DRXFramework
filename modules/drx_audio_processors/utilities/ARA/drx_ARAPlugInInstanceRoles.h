...
public:
    ...
    */
    ...
    virtual z0 releaseResources() {}

    /** Resets the internal state variables of the renderer. */
    virtual z0 reset() {}

    /** Renders the output into the given buffer. Возвращает true, если rendering executed without error,
        false otherwise.
    ...
    */
    virtual b8 processBlock (AudioBuffer<f32>& buffer,
    ...
    /** Renders the output into the given buffer. Возвращает true, если rendering executed without error,
        ...
        Returns false if non-ARA fallback rendering is required and true otherwise.
    */
    virtual b8 processBlock (AudioBuffer<f64>& buffer,
                               AudioProcessor::Realtime realtime,
...
/** Base class for a renderer fulfilling the ARAPlaybackRenderer role as described in the ARA SDK.
    ...
    @tags{ARA}
*/
class DRX_API  ARAPlaybackRenderer  : public ARA::PlugIn::PlaybackRenderer,
                                       public ARARenderer
{
public:
    ...
    // Shadowing templated getters to default to DRX versions of the returned classes
    ...
    z0 addPlaybackRegion (ARA::ARAPlaybackRegionRef playbackRegionRef) noexcept override;
    z0 removePlaybackRegion (ARA::ARAPlaybackRegionRef playbackRegionRef) noexcept override;
    AudioProcessorARAExtension* araExtension {};
#endif

private:
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ARAPlaybackRenderer)
};
...
/** Base class for a renderer fulfilling the ARAEditorRenderer role as described in the ARA SDK.
    ...
    @tags{ARA}
*/
class DRX_API  ARAEditorRenderer  : public ARA::PlugIn::EditorRenderer,
                                     public ARARenderer
{
public:
...
private:
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ARAEditorRenderer)
};
...
/** Base class for fulfilling the ARAEditorView role as described in the ARA SDK.
    ...
    @tags{ARA}
*/
class DRX_API  ARAEditorView  : public ARA::PlugIn::EditorView
{
public:
    ...
    z0 doNotifySelection (const ARA::PlugIn::ViewSelection* currentSelection) noexcept override;

    // Base class implementation must be called if overridden
    z0 doNotifyHideRegionSequences (const std::vector<ARA::PlugIn::RegionSequence*>& regionSequences) noexcept override;
    ...
    */
    ...
    public:
        ...
        */
        virtual z0 onNewSelection (const ARAViewSelection& viewSelection);
        ...
        */
        virtual z0 onHideRegionSequences (const std::vector<ARARegionSequence*>& regionSequences);
    };
...
private:
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ARAEditorView)
};
