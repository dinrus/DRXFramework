...
/** This class contains the customisation points for the DRX provided ARA document controller
    implementation.
    ...
    {
        return drx::ARADocumentControllerSpecialisation::createARAFactory<MyDocumentController>();
    }
    @endcode
...
protected:
    //==============================================================================
    /** Read an ARADocument archive from a drx::InputStream.
    ...
    */
    virtual b8 doRestoreObjectsFromStream (ARAInputStream& input, const ARARestoreObjectsFilter* filter) = 0;

    /** Write an ARADocument archive to a drx::OutputStream.

        @param output Data stream that should be used to write the persistent ARADocument data
        @param filter A filter to be applied to the stream

        Возвращает true, если the operation is successful.

        @see ARADocumentControllerInterface::storeObjectsToArchive
    */
    virtual b8 doStoreObjectsToStream (ARAOutputStream& output, const ARAStoreObjectsFilter* filter) = 0;
    ...
    */
    virtual b8                        doIsAudioSourceContentAvailable            (const ARA::PlugIn::AudioSource* audioSource,
    ...
    */
    virtual b8                        doIsAudioModificationContentAvailable      (const ARA::PlugIn::AudioModification* audioModification,
                                                                                    ARA::ARAContentType type);
    ...
    */
    virtual b8                        doIsPlaybackRegionContentAvailable         (const ARA::PlugIn::PlaybackRegion* playbackRegion,
                                                                                    ARA::ARAContentType type);
    ...
    */
    virtual z0                        doGetPlaybackRegionHeadAndTailTime         (const ARA::PlugIn::PlaybackRegion* playbackRegion,
    ...
    */
    virtual b8                        doIsAudioSourceContentAnalysisIncomplete   (const ARA::PlugIn::AudioSource* audioSource,
                                                                                    ARA::ARAContentType type);
    ...
    */
    virtual z0                        doRequestAudioSourceContentAnalysis        (ARA::PlugIn::AudioSource*  audioSource,
    ...
    */
    virtual z0                        doRequestProcessingAlgorithmForAudioSource (ARA::PlugIn::AudioSource* audioSource, ARA::ARAInt32 algorithmIndex);
...
private:
    ...
    public:
        ...
        {
            const drx::Txt compatibleDocumentArchiveIDString = DrxPlugin_ARACompatibleArchiveIDs;

            if (compatibleDocumentArchiveIDString.isNotEmpty())
            {
                compatibleDocumentArchiveIDStrings = drx::StringArray::fromLines (compatibleDocumentArchiveIDString);
            ...
            DRX_BEGIN_IGNORE_WARNINGS_MSVC (6313)

            for (size_t i = 0; i < contentTypes.size(); ++i)
                if (DrxPlugin_ARAContentTypes & (1 << i))
            ...
            for (size_t i = 0; i < playbackTransformationFlags.size(); ++i)
                if (DrxPlugin_ARATransformationFlags & (1 << i))
                    supportedPlaybackTransformationFlags |= playbackTransformationFlags[i];

            DRX_END_IGNORE_WARNINGS_MSVC
        }

        tukk getFactoryID() const noexcept override         { return DrxPlugin_ARAFactoryID; }
        tukk getPlugInName() const noexcept override        { return DrxPlugin_Name; }
        tukk getManufacturerName() const noexcept override  { return DrxPlugin_Manufacturer; }
        tukk getInformationURL() const noexcept override    { return DrxPlugin_ManufacturerWebsite; }
        tukk getVersion() const noexcept override           { return DrxPlugin_VersionString; }
        tukk getDocumentArchiveID() const noexcept override { return DrxPlugin_ARADocumentArchiveID; }
        ...
        z0 destroyDocumentController (ARA::PlugIn::DocumentController* controller) const noexcept override
        {
            delete getSpecialisedDocumentController (controller);
        }

    private:
        drx::StringArray compatibleDocumentArchiveIDStrings;
...
public:
    ...
    z64 getPosition() override { return position; }
    z64 getTotalLength() override { return size; }

    i32 read (uk, i32) override;
    b8 setPosition (z64) override;
    b8 isExhausted() override;

    b8 failed() const { return failure; }
