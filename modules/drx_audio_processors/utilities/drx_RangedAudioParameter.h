...
/**
    ...
    i.e. the identifiers AudioParameterFloatAttributes and RangedAudioParameterAttributes<f32>
    should not be interchangable because we might need to add f32-specific attributes in
...
public:
    ...
    using StringFromValue = std::function<Txt (Value, i32)>;
    using ValueFromString = std::function<Value (const Txt&)>;
    ...
    [[nodiscard]] auto withLabel (Txt x)                                                  const { return withMember (asDerived(), &Derived::attributes, attributes.withLabel (std::move (x))); }
    ...
    [[nodiscard]] auto withMeta (b8 x)                                                     const { return withMember (asDerived(), &Derived::attributes, attributes.withMeta (std::move (x))); }

    /** See AudioProcessorParameter::isAutomatable() */
    [[nodiscard]] auto withAutomatable (b8 x)                                              const { return withMember (asDerived(), &Derived::attributes, attributes.withAutomatable (std::move (x))); }

    /** See AudioProcessorParameter::isOrientationInverted() */
    [[nodiscard]] auto withInverted (b8 x)                                                 const { return withMember (asDerived(), &Derived::attributes, attributes.withInverted (std::move (x))); }
...
/**
    ...
    @tags{Audio}
*/
class DRX_API RangedAudioParameter   : public AudioProcessorParameterWithID
{
public:
    using AudioProcessorParameterWithID::AudioProcessorParameterWithID;

    /** Returns the range of values that the parameter can take. */
    virtual const NormalisableRange<f32>& getNormalisableRange() const = 0;
    ...
    */
    ...
    f32 convertTo0to1 (f32 v) const noexcept;

    /** Denormalises and snaps a value based on the normalisable range. */
    f32 convertFrom0to1 (f32 v) const noexcept;
};
