...
template <typename RangeType, typename std::enable_if<hasGetStartFunction<RangeType>, i32>::type = 0>
...
static Txt& operator<< (Txt& s, Range<z64> r)
{
    return s += "[" + Txt { r.getStart() } + ", " + Txt { r.getEnd() } + ")";
}
...
public:
    ...
    z0 expectRange (Range<z64> actual, Range<z64> expected)
    {
        Txt failureMessage { "range " };
...
public:
    ...
    z0 runTest() override
    {
        ...
        beginTest ("Ranges::set() - neighbouring ranges extents are modified");
        {
            ...
            // size_t doesn't always map to an existing overload for Txt::operator<< on all platforms
            expectEquals ((z64) ranges.size(), (z64)  3);
        ...
        beginTest ("Ranges::set() - setting a range inside another one splits that range");
        {
            ...
            expectEquals ((z64) ranges.size(), (z64) 1);
        ...
        beginTest ("Ranges::set() - old ranges falling within the bounds of a newly set are erased");
        {
            ...
            expectEquals ((z64) ranges.size(), (z64) 5);
        ...
        beginTest ("Ranges::set() - setting a range inside another range");
        {
            ...
            expectEquals ((z64) ranges.size(), (z64) 4, "");
        ...
        beginTest ("Ranges::split()");
        {
            ...
            expectEquals ((z64) ops.size(), (z64) 1, "");
            const auto op0 = std::get_if<Ranges::Ops::Split> (&ops[0]);
            expect (op0 != nullptr);

            if (op0 != nullptr)
                expectEquals ((i32) op0->index, 0, "The 0th element should be split");
        ...
        beginTest ("Ranges::split() - splitting has no effect when no range begins before and ends after the location");
        {
            ...
            ops.clear();
            ranges.split (48, ops);
            expectEquals ((z64) ops.size(), (z64) 0, "");

            expectEquals ((z64) ranges.size(), (z64) 2, "");
            expectRange (ranges.get (0), { 0, 48 });
...
public:
    ...
    z0 expectRangedValuesItem (ItemType item, Range<z64> range, t8 value)
    {
        {
            Txt failureMessage { "range " };
    ...
    z0 runTest() override
    {
        ...
        beginTest ("RangedValues::set() - setting a range inside another range");
        {
            ...
            expectEquals ((z64) rangedValues.size(), (z64) 4, "");
        ...
        beginTest ("RangedValues::insert() fuzzing - insert always increases the total covered range");
        {
            for (auto i = 0; i != 100; ++i)
            {
                ...
                const auto beginInsertionAt = (z64) random.nextInt (100) - 50;
                const auto numElemsToInsert = (z64) random.nextInt (1000);

                rangedValuesNotMerged.insert ({ Range<z64>::withStartAndLength (beginInsertionAt, numElemsToInsert) },
                                              'a' + (t8) random.nextInt (25),
                                              ops,
                                              MergeEqualItemsNo{});

                expectEquals (getCumulativeRangeLengths (rangedValuesNotMerged) - totalLengthBeforeInsert, numElemsToInsert);

                rangedValuesMerged.insert ({ Range<z64>::withStartAndLength (beginInsertionAt, numElemsToInsert) },
                                           'a' + (t8) random.nextInt (25),
                                           ops,
                                           MergeEqualItemsYes{});
        ...
        beginTest ("RangedValues::insert() with distinct value inside another range");
        {
            ...
            expectEquals ((z64) rangedValues.size(), (z64) 3);

            rangedValues.insert ({ 2, 4 }, 'd', ops);

            expectEquals ((z64) rangedValues.size(), (z64) 5);
        ...
        {
            {
                RangedValues<t8> emptyRangedValues;
...

            {
                ...
                expect (ops.empty());
                expectEquals ((z64) rangedValues.size(), (z64) 1);
...
public:
    ...
    z0 runTest() override
    {
        ...
        {
            RangedValues<i32> rv1;
            rv1.set ({ 3, 8},   1, ops);
            rv1.set ({ 9, 16},  2, ops);
            rv1.set ({ 30, 40}, 3, ops);

            RangedValues<i32> rv2;
            rv2.set ({ 0, 4},   7, ops);
            rv2.set ({ 4, 6},   11, ops);
            rv2.set ({ 6, 25},  13, ops);
            rv2.set ({ 27, 55}, 17, ops);

            RangedValues<i32> rv3;
            rv3.set ({ -2, 10}, -1, ops);
            rv3.set ({ 15, 19}, -2, ops);
            rv3.set ({ 22, 36}, -3, ops);
            ...
            for (const auto [range, v1, v2, v3] : makeIntersectingRangedValues (&rv1, &rv2, &rv3))
            {
                if (iteration == 0)
                {
                     expectRange (range, Range<z64> { 3, 4 });
                     expect (v1 == 1 && v2 == 7 && v3 == -1);
                }

                if (iteration == 1)
                {
                     expectRange (range, Range<z64> { 4, 6 });
                     expect (v1 == 1 && v2 == 11 && v3 == -1);
                }

                if (iteration == 2)
                {
                     expectRange (range, Range<z64> { 6, 8 });
                     expect (v1 == 1 && v2 == 13 && v3 == -1);
                }

                if (iteration == 3)
                {
                     expectRange (range, Range<z64> { 9, 10 });
                     expect (v1 == 2 && v2 == 13 && v3 == -1);
                }

                if (iteration == 4)
                {
                     expectRange (range, Range<z64> { 15, 16 });
                     expect (v1 == 2 && v2 == 13 && v3 == -2);
                }

                if (iteration == 5)
                {
                     expectRange (range, Range<z64> { 30, 36 });
