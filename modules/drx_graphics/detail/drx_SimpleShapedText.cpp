...
SimpleShapedText::SimpleShapedText (const Txt* data,
...
struct Utf8Lookup
{
    ...
    auto getByteIndex (z64 codepointIndex) const
    {
        jassert (codepointIndex <= (z64) indices.size());

        if (codepointIndex == (z64) indices.size())
            return beyondEnd;
    ...
    auto getCodepointIndex (size_t byteIndex) const
    {
        ...
        return (z64) std::distance (indices.begin(), it);
    }

    auto getByteRange (Range<z64> range)
    {
...
static auto findControlCharacters (Span<t32> text)
{
    constexpr t32 lf = 0x0a;
    constexpr t32 cr = 0x0d;
    constexpr t32 tab = 0x09;
...
static std::vector<ShapedGlyph> lowLevelShape (const Txt& string,
                                               Range<z64> range,
                                               const Font& font,
                                               TextScript script,
                                               const Txt& language,
                                               u8 embeddingLevel)
{
    ...
    const auto preContextByteRange = utf8Lookup.getByteRange (Range<z64> { 0, range.getStart() });

    hb_buffer_add_utf8 (buffer.get(),
                        string.toRawUTF8() + preContextByteRange.getStart(),
                        (i32) preContextByteRange.getLength(),
                        0,
                        0);
    ...
    for (const auto pair : enumerate (utf32Span, size_t{}))
    {
        const auto charToAdd = [&]
        {
            ...
            constexpr t32 wordJoiner       = 0x2060;
            constexpr t32 nonBreakingSpace = 0x00a0;
        ...
        hb_buffer_add (buffer.get(), static_cast<hb_codepoint_t> (charToAdd), (u32) pair.index);
    }

    const auto postContextByteRange = utf8Lookup.getByteRange (Range<z64> { range.getEnd(), (z64) string.length() });

    hb_buffer_add_utf8 (buffer.get(),
                        string.toRawUTF8() + postContextByteRange.getStart(),
                        (i32) postContextByteRange.getLength(),
                        0,
                        0);
    ...
    hb_shape (nativeFont.get(), buffer.get(), features.data(), (u32) features.size());
    ...
    // This normally shouldn't happen if font fallback is enabled, unless the Txt contains
    // control characters that DRX doesn't know how to handle appropriately.
    ...
    std::optional<z64> lastCluster;

    const auto ltr = (embeddingLevel % 2) == 0;

    const auto getNextCluster = [&ltr, &infosCapt = infos, &range] (size_t visualIndex)
    {
        const auto next = (z64) visualIndex + (ltr ? 1 : -1);

        if (next < 0)
            return ltr ? range.getStart() : range.getEnd();

        if (next >= (z64) infosCapt.size())
            return ltr ? range.getEnd() : range.getStart();

        return (z64) infosCapt[(size_t) next].cluster + range.getStart();
    };

    for (size_t visualIndex = 0; visualIndex < infos.size(); ++visualIndex)
    {
        const auto glyphId = infos[visualIndex].codepoint;
        const auto xAdvanceBase = HbScale::hbToDrx (positions[visualIndex].x_advance);
        const auto yAdvanceBase = -HbScale::hbToDrx (positions[visualIndex].y_advance);
        ...
        const auto whitespace = extentsDataAvailable
                                && font.getTypefacePtr()->getGlyphBounds (font.getMetricsKind(), (i32) glyphId).isEmpty()
                                && xAdvanceBase > 0;
        ...
        const auto cluster = (z64) infos[visualIndex].cluster + range.getStart();

        const auto numLigaturePlaceholders = std::max ((z64) 0,
        ...
        const auto advanceMultiplier = numLigaturePlaceholders == 0 ? 1.0f
                                                                    : 1.0f / (f32) (numLigaturePlaceholders + 1);

        Point<f32> advance { xAdvanceBase * advanceMultiplier + appliedTracking, yAdvanceBase * advanceMultiplier };
        ...
        glyphs.push_back ({
            ...
            whitespace,
            newline,
            numLigaturePlaceholders == 0 ? (i8) 0 : (i8) -numLigaturePlaceholders ,
            advance,
            Point<f32> { HbScale::hbToDrx (positions[visualIndex].x_offset),
                           -HbScale::hbToDrx (positions[visualIndex].y_offset) },
        });

        for (i32 l = 0; l < numLigaturePlaceholders; ++l)
        {
...
struct SubSpanLookup
{
    ...
    auto getRange (Span<T> span) const
    {
        ...
        return Range<z64>::withStartAndLength ((z64) std::distance (enclosing.begin(), span.begin()),
                                                 (z64) span.size());
    }

    auto getSpan (Range<z64> r) const
    {
        jassert (r.getStart() + r.getLength() <= (z64) enclosing.size());
...
struct IntegralCanBreakBeforeIterator
{
    ...
    z0 reset()
    {
        reset ({ std::numeric_limits<z64>::min(),  std::numeric_limits<z64>::max() });
    }

    z0 reset (Range<z64> r)
    {
    ...
    std::optional<z64> next()
    {
        const auto intValue = [&] (auto p) { return (z64) std::distance (span.begin(), p); };
        ...
        if (std::exchange (rangeEndReturned, true) == false)
            return std::min ((z64) span.size(), restrictedTo.getEnd());
...
private:
    ...
    Range<z64> restrictedTo { std::numeric_limits<z64>::min(),  std::numeric_limits<z64>::max() };

    b8 rangeEndReturned = false;
};
...
// Used to avoid signedness warning for types for which std::size() is i32
template <typename T>
                                                            ...
                                                            const Txt& language = {})
{
    ...
    fonts.set ({ 0, (z64) text.length() }, font, ops);
    ops.clear();
    ...
    {
        std::vector<std::pair<Range<z64>, Font>> changes;

        for (const auto [r, f] : fonts)
        {
            if (! f.has_value())
            {
                changes.emplace_back (r, font.findSuitableFontForText (text.substring ((i32) r.getStart(),
                                                                                       (i32) r.getEnd()),
                                                                       language));
            }
        }
...
static RangedValues<Font> resolveFontsWithFallback (const Txt& string, const RangedValues<Font>& fonts)
{
    ...
    for (const auto [r, f] : fonts)
    {
        auto rf = findSuitableFontsForText (f, string.substring ((i32) r.getStart(),
                                                                 (i32) std::min (r.getEnd(), (z64) string.length())));
...
public:
    ...
    b8 operator== (const OwnedGlyphsSpan& other) const
    {
    ...
    z0 setTextRange (Range<z64> newRange)
    {
        textRange = newRange;
    }
...
struct WrappedGlyphsCursor
{
    ...
    b8 operator== (const WrappedGlyphsCursor& other) const
    {
...
private:
    struct Index
    {
        ...
        b8 operator== (const Index& other) const { return asTuple() == other.asTuple(); }
        b8 operator<  (const Index& other) const { return asTuple() <  other.asTuple(); }
    };
...
template <typename T>
static auto rangedValuesWithOffset (detail::RangedValues<T> rv, z64 offset = 0)
{
    detail::Ranges::Operations ops;
    rv.shift (std::numeric_limits<z64>::min(), -offset, ops);
    ops.clear();
    rv.eraseUpTo (0, ops);
    return rv;
}
...
struct Shaper
{
    Shaper (const Txt& stringIn, Range<z64> lineRange, const ShapedTextOptions& options)
        : string { stringIn.substring ((i32) lineRange.getStart(), (i32) lineRange.getEnd()) }
    {
        const auto analysis = Unicode::performAnalysis (string);

        const auto string32 = std::invoke ([this]
                                           {
                                               std::vector<t32> s32 (CharPointer_UTF32::getBytesRequiredFor (string.getCharPointer()));
                                               string.copyToUTF32 (s32.data(), s32.size() * sizeof (t32));
                                               jassert (! s32.empty());
        ...
        for (Unicode::ScriptRunIterator scriptIter { makeSpan (analysis) }; auto scriptRun = scriptIter.next();)
        {
            ...
            for (auto it = begin; it != end;)
            {
                ...
                const auto bidiStart = (z64) std::distance (bidiLevels.data(), it);
                const auto bidiLength = (z64) std::distance (it, next);
                const auto bidiRange = Range<z64>::withStartAndLength (bidiStart, bidiLength);
    ...
    WrappedGlyphs getChunksUpToNextSafeBreak (z64 startFrom)
    {
        ...
        if (! shapedGlyphs.getRanges().covers ({ startFrom, nextSoftBreakBefore }))
        {
            for (auto it = shaperRuns.find (startFrom);
                 ...
                 ++it)
            {
                const Range<z64> shapingRange { std::max (startFrom, it->range.getStart()), it->range.getEnd() };
    ...
    std::vector<z64> softBreakBeforePoints;
...
struct LineState
{
    ...
    LineState (f32 w, b8 f)
        : maxWidth { w },
          trailingWhitespaceCanExtendBeyondMargin { f }
    {}

    b8 isInTrailingPosition (const ShapedGlyph& glyph) const
    {
    ...
    z64 largestVisualOrderInLine = -1;
    f32 maxWidth{};
    f32 width{};
    b8 trailingWhitespaceCanExtendBeyondMargin;
};
...
public:
    ...
    LineOfWrappedGlyphCursorRanges (f32 maxWidth, b8 trailingWhitespaceCanExtendBeyondMargin)
    ...
    /*  Consumes as many glyphs from the provided cursor as the line will still fit. Returns the end
        ...
        one word.
    */
    WrappedGlyphsCursor consume (const WrappedGlyphsCursor& glyphIt, b8 forceConsumeFirstWord)
    {
...
private:
    static std::pair<LineState, WrappedGlyphsCursor> consumeIf (const LineState& state,
                                                                ...
                                                                std::function<b8 (const LineState&, const ShapedGlyph&)> predicate)
    {
        auto newState = state;
        auto newIt = it;
...
{
    FillLinesOptions withWidth (f32 x) const
    {
        return withMember (*this, &FillLinesOptions::width, x);
    }

    FillLinesOptions withFirstLinePadding (f32 x) const
    {
        return withMember (*this, &FillLinesOptions::firstLinePadding, x);
    }

    FillLinesOptions withTrailingWhitespaceCanExtendBeyondMargin (b8 x = true) const
    {
        return withMember (*this, &FillLinesOptions::trailingWhitespaceCanExtendBeyondMargin, x);
    }

    FillLinesOptions withForceConsumeFirstWord (b8 x = true) const
    {
    ...
    f32 width{};
    f32 firstLinePadding{};
    b8 trailingWhitespaceCanExtendBeyondMargin = false;
    b8 forceConsumeFirstWord = false;
};
...
static auto getLineRanges (const Txt& data)
{
    std::vector<Range<z64>> lineRanges;
...
static z0 foldLinesBeyondLineLimit (std::vector<std::vector<WrappedGlyphsCursorRange>>& lines,
                                      size_t maxNumLines)
{
...
z0 SimpleShapedText::shape (const Txt& data,
                              const ShapedTextOptions& options)
{
    detail::Ranges::Operations ops;
    ...
    for (const auto& lineRange : getLineRanges (data))
    {
        ...
        auto lineDataAndStorage = FillLinesOptions{}.withWidth (options.getMaxWidth().value_or ((f32) 1e6))
        ...
        for (const auto& line : lineData)
        {
            ...
            const auto lineStart = (z64) glyphsInVisualOrder.size();

            for (const auto& s : glyphSpansInLine)
            {
                const auto start = (z64) glyphsInVisualOrder.size();
                b8 ltr = true;
                ...
                const auto end = (z64) glyphsInVisualOrder.size();
            ...
            const auto lineTextRange = std::accumulate (glyphSpansInLine.begin(),
                                                        ...
                                                        std::make_pair (std::numeric_limits<z64>::max(),
                                                                        std::numeric_limits<z64>::min()),
                                                        [&] (auto sum, auto& elem) -> std::pair<z64, z64>
                                                        {
            ...
            const auto lineEnd = (z64) glyphsInVisualOrder.size();
            lineNumbersForGlyphRanges.set ({ lineStart, lineEnd}, (z64) lineNumbersForGlyphRanges.size(), ops);
            ops.clear();
        }
    }
}

Span<const ShapedGlyph> SimpleShapedText::getGlyphs (Range<z64> glyphRange) const
{
    const auto r = glyphRange.getIntersectionWith ({ 0, (z64) glyphsInVisualOrder.size() });
...
t32 SimpleShapedText::getCodepoint (z64 glyphIndex) const
{
    return string[(i32) glyphsInVisualOrder[(size_t) glyphIndex].cluster];
}

Range<z64> SimpleShapedText::getTextRange (z64 glyphIndex) const
{
    ...
    return Range<z64>::withStartAndLength (cluster, std::max ((z64) 1, nextAdjacentCluster - cluster));
}

b8 SimpleShapedText::isLtr (z64 glyphIndex) const
{
...
z0 SimpleShapedText::getGlyphRanges (Range<z64> textRange, std::vector<Range<z64>>& outRanges) const
{
    outRanges.clear();
    ...
    for (const auto is : glyphLookup.getIntersectionsWith (textRange))
    {
        ...
        const auto getGlyphSubRange = [&] (auto begin, auto end)
        {
            ...
            return Range<z64> { (z64) std::distance (begin, startIt), std::distance (begin, endIt) };
        };
...
z64 SimpleShapedText::getTextIndexAfterGlyph (z64 glyphIndex) const
{
...
struct SimpleShapedTextTests : public UnitTest
{
    ...
    static constexpr tukk testStrings[]
    {
        "Some trivial text",
    ...
    z0 runTest (tukk text, f32 maxWidth)
    {
        ...
        for (z64 glyphIndex = 0; glyphIndex < st.getNumGlyphs(); ++glyphIndex)
        {
        ...
        expect (success, Txt { "Failed for test string: " } + testString.replace ("\r", "<CR>")
                                                                           .replace ("\n", "<LF>"));
    }
