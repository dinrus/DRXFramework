...
//==============================================================================
Color Colors::findColorForName (const Txt& colourName,
                                   Color defaultColor)
{
    struct StringHashAndColor { u32 stringHash, colour; };

    static const StringHashAndColor presets[]
    {
    ...
    const auto hash = (u32) colourName.trim().toLowerCase().hashCode();

    for (auto entry : presets)
        if (entry.stringHash == hash)
            return Color (entry.colour);
