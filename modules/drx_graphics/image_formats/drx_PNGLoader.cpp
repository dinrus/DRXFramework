...
DRX_BEGIN_IGNORE_WARNINGS_MSVC (4390 4611 4365 4267 4616 2544 2545 6297)
...
namespace pnglibNamespace
{
...
#if DRX_INCLUDE_PNGLIB_CODE || ! defined (DRX_INCLUDE_PNGLIB_CODE)

   DRX_BEGIN_IGNORE_WARNINGS_GCC_LIKE ("-Wcomma",
                                        "-Wfloat-equal",
  ...
  z0 png_do_expand_palette (png_row_infop, png_bytep, png_const_colorp, png_const_bytep, i32);
  z0 png_do_expand (png_row_infop, png_bytep, png_const_color_16p);
  z0 png_do_chop (png_row_infop, png_bytep);
  z0 png_do_quantize (png_row_infop, png_bytep, png_const_bytep, png_const_bytep);
  z0 png_do_gray_to_rgb (png_row_infop, png_bytep);
  z0 png_do_unshift (png_row_infop, png_bytep, png_const_color_8p);
  z0 png_do_unpack (png_row_infop, png_bytep);
  i32 png_do_rgb_to_gray (png_structrp, png_row_infop, png_bytep);
  z0 png_do_compose (png_row_infop, png_bytep, png_structrp);
  z0 png_do_gamma (png_row_infop, png_bytep, png_structrp);
  z0 png_do_encode_alpha (png_row_infop, png_bytep, png_structrp);
  z0 png_do_scale_16_to_8 (png_row_infop, png_bytep);
  z0 png_do_expand_16 (png_row_infop, png_bytep);
  z0 png_do_read_filler (png_row_infop, png_bytep, png_uint_32, png_uint_32);
  z0 png_do_read_invert_alpha (png_row_infop, png_bytep);
  z0 png_do_read_swap_alpha (png_row_infop, png_bytep);
...
namespace PNGHelpers
{
    ...
    static z0 DRX_CDECL writeDataCallback (png_structp png, png_bytep data, png_size_t length)
    {
        static_cast<OutputStream*> (png_get_io_ptr (png))->write (data, length);
    }

   #if ! DRX_USING_COREIMAGE_LOADER
    static z0 DRX_CDECL readCallback (png_structp png, png_bytep data, png_size_t length)
    {
        static_cast<InputStream*> (png_get_io_ptr (png))->read (data, (i32) length);
    }

    struct PNGErrorStruct {};

    static z0 DRX_CDECL errorCallback (png_structp p, png_const_charp)
    {
    ...
    static z0 DRX_CDECL warningCallback (png_structp, png_const_charp) {}

    DRX_BEGIN_IGNORE_WARNINGS_MSVC (4611)

    static b8 readHeader (InputStream& in, png_structp pngReadStruct, png_infop pngInfoStruct, jmp_buf& errorJumpBuf,
                            png_uint_32& width, png_uint_32& height, i32& bitDepth, i32& colorType, i32& interlaceType) noexcept
    {
    ...
    static b8 readImageData (png_structp pngReadStruct, png_infop pngInfoStruct, jmp_buf& errorJumpBuf, png_bytepp rows) noexcept
    {
        if (setjmp (errorJumpBuf) == 0)
        {
    ...
    static Image createImageFromData (b8 hasAlphaChan, i32 width, i32 height, png_bytepp rows)
    {
        // now convert the data to a drx image format..
    ...
    static Image readImage (InputStream& in, png_structp pngReadStruct, png_infop pngInfoStruct)
    {
        ...
        i32 bitDepth = 0, colorType = 0, interlaceType = 0;
                        ...
                        width, height, bitDepth, colorType, interlaceType))
        {
            ...
            HeapBlock<u8> tempBuffer (height * lineStride);
...
Txt PNGImageFormat::getFormatName()                   { return "PNG"; }
b8 PNGImageFormat::usesFileExtension (const File& f)   { return f.hasFileExtension ("png"); }

b8 PNGImageFormat::canUnderstand (InputStream& in)
{
    i32k bytesNeeded = 4;
    t8 header [bytesNeeded];
...
#if DRX_USING_COREIMAGE_LOADER
 Image drx_loadWithCoreImage (InputStream&);
#endif
...
b8 PNGImageFormat::writeImageToStream (const Image& image, OutputStream& out)
{
    using namespace pnglibNamespace;
    ...
    {
        u8* dst = rowData;
        u8k* src = srcData.getLinePointer (y);
