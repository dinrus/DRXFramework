...
typedef struct {
  ...
  i32 * Cr_r_tab;		/* => table for Cr to R conversion */
  i32 * Cb_b_tab;		/* => table for Cb to B conversion */
...
build_ycc_rgb_table (j_decompress_ptr cinfo)
{
  ...
  cconvert->Cr_r_tab = (i32 *)
    (*cinfo->mem->alloc_small) ((j_common_ptr) cinfo, JPOOL_IMAGE,
				(MAXJSAMPLE+1) * SIZEOF(i32));
  cconvert->Cb_b_tab = (i32 *)
    (*cinfo->mem->alloc_small) ((j_common_ptr) cinfo, JPOOL_IMAGE,
				(MAXJSAMPLE+1) * SIZEOF(i32));
  ...
  for (i = 0, x = -CENTERJSAMPLE; i <= MAXJSAMPLE; i++, x++) {
    ...
    /* Cr=>R value is nearest i32 to 1.40200 * x */
    cconvert->Cr_r_tab[i] = (i32)
		    RIGHT_SHIFT(FIX(1.40200) * x + ONE_HALF, SCALEBITS);
    /* Cb=>B value is nearest i32 to 1.77200 * x */
    cconvert->Cb_b_tab[i] = (i32)
		 ...
		 JSAMPARRAY output_buf, i32 num_rows)
{
  ...
  i32 * Crrtab = cconvert->Cr_r_tab;
  i32 * Cbbtab = cconvert->Cb_b_tab;
  ...
  while (--num_rows >= 0) {
    ...
    for (col = 0; col < num_cols; col++) {
      ...
      outptr[RGB_GREEN] = range_limit[y +
			      ((i32) RIGHT_SHIFT(Cbgtab[cb] + Crgtab[cr],
						 SCALEBITS))];
	      ...
	      JSAMPARRAY output_buf, i32 num_rows)
{
  ...
  JDIMENSION count;
  i32 num_components = cinfo->num_components;
  JDIMENSION num_cols = cinfo->output_width;
  i32 ci;
...
grayscale_convert2 (j_decompress_ptr cinfo,
		   ...
		   JSAMPARRAY output_buf, i32 num_rows)
{
  jcopy_sample_rows(input_buf[0], (i32) input_row, output_buf, 0,
