...
typedef struct {
  ...
  i32 whichptr;			/* indicates which pointer set is now in use */
  i32 context_state;		/* process_data state machine status */
...
METHODDEF(z0) process_data_simple_main2
	JPP((j_decompress_ptr cinfo, JSAMPARRAY output_buf,
	     JDIMENSION *out_row_ctr, JDIMENSION out_rows_avail));
METHODDEF(z0) process_data_context_main
	JPP((j_decompress_ptr cinfo, JSAMPARRAY output_buf,
	     JDIMENSION *out_row_ctr, JDIMENSION out_rows_avail));
#ifdef QUANT_2PASS_SUPPORTED
METHODDEF(z0) process_data_crank_post
	JPP((j_decompress_ptr cinfo, JSAMPARRAY output_buf,
	     JDIMENSION *out_row_ctr, JDIMENSION out_rows_avail));
#endif
 ...
 */
{
  ...
  i32 ci, rgroup;
  i32 M = cinfo->min_DCT_scaled_size;
...
{
  my_main_ptr4 main_ = (my_main_ptr4) cinfo->main;
  i32 ci, i, rgroup, iMCUheight, rows_left;
  jpeg_component_info *compptr;
  JSAMPARRAY xbuf;
  ...
  for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
    ...
    rows_left = (i32) (compptr->downsampled_height % (JDIMENSION) iMCUheight);
