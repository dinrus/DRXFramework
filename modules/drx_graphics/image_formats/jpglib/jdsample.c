...
typedef JMETHOD(z0, upsample1_ptr,
...
typedef struct {
  ...
  i32 next_row_out;		/* counts rows emitted from color_buf */
  JDIMENSION rows_to_go;	/* counts rows remaining in image */

  /* Height of an input row group for each component. */
  i32 rowgroup_height[MAX_COMPONENTS];
...
jinit_upsampler (j_decompress_ptr cinfo)
{
  ...
  i32 h_in_group, v_in_group, h_out_group, v_out_group;
  ...
  for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
	...
	((j_common_ptr) cinfo, JPOOL_IMAGE,
	 (JDIMENSION) jround_up((i64) cinfo->output_width,
