...
public:
    ...
    */
    ...
    Direct2DPixelData (Image::PixelFormat, i32, i32, b8);
    ...
    */
    ...
    /*  Provides access to the software image storage.
        ...
        accessed.
    */
    z0 initialiseBitmapData (Image::BitmapData&, i32, i32, Image::BitmapData::ReadWriteMode) override;

    z0 applyGaussianBlurEffectInArea (Rectangle<i32>, f32) override;
    z0 applySingleChannelBoxBlurEffectInArea (Rectangle<i32>, i32) override;
    z0 multiplyAllAlphasInArea (Rectangle<i32>, f32) override;
    z0 desaturateInArea (Rectangle<i32>) override;
...
private:
    ...
    /*  Attempts to copy the content of the corresponding texture in graphics storage into
        ...
        be recreated.
    */
    b8 createPersistentBackup (ComSmartPtr<ID2D1Device1> deviceHint);

    struct Context;
    std::unique_ptr<Context> createNativeContext();

    template <typename Fn>
    b8 applyEffectInArea (Rectangle<i32>, Fn&&);

    z0 setBackupEnabled (b8) override;
    b8 isBackupEnabled() const override;
    b8 backupNow() override;
    b8 needsBackup() const override;
    b8 canBackup() const override;

    z0 adapterCreated (DxgiAdapter::Ptr) override {}
    z0 adapterRemoved (DxgiAdapter::Ptr adapter) override
    {
        if (adapter != nullptr)
    ...
    DRX_LEAK_DETECTOR (Direct2DPixelData)
};
