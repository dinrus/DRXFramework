                                                     ...
                                                     b8 needsClear)
{
    ...
    for (size_t h = 0; h < height; h += maxDim)
    {
        ...
        for (size_t w = 0; w < width; w += maxDim)
        {
            ...
            result.push_back (Direct2DPixelDataPage { bitmap, { (i32) w, (i32) h } });
        }
    }
...
/*  Maps the content of the provided bitmap and copies it into target, which should be a software
    bitmap.
*/
static b8 readFromDirect2DBitmap (ComSmartPtr<ID2D1DeviceContext1> context,
                                    ...
                                    ImagePixelData::Ptr target)
{
    ...
    if (std::tuple (target->width, target->height) != std::tuple ((i32) size.width, (i32) size.height))
    {
    ...
    {
        TexturePixelData (ComSmartPtr<ID2D1Bitmap1> bitmapIn, Image::PixelFormat format, i32 w, i32 h)
        ...
        z0 applyGaussianBlurEffectInArea (Rectangle<i32>, f32) override { jassertfalse; }
        // Unimplemented, should never be called
        z0 applySingleChannelBoxBlurEffectInArea (Rectangle<i32>, i32) override { jassertfalse; }
        // Unimplemented, should never be called
        z0 multiplyAllAlphasInArea (Rectangle<i32>, f32) override { jassertfalse; }
        // Unimplemented, should never be called
        z0 desaturateInArea (Rectangle<i32>) override { jassertfalse; }

        z0 initialiseBitmapData (Image::BitmapData& bd, i32 x, i32 y, Image::BitmapData::ReadWriteMode mode) override
        {
            ...
            bd.lineStride = (i32) mapped.pitch;
            bd.data = mapped.bits + x * bd.pixelStride + y * (i32) mapped.pitch;
...
Direct2DPixelData::Direct2DPixelData (Image::PixelFormat formatToUse, i32 w, i32 h, b8 clearIn)
...
b8 Direct2DPixelData::createPersistentBackup (ComSmartPtr<ID2D1Device1> deviceHint)
{
...
auto Direct2DPixelData::createNativeContext() -> std::unique_ptr<Context>
{
    ...
    const auto maxSize = (i32) context->GetMaximumBitmapSize();
...
std::unique_ptr<LowLevelGraphicsContext> Direct2DPixelData::createLowLevelContext()
{
    if (state == State::drawing)
    {
        ...
        struct InertContext : public LowLevelGraphicsContext
        {
            ...
            b8 clipToRectangle (const Rectangle<i32>&) override { return false; }
            b8 clipToRectangleList (const RectangleList<i32>&) override { return false; }
            ...
            b8 clipRegionIntersects (const Rectangle<i32>&) override { return false; }
            Rectangle<i32> getClipBounds() const override { return {}; }
            b8 isClipEmpty() const override { return true; }
            z0 saveState() override {}
            z0 restoreState() override {}
            z0 beginTransparencyLayer (f32) override {}
            ...
            z0 fillRect (const Rectangle<i32>&, b8) override {}
            ...
            z0 setFont (const Font& f) override { font = f; }
            const Font& getFont() override { return font; }
            z0 drawGlyphs (Span<u16k>, Span<const Point<f32>>, const AffineTransform&) override {}
...
z0 Direct2DPixelData::initialiseBitmapData (Image::BitmapData& bitmap,
                                              i32 x,
                                              i32 y,
...
b8 Direct2DPixelData::applyEffectInArea (Rectangle<i32> area, Fn&& configureEffect)
{
    ...
    const auto destPoint = D2D1::Point2F ((f32) area.getX(), (f32) area.getY());
...
z0 Direct2DPixelData::applyGaussianBlurEffectInArea (Rectangle<i32> b, f32 radius)
{
...
z0 Direct2DPixelData::applySingleChannelBoxBlurEffectInArea (Rectangle<i32> b, i32 radius)
{
    applyEffectInArea (b, [&] (auto dc, auto input) -> ComSmartPtr<ID2D1Effect>
    {
...
z0 Direct2DPixelData::multiplyAllAlphasInArea (Rectangle<i32> b, f32 value)
{
    applyEffectInArea (b, [&] (auto dc, auto input) -> ComSmartPtr<ID2D1Effect>
    {
        ComSmartPtr<ID2D1Effect> effect;
...
z0 Direct2DPixelData::desaturateInArea (Rectangle<i32> b)
{
    applyEffectInArea (b, [&] (auto dc, auto input) -> ComSmartPtr<ID2D1Effect>
    {
        ComSmartPtr<ID2D1Effect> effect;
...
z0 Direct2DPixelData::setBackupEnabled (b8 x)
{
    sync = x;
}

b8 Direct2DPixelData::isBackupEnabled() const
{
    return sync;
}

b8 Direct2DPixelData::backupNow()
{
    return createPersistentBackup (nullptr);
}

b8 Direct2DPixelData::needsBackup() const
{
    return state == State::outdated;
}

b8 Direct2DPixelData::canBackup() const
{
...
public:
    ...
    {
        compareFunctions[{ Image::RGB, Image::RGB }] = [] (u8* rgb1, u8* rgb2)
        {
            return rgb1[0] == rgb2[0] && rgb1[1] == rgb2[1] && rgb1[2] == rgb2[2];
        };

        compareFunctions[{ Image::RGB, Image::ARGB }] = [] (u8* rgb, u8* argb)
        {
        ...
        compareFunctions[{ Image::RGB, Image::SingleChannel }] = [] (u8*, u8* singleChannel)
        {
            return *singleChannel == 0xff;
        };

        compareFunctions[{ Image::ARGB, Image::RGB }] = [] (u8* argb, u8* rgb)
        {
            // Compare bytes directly to avoid alpha premultiply issues
            return argb[0] == rgb[0] && argb[1] == rgb[1] && argb[2] == rgb[2];
        };

        compareFunctions[{ Image::ARGB, Image::ARGB }] = [] (u8* argb1, u8* argb2)
        {
            return *reinterpret_cast<u32*> (argb1) == *reinterpret_cast<u32*> (argb2);
        };

        compareFunctions[{ Image::ARGB, Image::SingleChannel }] = [] (u8* argb, u8* singleChannel)
        {
            return argb[3] == *singleChannel;
        };

        compareFunctions[{ Image::SingleChannel, Image::RGB }] = [] (u8* singleChannel, u8* rgb)
        {
            auto alpha = *singleChannel;
            return rgb[0] == alpha && rgb[1] == alpha && rgb[2] == alpha;
        };

        compareFunctions[{ Image::SingleChannel, Image::ARGB }] = [] (u8* singleChannel, u8* argb)
        {
            return *singleChannel == argb[3];
        };

        compareFunctions[{ Image::SingleChannel, Image::SingleChannel }] = [] (u8* singleChannel1, u8* singleChannel2)
        {
            ...
            for (auto sourceFormat : formats)
            {
...

                {
                    ...
                    for (i32 y = 0; y < bitmap.height; y++)
                    {
                        ...
                        for (i32 x = 0; x < bitmap.lineStride; x++)
                            line[x] = (u8) jmap (x, 0, bitmap.lineStride, 0, 256);
                    }
                }
                ...
                for (auto destFormat : formats)
                {
                    ...
                    for (i32 y = 0; y < data1.height; y++)
                    {
                        for (i32 x = 0; x < data1.width; x++)
                        {
    ...
    Rectangle<i32> randomRectangleWithin (Rectangle<i32> container) noexcept
    {
    ...
    z0 compareSameFormat (Image::PixelFormat format, i32 width, i32 height)
    {
    ...
    z0 compareImages (Image& image1, Image& image2, std::function<b8 (u8*, u8*)> compareBytes)
    {
        {
            // BitmapData width & height should match
            Rectangle<i32> area = randomRectangleWithin (image1.getBounds());
...

        {
            ...
            for (i32 y = 0; y < data1.height; ++y)
            {
                ...
                for (i32 x = 0; x < data1.width; ++x)
                {
                    expect (compareBytes (line1, line2), "Failed comparing format " + Txt { image1.getFormat() } + " to " + Txt { image2.getFormat() });
...

        {
            ...
            Rectangle<i32> area1 = randomRectangleWithin (image1.getBounds());
            Rectangle<i32> area2 = randomRectangleWithin (image1.getBounds());
            Image::BitmapData data1 { image1, Image::BitmapData::ReadWriteMode::readOnly };
            ...
            auto compareSubsection = [&] (Image::BitmapData& subsection1, Image::BitmapData& subsection2, Rectangle<i32> area)
            {
                for (i32 y = 0; y < area.getHeight(); ++y)
                {
                    auto line1 = subsection1.getLinePointer (y + area.getY());
                    auto line2 = subsection2.getLinePointer (y);

                    for (i32 x = 0; x < area.getWidth(); ++x)
                    {
    ...
    z0 checkReadWriteModes (Image& image)
    {
        // Check read and write modes
        i32 x = random.nextInt (image.getWidth());
        ...
        switch (image.getFormat())
        {
            case Image::SingleChannel:
            {
                ...
                expectedColor = Color { alpha, alpha, alpha, alpha };
                break;
            }
...

        {
            ...
            for (i32 y = 0; y < data.height; ++y)
                data.setPixelColor (x, y, writeColor);
        }

        {
            ...
            for (i32 y = 0; y < data.height; ++y)
            {
                auto color = data.getPixelColor (x, y);
                expect (color == expectedColor);
            }
        }
    }

    z0 testFormatConversion (i32 width, i32 height)
    {
        for (auto sourceFormat : formats)
        {
            for (auto destFormat : formats)
    ...
    {
        u8 red   = (u8) random.nextInt (255);
        u8 green = (u8) random.nextInt (255);
        u8 blue  = (u8) random.nextInt (255);
        u8 alpha = (u8) random.nextInt (255);
    ...
    std::map<std::pair<Image::PixelFormat, Image::PixelFormat>, std::function<b8 (u8*, u8*)>> compareFunctions;
};
