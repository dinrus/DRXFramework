...
private:
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ScopedMultithread)
...
struct ScopedGeometryWithSink
{
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ScopedGeometryWithSink)
};
...
struct D2DHelpers
{
    static b8 isTransformAxisAligned (const AffineTransform& transform)
    {
        return transform.mat01 == 0.0f && transform.mat10 == 0.0f;
    }

    static z0 pathToGeometrySink (const Path& path,
                                    ...
                                    D2D1_FIGURE_BEGIN figureMode)
    {
        ...
        private:
            ...
            DRX_DECLARE_NON_COPYABLE (ScopedFigure)
            DRX_DECLARE_NON_MOVEABLE (ScopedFigure)
        };
        ...
        const auto doTransform = [&transform] (f32 x, f32 y)
        {
            transform.transformPoint (x, y);
            return D2D1_POINT_2F { x, y };
        };

        const auto updateFigure = [&] (f32 x, f32 y)
        {
            if (! figure.has_value())
    ...
    static D2D1_POINT_2F pointTransformed (Point<f32> pt, const AffineTransform& transform)
    {
        transform.transformPoint (pt.x, pt.y);
        return { (FLOAT) pt.x, (FLOAT) pt.y };
    }

    static z0 rectToGeometrySink (const Rectangle<f32>& rect,
                                    ID2D1GeometrySink* sink,
                                    const AffineTransform& transform,
                                    D2D1_FIGURE_BEGIN figureMode)
    {
    ...
    static ComSmartPtr<ID2D1Geometry> rectListToPathGeometry (ID2D1Factory* factory,
                                                              const RectangleList<f32>& clipRegion,
                                                              const AffineTransform& transform,
                                                              D2D1_FILL_MODE fillMode,
                                                              D2D1_FIGURE_BEGIN figureMode,
                                                              [[maybe_unused]] Direct2DMetrics* metrics)
    {
        DRX_D2DMETRICS_SCOPED_ELAPSED_TIME (metrics, createGeometryTime)
        ScopedGeometryWithSink objects { factory, fillMode };

        if (objects.sink == nullptr)
            return {};

        for (i32 i = clipRegion.getNumRectangles(); --i >= 0;)
...
{
public:
    z0 replace (Span<const Point<f32>> positions, f32 scale)
    {
