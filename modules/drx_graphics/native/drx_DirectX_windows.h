...
struct DxgiAdapter : public ReferenceCountedObject
{
    ...
    b8 uniqueIDMatches (ReferenceCountedObjectPtr<DxgiAdapter> other) const
    {
        if (other == nullptr)
            return false;
...
struct DxgiAdapterListener
{
    ...
    virtual z0 adapterCreated (DxgiAdapter::Ptr adapter) = 0;
    virtual z0 adapterRemoved (DxgiAdapter::Ptr adapter) = 0;
};
...
public:
    ...
    z0 addListener (DxgiAdapterListener& l)
    {
        listeners.add (&l);
    }

    z0 removeListener (DxgiAdapterListener& l)
    {
        listeners.remove (&l);
    }
...
private:
    static ComSmartPtr<IDXGIFactory2> makeDxgiFactory()
    {
        ...
        if (const auto hr = CreateDXGIFactory2 (0, __uuidof (IDXGIFactory2), (uk*) result.resetAndGetPointerAddress()); SUCCEEDED (hr))
            return result;
...
private:
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (DirectX)
};
...
struct D2DUtilities
{
    ...
    {
        return { (f32) r.getX(), (f32) r.getY(), (f32) r.getRight(), (f32) r.getBottom() };
    }
    ...
    static Rectangle<i32> toRectangle (const RECT& r)
    {
        return Rectangle<i32>::leftTopRightBottom (r.left, r.top, r.right, r.bottom);
    }

    static Point<i32> toPoint (POINT p) noexcept          { return { p.x, p.y }; }
    static POINT toPOINT (Point<i32> p) noexcept          { return { p.x, p.y }; }

    static D2D1_POINT_2U toPOINT_2U (Point<i32> p)        { return D2D1::Point2U ((UINT32) p.x, (UINT32) p.y); }

    static D2D1_COLOR_F toCOLOR_F (Color c)
    {
    ...
    static Rectangle<i32> rectFromSize (D2D1_SIZE_U s)
    {
...
struct Direct2DBitmap
{
    ...
    {
        DRX_D2DMETRICS_SCOPED_ELAPSED_TIME (Direct2DMetricsHub::getInstance()->imageContextMetrics, createBitmapTime);

        jassert (outputFormat == Image::ARGB || outputFormat == Image::SingleChannel);

        DRX_TRACE_LOG_D2D_PAINT_CALL (etw::createDirect2DBitmapFromImage, etw::graphicsKeyword);
    ...
    {
        DRX_TRACE_LOG_D2D_PAINT_CALL (etw::createDirect2DBitmap, etw::graphicsKeyword);
...
{
public:
    z0 findRECTAndValidate (HWND windowHandle)
    {
        numRect = 0;
