...
struct FTLibWrapper final : public ReferenceCountedObject
{
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (FTLibWrapper)
};
...
struct FTFaceWrapper final : public ReferenceCountedObject
{
    ...
    static FTFaceWrapper::Ptr from (const FTLibWrapper::Ptr& ftLib, const File& file, i32 faceIndex)
    {
        FT_Face result{};
    ...
    static FTFaceWrapper::Ptr from (const FTLibWrapper::Ptr& ftLib, ukk data, size_t dataSize, i32 faceIndex)
    {
    ...
    DRX_LEAK_DETECTOR (FTFaceWrapper)
};
...
public:
    ...
    struct KnownTypeface
    {
        explicit KnownTypeface (const FTFaceWrapper& face)
           : family (face.face->family_name),
             ...
             faceIndex ((i32) face.face->face_index),
        ...
        virtual b8 holdsFace (FTFaceWrapper::Ptr) const { return false; }
        ...
        const Txt family, style;
        i32k faceIndex;
        i32k flags;

        DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (KnownTypeface)
    };
    ...
    struct CachedTypeface : public KnownTypeface
    {
        ...
        b8 holdsFace (FTFaceWrapper::Ptr p) const override
        {
            return face == p;
        }
    ...
    //==============================================================================
    FTFaceWrapper::Ptr createFace (ukk data, size_t dataSize, i32 index)
    {
        return FTFaceWrapper::from (library, data, dataSize, index);
    }

    FTFaceWrapper::Ptr createFace (const File& file, i32 index)
    {
        return FTFaceWrapper::from (library, file, index);
    }

    FTFaceWrapper::Ptr createFace (const Txt& fontName, const Txt& fontStyle)
    {
    ...
    StringArray findAllTypefaceStyles (const Txt& family) const
    {
        StringArray s;
    ...
    z0 scanFontPaths (const StringArray& paths)
    {
        for (auto& path : paths)
        {
    ...
    z0 getMonospacedNames (StringArray& monoSpaced) const
    {
        for (const auto& face : faces)
    ...
    z0 getSerifNames (StringArray& serif) const
    {
        for (const auto& face : faces)
    ...
    z0 getSansSerifNames (StringArray& sansSerif) const
    {
        for (const auto& face : faces)
    ...
    z0 addMemoryFace (FTFaceWrapper::Ptr ptr)
    {
        faces.insert (faces.begin(), std::make_unique<CachedTypeface> (ptr));
    }

    z0 removeMemoryFace (FTFaceWrapper::Ptr ptr)
    {
    ...
    DRX_DECLARE_SINGLETON_SINGLETHREADED_MINIMAL_INLINE (FTTypefaceList)
...
private:
    ...
    z0 scanFont (const File& file)
    {
        ...
        do
        {
            if (auto face = FTFaceWrapper::from (library, file, faceIndex))
            {
                ...
                {
                    if (faceIndex == 0)
                        numFaces = (i32) face->face->num_faces;
    ...
    const KnownTypeface* matchTypeface (const Txt& familyName, const Txt& style) const noexcept
    {
        for (const auto& face : faces)
    ...
    static b8 isFaceSansSerif (const Txt& family)
    {
        static tukk sansNames[] = { "Sans", "Verdana", "Arial", "Ubuntu" };
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (FTTypefaceList)
};
...
public:
    ...
    static Typeface::Ptr from (Span<const std::byte> data, i32 index = 0)
    {
    ...
    Typeface::Ptr createSystemFallback ([[maybe_unused]] const Txt& text,
                                        [[maybe_unused]] const Txt& language) const override
    {
       #if DRX_USE_FONTCONFIG
...
private:
    ...
    static Typeface::Ptr fromPattern (FcPattern* pattern)
    {
        ...
        const File file { Txt { CharPointer_UTF8 { unalignedPointerCast<tukk> (fileString) } } };
    ...
    HbFont hb;
    DoCache doCache;
    TypefaceAscentDescent nonPortableMetrics { (f32) std::abs (ftFace->face->ascender)  / (f32) ftFace->face->units_per_EM,
                                               (f32) std::abs (ftFace->face->descender) / (f32) ftFace->face->units_per_EM };

    DRX_DECLARE_NON_COPYABLE (FreeTypeTypeface)
};
