...
static Txt getLocalisedName (IDWriteLocalizedStrings* names)
{
...
static Txt getFontFamilyName (IDWriteFontFamily* family)
{
...
static Txt getFontFaceName (IDWriteFont* font)
{
...
public:
    ...
    StringArray findAllTypefaceStyles (const Txt& family)
    {
        const std::scoped_lock lock { mutex };
    ...
    z0 addCollection (ComSmartPtr<IDWriteFontCollection> collection)
    {
        const std::scoped_lock lock { mutex };
        collections.push_back (std::move (collection));
    }

    z0 removeCollection (ComSmartPtr<IDWriteFontCollection> collection)
    {
        const std::scoped_lock lock { mutex };
...
public:
    ...
    DRX_COMRESULT GetFileSize (UINT64* fileSize) noexcept override
    {
        *fileSize = block->getSize();
        return S_OK;
    }

    DRX_COMRESULT GetLastWriteTime (UINT64* lastWriteTime) noexcept override
    {
        *lastWriteTime = 0;
        return S_OK;
    }

    DRX_COMRESULT ReadFileFragment (ukk* fragmentStart,
                                     UINT64 fileOffset,
                                     UINT64 fragmentSize,
                                     uk* fragmentContext) noexcept override
    {
    ...
    z0 WINAPI ReleaseFileFragment (uk) noexcept override {}
...
public:
    ...
    HRESULT WINAPI CreateStreamFromKey (ukk fontFileReferenceKey,
                                        UINT32 keySize,
                                        IDWriteFontFileStream** fontFileStream) noexcept override
    {
        ...
        Uuid requestedKey { static_cast<u8k*> (fontFileReferenceKey) };
...
private:
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (MemoryFontFileLoader)
};
...
private:
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (FontFileEnumerator)
};
...
public:
                                            ...
                                            IDWriteFontFileEnumerator** fontFileEnumerator) noexcept override
    {
        ...
        const Uuid requestedCollectionKey { static_cast<u8k*> (collectionKey) };
...
private:
    ...
    {
        DRX_LOAD_WINAPI_FUNCTION (direct2dDll,
                                   D2D1CreateFactory,
                                   d2d1CreateFactory,
                                   HRESULT,
                                   (D2D1_FACTORY_TYPE, REFIID, D2D1_FACTORY_OPTIONS*, uk*))
        ...
        d2d1CreateFactory (D2D1_FACTORY_TYPE_SINGLE_THREADED,
                            ...
                            &options,
                           (uk*) result.resetAndGetPointerAddress());
    ...
    {
        DRX_LOAD_WINAPI_FUNCTION (directWriteDll,
                                   DWriteCreateFactory,
                                   dWriteCreateFactory,
                                   HRESULT,
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (Direct2DFactories)
};
...
private:
    ...
    {
    public:
        AnalysisSource (Txt cIn, Txt langIn)
            : character (cIn), language (langIn) {}

        ~AnalysisSource() override = default;

        DRX_COMCALL GetLocaleName (UINT32, UINT32*, const WCHAR** localeName) noexcept override
        {
            *localeName = language.isNotEmpty() ? language.toWideCharPointer() : nullptr;
            return S_OK;
        }

        DRX_COMCALL GetNumberSubstitution (UINT32, UINT32*, IDWriteNumberSubstitution** substitution) noexcept override
        {
        ...
        DRX_COMCALL GetTextAtPosition (UINT32 textPosition, const WCHAR** textString, UINT32* textLength) noexcept override
        {
            if (textPosition == 0)
            {
            ...
            {
                // We don't expect this to be hit. If you see this, alert the DRX team!
                jassertfalse;
                *textString = nullptr;
                *textLength = 0;
            }
        ...
        DRX_COMCALL GetTextBeforePosition (UINT32, const WCHAR** textString, UINT32* textLength) noexcept override
        {
            // We don't expect this to be hit. If you see this, alert the DRX team!
            jassertfalse;
    ...
    static Txt getLocalisedFamilyName (IDWriteFont& font)
    {
        ComSmartPtr<IDWriteFontFamily> family;
    ...
    static Txt getLocalisedFamilyName (IDWriteFontFamily& fontFamily)
    {
        ComSmartPtr<IDWriteLocalizedStrings> familyNames;
    ...
    static Txt getLocalisedStyle (IDWriteFont& font)
    {
        ComSmartPtr<IDWriteLocalizedStrings> faceNames;
    ...
    WindowsDirectWriteTypeface (const Txt& name,
                                const Txt& style,
                                ComSmartPtr<IDWriteFont> font,
    ...
    static TypefaceAscentDescent getDwriteMetrics (IDWriteFontFace& face)
    {
        ...
        return TypefaceAscentDescent { (f32) dwriteFontMetrics.ascent  / (f32) dwriteFontMetrics.designUnitsPerEm,
                                       (f32) dwriteFontMetrics.descent / (f32) dwriteFontMetrics.designUnitsPerEm };
    }
    ...
    static std::optional<TypefaceAscentDescent> getGdiMetrics (hb_font_t* font)
    {
        ...
        const auto upem = (f32) hb_face_get_upem (hb_font_get_face (font));
        return TypefaceAscentDescent { (f32) std::abs (ascent) / upem, (f32) std::abs (descent) / upem };
    }
    ...
    // This attempts to replicate the behaviour of the non-directwrite typeface lookup in DRX 7 and older
...
struct DefaultFontNames
{
    ...
    Txt defaultSans, defaultSerif, defaultFixed, defaultFallback;
};
