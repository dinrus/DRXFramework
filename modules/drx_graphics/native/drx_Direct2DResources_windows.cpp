...
static ComSmartPtr<ID2D1GradientStopCollection> makeGradientStopCollection (const ColorGradient& gradient,
                                                                            ComSmartPtr<ID2D1DeviceContext1> deviceContext,
                                                                            [[maybe_unused]] Direct2DMetrics* metrics) noexcept
{
    DRX_D2DMETRICS_SCOPED_ELAPSED_TIME (metrics, createGradientTime);

    i32k numColors = gradient.getNumColors();

    std::vector<D2D1_GRADIENT_STOP> stops ((size_t) numColors);

    for (auto [index, stop] : enumerate (stops, i32{}))
    {
        stop.color = D2DUtilities::toCOLOR_F (gradient.getColor (index));
        stop.position = (FLOAT) gradient.getColorPosition (index);
    }
...
{
public:
    ComSmartPtr<ID2D1LinearGradientBrush> get (const ColorGradient& gradient,
                                               ComSmartPtr<ID2D1DeviceContext1> deviceContext,
...
private:
    LruCache<ColorGradient, ComSmartPtr<ID2D1LinearGradientBrush>> cache;
};

class RadialGradientCache
{
public:
    ComSmartPtr<ID2D1RadialGradientBrush> get (const ColorGradient& gradient,
                                               ComSmartPtr<ID2D1DeviceContext1> deviceContext,
                                               Direct2DMetrics* metrics)
    {
        jassert (gradient.isRadial);
...
private:
    LruCache<ColorGradient, ComSmartPtr<ID2D1RadialGradientBrush>> cache;
};
...
public:
    ...
    z0 fillRectangles (ComSmartPtr<ID2D1DeviceContext1> deviceContext,
                         const RectangleList<f32>& rectangles,
                         const Color colour,
                         ...
                         [[maybe_unused]] Direct2DMetrics* metrics)
    {
        ...
        DRX_D2DMETRICS_SCOPED_ELAPSED_TIME (metrics, spriteBatchTime)
        ...
        while (numRectanglesPainted < rectangles.getNumRectangles())
        {
            ...
            {
                DRX_D2DMETRICS_SCOPED_ELAPSED_TIME (metrics, spriteBatchSetupTime);
                ...
                for (i32 i = numRectanglesPainted; i < numRectanglesPainted + spriteBatchSize; ++i)
                {
            ...
            if (! whiteRectangle)
            {
                DRX_D2DMETRICS_SCOPED_ELAPSED_TIME (metrics, createSpriteSourceTime);

                auto hr = deviceContext->CreateCompatibleRenderTarget (D2D1_SIZE_F { (f32) rectangleSize, (f32) rectangleSize },
            ...
            if (auto hr = whiteRectangle->GetBitmap (bitmap.resetAndGetPointerAddress()); SUCCEEDED (hr))
            {
                ...
                {
                    auto d2dColor = D2DUtilities::toCOLOR_F (colour);
                    auto spriteBatch = getSpriteBatch (*deviceContext3, (u32) spriteBatchSize);

                    if (spriteBatch == nullptr)
                        return;

                    auto setCount = jmin ((u32) spriteBatchSize, spriteBatch->GetSpriteCount());
                    auto addCount = (u32) spriteBatchSize > setCount ? (u32) spriteBatchSize - setCount : 0;

                    if (setCount != 0)
                    {
                        DRX_D2DMETRICS_SCOPED_ELAPSED_TIME (metrics, setSpritesTime);

                        spriteBatch->SetSprites (0, setCount, destinations.getData(), nullptr, &d2dColor, nullptr, sizeof (D2D1_RECT_F), 0, 0, 0);
                    }

                    if (addCount != 0)
                    {
                        DRX_D2DMETRICS_SCOPED_ELAPSED_TIME (metrics, addSpritesTime);

                        spriteBatch->AddSprites (addCount, destinations.getData() + setCount, nullptr, &d2dColor, nullptr, sizeof (D2D1_RECT_F), 0, 0, 0);
                    }

                    DRX_D2DMETRICS_SCOPED_ELAPSED_TIME (metrics, drawSpritesTime);
...
private:
    ComSmartPtr<ID2D1SpriteBatch> getSpriteBatch (ID2D1DeviceContext3& dc, u32 key)
    {
        return spriteBatches.get ((u32) key, [&dc] (auto) -> ComSmartPtr<ID2D1SpriteBatch>
        {
...
public:
    static DxgiAdapter::Ptr findAdapter (const DxgiAdapters& adapters, ID2D1Bitmap1* bitmap)
    {
        ...
        surface->GetDevice (__uuidof (device), (uk*) device.resetAndGetPointerAddress());
...
public:
    ...
    HRESULT create (HWND hwnd, Rectangle<i32> size, DxgiAdapter::Ptr adapter)
    {
    ...
    HRESULT resize (Rectangle<i32> newSize)
    {
        if (chain == nullptr)
            return E_FAIL;
        ...
        ComSmartPtr<IDXGIDevice> device;
        DRX_BEGIN_IGNORE_WARNINGS_GCC_LIKE ("-Wlanguage-extension-token")
        chain->GetDevice (__uuidof (device), (uk*) device.resetAndGetPointerAddress());
        DRX_END_IGNORE_WARNINGS_GCC_LIKE
    ...
    Rectangle<i32> getSize() const
    {
        ...
        return { (i32) desc.Width, (i32) desc.Height };
    }
    ...
    static constexpr u32 swapChainFlags = DXGI_SWAP_CHAIN_FLAG_FRAME_LATENCY_WAITABLE_OBJECT;
    static constexpr u32 presentSyncInterval = 1;
    static constexpr u32 presentFlags = 0;
...
private:
    ComSmartPtr<IDXGISurface> getSurface() const
    {
        ...
        ComSmartPtr<IDXGISurface> surface;
        DRX_BEGIN_IGNORE_WARNINGS_GCC_LIKE ("-Wlanguage-extension-token")
        if (const auto hr = chain->GetBuffer (0, __uuidof (surface), reinterpret_cast<uk*> (surface.resetAndGetPointerAddress())); FAILED (hr))
            return nullptr;
        DRX_END_IGNORE_WARNINGS_GCC_LIKE
    ...
    z0 createBuffer (DxgiAdapter::Ptr adapter)
    {
        buffer = nullptr;
...
public:
    static std::optional<CompositionTree> create (IDXGIDevice* dxgiDevice, HWND hwnd, IDXGISwapChain1* swapChain)
    {
        ...
        if (const auto hr = DCompositionCreateDevice (dxgiDevice,
                                                      ...
                                                      reinterpret_cast<uk*> (result.compositionDevice.resetAndGetPointerAddress()));
                FAILED (hr))
        {
            return {};
        }
