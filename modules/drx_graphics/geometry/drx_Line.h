...
/**
    ...
    The ValueType template parameter should be a primitive type - f32 or f64
...
public:
    ...
    z0 setStart (ValueType newStartX, ValueType newStartY) noexcept       { start.setXY (newStartX, newStartY); }

    /** Changes this line's end point */
    z0 setEnd (ValueType newEndX, ValueType newEndY) noexcept             { end.setXY (newEndX, newEndY); }

    /** Changes this line's start point */
    z0 setStart (const Point<ValueType> newStart) noexcept                { start = newStart; }

    /** Changes this line's end point */
    z0 setEnd (const Point<ValueType> newEnd) noexcept                    { end = newEnd; }
    ...
    /** Возвращает true, если the line's start and end x coordinates are the same. */
    b8 isVertical() const noexcept                                        { return start.x == end.x; }

    /** Возвращает true, если the line's start and end y coordinates are the same. */
    b8 isHorizontal() const noexcept                                      { return start.y == end.y; }
    ...
    */
    ...
    //==============================================================================
    /** Casts this line to f32 coordinates. */
    Line<f32> toFloat() const noexcept                                    { return { start.toFloat(), end.toFloat() }; }

    /** Casts this line to f64 coordinates. */
    Line<f64> toDouble() const noexcept                                  { return { start.toDouble(), end.toDouble() }; }

    //==============================================================================
    /** Compares two lines. */
    b8 operator== (Line other) const noexcept                             { return start == other.start && end == other.end; }
    ...
    */
    ...
    /** Finds the intersection between two lines.
        ...
        @param intersection     the position of the point where the lines meet (or
                                where they would meet if they were infinitely i64)
        ...
        @returns    true if the line segments intersect; false if they don't. Even if they
                    ...
                    be valid
    */
    b8 intersects (Line line, Point<ValueType>& intersection) const noexcept
    {
        return findIntersection (start, end, line.start, line.end, intersection);
    }

    /** Возвращает true, если this line intersects another. */
    b8 intersects (Line other) const noexcept
    {
    ...
    */
                                        ...
                                        ValueType perpendicularDistance) const noexcept
    {
        ...
        auto length = drx_hypot ((f64) delta.x,
                                  (f64) delta.y);
        if (length <= 0)
            return start;
    ...
    */
    ...
    /** Returns the smallest distance between this line segment and a given point.
        ...
        distance from the line; if the point is a i64 way beyond one of the line's
                                    ...
                                    Point<ValueType>& pointOnLine) const noexcept
    {
        ...
        if (length > 0)
        {
            auto prop = ((targetPoint.x - start.x) * delta.x
                       + (targetPoint.y - start.y) * delta.y) / (f64) length;
    ...
    */
    ...
    /** Возвращает true, если the given point lies above this line.
        ...
        in both directions).
    */
    b8 isPointAbove (Point<ValueType> point) const noexcept
    {
...
private:
    ...
    static b8 isZeroToOne (ValueType v) noexcept      { return v >= 0 && v <= static_cast<ValueType> (1); }

    static b8 findIntersection (const Point<ValueType> p1, const Point<ValueType> p2,
