...
PathStrokeType::PathStrokeType (f32 strokeThickness) noexcept
    : thickness (strokeThickness), jointStyle (mitered), endStyle (butt)
{
}

PathStrokeType::PathStrokeType (f32 strokeThickness, JointStyle joint, EndCapStyle end) noexcept
...
b8 PathStrokeType::operator== (const PathStrokeType& other) const noexcept
{
...
namespace PathStrokeHelpers
{
    ...
    {
        Point<f32> point;
        f32 distanceBeyondLine1EndSquared;
        b8 intersects;
    };

    static LineIntersection lineIntersection (const f32 x1, const f32 y1,
                                              const f32 x2, const f32 y2,
                                              const f32 x3, const f32 y3,
                                              const f32 x4, const f32 y4)
    {
    ...
    static z0 addEdgeAndJoint (Path& destPath,
                                 const PathStrokeType::JointStyle style,
                                 const f32 maxMiterExtensionSquared, const f32 width,
                                 const f32 x1, const f32 y1,
                                 const f32 x2, const f32 y2,
                                 const f32 x3, const f32 y3,
                                 const f32 x4, const f32 y4,
                                 const f32 midX, const f32 midY)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    // curved joints
                    f32 angle1 = std::atan2 (x2 - midX, y2 - midY);
                    f32 angle2 = std::atan2 (x3 - midX, y3 - midY);
                    const f32 angleIncrement = 0.1f;

                    destPath.lineTo (x2, y2);

                    if (std::abs (angle1 - angle2) > angleIncrement)
                    {
                        if (angle2 > angle1 + MathConstants<f32>::pi
                             || (angle2 < angle1 && angle2 >= angle1 - MathConstants<f32>::pi))
                        {
                            if (angle2 > angle1)
                                angle2 -= MathConstants<f32>::twoPi;

                            jassert (angle1 <= angle2 + MathConstants<f32>::pi);
                        ...
                        {
                            if (angle1 > angle2)
                                angle1 -= MathConstants<f32>::twoPi;
    ...
    static z0 addLineEnd (Path& destPath,
                            ...
                            const f32 width)
    {
        ...
        {
            f32 offx1, offy1, offx2, offy2;

            auto dx = x2 - x1;
            auto dy = y2 - y1;
            auto len = drx_hypot (dx, dy);
    ...
    struct Arrowhead
    {
        f32 startWidth, startLength;
        f32 endWidth, endLength;
    };

    static z0 addArrowhead (Path& destPath,
                              const f32 x1, const f32 y1,
                              const f32 x2, const f32 y2,
                              const f32 tipX, const f32 tipY,
                              const f32 width,
                              const f32 arrowheadWidth)
    {
        Line<f32> line (x1, y1, x2, y2);
    ...
    struct LineSection
    {
        f32 x1, y1, x2, y2;      // original line
        f32 lx1, ly1, lx2, ly2;  // the left-hand stroke
        f32 rx1, ry1, rx2, ry2;  // the right-hand stroke
    };

    static z0 shortenSubPath (Array<LineSection>& subPath, f32 amountAtStart, f32 amountAtEnd)
    {
    ...
    static z0 addSubPath (Path& destPath, Array<LineSection>& subPath,
                            const b8 isClosed, const f32 width, const f32 maxMiterExtensionSquared,
    ...
    static z0 createStroke (const f32 thickness, const PathStrokeType::JointStyle jointStyle,
                              const PathStrokeType::EndCapStyle endStyle,
                              Path& destPath, const Path& source,
                              const AffineTransform& transform,
                              const f32 extraAccuracy, const Arrowhead* const arrowhead)
    {
        ...
        const f32 maxMiterExtensionSquared = 9.0f * thickness * thickness;
        const f32 width = 0.5f * thickness;
        ...
        const f32 minSegmentLength = 0.0001f;
...
z0 PathStrokeType::createStrokedPath (Path& destPath, const Path& sourcePath,
                                        const AffineTransform& transform, f32 extraAccuracy) const
{
    PathStrokeHelpers::createStroke (thickness, jointStyle, endStyle, destPath, sourcePath,
                                     transform, extraAccuracy, nullptr);
}

z0 PathStrokeType::createDashedStroke (Path& destPath,
                                         const Path& sourcePath,
                                         const f32* dashLengths,
                                         i32 numDashLengths,
                                         const AffineTransform& transform,
                                         f32 extraAccuracy) const
{
    jassert (extraAccuracy > 0);
    ...
    b8 first = true;
    i32 dashNum = 0;
    f32 pos = 0.0f, lineLen = 0.0f, lineEndPos = 0.0f;
    f32 dx = 0.0f, dy = 0.0f;

    for (;;)
    {
        const b8 isSolid = ((dashNum & 1) == 0);
        const f32 dashLen = dashLengths [dashNum++ % numDashLengths];
        ...
        while (pos > lineEndPos)
        {
            ...
            dx = it.x2 - it.x1;
            dy = it.y2 - it.y1;
            lineLen = drx_hypot (dx, dy);
            lineEndPos += lineLen;
            first = it.closesSubPath;
        }

        const f32 alpha = (pos - (lineEndPos - lineLen)) / lineLen;
...
z0 PathStrokeType::createStrokeWithArrowheads (Path& destPath,
                                                 const Path& sourcePath,
                                                 const f32 arrowheadStartWidth, const f32 arrowheadStartLength,
                                                 const f32 arrowheadEndWidth, const f32 arrowheadEndLength,
                                                 const AffineTransform& transform,
                                                 const f32 extraAccuracy) const
{
