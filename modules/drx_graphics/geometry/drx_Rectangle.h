...
inline auto getNumericValue (StringRef s, Tag<i32>)    { return s.text.getIntValue32(); }
inline auto getNumericValue (StringRef s, Tag<f64>) { return s.text.getDoubleValue(); }
inline auto getNumericValue (StringRef s, Tag<f32>)  { return static_cast<f32> (s.text.getDoubleValue()); }
...
inline i32 floorAsInt (i32 n) noexcept     { return n; }
inline i32 floorAsInt (f32 n) noexcept   { return n > (f32)  std::numeric_limits<i32>::min() ? (i32) std::floor (n) : std::numeric_limits<i32>::min(); }
inline i32 floorAsInt (f64 n) noexcept  { return n > (f64) std::numeric_limits<i32>::min() ? (i32) std::floor (n) : std::numeric_limits<i32>::min(); }

inline i32 ceilAsInt (i32 n) noexcept      { return n; }
inline i32 ceilAsInt (f32 n) noexcept    { return n < (f32)  std::numeric_limits<i32>::max() ? (i32) std::ceil (n) : std::numeric_limits<i32>::max(); }
inline i32 ceilAsInt (f64 n) noexcept   { return n < (f64) std::numeric_limits<i32>::max() ? (i32) std::ceil (n) : std::numeric_limits<i32>::max(); }
...
public:
    ...
    */
    ...
    //==============================================================================
    /** Возвращает true, если the rectangle's width or height are zero or less */
    b8 isEmpty() const noexcept                                   { return w <= ValueType() || h <= ValueType(); }

    /** Возвращает true, если the rectangle's values are all finite numbers, i.e. not NaN or infinity. */
    inline b8 isFinite() const noexcept                           { return pos.isFinite() && drx_isfinite (w) && drx_isfinite (h); }
    ...
    ValueType getAspectRatio (b8 widthOverHeight = true) const noexcept                           { return widthOverHeight ? w / h : h / w; }
    ...
    inline z0 setPosition (Point<ValueType> newPos) noexcept                                      { pos = newPos; }

    /** Changes the position of the rectangle's top-left corner (leaving its size unchanged). */
    inline z0 setPosition (ValueType newX, ValueType newY) noexcept                               { pos.setXY (newX, newY); }
    ...
    z0 setSize (ValueType newWidth, ValueType newHeight) noexcept                                 { w = newWidth; h = newHeight; }

    /** Changes all the rectangle's coordinates. */
    z0 setBounds (ValueType newX, ValueType newY,
    ...
    inline z0 setX (ValueType newX) noexcept                                                      { pos.x = newX; }

    /** Changes the rectangle's Y coordinate */
    inline z0 setY (ValueType newY) noexcept                                                      { pos.y = newY; }

    /** Changes the rectangle's width */
    inline z0 setWidth (ValueType newWidth) noexcept                                              { w = newWidth; }

    /** Changes the rectangle's height */
    inline z0 setHeight (ValueType newHeight) noexcept                                            { h = newHeight; }

    /** Changes the position of the rectangle's centre (leaving its size unchanged). */
    inline z0 setCentre (ValueType newCentreX, ValueType newCentreY) noexcept                     { pos.x = newCentreX - w / (ValueType) 2;
                                                                                                      pos.y = newCentreY - h / (ValueType) 2; }

    /** Changes the position of the rectangle's centre (leaving its size unchanged). */
    inline z0 setCentre (Point<ValueType> newCentre) noexcept                                     { setCentre (newCentre.x, newCentre.y); }

    /** Changes the position of the rectangle's left and right edges. */
    z0 setHorizontalRange (Range<ValueType> range) noexcept                                       { pos.x = range.getStart(); w = range.getLength(); }

    /** Changes the position of the rectangle's top and bottom edges. */
    z0 setVerticalRange (Range<ValueType> range) noexcept                                         { pos.y = range.getStart(); h = range.getLength(); }
    ...
    /** Moves the x position, adjusting the width so that the right-hand edge remains in the same place.
        ...
        @see withLeft
    */
    z0 setLeft (ValueType newLeft) noexcept                   { w = jmax (ValueType(), pos.x + w - newLeft); pos.x = newLeft; }
    ...
    */
    ...
    /** Moves the y position, adjusting the height so that the bottom edge remains in the same place.
        ...
        @see withTop
    */
    z0 setTop (ValueType newTop) noexcept                     { h = jmax (ValueType(), pos.y + h - newTop); pos.y = newTop; }
    ...
    */
    z0 setRight (ValueType newRight) noexcept                 { pos.x = jmin (pos.x, newRight); w = newRight - pos.x; }
    ...
    */
    z0 setBottom (ValueType newBottom) noexcept               { pos.y = jmin (pos.y, newBottom); h = newBottom - pos.y; }
    ...
    */
    ...
    /** Returns a rectangle that has been scaled by the given amount, centred around the origin.
        Note that if the rectangle has i32 coordinates and it's scaled by a
    ...
    */
    ...
    //==============================================================================
    /** Возвращает true, если the two rectangles are identical. */
    b8 operator== (const Rectangle& other) const noexcept
    {
        const auto tie = [] (const Rectangle& r) { return std::tie (r.pos, r.w, r.h); };
        return tie (*this) == tie (other);
    }

    /** Возвращает true, если the two rectangles are not identical. */
    b8 operator!= (const Rectangle& other) const noexcept     { return ! operator== (other); }

    /** Возвращает true, если this coordinate is inside the rectangle. */
    b8 contains (ValueType xCoord, ValueType yCoord) const noexcept
    {
        return xCoord >= pos.x && yCoord >= pos.y && xCoord < pos.x + w && yCoord < pos.y + h;
    }

    /** Возвращает true, если this coordinate is inside the rectangle. */
    b8 contains (Point<ValueType> point) const noexcept
    {
        return point.x >= pos.x && point.y >= pos.y && point.x < pos.x + w && point.y < pos.y + h;
    }

    /** Возвращает true, если this other rectangle is completely inside this one. */
    b8 contains (Rectangle other) const noexcept
    {
        return pos.x <= other.pos.x && pos.y <= other.pos.y
            && pos.x + w >= other.pos.x + other.w && pos.y + h >= other.pos.y + other.h;
    }

    /** Возвращает true, если any part of another rectangle overlaps this one. */
    b8 intersects (Rectangle other) const noexcept
    {
    ...
    /** Возвращает true, если any part of the given line lies inside this rectangle. */
    b8 intersects (const Line<ValueType>& line) const noexcept
    {
    ...
    */
    b8 intersectRectangle (ValueType& otherX, ValueType& otherY, ValueType& otherW, ValueType& otherH) const noexcept
    {
    ...
    /** Clips a rectangle so that it lies only within this one.
        Returns false if the two rectangles didn't overlap.
    */
    b8 intersectRectangle (Rectangle<ValueType>& rectangleToClip) const noexcept
    {
    ...
    */
    b8 enlargeIfAdjacent (Rectangle other) noexcept
    {
    ...
    */
    b8 reduceIfPartlyContainedIn (Rectangle other) noexcept
    {
        i32 inside = 0;
    ...
    */
    Rectangle<i32> getSmallestIntegerContainer() const noexcept
    {
        return Rectangle<i32>::leftTopRightBottom (detail::floorAsInt (pos.x),
    ...
    */
    Rectangle<i32> getLargestIntegerWithin() const noexcept
    {
    ...
    /** Casts this rectangle to a Rectangle<i32>.
    ...
    */
    Rectangle<i32> toNearestInt() const noexcept
    {
    ...
    */
    Rectangle<i32> toNearestIntEdges() const noexcept
    {
        return Rectangle<i32>::leftTopRightBottom (roundToInt (pos.x),       roundToInt (pos.y),
                                                   roundToInt (getRight()),  roundToInt (getBottom()));
    }

    /** Casts this rectangle to a Rectangle<f32>.
        @see getSmallestIntegerContainer
    */
    Rectangle<f32> toFloat() const noexcept
    {
        return { static_cast<f32> (pos.x), static_cast<f32> (pos.y),
                 static_cast<f32> (w),     static_cast<f32> (h) };
    }

    /** Casts this rectangle to a Rectangle<f64>.
        @see getSmallestIntegerContainer
    */
    Rectangle<f64> toDouble() const noexcept
    {
        return { static_cast<f64> (pos.x), static_cast<f64> (pos.y),
                 static_cast<f64> (w),     static_cast<f64> (h) };
    }

    /** Casts this rectangle to a Rectangle with the given type.
        If the target type is a conversion from f32 to i32, then the conversion
    ...
    */
    ...
    static Rectangle findAreaContainingPoints (const Point<ValueType>* points, i32 numPoints) noexcept
    {
        if (numPoints <= 0)
            return {};
    ...
    */
    static b8 intersectRectangles (ValueType& x1, ValueType& y1, ValueType& w1, ValueType& h1,
...
private:
    ...
    z0 copyWithRounding (Rectangle<i32>&    result) const noexcept { result = getSmallestIntegerContainer(); }
    z0 copyWithRounding (Rectangle<f32>&  result) const noexcept { result = toFloat(); }
    z0 copyWithRounding (Rectangle<f64>& result) const noexcept { result = toDouble(); }
};
