...
public:
    ...
    //==============================================================================
    /** Возвращает true, если the region is empty. */
    b8 isEmpty() const noexcept                               { return rects.isEmpty(); }

    /** Returns the number of rectangles in the list. */
    i32 getNumRectangles() const noexcept                       { return rects.size(); }
    ...
    */
    RectangleType getRectangle (i32 index) const noexcept       { return rects[index]; }
    ...
    */
    z0 add (RectangleType rect)
    {
        ...
        if (! rect.isEmpty())
        {
            ...
            {
                b8 anyOverlaps = false;

                for (i32 j = rects.size(); --j >= 0;)
                {
    ...
    */
    z0 add (ValueType x, ValueType y, ValueType width, ValueType height)
    {
    ...
    /** Dumbly adds a rectangle to the list without checking for overlaps.
        ...
        then it's expected to not contain any INF values.
    */
    z0 addWithoutMerging (RectangleType rect)
    {
        jassert (rect.isFinite()); // You must provide a valid rectangle to this method!
    ...
    */
    z0 add (const RectangleList& other)
    {
        for (auto& r : other)
            add (r);
    }
    ...
    /** Removes a rectangular region from the list.
        ...
        if necessary.
    */
    z0 subtract (const RectangleType rect)
    {
    ...
    */
    b8 subtract (const RectangleList& otherList)
    {
    ...
    /** Removes any areas of the region that lie outside a given rectangle.
        ...
        Возвращает true, если the resulting region is not empty, false if it is empty.

        @see getIntersectionWith
    */
    b8 clipTo (RectangleType rect)
    {
        jassert (rect.isFinite()); // You must provide a valid rectangle to this method!
    ...
    */
    ...
    b8 clipTo (const RectangleList<OtherValueType>& other)
    {
        if (isEmpty())
            return false;
    ...
    /** Creates a region which is the result of clipping this one to a given rectangle.
        ...
        @see clipTo
    */
    b8 getIntersectionWith (RectangleType rect, RectangleList& destRegion) const
    {
        jassert (rect.isFinite()); // You must provide a valid rectangle to this method!
    ...
    /** Swaps the contents of this and another list.
        ...
        to swap them.
    */
    z0 swapWith (RectangleList& otherList) noexcept
    {
    ...
    */
    b8 containsPoint (Point<ValueType> point) const noexcept
    {
        for (auto& r : rects)
            if (r.contains (point))
                return true;
    ...
    /** Checks whether the region contains a given point.
        @returns true if the point lies within one of the rectangles in the list
    */
    b8 containsPoint (ValueType x, ValueType y) const noexcept
    {
    ...
    */
    b8 containsRectangle (RectangleType rectangleToCheck) const
    {
        if (rects.size() > 1)
        {
    ...
    */
    b8 intersectsRectangle (RectangleType rectangleToCheck) const noexcept
    {
        for (auto& r : rects)
    ...
    */
    b8 intersects (const RectangleList& other) const noexcept
    {
        for (auto& r : rects)
    ...
    */
    ...
    z0 offsetAll (Point<ValueType> offset) noexcept
    {
        for (auto& r : rects)
            r += offset;
    }
    ...
    /** Adds an x and y value to all the coordinates. */
    z0 offsetAll (ValueType dx, ValueType dy) noexcept
    {
    ...
    z0 scaleAll (ScaleType scaleFactor) noexcept
    {
        for (auto& r : rects)
            r *= scaleFactor;
    }
    ...
    /** Applies a transform to all the rectangles.
        ...
        rotation or skewing.
    */
    z0 transformAll (const AffineTransform& transform) noexcept
    {
        for (auto& r : rects)
    ...
    */
    z0 ensureStorageAllocated (i32 minNumRectangles)
    {
