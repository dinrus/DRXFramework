...
class HbScale
{
    static constexpr f32 factor = 1 << 16;

public:
    HbScale() = delete;

    static constexpr hb_position_t juceToHb (f32 pos)
    {
        return (hb_position_t) (pos * factor);
    }

    static constexpr f32 hbToDrx (hb_position_t pos)
    {
        return (f32) pos / (f32) factor;
    }
};
...
z0 getAdvancesForGlyphs (hb_font_t* hbFont, CTFontRef ctFont, Span<const CGGlyph> glyphs, Span<CGSize> advances)
{
    jassert (glyphs.size() == advances.size());

    i32 x, y;
    hb_font_get_scale (hbFont, &x, &y);
    const auto scaleAdjustment = HbScale::hbToDrx (orientation == kCTFontOrientationHorizontal ? x : y) / CTFontGetSize (ctFont);
...
{
    return [] (hb_font_t* f, uk, hb_codepoint_t glyph, uk voidFontRef) -> hb_position_t
    {
        ...
        return HbScale::juceToHb ((f32) (orientation == kCTFontOrientationHorizontal ? advances->width : advances->height));
    };
}
...
static auto getAdvancesFn()
{
               ...
               uk voidFontRef)
    {
        ...
        for (auto [index, advance] : enumerate (advances))
            *addBytesToPointer (firstAdvance, advanceStride * index) = HbScale::juceToHb ((f32) (orientation == kCTFontOrientationHorizontal ? advance.width : advance.height));
    };
}
...
/*  This function overrides the callbacks that fetch glyph advances for fonts on macOS.
    ...
    platforms, as i64 as the CTFont is set to the size that will eventually be rendered.
...
*/
static z0 overrideCTFontAdvances (hb_font_t* hb, CTFontRef fontRef)
{
    ...
    hb_font_funcs_set_glyph_h_advance_func  (funcs.get(), getAdvanceFn <kCTFontOrientationHorizontal>(), (uk) fontRef, nullptr);
    hb_font_funcs_set_glyph_v_advance_func  (funcs.get(), getAdvanceFn <kCTFontOrientationVertical>(),   (uk) fontRef, nullptr);
    hb_font_funcs_set_glyph_h_advances_func (funcs.get(), getAdvancesFn<kCTFontOrientationHorizontal>(), (uk) fontRef, nullptr);
    hb_font_funcs_set_glyph_v_advances_func (funcs.get(), getAdvancesFn<kCTFontOrientationVertical>(),   (uk) fontRef, nullptr);
    ...
    hb_font_set_funcs (hb, funcs.get(), (uk) fontRef, [] (uk ptr)
    {
...
{
    f32 ascent{};  // in em units
    f32 descent{}; // in em units

    f32 getScaledAscent()  const { return ascent  * getHeightToPointsFactor(); }
    f32 getScaledDescent() const { return descent * getHeightToPointsFactor(); }

    f32 getPointsToHeightFactor() const { return ascent + descent; }
    f32 getHeightToPointsFactor() const { return 1.0f / getPointsToHeightFactor(); }
...
public:
    ...
    HbFont getFontAtPointSizeAndScale (f32 points, f32 horizontalScale) const
    {
...
private:
    static TypefaceAscentDescent findPortableMetrics (hb_font_t* f, TypefaceAscentDescent fallback)
    {
        ...
        const auto ascent  = std::abs ((f32) extents.ascender);
        const auto descent = std::abs ((f32) extents.descender);
        const auto upem    = (f32) hb_face_get_upem (hb_font_get_face (f));
...
{
    static z0 initSynthetics (hb_font_t* hb, const Font& font)
    {
    ...
    static tukk getStyleName (const b8 bold,
                                     const b8 italic) noexcept
    {
    ...
    struct ConcreteFamilyNames
    {
        ...
        Txt lookUp (const Txt& placeholder)
        {
    ...
    private:
        static Txt findName (const Txt& placeholder)
        {
    ...
    static Txt getConcreteFamilyNameFromPlaceholder (const Txt& placeholder)
    {
        static ConcreteFamilyNames names;
        return names.lookUp (placeholder);
    }

    static Txt getConcreteFamilyName (const Font& font)
    {
        const Txt& family = font.getTypefaceName();

        return isPlaceholderFamilyName (family) ? getConcreteFamilyNameFromPlaceholder (family)
                                                : family;
    }

    static HbFace getFaceForBlob (Span<const t8> bytes, u32 index)
    {
...
//==============================================================================
Typeface::Typeface (const Txt& faceName, const Txt& faceStyle) noexcept
    : name (faceName),
      style (faceStyle)
{
}
...
static HbDrawFuncs getPathDrawFuncs()
{
    ...
    hb_draw_funcs_set_move_to_func (funcs.get(), [] (auto*, uk data, auto*, f32 x, f32 y, auto*)
    {
        auto& path = *static_cast<Path*> (data);
        path.startNewSubPath ({ x, y });
    }, nullptr, nullptr);
    hb_draw_funcs_set_line_to_func (funcs.get(), [] (auto*, uk data, auto*, f32 x, f32 y, auto*)
    {
        auto& path = *static_cast<Path*> (data);
        path.lineTo ({ x, y });
    }, nullptr, nullptr);
    hb_draw_funcs_set_quadratic_to_func (funcs.get(), [] (auto*, uk data, auto*, f32 ctlX, f32 ctlY, f32 toX, f32 toY, auto*)
    {
        auto& path = *static_cast<Path*> (data);
        path.quadraticTo ({ ctlX, ctlY }, { toX, toY });
    }, nullptr, nullptr);
    hb_draw_funcs_set_cubic_to_func (funcs.get(), [] (auto*, uk data, auto*, f32 ctlX1, f32 ctlY1, f32 ctlX2, f32 ctlY2, f32 toX, f32 toY, auto*)
    {
        auto& path = *static_cast<Path*> (data);
        path.cubicTo ({ ctlX1, ctlY1 }, { ctlX2, ctlY2 }, { toX, toY });
    }, nullptr, nullptr);
    hb_draw_funcs_set_close_path_func (funcs.get(), [] (auto*, uk data, auto*, auto*)
    {
        auto& path = *static_cast<Path*> (data);
        path.closeSubPath();
    }, nullptr, nullptr);
...
z0 Typeface::getOutlineForGlyph (TypefaceMetricsKind kind, i32 glyphNumber, Path& path) const
{
    ...
    const auto scale = factor / (f32) hb_face_get_upem (hb_font_get_face (font));

    // getTypefaceGlyph returns glyphs in em space, getOutlineForGlyph returns glyphs in "special DRX units" space
...
Rectangle<f32> Typeface::getGlyphBounds (TypefaceMetricsKind kind, i32 glyphNumber) const
{
    ...
    return Rectangle { (f32) extents.width, (f32) extents.height }
            .withPosition ((f32) extents.x_bearing, (f32) extents.y_bearing)
            .transformedBy (AffineTransform::scale (scale).scaled (1.0f, -1.0f));
}

z0 Typeface::applyVerticalHintingTransform (f32, Path&)
{
    jassertfalse;
}

EdgeTable* Typeface::getEdgeTableForGlyph (TypefaceMetricsKind kind, i32 glyphNumber, const AffineTransform& transform, f32)
{
    Path path;
...
static Color makeColor (hb_color_t c)
{
...
static std::vector<GlyphLayer> getCOLRv0Layers (const Typeface& typeface, i32 glyphNumber, const AffineTransform& transform)
{
    ...
    {
        const auto hbFillColor = layer.color_index == 0xffff ? std::optional<hb_color_t>() : [&]
        {
            hb_color_t colour{};
            u32 numColors = 1;
            hb_ot_color_palette_get_colors (face, palette, layer.color_index, &numColors, &colour);
            return colour;
        }();

        const auto juceFillColor = hbFillColor.has_value() ? makeColor (*hbFillColor) : std::optional<Color>();

        auto path = getGlyphPathInGlyphUnits (layer.glyph, font);
        path.applyTransform (transform);
        result.push_back ({ ColorLayer
        {
...
static std::vector<GlyphLayer> getBitmapLayer (const Typeface& typeface, i32 glyphNumber, const AffineTransform& t)
{
    if ((typeface.getColorGlyphFormats() & Typeface::colourGlyphFormatBitmap) == 0)
        return {};
    ...
    u32 imageDataSize{};
    tukk imageData = hb_blob_get_data (blob.get(), &imageDataSize);
    ...
    const auto transform = AffineTransform::scale ((f32) extents.width  / (f32) wDenom,
                                                   (f32) extents.height / (f32) hDenom)
            .translated ((f32) extents.x_bearing,
                         (f32) extents.y_bearing)
            .followedBy (t);
    return { GlyphLayer { ImageLayer { juceImage, transform } } };
}

std::vector<GlyphLayer> Typeface::getLayersForGlyph (TypefaceMetricsKind kind, i32 glyphNumber, const AffineTransform& transform, f32) const
{
    auto* font = getNativeDetails().getFont();
    ...
    // Instead of calling through to the 'paint' API, which DRX can't easily support due to complex
    ...
    return { GlyphLayer { ColorLayer { EdgeTable { path.getBounds().getSmallestIntegerContainer().expanded (1, 0), path, {} }, {} } } };
}

i32 Typeface::getColorGlyphFormats() const
{
...
Typeface::Ptr Typeface::createSystemTypefaceFor (ukk fontFileData, size_t fontFileDataSize)
{
    return createSystemTypefaceFor (Span (static_cast<const std::byte*> (fontFileData), fontFileDataSize));
}

//==============================================================================
std::optional<u32> Typeface::getNominalGlyphForCodepoint (t32 cp) const
{
    auto* font = getNativeDetails().getFont();
...
static constexpr auto hbTag (const t8 (&arr)[5])
{
    return HB_TAG (arr[0], arr[1], arr[2], arr[3]);
}

template <typename Consumer>
static f32 doSimpleShapeWithNoBreaks (const Typeface& typeface,
                                        ...
                                        Consumer&& consumer)
{
    ...
    hb_shape (font, buffer.get(), features.data(), (u32) features.size());
    ...
    for (auto i = decltype (numGlyphs){}; i < numGlyphs; ++i)
    {
        ...
        consumer (info.codepoint, HbScale::hbToDrx (cursor.x + position.x_offset));
        cursor += Point { position.x_advance, position.y_advance };
    }

    return HbScale::hbToDrx (cursor.x);
}

template <typename Consumer>
static f32 doSimpleShape (const Typeface& typeface,
                            TypefaceMetricsKind kind,
                            ...
                            Consumer&& consumer)
{
    const t32 zeroWidthSpace = 0x200b;
    ...
    for (auto iter = text.begin(), end = text.end(); iter != end;)
    {
        ...
        lastX += doSimpleShapeWithNoBreaks (typeface, kind, Txt (iter, next), height, horizontalScale, [&] (auto codepoint, auto x)
        {
...
f32 Typeface::getStringWidth (TypefaceMetricsKind kind, const Txt& text, f32 height, f32 horizontalScale)
{
    return doSimpleShape (*this, kind, text, height, horizontalScale, [&] (auto, auto) {});
}

z0 Typeface::getGlyphPositions (TypefaceMetricsKind kind,
                                  const Txt& text,
                                  Array<i32>& glyphs,
                                  Array<f32>& xOffsets,
                                  f32 height,
...
public:
    ...
    z0 runTest() override
    {
        ...
        beginTest ("Metrics regression");
        {
           ...
           #undef DRX_TEST_TYPEFACES

            static constexpr auto generate = false;

            if constexpr (generate)
            {
                DBG ("Copy the following lines into the TypefaceTests::inputs array in drx_Typeface.cpp");
            ...
            else
            {
                ...
                std::map<std::tuple<Txt, i32>, MetricsRecord> uniqueRecords;
    ...
    static std::set<Txt> getFontFamilyNamesAsSet()
    {
        std::set<Txt> result;
    ...
    static Typeface::Ptr loadTypeface (Span<u8k> data)
    {
    ...
    static std::vector<GlyphAdvance> getGlyphPositions (Typeface::Ptr typeface, StringRef text)
    {
        ...
        for (const auto [index, glyph] : enumerate (glyphs, i32{}))
    ...
    static b8 glyphsEqual (const GlyphAdvance& a, const GlyphAdvance& b)
    {
