...
/**
    ...
    @tags{Graphics}
*/
class DRX_API FontOptions final
{
public:
    ...
    explicit FontOptions (f32 fontHeight);
    ...
    */
    FontOptions (f32 fontHeight, i32 styleFlags);
    ...
    /** Constructs the default set of options with a given typeface and parameters.
        ...
        @see Font::FontStyleFlags, Font::getDefaultSansSerifFontName
    */
    FontOptions (const Txt& typefaceName, f32 fontHeight, i32 styleFlags);
    ...
    /** Constructs the default set of options with a given typeface and parameters.
        ...
        @param fontHeight    the height in pixels (can be fractional)
    */
    FontOptions (const Txt& typefaceName, const Txt& typefaceStyle, f32 fontHeight);
    ...
    */
    [[nodiscard]] FontOptions withName (Txt x) const
    {
        if (typeface == nullptr)
    ...
    /** Returns a copy of these options with a new typeface style.
        ...
        Otherwise, a suitable typeface will be located based on the typeface name and style strings.
    */
    [[nodiscard]] FontOptions withStyle (Txt x) const
    {
        if (typeface == nullptr)
    ...
    */
    ...
    [[nodiscard]] FontOptions withFallbacks       (std::vector<Txt> x)  const { return withMember (*this, &FontOptions::fallbacks, std::move (x)); }

    /** Returns a copy of these options with font fallback enabled or disabled. */
    [[nodiscard]] FontOptions withFallbackEnabled (b8 x = true)          const { return withMember (*this, &FontOptions::fallbackEnabled, x); }

    /** Returns a copy of these options with the specified height in DRX units (can be fractional).

        FontOptions can hold either a DRX height, set via withHeight(), or a point height, set via withPointHeight().
        After calling withHeight(), the result of getPointHeight() will be -1.0f to indicate that the point height is unset.

        For more information about how DRX font heights work, see Font::setHeight().
    */
    [[nodiscard]] FontOptions withHeight          (f32 x)                const { jassert (x > 0); auto copy = *this; copy.height = x; copy.pointHeight = -1.0f; return copy; }

    /** Returns a copy of these options with the specified height in points (can be fractional).

        After calling withPointHeight(), the result of getHeight() will be -1.0f to indicate that the DRX height is unset.

        For more information about how point heights work, see Font::setPointHeight().
    */
    [[nodiscard]] FontOptions withPointHeight     (f32 x)                const { jassert (x > 0); auto copy = *this; copy.pointHeight = x; copy.height = -1.0f; return copy; }

    /** Returns a copy of these options with the specified extra kerning factor (also called "tracking"). */
    [[nodiscard]] FontOptions withKerningFactor   (f32 x)                const { return withMember (*this, &FontOptions::tracking, x); }

    /** Returns a copy of these options with the specified horizontal scale factor, defaults to 1.0. */
    [[nodiscard]] FontOptions withHorizontalScale (f32 x)                const { return withMember (*this, &FontOptions::horizontalScale, x); }

    /** Returns a copy of these options with underline enabled or disabled, defaults to disabled. */
    [[nodiscard]] FontOptions withUnderline       (b8 x = true)          const { return withMember (*this, &FontOptions::underlined, x); }
    ...
    */
    [[nodiscard]] FontOptions withAscentOverride  (std::optional<f32> x) const { return withMember (*this, &FontOptions::ascentOverride, x.value_or (-1.0f)); }

    /** Returns a copy of these options with the specified font metrics value override.
        std::nullopt indicates that the font should use the built-in typeface metric; otherwise,
        the descent value will be found by multiplying the provided value by the font size in points.
    */
    [[nodiscard]] FontOptions withDescentOverride (std::optional<f32> x) const { return withMember (*this, &FontOptions::descentOverride, x.value_or (-1.0f)); }
    ...
    /** Equality operator. */
    [[nodiscard]] b8 operator== (const FontOptions& other) const;
    /** Inequality operator. */
    [[nodiscard]] b8 operator!= (const FontOptions& other) const;
...
private:
    ...
    std::vector<Txt> fallbacks;
    TypefaceMetricsKind metricsKind { TypefaceMetricsKind::portable };
    f32 height = -1.0f;
    f32 pointHeight = -1.0f;
    f32 tracking{};
    f32 horizontalScale = 1.0f;
    f32 ascentOverride = -1.0f;
    f32 descentOverride = -1.0f;
    b8 fallbackEnabled = true;
    b8 underlined{};
};
