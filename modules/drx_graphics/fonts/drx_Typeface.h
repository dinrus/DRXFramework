...
{
    EdgeTable clip;
    std::optional<Color> colour; ///< nullopt indicates 'foreground'
};
...
struct GlyphLayer
{
    std::variant<ColorLayer, ImageLayer> layer;
};
...
/** Identifiers for different styles of typeface metrics.
    ...
    DRX components that display text will query LookAndFeel::getDefaultMetricsKind() to find the
...
enum class TypefaceMetricsKind
{
    legacy,         ///< Old-style metrics that may differ for the same font file when running on different platforms.
                    ///< This was the default behaviour prior to DRX 8.
    portable        ///< Where possible, will return the same font metrics on all platforms.
                    ///< It's a good idea to use this for new DRX projects, to keep text layout and
                    ///< sizing consistent on all platforms.
};

/** Font metrics using DRX conventions.
...
struct TypefaceMetrics
{
    ...
    */
    ...
    /** The factor by which a DRX font height should be multiplied in order to convert to a font
        size in points.
...
/**
    ...
    @tags{Graphics}
*/
class DRX_API  Typeface  : public ReferenceCountedObject
{
public:
    Typeface (const Txt& name, const Txt& newStyle) noexcept;
    ...
    */
    const Txt& getStyle() const noexcept            { return style; }
    ...
    /** Attempts to create a font from some raw font file data (e.g. a TTF or OTF file image).
        ...
        The typeface will remain registered with the system for as i64 as there is at least one
    ...
    */
    static Ptr createSystemTypefaceFor (ukk fontFileData, size_t fontFileDataSize);
    ...
    */
    ...
    /** @deprecated
        ...
        - The height parameter is specified in DRX units rather than in points.
    ...
    */
    f32 getStringWidth (TypefaceMetricsKind,
                          const Txt& text,
                          f32 normalisedHeight = 1.0f,
    ...
    /** @deprecated
        ...
        You should never need to call this!
    */
    z0 getGlyphPositions (TypefaceMetricsKind,
                            const Txt& text,
                            Array<i32>& glyphs,
                            Array<f32>& xOffsets,
                            f32 normalisedHeight = 1.0f,
    ...
    */
    z0 getOutlineForGlyph (TypefaceMetricsKind, i32 glyphNumber, Path& path) const;

    /** Returns glyph bounds, normalised to a font height of 1.0. */
    Rectangle<f32> getGlyphBounds (TypefaceMetricsKind, i32 glyphNumber) const;
    ...
    /** @deprecated
        ...
        The height is specified in DRX font-height units.
    ...
    */
    ...
    /** Returns the layers that should be painted in order to display this glyph.
        ...
        would require significant additions to DRX's rendering code, so it has been omitted for
        now.
    ...
    */
    ...
    /** Kinds of colour glyph format that may be implemented by a particular typeface.
        ...
        At this time, DRX is able to render only bitmap and COLRv0 fonts.
    ...
    */
    ...
    /** Returns an i32 with bits set indicating the format of colour glyphs contained in the typeface.
        ...
        operation with the members of the ColorGlyphFormat enum to determine whether a particular
        format is supported.

        @code
        const auto isMonochrome   = typeface->getColorGlyphFormats() == 0;
        const auto isSvg          = (typeface->getColorGlyphFormats() & Typeface::colourGlyphFormatSvg) != 0;
        const auto isSimpleColor = (typeface->getColorGlyphFormats() & (Typeface::colourGlyphFormatBitmap | Typeface::colourGlyphFormatCOLRv0)) != 0;
        @endcode
    */
    i32 getColorGlyphFormats() const;

    //==============================================================================
    /** Changes the number of fonts that are cached in memory. */
    static z0 setTypefaceCacheSize (i32 numFontsToCache);

    /** Clears any fonts that are currently cached in memory. */
    static z0 clearTypefaceCache();
    ...
    */
    static z0 scanFolderForFonts (const File& folder);
    ...
    */
    [[deprecated]]
    z0 applyVerticalHintingTransform (f32 fontHeight, Path& path);

    /** Returns the glyph index corresponding to the provided codepoint, or nullopt if no
        such glyph is found.
    */
    std::optional<u32> getNominalGlyphForCodepoint (t32) const;
    ...
    */
    virtual Typeface::Ptr createSystemFallback (const Txt& text, const Txt& language) const = 0;
    ...
    /** Returns the system's default UI font.
        ...
        the sans-serif font that DRX would normally select to be the default font. This is
        ...
        but for Verdana (the DRX default sans-serif font on Windows) the sum of ascender and
        descender is closer to the em-size. When the size of a font is set via
        FontOptions::withHeight() or Font::setHeight(), DRX will scale fonts based on the sum of
...
private:
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (Typeface)
};
