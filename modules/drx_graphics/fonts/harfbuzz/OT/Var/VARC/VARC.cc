...
static inline z0 free_static_transforming_pen_funcs ();
...
static inline
z0 free_static_transforming_pen_funcs ()
{
...
{
  u8k *end = total_record.arrayZ + total_record.length;
  u8k *record = total_record.arrayZ;
...
#define READ_UINT32VAR(name) \
  HB_STMT_START { \
    if (unlikely (u32 (end - record) < HBUINT32VAR::min_size)) return hb_ubytes_t (); \
    hb_barrier (); \
    auto &varint = * (const HBUINT32VAR *) record; \
    u32 size = varint.get_size (); \
    if (unlikely (u32 (end - record) < size)) return hb_ubytes_t (); \
    name = (u32) varint; \
    record += size; \
  } HB_STMT_END
  ...
  if (flags & (u32) flags_t::GID_IS_24BIT)
  {
    if (unlikely (u32 (end - record) < HBGlyphID24::static_size))
      return hb_ubytes_t ();
    hb_barrier ();
  ...
  {
    if (unlikely (u32 (end - record) < HBGlyphID16::static_size))
      return hb_ubytes_t ();
    hb_barrier ();
  ...
  // Condition
  b8 show = true;
  if (flags & (u32) flags_t::HAVE_CONDITION)
  {
    u32 conditionIndex;
  ...
  hb_vector_t<u32> axisIndices;
  hb_vector_t<f32> axisValues;
  if (flags & (u32) flags_t::HAVE_AXES)
  {
    u32 axisIndicesIndex;
  ...
  if (flags & (u32) flags_t::AXIS_VALUES_HAVE_VARIATION)
  {
    u32 axisValuesVarIdx;
  ...
  if ((flags & (u32) flags_t::RESET_UNSPECIFIED_AXES) ||
      coords.length > HB_VAR_COMPOSITE_MAX_AXES)
    component_coords = hb_array<i32> (font->coords, font->num_coords);

  // Transform

  u32 transformVarIdx = VarIdx::NO_VARIATION;
  if (flags & (u32) flags_t::TRANSFORM_HAS_VARIATION)
...
#define PROCESS_TRANSFORM_COMPONENT(type, flag, name) \
	if (flags & (u32) flags_t::flag) \
	{ \
	  static_assert (type::static_size == HBINT16::static_size, ""); \
	  if (unlikely (u32 (end - record) < HBINT16::static_size)) \
	    return hb_ubytes_t (); \
	  hb_barrier (); \
...
#undef PROCESS_TRANSFORM_COMPONENT
  ...
  u32 i = flags & (u32) flags_t::RESERVED_MASK;
  while (i)
  {
    HB_UNUSED u32 discard;
  ...
  if (show)
  {
    ...
    coord_setter_t coord_setter (axisIndices ? component_coords : hb_array<i32> ());
    // Go backwards, to reduce coord_setter vector reallocations.
    for (u32 i = axisIndices.length; i; i--)
    ...
    {
      f32 transformValues[9];
      u32 numTransformValues = 0;
#define PROCESS_TRANSFORM_COMPONENT(type, flag, name) \
	  if (flags & (u32) flags_t::flag) \
...
#define PROCESS_TRANSFORM_COMPONENT(type, flag, name) \
	  ...
	  { \
	    ...
	    f32 float_v = (f32) typed_v; \
	    transform.name = float_v; \
	  }
    PROCESS_TRANSFORM_COMPONENTS;
#undef PROCESS_TRANSFORM_COMPONENT

    if (!(flags & (u32) flags_t::HAVE_SCALE_Y))
      transform.scaleY = transform.scaleX;

    // Scale the transform by the font's scale
    f32 x_scale = font->x_multf;
    f32 y_scale = font->y_multf;
