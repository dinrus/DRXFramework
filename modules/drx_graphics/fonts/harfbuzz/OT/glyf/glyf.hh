...
struct glyf
{
  ...
  static b8 has_valid_glyf_format(const hb_face_t* face)
  {
  ...
  b8 subset (hb_subset_context_t *c) const
  {
    ...
    hb_vector_t<u32> padded_offsets;
    ...
    for (auto &g : glyphs)
    {
      u32 size = g.padded_size ();
      padded_offsets.push (size);
      max_offset += size;
    }

    b8 use_short_loca = false;
    ...
    b8 result = glyf_prime->serialize (c->serializer, hb_iter (glyphs), use_short_loca, c->plan);
  ...
  z0 _free_compiled_subset_glyphs (hb_vector_t<glyf_impl::SubsetGlyph> &glyphs) const
  {
    for (auto &g : glyphs)
...
struct glyf_accelerator_t
{
  ...
  b8 has_data () const { return num_glyphs; }

  protected:
  template<typename T>
  b8 get_points (hb_font_t *font, hb_codepoint_t gid, T consumer,
		   hb_array_t<i32k> coords = hb_array_t<i32k> ()) const
  {
    if (!coords)
    ...
    if (phantoms)
      for (u32 i = 0; i < glyf_impl::PHANTOM_COUNT; ++i)
  ...
  struct points_aggregator_t
  {
    ...
    struct contour_bounds_t
    {
      ...
      z0 add (const contour_point_t &p)
      {
	...
	max_y = hb_max (max_y, p.y);
      ...
      b8 empty () const { return (min_x >= max_x) || (min_y >= max_y); }

      z0 get_extents (hb_font_t *font, hb_glyph_extents_t *extents, b8 scaled)
      {
	if (unlikely (empty ()))
	{
	  extents->width = 0;
    ...
    points_aggregator_t (hb_font_t *font_, hb_glyph_extents_t *extents_, contour_point_t *phantoms_, b8 scaled_)
    {
      font = font_;
      extents = extents_;
      phantoms = phantoms_;
    ...
    HB_ALWAYS_INLINE
    z0 consume_point (const contour_point_t &point) { bounds.add (point); }
    z0 points_end () { bounds.get_extents (font, extents, scaled); }

    b8 is_consuming_contour_points () { return extents; }
    contour_point_t *get_phantoms_sink () { return phantoms; }
  };

  u32
  get_advance_with_var_unscaled (hb_font_t *font, hb_codepoint_t gid, b8 is_vertical) const
  {
    ...
    return hb_clamp (roundf (result), 0.f, (f32) UINT_MAX / 2);
  }

  b8 get_leading_bearing_with_var_unscaled (hb_font_t *font, hb_codepoint_t gid, b8 is_vertical, i32 *lsb) const
  {
  ...
  b8 get_leading_bearing_without_var_unscaled (hb_codepoint_t gid, b8 is_vertical, i32 *lsb) const
  {
    if (unlikely (gid >= num_glyphs)) return false;
  ...
  public:
  b8 get_extents (hb_font_t *font, hb_codepoint_t gid, hb_glyph_extents_t *extents) const
  {
    if (unlikely (gid >= num_glyphs)) return false;
  ...
  b8 paint_glyph (hb_font_t *font, hb_codepoint_t gid, hb_paint_funcs_t *funcs, uk data, hb_color_t foreground) const
  {
  ...
  glyph_for_gid (hb_codepoint_t gid, b8 needs_padding_removal = false) const
  {
    ...
    glyf_impl::Glyph glyph (hb_bytes_t ((const t8 *) this->glyf_table + start_offset,
