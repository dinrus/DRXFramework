...
struct GlyphHeader
{
  b8 has_data () const { return numberOfContours; }

  template <typename accelerator_t>
  b8 get_extents_without_var_scaled (hb_font_t *font, const accelerator_t &glyf_accelerator,
				       hb_codepoint_t gid, hb_glyph_extents_t *extents) const
  {
    /* Undocumented rasterizer behavior: shift glyph to the left by (lsb - xMin), i.e., xMin = lsb */
    /* extents->x_bearing = hb_min (glyph_header.xMin, glyph_header.xMax); */
    i32 lsb = hb_min (xMin, xMax);
    (z0) glyf_accelerator.hmtx->get_leading_bearing_without_var_unscaled (gid, &lsb);
