...
struct Glyph
{
  ...
  b8 is_composite () const
  { return type == COMPOSITE; }

  b8 get_all_points_without_var (const hb_face_t *face,
                                   contour_point_vector_t &points /* OUT */) const
  {
    ...
    {
      i32 lsb = 0;
      i32 h_delta = face->table.hmtx->get_leading_bearing_without_var_unscaled (gid, &lsb) ?
                    (i32) header->xMin - lsb : 0;
      HB_UNUSED i32 tsb = 0;
      i32 v_orig  = (i32) header->yMax +
#ifndef HB_NO_VERTICAL
                    ((z0) face->table.vmtx->get_leading_bearing_without_var_unscaled (gid, &tsb), tsb)
#else
                    0
#endif
                    ;
      u32 h_adv = face->table.hmtx->get_advance_without_var_unscaled (gid);
      u32 v_adv =
#ifndef HB_NO_VERTICAL
      ...
      phantoms[PHANTOM_RIGHT].x = (i32) h_adv + h_delta;
      phantoms[PHANTOM_TOP].y = v_orig;
      phantoms[PHANTOM_BOTTOM].y = v_orig - (i32) v_adv;
    }
    return true;
  }

  z0 update_mtx (const hb_subset_plan_t *plan,
                   i32 xMin, i32 xMax,
                   i32 yMin, i32 yMax,
                   const contour_point_vector_t &all_points) const
  {
    ...
    u32 len = all_points.length;
    f32 leftSideX = all_points[len - 4].x;
    f32 rightSideX = all_points[len - 3].x;
    f32 topSideY = all_points[len - 2].y;
    f32 bottomSideY = all_points[len - 1].y;

    u32 hash = hb_hash (new_gid);

    signed hori_aw = roundf (rightSideX - leftSideX);
    if (hori_aw < 0) hori_aw = 0;
    i32 lsb = roundf (xMin - leftSideX);
    plan->hmtx_map.set_with_hash (new_gid, hash, hb_pair ((u32) hori_aw, lsb));
    ...
    i32 tsb = roundf (topSideY - yMax);
    plan->vmtx_map.set_with_hash (new_gid, hash, hb_pair ((u32) vert_aw, tsb));
  }

  b8 compile_header_bytes (const hb_subset_plan_t *plan,
                             ...
                             hb_bytes_t &dest_bytes /* OUT */) const
  {
    ...
    if (all_points.length > 4)
    {
      ...
      u32 count = all_points.length - 4;
      for (u32 i = 1; i < count; i++)
      {
	f32 x = all_points[i].x;
	f32 y = all_points[i].y;
	xMin = hb_min (xMin, x);
	xMax = hb_max (xMax, x);
	yMin = hb_min (yMin, y);
	yMax = hb_max (yMax, y);
    ...
    i32 rounded_xMin = hb_clamp (roundf (xMin), -32768.0f, 32767.0f);
    i32 rounded_xMax = hb_clamp (roundf (xMax), -32768.0f, 32767.0f);
    i32 rounded_yMin = hb_clamp (roundf (yMin), -32768.0f, 32767.0f);
    i32 rounded_yMax = hb_clamp (roundf (yMax), -32768.0f, 32767.0f);
    ...
    dest_bytes = hb_bytes_t ((const t8 *)glyph_header, GlyphHeader::static_size);
    return true;
  }
                                  ...
                                  hb_bytes_t &dest_end /* OUT */)
  {
    ...
    u32 composite_contours = 0;
    head_maxp_info_t *head_maxp_info_p = &plan->head_maxp_info;
    u32 *composite_contours_p = &composite_contours;
  ...
  b8 get_points (hb_font_t *font, const accelerator_t &glyf_accelerator,
		   contour_point_vector_t &all_points /* OUT */,
		   contour_point_vector_t *points_with_deltas = nullptr, /* OUT */
		   head_maxp_info_t * head_maxp_info = nullptr, /* OUT */
		   u32 *composite_contours = nullptr, /* OUT */
		   b8 shift_points_hori = true,
		   b8 use_my_metrics = true,
		   b8 phantom_only = false,
		   hb_array_t<i32k> coords = hb_array_t<i32k> (),
		   hb_map_t *current_glyphs = nullptr,
		   u32 depth = 0,
		   u32 *edge_count = nullptr) const
  {
    if (unlikely (depth > HB_MAX_NESTING_LEVEL)) return false;
    u32 stack_edge_count = 0;
    ...
    case SIMPLE:
      if (depth == 0 && head_maxp_info)
        head_maxp_info->maxContours = hb_max (head_maxp_info->maxContours, (u32) header->numberOfContours);
      if (depth > 0 && composite_contours)
        *composite_contours += (u32) header->numberOfContours;
    ...
    {
      i32 lsb = 0;
      i32 h_delta = glyf_accelerator.hmtx->get_leading_bearing_without_var_unscaled (gid, &lsb) ?
		    (i32) header->xMin - lsb : 0;
      HB_UNUSED i32 tsb = 0;
      i32 v_orig  = (i32) header->yMax +
#ifndef HB_NO_VERTICAL
		    ((z0) glyf_accelerator.vmtx->get_leading_bearing_without_var_unscaled (gid, &tsb), tsb)
#else
		    0
#endif
		    ;
      u32 h_adv = glyf_accelerator.hmtx->get_advance_without_var_unscaled (gid);
      u32 v_adv =
#ifndef HB_NO_VERTICAL
	...
	u32 old_count = all_points.length;
  ...
  b8 get_extents_without_var_scaled (hb_font_t *font, const glyf_accelerator_t &glyf_accelerator,
  ...
  Glyph (hb_bytes_t bytes_,
	 hb_codepoint_t gid_ = (u32) -1) : bytes (bytes_),
                                                header (bytes.as<GlyphHeader> ()),
                                                gid (gid_)
  {
