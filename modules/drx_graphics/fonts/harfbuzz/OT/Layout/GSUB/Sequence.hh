...
struct Sequence
{
  ...
  b8 apply (hb_ot_apply_context_t *c) const
  {
    ...
    u32 klass = _hb_glyph_info_is_ligature (&c->buffer->cur()) ?
                         HB_OT_LAYOUT_GLYPH_PROPS_BASE_GLYPH : 0;
    u32 lig_id = _hb_glyph_info_get_lig_id (&c->buffer->cur());
    ...
    if (HB_BUFFER_MESSAGE_MORE && c->buffer->messaging ())
    {
      ...
      for (u32 i = c->buffer->idx - count; i < c->buffer->idx; i++)
      {
	if (buf < p)
	  *p++ = ',';
