...
struct PairPosFormat1_3
{
  ...
  b8 sanitize (hb_sanitize_context_t *c) const
  {
    ...
    u32 len1 = valueFormat[0].get_len ();
    u32 len2 = valueFormat[1].get_len ();
  ...
  b8 intersects (const hb_set_t *glyphs) const
  {
    ...
    if (pairSet.len > glyphs->get_population () * hb_bit_storage ((u32) pairSet.len) / 4)
    {
      for (hb_codepoint_t g : glyphs->iter())
      {
	u32 i = cov.get_coverage (g);
  ...
  b8 subset (hb_subset_context_t *c) const
  {
    ...
    hb_pair_t<u32, u32> newFormats = hb_pair (valueFormat[0], valueFormat[1]);
    ...
    | hb_filter ([this, c, out] (const typename Types::template OffsetTo<PairSet>& _)
                 {
                   ...
                   b8 ret = o->serialize_subset (c, _, this, valueFormat, out->valueFormat);
                   if (!ret)
                   {
                     out->pairSet.pop ();
  ...
  hb_pair_t<u32, u32> compute_effective_value_formats (const hb_set_t& glyphset,
                                                                 b8 strip_hints, b8 strip_empty,
                                                                 const hb_hashmap_t<u32, hb_pair_t<u32, i32>> *varidx_delta_map = nullptr) const
  {
    u32 record_size = PairSet::get_size (valueFormat);
