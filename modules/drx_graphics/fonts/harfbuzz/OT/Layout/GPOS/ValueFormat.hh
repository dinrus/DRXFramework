...
/* All fields are options.  Only those available advance the value pointer. */
  ...
  IntType& operator = (u16 i) { v = i; return *this; }

  u32 get_len () const  { return hb_popcount ((u32) *this); }
  u32 get_size () const { return get_len () * Value::static_size; }

  hb_vector_t<u32> get_device_table_indices () const {
    u32 i = 0;
    hb_vector_t<u32> result;
    u32 format = *this;
  ...
  b8 apply_value (hb_ot_apply_context_t *c,
                    ...
                    hb_glyph_position_t   &glyph_pos) const
  {
    ...
    b8 use_x_device = font->x_ppem || font->num_coords;
    b8 use_y_device = font->y_ppem || font->num_coords;
  ...
  u32 get_effective_format (const Value *values, b8 strip_hints, b8 strip_empty, const ValueBase *base,
                                     const hb_hashmap_t<u32, hb_pair_t<u32, i32>> *varidx_delta_map) const
  {
    u32 format = *this;
    for (u32 flag = xPlacement; flag <= yAdvDevice; flag = flag << 1) {
      if (format & flag)
      {
  ...
  u32 get_effective_format (Iterator it, b8 strip_hints, b8 strip_empty, const ValueBase *base,
                                     const hb_hashmap_t<u32, hb_pair_t<u32, i32>> *varidx_delta_map) const {
    u32 new_format = 0;
  ...
  z0 copy_values (hb_serialize_context_t *c,
                    u32 new_format,
                    const ValueBase *base,
                    const Value *values,
  ...
  private:
  b8 sanitize_value_devices (hb_sanitize_context_t *c, const ValueBase *base, const Value *values) const
  {
    u32 format = *this;
  ...
  z0 add_delta_to_value (HBINT16 *value,
                           const ValueBase *base,
                           const Value *src_value,
                           const hb_hashmap_t<u32, hb_pair_t<u32, i32>> *layout_variation_idx_delta_map) const
  {
    if (!value) return;
    u32 varidx = (base + get_device (src_value)).get_variation_index ();
    hb_pair_t<u32, i32> *varidx_delta;
    if (!layout_variation_idx_delta_map->has (varidx, &varidx_delta)) return;

    *value += hb_second (*varidx_delta);
  }

  b8 copy_device (hb_serialize_context_t *c,
                    const ValueBase *base,
                    const Value *src_value,
                    const hb_hashmap_t<u32, hb_pair_t<u32, i32>> *layout_variation_idx_delta_map,
                    u32 new_format, Flags flag) const
  {
    // Filter by new format.
  ...
  static inline const HBINT16& get_short (const Value* value, b8 *worked=nullptr)
  {
    if (worked) *worked |= b8 (*value);
  ...
  b8 sanitize_value (hb_sanitize_context_t *c, const ValueBase *base, const Value *values) const
  {
    TRACE_SANITIZE (this);
  ...
  b8 sanitize_values (hb_sanitize_context_t *c, const ValueBase *base, const Value *values, u32 count) const
  {
    TRACE_SANITIZE (this);
    u32 size = get_size ();
  ...
  b8 sanitize_values_stride_unsafe (hb_sanitize_context_t *c, const ValueBase *base, const Value *values, u32 count, u32 stride) const
  {
    TRACE_SANITIZE (this);
 ...
 private:
  ...
  z0 should_drop (Value value, Flags flag, u32* format) const
  {
    if (value) return;
    *format = *format & ~flag;
  }

  z0 update_var_flag (const Value* value, Flags flag,
                        u32* format, const ValueBase *base,
                        const hb_hashmap_t<u32, hb_pair_t<u32, i32>> *varidx_delta_map) const
  {
    if (*value)
    {
      u32 varidx = (base + get_device (value)).get_variation_index ();
      hb_pair_t<u32, i32> *varidx_delta;
