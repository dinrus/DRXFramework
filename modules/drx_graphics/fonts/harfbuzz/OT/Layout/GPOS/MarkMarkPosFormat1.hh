...
struct MarkMarkPosFormat1_2
{
  ...
  b8 sanitize (hb_sanitize_context_t *c) const
  {
    ...
    return_trace (c->check_struct (this) &&
		  ...
		  hb_barrier () &&
                  mark2Array.sanitize (c, this, (u32) classCount));
  }
  ...
  z0 collect_variation_indices (hb_collect_variation_indices_context_t *c) const
  {
    ...
    u32 mark2_count = (this+mark2Array).rows;
    auto mark2_iter =
    ...
    hb_sorted_vector_t<u32> mark2_indexes;
    for (const u32 row : mark2_iter)
    {
      + hb_range ((u32) classCount)
      | hb_filter (klass_mapping)
      | hb_map ([&] (const u32 col) { return row * (u32) classCount + col; })
  ...
  b8 apply (hb_ot_apply_context_t *c) const
  {
    ...
    u32 mark1_index = (this+mark1Coverage).get_coverage  (buffer->cur().codepoint);
    if (likely (mark1_index == NOT_COVERED)) return_trace (false);

    /* now we search backwards for a suitable mark glyph until a non-mark glyph */
    hb_ot_apply_context_t::skipping_iterator_t &skippy_iter = c->iter_input;
    skippy_iter.reset_fast (buffer->idx);
    skippy_iter.set_lookup_props (c->lookup_props & ~(u32)LookupFlag::IgnoreFlags);
    u32 unsafe_from;
    ...
    u32 j = skippy_iter.idx;

    u32 id1 = _hb_glyph_info_get_lig_id (&buffer->cur());
    u32 id2 = _hb_glyph_info_get_lig_id (&buffer->info[j]);
    u32 comp1 = _hb_glyph_info_get_lig_comp (&buffer->cur());
    u32 comp2 = _hb_glyph_info_get_lig_comp (&buffer->info[j]);
    ...
    good:
    u32 mark2_index = (this+mark2Coverage).get_coverage  (buffer->info[j].codepoint);
  ...
  b8 subset (hb_subset_context_t *c) const
  {
    ...
    u32 mark2count = (this+mark2Array).rows;
    auto mark2_iter =
