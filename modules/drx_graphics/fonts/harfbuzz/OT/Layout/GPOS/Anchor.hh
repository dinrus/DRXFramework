...
struct Anchor
{
  ...
  b8 subset (hb_subset_context_t *c) const
  {
    TRACE_SUBSET (this);
    switch (u.format) {
    case 1: return_trace (b8 (reinterpret_cast<Anchor *> (u.format1.copy (c->serializer))));
    case 2:
      if (c->plan->flags & HB_SUBSET_FLAGS_NO_HINTING)
      {
        // AnchorFormat 2 just containins extra hinting information, so
        // if hints are being dropped convert to format 1.
        return_trace (b8 (reinterpret_cast<Anchor *> (u.format1.copy (c->serializer))));
      }
      return_trace (b8 (reinterpret_cast<Anchor *> (u.format2.copy (c->serializer))));
