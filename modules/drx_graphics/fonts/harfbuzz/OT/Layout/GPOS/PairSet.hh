...
struct PairSet : ValueBase
{
  ...
  static u32 get_size (u32 len1, u32 len2)
  {
    return Types::HBGlyphID::static_size + Value::static_size * (len1 + len2);
  }
  static u32 get_size (const ValueFormat valueFormats[2])
  {
    u32 len1 = valueFormats[0].get_len ();
    u32 len2 = valueFormats[1].get_len ();
  ...
  b8 sanitize (hb_sanitize_context_t *c, const sanitize_closure_t *closure) const
  {
    TRACE_SANITIZE (this);
  ...
  {
    u32 record_size = get_size (valueFormats);
              ...
              u32 pos) const
  {
    ...
    u32 len1 = valueFormats[0].get_len ();
    u32 len2 = valueFormats[1].get_len ();
    u32 record_size = get_size (len1, len2);
	...
	c->buffer->message (c->font,
      ...
      b8 applied_first = len1 && valueFormats[0].apply_value (c, this, &record->values[0], buffer->cur_pos());
      b8 applied_second = len2 && valueFormats[1].apply_value (c, this, &record->values[len1], buffer->pos[pos]);
