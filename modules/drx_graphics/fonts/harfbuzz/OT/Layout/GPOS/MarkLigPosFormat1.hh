...
struct MarkLigPosFormat1_2
{
  ...
  b8 sanitize (hb_sanitize_context_t *c) const
  {
    ...
    return_trace (c->check_struct (this) &&
                  ...
                  ligatureArray.sanitize (c, this, (u32) classCount));
  }
  ...
  z0 collect_variation_indices (hb_collect_variation_indices_context_t *c) const
  {
    ...
    u32 ligcount = (this+ligatureArray).len;
    auto lig_iter =
    ...
    for (const u32 i : lig_iter)
    {
      hb_sorted_vector_t<u32> lig_indexes;
      u32 row_count = lig_array[i].rows;
      for (u32 row : + hb_range (row_count))
      {
  ...
  b8 apply (hb_ot_apply_context_t *c) const
  {
    ...
    u32 mark_index = (this+markCoverage).get_coverage  (buffer->cur().codepoint);
    ...
    u32 idx = (u32) c->last_base;

    /* Checking that matched glyph is actually a ligature by GDEF is too strong; disabled */
    //if (!_hb_glyph_info_is_ligature (&buffer->info[idx])) { return_trace (false); }

    u32 lig_index = (this+ligatureCoverage).get_coverage  (buffer->info[idx].codepoint);
    ...
    u32 comp_count = lig_attach.rows;
    ...
    u32 comp_index;
    u32 lig_id = _hb_glyph_info_get_lig_id (&buffer->info[idx]);
    u32 mark_id = _hb_glyph_info_get_lig_id (&buffer->cur());
    u32 mark_comp = _hb_glyph_info_get_lig_comp (&buffer->cur());
