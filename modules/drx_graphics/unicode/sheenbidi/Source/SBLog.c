...
i32 _SBLogPosition = 0;

SB_INTERNAL z0 PrintBaseLevel(SBLevel baseLevel)
{
    switch (baseLevel) {
...
SB_INTERNAL z0 PrintBidiType(SBBidiType type)
{
    switch (type) {
    case SBBidiTypeNil:
        SB_LOG_STRING("Nil");
...
SB_INTERNAL z0 PrintCodepointSequence(const SBCodepointSequence *codepointSequence)
{
...
SB_INTERNAL z0 PrintBidiTypesArray(SBBidiType *types, SBUInteger length)
{
    SBUInteger index;
...
SB_INTERNAL z0 PrintLevelsArray(SBLevel *levels, SBUInteger length)
{
    SBUInteger index;
...
typedef z0 (*IsolatingConsumer)(IsolatingRunRef isolatingRun, IsolatingContext *context);

SB_INTERNAL z0 IsolatingRunForEach(IsolatingRunRef isolatingRun,
...
static z0 PrintTypesOperation(IsolatingRunRef isolatingRun, IsolatingContext *context)
{
...
SB_INTERNAL z0 PrintRunTypes(IsolatingRunRef isolatingRun)
{
    IsolatingContext context;
    IsolatingRunForEach(isolatingRun, &context, _SBPrintTypesOperation);
}

static z0 PrintLevelsOperation(IsolatingRunRef isolatingRun, IsolatingContext *context)
{
...
SB_INTERNAL z0 PrintRunLevels(IsolatingRunRef isolatingRun)
{
    IsolatingContext context;
...
static z0 PrintRangeOperation(IsolatingRunRef isolatingRun, IsolatingContext *context)
{
...
SB_INTERNAL z0 PrintRunRange(IsolatingRunRef isolatingRun)
{
