...
public:
    ...
    */
    Image (PixelFormat format, i32 imageWidth, i32 imageHeight, b8 clearImage);
    ...
    */
    Image (PixelFormat format, i32 imageWidth, i32 imageHeight, b8 clearImage, const ImageType& type);
    ...
    */
    ...
    /** Возвращает true, если the two images are referring to the same internal, shared image. */
    b8 operator== (const Image& other) const noexcept     { return image == other.image; }

    /** Возвращает true, если the two images are not referring to the same internal, shared image. */
    b8 operator!= (const Image& other) const noexcept     { return image != other.image; }

    /** Возвращает true, если this image isn't null.
    ...
    */
    ...
    /** Возвращает true, если this image is not valid.
        If you create an Image with the default constructor, it has no size or content, and is null
        until you reassign it to an Image which contains some actual data.
        The isNull() method is the opposite of isValid().
        @see isValid
    */
    b8 isNull() const noexcept { return ! isValid(); }
    ...
    */
    z0 clear (const Rectangle<i32>& area, Color colourToClearTo = Color (0x00000000));
    ...
    */
    Image rescaled (i32 newWidth, i32 newHeight,
    ...
    */
    Image getClippedImage (const Rectangle<i32>& area) const;
    ...
    */
    Color getPixelAt (i32 x, i32 y) const;
    ...
    */
    z0 setPixelAt (i32 x, i32 y, Color colour);
    ...
    /** Changes the opacity of a pixel.
        ...
        @see setPixelAt
    */
    z0 multiplyAlphaAt (i32 x, i32 y, f32 multiplier);
    ...
    */
    z0 multiplyAllAlphas (f32 amountToMultiplyBy);
    ...
    */
    class DRX_API  BitmapData  final
    {
    public:
        enum ReadWriteMode
        {
        ...
        BitmapData (Image& image, i32 x, i32 y, i32 w, i32 h, ReadWriteMode mode);
        BitmapData (const Image& image, Rectangle<i32>, ReadWriteMode mode);
        BitmapData (const Image& image, i32 x, i32 y, i32 w, i32 h);
        ...
        */
        inline u8* getLinePointer (i32 y) const noexcept                 { return data + (size_t) y * (size_t) lineStride; }

        /** Returns a pointer to a pixel in the image.
            The coordinates you give here are not checked, so it's the caller's responsibility to make sure they're
            not out-of-range.
        */
        inline u8* getPixelPointer (i32 x, i32 y) const noexcept         { return data + (size_t) y * (size_t) lineStride + (size_t) x * (size_t) pixelStride; }
        ...
        */
        Color getPixelColor (i32 x, i32 y) const noexcept;

        /** Sets the colour of a given pixel.
            For performance reasons, this won't do any bounds-checking on the coordinates, so it's the caller's
            responsibility to make sure they're within the image's size.
        */
        z0 setPixelColor (i32 x, i32 y, Color colour) const noexcept;

        /** Returns the size of the bitmap. */
        Rectangle<i32> getBounds() const noexcept                           { return Rectangle<i32> (width, height); }
        ...
        /** Attempts to copy the contents of src into this bitmap data.
            ...
            yourself!
        */
        b8 convertFrom (const Image::BitmapData& src);

        u8* data;             /**< The raw pixel data, packed according to the image's pixel format. */
        ...
        i32 lineStride;          /**< The number of bytes between each line. */
        i32 pixelStride;         /**< The number of bytes between each pixel. */
        i32 width, height;
    ...
    private:
        DRX_DECLARE_NON_COPYABLE (BitmapData)
    };

    //==============================================================================
    /** Copies a section of the image to somewhere else within itself. */
    z0 moveImageSection (i32 destX, i32 destY,
                           i32 sourceX, i32 sourceY,
    ...
    */
    z0 createSolidAreaMask (RectangleList<i32>& result, f32 alphaThreshold) const;
...
public:
    ...
    */
    virtual z0 setBackupEnabled (b8) = 0;

    /** @see setBackupEnabled(), backupNow() */
    virtual b8 isBackupEnabled() const = 0;
    ...
    */
    virtual b8 backupNow() = 0;

    /** Возвращает true, если the main-memory copy of the image is out-of-date, false if it's up-to-date.

        @see canBackup()
    */
    virtual b8 needsBackup() const = 0;

    /** Returns if there is an up-to-date copy of this image in graphics memory, or false otherwise.

        @see needsBackup()
    */
    virtual b8 canBackup() const = 0;
...
/**
    ...
    @tags{Graphics}
*/
class DRX_API  ImagePixelData  : public ReferenceCountedObject
{
public:
    using BackupExtensions = ImagePixelDataBackupExtensions;
    using NativeExtensions = ImagePixelDataNativeExtensions;

    ImagePixelData (Image::PixelFormat, i32 width, i32 height);
    ...
    */
    ...
    virtual z0 initialiseBitmapData (Image::BitmapData&, i32 x, i32 y, Image::BitmapData::ReadWriteMode) = 0;
    /** Returns the number of Image objects which are currently referring to the same internal
    ...
    */
    virtual i32 getSharedCount() const noexcept;
    ...
    */
    virtual z0 applyGaussianBlurEffectInArea (Rectangle<i32> bounds, f32 radius);

    /** @see applyGaussianBlurEffectInArea() */
    z0 applyGaussianBlurEffect (f32 radius)
    {
    ...
    /** Applies a native blur effect to this image, if available.
        ...
        Native image types may provide optimised implementations.
    */
    virtual z0 applySingleChannelBoxBlurEffectInArea (Rectangle<i32> bounds, i32 radius);

    /** @see applySingleChannelBoxBlurEffectInArea() */
    z0 applySingleChannelBoxBlurEffect (i32 radius)
    {
    ...
    /** Multiples all alpha-channel values in the image by the specified amount.
        ...
        Native image types may provide optimised implementations.
    */
    virtual z0 multiplyAllAlphasInArea (Rectangle<i32> bounds, f32 amount);

    /** @see multiplyAllAlphasInArea() */
    z0 multiplyAllAlphas (f32 amount)
    {
    ...
    /** Changes all the colours to be shades of grey, based on their current luminosity.
        ...
        Native image types may provide optimised implementations.
    */
    virtual z0 desaturateInArea (Rectangle<i32> bounds);
    ...
    */
    ...
    struct Listener
    {
        ...
        virtual z0 imageDataChanged (ImagePixelData*) = 0;
        virtual z0 imageDataBeingDeleted (ImagePixelData*) = 0;
    };

    ListenerList<Listener> listeners;

    z0 sendDataChangeMessage();

    /** @internal intentionally not callable from user code */
    virtual NativeExtensions getNativeExtensions();

private:
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ImagePixelData)
};
...
public:
    ...
    virtual ImagePixelData::Ptr create (Image::PixelFormat, i32 width, i32 height, b8 shouldClearImage) const = 0;

    /** Must return a unique number to identify this type. */
    virtual i32 getTypeID() const = 0;
...
/**
    ...
    @tags{Graphics}
*/
class DRX_API  SoftwareImageType   : public ImageType
{
public:
    SoftwareImageType();
    ~SoftwareImageType() override;

    ImagePixelData::Ptr create (Image::PixelFormat, i32 width, i32 height, b8 clearImage) const override;
...
/**
    ...
    @tags{Graphics}
*/
class DRX_API  NativeImageType   : public ImageType
{
public:
    NativeImageType();
