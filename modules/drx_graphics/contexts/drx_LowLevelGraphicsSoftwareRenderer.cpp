...
{
    DRX_TRACE_LOG_PAINT_CALL (etw::startGDIImage, getFrameId());
}

LowLevelGraphicsSoftwareRenderer::LowLevelGraphicsSoftwareRenderer (const Image& image, Point<i32> origin,
                                                                    const RectangleList<i32>& initialClip)
    : RenderingHelpers::StackBasedLowLevelGraphicsContext<RenderingHelpers::SoftwareRendererSavedState>
        (new RenderingHelpers::SoftwareRendererSavedState (image, initialClip, origin))
{

    DRX_TRACE_EVENT_INT_RECT_LIST (etw::startGDIFrame, etw::softwareRendererKeyword, getFrameId(), initialClip);
}

LowLevelGraphicsSoftwareRenderer::~LowLevelGraphicsSoftwareRenderer()
{
    DRX_TRACE_LOG_PAINT_CALL (etw::endGDIFrame, getFrameId());
}
