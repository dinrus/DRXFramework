...
{
public:
    using VectorType      = Vector3D<f32>;
    using QuaternionType  = Quaternion<f32>;

    /** Creates a Draggable3DOrientation, initially set up to be aligned along the X axis. */
    Draggable3DOrientation (f32 objectRadius = 0.5f) noexcept
    ...
    Draggable3DOrientation (const Quaternion<f32>& quaternionToUse,
                            f32 objectRadius = 0.5f) noexcept
        : radius (jmax (0.1f, objectRadius)),
    ...
    z0 reset (const VectorType& axis) noexcept
    {
    ...
    */
    z0 setViewport (Rectangle<i32> newArea) noexcept
    {
        area = newArea;
    }
    ...
    /** Sets the size of the rotated object, as a proportion of the viewport's size.
        @see setViewport
    */
    z0 setRadius (f32 newRadius) noexcept
    {
    ...
    */
    ...
    z0 mouseDown (Point<Type> mousePos) noexcept
    {
    ...
    */
    template <typename Type>
    z0 mouseDrag (Point<Type> mousePos) noexcept
    {
    ...
    */
    Matrix3D<f32> getRotationMatrix() const noexcept
    {
...
private:
    ...
    Point<f32> mousePosToProportion (Point<f32> mousePos) const noexcept
    {
        ...
        return { (mousePos.x - (f32) area.getCentreX()) / (f32) scale,
                 ((f32) area.getCentreY() - mousePos.y) / (f32) scale };
    }

    VectorType projectOnSphere (Point<f32> pos) const noexcept
    {
