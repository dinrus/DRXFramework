...
//==============================================================================
// This byte-code is generated from native/java/com/rmsl/drx/DrxOpenGLView.java with min sdk version 16
// See drx_core/native/java/README.txt on how to generate this byte-code.
static u8k javaDrxOpenGLView[] =
{
...
public:
        ...
        : component (comp)
    {
        ...
        surfaceView = GlobalRef (LocalRef<jobject> (env->NewObject (DrxOpenGLViewSurface,
                                                                    DrxOpenGLViewSurface.constructor,
                                                                    getAppContext().get(),
    ...
    ~NativeContext() override
    {
        ...
        if (jobject viewParent = env->CallObjectMethod (surfaceView.get(), DrxOpenGLViewSurface.getParent))
    ...
    //==============================================================================
    z0 swapBuffers() const noexcept           { eglSwapBuffers (display, surface.get()); }
    b8 setSwapInterval (i32)                  { return false; }
    i32 getSwapInterval() const                 { return 0; }

    //==============================================================================
    b8 createdOk() const noexcept             { return hasInitialised; }
    uk getRawContext() const noexcept        { return surfaceView.get(); }
    ...
    z0 updateWindowPosition (Rectangle<i32> bounds)
    {
        if (lastBounds != bounds)
        {
            ...
            env->CallVoidMethod (surfaceView.get(), DrxOpenGLViewSurface.layout,
    ...
    z0 surfaceChanged ([[maybe_unused]] LocalRef<jobject> holder,
                         [[maybe_unused]] i32 format,
                         [[maybe_unused]] i32 width,
                         [[maybe_unused]] i32 height) override
    {
    }

    z0 surfaceCreated (LocalRef<jobject>) override;
    z0 surfaceDestroyed (LocalRef<jobject>) override;
...
private:
    #define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
     ...
     DECLARE_JNI_CLASS_WITH_BYTECODE (DrxOpenGLViewSurface, "com/rmsl/drx/DrxOpenGLView", 16, javaDrxOpenGLView)
    #undef JNI_CLASS_MEMBERS

    //==============================================================================
    static z0 attachedToWindow (JNIEnv* env, NativeContext& t)
    {
        LocalRef<jobject> holder (env->CallObjectMethod (t.surfaceView.get(), DrxOpenGLViewSurface.getHolder));
    ...
    static z0 detachedFromWindow (JNIEnv* env, NativeContext& t)
    {
        if (t.surfaceHolderCallback != nullptr)
        {
            LocalRef<jobject> holder (env->CallObjectMethod (t.surfaceView.get(), DrxOpenGLViewSurface.getHolder));
    ...
    static z0 dispatchDraw (JNIEnv*, NativeContext& t, jobject /*canvas*/)
    {
    ...
    b8 tryChooseConfig (const std::vector<EGLint>& optionalAttribs)
    {
    ...
    //==============================================================================
    b8 initEGLDisplay (const OpenGLPixelFormat& pixelFormat, b8 multisample)
    {
        // already initialised?
    ...
    {
        z0 operator() (ANativeWindow* ptr) const { if (ptr != nullptr) ANativeWindow_release (ptr); }
    };
    ...
    std::unique_ptr<ANativeWindow, NativeWindowReleaser> getNativeWindow() const
    {
        ...
        const LocalRef<jobject> holder (env->CallObjectMethod (surfaceView.get(), DrxOpenGLViewSurface.getHolder));
    ...
    {
        z0 operator() (EGLSurface x) const { if (x != EGL_NO_SURFACE) eglDestroySurface (display, x); }
    };

    struct ContextDestructor
    {
        z0 operator() (EGLContext x) const { if (x != EGL_NO_CONTEXT) eglDestroyContext (display, x); }
    };
