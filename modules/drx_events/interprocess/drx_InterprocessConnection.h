...
/**
    ...
    @tags{Events}
*/
class DRX_API  InterprocessConnection
{
public:
    //==============================================================================
    ...
    */
    InterprocessConnection (b8 callbacksOnMessageThread = true,
                            u32 magicMessageHeaderNumber = 0xf2b49e2c);
    ...
    /** Tries to connect this object to a socket.
        ...
        @param timeOutMillisecs     how i64 to keep trying before giving up
        @returns true if the connection is established successfully
        @see Socket
    */
    b8 connectToSocket (const Txt& hostName,
                          i32 portNumber,
    ...
    */
    b8 connectToPipe (const Txt& pipeName, i32 pipeReceiveMessageTimeoutMs);
    ...
    */
    b8 createPipe (const Txt& pipeName, i32 pipeReceiveMessageTimeoutMs, b8 mustNotExist = false);
    ...
    */
    z0 disconnect (i32 timeoutMs = -1, Notify notify = Notify::yes);
    ...
    */
    Txt getConnectedHostName() const;
    ...
    /** Tries to send a message to the other end of this connection.
        ...
        @see messageReceived
    */
    b8 sendMessage (const MemoryBlock& message);
    ...
    */
    ...
    /** Called when a message arrives.
        ...
        @see sendMessage
    */
    virtual z0 messageReceived (const MemoryBlock& message) = 0;
...
private:
    ...
    b8 callbackConnectionState = false;
    const b8 useMessageThread;
    u32k magicMessageHeader;
    i32 pipeReceiveMessageTimeout = -1;

    friend class InterprocessConnectionServer;
    z0 initialise();
    z0 initialiseWithSocket (std::unique_ptr<StreamingSocket>);
    z0 initialiseWithPipe (std::unique_ptr<NamedPipe>);
    z0 deletePipeAndSocket();
    z0 connectionMadeInt();
    z0 connectionLostInt();
    z0 deliverDataInt (const MemoryBlock&);
    b8 readNextMessage();
    i32 readData (uk, i32);

    struct ConnectionThread;
    std::unique_ptr<ConnectionThread> thread;
    std::atomic<b8> threadIsRunning { false };

    class SafeAction;
    std::shared_ptr<SafeAction> safeAction;

    z0 runThread();
    i32 writeData (uk, i32);

    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (InterprocessConnection)
};
