...
/**
    ...
    add some code to your main() or DRXApplication::initialise() function that
    ...
    The drx demo app has a good example of this class in action.

    @see ChildProcessCoordinator, InterprocessConnection, ChildProcess

    @tags{Events}
*/
class DRX_API  ChildProcessWorker
{
public:
    ...
    */
    ...
    /** This checks some command-line parameters to see whether they were generated by
        ...
        main() or DRXApplication::initialise() that calls this method.
        ...
        The timeoutMs parameter lets you specify how i64 the child process is allowed
        ...
        Возвращает true, если the command-line matches and the connection is made successfully.
    */
    b8 initialiseFromCommandLine (const Txt& commandLine,
    ...
    */
    virtual z0 handleMessageFromCoordinator (const MemoryBlock& mb);

    [[deprecated ("Replaced by handleMessageFromCoordinator.")]]
    virtual z0 handleMessageFromMaster (const MemoryBlock&) {}
    ...
    */
    virtual z0 handleConnectionMade();
    ...
    /** This will be called when the connection to the coordinator process is lost.
        ...
        when this happens.
    */
    virtual z0 handleConnectionLost();
    ...
    */
    b8 sendMessageToCoordinator (const MemoryBlock&);

    [[deprecated ("Replaced by sendMessageToCoordinator.")]]
    b8 sendMessageToMaster (const MemoryBlock& mb) { return sendMessageToCoordinator (mb); }

private:
    struct Connection;
    std::unique_ptr<Connection> connection;

    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ChildProcessWorker)
};
...
/**
    ...
    @tags{Events}
*/
class DRX_API ChildProcessCoordinator
{
public:
    ...
    */
    ...
    /** Attempts to launch and connect to a worker process.
        ...
        start a new one.
    */
    b8 launchWorkerProcess (const File& executableToLaunch,
                              const Txt& commandLineUniqueID,
                              i32 timeoutMs = 0,
                              i32 streamFlags = ChildProcess::wantStdOut | ChildProcess::wantStdErr);

    [[deprecated ("Replaced by launchWorkerProcess.")]]
    b8 launchSlaveProcess (const File& executableToLaunch,
                             const Txt& commandLineUniqueID,
                             i32 timeoutMs = 0,
                             i32 streamFlags = ChildProcess::wantStdOut | ChildProcess::wantStdErr)
    {
    ...
    */
    z0 killWorkerProcess();

    [[deprecated ("Replaced by killWorkerProcess.")]]
    z0 killSlaveProcess() { killWorkerProcess(); }

    /** This will be called to deliver a message from the worker process.
        The call will probably be made on a background thread, so be careful with your thread-safety!
    */
    virtual z0 handleMessageFromWorker (const MemoryBlock&);

    [[deprecated ("Replaced by handleMessageFromWorker")]]
    virtual z0 handleMessageFromSlave (const MemoryBlock&) {}
    ...
    */
    b8 sendMessageToWorker (const MemoryBlock&);

    [[deprecated ("Replaced by sendMessageToWorker.")]]
    b8 sendMessageToSlave (const MemoryBlock& mb) { return sendMessageToWorker (mb); }
...
private:
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ChildProcessCoordinator)
};
