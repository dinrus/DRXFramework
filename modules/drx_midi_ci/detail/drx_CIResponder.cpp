                                                  ...
                                                  Span<ResponderDelegate* const> listeners)
{
    ...
    {
    public:
        Output (BufferOutput& o, Message::Header h, u8 g)
        ...
        u8 getIncomingGroup() const override { return group; }
        std::vector<std::byte>& getOutputBuffer() override { return innerOutput.getOutputBuffer(); }
        z0 send (u8 g) override { innerOutput.send (g); }
    ...
    z0 runTest() override
    {
        ...
        const auto makeOutput = [&]
        {
            struct Output : public BufferOutput
            {
                ...
                z0 send (u8) override { sent.push_back (buf); }
...
private:
    ...
    struct SilentResponderListener : public ResponderDelegate
    {
        ...
        b8 tryRespond (ResponderOutput&, const Message::Parsed& p) override
        {
            parsed = p;
            return false;
        }
        ...
        b8 operator== (const SilentResponderListener& other) const { return parsed == other.parsed; }
        b8 operator!= (const SilentResponderListener& other) const { return ! operator== (other); }
    ...
    {
        z0 operator() (Span<const std::byte> bytes)
        {
