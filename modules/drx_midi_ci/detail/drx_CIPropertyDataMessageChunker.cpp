...
Span<const std::byte> PropertyDataMessageChunker::operator*() const noexcept
{
    ...
    jassert (storage != nullptr && (i32) storage->size() <= chunkSize);
    return *storage;
}
...
i32 PropertyDataMessageChunker::getRoomForBody() const
{
    return chunkSize - (i32) (getHeaderForBlock().size() + 22);
}

b8 PropertyDataMessageChunker::hasRoomForBody() const
{
...
z0 PropertyDataMessageChunker::populateStorage() const
{
    storage->clear();
    storage->resize ((size_t) getRoomForBody());

    // Read body data into buffer
    const auto numBytesRead = (u16) jmax (ssize_t (0), body->read (storage->data(), storage->size()));

    const auto [numChunks, thisChunkNum] = [&]() -> std::tuple<u16, u16>
    {
        ...
        if (totalLength < 0)
            return std::tuple ((u16) 0, thisChunk); // 0 means "unknown number"

        const auto roomForBody = getRoomForBody();

        if (roomForBody != 0)
            return std::tuple ((u16) ((totalLength + roomForBody - 1) / roomForBody), thisChunk);
        ...
        jassertfalse;
        return std::tuple (thisChunk, (u16) 0); // 0 means "data potentially unusable"
    }();
