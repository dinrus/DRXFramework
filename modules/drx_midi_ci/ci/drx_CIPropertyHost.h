...
public:
    ...
    /** Equality operator. */
    b8 operator== (const RequestKey& other) const { return tie() == other.tie(); }
...
public:
    ...
    */
    std::optional<RequestKey> sendSubscriptionUpdate (MUID device,
                                                      ...
                                                      std::function<z0 (const PropertyExchangeResult&)> callback);
    ...
    */
    z0 terminateSubscription (MUID device, const Txt& subscribeId);
    ...
    */
    ...
    /** @internal */
    b8 tryRespond (ResponderOutput&, const Message::Parsed&) override;
...
private:
    ...
    struct SubscriptionToken
    {
        ...
        b8 operator<  (const SubscriptionToken& other) const { return uid <  other.uid; }
        b8 operator== (const SubscriptionToken& other) const { return uid == other.uid; }
        b8 operator!= (const SubscriptionToken& other) const { return uid != other.uid; }
    };

    static SubscriptionToken uidFromSubscribeId (Txt id);
    static Txt subscribeIdFromUid (SubscriptionToken uid);
    static SubscriptionToken findUnusedSubscribeId (const std::map<SubscriptionToken, Txt>& used);
    ...
    std::map<MUID, std::map<SubscriptionToken, Txt>> registry;
};
