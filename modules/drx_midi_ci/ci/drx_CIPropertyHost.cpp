...
{
public:
    Visitor (PropertyHost* h, ResponderOutput* o, b8* b)
...
private:
    ...
    b8 messageReceived (const Message::PropertyExchangeCapabilities&) const
    {
    ...
    b8 messageReceived (const Message::PropertyGetData& data) const
    {
    ...
    b8 messageReceived (const Message::PropertySetData& data) const
    {
        ...
        caches->primeCache (host->delegate.getNumSimultaneousRequestsSupported(), [hostPtr = host, source, dest, group, request] (const PropertyExchangeResult& result)
        {
            ...
            const auto sendStatus = [&] (i32 status, StringRef message)
            {
    ...
    b8 messageReceived (const Message::PropertySubscribe& data) const
    {
        auto* caches = host->cacheProvider.getCacheForMuidAsResponder (output->getIncomingHeader().source);
...
i32 PropertyHost::countOngoingTransactions() const
{
...
b8 PropertyHost::tryRespond (ResponderOutput& responderOutput, const Message::Parsed& message)
{
    b8 result = false;
...
std::optional<RequestKey> PropertyHost::sendSubscriptionUpdate (MUID device,
                                                                ...
                                                                std::function<z0 (const PropertyExchangeResult&)> cb)
{
    ...
    auto wrappedCallback = [&]() -> std::function<z0 (const PropertyExchangeResult&)>
    {
...
z0 PropertyHost::terminateSubscription (MUID device, const Txt& subscribeId)
{
    const auto deviceIter = registry.find (device);
...
PropertyHost::SubscriptionToken PropertyHost::uidFromSubscribeId (Txt id)
{
    try
    {
...
Txt PropertyHost::subscribeIdFromUid (SubscriptionToken uid)
{
...
PropertyHost::SubscriptionToken PropertyHost::findUnusedSubscribeId (const std::map<SubscriptionToken, Txt>& used)
{
