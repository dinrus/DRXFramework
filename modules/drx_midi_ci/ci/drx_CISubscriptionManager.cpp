...
struct RequestRetryQueueEntry
{
    ...
    b8 inFlight = false;   ///< True if the message has been sent and we're waiting for a reply, false otherwise
};
...
private:
    auto getIter (Token64 k)
    {
        const auto iter = std::lower_bound (entries.begin(),
                                            entries.end(),
                                            (zu64) k,
                                            [] (const Entry& e, zu64 v) { return (zu64) e.key < v; });
...
struct SubscriptionState
{
    ...
    std::variant<std::monostate, Token64, Txt> state;
    Txt resource;
};
...
public:
    ...
    std::vector<Token64> endSubscription (Txt subscribeId)
    {
        std::vector<Token64> ended;

        for (auto it = stateForSubscription.begin(); it != stateForSubscription.end();)
        {
            if (const auto* id = std::get_if<Txt> (&it->second.state))
            {
                if (*id == subscribeId)
                {
    ...
    z0 setRequestIdForKey (Token64 key, Token64 request)
    {
        const auto iter = stateForSubscription.find (key);
    ...
    z0 setSubscribeIdForKey (Token64 key, Txt subscribeId)
    {
        const auto iter = stateForSubscription.find (key);
...
public:
    ...
    z0 endSubscription (SubscriptionKey key)
    {
        ...
        else if (auto* subscribeId = std::get_if<Txt> (&ended->state))
        {
    ...
    z0 endSubscriptionFromResponder (MUID m, Txt sub)
    {
    ...
    z0 endSubscriptionsFromResponder (MUID m)
    {
        const auto iter = infoForMuid.find (m);
...
private:
    z0 handleReply (SubscriptionKey subscriptionKey, PropertySubscriptionCommand command, const PropertyExchangeResult& r)
    {
    ...
    b8 sendPendingMessages (MUID m, DeviceSubscriptionStates& info)
    {
...
z0 SubscriptionManager::endSubscription (SubscriptionKey key)
{
    pimpl->endSubscription (key);
}

z0 SubscriptionManager::endSubscriptionFromResponder (MUID m, Txt sub)
{
    pimpl->endSubscriptionFromResponder (m, sub);
}

z0 SubscriptionManager::endSubscriptionsFromResponder (MUID m)
{
...
std::optional<Txt> SubscriptionManager::getSubscribeIdForKey (SubscriptionKey key) const
{
    if (const auto info = pimpl->getInfoForSubscriptionKey (key))
        if (const auto* subscribeId = std::get_if<Txt> (&info->state))
            return *subscribeId;

    return {};
}

std::optional<Txt> SubscriptionManager::getResourceForKey (SubscriptionKey key) const
{
    if (const auto info = pimpl->getInfoForSubscriptionKey (key))
...
b8 SubscriptionManager::sendPendingMessages()
{
    ...
    z0 runTest() override
    {
        ...
        public:
            std::optional<RequestKey> sendPropertySubscribe (MUID m,
                                                             ...
                                                             std::function<z0 (const PropertyExchangeResult&)> cb) override
            {
                ++sendCount;
            ...
            z0 propertySubscriptionChanged (SubscriptionKey, const std::optional<Txt>&) override
            {
                subChanged = true;
            }

            z0 setSendShouldSucceed (b8 b) { sendShouldSucceed = b; }

            z0 sendResult (RequestKey key, const PropertyExchangeResult& r)
            {
            ...
            zu64 getAndClearSendCount()  { return std::exchange (sendCount,  0); }
            zu64 getAndClearAbortCount() { return std::exchange (abortCount, 0); }
            b8 getAndClearSubChanged()     { return std::exchange (subChanged, false); }

        private:
            std::map<RequestKey, std::function<z0 (const PropertyExchangeResult&)>> callbacks;
            zu64 sendCount = 0, abortCount = 0;
            zu64 lastKey = 0;
            b8 sendShouldSucceed = true, subChanged = false;
        };
