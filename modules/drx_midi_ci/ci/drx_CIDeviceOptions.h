...
public:
    ...
    */
    static std::array<t8, 16> makeProductInstanceId (Random& random)
    {
        std::array<t8, 16> result{};

        for (auto& c : result)
            c = (t8) random.nextInt ({ beginValidAscii, endValidAscii });
    ...
    */
    ...
    [[nodiscard]] DeviceOptions withProductInstanceId (const std::array<t8, 16>& x) const
    {
        const auto null = std::find (x.begin(), x.end(), 0);

        if (! std::all_of (x.begin(), null, [] (t8 c) { return beginValidAscii <= c && c < endValidAscii; }))
        {
...
private:
    ...
    std::array<t8, 16> productInstanceId{};
