...
Txt Encodings::stringFrom7BitText (Span<const std::byte> bytes)
{
    std::vector<CharPointer_UTF16::CharType> chars;

    while (! bytes.empty())
    {
        const auto front = (u8) bytes.front();
        ...
        if (front == '\\')
        {
            ...
            const auto kind = (u8) bytes.front();
            ...
            switch (kind)
            {
                ...
                case 'u':
                {
                    ...
                    std::string byteStr (reinterpret_cast<tukk> (bytes.data()), 4);
    ...
    chars.push_back ({});
    return Txt { CharPointer_UTF16 { chars.data() } };
}

std::vector<std::byte> Encodings::stringTo7BitText (const Txt& text)
{
    ...
    for (const auto character : text)
    {
        ...
        else
        {
            ...
            {
                const auto str = Txt::toHexString (unit);
...
std::vector<std::byte> Encodings::fromMcoded7 (Span<const std::byte> bytes)
{
    ...
    for (size_t index = 0; index < bytes.size(); index += 8)
    {
        ...
        {
            const auto highBit = (bytes[index] & std::byte { (u8) (1 << (6 - i)) }) << (i + 1);
...
public:
    ...
    z0 runTest() override
    {
        ...
        {
            {
                const auto converted = Encodings::stringTo7BitText (drx::CharPointer_UTF8 ("Accepted Beat \xe2\x99\xaa"));
            ...
            {
                const auto converted = Encodings::stringTo7BitText (drx::CharPointer_UTF8 ("\xe6\xae\x8b\xe3\x82\x8a\xe3\x82\x8f\xe3\x81\x9a\xe3\x81\x8b""5\xe3\x83\x90\xe3\x82\xa4\xe3\x83\x88"));
        ...
        {
            {
                ...
                const Txt expected = drx::CharPointer_UTF8 ("Accepted Beat \xe2\x99\xaa");
...

            {
                const auto converted = Encodings::stringFrom7BitText (makeByteArray ('\\', 'u', '6', 'b', '8', 'b',
                                                                                     ...
                                                                                     '\\', 'u', '3', '0', 'a', '4',
                                                                                     '\\', 'u', '3', '0', 'c', '8'));
                const Txt expected = drx::CharPointer_UTF8 ("\xe6\xae\x8b\xe3\x82\x8a\xe3\x82\x8f\xe3\x81\x9a\xe3\x81\x8b""5\xe3\x83\x90\xe3\x82\xa4\xe3\x83\x88");
                expect (converted == expected);
            }
        }
