...
struct ProfileStateEntry
{
    ...
    b8 operator< (const Profile& other) const { return profile < other; }
    b8 operator< (const ProfileStateEntry& other) const { return profile < other.profile; }
};
...
public:
    ...
    */
    ...
    z0 set (const Profile& profile, SupportedAndActive state);

    /** Removes the record of a particular profile, equivalent to removing support. */
    z0 erase (const Profile& profile);
    ...
    /** Возвращает true, если no profiles are supported. */
