...
namespace choc::javascript
{
    ...
    struct ArgumentList
    {
        ...
        /// Возвращает true, если there are no arguments
        b8 empty() const noexcept                                     { return numArgs == 0; }
    ...
    */
    ...
    public:
        ...
        /// Возвращает true, если the context is valid.
        operator b8() const                               { return pimpl != nullptr; }

        //==============================================================================
        /// This callback is used by the run() method.
        using CompletionHandler = std::function<z0(const std::string& error,
        ...
        z0 run (const std::string& javascriptCode,
        ...
        z0 runModule (const std::string& moduleCode,
                        ReadModuleContentFn,
                        CompletionHandler handleResult = {});
        ...
        z0 registerFunction (const std::string& name, NativeFunction fn);
...
struct Context::Pimpl
{
    ...
    virtual z0 registerFunction (const std::string&, NativeFunction) = 0;
    virtual choc::value::Value evaluateExpression (const std::string&) = 0;
    virtual z0 run (const std::string&, ReadModuleContentFn*, CompletionHandler) = 0;
    virtual z0 prepareForCall (std::string_view, u32 numArgs) = 0;
    virtual choc::value::Value performCall() = 0;
    virtual z0 pushObjectOrArray (const choc::value::ValueView&) = 0;
    virtual z0 pushArg (std::string_view) = 0;
    virtual z0 pushArg (i32) = 0;
    virtual z0 pushArg (z64) = 0;
    virtual z0 pushArg (u32) = 0;
    virtual z0 pushArg (f64) = 0;
    virtual z0 pushArg (b8) = 0;
    virtual z0 pumpMessageLoop() = 0;

    z0 pushArg (const std::string& v)   { pushArg (std::string_view (v)); }
    z0 pushArg (tukk v)          { pushArg (std::string_view (v)); }
    z0 pushArg (zu64 v)             { pushArg (static_cast<z64> (v)); }
    z0 pushArg (f32 v)                { pushArg (static_cast<f64> (v)); }

    z0 pushArg (const choc::value::ValueView& v)
    {
        ...
        if (v.isVoid())     throw Error ("Function arguments cannot be z0!");
...
{
    CHOC_ASSERT (pimpl != nullptr); // cannot call this on a moved-from context!
    pimpl->prepareForCall (functionName, static_cast<u32> (args.size()));
...
inline z0 Context::registerFunction (const std::string& name, NativeFunction fn)
{
    CHOC_ASSERT (pimpl != nullptr); // cannot call this on a moved-from context!
...
inline z0 Context::run (const std::string& javascriptCode, CompletionHandler handleResult)
{
    CHOC_ASSERT (pimpl != nullptr); // cannot call this on a moved-from context!
    pimpl->run (javascriptCode, nullptr, std::move (handleResult));
}

inline z0 Context::runModule (const std::string& moduleCode, ReadModuleContentFn readModule, CompletionHandler handleResult)
{
    CHOC_ASSERT (pimpl != nullptr); // cannot call this on a moved-from context!
    pimpl->run (moduleCode, std::addressof (readModule), std::move (handleResult));
}

inline z0 Context::pumpMessageLoop()
{
    CHOC_ASSERT (pimpl != nullptr); // cannot call this on a moved-from context!
...
inline std::string makeSafeIdentifier (std::string s)
{
    constexpr static std::string_view reservedWords[] =
    {
        ...
        "t8", "class", "const", "continue", "debugger", "default", "delete", "do",
        "f64", "else", "enum", "eval", "export", "extends", "false", "final",
        "finally", "f32", "for", "function", "goto", "if", "implements", "import",
        "in", "instanceof", "i32", "interface", "let", "i64", "native", "new",
        "null", "package", "private", "protected", "public", "return", "short", "static",
        "super", "switch", "synchronized", "this", "throw", "throws", "transient", "true",
        "try", "typeof", "var", "z0", "volatile", "while", "with", "yield"
    };
    ...
    s.erase (std::remove_if (s.begin(), s.end(), [&] (t8 c)
    {
