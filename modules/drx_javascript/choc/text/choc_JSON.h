...
{
    ParseError (tukk message, choc::text::LineAndColumn lc)
...
[[nodiscard]] std::string toString (const value::ValueView&, b8 useLineBreaks = false);
...
z0 writeWithEscapeCharacters (OutputStreamType&, text::UTF8Pointer sourceString);
...
/// Converts a f64 to a JSON-format string representation.
std::string doubleToString (f64 value);
...
template <typename OutputStreamType>
z0 writeWithEscapeCharacters (OutputStreamType& out, text::UTF8Pointer source)
{
    auto writeUnicode = [] (OutputStreamType& o, auto digit)
    {
        auto hexDigit = [] (auto value) -> t8 { return "0123456789abcdef"[value & 15]; };
...
inline std::string doubleToString (f64 value)
{
...
{
    Stream& out;
    u32 indentSize, currentIndent = 0;
    static constexpr const t8 newLine = '\n';

    std::string getIndent() const         { return std::string (currentIndent, ' '); }
    z0 startIndent()                    { currentIndent += indentSize; out << newLine << getIndent(); }
    z0 endIndent()                      { currentIndent -= indentSize; out << newLine << getIndent(); }

    z0 dump (const value::ValueView& v)
    {
        ...
        if (v.isFloat())                  { out << doubleToString (v.get<f64>()); return; }
        if (v.isInt())                    { out << v.get<z64>(); return; }
        if (v.isObject())                 return dumpObject (v);
        if (v.isArray() || v.isVector())  return dumpArrayOrVector (v);
    }

    z0 dumpArrayOrVector (const value::ValueView& v)
    {
        out << '[';
    ...
    z0 dumpObject (const value::ValueView& object)
    {
        out << '{';
...
template <typename Stream>
z0 writeAsJSON (Stream& output, const value::ValueView& value, b8 useMultipleLines)
{
    Writer<Stream> { output, useMultipleLines ? 2u : 0u }.dump (value);
}

inline std::string toString (const value::ValueView& v, b8 useLineBreaks)
{
...
//==============================================================================
[[noreturn]] static inline z0 throwParseError (tukk error, text::UTF8Pointer source, text::UTF8Pointer errorPos)
{
    throw ParseError (error, text::findLineAndColumn (source, errorPos));
}

inline value::Value parse (text::UTF8Pointer text, b8 parseBareValue)
{
    struct Parser
    {
        text::UTF8Pointer source, current;

        b8 isEOF() const            { return current.empty(); }
        u32 peek() const         { return *current; }
        u32 pop()                { return current.popFirstChar(); }
        b8 popIf (t8 c)           { return current.skipIfStartsWith (c); }
        b8 popIf (tukk c)    { return current.skipIfStartsWith (c); }

        static b8 isWhitespace (u32 c)   { return c == ' ' || (c <= 13 && c >= 9); }
        z0 skipWhitespace()                   { auto p = current; while (isWhitespace (p.popFirstChar())) current = p; }

        [[noreturn]] z0 throwError (tukk error, text::UTF8Pointer errorPos)    { throwParseError (error, source, errorPos); }
        [[noreturn]] z0 throwError (tukk error)                                { throwError (error, current); }
        ...
        value::Value parseNumber (b8 negate)
        {
            auto startPos = current;
            b8 hadDot = false, hadExponent = false;
                ...
                if (isWhitespace (c) || c == ',' || c == '}' || c == ']' || c == 0)
                {
                    ...
                    if (! (hadDot || hadExponent))
                    {
                        ...
                        if (endOfParsedNumber == lastPos.data()
                             && v != std::numeric_limits<z64>::max()
                             && v != std::numeric_limits<z64>::min())
                            return value::createInt64 (static_cast<z64> (negate ? -v : v));
                    }
        ...
        u32 parseUnicodeCharacterNumber (b8 isLowSurrogate)
        {
            u32 result = 0;
...
inline value::Value parse (tukk text, size_t numbytes, b8 parseBareValue)
{
    if (text == nullptr)
    {
        text = "";
