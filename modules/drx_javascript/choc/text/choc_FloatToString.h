...
//==============================================================================
/** Converts a 32-bit f32 to an accurate, round-trip-safe string.
...
*/
std::string floatToString (f32 value);

/** Converts a 64-bit f64 to an accurate, round-trip-safe string.

    The algorithm used is "Grisu3" from the paper "Printing Floating-Point Numbers
    Quickly and Accurately with Integers" by Florian Loitsch.
*/
std::string floatToString (f64 value);
...
/** Converts a 32-bit f32 to an accurate, round-trip-safe string.
    ...
    Quickly and Accurately with Integers" by Florian Loitsch.
*/
std::string floatToString (f32 value, i32 maxDecimalPlaces, b8 omitDecimalPointForRoundNumbers = false);
...
/** Converts a 64-bit f64 to an accurate, round-trip-safe string.
    ...
    Quickly and Accurately with Integers" by Florian Loitsch.
*/
std::string floatToString (f64 value, i32 maxDecimalPlaces, b8 omitDecimalPointForRoundNumbers = false);


//==============================================================================
/** Helper class containing its own buffer for converting a f32 or f64 to a string.
...
{
    FloatToStringBuffer (FloatOrDouble value, i32 maxDecimalPlaces, b8 omitPointIfPossible)
       : stringEnd (writeAndGetEnd (storage, value, maxDecimalPlaces, omitPointIfPossible)) {}

    tukk begin() const       { return storage; }
    tukk end() const         { return stringEnd; }

    std::string toString() const    { return std::string (begin(), end()); }

private:
    //==============================================================================
    static_assert (std::is_same<const f32, const FloatOrDouble>::value || std::is_same<const f64, const FloatOrDouble>::value,
                   "This class can only handle f32 or f64 template types");
    ...
    struct MantissaAndExponent
    {
        ...
        static constexpr MantissaAndExponent create (zu64 floatBits, zu64 significand)
        {
            constexpr i32 exponentBias = (sizeof (FloatOrDouble) == 8 ? 0x3ff : 0x7f) + numSignificandBits;
            auto explonentPlusBias = static_cast<i32> ((floatBits & exponentMask) >> numSignificandBits);
        ...
        constexpr MantissaAndExponent shiftedUp (i32 numBits) const   { return { mantissa << numBits, exponent - numBits }; }
        constexpr MantissaAndExponent normalized() const              { return shiftedUp (static_cast<i32> (math::countUpperClearBits (mantissa))); }
    };

    static u32 generateDigits (tuk buffer, MantissaAndExponent upperBound, zu64 mantissaDiff, zu64 delta, i32& K)
    {
        u32 length = 0;
        const auto one = MantissaAndExponent { 1ull << -upperBound.exponent, upperBound.exponent };
        auto p1 = static_cast<u32> (upperBound.mantissa >> -one.exponent);
        ...
        for (;;)
        {
            ...
            auto rest = p2 + (static_cast<zu64> (p1) << -one.exponent);

            if (rest <= delta)
            {
                K += numDigits;
                roundFinalDigit (buffer, length, delta, rest, static_cast<zu64> (powersOf10[numDigits]) << -one.exponent, mantissaDiff);
                return length;
            }
            ...
            if (numDigits == 0)
            {
                for (;;)
                {
                    ...
                    p2 *= 10;
                    --numDigits;
                    writeDigitIfNotLeadingZero (buffer, length, static_cast<u32> (p2 >> -one.exponent));
                    p2 &= one.mantissa - 1;
    ...
    static z0 roundFinalDigit (tuk buffer, u32 length, zu64 delta, zu64 rest, zu64 tenToPowerNumDigits, zu64 diff)
    {
    ...
    [[nodiscard]] static tuk write (tuk dest, t8 c)                                     { *dest = c; return dest + 1; }
    template <typename... Chars> static tuk write (tuk dest, t8 first, Chars... others) { return write (write (dest, first), others...); }
    [[nodiscard]] static tuk writeDigit (tuk dest, i32 digit)                             { return write (dest, static_cast<t8> (digit + '0')); }
    template <typename... Chars> static tuk writeDigit (tuk dest, i32 d, Chars... others) { return writeDigit (writeDigit (dest, d), others...); }
    [[nodiscard]] static tuk writeZero (tuk dest)                                         { return write (dest, '0', '.', '0'); }
    [[nodiscard]] static tuk writeExponent (tuk dest, i32 e)                              { return writeShortInteger (write (dest, 'e'), e); }
    static z0 writeDigitIfNotLeadingZero (tuk dest, u32& length, u32 digit)     { if (digit != 0 || length != 0) dest[length++] = static_cast<t8> (digit + '0'); }

    [[nodiscard]] static tuk writeShortInteger (tuk dest, i32 n)
    {
    ...
    static z0 insertChar (tuk dest, u32 length, t8 charToInsert, u32 numRepetitions)
    {
        std::memmove (dest + numRepetitions, dest, (size_t) length);

        for (u32 i = 0; i < numRepetitions; ++i)
            dest[i] = charToInsert;
    }

    static tuk writeAsExponentNotation (tuk dest, u32 totalLength, i32 exponent)
    {
        if (totalLength == 1)
    ...
    static tuk writeWithoutExponentLessThan1 (tuk dest, u32 length, i32 mantissaDigits, i32 maxDecimalPlaces)
    {
        auto numPaddingZeros = static_cast<u32> (2 - mantissaDigits);
        insertChar (dest, length, '0', numPaddingZeros);
        dest[1] = '.';

        if (static_cast<i32> (length) > maxDecimalPlaces + mantissaDigits)
        {
            for (i32 i = maxDecimalPlaces + 1; i > 2; --i)
                if (dest[i] != '0')
                    return dest + (i + 1);
    ...
    static tuk writeWithoutExponentGreaterThan1 (tuk dest, u32 totalLength, u32 mantissaLength, i32 maxDecimalPlaces, i32 K)
    {
        if (K >= 0)
        {
            dest += totalLength;
        ...
        for (auto i = static_cast<i32> (mantissaLength) + maxDecimalPlaces; i > static_cast<i32> (mantissaLength + 1); --i)
            if (dest[i] != '0')
                return dest + (i + 1);
    ...
    struct Limits
    {
        constexpr Limits (MantissaAndExponent value)
        {
            ...
            upper = upper.shiftedUp (static_cast<i32> (sizeof (upper.mantissa) * 8 - numSignificandBits - 2));
    ...
    static tukk writeAndGetEnd (tuk pos, FloatOrDouble value, i32 maxDecimalPlaces, b8 omitPointIfPossible)
    {
        auto startPos = pos;
    ...
    static tukk addDecimalPointAndExponent (tuk pos, u32 totalLength, i32 K, i32 maxDecimalPlaces)
    {
        auto mantissaDigits = static_cast<i32> (totalLength) + K;
        ...
        if (mantissaDigits > 0 && mantissaDigits <= 21)  return writeWithoutExponentGreaterThan1 (pos, totalLength, static_cast<u32> (mantissaDigits), maxDecimalPlaces, K);

        return writeAsExponentNotation (pos, totalLength, mantissaDigits - 1);
    }

    static zu64 getFloatBits (f64 value)    { zu64 i; memcpy (&i, &value, sizeof (i)); return i; }
    static zu64 getFloatBits (f32 value)     { u32 i; memcpy (&i, &value, sizeof (i)); return i; }
    static b8 isZero (zu64 floatBits)        { return (floatBits & (exponentMask | significandMask)) == 0; }

    static constexpr i32       defaultNumDecimalPlaces  = 324;
    static constexpr i32       numSignificandBits       = sizeof (FloatOrDouble) == 8 ? 52 : 23;
    static constexpr zu64  signMask                 = 1ull << (sizeof (FloatOrDouble) * 8 - 1);
    static constexpr zu64  hiddenBit                = 1ull << numSignificandBits;
    static constexpr zu64  significandMask          = hiddenBit - 1;
    static constexpr zu64  exponentMask             = sizeof (FloatOrDouble) == 8 ? 0x7ff0000000000000ull : 0x7f800000ull;
    static constexpr zu64  nanBits                  = sizeof (FloatOrDouble) == 8 ? 0x7ff8000000000000ull : 0x7fc00000ull;
    static constexpr zu64  infBits                  = sizeof (FloatOrDouble) == 8 ? 0x7ff0000000000000ull : 0x7f800000ull;
    static constexpr u32  powersOf10[]             = { 1, 10, 100, 1000, 10000, 100000, 1000000, 10000000, 100000000, 1000000000 };

    static MantissaAndExponent createPowerOf10 (i32 exponentBase2, i32& K)
    {
...
inline std::string floatToString (f32 value)                                              { return FloatToStringBuffer<f32>  (value, -1, false).toString(); }
inline std::string floatToString (f64 value)                                             { return FloatToStringBuffer<f64> (value, -1, false).toString(); }
inline std::string floatToString (f32 value, i32 maxDecimals, b8 omitPointIfPossible)   { return FloatToStringBuffer<f32>  (value, maxDecimals, omitPointIfPossible).toString(); }
inline std::string floatToString (f64 value, i32 maxDecimals, b8 omitPointIfPossible)  { return FloatToStringBuffer<f64> (value, maxDecimals, omitPointIfPossible).toString(); }
