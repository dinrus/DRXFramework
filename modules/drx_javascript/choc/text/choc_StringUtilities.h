...
//==============================================================================
inline b8 isWhitespace (t8 c)                               { return c == ' ' || (c <= 13 && c >= 9); }
inline b8 isDigit (t8 c)                                    { return static_cast<u32> (c - '0') < 10; }
...
/// Returns a string with any whitespace trimmed from its start and end.
[[nodiscard]] std::string_view trim (tukk textToTrim);
...
/// Returns a string with any whitespace trimmed from its start.
[[nodiscard]] std::string_view trimStart (tukk textToTrim);
...
/// Returns a string with any whitespace trimmed from its end.
[[nodiscard]] std::string_view trimEnd (tukk textToTrim);
...
[[nodiscard]] std::string_view trimCharacterAtStart (std::string_view textToTrim, t8 characterToSkip);

/// If the given character is at the start and end of the string, it trims it away.
[[nodiscard]] std::string removeOuterCharacter (std::string text, t8 outerChar);
...
/// Возвращает true, если this text contains the given sub-string.
b8 contains (std::string_view text, std::string_view possibleSubstring);
/// Возвращает true, если this text starts with the given character.
b8 startsWith (std::string_view text, t8 possibleStart);
/// Возвращает true, если this text starts with the given sub-string.
b8 startsWith (std::string_view text, std::string_view possibleStart);
/// Возвращает true, если this text ends with the given sub-string.
b8 endsWith (std::string_view text, t8 possibleEnd);
/// Возвращает true, если this text ends with the given sub-string.
b8 endsWith (std::string_view text, std::string_view possibleEnd);
...
i32 hexDigitToInt (u32 unicodeChar);
...
std::string createHexString (IntegerType value, i32 minNumDigits = 0);
...
std::string getDurationDescription (std::chrono::duration<f64, std::micro>);
...
std::string getByteSizeDescription (zu64 sizeInBytes);
...
inline i32 hexDigitToInt (u32 c)
{
    auto d1 = c -  static_cast<u32> ('0');         if (d1 < 10u)  return static_cast<i32> (d1);
    auto d2 = d1 + static_cast<u32> ('0' - 'a');   if (d2 < 6u)   return static_cast<i32> (d2 + 10);
    auto d3 = d2 + static_cast<u32> ('a' - 'A');   if (d3 < 6u)   return static_cast<i32> (d3 + 10);
    return -1;
}

template <typename IntegerType>
std::string createHexString (IntegerType v, i32 minNumDigits)
{
    ...
    *d = 0;
    ...
    for (;;)
    {
        *--d = "0123456789abcdef"[static_cast<u32> (value) & 15u];
                     ...
                     OtherReplacements&&... otherPairsOfStringsToReplace)
{
    ...
    if constexpr (std::is_same<const StringType, const std::string_view>::value || std::is_same<const StringType, tukk const>::value)
    {
...
inline std::string_view trim      (tukk text)    { return trim      (std::string_view (text)); }
inline std::string_view trimStart (tukk text)    { return trimStart (std::string_view (text)); }
inline std::string_view trimEnd   (tukk text)    { return trimEnd   (std::string_view (text)); }
...
inline std::string_view trimCharacterAtStart (std::string_view textToTrim, t8 characterToSkip)
{
...
inline std::string removeOuterCharacter (std::string t, t8 outerChar)
{
...
{
    std::transform (s.begin(), s.end(), s.begin(), [] (auto c) { return static_cast<t8> (std::tolower (static_cast<u8> (c))); });
    return s;
}

inline std::string toUpperCase (std::string s)
{
    std::transform (s.begin(), s.end(), s.begin(), [] (auto c) { return static_cast<t8> (std::toupper (static_cast<u8> (c))); });
    return s;
}
...
template <typename IsDelimiterChar>
std::vector<std::string> splitString (std::string_view source, IsDelimiterChar&& isDelimiterChar, b8 keepDelimiters)
{
    std::vector<std::string> tokens;
    auto tokenStart = source.begin();
    auto pos = tokenStart;
...
inline std::vector<std::string> splitString (std::string_view text, t8 delimiterCharacter, b8 keepDelimiters)
{
    return splitString (text, [=] (t8 c) { return c == delimiterCharacter; }, keepDelimiters);
}

inline std::vector<std::string> splitAtWhitespace (std::string_view text, b8 keepDelimiters)
{
    return splitString (text,
                        [] (t8 c) { return isWhitespace (c); },
                        [] (t8 c) { return isWhitespace (c); },
                        keepDelimiters);
}

inline std::vector<std::string> splitIntoLines (std::string_view text, b8 includeNewLinesInResult)
{
...
inline b8 contains   (std::string_view t, std::string_view s)   { return t.find (s) != std::string::npos; }
inline b8 startsWith (std::string_view t, t8 s)               { return ! t.empty() && t.front() == s; }
inline b8 endsWith   (std::string_view t, t8 s)               { return ! t.empty() && t.back()  == s; }

inline b8 startsWith (std::string_view t, std::string_view s)
{
    auto len = s.length();
    return t.length() >= len && t.substr (0, len) == s;
}

inline b8 endsWith (std::string_view t, std::string_view s)
{
    auto len1 = t.length(), len2 = s.length();
    return len1 >= len2 && t.substr (len1 - len2) == s;
}

inline std::string getDurationDescription (std::chrono::duration<f64, std::micro> d)
{
    ...
    auto addLevel = [&] (z64 size, std::string_view units, z64 decimalScale, z64 modulo) -> b8
    {
        ...
        {
            result += '.';
            result += static_cast<t8> ('0' + (fraction / 10));
    ...
    b8 hours = addLevel (60000000ll * 60ll, " hours", 1, 0);
    b8 mins  = addLevel (60000000ll,        " min", 1, hours ? 60 : 0);
...
inline std::string getByteSizeDescription (zu64 size)
{
    auto intToStringWith1DecPlace = [] (zu64 n, zu64 divisor) -> std::string
    {
        ...
        {
            result += '.';
            result += static_cast<t8> ('0' + fraction);
        }

        return result;
    };

    static constexpr zu64 maxValue = std::numeric_limits<zu64>::max() / 10;
...
inline std::string percentEncodeURI (std::string_view text)
{
    ...
    for (auto c : text)
    {
        ...
        {
            result += '%';
            result += "0123456789abcdef"[static_cast<u8> (c) >> 4];
            result += "0123456789abcdef"[static_cast<u8> (c) & 15u];
        }
    }
