...
z0 JSCursor::set (const var& value) const
{
    ...
    if (auto* prop = std::get_if<z64> (&(*property)))
    {
        object.setProperty (*prop, value);
        return;
    }
...
JSCursor JSCursor::getChild (z64 index) const
{
    auto copy = *this;
...
JSCursor JSCursor::operator[] (z64 index) const
{
...
JSObject JSCursor::getOrCreateObject() const
{
    ...
    auto* integerValue = std::get_if<z64> (&(*property));
...
b8 JSCursor::isValid() const
{
    return getPartialResolution().has_value();
}

b8 JSCursor::isArray() const
{
...
{
    if (auto* index = std::get_if<z64> (&property))
    {
        if (! object.isArray())
            return std::nullopt;
...
std::optional<JSCursor::PartialResolution> JSCursor::getPartialResolution() const
{
    ...
    for (i32 i = 0, iEnd = (i32) path.size() - 1; i < iEnd; ++i)
    {
