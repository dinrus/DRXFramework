...
static constexpr const t8 javascriptTestSource[] = R"x(
...
static constexpr const t8 accessNewObject[] = R"x(
var ref = newObject;
)x";

static constexpr const t8 createAccumulator[] = R"x(
...
static constexpr const t8 replaceObjectAtCommHandleLocation[] = R"x(
...
public:
    ...
    z0 runTest() override
    {
        ...
        beginTest ("JSCursor::invokeMethod");
        {
            ...
            expect (exactlyEqual ((f64) result, 7.0));
        }
        ...
        beginTest ("JSCursor Array access");
        {
            ...
            expectEquals ((f64) root["array"][2].get(), -1.25);
        }
        ...
        beginTest ("The object referenced by the cursor shouldn't disappear/change");
        {
            ...
            // communicate.
            expectEquals ((i32) accumulatorCursor["accumulate"]({}), 5);
            ...
            expectEquals ((i32) accumulatorCursor["accumulate"]({}), 7,
        ...
        beginTest ("A JSCursor instance can be used to retrieve whatever value is at a given location");
        {
            ...
            expectEquals ((i32) cursor.get(), 5);

            engine.execute ("path.to = new Object();"
                            "path.to.location = 6;");

            expectEquals ((i32) cursor.get(), 6);
        }
        ...
        beginTest ("Methods of javascript objects can be called from C++");
        {
            ...
            const var a[] { var { 10 } };
            const auto aResult = obj->invokeMethod ("foo", { val, std::data (a), (i32) std::size (a) });
            ...
            const var b[] { var { -10 } };
            const auto bResult = obj->invokeMethod ("foo", { val, std::data (b), (i32) std::size (b) });
        ...
        beginTest ("Destructors of custom callables are called, eventually");
        {
            ...
            {
                explicit CustomCallable (i32& instances)
        ...
        beginTest ("calling a C function that returns z0 is converted correctly");
        {
            i32 numCalls = 0;
        ...
        beginTest ("Properties of registered native objects are enumerable");
        {
            ...
            expect (res.wasOk());
            expectEquals (val.toString(), Txt (R"({"nested":{"present":true},"one":1,"hello":"world"})"));
        }
