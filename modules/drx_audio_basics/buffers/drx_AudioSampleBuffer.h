...
public:
    ...
    */
    AudioBuffer (i32 numChannelsToAllocate,
                 i32 numSamplesToAllocate)
    ...
    */
    ...
    {
        if (numChannels < (i32) numElementsInArray (preallocatedChannelSpace))
        {
    ...
    */
    i32 getNumChannels() const noexcept                             { return numChannels; }

    /** Returns the number of samples allocated in each of the buffer's channels.

        @see getNumChannels, getReadPointer, getWritePointer
    */
    i32 getNumSamples() const noexcept                              { return size; }
    ...
    */
    const Type* getReadPointer (i32 channelNumber) const noexcept
    {
    ...
    /** Returns a pointer to an array of read-only samples in one of the buffer's channels.
        ...
        planning on modifying the data.
    */
    const Type* getReadPointer (i32 channelNumber, i32 sampleIndex) const noexcept
    {
        jassert (isPositiveAndBelow (channelNumber, numChannels));
    ...
    /** Returns a writeable pointer to one of the buffer's channels.
        ...
        @see setNotClear
    */
    Type* getWritePointer (i32 channelNumber) noexcept
    {
        jassert (isPositiveAndBelow (channelNumber, numChannels));
        isClear = false;
        return channels[channelNumber];
    ...
    /** Returns a writeable pointer to one of the buffer's channels.
        ...
        @see setNotClear
    */
    Type* getWritePointer (i32 channelNumber, i32 sampleIndex) noexcept
    {
        jassert (isPositiveAndBelow (channelNumber, numChannels));
        jassert (isPositiveAndBelow (sampleIndex, size));
        isClear = false;
    ...
    */
    z0 setSize (i32 newNumChannels,
                  i32 newNumSamples,
                  b8 keepExistingContent = false,
                  b8 clearExtraSpace = false,
                  b8 avoidReallocating = false)
    {
        ...
        if (newNumSamples != size || newNumChannels != numChannels)
        {
            ...
            if (keepExistingContent)
            {
                ...
                else
                {
                    ...
                    for (i32 j = 0; j < newNumChannels; ++j)
                    {
                    ...
                    if (! isClear)
                    {
                        ...
                        for (i32 i = 0; i < numChansToCopy; ++i)
                            FloatVectorOperations::copy (newChannels[i], channels[i], (i32) numSamplesToCopy);
                    }
            ...
            else
            {
                ...
                for (i32 i = 0; i < newNumChannels; ++i)
                {
                    channels[i] = chan;
    ...
    /** Makes this buffer point to a pre-allocated set of channel data arrays.
        ...
        @param newNumSamples    the number of samples to use - this must correspond to the
                                size of the arrays passed in
    */
    z0 setDataToReferTo (Type* const* dataToReferTo,
                           i32 newNumChannels,
                           i32 newStartSample,
                           i32 newNumSamples)
    {
    ...
    */
    ...
    /** Resizes this buffer to match the given one, and copies all of its content across.
        ...
        convert between 32 and 64 bit f32 buffer types.

        The hasBeenCleared method will return false after this call if the other buffer
        contains data.
    */
    template <typename OtherType>
    z0 makeCopyOf (const AudioBuffer<OtherType>& other, b8 avoidReallocating = false)
    {
    ...
    */
    ...
    /** Clears a specified region of all the channels.
        ...
        @see hasBeenCleared, setNotClear
    */
    z0 clear (i32 startSample, i32 numSamples) noexcept
    {
    ...
    /** Clears a specified region of just one channel.
        ...
        @see hasBeenCleared, setNotClear
    */
    z0 clear (i32 channel, i32 startSample, i32 numSamples) noexcept
    {
    ...
    /** Возвращает true, если the buffer has been entirely cleared.
    ...
    */
    b8 hasBeenCleared() const noexcept                            { return isClear; }
    ...
    */
    z0 setNotClear() noexcept                                     { isClear = false; }
    ...
    /** Returns a sample from the buffer.
        ...
        territory.
    */
    Type getSample (i32 channel, i32 sampleIndex) const noexcept
    {
        jassert (isPositiveAndBelow (channel, numChannels));
        jassert (isPositiveAndBelow (sampleIndex, size));
    ...
    /** Sets a sample in the buffer.
        ...
        The hasBeenCleared method will return false after this call.
    */
    z0 setSample (i32 destChannel, i32 destSample, Type newValue) noexcept
    {
    ...
    /** Adds a value to a sample in the buffer.
        ...
        The hasBeenCleared method will return false after this call.
    */
    z0 addSample (i32 destChannel, i32 destSample, Type valueToAdd) noexcept
    {
        jassert (isPositiveAndBelow (destChannel, numChannels));
        jassert (isPositiveAndBelow (destSample, size));
    ...
    /** Applies a gain multiple to a region of one channel.
        ...
        are in-range, so be careful!
    */
    z0 applyGain (i32 channel, i32 startSample, i32 numSamples, Type gain) noexcept
    {
        jassert (isPositiveAndBelow (channel, numChannels));
        jassert (startSample >= 0 && numSamples >= 0 && startSample + numSamples <= size);
    ...
    /** Applies a gain multiple to a region of all the channels.
        ...
        are in-range, so be careful!
    */
    z0 applyGain (i32 startSample, i32 numSamples, Type gain) noexcept
    {
        for (i32 i = 0; i < numChannels; ++i)
    ...
    z0 applyGain (Type gain) noexcept
    {
    ...
    /** Applies a range of gains to a region of a channel.
        ...
        are in-range, so be careful!
    */
    z0 applyGainRamp (i32 channel, i32 startSample, i32 numSamples,
                        Type startGain, Type endGain) noexcept
    {
        ...
        const auto increment = (endGain - startGain) / (f32) numSamples;
        auto* d = channels[channel] + startSample;
    ...
    /** Applies a range of gains to a region of all channels.
        ...
        are in-range, so be careful!
    */
    z0 applyGainRamp (i32 startSample, i32 numSamples,
                        Type startGain, Type endGain) noexcept
    {
        for (i32 i = 0; i < numChannels; ++i)
    ...
    /** Adds samples from another buffer to this one.
        ...
        @see copyFrom
    */
    z0 addFrom (i32 destChannel,
                  i32 destStartSample,
                  const AudioBuffer& source,
                  i32 sourceChannel,
                  i32 sourceStartSample,
                  i32 numSamples,
    ...
    */
    z0 addFromWithRamp (i32 destChannel,
                          i32 destStartSample,
                          const Type* source,
                          i32 numSamples,
                          Type startGain,
    ...
    /** Copies samples from another buffer to this one.
        ...
        @see addFrom
    */
    z0 copyFrom (i32 destChannel,
                   i32 destStartSample,
                   const AudioBuffer& source,
                   i32 sourceChannel,
                   i32 sourceStartSample,
                   i32 numSamples) noexcept
    {
        jassert (&source != this
                 || sourceChannel != destChannel
                 || sourceStartSample + numSamples <= destStartSample
    ...
    */
    ...
    /** Copies samples from an array of floats into one of the channels, applying a gain ramp.
        ...
        @see addFrom
    */
    z0 copyFromWithRamp (i32 destChannel,
                           i32 destStartSample,
                           const Type* source,
                           i32 numSamples,
                           Type startGain,
    ...
    */
    Range<Type> findMinMax (i32 channel, i32 startSample, i32 numSamples) const noexcept
    {
        jassert (isPositiveAndBelow (channel, numChannels));
        jassert (startSample >= 0 && numSamples >= 0 && startSample + numSamples <= size);
    ...
    Type getMagnitude (i32 channel, i32 startSample, i32 numSamples) const noexcept
    {
        jassert (isPositiveAndBelow (channel, numChannels));
        jassert (startSample >= 0 && numSamples >= 0 && startSample + numSamples <= size);
    ...
    Type getMagnitude (i32 startSample, i32 numSamples) const noexcept
    {
        Type mag (0);
    ...
    Type getRMSLevel (i32 channel, i32 startSample, i32 numSamples) const noexcept
    {
        jassert (isPositiveAndBelow (channel, numChannels));
        jassert (startSample >= 0 && numSamples >= 0 && startSample + numSamples <= size);
    ...
    z0 reverse (i32 channel, i32 startSample, i32 numSamples) const noexcept
    {
        jassert (isPositiveAndBelow (channel, numChannels));
        jassert (startSample >= 0 && numSamples >= 0 && startSample + numSamples <= size);
    ...
    z0 reverse (i32 startSample, i32 numSamples) const noexcept
    {
        jassert (startSample >= 0 && numSamples >= 0 && startSample + numSamples <= size);
...
private:
    ...
    z0 allocateData()
    {
       #if (! DRX_GCC || (__GNUC__ * 100 + __GNUC_MINOR__) >= 409)
    ...
    z0 allocateChannels (Type* const* dataToReferTo, i32 offset)
    {
        jassert (offset >= 0);
    ...
    /*  On iOS/arm7 the alignment of `f64` is greater than the alignment of
    ...
    */
    ...
    i32 numChannels = 0, size = 0;
    size_t allocatedBytes = 0;
    Type** channels;
    HeapBlock<t8, true> allocatedData;
    Type* preallocatedChannelSpace[32];
    b8 isClear = false;
    static constexpr size_t maxAlignment = getMaxAlignment();

    DRX_LEAK_DETECTOR (AudioBuffer)
};

//==============================================================================
template <typename Type>
b8 operator== (const AudioBuffer<Type>& a, const AudioBuffer<Type>& b)
{
...
/**
    ...
    @see AudioBuffer
*/
using AudioSampleBuffer = AudioBuffer<f32>;
