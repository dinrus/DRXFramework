...
AudioChannelSet::AudioChannelSet (u32 c) : channels (static_cast<z64> (c))
{
}
...
b8 AudioChannelSet::operator== (const AudioChannelSet& other) const noexcept  { return channels == other.channels; }
b8 AudioChannelSet::operator!= (const AudioChannelSet& other) const noexcept  { return channels != other.channels; }
b8 AudioChannelSet::operator<  (const AudioChannelSet& other) const noexcept  { return channels <  other.channels; }

Txt AudioChannelSet::getChannelTypeName (AudioChannelSet::ChannelType type)
{
    if (type >= discreteChannel0)
        return "Discrete " + Txt (type - discreteChannel0 + 1);
...
Txt AudioChannelSet::getAbbreviatedChannelTypeName (AudioChannelSet::ChannelType type)
{
    if (type >= discreteChannel0)
        return Txt (type - discreteChannel0 + 1);
...
AudioChannelSet::ChannelType AudioChannelSet::getChannelTypeFromAbbreviation (const Txt& abbr)
{
    if (abbr.length() > 0 && (abbr[0] >= '0' && abbr[0] <= '9'))
        return static_cast<AudioChannelSet::ChannelType> (static_cast<i32> (discreteChannel0)
                                                               + abbr.getIntValue() - 1);
...
Txt AudioChannelSet::getSpeakerArrangementAsString() const
{
...
AudioChannelSet AudioChannelSet::fromAbbreviatedString (const Txt& str)
{
    AudioChannelSet set;
...
Txt AudioChannelSet::getDescription() const
{
    if (isDiscreteLayout())                return "Discrete #" + Txt (size());
    ...
    // ambisonics
    {
        ...
        if (order >= 0)
        {
            ...
            return Txt (order) + suffix + " Order Ambisonics";
        }
    }
...
b8 AudioChannelSet::isDiscreteLayout() const noexcept
{
...
i32 AudioChannelSet::size() const noexcept
{
    return channels.countNumberOfSetBits();
}

AudioChannelSet::ChannelType AudioChannelSet::getTypeOfChannel (i32 index) const noexcept
{
    i32 bit = channels.findNextSetBit (0);

    for (i32 i = 0; i < index && bit >= 0; ++i)
        bit = channels.findNextSetBit (bit + 1);

    return static_cast<ChannelType> (bit);
}

i32 AudioChannelSet::getChannelIndexForType (AudioChannelSet::ChannelType type) const noexcept
{
    i32 idx = 0;

    for (i32 bit = channels.findNextSetBit (0); bit >= 0; bit = channels.findNextSetBit (bit + 1))
    {
...
z0 AudioChannelSet::addChannel (ChannelType newChannel)
{
    i32k bit = static_cast<i32> (newChannel);
    jassert (bit >= 0 && bit < 1024);
    channels.setBit (bit);
}

z0 AudioChannelSet::removeChannel (ChannelType newChannel)
{
    i32k bit = static_cast<i32> (newChannel);
    jassert (bit >= 0 && bit < 1024);
...
AudioChannelSet AudioChannelSet::ambisonic (i32 order)
{
    jassert (isPositiveAndBelow (order, 8));

    static constexpr std::pair<i32, i32> continuousRanges[] { { ambisonicACN0,  ambisonicACN3 },
    ...
    for (i32 rangeIdx = 0, bitsSet = 0; bitsSet < numAmbisonicChannels; ++rangeIdx)
    {
...
i32 AudioChannelSet::getAmbisonicOrder() const
{
...
AudioChannelSet AudioChannelSet::discreteChannels (i32 numChannels)
{
    AudioChannelSet s;
...
AudioChannelSet AudioChannelSet::canonicalChannelSet (i32 numChannels)
{
...
AudioChannelSet AudioChannelSet::namedChannelSet (i32 numChannels)
{
    if (numChannels == 1)  return AudioChannelSet::mono();
    if (numChannels == 2)  return AudioChannelSet::stereo();
    if (numChannels == 3)  return AudioChannelSet::createLCR();
...
Array<AudioChannelSet> AudioChannelSet::channelSetsWithNumberOfChannels (i32 numChannels)
{
...
AudioChannelSet DRX_CALLTYPE AudioChannelSet::channelSetWithChannels (const Array<ChannelType>& channelArray)
{
    AudioChannelSet set;

    for (auto ch : channelArray)
    {
        jassert (! set.channels[static_cast<i32> (ch)]);
...
//==============================================================================
AudioChannelSet DRX_CALLTYPE AudioChannelSet::fromWaveChannelMask (i32 dwChannelMask)
{
    return AudioChannelSet (static_cast<u32> ((dwChannelMask & ((1 << 18) - 1)) << 1));
}

i32 AudioChannelSet::getWaveChannelMask() const noexcept
{
...
//==============================================================================
i32 AudioChannelSet::getAmbisonicOrderForNumChannels (i32 numChannels, i32 maxOrderToCheck)
{
...
private:
    z0 checkAmbisonic (zu64 mask, i32 order, tukk layoutName)
    {
        auto expected = AudioChannelSet::ambisonic (order);
        auto numChannels = expected.size();

        expect (numChannels == BigInteger ((z64) mask).countNumberOfSetBits());
    ...
    static AudioChannelSet channelSetFromMask (zu64 mask)
    {
        Array<AudioChannelSet::ChannelType> channels;
        for (i32 bit = 0; bit <= 62; ++bit)
            if ((mask & (1ull << bit)) != 0)
