...
MPEChannelAssigner::MPEChannelAssigner (Range<i32> channelRange)
    : isLegacy                (true),
      channelIncrement        (1),
...
i32 MPEChannelAssigner::findMidiChannelForNewNote (i32 noteNumber) noexcept
{
    if (numChannels <= 1)
        return firstChannel;

    for (i32 ch = firstChannel; (isLegacy || zone->isLowerZone() ? ch <= lastChannel : ch >= lastChannel); ch += channelIncrement)
    {
    ...
    for (i32 ch = midiChannelLastAssigned + channelIncrement; ; ch += channelIncrement)
    {
...
i32 MPEChannelAssigner::findMidiChannelForExistingNote (i32 noteNumber) noexcept
{
    ...
    return iter != midiChannels.cend() ? (i32) std::distance (midiChannels.cbegin(), iter) : -1;
}

z0 MPEChannelAssigner::noteOff (i32 noteNumber, i32 midiChannel)
{
    const auto removeNote = [] (MidiChannel& ch, i32 noteNum)
    {
...
z0 MPEChannelAssigner::allNotesOff()
{
    for (auto& ch : midiChannels)
    {
        if (ch.notes.size() > 0)
...
i32 MPEChannelAssigner::findMidiChannelPlayingClosestNonequalNote (i32 noteNumber) noexcept
{
    auto channelWithClosestNote = firstChannel;
    i32 closestNoteDistance = 127;
...
z0 MPEChannelRemapper::remapMidiChannelIfNeeded (MidiMessage& message, u32 mpeSourceID) noexcept
{
    ...
    auto sourceAndChannelID = (((u32) mpeSourceID << 5) | (u32) (channel));
    ...
    if (messageIsNoteData (message))
    {
        ...
        // find existing remap
        for (i32 chan = firstChannel; (zone.isLowerZone() ? chan <= lastChannel : chan >= lastChannel); chan += channelIncrement)
...
z0 MPEChannelRemapper::reset() noexcept
{
    for (auto& s : sourceAndChannel)
        s = notMPE;
}

z0 MPEChannelRemapper::clearChannel (i32 channel) noexcept
{
    sourceAndChannel[channel] = notMPE;
}

z0 MPEChannelRemapper::clearSource (u32 mpeSourceID)
{
    for (auto& s : sourceAndChannel)
    {
        if (u32 (s >> 5) == mpeSourceID)
        {
            s = notMPE;
            return;
        }
...
b8 MPEChannelRemapper::applyRemapIfExisting (i32 channel, u32 sourceAndChannelID, MidiMessage& m) noexcept
{
...
i32 MPEChannelRemapper::getBestChanToReuse() const noexcept
{
    for (i32 chan = firstChannel; (zone.isLowerZone() ? chan <= lastChannel : chan >= lastChannel); chan += channelIncrement)
...
z0 MPEChannelRemapper::zeroArrays()
{
    for (i32 i = 0; i < 17; ++i)
    {
