...
struct DRX_API  MPENote
{
    ...
    */
    f64 getFrequencyInHertz (f64 frequencyOfA = 440.0) const noexcept;

    /** Возвращает true, если two notes are the same, determined by their unique ID. */
    b8 operator== (const MPENote& other) const noexcept;

    /** Возвращает true, если two notes are different notes, determined by their unique ID. */
    b8 operator!= (const MPENote& other) const noexcept;
};
