...
/**
    ...
    @tags{Audio}
*/
class DRX_API  SynthesiserSound    : public ReferenceCountedObject
{
protected:
    //==============================================================================
    SynthesiserSound();
...
public:
    ...
    //==============================================================================
    /** Возвращает true, если this sound should be played when a given midi note is pressed.

        The Synthesiser will use this information when deciding which sounds to trigger
        for a given note.
    */
    virtual b8 appliesToNote (i32 midiNoteNumber) = 0;

    /** Возвращает true, если the sound should be triggered by midi events on a given channel.

        The Synthesiser will use this information when deciding which sounds to trigger
        for a given note.
    */
    virtual b8 appliesToChannel (i32 midiChannel) = 0;
...
private:
    //==============================================================================
    DRX_LEAK_DETECTOR (SynthesiserSound)
};
...
/**
    ...
    @tags{Audio}
*/
class DRX_API  SynthesiserVoice
{
public:
    //==============================================================================
    /** Creates a voice. */
    ...
    */
    i32 getCurrentlyPlayingNote() const noexcept                        { return currentlyPlayingNote; }
    ...
    */
    virtual b8 canPlaySound (SynthesiserSound*) = 0;

    /** Called to start a new note.
        This will be called during the rendering callback, so must be fast and thread-safe.
    */
    virtual z0 startNote (i32 midiNoteNumber,
                            f32 velocity,
                            SynthesiserSound* sound,
                            i32 currentPitchWheelPosition) = 0;
    ...
    /** Called to stop a note.
        ...
        clearCurrentNote().
    */
    virtual z0 stopNote (f32 velocity, b8 allowTailOff) = 0;

    /** Возвращает true, если this voice is currently busy playing a sound.
        By default this just checks the getCurrentlyPlayingNote() value, but can
        be overridden for more advanced checking.
    */
    virtual b8 isVoiceActive() const;

    /** Called to let the voice know that the pitch wheel has been moved.
        This will be called during the rendering callback, so must be fast and thread-safe.
    */
    virtual z0 pitchWheelMoved (i32 newPitchWheelValue) = 0;

    /** Called to let the voice know that a midi controller has been moved.
        This will be called during the rendering callback, so must be fast and thread-safe.
    */
    virtual z0 controllerMoved (i32 controllerNumber, i32 newControllerValue) = 0;

    /** Called to let the voice know that the aftertouch has changed.
        This will be called during the rendering callback, so must be fast and thread-safe.
    */
    virtual z0 aftertouchChanged (i32 newAftertouchValue);

    /** Called to let the voice know that the channel pressure has changed.
        This will be called during the rendering callback, so must be fast and thread-safe.
    */
    virtual z0 channelPressureChanged (i32 newChannelPressureValue);
    ...
    */
    ...
    /** A f64-precision version of renderNextBlock() */
    virtual z0 renderNextBlock (AudioBuffer<f64>& outputBuffer,
                                  i32 startSample,
                                  i32 numSamples);
    ...
    */
    virtual z0 setCurrentPlaybackSampleRate (f64 newRate);

    /** Возвращает true, если the voice is currently playing a sound which is mapped to the given
        midi channel.

        If it's not currently playing, this will return false.
    */
    virtual b8 isPlayingChannel (i32 midiChannel) const;
    ...
    */
    ...
    /** Возвращает true, если the key that triggered this voice is still held down.
        Note that the voice may still be playing after the key was released (e.g because the
        sostenuto pedal is down).
    */
    b8 isKeyDown() const noexcept                             { return keyIsDown; }

    /** Allows you to modify the flag indicating that the key that triggered this voice is still held down.
        @see isKeyDown
    */
    z0 setKeyDown (b8 isNowDown) noexcept                   { keyIsDown = isNowDown; }

    /** Возвращает true, если the sustain pedal is currently active for this voice. */
    b8 isSustainPedalDown() const noexcept                    { return sustainPedalDown; }

    /** Modifies the sustain pedal flag. */
    z0 setSustainPedalDown (b8 isNowDown) noexcept          { sustainPedalDown = isNowDown; }

    /** Возвращает true, если the sostenuto pedal is currently active for this voice. */
    b8 isSostenutoPedalDown() const noexcept                  { return sostenutoPedalDown; }

    /** Modifies the sostenuto pedal flag. */
    z0 setSostenutoPedalDown (b8 isNowDown) noexcept        { sostenutoPedalDown = isNowDown; }

    /** Возвращает true, если a voice is sounding in its release phase **/
    b8 isPlayingButReleased() const noexcept
    {
        return isVoiceActive() && ! (isKeyDown() || isSostenutoPedalDown() || isSustainPedalDown());
    }

    /** Возвращает true, если this voice started playing its current note before the other voice did. */
    b8 wasStartedBefore (const SynthesiserVoice& other) const noexcept;
...
private:
    ...
    i32 currentlyPlayingNote = -1, currentPlayingMidiChannel = 0;
    u32 noteOnTime = 0;
    SynthesiserSound::Ptr currentlyPlayingSound;
    b8 keyIsDown = false, sustainPedalDown = false, sostenutoPedalDown = false;

    AudioBuffer<f32> tempBuffer;

    DRX_LEAK_DETECTOR (SynthesiserVoice)
};
...
public:
    ...
    */
    ...
    SynthesiserVoice* getVoice (i32 index) const;
    ...
    */
    ...
    i32 getNumSounds() const noexcept                               { return sounds.size(); }

    /** Returns one of the sounds. */
    SynthesiserSound::Ptr getSound (i32 index) const noexcept       { return sounds[index]; }
    ...
    */
    z0 setNoteStealingEnabled (b8 shouldStealNotes);

    /** Возвращает true, если note-stealing is enabled.
        @see setNoteStealingEnabled
    */
    b8 isNoteStealingEnabled() const noexcept                     { return shouldStealNotes; }
    ...
    */
    virtual z0 noteOn (i32 midiChannel,
                         i32 midiNoteNumber,
                         f32 velocity);
    ...
    /** Triggers a note-off event.
        ...
        The midiChannel parameter is the channel, between 1 and 16 inclusive.
    */
    virtual z0 noteOff (i32 midiChannel,
                          i32 midiNoteNumber,
                          f32 velocity,
                          b8 allowTailOff);
    ...
    /** Turns off all notes.
        ...
        renderNextBlock(), but may be called explicitly too.
    */
    virtual z0 allNotesOff (i32 midiChannel,
                              b8 allowTailOff);
    ...
    */
    virtual z0 handlePitchWheel (i32 midiChannel,
                                   i32 wheelValue);
    ...
    */
    virtual z0 handleAftertouch (i32 midiChannel, i32 midiNoteNumber, i32 aftertouchValue);
    ...
    */
    virtual z0 handleChannelPressure (i32 midiChannel, i32 channelPressureValue);

    /** Handles a sustain pedal event. */
    virtual z0 handleSustainPedal (i32 midiChannel, b8 isDown);

    /** Handles a sostenuto pedal event. */
    virtual z0 handleSostenutoPedal (i32 midiChannel, b8 isDown);

    /** Can be overridden to handle soft pedal events. */
    virtual z0 handleSoftPedal (i32 midiChannel, b8 isDown);
    ...
    */
    ...
    /** Tells the synthesiser what the sample rate is for the audio it's being used to render.
        ...
        pitches.
    */
    virtual z0 setCurrentPlaybackSampleRate (f64 sampleRate);
    ...
    */
    z0 renderNextBlock (AudioBuffer<f32>& outputAudio,
                          const MidiBuffer& inputMidi,
                          i32 startSample,
                          i32 numSamples);

    z0 renderNextBlock (AudioBuffer<f64>& outputAudio,
                          const MidiBuffer& inputMidi,
                          i32 startSample,
                          i32 numSamples);
    ...
    /** Returns the current target sample rate at which rendering is being done.
        Subclasses may need to know this so that they can pitch things correctly.
    */
    f64 getSampleRate() const noexcept                       { return sampleRate; }
    ...
    */
    z0 setMinimumRenderingSubdivisionSize (i32 numSamples, b8 shouldBeStrict = false) noexcept;
...
protected:
    ...
    */
    z0 stopVoice (SynthesiserVoice*, f32 velocity, b8 allowTailOff);

    /** Can be overridden to do custom handling of incoming midi events. */
    virtual z0 handleMidiEvent (const MidiMessage&);

private:
    //==============================================================================
    f64 sampleRate = 0;
    u32 lastNoteOnCounter = 0;
    i32 minimumSubBlockSize = 32;
    b8 subBlockSubdivisionIsStrict = false;
    b8 shouldStealNotes = true;
    BigInteger sustainPedalsDown;
    mutable CriticalSection stealLock;
    mutable Array<SynthesiserVoice*> usableVoicesToStealArray;

    template <typename floatType>
    z0 processNextBlock (AudioBuffer<floatType>&, const MidiBuffer&, i32 startSample, i32 numSamples);

    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (Synthesiser)
};
