...
constexpr u8  operator""_u8  (zu64 i) { return static_cast<u8>  (i); }
constexpr u16 operator""_u16 (zu64 i) { return static_cast<u16> (i); }
constexpr u32 operator""_u32 (zu64 i) { return static_cast<u32> (i); }
constexpr zu64 operator""_u64 (zu64 i) { return static_cast<zu64> (i); }
    ...
    z0 runTest() override
    {
        ...
        const auto checkRoundTrip = [&] (const MidiBuffer& expected)
        {
            ...
            converter.dispatch (packets.data(),
                                ...
                                [&] (const BytestreamMidiView& roundTripped)
                                {
                                    output.addEvent (roundTripped.getMessage(), i32 (roundTripped.timestamp));
                                });
            packets.clear();
        ...
        beginTest ("SysEx messages are terminated by non-Utility, non-Realtime messages");
        {
            ...
            {
                MidiBuffer b;
                b.addEvent (MidiMessage::noteOn (1, u8 (64), u8 (64)), 0);
                return b;
            }();
        ...
        beginTest ("Widening conversions work");
        {
            ...
            const auto baselineScale = [] (u32 srcVal, u32 srcBits, u32 dstBits)
            {
                const auto scaleBits = (u32) (dstBits - srcBits);

                auto bitShiftedValue = (u32) (srcVal << scaleBits);

                const auto srcCenter = (u32) (1 << (srcBits - 1));

                if (srcVal <= srcCenter)
                    return bitShiftedValue;

                const auto repeatBits = (u32) (srcBits - 1);
                const auto repeatMask = (u32) ((1 << repeatBits) - 1);

                auto repeatValue = (u32) (srcVal & repeatMask);
            ...
            const auto baselineScale7To8 = [&] (u8 in)
            {
                return baselineScale (in, 7, 8);
            };

            const auto baselineScale7To16 = [&] (u8 in)
            {
                return baselineScale (in, 7, 16);
            };

            const auto baselineScale14To16 = [&] (u16 in)
            {
                return baselineScale (in, 14, 16);
            };

            const auto baselineScale7To32 = [&] (u8 in)
            {
                return baselineScale (in, 7, 32);
            };

            const auto baselineScale14To32 = [&] (u16 in)
            {
            ...
            {
                const auto rand = (u8) random.nextInt (0x80);
                expectEquals ((z64) Conversion::scaleTo8 (rand),
                              (z64) baselineScale7To8 (rand));
            }

            expectEquals ((z64) Conversion::scaleTo16 ((u8) 0x00), (z64) 0x0000);
            expectEquals ((z64) Conversion::scaleTo16 ((u8) 0x0a), (z64) 0x1400);
            expectEquals ((z64) Conversion::scaleTo16 ((u8) 0x40), (z64) 0x8000);
            expectEquals ((z64) Conversion::scaleTo16 ((u8) 0x57), (z64) 0xaeba);
            expectEquals ((z64) Conversion::scaleTo16 ((u8) 0x7f), (z64) 0xffff);

            for (auto i = 0; i != 100; ++i)
            {
                const auto rand = (u8) random.nextInt (0x80);
                expectEquals ((z64) Conversion::scaleTo16 (rand),
                              (z64) baselineScale7To16 (rand));
            }

            for (auto i = 0; i != 100; ++i)
            {
                const auto rand = (u16) random.nextInt (0x4000);
                expectEquals ((z64) Conversion::scaleTo16 (rand),
                              (z64) baselineScale14To16 (rand));
            }

            for (auto i = 0; i != 100; ++i)
            {
                const auto rand = (u8) random.nextInt (0x80);
                expectEquals ((z64) Conversion::scaleTo32 (rand),
                              (z64) baselineScale7To32 (rand));
            }

            expectEquals ((z64) Conversion::scaleTo32 ((u16) 0x0000), (z64) 0x00000000);
            expectEquals ((z64) Conversion::scaleTo32 ((u16) 0x2000), (z64) 0x80000000);
            expectEquals ((z64) Conversion::scaleTo32 ((u16) 0x3fff), (z64) 0xffffffff);

            for (auto i = 0; i != 100; ++i)
            {
                const auto rand = (u16) random.nextInt (0x4000);
                expectEquals ((z64) Conversion::scaleTo32 (rand),
                              (z64) baselineScale14To32 (rand));
            }
        }
        ...
        beginTest ("Round-trip widening/narrowing conversions work");
        {
            for (auto i = 0; i != 100; ++i)
            {
                ...
                {
                    const auto rand = (u16) random.nextInt (0x4000);
                    expectEquals ((zu64) Conversion::scaleTo14 (Conversion::scaleTo16 (rand)), (zu64) rand);
                }

                {
                    const auto rand = (u16) random.nextInt (0x4000);
                    expectEquals ((zu64) Conversion::scaleTo14 (Conversion::scaleTo32 (rand)), (zu64) rand);
                }
            }
        }
        ...
        {
            u8k typecodesX1[] { 0x0, 0x1, 0x2 };

            for (const auto typecode : typecodesX1)
            {
                Packets p;
                p.add (PacketX1 { (u32) ((z64) typecode << 0x1c | (random.nextInt64() & 0xffffff)) });
            ...
            {
                Packets p;
                p.add (PacketX2 { (u32) (0x3 << 0x1c | (random.nextInt64() & 0xffffff)),
                                  (u32) (random.nextInt64() & 0xffffffff) });
            ...
            {
                Packets p;
                p.add (PacketX4 { (u32) (0x5 << 0x1c | (random.nextInt64() & 0xffffff)),
                                  (u32) (random.nextInt64() & 0xffffffff),
                                  (u32) (random.nextInt64() & 0xffffffff),
                                  (u32) (random.nextInt64() & 0xffffffff) });
        ...
        {
            u8k CCs[] { 6, 38, 98, 99, 100, 101, 0, 32 };

            for (const auto cc : CCs)
            {
                Packets midi2;
                midi2.add (PacketX2 { (u32) (0x40b00000 | (cc << 0x8)), 0x00000000 });
        ...
        {
            {
                ...
                Packets midi2;
                midi2.add (PacketX2 { 0x40904000, static_cast<u32> (Conversion::scaleTo16 (0x40_u8)) << 0x10 });
        ...
        beginTest ("MIDI 1 -> 2 note off conversions");
        {
            ...
            Packets midi2;
            midi2.add (PacketX2 { 0x41831000, static_cast<u32> (Conversion::scaleTo16 (0x20_u8)) << 0x10 });
        ...
        beginTest ("MIDI 1 -> 2 control change conversions");
        {
            ...
            // nrpn
            {
                ...
                Packets midi2;
                midi2.add (PacketX2 { 0x40300123, Conversion::scaleTo32 (static_cast<u16> ((0x45 << 7) | 0x67)) });
            ...
            // rpn
            {
                ...
                Packets midi2;
                midi2.add (PacketX2 { 0x40204321, Conversion::scaleTo32 (static_cast<u16> ((0x01 << 7) | 0x23)) });
        ...
        beginTest ("MIDI 1 -> MIDI 2 pitch bend conversions");
        {
            ...
            Packets midi2;
            midi2.add (PacketX2 { 0x40e70000, Conversion::scaleTo32 (static_cast<u16> ((0x67 << 7) | 0x45)) });
...
private:
    ...
    z0 checkBytestreamConversion (const Packets& actual, const Packets& expected)
    {
        expectEquals ((i32) actual.size(), (i32) expected.size());
        ...
        std::for_each (expected.data(),
                       ...
                       [&] (u32k word) { expectEquals ((zu64) *actualPtr++, (zu64) word); });
    }

    z0 checkMidi2ToMidi1Conversion (const Packets& midi2, const Packets& expected)
    {
        checkBytestreamConversion (convertMidi2ToMidi1 (midi2), expected);
    }

    z0 checkMidi1ToMidi2Conversion (const Packets& midi1, const Packets& expected)
    {
    ...
    MidiMessage createRandomSysEx (Random& random, size_t sysExBytes)
    {
        ...
        for (size_t i = 0; i != sysExBytes; ++i)
            data.push_back (u8 (random.nextInt (0x80)));

        return MidiMessage::createSysExMessage (data.data(), i32 (data.size()));
    }
    ...
    z0 forEachNonSysExTestMessage (Random& random, Fn&& fn)
    {
        for (u16 counter = 0x80; counter != 0x100; ++counter)
        {
            const auto firstByte = (u8) counter;
            ...
            const auto getDataByte = [&] { return u8 (random.nextInt (256) & 0x7f); };
    ...
    static b8 equal (const MidiMessage& a, const MidiMessage& b) noexcept
    {
        return a.getRawDataSize() == b.getRawDataSize()
               && std::equal (a.getRawData(), a.getRawData() + a.getRawDataSize(), b.getRawData());
    }

    static b8 equal (const MidiBuffer& a, const MidiBuffer& b) noexcept
    {
