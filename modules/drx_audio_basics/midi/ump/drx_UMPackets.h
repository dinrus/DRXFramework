...
public:
    /** Adds a single packet to the collection.
        ...
        undefined behaviour.
    */
    z0 add (const View& v) { storage.insert (storage.end(), v.cbegin(), v.cend()); }

    z0 add (const PacketX1& p) { addImpl (p); }
    z0 add (const PacketX2& p) { addImpl (p); }
    z0 add (const PacketX3& p) { addImpl (p); }
    z0 add (const PacketX4& p) { addImpl (p); }

    /** Pre-allocates space for at least `numWords` 32-bit words in this collection. */
    z0 reserve (size_t numWords)          { storage.reserve (numWords); }

    /** Removes all previously-added packets from this collection. */
    z0 clear()                            { storage.clear(); }
    ...
    u32k* data() const noexcept   { return storage.data(); }

    /** Returns the number of u32 words in storage.
...
private:
    ...
    z0 addImpl (const Packet<numWords>& p)
    {
