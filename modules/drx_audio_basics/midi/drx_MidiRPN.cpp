...
b8 MidiRPNDetector::parseControllerMessage (i32 midiChannel,
...
std::optional<MidiRPNMessage> MidiRPNDetector::tryParse (i32 midiChannel,
                                                         i32 controllerNumber,
                                                         i32 controllerValue)
{
...
z0 MidiRPNDetector::reset() noexcept
{
...
//==============================================================================
std::optional<MidiRPNMessage> MidiRPNDetector::ChannelState::handleController (i32 channel,
                                                                               i32 controllerNumber,
                                                                               i32 value) noexcept
{
    switch (controllerNumber)
    {
        case 0x62:  parameterLSB = u8 (value); resetValue(); isNRPN = true;  break;
        case 0x63:  parameterMSB = u8 (value); resetValue(); isNRPN = true;  break;

        case 0x64:  parameterLSB = u8 (value); resetValue(); isNRPN = false; break;
        case 0x65:  parameterMSB = u8 (value); resetValue(); isNRPN = false; break;

        case 0x06:  valueMSB = u8 (value); valueLSB = 0xff; return sendIfReady (channel);
        case 0x26:  valueLSB = u8 (value);                  return sendIfReady (channel);
    }

    return {};
}

z0 MidiRPNDetector::ChannelState::resetValue() noexcept
{
    valueMSB = 0xff;
    valueLSB = 0xff;
}
...
//==============================================================================
std::optional<MidiRPNMessage> MidiRPNDetector::ChannelState::sendIfReady (i32 channel) noexcept
{
...
MidiBuffer MidiRPNGenerator::generate (i32 midiChannel,
                                       i32 parameterNumber,
                                       i32 value,
                                       b8 isNRPN,
                                       b8 use14BitValue)
{
    ...
    auto parameterLSB = u8 (parameterNumber & 0x0000007f);
    auto parameterMSB = u8 (parameterNumber >> 7);

    u8 valueLSB = use14BitValue ? u8 (value & 0x0000007f) : 0x00;
    u8 valueMSB = use14BitValue ? u8 (value >> 7) : u8 (value);

    auto channelByte = u8 (0xb0 + midiChannel - 1);
...
private:
    //==============================================================================
    z0 expectContainsRPN (const MidiBuffer& midiBuffer,
                            i32 channel,
                            i32 parameterNumber,
                            i32 value,
                            b8 isNRPN,
    ...
    //==============================================================================
    z0 expectContainsRPN (const MidiBuffer& midiBuffer, MidiRPNMessage expected)
    {
