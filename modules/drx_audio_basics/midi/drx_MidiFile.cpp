...
{
    static z0 writeVariableLengthInt (OutputStream& out, u32 v)
    {
        auto buffer = v & 0x7f;
    ...
    template <>
    struct ReadTrait<u32> { static constexpr auto read = ByteOrder::bigEndianInt; };

    template <>
    struct ReadTrait<u16> { static constexpr auto read = ByteOrder::bigEndianShort; };

    template <typename Integral>
    Optional<Integral> tryRead (u8k*& data, size_t& remaining)
    {
    ...
    static Optional<HeaderDetails> parseMidiHeader (u8k* const initialData,
                                                    const size_t maxSize)
    {
        auto* data = initialData;
        auto remaining = maxSize;

        auto ch = tryRead<u32> (data, remaining);
        ...
        const auto bytesRemaining = tryRead<u32> (data, remaining);

        if (! bytesRemaining.hasValue() || *bytesRemaining > remaining)
            return {};

        const auto optFileType = tryRead<u16> (data, remaining);

        if (! optFileType.hasValue() || 2 < *optFileType)
            return {};

        const auto optNumTracks = tryRead<u16> (data, remaining);

        if (! optNumTracks.hasValue() || (*optFileType == 0 && *optNumTracks != 1))
            return {};

        const auto optTimeFormat = tryRead<u16> (data, remaining);
    ...
    static f64 convertTicksToSeconds (f64 time,
                                         ...
                                         i32 timeFormat)
    {
        ...
        f64 lastTime = 0, correctedTime = 0;
    ...
    static z0 findAllMatchingEvents (const OwnedArray<MidiMessageSequence>& tracks,
    ...
    static MidiMessageSequence readTrack (u8k* data, i32 size)
    {
        f64 time = 0;
        u8 lastStatusByte = 0;
        ...
        while (size > 0)
        {
            const auto delay = MidiMessage::readVariableLengthValue (data, (i32) size);
...
//==============================================================================
MidiFile::MidiFile()  : timeFormat ((short) (u16) 0xe728) {}
...
//==============================================================================
i32 MidiFile::getNumTracks() const noexcept
{
    return tracks.size();
}

const MidiMessageSequence* MidiFile::getTrack (i32 index) const noexcept
{
    return tracks[index];
}

z0 MidiFile::addTrack (const MidiMessageSequence& trackSequence)
{
...
z0 MidiFile::setTicksPerQuarterNote (i32 ticks) noexcept
{
    timeFormat = (short) ticks;
}

z0 MidiFile::setSmpteTimeFormat (i32 framesPerSecond, i32 subframeResolution) noexcept
{
    timeFormat = (short) (((-framesPerSecond) << 8) | subframeResolution);
}

//==============================================================================
z0 MidiFile::findAllTempoEvents (MidiMessageSequence& results) const
{
    MidiFileHelpers::findAllMatchingEvents (tracks, results, &MidiMessage::isTempoMetaEvent);
}

z0 MidiFile::findAllTimeSigEvents (MidiMessageSequence& results) const
{
    MidiFileHelpers::findAllMatchingEvents (tracks, results, &MidiMessage::isTimeSignatureMetaEvent);
}

z0 MidiFile::findAllKeySigEvents (MidiMessageSequence& results) const
{
    MidiFileHelpers::findAllMatchingEvents (tracks, results, &MidiMessage::isKeySignatureMetaEvent);
}

f64 MidiFile::getLastTimestamp() const
{
    f64 t = 0.0;
...
//==============================================================================
b8 MidiFile::readFrom (InputStream& sourceStream,
                         b8 createMatchingNoteOffs,
                         i32* fileType)
{
    clear();
    MemoryBlock data;

    i32k maxSensibleMidiFileSize = 200 * 1024 * 1024;
    ...
    auto d = static_cast<u8k*> (data.getData());
    ...
    for (i32 track = 0; track < header.numberOfTracks; ++track)
    {
        const auto optChunkType = MidiFileHelpers::tryRead<u32> (d, size);

        if (! optChunkType.hasValue())
            return false;

        const auto optChunkSize = MidiFileHelpers::tryRead<u32> (d, size);
        ...
        if (*optChunkType == ByteOrder::bigEndianInt ("MTrk"))
            readNextTrack (d, (i32) chunkSize, createMatchingNoteOffs);
...
static z0 reorderNoteOnsAfterNoteOffs (const It begin, const It end)
{
...
z0 MidiFile::readNextTrack (u8k* data, i32 size, b8 createMatchingNoteOffs)
{
...
//==============================================================================
z0 MidiFile::convertTimestampTicksToSeconds()
{
...
//==============================================================================
b8 MidiFile::writeTo (OutputStream& out, i32 midiFileType) const
{
    jassert (midiFileType >= 0 && midiFileType <= 2);

    if (! out.writeIntBigEndian ((i32) ByteOrder::bigEndianInt ("MThd"))) return false;
...
b8 MidiFile::writeTrack (OutputStream& mainOut, const MidiMessageSequence& ms) const
{
    MemoryOutputStream out;

    i32 lastTick = 0;
    u8 lastStatusByte = 0;
    b8 endOfTrackEventWritten = false;

    for (i32 i = 0; i < ms.getNumEvents(); ++i)
    {
        ...
        MidiFileHelpers::writeVariableLengthInt (out, (u32) delta);
        lastTick = tick;
        ...
        {
            out.writeByte ((t8) statusByte);

            ++data;
            --dataSize;

            MidiFileHelpers::writeVariableLengthInt (out, (u32) dataSize);
        }
    ...
    if (! mainOut.writeIntBigEndian ((i32) ByteOrder::bigEndianInt ("MTrk"))) return false;
    if (! mainOut.writeIntBigEndian ((i32) out.getDataSize()))                return false;
...
struct MidiFileTest final : public UnitTest
{
    ...
    z0 runTest() override
    {
        ...
        beginTest ("ReadTrack returns available messages if input is truncated");
        {
...

            {
                ...
                expectEquals (sequence.getEventPointer (0)->message.getVelocity(), (u8) 0x00);
            }
        }
        ...
        beginTest ("Header parsing works");
        {
...

            {
                ...
                expectEquals ((i32) header->bytesRead, 14);
            }
        }
        ...
        beginTest ("Read from stream");
        {
...

            {
                ...
                expectEquals (track.getEventPointer (0)->message.getTimeStamp(), (f64) 0x0f);
            }
        }
    ...
    static MidiMessageSequence parseSequence (Fn&& fn)
    {
        ...
        return MidiFileHelpers::readTrack (reinterpret_cast<u8k*> (os.getData()),
                                           (i32) os.getDataSize());
    }
    ...
    static Optional<MidiFileHelpers::HeaderDetails> parseHeader (Fn&& fn)
    {
        ...
        return MidiFileHelpers::parseMidiHeader (reinterpret_cast<u8k*> (os.getData()),
                                                 os.getDataSize());
    }
    ...
    static z0 writeBytes (OutputStream& os, const std::vector<u8>& bytes)
    {
        for (const auto& byte : bytes)
            os.writeByte ((t8) byte);
    }
};
