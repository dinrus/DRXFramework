...
public:
    ...
    b8 isSmoothing() const noexcept                    { return countdown > 0; }
    ...
    */
    z0 setCurrentAndTargetValue (FloatType newValue)
    {
    ...
    */
    z0 applyGain (FloatType* samples, i32 numSamples) noexcept
    {
    ...
    */
    z0 applyGain (FloatType* samplesOut, const FloatType* samplesIn, i32 numSamples) noexcept
    {
        jassert (numSamples >= 0);
    ...
    z0 applyGain (AudioBuffer<FloatType>& buffer, i32 numSamples) noexcept
    {
        jassert (numSamples >= 0);
...
/**
    ...
    For example:
    @code
    SmoothedValue<f32, ValueSmoothingTypes::Multiplicative> frequency (1.0f);
    @endcode
*/
...
/**
    ...
    @code
    SmoothedValue<f32, ValueSmoothingTypes::Linear> yourSmoothedValue;
    @endcode
    ...
    @code
    SmoothedValue<f32, ValueSmoothingTypes::Multiplicative> yourSmoothedValue;
    @endcode
...
public:
    ...
    */
    ...
    z0 setValue (FloatType newValue, b8 force = false) noexcept
    {
        if (force)
        {
            this->setCurrentAndTargetValue (newValue);
...
private:
    ...
    //==============================================================================
    template <typename T = SmoothingType>
    z0 skipCurrentValue (i32 numSamples) noexcept
    {
        if constexpr (std::is_same_v<T, ValueSmoothingTypes::Linear>)
        {
...
public:
    ...
    z0 runTest() override
    {
        ...
        beginTest ("Block processing");
        {
            ...
            AudioBuffer<f32> referenceData (1, numSamples);
            ...
            auto getUnitData = [] (i32 numSamplesToGenerate)
            {
                AudioBuffer<f32> result (1, numSamplesToGenerate);

                for (i32 i = 0; i < numSamplesToGenerate; ++i)
                    result.setSample (0, i, 1.0f);

                return result;
            };

            auto compareData = [this] (const AudioBuffer<f32>& test,
                                       const AudioBuffer<f32>& reference)
            {
                for (i32 i = 0; i < test.getNumSamples(); ++i)
            ...
            AudioBuffer<f32> destData (1, numSamples);
            sv.setCurrentAndTargetValue (1.0f);
            sv.setTargetValue (2.0f);
        ...
        beginTest ("Negative");
        {
            ...
            std::vector<std::pair<f32, f32>> ranges = { { -1.0f, -2.0f },
                                                            { -100.0f, -3.0f } };
