...
public:
    ...
    */
    ...
    /** Возвращает true, если and only if getTokenProvider() returns non-null. */
    explicit operator b8() const { return getTokenProvider() != nullptr; }
...
/**
    ...
    z0 AudioProcessor::processBlock()
    {
        workerThread->notify();
    }
    @endcode
...
public:
    ...
    */
    ...
    /** Equality operator. */
    b8 operator== (const AudioWorkgroup& other) const;

    /** Inequality operator. */
    b8 operator!= (const AudioWorkgroup& other) const { return ! operator== (other); }

    /** Возвращает true, если and only if this object represents a workgroup. */
    explicit operator b8() const;

    /** Disengages this instance so that it no longer represents a workgroup.

        After this call, operator b8() will return false.
    */
    z0 reset() { erased = nullptr; }
...
private:
    ...
    z0 swap (AudioWorkgroup& other) noexcept { std::swap (other.erased, erased); }
