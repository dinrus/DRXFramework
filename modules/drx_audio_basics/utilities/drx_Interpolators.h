...
private:
    ...
    {
        static constexpr f32 algorithmicLatency = 100.0f;

        static forcedinline f32 windowedSinc (f32 firstFrac, i32 index) noexcept
        {
            auto index2 = index + 1;
            auto frac = firstFrac;
        ...
        static forcedinline f32 valueAtOffset (const f32* const inputs, const f32 offset, i32 indexBuffer) noexcept
        {
            i32k numCrossings = 100;
            const f32 floatCrossings = (f32) numCrossings;
            f32 result = 0.0f;

            auto samplePosition = indexBuffer;
            f32 firstFrac = 0.0f;
            f32 lastSincPosition = -1.0f;
            i32 index = 0, sign = -1;

            for (i32 i = -numCrossings; i <= numCrossings; ++i)
            {
                auto sincPosition = (1.0f - offset) + (f32) i;
        ...
        static const f32 lookupTable[10001];
    };

    struct LagrangeTraits
    {
        static constexpr f32 algorithmicLatency = 2.0f;

        static f32 valueAtOffset (const f32*, f32, i32) noexcept;
    };
    ...
    struct CatmullRomTraits
    {
        ...
        static forcedinline f32 valueAtOffset (const f32* const inputs, const f32 offset, i32 index) noexcept
        {
    ...
    struct LinearTraits
    {
        static constexpr f32 algorithmicLatency = 1.0f;
    ...
    {
        static constexpr f32 algorithmicLatency = 0.0f;

        static forcedinline f32 valueAtOffset (const f32* const inputs, const f32, i32) noexcept
        {
            return inputs[0];
        }
    };
