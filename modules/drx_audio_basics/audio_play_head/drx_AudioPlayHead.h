...
public:
    ...
    public:
        ...
        */
        ...
        i32 getBaseRate() const                         { return base; }

        /** Возвращает true, если drop-frame timecode is in use. */
        b8 isDrop() const                             { return drop; }

        /** Возвращает true, если the effective framerate is actually equal to the base rate divided by 1.001 */
        b8 isPullDown() const                         { return pulldown; }

        /** Returns the actual rate described by this object, taking pulldown into account. */
        f64 getEffectiveRate() const                 { return pulldown ? (f64) base / 1.001 : (f64) base; }

        /** Returns a copy of this object with the specified base rate. */
        [[nodiscard]] FrameRate withBaseRate (i32 x) const            { return with (&FrameRate::base, x); }

        /** Returns a copy of this object with drop frames enabled or disabled, as specified. */
        [[nodiscard]] FrameRate withDrop (b8 x = true) const        { return with (&FrameRate::drop, x); }

        /** Returns a copy of this object with pulldown enabled or disabled, as specified. */
        [[nodiscard]] FrameRate withPullDown (b8 x = true) const    { return with (&FrameRate::pulldown, x); }

        /** Возвращает true, если this instance is equal to other. */
        b8 operator== (const FrameRate& other) const
        {
            const auto tie = [] (const FrameRate& x) { return std::tie (x.base, x.drop, x.pulldown); };
            return tie (*this) == tie (other);
        }

        /** Возвращает true, если this instance is not equal to other. */
        b8 operator!= (const FrameRate& other) const { return ! (*this == other); }
    ...
    */
    struct DRX_API TimeSignature
    {
        ...
        b8 operator== (const TimeSignature& other) const
        {
    ...
    */
    struct DRX_API LoopPoints
    {
        ...
        b8 operator== (const LoopPoints& other) const
        {
    ...
    */
    struct DRX_API  CurrentPositionInfo
    {
        /** The tempo in BPM */
        f64 bpm = 120.0;

        /** Time signature numerator, e.g. the 3 of a 3/4 time sig */
        i32 timeSigNumerator = 4;

        /** Time signature denominator, e.g. the 4 of a 3/4 time sig */
        i32 timeSigDenominator = 4;
        ...
        */
        f64 ppqPositionOfLastBarStart = 0;
        ...
        */
        ...
        //==============================================================================
        b8 operator== (const CurrentPositionInfo& other) const noexcept
        {
    ...
    */
    ...
    public:
        ...
        Optional<z64> getTimeInSamples() const                      { return getOptional (flagTimeSamples, timeInSamples); }

        /** @see getTimeInSamples() */
        z0 setTimeInSamples (Optional<z64> timeInSamplesIn)       {        setOptional (flagTimeSamples, timeInSamples, timeInSamplesIn); }

        /** Returns the number of seconds that have elapsed. */
        Optional<f64> getTimeInSeconds() const                       { return getOptional (flagTimeSeconds, timeInSeconds); }

        /** @see getTimeInSamples() */
        z0 setTimeInSeconds (Optional<f64> timeInSecondsIn)        {        setOptional (flagTimeSeconds, timeInSeconds, timeInSecondsIn); }

        /** Returns the bpm, if available. */
        Optional<f64> getBpm() const                                 { return getOptional (flagTempo, tempoBpm); }

        /** @see getBpm() */
        z0 setBpm (Optional<f64> bpmIn)                            {        setOptional (flagTempo, tempoBpm, bpmIn); }
        ...
        z0 setTimeSignature (Optional<TimeSignature> timeSignatureIn) {        setOptional (flagTimeSignature, timeSignature, timeSignatureIn); }
        ...
        /** @see getLoopPoints() */
        z0 setLoopPoints (Optional<LoopPoints> loopPointsIn)          {        setOptional (flagLoopPoints, loopPoints, loopPointsIn); }

        /** The number of bars since the beginning of the timeline.

            This value isn't available in all hosts or in all plugin formats.
        */
        Optional<z64> getBarCount() const                           { return getOptional (flagBarCount, barCount); }

        /** @see getBarCount() */
        z0 setBarCount (Optional<z64> barCountIn)                 {        setOptional (flagBarCount, barCount, barCountIn); }
        ...
        */
        Optional<f64> getPpqPositionOfLastBarStart() const           { return getOptional (flagLastBarStartPpq, lastBarStartPpq); }

        /** @see getPpqPositionOfLastBarStart() */
        z0 setPpqPositionOfLastBarStart (Optional<f64> positionIn) {        setOptional (flagLastBarStartPpq, lastBarStartPpq, positionIn); }

        /** The video frame rate, if available. */
        Optional<FrameRate> getFrameRate() const                        { return getOptional (flagFrameRate, frame); }

        /** @see getFrameRate() */
        z0 setFrameRate (Optional<FrameRate> frameRateIn)             {        setOptional (flagFrameRate, frame, frameRateIn); }

        /** The current play position, in units of quarter-notes. */
        Optional<f64> getPpqPosition() const                         { return getOptional (flagPpqPosition, positionPpq); }

        /** @see getPpqPosition() */
        z0 setPpqPosition (Optional<f64> ppqPositionIn)            {        setOptional (flagPpqPosition, positionPpq, ppqPositionIn); }

        /** For timecode, the position of the start of the timeline, in seconds from 00:00:00:00. */
        Optional<f64> getEditOriginTime() const                      { return getOptional (flagOriginTime, originTime); }

        /** @see getEditOriginTime() */
        z0 setEditOriginTime (Optional<f64> editOriginTimeIn)      {        setOptional (flagOriginTime, originTime, editOriginTimeIn); }

        /** Get the host's callback time in nanoseconds, if available. */
        Optional<zu64> getHostTimeNs() const                        { return getOptional (flagHostTimeNs, hostTimeNs); }

        /** @see getHostTimeNs() */
        z0 setHostTimeNs (Optional<zu64> hostTimeNsIn)            {        setOptional (flagHostTimeNs, hostTimeNs, hostTimeNsIn); }

        /** True if the transport is currently playing. */
        b8 getIsPlaying() const                                       { return getFlag (flagIsPlaying); }

        /** @see getIsPlaying() */
        z0 setIsPlaying (b8 isPlayingIn)                            {        setFlag (flagIsPlaying, isPlayingIn); }

        /** True if the transport is currently recording.

            (When isRecording is true, then isPlaying will also be true).
        */
        b8 getIsRecording() const                                     { return getFlag (flagIsRecording); }

        /** @see getIsRecording() */
        z0 setIsRecording (b8 isRecordingIn)                        {        setFlag (flagIsRecording, isRecordingIn); }

        /** True if the transport is currently looping. */
        b8 getIsLooping() const                                       { return getFlag (flagIsLooping); }

        /** @see getIsLooping() */
        z0 setIsLooping (b8 isLoopingIn)                            {        setFlag (flagIsLooping, isLoopingIn); }

        b8 operator== (const PositionInfo& other) const noexcept
        {
    ...
    private:
        b8 getFlag (z64 flagToCheck) const
        {
            return (flagToCheck & flags) != 0;
        }

        z0 setFlag (z64 flagToCheck, b8 value)
        {
            flags = (value ? flags | flagToCheck : flags & ~flagToCheck);
        }

        template <typename Value>
        Optional<Value> getOptional (z64 flagToCheck, Value value) const
        {
            return getFlag (flagToCheck) ? makeOptional (std::move (value)) : nullopt;
        }

        template <typename Value>
        z0 setOptional (z64 flagToCheck, Value& value, Optional<Value> opt)
        {
            if (opt.hasValue())
                value = *opt;
    ...
    */
    ...
    b8 getCurrentPosition (CurrentPositionInfo& result)
    {
    ...
    */
    ...
    /** Возвращает true, если this object can control the transport. */
    virtual b8 canControlTransport();

    /** Starts or stops the audio. */
    virtual z0 transportPlay (b8 shouldStartPlaying);

    /** Starts or stops recording the audio. */
    virtual z0 transportRecord (b8 shouldStartRecording);

    /** Rewinds the audio. */
    virtual z0 transportRewind();
};
