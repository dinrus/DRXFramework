...
struct CoreAudioLayouts
{
    ...
    // Apple channel labels have been replaced by DRX channel names
    // This means that some layouts will be identical in DRX but not in CoreAudio

    // In Apple's official definition the following tags exist with the same speaker layout and order
    // even when *not* represented in DRX channels
...
public:
    ...
    //==============================================================================
    /** Convert CoreAudio's native AudioChannelLayout to DRX's AudioChannelSet.
    ...
    */
    ...
    /** Convert CoreAudio's native AudioChannelLayoutTag to DRX's AudioChannelSet.
    ...
    */
    ...
    /** Convert DRX's AudioChannelSet to CoreAudio's AudioChannelLayoutTag.
    ...
    */
    static AudioChannelLayoutTag toCoreAudio (const AudioChannelSet& set)
    {
        if (set.getAmbisonicOrder() >= 0)
            return coreAudioHOASN3DLayoutTag | static_cast<u32> (set.size());

        for (const auto& item : SpeakerLayoutTable::get())
        {
            AudioChannelSet caSet;

            for (i32 i = 0; i < numElementsInArray (item.channelTypes)
    ...
    //==============================================================================
    /** Convert CoreAudio's native AudioChannelLayout to an array of DRX ChannelTypes. */
    static Array<AudioChannelSet::ChannelType> getCoreAudioLayoutChannels (const AudioChannelLayout& layout)
    {
        switch (layout.mChannelLayoutTag & 0xffff0000)
        {
            case kAudioChannelLayoutTag_UseChannelBitmap:
                return AudioChannelSet::fromWaveChannelMask (static_cast<i32> (layout.mChannelBitmap)).getChannelTypes();
            case kAudioChannelLayoutTag_UseChannelDescriptions:
            {
               ...
               // Suppressing clang-analyzer-optin.core.EnumCastOutOfRange
               #ifndef __clang_analyzer__
                // different speaker mappings may point to the same DRX speaker so fill up
                // this array with discrete channels
                for (i32 j = 0; channels.size() < static_cast<i32> (layout.mNumberChannelDescriptions); ++j)
            ...
            case kAudioChannelLayoutTag_DiscreteInOrder:
                return AudioChannelSet::discreteChannels (static_cast<i32> (layout.mChannelLayoutTag) & 0xffff).getChannelTypes();
            default:
                break;
        }
    ...
    static Array<AudioChannelSet::ChannelType> getSpeakerLayoutForCoreAudioTag (AudioChannelLayoutTag tag)
    {
        ...
        {
            const auto ambisonicOrder = AudioChannelSet::getAmbisonicOrderForNumChannels (static_cast<i32> (numChannels));
...
private:
    ...
    static AudioChannelSet::ChannelType getChannelTypeFromAudioChannelLabel (AudioChannelLabel label) noexcept
    {
        ...
        {
            u32k discreteChannelNum = label - kAudioChannelLabel_Discrete_0;
