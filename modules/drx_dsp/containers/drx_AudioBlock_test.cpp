...
Txt& operator<< (Txt& str, SIMDRegister<SampleType>) { return str; }
#endif
...
public:
    ...
    z0 runTest() override
    {
        ...
        beginTest ("Getters and setters");
        {
            ...
            expectEquals ((i32) block.getNumChannels(), (i32) data.size());
            expectEquals ((i32) block.getNumSamples(), numSamples);
...
private:
    ...
    z0 copyingTests()
    {
        if constexpr (std::is_scalar_v<SampleType>)
        {
            ...
            AudioBuffer<SampleType> otherBuffer (otherData.data(), (i32) otherData.size(), numSamples);
        ...
        else
        {
            ...
            AudioBuffer<NumericType> numericData ((i32) block.getNumChannels(),
                                                  (i32) (block.getNumSamples() * numSIMDElements));

            for (i32 c = 0; c < numericData.getNumChannels(); ++c)
            ...
            auto lastUnchangedIndexBeforeCopiedRange = (i32) ((numSIMDElements * 2) - 1);
            auto firstUnchangedIndexAfterCopiedRange = (i32) ((numSIMDElements * 4) + 1);
            ...
            expect (SampleType (numericData.getSample (0, 2 * (i32) numSIMDElements)) == block.getSample (0, 1));
            expect (SampleType (numericData.getSample (1, 3 * (i32) numSIMDElements)) == block.getSample (1, 2));
            ...
            expectEquals (block.getSample (0, 2).get (0), numericData.getSample (0, (i32) numSIMDElements));
            expectEquals (block.getSample (1, 3).get (0), numericData.getSample (1, (i32) (numSIMDElements * 2)));

            if (numSIMDElements > 1)
            {
                expectEquals (block.getSample (0, 2).get (1), numericData.getSample (0, (i32) (numSIMDElements + 1)));
                expectEquals (block.getSample (1, 3).get (1), numericData.getSample (1, (i32) ((numSIMDElements * 2) + 1)));
            }
        }
       #endif
    }
    ...
    z0 resetBlocks()
    {
        ...
        for (size_t c = 0; c < block.getNumChannels(); ++c)
        {
            ...
            {
                block.setSample ((i32) c, (i32) i, value);
    ...
    //==============================================================================
    static SampleType* allocateAlignedMemory (i32 numSamplesToAllocate)
    {
        auto alignmentLowerBound = std::alignment_of_v<SampleType>;
       #if ! DRX_WINDOWS
        alignmentLowerBound = jmax (sizeof (uk), alignmentLowerBound);
       #endif
    ...
    z0 deallocateAlignedMemory (uk address)
    {
       #if DRX_WINDOWS
    ...
    //==============================================================================
    static constexpr i32 numChannels = 2, numSamples = 6;
    std::array<SampleType*, numChannels> data, otherData;
    AudioBlock<SampleType> block, otherBlock;
};

static AudioBlockUnitTests<f32> audioBlockFloatUnitTests;
static AudioBlockUnitTests<f64> audioBlockDoubleUnitTests;

#if DRX_USE_SIMD
static AudioBlockUnitTests<SIMDRegister<f32>> audioBlockSIMDFloatUnitTests;
static AudioBlockUnitTests<SIMDRegister<f64>> audioBlockSIMDDoubleUnitTests;
#endif
