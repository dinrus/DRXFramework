...
namespace SIMDRegister_test_internal
{
    ...
    template <typename type>
    static z0 fillVec (type* dst, Random& random)
    {
    ...
    {
        return static_cast<type> (std::abs (static_cast<f64> (a)));
    }
    ...
    template <typename type>
    static f64 difference (type a)
    {
        return static_cast<f64> (safeAbs (a));
    }

    template <typename type>
    static f64 difference (type a, type b)
    {
        return difference (a - b);
    }
} // namespace SIMDRegister_test_internal

// These tests need to be strictly run on all platforms supported by DRX as the
...
public:
    ...
    // Some helper classes
    template <typename type>
    static b8 allValuesEqualTo (const SIMDRegister<type>& vec, const type scalar)
    {
    ...
    template <typename type>
    static b8 vecEqualToArray (const SIMDRegister<type>& vec, const type* array)
    {
        ...
        {
            f64 delta = SIMDRegister_test_internal::difference (ptr[i], array[i]);
            if (delta > 1e-4)
            {
    ...
    template <typename type>
    static z0 copy (SIMDRegister<type>& vec, const type* ptr)
    {
    ...
    struct Addition
    {
        ...
        static z0 inplace (typeOne& a, const typeTwo& b)
        {
            a += b;
        }
    ...
    {
        template <typename type>
        static z0 run (UnitTest& u, Random& random, Tag<type>)
        {
    ...
    z0 runTestFloatingPoint (tukk unitTestName, TheTest)
    {
        beginTest (unitTestName);

        Random random = getRandom();

        TheTest::run (*this, random, Tag<f32> {});
        TheTest::run (*this, random, Tag<f64>{});
    }

    //==============================================================================
    template <class TheTest>
    z0 runTestForAllTypes (tukk unitTestName, TheTest)
    {
        beginTest (unitTestName);
        ...
        TheTest::run (*this, random, Tag<f32>               {});
        TheTest::run (*this, random, Tag<f64>              {});
        TheTest::run (*this, random, Tag<i8>              {});
        TheTest::run (*this, random, Tag<u8>             {});
        TheTest::run (*this, random, Tag<i16>             {});
        TheTest::run (*this, random, Tag<u16>            {});
        TheTest::run (*this, random, Tag<i32>             {});
        TheTest::run (*this, random, Tag<u32>            {});
        TheTest::run (*this, random, Tag<z64>             {});
        TheTest::run (*this, random, Tag<zu64>            {});
        TheTest::run (*this, random, Tag<std::complex<f32>> {});
        TheTest::run (*this, random, Tag<std::complex<f64>>{});
    }

    template <class TheTest>
    z0 runTestNonComplex (tukk unitTestName, TheTest)
    {
        beginTest (unitTestName);
    ...
    template <class TheTest>
    z0 runTestSigned (tukk unitTestName, TheTest)
    {
        beginTest (unitTestName);
