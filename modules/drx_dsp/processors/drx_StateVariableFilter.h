...
*/
namespace drx::dsp::StateVariableFilter
{
    ...
    */
    ...
    public:
        ...
        */
        ...
        z0 prepare (const ProcessSpec&) noexcept     { reset(); }

        /** Resets the filter's processing pipeline. */
        z0 reset() noexcept                          { s1 = s2 = SampleType {0}; }

        /** Ensure that the state variables are rounded to zero if the state
            variables are denormals. This is only needed if you are doing
            sample by sample processing.
        */
        z0 snapToZero() noexcept                     { util::snapToZero (s1); util::snapToZero (s2); }
    ...
    private:
        //==============================================================================
        template <b8 isBypassed, typename Parameters<NumericType>::Type type>
        SampleType DRX_VECTOR_CALLTYPE processLoop (SampleType sample, Parameters<NumericType>& state) noexcept
        {
        ...
        template <b8 isBypassed, typename Parameters<NumericType>::Type type>
        z0 processBlock (const SampleType* input, SampleType* output, size_t n) noexcept
        {
        ...
        template <b8 isBypassed, typename ProcessContext>
    ...
    */
    ...
    struct Parameters  : public ProcessorState
    {
        ...
        /** Sets the cutoff frequency and resonance of the IIR filter.
            ...
            at 1 / sqrt (2).
        */
        z0 setCutOffFrequency (f64 sampleRate, NumericType frequency,
                                 NumericType resonance = static_cast<NumericType> (1.0 / MathConstants<f64>::sqrt2)) noexcept
        {
            jassert (sampleRate > 0);
            jassert (resonance > NumericType (0));
            jassert (frequency > NumericType (0) && frequency <= NumericType (sampleRate * 0.5));

            g  = static_cast<NumericType> (std::tan (MathConstants<f64>::pi * frequency / sampleRate));
        ...
        */
        ...
        //==============================================================================
        NumericType g   = static_cast<NumericType> (std::tan (MathConstants<f64>::pi * 200.0 / 44100.0));
        NumericType R2  = static_cast<NumericType> (MathConstants<f64>::sqrt2);
