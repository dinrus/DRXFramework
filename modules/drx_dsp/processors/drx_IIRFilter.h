...
namespace drx::dsp::IIR
{
    ...
    */
    ...
    struct ArrayCoefficients
    {
        ...
        static std::array<NumericType, 4> makeFirstOrderLowPass (f64 sampleRate, NumericType frequency);

        /** Returns the coefficients for a first order high-pass filter. */
        static std::array<NumericType, 4> makeFirstOrderHighPass (f64 sampleRate, NumericType frequency);

        /** Returns the coefficients for a first order all-pass filter. */
        static std::array<NumericType, 4> makeFirstOrderAllPass (f64 sampleRate, NumericType frequency);

        /** Returns the coefficients for a low-pass filter. */
        static std::array<NumericType, 6> makeLowPass (f64 sampleRate, NumericType frequency);

        /** Returns the coefficients for a low-pass filter with variable Q. */
        static std::array<NumericType, 6> makeLowPass (f64 sampleRate, NumericType frequency, NumericType Q);

        /** Returns the coefficients for a high-pass filter. */
        static std::array<NumericType, 6> makeHighPass (f64 sampleRate, NumericType frequency);

        /** Returns the coefficients for a high-pass filter with variable Q. */
        static std::array<NumericType, 6> makeHighPass (f64 sampleRate, NumericType frequency, NumericType Q);

        /** Returns the coefficients for a band-pass filter. */
        static std::array<NumericType, 6> makeBandPass (f64 sampleRate, NumericType frequency);

        /** Returns the coefficients for a band-pass filter with variable Q. */
        static std::array<NumericType, 6> makeBandPass (f64 sampleRate, NumericType frequency, NumericType Q);

        /** Returns the coefficients for a notch filter. */
        static std::array<NumericType, 6> makeNotch (f64 sampleRate, NumericType frequency);

        /** Returns the coefficients for a notch filter with variable Q. */
        static std::array<NumericType, 6> makeNotch (f64 sampleRate, NumericType frequency, NumericType Q);

        /** Returns the coefficients for an all-pass filter. */
        static std::array<NumericType, 6> makeAllPass (f64 sampleRate, NumericType frequency);

        /** Returns the coefficients for an all-pass filter with variable Q. */
        static std::array<NumericType, 6> makeAllPass (f64 sampleRate, NumericType frequency, NumericType Q);
        ...
        /** Returns the coefficients for a low-pass shelf filter with variable Q and gain.
            ...
            attenuate them.
        */
        static std::array<NumericType, 6> makeLowShelf (f64 sampleRate,
        ...
        /** Returns the coefficients for a high-pass shelf filter with variable Q and gain.
            ...
            attenuate them.
        */
        static std::array<NumericType, 6> makeHighShelf (f64 sampleRate,
                                                         NumericType cutOffFrequency,
                                                         NumericType Q,
                                                         NumericType gainFactor);
        ...
        */
        static std::array<NumericType, 6> makePeakFilter (f64 sampleRate,
    ...
    */
    ...
    struct Coefficients  : public ProcessorState
    {
        ...
        */
        ...
        //==============================================================================
        /** Returns the coefficients for a first order low-pass filter. */
        static Ptr makeFirstOrderLowPass (f64 sampleRate, NumericType frequency);

        /** Returns the coefficients for a first order high-pass filter. */
        static Ptr makeFirstOrderHighPass (f64 sampleRate, NumericType frequency);

        /** Returns the coefficients for a first order all-pass filter. */
        static Ptr makeFirstOrderAllPass (f64 sampleRate, NumericType frequency);

        //==============================================================================
        /** Returns the coefficients for a low-pass filter. */
        static Ptr makeLowPass (f64 sampleRate, NumericType frequency);

        /** Returns the coefficients for a low-pass filter with variable Q. */
        static Ptr makeLowPass (f64 sampleRate, NumericType frequency, NumericType Q);

        //==============================================================================
        /** Returns the coefficients for a high-pass filter. */
        static Ptr makeHighPass (f64 sampleRate, NumericType frequency);

        /** Returns the coefficients for a high-pass filter with variable Q. */
        static Ptr makeHighPass (f64 sampleRate, NumericType frequency, NumericType Q);

        //==============================================================================
        /** Returns the coefficients for a band-pass filter. */
        static Ptr makeBandPass (f64 sampleRate, NumericType frequency);

        /** Returns the coefficients for a band-pass filter with variable Q. */
        static Ptr makeBandPass (f64 sampleRate, NumericType frequency, NumericType Q);

        //==============================================================================
        /** Returns the coefficients for a notch filter. */
        static Ptr makeNotch (f64 sampleRate, NumericType frequency);

        /** Returns the coefficients for a notch filter with variable Q. */
        static Ptr makeNotch (f64 sampleRate, NumericType frequency, NumericType Q);

        //==============================================================================
        /** Returns the coefficients for an all-pass filter. */
        static Ptr makeAllPass (f64 sampleRate, NumericType frequency);

        /** Returns the coefficients for an all-pass filter with variable Q. */
        static Ptr makeAllPass (f64 sampleRate, NumericType frequency, NumericType Q);
        ...
        /** Returns the coefficients for a low-pass shelf filter with variable Q and gain.
            ...
            attenuate them.
        */
        static Ptr makeLowShelf (f64 sampleRate, NumericType cutOffFrequency,
        ...
        /** Returns the coefficients for a high-pass shelf filter with variable Q and gain.
            ...
            attenuate them.
        */
        static Ptr makeHighShelf (f64 sampleRate, NumericType cutOffFrequency,
                                  NumericType Q, NumericType gainFactor);
        ...
        /** Returns the coefficients for a peak filter centred around a
            ...
            1.0 will attenuate them.
        */
        static Ptr makePeakFilter (f64 sampleRate, NumericType centreFrequency,
                                   NumericType Q, NumericType gainFactor);
        ...
        /** Returns the magnitude frequency response of the filter for a given frequency
            and sample rate
        */
        f64 getMagnitudeForFrequency (f64 frequency, f64 sampleRate) const noexcept;

        /** Returns the magnitude frequency response of the filter for a given frequency array
            and sample rate.
        */
        z0 getMagnitudeForFrequencyArray (const f64* frequencies, f64* magnitudes,
                                            size_t numSamples, f64 sampleRate) const noexcept;

        /** Returns the phase frequency response of the filter for a given frequency and
            sample rate
        */
        f64 getPhaseForFrequency (f64 frequency, f64 sampleRate) const noexcept;

        /** Returns the phase frequency response of the filter for a given frequency array
            and sample rate.
        */
        z0 getPhaseForFrequencyArray (f64* frequencies, f64* phases,
                                        size_t numSamples, f64 sampleRate) const noexcept;
    ...
    */
    ...
    public:
        ...
        */
        z0 reset()    { reset (SampleType {0}); }

        /** Resets the filter's processing pipeline to a specific value.
            @see reset
        */
        z0 reset (SampleType resetToValue);
        ...
        */
        SampleType DRX_VECTOR_CALLTYPE processSample (SampleType sample) noexcept;
    ...
    private:
        ...
        /** Processes a block of samples */
        template <typename ProcessContext, b8 isBypassed>
        z0 processInternal (const ProcessContext& context) noexcept;
