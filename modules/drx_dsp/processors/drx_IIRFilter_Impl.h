...
Coefficients<NumericType>& Coefficients<NumericType>::assignImpl (const NumericType* values)
{
    ...
    coefficients.ensureStorageAllocated ((i32) jmax ((size_t) 8, Num));
...
template <typename SampleType>
z0 Filter<SampleType>::reset (SampleType resetToValue)
{
...
template <typename SampleType>
z0 Filter<SampleType>::prepare (const ProcessSpec&) noexcept     { reset(); }

template <typename SampleType>
template <typename ProcessContext, b8 bypassed>
z0 Filter<SampleType>::processInternal (const ProcessContext& context) noexcept
{
...
template <typename SampleType>
SampleType DRX_VECTOR_CALLTYPE Filter<SampleType>::processSample (SampleType sample) noexcept
{
    check();
...
template <typename SampleType>
z0 Filter<SampleType>::snapToZero() noexcept
{
    for (size_t i = 0; i < order; ++i)
