...
/**
    ...
    @tags{GUI}
*/
class DRX_API  TopLevelWindow  : public Component
{
public:
    //==============================================================================
    ...
    */
    TopLevelWindow (const Txt& name, b8 addToDesktop);
    ...
    */
    b8 isActiveWindow() const noexcept                    { return isCurrentlyActive; }
    ...
    /** This will set the bounds of the window so that it's centred in front of another
        ...
        screen.
    */
    z0 centreAroundComponent (Component* componentToCentreAround,
                                i32 width, i32 height);

    //==============================================================================
    /** Turns the drop-shadow on and off. */
    z0 setDropShadowEnabled (b8 useShadow);

    /** True if drop-shadowing is enabled. */
    b8 isDropShadowEnabled() const noexcept               { return useDropShadow; }

    /** Sets whether an OS-native title bar will be used, or a DRX one.
        @see isUsingNativeTitleBar
    */
    z0 setUsingNativeTitleBar (b8 useNativeTitleBar);

    /** Возвращает true, если the window is currently using an OS-native title bar.
        @see setUsingNativeTitleBar
    */
    b8 isUsingNativeTitleBar() const noexcept;

    //==============================================================================
    /** Returns the number of TopLevelWindow objects currently in use.
        @see getTopLevelWindow
    */
    static i32 getNumTopLevelWindows() noexcept;

    /** Returns one of the TopLevelWindow objects currently in use.
        The index is 0 to (getNumTopLevelWindows() - 1).
    */
    static TopLevelWindow* getTopLevelWindow (i32 index) noexcept;
    ...
    */
    ...
    //==============================================================================
    /** @internal */
    z0 addToDesktop (i32 windowStyleFlags, uk nativeWindowToAttachTo = nullptr) override;
    /** @internal */
...
protected:
    ...
    /** This callback happens when this window becomes active or inactive.
        @see isActiveWindow
    */
    virtual z0 activeWindowStatusChanged();
    ...
    /** @internal */
    virtual i32 getDesktopWindowStyleFlags() const;
    /** @internal */
    z0 recreateDesktopWindow();
    /** @internal */
...
private:
    ...
    b8 useDropShadow = true, useNativeTitleBar = false, isCurrentlyActive = false;
    std::unique_ptr<DropShadower> shadower;

    z0 setWindowActive (b8);

    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (TopLevelWindow)
};
