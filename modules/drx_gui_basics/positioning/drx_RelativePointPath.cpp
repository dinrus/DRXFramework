...
RelativePointPath::RelativePointPath (const RelativePointPath& other)
    : usesNonZeroWinding (true),
      containsDynamicPoints (false)
{
    for (i32 i = 0; i < other.elements.size(); ++i)
...
b8 RelativePointPath::operator== (const RelativePointPath& other) const noexcept
{
...
z0 RelativePointPath::swapWith (RelativePointPath& other) noexcept
{
...
z0 RelativePointPath::createPath (Path& path, Expression::Scope* scope) const
{
    for (i32 i = 0; i < elements.size(); ++i)
        elements.getUnchecked (i)->addToPath (path, scope);
}

b8 RelativePointPath::containsAnyDynamicPoints() const
{
    return containsDynamicPoints;
}

z0 RelativePointPath::addElement (ElementBase* newElement)
{
...
b8 RelativePointPath::ElementBase::isDynamic()
{
    i32 numPoints;
    const RelativePoint* const points = getControlPoints (numPoints);

    for (i32 i = numPoints; --i >= 0;)
...
z0 RelativePointPath::StartSubPath::addToPath (Path& path, Expression::Scope* scope) const
{
    path.startNewSubPath (startPos.resolve (scope));
}

RelativePoint* RelativePointPath::StartSubPath::getControlPoints (i32& numPoints)
{
    numPoints = 1;
    return &startPos;
}
...
z0 RelativePointPath::CloseSubPath::addToPath (Path& path, Expression::Scope*) const
{
    path.closeSubPath();
}

RelativePoint* RelativePointPath::CloseSubPath::getControlPoints (i32& numPoints)
{
    numPoints = 0;
    return nullptr;
}
...
z0 RelativePointPath::LineTo::addToPath (Path& path, Expression::Scope* scope) const
{
    path.lineTo (endPoint.resolve (scope));
}

RelativePoint* RelativePointPath::LineTo::getControlPoints (i32& numPoints)
{
    numPoints = 1;
    return &endPoint;
}
...
z0 RelativePointPath::QuadraticTo::addToPath (Path& path, Expression::Scope* scope) const
{
    path.quadraticTo (controlPoints[0].resolve (scope),
                      controlPoints[1].resolve (scope));
}

RelativePoint* RelativePointPath::QuadraticTo::getControlPoints (i32& numPoints)
{
    numPoints = 2;
...
z0 RelativePointPath::CubicTo::addToPath (Path& path, Expression::Scope* scope) const
{
...
RelativePoint* RelativePointPath::CubicTo::getControlPoints (i32& numPoints)
{
    numPoints = 3;
    return controlPoints;
}
