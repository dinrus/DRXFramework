...
{
    static Txt getStateId (const ValueTree& state)
    {
        return state [ComponentBuilder::idProperty].toString();
    }

    static Component* removeComponentWithID (OwnedArray<Component>& components, const Txt& compId)
    {
    ...
    static Component* findComponentWithID (Component& c, const Txt& compId)
    {
        jassert (compId.isNotEmpty());
        if (c.getComponentID() == compId)
            return &c;
    ...
    static z0 updateComponent (ComponentBuilder& builder, const ValueTree& state)
    {
        if (Component* topLevelComp = builder.getManagedComponent())
        {
            ...
            const Txt uid (getStateId (state));
...
z0 ComponentBuilder::registerTypeHandler (ComponentBuilder::TypeHandler* const type)
{
...
i32 ComponentBuilder::getNumHandlers() const noexcept
{
    return types.size();
}

ComponentBuilder::TypeHandler* ComponentBuilder::getHandler (i32k index) const noexcept
{
    return types [index];
}

z0 ComponentBuilder::registerStandardComponentTypes()
{
}

z0 ComponentBuilder::setImageProvider (ImageProvider* newImageProvider) noexcept
{
...
z0 ComponentBuilder::valueTreePropertyChanged (ValueTree& tree, const Identifier&)
{
    ComponentBuilderHelpers::updateComponent (*this, tree);
}

z0 ComponentBuilder::valueTreeChildAdded (ValueTree& tree, ValueTree&)
{
    ComponentBuilderHelpers::updateComponent (*this, tree);
}

z0 ComponentBuilder::valueTreeChildRemoved (ValueTree& tree, ValueTree&, i32)
{
    ComponentBuilderHelpers::updateComponent (*this, tree);
}

z0 ComponentBuilder::valueTreeChildOrderChanged (ValueTree& tree, i32, i32)
{
    ComponentBuilderHelpers::updateComponent (*this, tree);
}

z0 ComponentBuilder::valueTreeParentChanged (ValueTree& tree)
{
    ComponentBuilderHelpers::updateComponent (*this, tree);
}
...
z0 ComponentBuilder::updateChildComponents (Component& parent, const ValueTree& children)
{
    ...
    if (componentsInOrder.size() > 0)
    {
        ...
        for (i32 i = componentsInOrder.size() - 1; --i >= 0;)
