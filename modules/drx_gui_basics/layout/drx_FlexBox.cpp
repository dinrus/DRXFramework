...
struct FlexBoxLayoutCalculation
{
    ...
    struct ItemWithState
    {
        ...
        z0 resetItemLockedSize() noexcept
        {
    ...
    ItemWithState& getItem (i32 x, i32 y) const noexcept     { return *lineItems[y * numItems + x]; }

    static b8 isAuto (Coord value) noexcept
    {
        return exactlyEqual (value, static_cast<Coord> (FlexItem::autoValue));
    }

    static b8 isAssigned (Coord value) noexcept
    {
    ...
    //==============================================================================
    b8 isSingleLine() const { return owner.flexWrap == FlexBox::Wrap::noWrap; }
    ...
    z0 resolveFlexibleLengths() noexcept
    {
        for (i32 row = 0; row < numberOfRows; ++row)
        {
            resetRowItems (row);

            for (i32 maxLoops = numItems; --maxLoops >= 0;)
            {
    ...
    z0 resolveAutoMarginsOnMainAxis() noexcept
    {
        for (i32 row = 0; row < numberOfRows; ++row)
        {
            Coord allFlexGrow = 0;
            ...
            for (i32 column = 0; column < numColumns; ++column)
            {
    ...
    z0 calculateCrossSizesByLine() noexcept
    {
    ...
    z0 calculateCrossSizeOfAllItems() noexcept
    {
        for (i32 row = 0; row < numberOfRows; ++row)
        {
            const auto numColumns = lineInfo[row].numItems;
    ...
    z0 alignLinesPerAlignContent() noexcept
    {
        ...
        {
            for (i32 row = 0; row < numberOfRows; ++row)
                for (i32 row2 = row; row2 < numberOfRows; ++row2)
        ...
        else
        {
            ...
            else if (owner.alignContent == FlexBox::AlignContent::spaceBetween)
            {
                ...
                for (i32 row = 1; row < numberOfRows; ++row)
    ...
    z0 resolveAutoMarginsOnCrossAxis() noexcept
    {
        for (i32 row = 0; row < numberOfRows; ++row)
        {
            const auto numColumns = lineInfo[row].numItems;
    ...
    z0 alignItemsInCrossAxisInLinesPerAlignSelf() noexcept
    {
        for (i32 row = 0; row < numberOfRows; ++row)
        {
            const auto numColumns = lineInfo[row].numItems;
    ...
    z0 alignItemsByJustifyContent() noexcept
    {
        ...
        for (i32 row = 0; row < numberOfRows; ++row)
        {
            ...
            for (i32 column = 0; column < numColumns; ++column)
            {
                ...
                item.item->currentBounds.setPosition (isRowDirection ? (f32) (x + item.lockedMarginLeft)
                                                                     : (f32) item.lockedMarginLeft,
                                                      isRowDirection ? (f32) item.lockedMarginTop
                                                                     : (f32) (x + item.lockedMarginTop));
    ...
    z0 layoutAllItems() noexcept
    {
        for (i32 row = 0; row < numberOfRows; ++row)
        {
            ...
            for (i32 column = 0; column < numColumns; ++column)
            {
                ...
                if (isRowDirection)
                    item.item->currentBounds.setY ((f32) (lineY + item.lockedMarginTop));
                else
                    item.item->currentBounds.setX ((f32) (lineY + item.lockedMarginLeft));

                item.item->currentBounds.setSize ((f32) item.lockedWidth,
                                                  (f32) item.lockedHeight);
            }
        }
...
private:
    z0 resetRowItems (i32k row) noexcept
    {
        const auto numColumns = lineInfo[row].numItems;
    ...
    z0 resetUnlockedRowItems (i32k row) noexcept
    {
        const auto numColumns = lineInfo[row].numItems;
    ...
    z0 resetItem (ItemWithState& item) noexcept
    {
        item.locked = false;
    ...
    b8 layoutRowItems (i32k row) noexcept
    {
        const auto numColumns = lineInfo[row].numItems;
        ...
        const b8 positiveFlexibility = difference > 0;
    ...
    z0 recalculateTotalItemLengthPerLineArray() noexcept
    {
        for (i32 row = 0; row < numberOfRows; ++row)
        {
    ...
    z0 reverseLocations() noexcept
    {
        if (owner.flexDirection == FlexBox::Direction::rowReverse)
        {
            for (auto& item : owner.items)
                item.currentBounds.setX ((f32) (containerLineLength - item.currentBounds.getRight()));
        }
        else if (owner.flexDirection == FlexBox::Direction::columnReverse)
        {
            for (auto& item : owner.items)
                item.currentBounds.setY ((f32) (containerLineLength - item.currentBounds.getBottom()));
        }
    }

    z0 reverseWrap() noexcept
    {
        if (owner.flexWrap == FlexBox::Wrap::wrapReverse)
        {
            ...
            {
                for (auto& item : owner.items)
                    item.currentBounds.setY ((f32) (containerCrossLength - item.currentBounds.getBottom()));
            }
            else
            {
                for (auto& item : owner.items)
                    item.currentBounds.setX ((f32) (containerCrossLength - item.currentBounds.getRight()));
            }
        }
    }
    ...
    b8 addToItemLength (ItemWithState& item, const Coord length, i32 row) const noexcept
    {
        b8 ok = false;
...
z0 FlexBox::performLayout (Rectangle<f32> targetArea)
{
    if (! items.isEmpty())
    {
        ...
        for (auto& item : items)
        {
            ...
            if (auto* comp = item.associatedComponent)
                comp->setBounds (Rectangle<i32>::leftTopRightBottom ((i32) item.currentBounds.getX(),
                                                                     (i32) item.currentBounds.getY(),
                                                                     (i32) item.currentBounds.getRight(),
                                                                     (i32) item.currentBounds.getBottom()));
...
z0 FlexBox::performLayout (Rectangle<i32> targetArea)
{
...
FlexItem::FlexItem (f32 w, f32 h) noexcept                  : currentBounds (w, h), minWidth (w), minHeight (h) {}
FlexItem::FlexItem (f32 w, f32 h, Component& c) noexcept    : FlexItem (w, h)  { associatedComponent = &c; }
FlexItem::FlexItem (f32 w, f32 h, FlexBox& fb) noexcept     : FlexItem (w, h)  { associatedFlexBox = &fb; }
...
FlexItem::Margin::Margin (f32 v) noexcept                             : left (v), right (v), top (v), bottom (v) {}
FlexItem::Margin::Margin (f32 t, f32 r, f32 b, f32 l) noexcept  : left (l), right (r), top (t), bottom (b) {}

//==============================================================================
FlexItem FlexItem::withFlex (f32 newFlexGrow) const noexcept
{
    auto fi = *this;
...
FlexItem FlexItem::withFlex (f32 newFlexGrow, f32 newFlexShrink) const noexcept
{
...
FlexItem FlexItem::withFlex (f32 newFlexGrow, f32 newFlexShrink, f32 newFlexBasis) const noexcept
{
...
FlexItem FlexItem::withWidth (f32 newWidth) const noexcept         { auto fi = *this; fi.width = newWidth; return fi; }
FlexItem FlexItem::withMinWidth (f32 newMinWidth) const noexcept   { auto fi = *this; fi.minWidth = newMinWidth; return fi; }
FlexItem FlexItem::withMaxWidth (f32 newMaxWidth) const noexcept   { auto fi = *this; fi.maxWidth = newMaxWidth; return fi; }

FlexItem FlexItem::withMinHeight (f32 newMinHeight) const noexcept { auto fi = *this; fi.minHeight = newMinHeight; return fi; }
FlexItem FlexItem::withMaxHeight (f32 newMaxHeight) const noexcept { auto fi = *this; fi.maxHeight = newMaxHeight; return fi; }
FlexItem FlexItem::withHeight (f32 newHeight) const noexcept       { auto fi = *this; fi.height = newHeight; return fi; }

FlexItem FlexItem::withMargin (Margin m) const noexcept              { auto fi = *this; fi.margin = m; return fi; }
FlexItem FlexItem::withOrder (i32 newOrder) const noexcept           { auto fi = *this; fi.order = newOrder; return fi; }
...
public:
    ...
    z0 runTest() override
    {
        ...
        const Rectangle<f32> rect (10.0f, 20.0f, 300.0f, 200.0f);
        ...
        {
            const auto test = [this, &doLayout] (Direction direction, AlignSelf alignment, Rectangle<f32> expectedBounds)
            {
                const auto flex = doLayout (direction, { drx::FlexItem{}.withAlignSelf (alignment) });
        ...
        beginTest ("flex item with specified width and height");
        {
            ...
            const auto test = [&] (Direction direction, AlignSelf alignment, Rectangle<f32> expectedBounds)
            {
                const auto flex = doLayout (direction, { drx::FlexItem().withAlignSelf (alignment)
                                                                         .withWidth (w)
                                                                         .withHeight (h) });
        ...
        beginTest ("flex item with oversized width and height");
        {
            ...
            const auto test = [this, &doLayout, &w, &h] (Direction direction, AlignSelf alignment, Rectangle<f32> expectedBounds)
            {
                const auto flex = doLayout (direction, { drx::FlexItem().withAlignSelf (alignment)
                                                                         .withWidth (w)
                                                                         .withHeight (h) });
            ...
            const Rectangle<f32> baseRow (rect.getX(), rect.getY(), rect.getWidth(), h);
            ...
            const Rectangle<f32> baseColumn (rect.getX(), rect.getY(), w, rect.getHeight());
        ...
        beginTest ("flex item with specified flex");
        {
            const auto test = [this, &doLayout] (Direction direction, AlignSelf alignment, Rectangle<f32> expectedBounds)
            {
                const auto flex = doLayout (direction, { drx::FlexItem().withAlignSelf (alignment).withFlex (1.0f) });
                expect (flex.items.getFirst().currentBounds == expectedBounds);
            };
        ...
        beginTest ("flex item with margin");
        {
            ...
            const auto test = [this, &doLayout, &margin] (Direction direction, AlignSelf alignment, Rectangle<f32> expectedBounds)
            {
                const auto flex = doLayout (direction, { drx::FlexItem().withAlignSelf (alignment).withMargin (margin) });
                expect (flex.items.getFirst().currentBounds == expectedBounds);
            };
        ...
        beginTest ("flex item with auto margin");
        {
            for (const auto& alignment : alignments)
            {
                ...
                {
                    const auto flex = doLayout (direction, { drx::FlexItem().withAlignSelf (alignment)
                                                                             .withMargin ((f32) FlexItem::autoValue) });
                    expect (flex.items.getFirst().currentBounds == Rectangle<f32> (rect.getCentre(), rect.getCentre()));
                }
            }

            const auto testTop = [this, &doLayout] (Direction direction, AlignSelf alignment, Rectangle<f32> expectedBounds)
            {
                const auto flex = doLayout (direction, { drx::FlexItem().withAlignSelf (alignment)
                                                                         .withMargin ({ (f32) FlexItem::autoValue, 0.0f, 0.0f, 0.0f }) });
                expect (flex.items.getFirst().currentBounds == expectedBounds);
            };
            ...
            const auto testBottom = [this, &doLayout] (Direction direction, AlignSelf alignment, Rectangle<f32> expectedBounds)
            {
                const auto flex = doLayout (direction, { drx::FlexItem().withAlignSelf (alignment)
                                                                         .withMargin ({ 0.0f, 0.0f, (f32) FlexItem::autoValue, 0.0f }) });
                expect (flex.items.getFirst().currentBounds == expectedBounds);
            };
            ...
            const auto testLeft = [this, &doLayout] (Direction direction, AlignSelf alignment, Rectangle<f32> expectedBounds)
            {
                const auto flex = doLayout (direction, { drx::FlexItem().withAlignSelf (alignment)
                                                                         .withMargin ({ 0.0f, 0.0f, 0.0f, (f32) FlexItem::autoValue }) });
                expect (flex.items.getFirst().currentBounds == expectedBounds);
            };
            ...
            const auto testRight = [this, &doLayout] (Direction direction, AlignSelf alignment, Rectangle<f32> expectedBounds)
            {
                const auto flex = doLayout (direction, { drx::FlexItem().withAlignSelf (alignment)
                                                                         .withMargin ({ 0.0f, (f32) FlexItem::autoValue, 0.0f, 0.0f }) });
                expect (flex.items.getFirst().currentBounds == expectedBounds);
            };
        ...
        beginTest ("in a multiline layout, items too large to fit on the main axis are given a line to themselves");
        {
            ...
            for (const auto alignment : alignments)
            {
                ...
                expect (flex.items[0].currentBounds == Rectangle<f32> (rect.getX(), rect.getY(), spacer, spacer));
                expect (flex.items[1].currentBounds == Rectangle<f32> (rect.getX(), rect.getY() + spacer, rect.getWidth(), rect.getHeight()));
                expect (flex.items[2].currentBounds == Rectangle<f32> (rect.getX(), rect.getBottom() + spacer, 10.0f, 10.0f));
            }
        }
    }
};
