...
struct AccessibilityHelpers
{
    ...
    static z0 notifyAccessibilityEvent (const AccessibilityHandler&, Event);

    static Txt getApplicationOrPluginName()
    {
       #if defined (DrxPlugin_Name)
        return DrxPlugin_Name;
       #else
        if (auto* app = DRXApplicationBase::getInstance())
            return app->getApplicationName();

        return "DRX Application";
       #endif
    }
