...
public:
    ...
    SelectedItemSet& operator= (const SelectedItemSet& other)
    {
        if (selectedItems != other.selectedItems)
        {
            ...
            for (i32 i = selectedItems.size(); --i >= 0;)
    ...
    */
    z0 selectOnly (ParameterType item)
    {
        if (isSelected (item))
        {
            for (i32 i = selectedItems.size(); --i >= 0;)
    ...
    */
    z0 addToSelection (ParameterType item)
    {
        if (! isSelected (item))
        {
            changed();
    ...
    */
    z0 addToSelectionBasedOnModifiers (ParameterType item,
    ...
    */
    b8 addToSelectionOnMouseDown (ParameterType item,
                                    ModifierKeys modifiers)
    {
        if (isSelected (item))
    ...
    */
    z0 addToSelectionOnMouseUp (ParameterType item,
                                  ModifierKeys modifiers,
                                  const b8 wasItemDragged,
                                  const b8 resultOfMouseDownSelectMethod)
    {
    ...
    /** Deselects an item. */
    z0 deselect (ParameterType item)
    {
        i32k i = selectedItems.indexOf (item);
    ...
    /** Returns the number of currently selected items.
        @see getSelectedItem
    */
    i32 getNumSelected() const noexcept                         { return selectedItems.size(); }
    ...
    /** Returns one of the currently selected items.
        ...
        @see getNumSelected
    */
    SelectableItemType getSelectedItem (i32k index) const  { return selectedItems [index]; }

    /** True if this item is currently selected. */
    b8 isSelected (ParameterType item) const noexcept         { return selectedItems.contains (item); }
    ...
    */
    virtual z0 itemSelected (SelectableItemType)              {}
    ...
    */
    virtual z0 itemDeselected (SelectableItemType)            {}
    ...
    */
    ...
    /** Used internally, but can be called to force a change message to be sent
        to the ChangeListeners.
    */
    z0 changed (const b8 synchronous)
    {
        if (synchronous)
...
private:
    ...
    DRX_LEAK_DETECTOR (SelectedItemSet<SelectableItemType>)
};
