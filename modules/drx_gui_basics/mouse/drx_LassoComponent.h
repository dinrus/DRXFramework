...
public:
    ...
    /** Returns the set of items that lie within a given lassoable region.
        ...
        component itself).
    */
    virtual z0 findLassoItemsInArea (Array<SelectableItemType>& itemsFound,
                                       const Rectangle<i32>& area) = 0;
...
public:
    ...
    */
    z0 beginLasso (const MouseEvent& e, LassoSource<SelectableItemType>* lassoSource)
    {
    ...
    */
    z0 dragLasso (const MouseEvent& e)
    {
        if (source != nullptr)
        {
            setBounds (Rectangle<i32> (dragStartPos, e.getPosition()));
            setVisible (true);
    ...
    */
    enum ColorIds
    {
        lassoFillColorId       = 0x1000440, /**< The colour to fill the lasso rectangle with. */
        lassoOutlineColorId    = 0x1000441, /**< The colour to draw the outline with. */
    };
    ...
    /** @internal */
    b8 hitTest (i32, i32) override        { return false; }

private:
    //==============================================================================
    Array<SelectableItemType> originalSelection;
    LassoSource<SelectableItemType>* source = nullptr;
    Point<i32> dragStartPos;

    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (LassoComponent)
};
