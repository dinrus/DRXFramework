...
/**
    ...
    @tags{GUI}
*/
class DRX_API  DragAndDropContainer
{
public:
    //==============================================================================
    ...
    */
    ...
    /** Begins a drag-and-drop operation.
        ...
        @param allowDraggingToOtherDrxWindows   if true, the dragged component will appear as a desktop
        ...
        @param inputSourceCausingDrag            the mouse input source which started the drag. When calling
                                                 ...
                                                 MouseEvent::source to this method. If this param is nullptr then DRX
                                                 ...
                                                 then DRX will use the mouseInputSource which is closest to the sourceComponent.
    */
    z0 startDragging (const var& sourceDescription,
                        Component* sourceComponent,
                        const ScaledImage& dragImage = ScaledImage(),
                        b8 allowDraggingToOtherDrxWindows = false,
                        const Point<i32>* imageOffsetFromMouse = nullptr,
                        ...
                        const MouseInputSource* inputSourceCausingDrag = nullptr)
    {
        startDragging (sourceDescription,
                       ...
                       allowDraggingToOtherDrxWindows,
                       imageOffsetFromMouse,
                       inputSourceCausingDrag);
    }

    /** Возвращает true, если something is currently being dragged. */
    b8 isDragAndDropActive() const;

    /** Returns the number of things currently being dragged. */
    i32 getNumCurrentDrags() const;
    ...
    */
    var getDragDescriptionForIndex (i32 index) const;
    ...
    */
    z0 setCurrentDragImage (const ScaledImage& newImage);
    ...
    z0 setCurrentDragImage (const Image& newImage) { setCurrentDragImage (ScaledImage (newImage)); }

    /** Same as the setCurrentDragImage() method but takes a touch index parameter.

        @see setCurrentDragImage
    */
    z0 setDragImageForIndex (i32 index, const ScaledImage& newImage);
    ...
    z0 setDragImageForIndex (i32 index, const Image& newImage) { setDragImageForIndex (index, ScaledImage (newImage)); }
    ...
    */
    ...
    /** This performs an asynchronous drag-and-drop of a set of files to some external
        ...
        @param sourceComponent  normally, DRX will assume that the component under the mouse is the source component
    ...
    */
    static b8 performExternalDragDropOfFiles (const StringArray& files, b8 canMoveFiles,
                                                Component* sourceComponent = nullptr,
                                                std::function<z0()> callback = nullptr);
    ...
    */
    static b8 performExternalDragDropOfText (const Txt& text, Component* sourceComponent = nullptr,
                                               std::function<z0()> callback = nullptr);
...
protected:
    /** Override this if you want to be able to perform an external drag of a set of files
        ...
        This method will be called when a drag operation moves outside the DRX window,
    ...
    */
    virtual b8 shouldDropFilesWhenDraggedExternally (const DragAndDropTarget::SourceDetails& sourceDetails,
    ...
    */
    virtual b8 shouldDropTextWhenDraggedExternally (const DragAndDropTarget::SourceDetails& sourceDetails,
                                                      Txt& text);

    /** Subclasses can override this to be told when a drag starts. */
    virtual z0 dragOperationStarted (const DragAndDropTarget::SourceDetails&);

    /** Subclasses can override this to be told when a drag finishes. */
    virtual z0 dragOperationEnded (const DragAndDropTarget::SourceDetails&);
...
private:
    ...
    b8 isAlreadyDragging (Component* sourceComponent) const noexcept;

    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (DragAndDropContainer)
};
