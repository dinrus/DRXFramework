...
public:
    //==============================================================================
    Native (FileChooser& fileChooser, i32 flags)    : owner (fileChooser)
    {
        if (currentFileChooser == nullptr)
        {
            ...
            tukk action = (selectsDirectories ? "android.intent.action.OPEN_DOCUMENT_TREE"
            ...
            if (! selectsDirectories)
            {
                ...
                else
                {
                    ...
                    if (mimeTypes.size() > 0)
                    {
                        ...
                        for (i32 i = 0; i < mimeTypes.size(); ++i)
                        {
    ...
    z0 launch() override
    {
        ...
        if (currentFileChooser != nullptr)
        {
            startAndroidActivityForResult (LocalRef<jobject> (env->NewLocalRef (intent.get())), /*READ_REQUEST_CODE*/ 42,
                                           [myself = WeakReference<Native> { this }] (i32 requestCode, i32 resultCode, LocalRef<jobject> intentData) mutable
                                           {
                                               if (myself != nullptr)
    ...
    z0 onActivityResult (i32 /*requestCode*/, i32 resultCode, const LocalRef<jobject>& intentData)
    {
        currentFileChooser = nullptr;
        auto* env = getEnv();
    ...
    static StringArray convertFiltersToMimeTypes (const Txt& fileFilters)
    {
        StringArray result;
...
private:
    DRX_DECLARE_WEAK_REFERENCEABLE (Native)
...
z0 FileChooser::registerCustomMimeTypeForFileExtension (const Txt& mimeType,
