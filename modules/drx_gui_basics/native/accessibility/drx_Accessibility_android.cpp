...
namespace
{
    constexpr i32 HOST_VIEW_ID = -1;

    constexpr i32 TYPE_VIEW_CLICKED                                = 0x00000001,
    ...
    constexpr i32 CONTENT_CHANGE_TYPE_SUBTREE             = 0x00000001,
                  CONTENT_CHANGE_TYPE_CONTENT_DESCRIPTION = 0x00000004;

    constexpr i32 ACTION_ACCESSIBILITY_FOCUS              = 0x00000040,
    ...
    constexpr i32 MOVEMENT_GRANULARITY_CHARACTER = 0x00000001,
    ...
    constexpr i32 ACCESSIBILITY_LIVE_REGION_POLITE = 0x00000001;
}
...
static z0 loadSDKDependentMethods()
{
    static b8 hasChecked = false;
...
static jobject makeAndroidRect (Rectangle<i32> r)
{
...
static jobject makeAndroidPoint (Point<i32> p)
{
...
{
public:
    static AccessibilityHandler* getAccessibilityHandlerForVirtualViewId (i32 virtualViewId)
    {
    ...
    i32 getVirtualViewId() const noexcept  { return virtualViewId; }

    jobject getNativeView (i32 viewId) const
    {
    ...
    z0 populateNodeInfo (jobject info, i32 infoVirtualViewId)
    {
        ...
        const ScopedValueSetter<b8> svs (inPopulateNodeInfo, true);
            ...
            else
            {
                if (auto* tableHeader = interface->getHeaderHandler())
                {
                    if (accessibilityHandler.getParent() == tableHeader)
                    {
                        ...
                        addCellInfo ({ 0, 1 }, { (i32) column, 1 }, IsHeader::yes);
                    }
                }
            }
        }
    ...
    b8 performAction (i32 action, jobject arguments)
    {
        switch (action)
        {
            ...
            case ACTION_SET_SELECTION:
            {
                if (auto* textInterface = accessibilityHandler.getTextInterface())
                {
                    ...
                    const auto selection = [&]() -> Range<i32>
                    {
                        ...
                        if (hasKey (selectionStartKey) && hasKey (selectionEndKey))
                        {
                            ...
                            return Range<i32>::between (start, end);
                        }
    ...
    b8 isInPopulateNodeInfo() const noexcept  { return inPopulateNodeInfo; }
    ...
    static z0 sendAccessibilityEventExtendedImpl (const AccessibilityHandler& handler,
                                                    i32 eventType,
    ...
    static z0 sendAccessibilityEventImpl (const AccessibilityHandler& handler, i32 eventType, i32 contentChangeTypes)
    {
...
private:
    static std::unordered_map<i32, AccessibilityHandler*> virtualViewIdMap;

    static i32 getNextVirtualViewId()
    {
    ...
    static i32 getVirtualViewIdForHandler (const AccessibilityHandler& handler)
    {
    ...
    {
        const auto valueString = [this]() -> Txt
        {
            if (auto* textInterface = accessibilityHandler.getTextInterface())
    ...
    b8 moveCursor (jobject arguments, b8 forwards)
    {
    ...
    i32k virtualViewId;
    std::optional<i32> nativeChildViewId;
    b8 inPopulateNodeInfo = false;

    //==============================================================================
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (AccessibilityNativeHandle)

    DRX_DECLARE_WEAK_REFERENCEABLE (AccessibilityNativeHandle)
};

std::unordered_map<i32, AccessibilityHandler*> AccessibilityNativeHandle::virtualViewIdMap;
...
z0 detail::AccessibilityHelpers::notifyAccessibilityEvent (const AccessibilityHandler& handler,
                                                             Event eventType)
{
...
z0 AccessibilityHandler::postAnnouncement (const Txt& announcementString,
