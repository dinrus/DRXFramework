...
static z0 juceFreeAccessibilityPlatformSpecificData (UIAccessibilityElement* element)
{
    if (auto* container = drx::getIvar<UIAccessibilityElement*> (element, "container"))
    {
...
struct SignatureSetSelectedTextRange            final : public Signature<z0 (UITextRange*)>                                                 { static auto sel() { return @selector (setSelectedTextRange:); } };
...
struct SignatureSetMarkedTextSelectedRange      final : public Signature<z0 (NSString*, NSRange)>                                           { static auto sel() { return @selector (setMarkedText:selectedRange:); } };
struct SignatureUnmarkText                      final : public Signature<z0()>                                                              { static auto sel() { return @selector (unmarkText); } };
...
struct SignatureSetMarkedTextStyle              final : public Signature<z0 (NSDictionary<NSAttributedStringKey, id>*)>                     { static auto sel() { return @selector (setMarkedTextStyle:); } };
...
struct SignatureReplaceRangeWithText            final : public Signature<z0 (UITextRange*, NSString*)>                                      { static auto sel() { return @selector (replaceRange:withText:); } };
struct SignatureSetBaseWritingDirection         final : public Signature<z0 (NSWritingDirection, UITextRange*)>                             { static auto sel() { return @selector (setBaseWritingDirection:forRange:); } };
...
struct SignatureSetInputDelegate                final : public Signature<z0 (id)>                                                           { static auto sel() { return @selector (setInputDelegate:); } };
...
private:
    ...
    public:
        AccessibilityContainer()
            : AccessibleObjCClass ("DRXUIAccessibilityContainer_")
        {
            ...
            addMethod (@selector (accessibilityDataTableCellElementForRow:column:), [] (id self, SEL, NSUInteger row, NSUInteger column) -> id
            {
                if (auto* tableHandler = detail::AccessibilityHelpers::getEnclosingHandlerWithInterface (getHandler (self), &AccessibilityHandler::getTableInterface))
                    if (auto* tableInterface = tableHandler->getTableInterface())
                        if (auto* cellHandler = tableInterface->getCellHandler ((i32) row, (i32) column))
    ...
    class AccessibilityElement final : public AccessibleObjCClass<UIAccessibilityElement>
    {
        ...
        static constexpr z0 forEach (Func&& func, Items&&... items)
        {
    ...
    private:
        ...
        static z0 getResult (NSInvocation*, detail::Tag<z0>) {}
        ...
        z0 addPassthroughMethodWithSignature (Signature signature)
        {
        ...
        static DrxTextView* getPeerTextInput (id self)
        {
        ...
        //==============================================================================
        DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (AccessibilityElement)
    };
...
static z0 sendAccessibilityEvent (UIAccessibilityNotifications notification, id argument)
{
...
z0 detail::AccessibilityHelpers::notifyAccessibilityEvent (const AccessibilityHandler& handler, Event eventType)
{
    ...
    {
        const b8 moveToHandler = (eventType == Event::focusChanged && handler.hasFocus (false));
...
z0 AccessibilityHandler::postAnnouncement (const Txt& announcementString, AnnouncementPriority)
{
...
{
    return drx::areAnyAccessibilityClientsActive();
}
