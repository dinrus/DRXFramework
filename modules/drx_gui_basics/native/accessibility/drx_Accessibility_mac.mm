...
static z0 juceFreeAccessibilityPlatformSpecificData (NSAccessibilityElement<NSAccessibility>*)  {}
...
private:
    ...
    private:
        AccessibilityElement()
        {
            ...
            {
                if (auto* textInterface = getTextInterface (self))
                    return juceStringToNS (textInterface->getText (nsRangeToDrx (range)));
            ...
            {
                if (auto* textInterface = getTextInterface (self))
                    return flippedScreenRect (makeCGRect (textInterface->getTextBounds (nsRangeToDrx (range)).getBounds()));
            ...
            addMethod (@selector (accessibilityLineForIndex:), [] (id self, SEL, NSInteger index)
            {
                if (auto* textInterface = getTextInterface (self))
                {
                    auto text = textInterface->getText ({ 0, (i32) index });
            ...
            {
                if (auto* textInterface = getTextInterface (self))
                    textInterface->setSelection (nsRangeToDrx (selectedRange));
            });
            ...
            addMethod (@selector (accessibilityRows), [] (id self, SEL) -> NSArray*
            {
                if (auto* tableInterface = getTableInterface (self))
                {
                    ...
                    for (i32 row = 0, numRows = tableInterface->getNumRows(); row < numRows; ++row)
                    {
        ...
        //==============================================================================
        static b8 isSelectable (AccessibleState state) noexcept
        {
        ...
        static z0 setSelected (id item, b8 selected)
        {
        ...
        static z0 setSelectedChildren (NSArray* children, NSArray* selected)
        {
            for (id child in children)
...
{
    const Txt voiceOverKeyString ("voiceOverOnOffKey");
    const Txt applicationIDString ("com.apple.universalaccess");
...
static z0 sendAccessibilityEvent (id accessibilityElement,
...
static z0 sendHandlerNotification (const AccessibilityHandler& handler,
                                     NSAccessibilityNotificationName notification)
{
...
z0 AccessibilityHandler::postAnnouncement (const Txt& announcementString, AnnouncementPriority priority)
{
    ...
    auto nsPriority = [priority]
    {
        ...
        DRX_BEGIN_IGNORE_WARNINGS_GCC_LIKE ("-Wunguarded-availability")
