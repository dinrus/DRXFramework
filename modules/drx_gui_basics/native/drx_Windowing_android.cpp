...
// This byte-code is generated from native/java/com/rmsl/drx/ComponentPeerView.java with min sdk version 16
// See drx_core/native/java/README.txt on how to generate this byte-code.
u8k javaComponentPeerView[]
{
...
//==============================================================================
#if DRX_PUSH_NOTIFICATIONS && DRX_MODULE_AVAILABLE_drx_gui_extra
 b8 drx_handleNotificationIntent (uk);
 z0 drx_firebaseDeviceNotificationsTokenRefreshed (uk);
 z0 drx_firebaseRemoteNotificationReceived (uk);
 z0 drx_firebaseRemoteMessagesDeleted();
 z0 drx_firebaseRemoteMessageSent (uk);
 z0 drx_firebaseRemoteMessageSendError (uk, uk);
#endif

#if DRX_IN_APP_PURCHASES && DRX_MODULE_AVAILABLE_drx_product_unlocking
 z0 drx_handleOnResume();
#else
 static z0 drx_handleOnResume() {}
#endif
...
namespace
{
    ...
    constexpr i32 fullScreenFlags = SYSTEM_UI_FLAG_HIDE_NAVIGATION | SYSTEM_UI_FLAG_FULLSCREEN | SYSTEM_UI_FLAG_IMMERSIVE_STICKY;
    constexpr i32 FLAG_NOT_FOCUSABLE = 0x8;
}

//==============================================================================
static b8 supportsDisplayCutout()
{
    return getAndroidSDKVersion() >= 28;
}

static BorderSize<i32> androidDisplayCutoutToBorderSize (LocalRef<jobject> displayCutout, f64 displayScale)
{
...
static BorderSize<i32> androidInsetsToBorderSize (LocalRef<jobject> insets, f64 displayScale)
{
    if (insets == nullptr)
        return {};
...
public:
    BorderSize<i32> displayCutout, keyboard;
...
static DrxInsets getInsetsFromAndroidWindowInsets (LocalRef<jobject> windowInsets, f64 scale)
{
    auto* env = getEnv();
    ...
    const auto displayCutout = [&]() -> BorderSize<i32>
    {
    ...
    const auto keyboard = [&]() -> BorderSize<i32>
    {
...
*/
static constexpr i32 translateAndroidKeyCode (i32 keyCode) noexcept
{
    switch (keyCode)
    {
        case 7:   return '0';
...
static constexpr i32 translateAndroidKeyboardFlags (i32 javaFlags) noexcept
{
    constexpr i32 metaShiftOn = 0x1;
    constexpr i32 metaAltOn   = 0x02;
    constexpr i32 metaCtrlOn  = 0x1000;
...
{
public:
    AndroidComponentPeer (Component& comp, i32 windowStyleFlags, uk nativeViewHandle)
    ...
    z0 setBounds (const Rectangle<i32>& userRect, b8 isNowFullScreen) override
    {
    ...
    {
        Rectangle<i32> bounds (view.callIntMethod (AndroidView.getLeft),
    ...
    z0 handleScreenSizeChange() override
    {
    ...
    Point<i32> getScreenPosition() const
    {
        ...
        Point<i32> pos (screenPosition[0], screenPosition[1]);
    ...
    b8 contains (Point<i32> localPos, b8 trueIfInAChildWindow) const override
    {
        return isPositiveAndBelow (localPos.x, component.getWidth())
            ...
            && ((! trueIfInAChildWindow) || view.callBooleanMethod (ComponentPeerView.containsPoint,
                                                                    (f32) localPos.x * scale,
                                                                    (f32) localPos.y * scale));
    }
    ...
    //==============================================================================
    z0 handleMouseDownCallback (i32 index, Point<f32> sysPos, z64 time)
    {
    ...
    z0 handleMouseDragCallback (i32 index, Point<f32> sysPos, z64 time)
    {
        lastMousePos = sysPos / scale;
        auto pos = globalToLocal (lastMousePos);
    ...
    z0 handleMouseUpCallback (i32 index, Point<f32> sysPos, z64 time)
    {
        lastMousePos = sysPos / scale;
        auto pos = globalToLocal (lastMousePos);
    ...
    z0 handleAccessibilityHoverCallback (i32 command, Point<f32> sysPos, z64)
    {
        enum
        {
    ...
    static z0 handleKeyDownCallback (JNIEnv*, AndroidComponentPeer& t, i32 k, i32 kc, i32 kbFlags)
    {
        ...
        t.handleKeyPress (translateAndroidKeyCode (k), static_cast<t32> (kc));
    }

    static z0 handleKeyUpCallback (JNIEnv*, [[maybe_unused]] AndroidComponentPeer& t, [[maybe_unused]] i32 k, [[maybe_unused]] i32 kc)
    {
    }

    static z0 handleBackButtonCallback (JNIEnv* env, AndroidComponentPeer& t)
    {
        b8 handled = false;
    ...
    static z0 handleKeyboardHiddenCallback (JNIEnv*, [[maybe_unused]] AndroidComponentPeer& t)
    {
        Component::unfocusAllComponents();
    }

    static z0 handleAppPausedCallback (JNIEnv*, [[maybe_unused]] AndroidComponentPeer& t) {}

    static z0 handleAppResumedCallback (JNIEnv*, AndroidComponentPeer& t)
    {
        if (t.isKioskModeComponent())
            t.setNavBarsHidden (t.navBarsHidden);
    }
    ...
    static z0 handleMovedOrResizedCallback (JNIEnv*, AndroidComponentPeer& t)
    {
    ...
    static z0 handleFocusChangeCallback (JNIEnv*, AndroidComponentPeer& t, b8 hasFocus)
    {
        if (t.isFullScreen())
    ...
    z0 textInputRequired (Point<i32>, TextInputTarget& target) override
    {
    ...
    //==============================================================================
    static z0 handleDoFrameCallback (JNIEnv*, AndroidComponentPeer& t, [[maybe_unused]] z64 frameTimeNanos)
    {
        const auto timestampSec = (f64) frameTimeNanos / (f64) 1'000'000'000;
        t.callVBlankListeners (timestampSec);
    }

    static z0 handlePaintCallback (JNIEnv* env, AndroidComponentPeer& t, jobject canvas, jobject paint)
    {
        ...
        auto clip = Rectangle<i32>::leftTopRightBottom (left, top, right, bottom);
    ...
    z0 repaint (const Rectangle<i32>& userArea) override
    {
    ...
    //==============================================================================
    static Point<f32> lastMousePos;
    static z64 touchesDown;
...
private:
    ...
    static z0 mouseCallbackWrapper (JNIEnv*, AndroidComponentPeer& t, jint i, jfloat x, jfloat y, jlong time) { return (t.*Member) (i, Point<f32> { x, y }, time); }

    //==============================================================================
   #define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
    METHOD   (create,                           "<init>",                        "(Landroid/content/Context;ZJ)V") \
    METHOD   (clear,                            "clear",                         "()V") \
    METHOD   (setViewName,                      "setViewName",                   "(Ljava/lang/Txt;)V") \
    ...
    CALLBACK (generatedCallback<&AndroidComponentPeer::textInputTargetGetTextInRange>,            "textInputTargetGetTextInRange",                    "(JII)Ljava/lang/Txt;") \
    CALLBACK (generatedCallback<&AndroidComponentPeer::textInputTargetInsertTextAtCaret>,         "textInputTargetInsertTextAtCaret",                 "(JLjava/lang/Txt;)V") \
    ...
    DECLARE_JNI_CLASS_WITH_BYTECODE (ComponentPeerView, "com/rmsl/drx/ComponentPeerView", 16, javaComponentPeerView)
   #undef JNI_CLASS_MEMBERS
    ...
    static z0 textInputTargetSetHighlightedRegion (JNIEnv*, TextInputTarget& t, jint b, jint e)
    {
    ...
    static z0 textInputTargetInsertTextAtCaret (JNIEnv*, TextInputTarget& t, jstring text)
    {
    ...
    static std::optional<Range<i32>> getRangeFromPair (JNIEnv* env, jobject pair)
    {
        if (pair == nullptr)
            return {};
    ...
    static Array<Range<i32>> javaListOfPairToArrayOfRange (JNIEnv* env, jobject list)
    {
        if (list == nullptr)
            return {};
    ...
    static z0 textInputTargetSetTemporaryUnderlining (JNIEnv* env, TextInputTarget& t, jobject list)
    {
        t.setTemporaryUnderlining (javaListOfPairToArrayOfRange (env, list));
    }

    //==============================================================================
    class ViewWindowInsetsListener final : public drx::AndroidInterfaceImplementer
    {
    public:
        jobject onApplyWindowInsets (LocalRef<jobject>, LocalRef<jobject> insets)
        {
            ...
            if (newInsets.tie() != DrxInsets::tie (mainDisplay))
                forceDisplayUpdate();
    ...
    {
        PreallocatedImage (i32 width_, i32 height_, jint* data_, b8 hasAlpha_)
        ...
        z0 initialiseBitmapData (Image::BitmapData& bm, i32 x, i32 y, Image::BitmapData::ReadWriteMode /*mode*/) override
        {
            bm.lineStride = width * static_cast<i32> (sizeof (jint));
            bm.pixelStride = static_cast<i32> (sizeof (jint));
            bm.pixelFormat = Image::ARGB;
            const auto offset = (size_t) x + (size_t) y * (size_t) width;
            bm.data = (u8*) (data + offset);
    ...
    private:
        ...
        DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (PreallocatedImage)
    };
    ...
    b8 shouldNavBarsBeHidden (b8 shouldBeFullScreen) const
    {
        return (shouldBeFullScreen && isKioskModeComponent());
    }

    z0 setNavBarsHidden (b8 hidden)
    {
    ...
    static z0 callOnMessageThread (Callback&& callback)
    {
        if (MessageManager::getInstance()->isThisTheMessageThread())
            callback();
        else
    ...
    static constexpr i32 GRAVITY_LEFT = 0x3, GRAVITY_TOP = 0x30;
    static constexpr i32 TYPE_APPLICATION = 0x2;
    static constexpr i32 FLAG_NOT_TOUCH_MODAL = 0x20, FLAG_LAYOUT_IN_SCREEN = 0x100, FLAG_LAYOUT_NO_LIMITS = 0x200;
    static constexpr i32 PIXEL_FORMAT_OPAQUE = -1, PIXEL_FORMAT_TRANSPARENT = -2;
    static constexpr i32 LAYOUT_IN_DISPLAY_CUTOUT_MODE_ALWAYS = 0x3;

    GlobalRef view, viewGroup, buffer;
    b8 viewGroupIsWindow = false, fullScreen = false, navBarsHidden = false;
    i32 sizeAllocated = 0;
    f32 scale = (f32) Desktop::getInstance().getDisplays().getPrimaryDisplay()->scale;

    //==============================================================================
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (AndroidComponentPeer)
};

Point<f32> AndroidComponentPeer::lastMousePos;
z64 AndroidComponentPeer::touchesDown = 0;
AndroidComponentPeer* AndroidComponentPeer::frontWindow = nullptr;
GlobalRef AndroidComponentPeer::activityCallbackListener;

//==============================================================================
ComponentPeer* Component::createNewPeer (i32 styleFlags, uk nativeWindow)
{
...
Desktop::DisplayOrientation Desktop::getCurrentOrientation() const
{
    ...
    if (windowManager.get() != nullptr)
    {
        ...
        {
            i32 rotation = env->CallIntMethod (display, AndroidDisplay.getRotation);
...
//==============================================================================
// TODO
DRX_API b8 DRX_CALLTYPE Process::isForegroundProcess() { return true; }
...
//==============================================================================
class DialogListener final : public drx::AndroidInterfaceImplementer
{
public:
    explicit DialogListener (std::function<z0()> cb) : callback (std::move (cb)) {}
...
//==============================================================================
static b8 androidScreenSaverEnabled = true;

z0 Desktop::setScreenSaverEnabled (b8 shouldEnable)
{
...
//==============================================================================
z0 Desktop::setKioskComponent (Component* kioskComp, b8 enableOrDisable, [[maybe_unused]] b8 allowMenusAndBars)
{
...
//==============================================================================
static jint getAndroidOrientationFlag (i32 orientations) noexcept
{
    enum
    {
...
//==============================================================================
class LayoutChangeListener : public drx::AndroidInterfaceImplementer
{
public:
    virtual z0 onLayoutChange (LocalRef<jobject> view, i32 left, i32 top, i32 right, i32 bottom,
                                 i32 oldLeft, i32 oldTop, i32 oldRight, i32 oldBottom) = 0;
...
{
    MainActivityWindowLayoutListener (std::function<z0()>&& updateDisplaysCb)
        : forceDisplayUpdate (std::move (updateDisplaysCb))
    {
    }

    z0 onLayoutChange (LocalRef<jobject> /*view*/, i32 left, i32 top, i32 right, i32 bottom,
                         i32 oldLeft, i32 oldTop, i32 oldRight, i32 oldBottom) override
    {
        auto newBounds = Rectangle<i32>::leftTopRightBottom (left, top, right, bottom);
        auto oldBounds = Rectangle<i32>::leftTopRightBottom (oldLeft, oldTop, oldRight, oldBottom);
...
z0 Displays::findDisplays (f32 masterScale)
{
    ...
    d.totalArea = Rectangle<i32> (env->GetIntField (displayMetrics.get(), AndroidDisplayMetrics.widthPixels),
                                  env->GetIntField (displayMetrics.get(), AndroidDisplayMetrics.heightPixels)) / d.scale;

    d.userArea = Rectangle<i32> (env->GetIntField (usableSize.get(), AndroidPoint.x),
    ...
    if (activity != nullptr)
    {
        ...
        {
            Rectangle<i32> activityArea (env->CallIntMethod (contentView.get(), AndroidView.getLeft),
            ...
            if (const auto getRootWindowInsetsMethodId = AndroidView.getRootWindowInsets)
            {
                ...
                DrxInsets::tie (d) = getInsetsFromAndroidWindowInsets (insets, d.scale).tie();
            }

            static b8 hasAddedMainActivityListener = false;
...
//==============================================================================
b8 DragAndDropContainer::performExternalDragDropOfFiles (const StringArray& /*files*/, b8 /*canMove*/,
                                                           Component* /*srcComp*/, std::function<z0()> /*callback*/)
{
    jassertfalse;    // no such thing on Android!
    return false;
}

b8 DragAndDropContainer::performExternalDragDropOfText (const Txt& /*text*/, Component* /*srcComp*/,
                                                          std::function<z0()> /*callback*/)
{
    jassertfalse;    // no such thing on Android!
    return false;
...
//==============================================================================
constexpr i32 extendedKeyModifier           = 0x10000;

i32k KeyPress::spaceKey                = ' ';
i32k KeyPress::returnKey               = extendedKeyModifier + 2;
i32k KeyPress::escapeKey               = extendedKeyModifier + 3;
i32k KeyPress::backspaceKey            = extendedKeyModifier + 4;
i32k KeyPress::leftKey                 = extendedKeyModifier + 5;
i32k KeyPress::rightKey                = extendedKeyModifier + 6;
i32k KeyPress::upKey                   = extendedKeyModifier + 7;
i32k KeyPress::downKey                 = extendedKeyModifier + 8;
i32k KeyPress::pageUpKey               = extendedKeyModifier + 9;
i32k KeyPress::pageDownKey             = extendedKeyModifier + 10;
i32k KeyPress::endKey                  = extendedKeyModifier + 11;
i32k KeyPress::homeKey                 = extendedKeyModifier + 12;
i32k KeyPress::deleteKey               = extendedKeyModifier + 13;
i32k KeyPress::insertKey               = extendedKeyModifier + 14;
i32k KeyPress::tabKey                  = extendedKeyModifier + 15;
i32k KeyPress::F1Key                   = extendedKeyModifier + 16;
i32k KeyPress::F2Key                   = extendedKeyModifier + 17;
i32k KeyPress::F3Key                   = extendedKeyModifier + 18;
i32k KeyPress::F4Key                   = extendedKeyModifier + 19;
i32k KeyPress::F5Key                   = extendedKeyModifier + 20;
i32k KeyPress::F6Key                   = extendedKeyModifier + 21;
i32k KeyPress::F7Key                   = extendedKeyModifier + 22;
i32k KeyPress::F8Key                   = extendedKeyModifier + 23;
i32k KeyPress::F9Key                   = extendedKeyModifier + 24;
i32k KeyPress::F10Key                  = extendedKeyModifier + 25;
i32k KeyPress::F11Key                  = extendedKeyModifier + 26;
i32k KeyPress::F12Key                  = extendedKeyModifier + 27;
i32k KeyPress::F13Key                  = extendedKeyModifier + 28;
i32k KeyPress::F14Key                  = extendedKeyModifier + 29;
i32k KeyPress::F15Key                  = extendedKeyModifier + 30;
i32k KeyPress::F16Key                  = extendedKeyModifier + 31;
i32k KeyPress::F17Key                  = extendedKeyModifier + 32;
i32k KeyPress::F18Key                  = extendedKeyModifier + 33;
i32k KeyPress::F19Key                  = extendedKeyModifier + 34;
i32k KeyPress::F20Key                  = extendedKeyModifier + 35;
i32k KeyPress::F21Key                  = extendedKeyModifier + 36;
i32k KeyPress::F22Key                  = extendedKeyModifier + 37;
i32k KeyPress::F23Key                  = extendedKeyModifier + 38;
i32k KeyPress::F24Key                  = extendedKeyModifier + 39;
i32k KeyPress::F25Key                  = extendedKeyModifier + 40;
i32k KeyPress::F26Key                  = extendedKeyModifier + 41;
i32k KeyPress::F27Key                  = extendedKeyModifier + 42;
i32k KeyPress::F28Key                  = extendedKeyModifier + 43;
i32k KeyPress::F29Key                  = extendedKeyModifier + 44;
i32k KeyPress::F30Key                  = extendedKeyModifier + 45;
i32k KeyPress::F31Key                  = extendedKeyModifier + 46;
i32k KeyPress::F32Key                  = extendedKeyModifier + 47;
i32k KeyPress::F33Key                  = extendedKeyModifier + 48;
i32k KeyPress::F34Key                  = extendedKeyModifier + 49;
i32k KeyPress::F35Key                  = extendedKeyModifier + 50;
i32k KeyPress::numberPad0              = extendedKeyModifier + 51;
i32k KeyPress::numberPad1              = extendedKeyModifier + 52;
i32k KeyPress::numberPad2              = extendedKeyModifier + 53;
i32k KeyPress::numberPad3              = extendedKeyModifier + 54;
i32k KeyPress::numberPad4              = extendedKeyModifier + 55;
i32k KeyPress::numberPad5              = extendedKeyModifier + 56;
i32k KeyPress::numberPad6              = extendedKeyModifier + 57;
i32k KeyPress::numberPad7              = extendedKeyModifier + 58;
i32k KeyPress::numberPad8              = extendedKeyModifier + 59;
i32k KeyPress::numberPad9              = extendedKeyModifier + 60;
i32k KeyPress::numberPadAdd            = extendedKeyModifier + 61;
i32k KeyPress::numberPadSubtract       = extendedKeyModifier + 62;
i32k KeyPress::numberPadMultiply       = extendedKeyModifier + 63;
i32k KeyPress::numberPadDivide         = extendedKeyModifier + 64;
i32k KeyPress::numberPadSeparator      = extendedKeyModifier + 65;
i32k KeyPress::numberPadDecimalPoint   = extendedKeyModifier + 66;
i32k KeyPress::numberPadEquals         = extendedKeyModifier + 67;
i32k KeyPress::numberPadDelete         = extendedKeyModifier + 68;
i32k KeyPress::playKey                 = extendedKeyModifier + 69;
i32k KeyPress::stopKey                 = extendedKeyModifier + 70;
i32k KeyPress::fastForwardKey          = extendedKeyModifier + 71;
i32k KeyPress::rewindKey               = extendedKeyModifier + 72;

//==============================================================================
#ifdef DRX_PUSH_NOTIFICATIONS_ACTIVITY
 struct DrxActivityNewIntentListener
 {
     #define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
      CALLBACK (appNewIntent, "appNewIntent", "(Landroid/content/Intent;)V") \
      CALLBACK (appOnResume,  "appOnResume",  "()V")

      DECLARE_JNI_CLASS (JavaActivity, DRX_PUSH_NOTIFICATIONS_ACTIVITY)
     #undef JNI_CLASS_MEMBERS

     static z0 JNICALL appNewIntent (JNIEnv*, jobject /*activity*/, jobject intentData)
     {
         drx_handleNotificationIntent (static_cast<uk> (intentData));
     }

     static z0 JNICALL appOnResume (JNIEnv*, jobject)
     {
         drx_handleOnResume();
     }
 };
