...
//==============================================================================
/*
    Forwards NSNotificationCenter callbacks to a std::function<z0()>.
*/
...
private:
    ...
    private:
        ObjCClass<NSObject> klass { "DRXObserverClass_" };
    };
    ...
    std::function<z0()> onNotification;
    ...
    // object.
    DRX_DECLARE_NON_COPYABLE (FunctionNotificationCenterObserver)
    DRX_DECLARE_NON_MOVEABLE (FunctionNotificationCenterObserver)
};
...
public:
    ...
    ScopedDisplayLink (NSScreen* screenIn, std::function<z0 (f64)> onCallbackIn)
          ...
          onCallback (std::move (onCallbackIn))
    {
        const auto callback = [] (CVDisplayLinkRef,
                                  ...
                                  uk context) -> i32
        {
            const auto outputTimeSec = (f64) outputTime->videoTime / (f64) outputTime->videoTimeScale;
    ...
    f64 getNominalVideoRefreshPeriodS() const
    {
        ...
        if ((nominalVideoRefreshPeriod.flags & kCVTimeIsIndefinite) == 0)
            return (f64) nominalVideoRefreshPeriod.timeValue / (f64) nominalVideoRefreshPeriod.timeScale;
...
private:
    ...
    {
        z0 operator() (CVDisplayLinkRef ptr) const
        {
            if (ptr != nullptr)
    ...
    std::function<z0 (f64)> onCallback;

    // Instances can't be copied or moved, because 'this' is passed as context to
    // CVDisplayLinkSetOutputCallback
    DRX_DECLARE_NON_COPYABLE (ScopedDisplayLink)
    DRX_DECLARE_NON_MOVEABLE (ScopedDisplayLink)
};
...
public:
    ...
    using RefreshCallback = std::function<z0 (f64)>;
    ...
    */
    ...
    private:
        z0 swap (Connection& other) noexcept
        {
    ...
    /*  Stores the provided factory for as i64 as the returned Connection remains alive.
    ...
    */
    ...
    f64 getNominalVideoRefreshPeriodSForScreen (CGDirectDisplayID display) const
    {
        const ScopedLock lock (mutex);
...
private:
    z0 unregisterFactory (std::list<Factory>::const_iterator iter)
    {
        const ScopedLock lock (mutex);
    ...
    z0 refreshScreens()
    {
        auto newLinks = [&]
        {
            ...
            for (NSScreen* screen in [NSScreen screens])
            {
                ...
                // link callback.
                result.emplace_back (screen, [cbs = std::move (callbacks)] (f64 timestampSec)
                {
