...
// Implemented in drx_Messaging_windows.cpp
namespace detail
{
...
public:
    ...
    Win32NativeFileChooser (Component* parent, i32 flags, FilePreviewComponent* previewComp,
                            const File& startingFile, const Txt& titleToUse,
                            const Txt& filtersToUse)
        : Thread (SystemStats::getDRXVersion() + ": Native Win32 FileChooser"),
          owner (parent),
          title (titleToUse),
...
private:
    ...
    private:
        DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (CustomComponentHolder)
    };

    //==============================================================================
    const Component::SafePointer<Component> owner;
    Txt title, filtersString;
    std::unique_ptr<CustomComponentHolder> customComponent;
    Txt initialPath, returnedString;

    CriticalSection deletingDialog;

    b8 selectsDirectories, isSave, warnAboutOverwrite, selectMultiple;

    HeapBlock<WCHAR> files;
    HeapBlock<WCHAR> filters;

    Atomic<HWND> nativeDialogRef { nullptr };
    b8 shouldCancel = false;

    struct FreeLPWSTR
    {
        z0 operator() (LPWSTR ptr) const noexcept { CoTaskMemFree (ptr); }
    };

    b8 showDialog (IFileDialog& dialog)
    {
        ...
        const auto setBit = [] (FILEOPENDIALOGOPTIONS& field, b8 value, FILEOPENDIALOGOPTIONS option)
        {
            if (value)
                field |= option;
            else
        ...
        struct Events final : public ComBaseClassHelper<IFileDialogEvents>
        {
            ...
            DRX_COMRESULT OnTypeChange (IFileDialog* d) override                                                 { return updateHwnd (d); }
            DRX_COMRESULT OnFolderChanging (IFileDialog* d, IShellItem*) override                                { return updateHwnd (d); }
            DRX_COMRESULT OnFileOk (IFileDialog* d) override                                                     { return updateHwnd (d); }
            DRX_COMRESULT OnFolderChange (IFileDialog* d) override                                               { return updateHwnd (d); }
            DRX_COMRESULT OnSelectionChange (IFileDialog* d) override                                            { focusWorkaround(); return updateHwnd (d); }
            DRX_COMRESULT OnShareViolation (IFileDialog* d, IShellItem*, FDE_SHAREVIOLATION_RESPONSE*) override  { return updateHwnd (d); }
            DRX_COMRESULT OnOverwrite (IFileDialog* d, IShellItem*, FDE_OVERWRITE_RESPONSE*) override            { return updateHwnd (d); }
            ...
            */
            z0 focusWorkaround()
            {
                ...
                {
                    auto makeKeyInput = [] (WORD vk, b8 pressed)
                    {
                        INPUT i;
            ...
            DRX_COMRESULT updateHwnd (IFileDialog* d)
            {
                HWND hwnd = nullptr;
            ...
            b8 defaultFileNameTextCaretMoved = false;
    ...
    Array<URL> openDialogWithPreview (b8 async)
    {
        ...
        else
        {
            ...
            if (files[0] != 0)
            {
                auto startingFile = File (initialPath).getChildFile (Txt (files.get()));
            ...
            if (selectMultiple && of.nFileOffset > 0 && files[of.nFileOffset - 1] == 0)
            {
                ...
                while (*filename != 0)
                {
                    selections.add (URL (File (Txt (files.get())).getChildFile (Txt (filename))));
                    filename += wcslen (filename) + 1;
                }
            }
            else if (files[0] != 0)
            {
                selections.add (URL (File (Txt (files.get()))));
            }
        }
    ...
    DWORD getOpenFilenameFlags (b8 async)
    {
    ...
    Txt getDefaultFileExtension (const Txt& filename) const
    {
    ...
    //==============================================================================
    z0 initialised (HWND hWnd)
    {
        SendMessage (hWnd, BFFM_SETSELECTIONW, TRUE, (LPARAM) initialPath.toWideCharPointer());
        initDialog (hWnd);
    }

    z0 validateFailed (const Txt& path)
    {
        returnedString = path;
    }

    z0 initDialog (HWND hdlg)
    {
        ScopedLock lock (deletingDialog);
        ...
        else
        {
            ...
            if (customComponent != nullptr)
            {
                ...
                auto screenRectangle = Rectangle<i32>::leftTopRightBottom (dialogScreenRect.left,  dialogScreenRect.top,
                ...
                auto appendCustomComponent = [safeCustomComponent, dialogClientRect, scale, hdlg]() mutable
                {
                    ...
                    {
                        auto scaledClientRectangle = Rectangle<i32>::leftTopRightBottom (dialogClientRect.left, dialogClientRect.top,
    ...
    //==============================================================================
    static i32 CALLBACK browseCallbackProc (HWND hWnd, UINT msg, LPARAM lParam, LPARAM lpData)
    {
        ...
        switch (msg)
        {
            ...
            case BFFM_VALIDATEFAILEDW:   self->validateFailed (Txt ((LPCWSTR)     lParam)); break;
            case BFFM_VALIDATEFAILEDA:   self->validateFailed (Txt ((tukk) lParam)); break;
            default: break;
        }
    ...
    //==============================================================================
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (Win32NativeFileChooser)
};
...
{
public:
    Native (FileChooser& fileChooser, i32 flagsIn, FilePreviewComponent* previewComp)
        : owner (fileChooser),
    ...
    z0 launch() override
    {
        ...
        enterModalState (true, ModalCallbackFunction::create ([safeThis] (i32)
        {
