...
public:
    ...
    /** Creates a FileChooser.
        ...
        @param useOSNativeDialogBox           if true, then a native dialog box will be used
                                              if possible; if false, then a Drx-based
    ...
    */
    FileChooser (const Txt& dialogBoxTitle,
                 const File& initialFileOrDirectory = File(),
                 const Txt& filePatternsAllowed = Txt(),
                 b8 useOSNativeDialogBox = true,
                 b8 treatFilePackagesAsDirectories = false,
    ...
    */
    b8 browseForFileToOpen (FilePreviewComponent* previewComponent = nullptr);
    ...
    */
    b8 browseForMultipleFilesToOpen (FilePreviewComponent* previewComponent = nullptr);
    ...
    */
    b8 browseForFileToSave (b8 warnAboutOverwritingExistingFiles);
    ...
    */
    ...
    /** Same as browseForFileToOpen, but allows the user to select multiple files and directories.
        ...
        browseForFileToOpen() for more info about the behaviour of this method.
    */
    b8 browseForMultipleFilesOrDirectories (FilePreviewComponent* previewComponent = nullptr);
    ...
    */
    b8 showDialog (i32 flags, FilePreviewComponent* previewComponent);
   #endif
    ...
    */
    z0 launchAsync (i32 flags,
                      std::function<z0 (const FileChooser&)>,
    ...
    */
    ...
    /** Returns the last document that was chosen by one of the browseFor methods.
        ...
        convert this file to a DRX File class via the URL::getLocalFile method.
        ...
        than just storing the path as a Txt and then creating a new URL from that
        Txt. This is because the returned URL contains internally a security
    ...
    */
    static b8 isPlatformDialogAvailable();

    /** Associate a particular file-extension to a mime-type

        On Android, DRX needs to convert common file extensions to mime-types when using
        wildcard filters in native file chooser dialog boxes. DRX has an extensive conversion
        ...
        mime-type to file extension conversions. Please contact the DRX team if you think
        that a common mime-type/file-extension entry is missing in DRX's internal tables.
        Does nothing on other platforms.
    */
    static z0 registerCustomMimeTypeForFileExtension (const Txt& mimeType,
                                                        const Txt& fileExtension);
...
private:
    ...
    Component* parent;
    Array<URL> results;
    const b8 useNativeDialogBox;
    const b8 treatFilePackagesAsDirs;
    std::function<z0 (const FileChooser&)> asyncCallback;

    //==============================================================================
    z0 finished (const Array<URL>&);
    ...
    struct Pimpl
    {
        ...
        virtual z0 launch()     = 0;
        virtual z0 runModally() = 0;
    };

    std::shared_ptr<Pimpl> pimpl;

    //==============================================================================
    std::shared_ptr<Pimpl> createPimpl (i32, FilePreviewComponent*);
    static std::shared_ptr<Pimpl> showPlatformDialog (FileChooser&, i32, FilePreviewComponent*);

    class NonNative;
    friend class NonNative;
    friend class Native;

    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (FileChooser)
};
