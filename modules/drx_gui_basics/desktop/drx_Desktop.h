...
/**
    ...
    @tags{GUI}
*/
class DRX_API  FocusChangeListener
{
public:
    /** Destructor. */
    ...
    virtual z0 globalFocusChanged (Component* focusedComponent) = 0;
};
...
/**
    ...
    @tags{GUI}
*/
class DRX_API  DarkModeSettingListener
{
public:
    /** Destructor. */
    ...
    virtual z0 darkModeSettingChanged() = 0;
};
...
/**
    ...
    @tags{GUI}
*/
class DRX_API  Desktop  : private DeletedAtShutdown,
                           private Timer,
...
public:
    ...
    static Desktop& DRX_CALLTYPE getInstance();
    ...
    */
    static Point<i32> getMousePosition();
    ...
    */
    static z0 setMousePosition (Point<i32> newPosition);
    ...
    */
    static Point<i32> getLastMouseDownPosition();
    ...
    */
    i32 getMouseButtonClickCounter() const noexcept;
    ...
    */
    i32 getMouseWheelMoveCounter() const noexcept;
    ...
    /** This lets you prevent the screensaver from becoming active.
        ...
        The disablement will only happen while the DRX application is the foreground
        process - if another task is running in front of it, then the screensaver will
        be unaffected.

        @see isScreenSaverEnabled
    */
    static z0 setScreenSaverEnabled (b8 isEnabled);

    /** Возвращает true, если the screensaver has not been turned off.
    ...
    */
    static b8 isScreenSaverEnabled();
    ...
    */
    z0 addGlobalMouseListener (MouseListener* listener);

    /** Unregisters a MouseListener that was added with addGlobalMouseListener().

        @see addGlobalMouseListener
    */
    z0 removeGlobalMouseListener (MouseListener* listener);
    ...
    */
    z0 addFocusChangeListener (FocusChangeListener* listener);

    /** Unregisters a FocusChangeListener that was added with addFocusChangeListener().

        @see addFocusChangeListener
    */
    z0 removeFocusChangeListener (FocusChangeListener* listener);
    ...
    */
    z0 addDarkModeSettingListener (DarkModeSettingListener* listener);

    /** Unregisters a DarkModeSettingListener that was added with addDarkModeSettingListener().

        @see addDarkModeSettingListener
    */
    z0 removeDarkModeSettingListener (DarkModeSettingListener* listener);
    ...
    */
    z0 setKioskModeComponent (Component* componentToUse,
                                b8 allowMenusAndBars = true);
    ...
    */
    i32 getNumComponents() const noexcept;
    ...
    */
    Component* getComponent (i32 index) const noexcept;
    ...
    /** Finds the component at a given screen location.
        ...
        Returns nullptr if the coordinates are inside a non-DRX window.
    */
    Component* findComponentAt (Point<i32> screenPosition) const;

    /** The ComponentAnimator has been superseded, it is now recommended you use the Animator
        class in the drx_animation module.
    ...
    */
    z0 setDefaultLookAndFeel (LookAndFeel* newDefaultLookAndFeel);
    ...
    */
    ...
    /** Returns the number of MouseInputSource objects the system has at its disposal.
        ...
        @see getMouseSource
    */
    i32 getNumMouseSources() const noexcept;
    ...
    */
    MouseInputSource* getMouseSource (i32 index) const noexcept;
    ...
    */
    ...
    /** Returns the number of mouse-sources that are currently being dragged.
        ...
        DRX component has the button down on it. In a multi-touch system, this could
    ...
    */
    MouseInputSource* getDraggingMouseSource (i32 index) const noexcept;
    ...
    /** Ensures that a non-stop stream of mouse-drag events will be sent during the
        ...
        @see mouseDrag
    */
    z0 beginDragAutoRepeat (i32 millisecondsBetweenCallbacks);
    ...
    */
    b8 isOrientationEnabled (DisplayOrientation orientation) const noexcept;
    ...
    */
    z0 setGlobalScaleFactor (f32 newScaleFactor) noexcept;

    /** Returns the current global scale factor, as set by setGlobalScaleFactor().
        @see setGlobalScaleFactor
    */
    f32 getGlobalScaleFactor() const noexcept         { return masterScaleFactor; }

    //==============================================================================
    /** True if the OS supports semitransparent windows */
    static b8 canUseSemiTransparentWindows() noexcept;

   #if DRX_MAC && ! defined (DOXYGEN)
    [[deprecated ("This macOS-specific method has been deprecated in favour of the cross-platform "
                  " isDarkModeActive() method.")]]
    static b8 isOSXDarkModeActive()  { return Desktop::getInstance().isDarkModeActive(); }
   #endif

    /** Возвращает true, если the desktop environment allows resizing the window by clicking and dragging
        just on/outside the window border.
        MacOS and Windows 10+ both support this. Linux doesn't seem to. Mobile platforms do not.
    */
    b8 supportsBorderlessNonClientResize() const;

    //==============================================================================
    /** Returns true on a headless system where there are no connected displays. */
    b8 isHeadless() const noexcept;
...
private:
    ...
    Point<f32> lastFakeMouseMove;
    z0 sendMouseMove();

    i32 mouseClickCounter = 0, mouseWheelCounter = 0;
    z0 incrementMouseClickCounter() noexcept;
    z0 incrementMouseWheelCounter() noexcept;
    ...
    Rectangle<i32> kioskComponentOriginalBounds;
    b8 kioskModeReentrant = false;

    i32 allowedOrientations = allOrientations;
    z0 allowedOrientationsChanged();

    f32 masterScaleFactor;
    ...
    z0 addDesktopComponent (Component*);
    z0 removeDesktopComponent (Component*);
    z0 componentBroughtToFront (Component*);

    z0 setKioskComponent (Component*, b8 shouldBeEnabled, b8 allowMenusAndBars);

    z0 triggerFocusCallback();
    z0 updateFocusOutline();
    z0 handleAsyncUpdate() override;

    static Point<f32> getMousePositionFloat();

    static f64 getDefaultMasterScale();
    ...
    //==============================================================================
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (Desktop)
};
