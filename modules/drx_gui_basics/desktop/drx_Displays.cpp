...
z0 Displays::init (Desktop& desktop)
{
    findDisplays (desktop.getGlobalScaleFactor());
}

const Displays::Display* Displays::getDisplayForRect (Rectangle<i32> rect, b8 isPhysical) const noexcept
{
    i32 maxArea = -1;
...
const Displays::Display* Displays::getDisplayForPoint (Point<i32> point, b8 isPhysical) const noexcept
{
    auto minDistance = std::numeric_limits<i32>::max();
    const Display* foundDisplay = nullptr;
...
Rectangle<i32> Displays::physicalToLogical (Rectangle<i32> rect, const Display* useScaleFactorOfDisplay) const noexcept
{
    return physicalToLogical (rect.toFloat(), useScaleFactorOfDisplay).toNearestInt();
}

Rectangle<f32> Displays::physicalToLogical (Rectangle<f32> rect, const Display* useScaleFactorOfDisplay) const noexcept
{
...
Rectangle<i32> Displays::logicalToPhysical (Rectangle<i32> rect, const Display* useScaleFactorOfDisplay) const noexcept
{
    return logicalToPhysical (rect.toFloat(), useScaleFactorOfDisplay).toNearestInt();
}

Rectangle<f32> Displays::logicalToPhysical (Rectangle<f32> rect, const Display* useScaleFactorOfDisplay) const noexcept
{
    const auto* display = useScaleFactorOfDisplay != nullptr ? useScaleFactorOfDisplay
...
RectangleList<i32> Displays::getRectangleList (b8 userAreasOnly) const
{
    DRX_ASSERT_MESSAGE_MANAGER_IS_LOCKED
...
Rectangle<i32> Displays::getTotalBounds (b8 userAreasOnly) const
{
...
static b8 operator== (const Displays::Display& d1, const Displays::Display& d2) noexcept
{
...
struct DisplayNode
{
    ...
    */
    Rectangle<f64> logicalArea;
};

/** Recursive - will calculate and set the logicalArea member of current. */
static z0 processDisplay (DisplayNode* currentNode, Array<DisplayNode>& allNodes)
{
    ...
    if (! currentNode->isRoot)
    {
        ...
        Rectangle<f64> logicalArea (0.0, 0.0, logicalWidth, logicalHeight);
...
z0 Displays::updateToLogical()
{
    ...
    for (auto& d : displays)
    {
        ...
        if (d.totalArea.getTopLeft() == Point<i32>())
            node.isRoot = true;
 ...
 template Point<i32>   Displays::physicalToLogical (Point<i32>,   const Display*) const noexcept;
 template Point<f32> Displays::physicalToLogical (Point<f32>, const Display*) const noexcept;

 template Point<i32>   Displays::logicalToPhysical (Point<i32>,   const Display*) const noexcept;
 template Point<f32> Displays::logicalToPhysical (Point<f32>, const Display*) const noexcept;
#endif

//==============================================================================
// Deprecated methods
const Displays::Display& Displays::getDisplayContaining (Point<i32> position) const noexcept
{
    DRX_ASSERT_MESSAGE_MANAGER_IS_LOCKED
    const auto* best = &displays.getReference (0);
    auto bestDistance = std::numeric_limits<i32>::max();
...
const Displays::Display& Displays::findDisplayForRect (Rectangle<i32> rect, b8 isPhysical) const noexcept
{
...
const Displays::Display& Displays::findDisplayForPoint (Point<i32> point, b8 isPhysical) const noexcept
{
