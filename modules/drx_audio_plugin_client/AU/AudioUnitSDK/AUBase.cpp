...
private:
	...
	static u32 GetCSR() { return _mm_getcsr(); }
	static z0 SetCSR(u32 x) { _mm_setcsr(x); }
#else
...
//_____________________________________________________________________________
//
z0 AUBase::PostConstructorInternal()
{
...
//_____________________________________________________________________________
//
z0 AUBase::PreDestructorInternal()
{
...
//_____________________________________________________________________________
//
z0 AUBase::SetMaxFramesPerSlice(UInt32 nFrames)
{
...
OSStatus AUBase::DispatchGetProperty(
	AudioUnitPropertyID inID, AudioUnitScope inScope, AudioUnitElement inElement, uk outData)
{
	...
	AudioUnitElement inElement, ukk inData, UInt32 inDataSize)
{
	...
	case kAudioUnitProperty_HostCallbacks: {
		...
		const b8 hasChanged = memcmp(&mHostCallbackInfo, inData, availSize) == 0;
		mHostCallbackInfo = {};
	...
	AudioUnitPropertyID inID, AudioUnitScope inScope, AudioUnitElement inElement)
{
	...
	case kAudioUnitProperty_HostCallbacks: {
		AUSDK_Require(inScope == kAudioUnitScope_Global, kAudioUnitErr_InvalidScope);
		b8 hasValue = false;
		ukk const ptr = &mHostCallbackInfo;
		for (size_t i = 0; i < sizeof(HostCallbackInfo); ++i) {
			if (static_cast<tukk>(ptr)[i] != 0) { // NOLINT
				hasValue = true;
				break;
			}
		}
...
//_____________________________________________________________________________
//
OSStatus AUBase::SetRenderNotification(AURenderCallback inProc, uk inRefCon)
{
	if (inProc == nullptr) {
...
//_____________________________________________________________________________
//
OSStatus AUBase::RemoveRenderNotification(AURenderCallback inProc, uk inRefCon)
{
...
{
	const b8 canScheduleParameters = CanScheduleParameters();
...
//
constexpr b8 ParameterEventListSortPredicate(
...
//_____________________________________________________________________________
//
z0 AUBase::SetWantsRenderThreadID(b8 inFlag)
{
	...
	AudioBufferList& ioData)
{
	const auto errorExit = [this](OSStatus error) {
		AUSDK_LogError("  from %s, render err: %d", GetLoggingString(), static_cast<i32>(error));
	...
	try {
		...
		if (inFramesToProcess > mMaxFramesPerSlice) {
			...
			if (static_cast<f64>(now - lastTimeMessagePrinted) >
				...
				AUSDK_LogError("kAudioUnitErr_TooManyFramesToProcess : inFramesToProcess=%u, "
					...
					static_cast<u32>(inFramesToProcess),
					static_cast<u32>(mMaxFramesPerSlice));
			}
#endif
		...
		if (ASBD::NumberChannelStreams(output.GetStreamFormat()) != ioData.mNumberBuffers) {
			AUSDK_LogError(
				...
				static_cast<u32>(ioData.mNumberBuffers),
				static_cast<u32>(ASBD::NumberChannelStreams(output.GetStreamFormat())));
			return errorExit(kAudio_ParamError);
		}

		const u32 expectedBufferByteSize =
			inFramesToProcess * output.GetStreamFormat().mBytesPerFrame;
		for (u32 ibuf = 0; ibuf < ioData.mNumberBuffers; ++ibuf) {
			...
			if (buf.mData != nullptr) {
				...
				if (buf.mDataByteSize < expectedBufferByteSize) {
					...
					AUSDK_LogError("%u frames, %u bytes/frame, expected %u-byte buffer; "
						...
						static_cast<u32>(inFramesToProcess),
						static_cast<u32>(output.GetStreamFormat().mBytesPerFrame),
						expectedBufferByteSize, ibuf, static_cast<u32>(buf.mDataByteSize));
					return errorExit(kAudio_ParamError);
				}
...
inline b8 CheckRenderArgs(AudioUnitRenderActionFlags flags)
{
...
{
	const auto errorExit = [this](OSStatus error) {
		AUSDK_LogError("  from %s, process err: %d", GetLoggingString(), static_cast<i32>(error));
		SetRenderError(error);
		return error;
	};
		...
		if (CheckRenderArgs(ioActionFlags)) {
			...
			if (ASBD::NumberChannelStreams(input.GetStreamFormat()) != ioData.mNumberBuffers) {
				AUSDK_LogError(
					...
					static_cast<u32>(ioData.mNumberBuffers),
					static_cast<u32>(ASBD::NumberChannelStreams(input.GetStreamFormat())));
				return errorExit(kAudio_ParamError);
			}
				...
				if (buf.mData != nullptr) {
					...
					if (buf.mDataByteSize < expectedBufferByteSize) {
						...
						AUSDK_LogError("%u frames, %u bytes/frame, expected %u-byte buffer; "
									   "ioData.mBuffers[%u].mDataByteSize=%u; kAudio_ParamError",
							static_cast<u32>(inFramesToProcess),
							static_cast<u32>(input.GetStreamFormat().mBytesPerFrame),
							expectedBufferByteSize, ibuf, static_cast<u32>(buf.mDataByteSize));
						return errorExit(kAudio_ParamError);
					}
					// Some clients incorrectly pass bigger buffers than expectedBufferByteSize.
	...
	AudioBufferList** ioOutputBufferLists)
{
	const auto errorExit = [this](OSStatus error) {
		AUSDK_LogError(
			"  from %s, processmultiple err: %d", GetLoggingString(), static_cast<i32>(error));
		SetRenderError(error);
		return error;
	};
	...
	try {
		if (CheckRenderArgs(ioActionFlags)) {
			...
			for (u32 ibl = 0; ibl < inNumberInputBufferLists; ++ibl) {
				if (inInputBufferLists[ibl] != nullptr) { // NOLINT
					...
					if (ASBD::NumberChannelStreams(input.GetStreamFormat()) !=
						...
						AUSDK_LogError("inInputBufferLists[%u]->mNumberBuffers=%u, "
									   ...
									   "kAudio_ParamError",
							ibl, static_cast<u32>(inInputBufferLists[ibl]->mNumberBuffers),
							static_cast<u32>(
			...
			for (u32 obl = 0; obl < inNumberOutputBufferLists; ++obl) {
				if (ioOutputBufferLists[obl] != nullptr) { // NOLINT
					...
					if (ASBD::NumberChannelStreams(output.GetStreamFormat()) !=
						...
						AUSDK_LogError("ioOutputBufferLists[%u]->mNumberBuffers=%u, "
									   ...
									   "kAudio_ParamError",
							obl, static_cast<u32>(ioOutputBufferLists[obl]->mNumberBuffers),
							static_cast<u32>(
...
//_____________________________________________________________________________
//
b8 AUBase::ValidFormat(AudioUnitScope /*inScope*/, AudioUnitElement /*inElement*/,
...
//_____________________________________________________________________________
//
b8 AUBase::IsStreamFormatWritable(AudioUnitScope scope, AudioUnitElement element)
{
	switch (scope) {
...
constexpr i32 kCurrentSavedStateVersion = 0;

static z0 AddNumToDictionary(CFMutableDictionaryRef dict, CFStringRef key, SInt32 value)
{
	...
	if (nameDict && (CFGetTypeID(nameDict) == CFDictionaryGetTypeID())) {
		for (AudioUnitScope i = 0; i < kNumScopes; ++i) {
			const CFStringRef key = CFStringCreateWithFormat(
				nullptr, nullptr, CFSTR("%u"), static_cast<u32>(i)); // NOLINT
...
b8 AUBase::SetAFactoryPresetAsCurrent(const AUPreset& inPreset)
{
...
//_____________________________________________________________________________
//
z0 AUBase::SetNumberOfElements(AudioUnitScope inScope, UInt32 numElements)
{
