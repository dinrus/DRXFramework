...
public:
	...
	static z0 set_instance(BufferAllocator& instance);
	...
	virtual z0 Deallocate(AllocatedBuffer* allocatedBuffer);
};
...
public:
	...
	z0 SetBuffer(UInt32 index, const AudioBuffer& ab)
	{
		auto& myabl = mBuffers->mAudioBufferList;
	...
	z0 InvalidateBufferList() noexcept { mPtrState = EPtrState::Invalid; }
	...
	z0 CopyBufferListTo(AudioBufferList& abl) const
	{
		ausdk::ThrowExceptionIf(mPtrState == EPtrState::Invalid, -1);
	...
	z0 CopyBufferContentsTo(AudioBufferList& destabl) const
	{
		ausdk::ThrowExceptionIf(mPtrState == EPtrState::Invalid, -1);
	...
	z0 Allocate(const AudioStreamBasicDescription& format, UInt32 nFrames);

	z0 Deallocate();

	// AudioBufferList utilities
	static z0 ZeroBuffer(AudioBufferList& abl)
	{
