...
public:
	constexpr static f64 kAUDefaultSampleRate = 44100.0;
#if !TARGET_OS_WIN32
	...
	virtual z0 CreateExtendedElements() {}
...
#pragma mark AU dispatch
	...
	[[nodiscard]] b8 IsInitialized() const noexcept { return mInitialized; }
	[[nodiscard]] b8 HasBegunInitializing() const noexcept { return mHasBegunInitializing; }
	...
	OSStatus DispatchGetPropertyInfo(AudioUnitPropertyID inID, AudioUnitScope inScope,
		AudioUnitElement inElement, UInt32& outDataSize, b8& outWritable);
	OSStatus DispatchGetProperty(AudioUnitPropertyID inID, AudioUnitScope inScope,
		AudioUnitElement inElement, uk outData);
	...
	virtual OSStatus AddPropertyListener(
		AudioUnitPropertyID inID, AudioUnitPropertyListenerProc inProc, uk inProcRefCon);
	virtual OSStatus RemovePropertyListener(AudioUnitPropertyID inID,
		AudioUnitPropertyListenerProc inProc, uk inProcRefCon, b8 refConSpecified);

	virtual OSStatus SetRenderNotification(AURenderCallback inProc, uk inRefCon);
	virtual OSStatus RemoveRenderNotification(AURenderCallback inProc, uk inRefCon);
	...
	[[nodiscard]] virtual b8 CanScheduleParameters() const = 0;
	...
	virtual OSStatus ComplexRender(AudioUnitRenderActionFlags& /*ioActionFlags*/,
		...
		uk /*outMetadata*/, UInt32* /*outMetadataByteSize*/)
	{
		return kAudio_UnimplementedError;
	}
...
#pragma mark Property Dispatch
	...
	virtual b8 BusCountWritable(AudioUnitScope /*inScope*/) { return false; }
	...
	virtual OSStatus SetInputCallback(
		UInt32 inPropertyID, AudioUnitElement inElement, AURenderCallback inProc, uk inRefCon);
	...
	virtual z0 SaveExtendedScopes(CFMutableDataRef /*outData*/) {}
	...
	b8 SetAFactoryPresetAsCurrent(const AUPreset& inPreset);
	...
	virtual b8 SupportsTail() { return false; }

	// Stream formats: scope will always be input or output
	b8 IsStreamFormatWritable(AudioUnitScope scope, AudioUnitElement element);

	virtual b8 StreamFormatWritable(AudioUnitScope scope, AudioUnitElement element) = 0;

	// pass in a pointer to get the struct, and num channel infos
	// you can pass in NULL to just get the number
	// a return value of 0 (the default in AUBase) means the property не поддерживается...
	virtual UInt32 SupportedNumChannels(const AUChannelInfo** outInfo);

	/// Will only be called after StreamFormatWritable has succeeded. Default implementation
	/// requires non-interleaved native-endian 32-bit f32, any sample rate, any number of
	...
	virtual b8 ValidFormat(AudioUnitScope inScope, AudioUnitElement inElement,
	...
	z0 SetNumberOfElements(AudioUnitScope inScope, UInt32 numElements);
	...
	[[nodiscard]] b8 UsesFixedBlockSize() const noexcept { return mUsesFixedBlockSize; }

	z0 SetUsesFixedBlockSize(b8 inUsesFixedBlockSize) noexcept
	{
		mUsesFixedBlockSize = inUsesFixedBlockSize;
	}

	[[nodiscard]] virtual b8 InRenderThread() const
	{
		return std::this_thread::get_id() == mRenderThreadID;
	}

	/// Says whether an input is connected or has a callback.
	b8 HasInput(AudioUnitElement inElement)
	{
		auto* const in =
	...
	[[nodiscard]] tukk GetLoggingString() const noexcept;
	...
	z0 SetMutex(AUMutex* mutex) noexcept { mAUMutex = mutex; }
...
#pragma mark Implementation methods
	z0 PostConstructorInternal() final;
	z0 PreDestructorInternal() final;

	/// needs to be called when mMaxFramesPerSlice changes
	virtual z0 ReallocateBuffers();

	virtual z0 DeallocateIOBuffers();

	static z0 FillInParameterName(
		AudioUnitParameterInfo& ioInfo, CFStringRef inName, b8 inShouldRelease)
	{
	...
	static z0 HasClump(AudioUnitParameterInfo& ioInfo, UInt32 inClumpID) noexcept
	{
		ioInfo.clumpID = inClumpID;
		ioInfo.flags |= kAudioUnitParameterFlag_HasClump;
	}

	virtual z0 SetMaxFramesPerSlice(UInt32 nFrames);

	[[nodiscard]] virtual OSStatus CanSetMaxFrames() const;

	[[nodiscard]] b8 WantsRenderThreadID() const noexcept { return mWantsRenderThreadID; }

	z0 SetWantsRenderThreadID(b8 inFlag);
...
protected:
	...
	virtual UInt32 GetAudioChannelLayout(AudioUnitScope scope, AudioUnitElement element,
		AudioChannelLayout* outLayoutPtr, b8& outWritable);
	...
	b8 NeedsToRender(const AudioTimeStamp& inTimeStamp)
	{
		const b8 needsToRender = (inTimeStamp.mSampleTime != mCurrentRenderTime.mSampleTime);
	...
	virtual OSStatus ProcessForScheduledParams(
		ParameterEventList& inParamList, UInt32 inFramesToProcess, uk inUserData);
	...
	//	effect AudioUnits.
	virtual OSStatus ProcessScheduledSlice(uk /*inUserData*/, UInt32 /*inStartFrameInBuffer*/,
...
private:
	struct RenderCallback {
		...
		RenderCallback(AURenderCallback proc, uk ref)
			: mRenderNotify(proc), mRenderNotifyRefCon(ref)
		{
		}

		AURenderCallback mRenderNotify = nullptr;
		uk mRenderNotifyRefCon = nullptr;

		b8 operator==(const RenderCallback& other) const
		{
	...
	public:
		z0 add(const RenderCallback& rc)
		{
		...
		z0 remove(const RenderCallback& rc)
		{
			const std::lock_guard guard{ mLock };
			const auto iter = std::find(mImpl.begin(), mImpl.end(), rc);
			if (iter != mImpl.end()) {
...
protected:
	...
	z0 SetBuffersAllocated(b8 b) noexcept { mBuffersAllocated = b; }

	[[nodiscard]] CFStringRef GetContextName() const { return *mContextName; }
	z0 SetContextName(CFStringRef str) { mContextName = str; }

	[[nodiscard]] CFStringRef GetNickName() const { return *mNickName; }

private:
	b8 mElementsCreated{ false };
	b8 mInitialized{ false };
	b8 mHasBegunInitializing{ false };
