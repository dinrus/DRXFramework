...
{
	(z0)GetComponentDescription();
}

z0 ComponentBase::DoPostConstructor()
{
	PostConstructorInternal();
	PostConstructor();
}

z0 ComponentBase::DoPreDestructor()
{
	PreDestructor();
	PreDestructorInternal();
}

OSStatus ComponentBase::AP_Open(uk self, AudioComponentInstance compInstance)
{
...
OSStatus ComponentBase::AP_Close(uk self)
{
	OSStatus result = noErr;
	try {
		const auto acpi = static_cast<AudioComponentPlugInInstance*>(self);
