...
inline z0 ThrowExceptionIf(b8 condition, OSStatus err)
{
	if (condition) {
		AUSDK_LogError("throwing %d", static_cast<i32>(err));
		throw AUException{ err };
	}
}

[[noreturn]] inline z0 Throw(OSStatus err)
{
	AUSDK_LogError("throwing %d", static_cast<i32>(err));
	throw AUException{ err };
}

inline z0 ThrowQuietIf(b8 condition, OSStatus err)
{
	if (condition) {
		throw AUException{ err };
	}
...
[[noreturn]] inline z0 ThrowQuiet(OSStatus err) { throw AUException{ err }; }
...
public:
	...
	virtual z0 lock() { mImpl.lock(); }
	virtual z0 unlock() { mImpl.unlock(); }
	virtual b8 try_lock() { return mImpl.try_lock(); }
...
constexpr b8 IsInterleaved(const AudioStreamBasicDescription& format) noexcept
{
...
constexpr b8 IsCommonFloat32(const AudioStreamBasicDescription& format) noexcept
{
	return (
		...
		&& format.mBytesPerFrame == NumberInterleavedChannels(format) * sizeof(f32));
}

constexpr AudioStreamBasicDescription CreateCommonFloat32(
	Float64 sampleRate, UInt32 numChannels, b8 interleaved = false) noexcept
{
...
constexpr b8 MinimalSafetyCheck(const AudioStreamBasicDescription& x) noexcept
{
...
constexpr b8 operator==(const AudioChannelLayout& lhs, const AudioChannelLayout& rhs) noexcept
{
...
public:
	...
	AUChannelLayout(u32 inNumberChannelDescriptions, AudioChannelLayoutTag inChannelLayoutTag,
	...
	b8 operator==(const AUChannelLayout& other) const noexcept
	{
		return ACL::operator==(Layout(), other.Layout());
	}

	b8 operator!=(const AUChannelLayout& y) const noexcept { return !(*this == y); }

	[[nodiscard]] b8 IsValid() const noexcept { return NumberChannels() > 0; }
	...
	[[nodiscard]] u32 NumberChannels() const noexcept { return NumberChannels(*LayoutPtr()); }

	[[nodiscard]] u32 Size() const noexcept { return static_cast<u32>(mStorage.size()); }

	static u32 NumberChannels(const AudioChannelLayout& inLayout) noexcept
	{
...
inline u32 IsBogusAudioBufferList(const AudioBufferList& abl)
{
	...
	u32 anyNull = 0;
	for (; buf < bufEnd; ++buf) {
		u32k* const p = static_cast<u32k*>(buf->mData);
		if (p == nullptr) {
			anyNull = 1;
			continue;
		}
...
inline zu64 Current() { return mach_absolute_time(); }

/// Returns the frequency of the host timebase, in ticks per second.
inline f64 Frequency()
{
	struct mach_timebase_info timeBaseInfo {
	}; // NOLINT
	mach_timebase_info(&timeBaseInfo);
	//	the frequency of that clock is: (sToNanosDenominator / sToNanosNumerator) * 10^9
	return static_cast<f64>(timeBaseInfo.denom) / static_cast<f64>(timeBaseInfo.numer) *
		   1.0e9; // NOLINT
}
...
template <typename T>
class Owned {
	explicit Owned(T obj, b8 fromget) noexcept : mImpl{ obj }
	{
		if (fromget) {
			retainRef();
		}
...
constexpr b8 safe_isprint(t8 in_char) noexcept { return (in_char >= ' ') && (in_char <= '~'); }

inline std::string make_string_from_4cc(u32 in_4cc) noexcept
{
#if !TARGET_RT_BIG_ENDIAN
	in_4cc = OSSwapInt32(in_4cc); // NOLINT
#endif

	tuk const string = reinterpret_cast<tuk>(&in_4cc); // NOLINT
