...
constexpr u32 RoundUpToMultipleOfPowerOf2(u32 x, u32 y) noexcept
{
	const auto mask = y - 1;
#if DEBUG
	...
	UInt32 numberBuffers, UInt32 maxBytesPerBuffer, UInt32 /*reservedFlags*/)
{
	...
	const auto implSize = static_cast<u32>(
...
z0 BufferAllocator::Deallocate(AllocatedBuffer* allocatedBuffer)
{
...
z0 AUBufferList::Allocate(const AudioStreamBasicDescription& format, UInt32 nFrames)
{
	auto& alloc = BufferAllocator::instance();
	if (mBuffers != nullptr) {
		alloc.Deallocate(mBuffers);
	}
	u32k nstreams = ASBD::IsInterleaved(format) ? 1 : format.mChannelsPerFrame;
