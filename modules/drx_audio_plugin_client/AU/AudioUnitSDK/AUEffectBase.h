...
public:
	explicit AUEffectBase(AudioComponentInstance audioUnit, b8 inProcessesInPlace = true);
	...
	OSStatus GetPropertyInfo(AudioUnitPropertyID inID, AudioUnitScope inScope,
		AudioUnitElement inElement, UInt32& outDataSize, b8& outWritable) override;
	OSStatus GetProperty(AudioUnitPropertyID inID, AudioUnitScope inScope,
		AudioUnitElement inElement, uk outData) override;
	OSStatus SetProperty(AudioUnitPropertyID inID, AudioUnitScope inScope,
		AudioUnitElement inElement, ukk inData, UInt32 inDataSize) override;
	...
	[[nodiscard]] b8 CanScheduleParameters() const override { return true; }

	// This is used for the property value - to reflect to the UI if an effect is bypassed
	[[nodiscard]] b8 IsBypassEffect() const noexcept { return mBypassEffect; }

	virtual z0 SetBypassEffect(b8 inFlag) { mBypassEffect = inFlag; }

	z0 SetParamHasSampleRateDependency(b8 inFlag) noexcept { mParamSRDep = inFlag; }
	[[nodiscard]] b8 GetParamHasSampleRateDependency() const noexcept { return mParamSRDep; }

	/// Context, passed as `uk userData`, for `ProcessScheduledSlice()`.
	...
	OSStatus ProcessScheduledSlice(uk inUserData, UInt32 inStartFrameInBuffer,
		UInt32 inSliceFramesToProcess, UInt32 inTotalBufferFrames) override;

	[[nodiscard]] b8 ProcessesInPlace() const noexcept { return mProcessesInPlace; }
	z0 SetProcessesInPlace(b8 inProcessesInPlace) noexcept
	{
...
protected:
	...
	virtual b8 ShouldBypassEffect() { return IsBypassEffect(); }
	...
	b8 IsInputSilent(AudioUnitRenderActionFlags inActionFlags, UInt32 inFramesToProcess)
	{
		b8 inputSilent = (inActionFlags & kAudioUnitRenderAction_OutputIsSilence) != 0;
...
#if TARGET_OS_IPHONE
	z0 SetOnlyOneKernel(b8 inUseOnlyOneKernel) noexcept
	{
...
private:
	...
	b8 mBypassEffect{ false };
	b8 mParamSRDep{ false };
	b8 mProcessesInPlace;
...
public:
	...
	virtual z0 Process(const Float32* /*inSourceP*/, Float32* /*inDestP*/,
		UInt32 /*inFramesToProcess*/, b8& /*ioSilence*/) = 0;
	...
	z0 SetChannelNum(UInt32 inChan) noexcept { mChannelNum = inChan; }
