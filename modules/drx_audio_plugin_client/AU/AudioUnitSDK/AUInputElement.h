...
public:
	...
	[[nodiscard]] b8 NeedsBufferSpace() const override { return IsCallback(); }
	z0 SetConnection(const AudioUnitConnection& conn);
	z0 SetInputCallback(AURenderCallback proc, uk refCon);
	[[nodiscard]] b8 IsActive() const noexcept { return mInputType != EInputType::NoInput; }
	[[nodiscard]] b8 IsCallback() const noexcept
	{
		return mInputType == EInputType::FromCallback;
	}
	[[nodiscard]] b8 HasConnection() const noexcept
	{
