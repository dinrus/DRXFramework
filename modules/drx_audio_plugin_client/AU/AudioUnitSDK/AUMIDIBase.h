...
public:
	...
	virtual z0 GetMaps(AUParameterMIDIMapping* outMapping) = 0;
	virtual z0 GetHotParameterMap(AUParameterMIDIMapping& outMapping) = 0;

	virtual z0 AddParameterMapping(
		const AUParameterMIDIMapping* maps, UInt32 count, AUBase& auBase) = 0;
	virtual z0 RemoveParameterMapping(
		const AUParameterMIDIMapping* maps, UInt32 count, b8& outDidChange) = 0;
	virtual z0 SetHotMapping(const AUParameterMIDIMapping& mapping) = 0;
	virtual z0 ReplaceAllMaps(
		const AUParameterMIDIMapping* maps, UInt32 count, AUBase& auBase) = 0;

	virtual b8 HandleHotMapping(UInt8 status, UInt8 channel, UInt8 data1, AUBase& auBase) = 0;
	virtual b8 FindParameterMapEventMatch(UInt8 status, UInt8 channel, UInt8 data1, UInt8 data2,
...
#if AUSDK_HAVE_MIDI_MAPPING
	z0 SetMIDIMapper(const std::shared_ptr<AUMIDIMapper>& mapper) { mMIDIMapper = mapper; }
#endif
