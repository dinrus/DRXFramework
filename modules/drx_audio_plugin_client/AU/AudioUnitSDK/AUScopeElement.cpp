...
//
z0 AUElement::UseIndexedParameters(UInt32 inNumberOfParameters)
{
...
//
b8 AUElement::HasParameterID(AudioUnitParameterID paramID) const
{
...
//_____________________________________________________________________________
//
z0 AUElement::SetParameter(
	AudioUnitParameterID paramID, AudioUnitParameterValue inValue, b8 okWhenInitialized)
{
	if (mUseIndexedParameters) {
		ausdk::ThrowExceptionIf(
			paramID >= mIndexedParameters.size(), kAudioUnitErr_InvalidParameter);
		...
		if (i == mParameters.end()) {
			if (mAudioUnit.IsInitialized() && !okWhenInitialized) {
				...
				AUSDK_LogError(
					...
					"Ignoring.",
					mAudioUnit.GetLoggingString(), static_cast<u32>(paramID));
			} else {
...
//_____________________________________________________________________________
//
z0 AUElement::SetScheduledEvent(AudioUnitParameterID paramID,
	const AudioUnitParameterEvent& inEvent, UInt32 /*inSliceOffsetInBuffer*/,
	UInt32 /*inSliceDurationFrames*/, b8 okWhenInitialized)
{
...
//_____________________________________________________________________________
//
z0 AUElement::GetParameterList(AudioUnitParameterID* outList)
{
	if (mUseIndexedParameters) {
...
//_____________________________________________________________________________
//
z0 AUElement::SaveState(AudioUnitScope scope, CFMutableDataRef data)
{
	AudioUnitParameterInfo paramInfo{};
	const CFIndex countOffset = CFDataGetLength(data);
	u32 paramsWritten = 0;

	const auto appendBytes = [data](ukk bytes, CFIndex length) {
	...
	const auto appendParameter = [&](AudioUnitParameterID paramID, AudioUnitParameterValue value) {
		struct {
			UInt32 paramID;
			UInt32 value; // really a big-endian f32
		} entry{};
...
z0 AUIOElement::AllocateBuffer(UInt32 inFramesToAllocate)
{
...
//_____________________________________________________________________________
//
z0 AUIOElement::DeallocateBuffer() { mIOBuffer.Deallocate(); }
...
UInt32 AUIOElement::GetAudioChannelLayout(AudioChannelLayout* outLayoutPtr, b8& outWritable)
{
	outWritable = true;
...
//_____________________________________________________________________________
//
z0 AUScope::SetNumberOfElements(UInt32 numElements)
{
...
//_____________________________________________________________________________
//
b8 AUScope::HasElementWithName() const
{
...
z0 AUScope::AddElementNamesToDict(CFMutableDictionaryRef inNameDict) const
{
		...
		for (UInt32 i = 0; i < GetNumberOfElements(); ++i) {
			...
			if (el != nullptr && el->HasName()) {
				const auto key = Owned<CFStringRef>::from_create(CFStringCreateWithFormat(
					nullptr, nullptr, CFSTR("%u"), static_cast<u32>(i)));
				CFDictionarySetValue(*elementDict, *key, *el->GetName());
			}
		}

		const auto key = Owned<CFStringRef>::from_create(
			CFStringCreateWithFormat(nullptr, nullptr, CFSTR("%u"), static_cast<u32>(mScope)));
...
std::vector<AudioUnitElement> AUScope::RestoreElementNames(CFDictionaryRef inNameDict) const
{
	...
	CFDictionaryGetKeysAndValues(
		inNameDict, reinterpret_cast<ukk*>(keys.data()), nullptr); // NOLINT
	for (size_t i = 0; i < dictSize; i++) {
		u32 intKey = 0;
		std::array<t8, 32> string{};
		CFStringGetCString(keys[i], string.data(), string.size(), kCFStringEncodingASCII);
		i32k result = sscanf(string.data(), "%u", &intKey); // NOLINT
...
z0 AUScope::SaveState(CFMutableDataRef data) const
{
