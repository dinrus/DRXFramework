...
//
AUEffectBase::AUEffectBase(AudioComponentInstance audioUnit, b8 inProcessesInPlace)
	...
	if ((numIOconfigs > 0) && (auChannelConfigs != nullptr)) {
		...
		for (UInt32 i = 0; (i < numIOconfigs) && !foundMatch; ++i) {
			...
			if ((configNumInputs < 0) && (configNumOutputs < 0)) {
				...
				const b8 inputMatch = (auNumInputs == configNumInputs) || (configNumInputs == -1);
				const b8 outputMatch =
	...
	AudioUnitElement inElement, ukk inData, UInt32 inDataSize)
{
	if (inScope == kAudioUnitScope_Global) {
		...
		case kAudioUnitProperty_BypassEffect: {
			...
			const b8 tempNewSetting = *static_cast<const UInt32*>(inData) != 0;
...
z0 AUEffectBase::MaintainKernels()
{
#if TARGET_OS_IPHONE
...
b8 AUEffectBase::StreamFormatWritable(AudioUnitScope /*scope*/, AudioUnitElement /*element*/)
{
...
//
OSStatus AUEffectBase::ProcessScheduledSlice(uk inUserData, UInt32 /*inStartFrameInBuffer*/,
	...
	const AudioBufferList& inBuffer, AudioBufferList& outBuffer, UInt32 inFramesToProcess)
{
	...
	const b8 silentInput = IsInputSilent(ioActionFlags, inFramesToProcess);
