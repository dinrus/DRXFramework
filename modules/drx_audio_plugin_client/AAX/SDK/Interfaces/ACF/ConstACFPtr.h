...
public:
    ...
    /// Constructor shorthand for reference0->QueryInterface(iid, (uk*)&_reference).
	...
	/// a T** or a uk*, in order to fill it in. (e.g. QueryInterface). NOTE: if the internal
    ...
    /// Allows calling operator ! just like a regular pointer. Возвращает true, если the 
    ...
    : _reference (0)
{
	if (!reference0) {
		...
		T* ref = NULL;;
		ACFRESULT result = reference0->QueryInterface (iid, reinterpret_cast <uk*> (&ref));
		_reference = ref;
		if (ACFFAILED(result))
#if ACFPTR_CAN_THROW
    ...
    : _reference (0)
{
	if (!reference0) {
		...
		T* ref = NULL;;
		ACFRESULT result = ref0->QueryInterface (iid, reinterpret_cast <uk*> (&ref));
		_reference = ref;
		if (ACFFAILED(result))
#if ACFPTR_CAN_THROW
			throw ACFRESULT(result);
...
template <typename T>
inline b8 ConstACFPtr<T>::isNull () const
{
...
template <typename T>
inline b8 ConstACFPtr<T>::operator! () const
{
    return isNull();
}
...
template <typename T>
inline z0 ConstACFPtr<T>::attach (const T * reference)
{
    clear();
    _reference = reference;
}
...
template <typename T>
inline const T * ConstACFPtr<T>::detach (z0)
{
#if ACFPTR_CAN_THROW
	if (NULL == _reference)
		throw ACFRESULT(ACF_E_POINTER);
...
template <typename T>
inline z0 ConstACFPtr<T>::acquire(z0)
{
	if ( _reference )
	{
...
template <typename T>
inline z0 ConstACFPtr<T>::clear (z0)
{
	if ( _reference )
	{
		T* ref = const_cast<T*>(_reference);
