...
#define ACF_DECLARE_STANDARD_UNKNOWN() \
  ACFMETHOD(QueryInterface)(const acfIID & riid, uk *ppvObjOut) ACF_OVERRIDE \
  { \
    return ExternalQueryInterface(riid, ppvObjOut); \
  } \
  ACFMETHOD_(acfUInt32, AddRef)(z0) ACF_OVERRIDE \
  { \
    return ExternalAddRef(); \
  } \
  ACFMETHOD_(acfUInt32, Release)(z0) ACF_OVERRIDE \
  { \
...
/*\
  ACFMETHOD(QueryInterface)(const acfIID & riid, uk *ppvObjOut) \
  { \
    return InternalQueryInterface(riid, ppvObjOut); \
  } \
  ACFMETHOD_(acfUInt32, InternalAddRef)(z0) \
  { \
    return 1; \
  } \
  ACFMETHOD_(acfUInt32, InternalRelease)(z0) \
  { \
    return 1; \
  } \
  ACFMETHOD_(acfUInt32, AddRef)(z0) \
  { \
    return 1; \
  } \
  ACFMETHOD_(acfUInt32, Release)(z0) \
  { \
    return 1; \
  }*/
...
#define ACF_DECLARE_COM_QUERYINTERFACE() \
  ACFMETHOD(QueryInterface)(REFIID riid, uk *ppvObjOut) \
  { \
...
// 
//   // Declare the factory for this class.
//   static ACFRESULT ACFCreate(IACFUnknown *pUnkOuter, uk *ppvObjOut);
//
//   // Declare the standard delegating unknown methods
//   ACFMETHOD(QueryInterface)(const acfIID & riid, uk *ppvObjOut)
//   {
...
#define ACF_DECLARE_FACTORY() \
    static ACFRESULT ACFCreate(IACFUnknown *pUnkHost, IACFUnknown *pUnkOuter, const acfIID & iid, uk *ppvObjOut)
...
// ACFRESULT CFoo::ACFCreate(IACFUnknown *pUnkHost, IACFUnknown *pUnkOuter, uk *ppvObjOut)
// {
...
ACFRESULT xclass::ACFCreate(IACFUnknown * pUnkHost, IACFUnknown *pUnkOuter, const acfIID & iid, uk *ppvObjOut) \
{ \
  ACFRESULT result = ACF_OK; \
  *ppvObjOut = 0; \
  xclass *pNewObject = new (std::nothrow) xclass(pUnkOuter); \
...
public:
  ...
  static acfUInt32 GetActiveObjectCount(z0);
...
protected:
  ...
  // unknown
  //
  inline IACFUnknown *GetPrivateUnknown (z0)
  {
    return &m_UnkPrivate;
  }
  ...
  ACFMETHOD(InternalQueryInterface)(const acfIID & riid, uk *ppvObjOut);
  ...
  // Manage total number of outstanding "live" objects. Used by ACFCanUnloadNow.
  static acfUInt32 IncrementActiveObjects(z0);
  static acfUInt32 DecrementActiveObjects(z0);
...
private:
  ...
  public:
    ...
    inline z0 SetParentUnknown(CACFUnknown *parent)
    {
      m_This = parent;
    }
