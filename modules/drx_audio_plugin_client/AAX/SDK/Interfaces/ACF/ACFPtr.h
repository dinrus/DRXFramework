    ...
    : _reference (0)
{
	if (!reference0) {
		...
		ACFRESULT result = reference0->QueryInterface (iid, reinterpret_cast <uk*> (&_reference));
		if (ACFFAILED(result))
#if ACFPTR_CAN_THROW
...
template <typename T>
inline b8 ACFPtr<T>::isNull () const
{
...
template <typename T>
inline b8 ACFPtr<T>::operator! () const
{
    return isNull();
}
...
template <typename T>
inline z0 ACFPtr<T>::attach (T * reference)
{
    clear();
    _reference = reference;
}
