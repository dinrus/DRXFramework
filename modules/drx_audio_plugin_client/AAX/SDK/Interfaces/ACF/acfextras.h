...
inline b8 operator == (const acfUID& lhs, const acfUID& rhs)
{
...
inline b8 operator != (const acfUID& lhs,
                         const acfUID& rhs)
{
    return !(lhs == rhs);
}
...
// pUnk->QueryInterface(IID_IXInterfaceType, (uk *)&pInterface);
//
...
#define IID_PPV_ARG(Type, Expr) IID_##Type, \
	reinterpret_cast<uk *>(static_cast<Type **>(Expr))

// Namespace version of IID_PPV_ARG.
#define NS_IID_PPV_ARG(NameSpace, Type, Expr) \
	NameSpace::IID_##Type, \
	reinterpret_cast<uk *>(static_cast<NameSpace::Type **>(Expr))
...
*/
inline z0 acfcheck(ACFRESULT result)
{
    if (ACFFAILED(result))
