...
*/
	...
	ACFRESULT _acfcheckM( ACFRESULT result, const wchar_t* file , const wchar_t* functioname, i32 line );
	#define acfcheckM(_result_) (_acfcheckM( _result_, WIDEN(__FILE__) , WIDEN(__FUNCTION__) , __LINE__ ))
#else
	#ifdef THROWACFRESULT
		inline z0 acfcheckM( ACFRESULT _result_) { if (ACFFAILED(_result_)) THROWRESULT ACFRESULT(_result_);}
	#else
