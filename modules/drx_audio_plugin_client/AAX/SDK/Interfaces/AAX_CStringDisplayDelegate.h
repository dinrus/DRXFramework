...
/**	\brief A string, or list, display format conforming to AAX_IDisplayDelegate
	...
	list parameters, which would internally use an integer parameter type.  The i32 value
...
public:
	/** \brief Constructor
	 *
	 *	Constructs a Txt Display Delegate with a provided string map.
	 *
...
template <typename T>
b8		AAX_CStringDisplayDelegate<T>::ValueToString(T value, AAX_CString* valueString) const
{
	...
	*valueString = AAX_CString("Txt Not Found");
	return false;
}

template <typename T>
b8		AAX_CStringDisplayDelegate<T>::ValueToString(T value, i32 /*maxNumChars*/, AAX_CString* valueString) const
{
	// First, get the full length string.
	b8 result = this->ValueToString(value, valueString);
...
template <typename T>
b8		AAX_CStringDisplayDelegate<T>::StringToValue(const AAX_CString& valueString, T* value) const
{
