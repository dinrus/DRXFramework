...
public:
	 ...
	 */
	AAX_CBinaryDisplayDelegate(tukk falseString, tukk trueString);
	...
	virtual z0						AddShortenedStrings(tukk falseString, tukk trueString, i32 iStrLength);
...
template <typename T>
AAX_CBinaryDisplayDelegate<T>::AAX_CBinaryDisplayDelegate(tukk falseString, tukk trueString) :
...
template <typename T>
z0 AAX_CBinaryDisplayDelegate<T>::AddShortenedStrings(tukk falseString, tukk trueString, i32 iStrLength)
{
...
template <typename T>
b8	AAX_CBinaryDisplayDelegate<T>::ValueToString(T value, AAX_CString* valueString) const
{
	if (value)
...
template <typename T>
b8	AAX_CBinaryDisplayDelegate<T>::ValueToString(T value, i32 maxNumChars, AAX_CString* valueString) const
{
	...
	u32k maxNumCharsUnsigned = (0 <= maxNumChars) ? static_cast<u32>(maxNumChars) : 0;
	...
	for ( i32 i = static_cast<i32>(mShortenedStrings.size())-1; i >= 0; i-- )
	{
		struct StringTable shortStrings = mShortenedStrings.at(static_cast<u32>(i));
...
template <typename T>
b8	AAX_CBinaryDisplayDelegate<T>::StringToValue(const AAX_CString& valueString, T* value) const
{
