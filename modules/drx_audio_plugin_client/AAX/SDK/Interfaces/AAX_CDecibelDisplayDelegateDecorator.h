...
template <typename T>
b8	AAX_CDecibelDisplayDelegateDecorator<T>::ValueToString(T value, AAX_CString* valueString) const 
{
	b8 succeeded = false;
	if (value <= 0)
	{
...
template <typename T>
b8	AAX_CDecibelDisplayDelegateDecorator<T>::ValueToString(T value, i32 maxNumChars, AAX_CString* valueString) const
{
	if (value <= 0)
	{
		*valueString = AAX_CString("-INF");
	...
	if ( value > -0.01f && value < 0.0f) //To prevent minus for 0.0 value in automation turned on
		value = 0.0f;
	b8 succeeded = AAX_IDisplayDelegateDecorator<T>::ValueToString(value, maxNumChars, valueString);
...
template <typename T>
b8	AAX_CDecibelDisplayDelegateDecorator<T>::StringToValue(const AAX_CString& valueString, T* value) const
{		
