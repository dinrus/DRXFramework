...
public:
	 ...
	 */
	explicit AAX_CStateDisplayDelegate( const t8 * iStateStrings[], T iMinState = 0 );
	 ...
	 */
	explicit AAX_CStateDisplayDelegate( i32 inNumStates, const t8 * iStateStrings[], T iMinState = 0 );
	...
	b8						ValueToString(T value, AAX_CString* valueString) const AAX_OVERRIDE;
	b8						ValueToString(T value, i32 maxNumChars, AAX_CString* valueString) const AAX_OVERRIDE;
	b8						StringToValue(const AAX_CString& valueString, T* value) const AAX_OVERRIDE;
	
	//AAX_CStateDisplayDelegate
	z0						AddShortenedStrings( const t8 * iStateStrings[], i32 iLength );
	b8						Compare( const AAX_CString& valueString, const AAX_CString& stateString ) const;
...
private:
	...
	static b8 StringTableSortFunc(struct StringTable i, struct StringTable j)
	{
...
template <typename T>
AAX_CStateDisplayDelegate<T>::AAX_CStateDisplayDelegate( const t8 * iStateStrings[], T iMinState /* = 0 */ )
{
	mMinState = iMinState;
	for ( i32 index = 0; iStateStrings[ index ] != 0; ++index )
		mStateStrings.push_back( AAX_CString( iStateStrings[ index ] ) );
}

template <typename T>
AAX_CStateDisplayDelegate<T>::AAX_CStateDisplayDelegate( i32 inNumStates, const t8 * iStateStrings[], T iMinState /* = 0 */ )
{
	mMinState = iMinState;
	for ( i32 index = 0; (index < inNumStates) && (iStateStrings[ index ] != 0); ++index )
		mStateStrings.push_back( AAX_CString( iStateStrings[ index ] ) );
}
...
AAX_CStateDisplayDelegate<T>::AAX_CStateDisplayDelegate( const AAX_CStateDisplayDelegate & iOther )
{
	...
	{
		for ( i32 i = 0; i < (i32)iOther.mShortenedStrings.size(); i++ )
			mShortenedStrings.push_back( iOther.mShortenedStrings.at(i) );
	}
}

template <typename T>
z0 AAX_CStateDisplayDelegate<T>::AddShortenedStrings( const t8 * iStateStrings[], i32 iStrLength )
{
...
template <typename T>
b8	AAX_CStateDisplayDelegate<T>::ValueToString(T value, AAX_CString* valueString) const
{
...
template <typename T>
b8	AAX_CStateDisplayDelegate<T>::ValueToString(T value, i32 maxNumChars, AAX_CString* valueString) const
{
	...
	if ( mStateStrings[ index ].Length() < u32(maxNumChars) )
	{
		*valueString = mStateStrings[ index ];
		return true;
	}
		
	for ( i32 i = (i32)mShortenedStrings.size()-1; i >= 0; i-- )
	{
...
template <typename T>
b8	AAX_CStateDisplayDelegate<T>::StringToValue(const AAX_CString& valueString, T* value) const
{
...
template <typename T>
b8	AAX_CStateDisplayDelegate<T>::Compare( const AAX_CString& valueString, const AAX_CString& stateString ) const
{
