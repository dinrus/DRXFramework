...
#define AAX_UINT32_MIN     0U              /** minimum u32 32 bit value */
#define AAX_UINT32_MAX     4294967295U     /** maximum u32 32 bit value */
#define AAX_INT16_MIN    (-32767 - 1)      /** minimum signed 16 bit value */
#define AAX_INT16_MAX      32767           /** maximum signed 16 bit value */
#define AAX_UINT16_MIN     0U              /** minimum u32 16 bit value */
#define AAX_UINT16_MAX     65535U          /** maximum u32 16 bit value */

/** \brief Macro to ensure enum type consistency across binaries
 */
#ifndef _TMS320C6X
#define AAX_ENUM_SIZE_CHECK(x) extern i32 __enumSizeCheck[ 2*(sizeof(u32)==sizeof(x)) - 1]
#else
...
// signed/u32 mismatch if anyone tries to create an
// AAX_STEM_FORMAT definition with a negative channel count.
#define AAX_STEM_FORMAT( aIndex, aChannelCount )		( static_cast<u32>( ( static_cast<u16>(aIndex) << 16 ) | ( (aChannelCount >= AAX_UINT16_MIN) && (aChannelCount <= 0xFFFF) ? aChannelCount & 0xFFFF : 0x0000 ) ) )
#define AAX_STEM_FORMAT_CHANNEL_COUNT( aStemFormat )	( static_cast<u16>( aStemFormat & 0xFFFF ) )
#define AAX_STEM_FORMAT_INDEX( aStemFormat )			( static_cast<i16>( ( aStemFormat >> 16 ) & 0xFFFF ) )
...
/*!	@brief Events IDs for %AAX notifications
	...
	\note All 'AX__' four-t8 IDs are reserved for the %AAX specification
 */
 ...
 */
typedef i32		AAX_EParameterOrientation;
