...
public: // AAX_IContainer
	virtual z0 Clear();                                                ///< @copydoc AAX_IPointerQueue::Clear()
...
template <typename T, size_t S>
inline z0 AAX_CAtomicQueue<T, S>::Clear()
{
	std::memset((uk)mRingBuffer, 0x0, sizeof(mRingBuffer));
}
...
inline AAX_IContainer::EStatus AAX_CAtomicQueue<T, S>::Push(typename AAX_CAtomicQueue<T, S>::value_type inElem)
{
    ...
    u32k idx = AAX_Atomic_IncThenGet_32(mWriteIdx);
    u32k widx = idx % S;
	
	// Do the push. If the value at the current write index is non-NULL then we have filled the buffer.
	const b8 cxResult = AAX_Atomic_CompareAndExchange_Pointer(mRingBuffer[widx], (value_type)0x0, inElem);
	
	if (false == cxResult)
	{
		result = AAX_IContainer::eStatus_Overflow;
				
		u32k ridx = (0 == idx) ? S : idx-1;
...
//		const b8 resetResult = AAX_Atomic_CompareAndExchange_32(mWriteIdx, idx, ridx);
...
inline typename AAX_CAtomicQueue<T, S>::value_type AAX_CAtomicQueue<T, S>::Peek() const
{
	...
	//    and write threads, as i64 as they are "close".
	u32k testIdx = (mReadIdx+1) % template_size;
...
inline typename AAX_CAtomicQueue<T, S>::value_type AAX_CAtomicQueue<T, S>::Pop()
{
	u32k idx = AAX_Atomic_IncThenGet_32(mReadIdx);
	u32k widx = idx % S;
