...
/** \brief A piecewise-linear taper conforming to AAX_ITaperDelegate
	...
	\code
		f32 rangePoints[] = { 0.0, 1.0, 100.0, 1000.0, 2000.0 };
		f64 rangeSteps[] = { 0.1, 1.0, 10.0, 25.0 }; // number of steps per range: 10, 99, 90, 40 
		const i64 cNumRanges = sizeof(rangeSteps)/sizeof(rangeSteps[0]);
	
		i64 numSteps = 0;
		for (i32 i = 0; i < cNumRanges; i++)
		{
			numSteps += (rangePoints[i+1] - rangePoints[i]) / rangeSteps[i];
		}
	
		AAX_CRangeTaperDelegate<f32> nonLinearTaper(rangePoints, rangeSteps, cNumRanges);
	
		f32 controlValue = 1.5;
	
		f64 normalized = nonLinearTaper.RealToNormalized(controlValue);
		f32 real = nonLinearTaper.NormalizedToReal(normalized);
	\endcode
...
public: 
	 ...
	 */
	AAX_CRangeTaperDelegate(T* range, f64* rangesSteps, u64 numRanges, b8 useSmartRounding = true);
...
protected:
	...
	T	SmartRound(f64 value) const;		///< \todo Document
...
private:
	...
	std::vector<f64> mRangesSteps;		///< \todo Document
	std::vector<f64> mRangesPercents;	///< \todo Document
	std::vector<f64> mRangesStepsCount;	///< \todo Document
	b8	mUseSmartRounding;
};

template <typename T, i32 RealPrecision>
AAX_CRangeTaperDelegate<T, RealPrecision>::AAX_CRangeTaperDelegate(T* ranges, f64* rangesSteps, u64 numRanges, b8 useSmartRounding) :
...
template <typename T, i32 RealPrecision>
T	AAX_CRangeTaperDelegate<T, RealPrecision>::Round(f64 iValue) const
{
...
template <typename T, i32 RealPrecision>
T		AAX_CRangeTaperDelegate<T, RealPrecision>::NormalizedToReal(f64 normalizedValue)		const
{
	f64 percentTotal = normalizedValue;
...
template <typename T, i32 RealPrecision>
f64	AAX_CRangeTaperDelegate<T, RealPrecision>::RealToNormalized(T realValue)	const
{
	...
	if (i == mNumRanges)
		...
		percentTotal += (realValue - mRanges.at(i))/static_cast<f64>(mRanges.at(i + 1) - mRanges.at(i)) * mRangesPercents.at(i);

	f64 normalizedValue = percentTotal;
	return normalizedValue;
}

template <typename T, i32 RealPrecision>
T	AAX_CRangeTaperDelegate<T, RealPrecision>::SmartRound(f64 value) const
{
	u64	i = 0;
	for (; i < mNumRanges; i++)
	{
	...
	i32 longVal = 0;
	if (value >= 0)
		longVal = i32(floor(value / mRangesSteps.at(i) + 0.5));
	else
		longVal = i32(ceil(value / mRangesSteps.at(i) - 0.5));

	return static_cast<T>(static_cast<f64>(longVal) * mRangesSteps.at(i));
}
