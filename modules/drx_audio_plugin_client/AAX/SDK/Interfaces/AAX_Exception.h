...
namespace AAX
{
	 ...
	 */
	inline std::string AsString(tukk inStr);
	inline const std::string& AsString(const std::string& inStr); ///< \copydoc AAX::AsString(tukk)
	inline const std::string& AsString(const Exception::Any& inStr); ///< \copydoc AAX::AsString(tukk)
	...
	namespace Exception
	{
		...
		public: // AAX::Exception::Any
			...
			operator const std::string&(z0) const { return mWhat; }
#endif
		...
		public:
			...
			: Any(ResultError::FormatResult(inWhatResult), inFunction, (tukk)NULL)
			, mResult(inWhatResult)
			{
			}
			...
			{
				return std::string(AAX::AsStringResult(inResult) + " (" + AAX::AsStringInt32((i32)inResult) + ")");
			}
	...
	std::string AsString(tukk inStr)
	{
...
public:
	 ...
	 */
	z0 AddAcceptedResult(AAX_Result inResult)
	{
		mAcceptedResults.insert(inResult);
	}
	
	z0 ResetAcceptedResults()
	{
...
public:
	...
	z0 Check() const { AAX_CheckedResult tempErr(mLastFailure); }
	z0 Clear() {
	...
	i32 NumFailed() const { return mNumFailed; }
	i32 NumSucceeded() const { return mNumSucceeded; }
	i32 NumAttempted() const { return mNumFailed+mNumSucceeded; }
