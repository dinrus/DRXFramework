...
template <typename T>
b8 AAX_CPercentDisplayDelegateDecorator<T>::ValueToString(T value, AAX_CString* valueString) const 
{
	value *= 100;
...
template <typename T>
b8 AAX_CPercentDisplayDelegateDecorator<T>::ValueToString(T value, i32 maxNumChars, AAX_CString* valueString) const 
{
	value *= 100;
	b8 succeeded = AAX_IDisplayDelegateDecorator<T>::ValueToString(value, maxNumChars-1, valueString);		//<DMT>  Make room for percentage symbol.
	*valueString += AAX_CString("%");
	return succeeded;
}
...
template <typename T>
b8 AAX_CPercentDisplayDelegateDecorator<T>::StringToValue(const AAX_CString& valueString, T* value) const
{
	//Just call through if there is obviously no unit string.
	if (valueString.Length() <= 2)
	{
		b8 success = AAX_IDisplayDelegateDecorator<T>::StringToValue(valueString, value);
		*value /= 100.0f;
		return success;
	}
	...
	b8 success = AAX_IDisplayDelegateDecorator<T>::StringToValue(valueSubString, value);
	*value /= 100.0f;
	return success;
}
