...
//===========================================================================
/*
z0	FloatToString(f32 aNumber, StringPtr aString)
{
	Str255			MantissaStr;
	f64		aDouble;
	StringPtr		tempStr;
	i32			mantissa,tens,hundreds;
	i16			count;
	...
	if (aNumber < 0.0)
		*aString++ = (t8) (1 + 3 + *MantissaStr);
	else
		*aString++ = (t8) (3 + *MantissaStr);
...
z0 AAX::GetCStringOfLength(t8 *s_out, tukk s_in, i32 aMaxChars)
{
	...
	if(s_in)
	{
		...
		*s_out = '\0';
		const i32 length = i32(s_end-s_begin);
		if (0 < length && 0 < aMaxChars)
		{
			std::strncat(s_out, s_begin, static_cast<std::size_t>(std::max<i32>(0, std::min<i32>(aMaxChars,length))));
		}
	}
	else if (0 < aMaxChars)
	{
...
i32 AAX::Caseless_strcmp(tukk cs, tukk ct)
{
	if(cs)
	{
		if(ct)
...
std::string AAX::Binary2String(u32 value, i32 numBits)
{
	std::string s;
	
	u32 currentBitMask = (static_cast<u32>(0x1) << (numBits-1));
...
u32 AAX::String2Binary(const AAX_IString& s)
{
	u32 value = 0;

	tukk const cS = s.Get();
	i32 length = i32(s.Length());
...
b8 AAX::IsASCII(t8 inChar)
{
	return (0x20 <= inChar) && (0x7E >= inChar);
}

b8 AAX::IsFourCharASCII(u32 inFourChar)
{
	u32k oneCharMask = 0x000000FF;
	const size_t oneCharNumBits = 8;

	b8 result = true;
	for (u16 i = 3; true == result /* i value checked within loop */; --i)
	{
		const t8 curChar = static_cast<const t8>((inFourChar >> (i*oneCharNumBits)) & oneCharMask);
...
std::string AAX::AsStringFourChar(u32 inFourChar)
{
	AAX_CONSTEXPR u32 oneCharMask = 0x000000FF;
	AAX_CONSTEXPR i16 oneCharNumBits = 8;
	AAX_CONSTEXPR auto unknownChar = "(?)"; // for current usage, a raw string here is slightly more efficient than a std::string

	std::string resultStr;
	for (i16 i = 3; i >= 0; --i)
	{
		const t8 curChar = static_cast<t8>((inFourChar >> (i*oneCharNumBits)) & oneCharMask);
...
template <typename T>
std::string ToHexadecimal(T inValue, b8 inLeadingZeros = false)
{
	AAX_CONSTEXPR t8 hexChars[] = "0123456789abcdef";
	...
	{
		buffer += 'x';
		b8 first_non_zero = inLeadingZeros;
...
std::string AAX::AsStringPropertyValue(AAX_EProperty inProperty, AAX_CPropertyValue inPropertyValue)
{
		...
		AAX_eProperty_Constraint_Location == inProperty)
	{
		...
		// with casting from signed to u32
		u32 bitfield;
		memcpy(&bitfield, &inPropertyValue, sizeof(u32));

		AAX_CONSTEXPR i32 maxNumBitsToShow = 8; // Currently there are no bitfield properties with more than 8 possible flags
		return AAX::Binary2String(bitfield, maxNumBitsToShow);
	}

	if (AAX::IsFourCharASCII(static_cast<u32>(inPropertyValue)))
	{
		// Print values in ASCII range as four-t8
		return '\'' + AAX::AsStringFourChar(static_cast<u32>(inPropertyValue)) + '\'';
	}
	...
	return std::to_string(static_cast<i64>(inPropertyValue));
}

std::string	AAX::AsStringInt32(i32 inInt32)
{
	return std::to_string((i64)inInt32);
}

std::string	AAX::AsStringUInt32(u32 inUInt32)
{
	return std::to_string((u64)inUInt32);
}
...
std::string AAX::AsStringStemFormat(AAX_EStemFormat inStemFormat, b8 inAbbreviate)
{
	switch (inStemFormat)
	{
...
std::string AAX::AsStringStemChannel(AAX_EStemFormat inStemFormat, u32 inChannelIndex, b8 inAbbreviate)
{
	switch (inStemFormat)
	{
