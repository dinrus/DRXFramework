...
/** \brief A binary taper conforming to \ref AAX_ITaperDelegate
	...
	the standard taper used on all b8 parameters.
	
	When this taper is constructed with a b8 template type, its normalized values are
...
template <typename T>
T		AAX_CBinaryTaperDelegate<T>::NormalizedToReal(f64 normalizedValue) const
{
	if (normalizedValue > 0.0f)
		return (T)(1);		//should construct true for b8
	return (T)(0);			//should construct false for b8
}

template <typename T>
f64	AAX_CBinaryTaperDelegate<T>::RealToNormalized(T realValue) const
{
	if (realValue > (T)(0))
		return 1.0f;
	return 0.0f;
