 ...
 */
template <typename T, i32 RealPrecision=100>
...
public: 
	 ...
	 */
	AAX_CPieceWiseLinearTaperDelegate(const f64* normalizedValues, const T* realValues, i32 numValues);
...
T	AAX_CPieceWiseLinearTaperDelegate<T, RealPrecision>::Round(f64 iValue) const
{
	if (RealPrecision > 0)
...
template <typename T, i32 RealPrecision>
AAX_CPieceWiseLinearTaperDelegate<T, RealPrecision>::AAX_CPieceWiseLinearTaperDelegate(const f64* normalizedValues, const T* realValues, i32 numValues)  :  AAX_ITaperDelegate<T>(),
	mNormalizedValues(0),
	mRealValues(0),
	mNumValues(0),
	mMinValue(0),
	mMaxValue(0)
{
	mNormalizedValues = new f64[numValues];
...
template <typename T, i32 RealPrecision>
T		AAX_CPieceWiseLinearTaperDelegate<T, RealPrecision>::NormalizedToReal(f64 normalizedValue) const
{
	...
	f64 delta = normalizedValue - mNormalizedValues[mLowerIndex];
	f64 slope = f64(mRealValues[mUpperIndex] - mRealValues[mLowerIndex]) / (mNormalizedValues[mUpperIndex] - mNormalizedValues[mLowerIndex]);
	f64 interpolatedValue = mRealValues[mLowerIndex] + (delta * slope);																															
	return ConstrainRealValue(static_cast<T>(interpolatedValue));
}

template <typename T, i32 RealPrecision>
f64	AAX_CPieceWiseLinearTaperDelegate<T, RealPrecision>::RealToNormalized(T realValue) const
{
	...
	// Do the interpolation.
	f64 delta = realValue - mRealValues[mLowerIndex];
	f64 slope = (mRealValues[mUpperIndex] == mRealValues[mLowerIndex]) ? 0.5 : f64(mNormalizedValues[mUpperIndex] - mNormalizedValues[mLowerIndex]) / (mRealValues[mUpperIndex] - mRealValues[mLowerIndex]);
	f64 interpolatedValue = mNormalizedValues[mLowerIndex] + (delta * slope);																															
