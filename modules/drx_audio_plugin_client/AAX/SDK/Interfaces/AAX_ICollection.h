...
public: // AAX_IACFCollection
	...
	*/
	virtual	AAX_Result				AddEffect ( const t8 * inEffectID, AAX_IEffectDescriptor* inEffectDescriptor ) = 0;
	...
	*/
	virtual	AAX_Result				SetManufacturerName( tukk inPackageName ) = 0;
	...
	*/
	virtual	AAX_Result				AddPackageName( const t8 *inPackageName ) = 0;
	...
	*/
	virtual	AAX_Result				SetPackageVersion( u32 inVersion ) = 0;
	...
	*/
	virtual AAX_Result GetHostVersion(u32* outVersion) const = 0;
...
public: // AAX_ICollection
	...
	/** Get a pointer to an \ref IACFDefinition, if supported by the host
	 ...
	 \ref IACFDefinition::DefineAttribute() не поддерживается on this object
	 */
