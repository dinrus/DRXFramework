...
namespace AAX
{
	...
	/// Возвращает true, если \c inPacket is a Note On message
	inline b8 IsNoteOn(const AAX_CMidiPacket* inPacket)
	{
		if (!inPacket) { return false; }
		u8k sn = (inPacket->mData[0] & 0xF0); // status nibble
		u8k data2 = inPacket->mData[2];
		return ((eStatusNibble_NoteOn == sn) &&
				(0x00 != data2));
	}
	
	/// Возвращает true, если \c inPacket is a Note Off message, or a Note On message with velocity zero
	inline b8 IsNoteOff(const AAX_CMidiPacket* inPacket)
	{
		if (!inPacket) { return false; }
		u8k sn = (inPacket->mData[0] & 0xF0); // status nibble
		u8k data2 = inPacket->mData[2];
	...
	/// Возвращает true, если \c inPacket is an All Sound Off or All Notes Off message
	inline b8 IsAllNotesOff(const AAX_CMidiPacket* inPacket)
	{
		if (!inPacket) { return false; }
		u8k sn = (inPacket->mData[0] & 0xF0); // status nibble
		u8k data1 = inPacket->mData[1];
		u8k data2 = inPacket->mData[2];
	...
	/// Возвращает true, если \c inPacket is a special Pro Tools accented click message
	inline b8 IsAccentedClick(const AAX_CMidiPacket* inPacket)
	{
		return ((inPacket) &&
	...
	/// Возвращает true, если \c inPacket is a special Pro Tools unaccented click message
	inline b8 IsUnaccentedClick(const AAX_CMidiPacket* inPacket)
	{
		return ((inPacket) &&
				(eStatusNibble_NoteOn == (inPacket->mData[0] & 0xF0)) &&
				(0x00 == (inPacket->mData[0] & 0x0F)) &&
	...
	/// Возвращает true, если \c inPacket is a special Pro Tools click message
	inline b8 IsClick(const AAX_CMidiPacket* inPacket)
	{
