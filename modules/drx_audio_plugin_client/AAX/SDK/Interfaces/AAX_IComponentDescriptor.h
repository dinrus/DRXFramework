...
public:
     ...
     *
     *  - Data type: f32**
     *  - Data kind: An array of f32 arrays, one for each input channel
	 *
	...
	*  Defines an audio out port for host-provided information in the algorithm's
     ...
     *
     *  - Data type: f32**
     *  - Data kind: An array of f32 arrays, one for each output channel
	*
	*  \param[in] inFieldIndex
	*		Unique identifier for the field, generated using \ref AAX_FIELD_INDEX
	*/	
	...
	*/	
	 ...
	 */	
	virtual AAX_Result					AddDataInPort ( AAX_CFieldIndex inFieldIndex, u32 inPacketSize, AAX_EDataInPortType inPortType = AAX_eDataInPortType_Buffered ) = 0;
	 ...
	 *		
	 */
	virtual AAX_Result					AddAuxOutputStem ( AAX_CFieldIndex inFieldIndex, i32 inStemFormat, const t8 inNameUTF8[]) = 0;
	/*!
	 ...
	 */
	virtual AAX_Result					AddPrivateData ( AAX_CFieldIndex inFieldIndex, i32 inDataSize, /* AAX_EPrivateDataOptions */ u32 inOptions = AAX_ePrivateDataOptions_DefaultOptions ) = 0;
    ...
    */
    virtual AAX_Result					AddTemporaryData( AAX_CFieldIndex inFieldIndex, u32 inDataElementSize) = 0;
	...
	*/
	 ...
	 */
	virtual AAX_Result					AddMeters ( AAX_CFieldIndex inFieldIndex, const AAX_CTypeID* inMeterIDs, u32k inMeterCount ) = 0;
	...
	*/
	virtual AAX_Result					AddMIDINode ( AAX_CFieldIndex inFieldIndex, AAX_EMIDINodeType inNodeType, const t8 inNodeName[], u32 channelMask ) = 0;
	...
	*/	
	virtual AAX_Result					AddReservedField ( AAX_CFieldIndex inFieldIndex, u32 inFieldType ) = 0;
	...
	*/
	 ...
	 *	- <tt>const t8 inDLLFileNameUTF8[]</tt>: \ref AAX_eProperty_TIDLLFileName (required)
	 *	- <tt>const t8 iProcessProcSymbol[]</tt>: \ref AAX_eProperty_TIProcessProc (required)
	 *	- <tt>const t8 iInstanceInitProcSymbol[]</tt>: \ref AAX_eProperty_TIInstanceInitProc (optional)
	 *	- <tt>const t8 iBackgroundProcSymbol[]</tt>: \ref AAX_eProperty_TIBackgroundProc (optional)
	 *
	...
	AAX_Result							AddProcessProc_Native (
		z0 (AAX_CALLBACK *inProcessProc) ( aContextType * const inInstancesBegin [], ukk  inInstancesEnd),
		AAX_IPropertyMap * inProperties = NULL,
		i32 (AAX_CALLBACK *inInstanceInitProc) ( const aContextType * inInstanceContextPtr, AAX_EComponentInstanceInitAction inAction ) = NULL, 
		i32 (AAX_CALLBACK *inBackgroundProc) ( z0 ) = NULL );
};
...
AAX_IComponentDescriptor::AddProcessProc_Native ( 
	z0 (AAX_CALLBACK *inProcessProc) ( aContextType * const	inInstancesBegin [], ukk  inInstancesEnd),
	AAX_IPropertyMap * inProperties,
	i32 (AAX_CALLBACK *inInstanceInitProc) ( const aContextType * inInstanceContextPtr, AAX_EComponentInstanceInitAction inAction ),
	i32 (AAX_CALLBACK *inBackgroundProc) ( z0 ) )
{
