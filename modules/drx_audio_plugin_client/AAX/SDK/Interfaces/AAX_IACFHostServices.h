...
public:
	/** \deprecated Legacy version of \ref AAX_IACFHostServices_V3::HandleAssertFailure() implemented by older hosts
	 ...
	 builds.
	 */
	virtual AAX_Result Assert ( const t8 * iFile, i32 iLine, const t8 * iNote ) = 0;
	
	virtual AAX_Result Trace ( i32 iPriority, const t8 * iMessage ) = 0;	///< \copydoc AAX_IHostServices::Trace()
};
...
{
public:
	virtual AAX_Result StackTrace ( i32 iTracePriority, i32 iStackTracePriority, const t8 * iMessage ) = 0;	///< \copydoc AAX_IHostServices::StackTrace()
};
...
{
public:
	virtual AAX_Result HandleAssertFailure ( const t8 * iFile, i32 iLine, const t8 * iNote, /* AAX_EAssertFlags */ i32 iFlags ) const = 0;	///< \copydoc AAX_IHostServices::HandleAssertFailure()
};
