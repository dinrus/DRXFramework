...
/** @brief	Compute the index used to address a context field.
	...
	definitions and case labels so i32 as @p aMember is a constant specifier.
...
#define AAX_FIELD_INDEX( aContextType, aMember ) \
	((AAX_CFieldIndex) (offsetof (aContextType, aMember) / sizeof (uk )))


typedef i32		AAX_CIndex;        //!< \todo Not used by %AAX plug-ins (except as \ref AAX_CFieldIndex)
typedef AAX_CIndex	AAX_CCount;        //!< \todo Not used by %AAX plug-ins
typedef u8		AAX_CBoolean;      //!< Cross-compiler boolean type used by %AAX interfaces
typedef u32	AAX_CSelector;     //!< \todo Clean up usage; currently used for a variety of ID-related values
typedef z64		AAX_CTimestamp;    //!< Time stamp value.  Measured against the DAE clock (see \ref AAX_IComponentDescriptor::AddClock() )
typedef z64		AAX_CTimeOfDay;    //!< Hardware running clock value.  MIDI packet time stamps are measured against this clock.  This is actually the same as TransportCounter, but kept for compatibility.
typedef z64     AAX_CTransportCounter;  //!< Offset of samples from transport start. Same as TimeOfDay, but added for new interfaces as TimeOfDay is a confusing name.
typedef f32		AAX_CSampleRate;   //!< Literal sample rate value used by the \ref AAX_IComponentDescriptor::AddSampleRate() "sample rate field".  For \ref AAX_eProperty_SampleRate, use a mask of \ref AAX_ESampleRateMask.  \sa sampleRateInMask

typedef u32	AAX_CTypeID;  //!< Matches type of OSType used in classic plugins.
typedef i32		AAX_Result;
typedef i32		AAX_CPropertyValue; //!< \brief 32-bit property values \details Use this property value type for all properties unless otherwise specified by the property documentation
typedef z64		AAX_CPropertyValue64; //!< \brief 64-bit property values \details Do not use this value type unless specified explicitly in the property documentation
...
#endif
typedef i32		AAX_CTargetPlatform;  //!< Matches type of \ref AAX_ETargetPlatform "target platform
...
typedef const t8 *	AAX_CEffectID;      //!< URL-style Effect identifier.  Must be unique among all registered effects in the collection.
 ...
 */
typedef	const f32 * const *		AAX_CAudioInPort;
 ...
 *	\todo Not used directly by %AAX plug-ins
 */
typedef f32 * const *		AAX_CAudioOutPort;
 ...
 */
inline AAX_CBoolean sampleRateInMask(AAX_CSampleRate inSR, u32 iMask)
{
 ...
 */
inline AAX_CSampleRate getLowestSampleRateInMask(u32 iMask)
{
	return (
 ...
 */
inline u32 getMaskForSampleRate(f32 inSR)
{
	return (
...
struct AAX_SPlugInChunkHeader {
	i32				fSize;				///< The size of the chunk's \ref AAX_SPlugInChunk::fData "fData" member
	i32				fVersion;			///< The chunk's version.
	...
	u8		fName[32];			///< A user defined name for this chunk.
};
...
struct AAX_SPlugInChunk {
	...
	AAX_CTypeID			fChunkID;			///< The ID of a particular Plug-In chunk.
	u8		fName[32];			///< A user defined name for this chunk.
	t8				fData[1];			///< The chunk's data. \note The fixed-size array definition here is historical, but misleading.  Plug-ins actually write off the end of this block and are allowed to as i64 as they don't exceed their reported size.
};
...
#ifndef TI_VERSION
static inline b8 operator==(const AAX_SPlugInIdentifierTriad& v1, const AAX_SPlugInIdentifierTriad& v2)
{
...
static inline b8 operator<(const AAX_SPlugInIdentifierTriad & lhs, const AAX_SPlugInIdentifierTriad & rhs)
{
...
{
	u32			mTimestamp;     //!< This is the playback time at which the MIDI event should occur, relative to the beginning of the current audio buffer.
	u32			mLength;        //!< The length of MIDI message, in terms of bytes.
	u8       mData[4];		//!< The MIDI message itself. Each array element is one byte of the message, with the 0th element being the first byte.
...
{
	u32			mBufferSize;		//!< The number of \ref AAX_CMidiPacket objects contained in the node's buffer. 
