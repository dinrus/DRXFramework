...
public:
	...
	DataType* GetPtr()
	{
		...
		uk  data = mPacketData.Get(mDataSize);
		return reinterpret_cast<DataType*> (data);
	}
	
	z0			SetDirty(b8 iDirty)	{ mDirty = iDirty; };
	b8			IsDirty() const			{ return mDirty; };
...
private:
	...
	public:
		...
		~SPacketData();
		ukk Get() const;
		uk Get(size_t newSize) const;
	private:	
		mutable uk	mData;
	} mPacketData;
};

// GetPtr() specialization for uk
template <>
inline ukk
AAX_CPacket::GetPtr<const z0>()
{
...
public:
	...
	z0 Initialize( AAX_IController* iPlugIn, AAX_IEffectParameters* iEffectParameters);
	...
	AAX_Result SetDirty(AAX_CParamID paramID, b8 iDirty = true);
