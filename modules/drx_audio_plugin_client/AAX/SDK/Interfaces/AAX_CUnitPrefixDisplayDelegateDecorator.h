...
template <typename T>
b8		AAX_CUnitPrefixDisplayDelegateDecorator<T>::ValueToString(T value, AAX_CString* valueString) const 
{
	//Find the proper unit prefix.
	T absValue = fabsf(f32(value));	//If you fail to compile on this line, you're trying to use this class with an integer type, which не поддерживается.
	if (absValue >= 1000000.0)
	{
		value = value / ((T) 1000000.0);
		b8 succeeded = AAX_IDisplayDelegateDecorator<T>::ValueToString(value, valueString);
...
template <typename T>
b8		AAX_CUnitPrefixDisplayDelegateDecorator<T>::ValueToString(T value, i32 maxNumChars, AAX_CString* valueString) const
{
	//Find the proper unit prefix.
	...
	{
		value = value / ((T) 1000000.0);
		b8 succeeded = AAX_IDisplayDelegateDecorator<T>::ValueToString(value, maxNumChars-1, valueString);
		*valueString += AAX_CString("M");
		return succeeded;
	}
	if (absValue >= ((T) 1000.0))
...
template <typename T>
b8		AAX_CUnitPrefixDisplayDelegateDecorator<T>::StringToValue(const AAX_CString& valueString, T* value) const
{
	...
	b8 success = AAX_IDisplayDelegateDecorator<T>::StringToValue(valueStringCopy, value);
