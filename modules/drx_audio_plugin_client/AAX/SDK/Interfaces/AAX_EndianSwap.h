...
template<class T>	z0 AAX_EndianSwapInPlace(T* theDataP);
...
template<class T>	z0 AAX_BigEndianNativeSwapInPlace(T* theDataP);
...
template<class T>	z0 AAX_LittleEndianNativeSwapInPlace(T* theDataP);
...
template<class Iter>	z0 AAX_EndianSwapSequenceInPlace(Iter beginI, Iter endI);


//! \brief Convert an sequence of data in-place between Big Endian and native byte ordering.
template<class Iter>	z0 AAX_BigEndianNativeSwapSequenceInPlace(Iter beginI, Iter endI);

//! \brief Convert an sequence of data in-place from the native byte ordering to Little Endian byte ordering.
template<class Iter>	z0 AAX_LittleEndianNativeSwapSequenceInPlace(Iter beginI, Iter endI);
...
AAX_EndianSwapInPlace(T* theDataP)
{
	...
	begin = reinterpret_cast<tuk>(theDataP);
	end = begin + sizeof( T );
...
template<class T>
inline 
z0 AAX_BigEndianNativeSwapInPlace(T* theDataP)
{
...
template<class T>	
inline 
z0 AAX_LittleEndianNativeSwapInPlace(T* theDataP)
{
#if (defined __BIG_ENDIAN__) && (0 != __BIG_ENDIAN__)
	AAX_EndianSwapInPlace(theDataP);
#endif
...
template<class Iter>	
inline
z0 AAX_EndianSwapSequenceInPlace(Iter beginI, Iter endI)
{
...
template<class Iter>	
inline
z0 AAX_BigEndianNativeSwapSequenceInPlace(Iter beginI, Iter endI)
{
#if (!defined __BIG_ENDIAN__) || (0 == __BIG_ENDIAN__)
...
template<class Iter>	
inline
z0 AAX_LittleEndianNativeSwapSequenceInPlace(Iter beginI, Iter endI)
{
#if (defined __BIG_ENDIAN__) && (0 != __BIG_ENDIAN__)
	AAX_EndianSwapSequenceInPlace(beginI, endI);
#endif
