 ...
 * 
 * The data payload is a \c tuk C string
 */
...
public:
	...
	explicit AAX_CStringDataBufferOfType (const t8 * inData) : mData{inData ? std::string{inData} : std::string{}} {}
	...
	~AAX_CStringDataBufferOfType (z0) AAX_OVERRIDE = default;
	...
	AAX_Result Size(i32 * oSize) const AAX_OVERRIDE {
	...
	AAX_Result Data(z0 const ** oBuffer) const AAX_OVERRIDE {
...
public:
	...
	AAX_CStringDataBuffer (AAX_CTypeID inType, const t8 * inData) : mType{inType}, mData{inData ? std::string{inData} : std::string{}} {}

	AAX_CStringDataBuffer(AAX_CStringDataBuffer const &) = delete;
	AAX_CStringDataBuffer(AAX_CStringDataBuffer &&) = delete;

	~AAX_CStringDataBuffer (z0) AAX_OVERRIDE = default;
