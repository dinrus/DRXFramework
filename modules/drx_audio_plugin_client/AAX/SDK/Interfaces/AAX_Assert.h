...
/** \def AAX_TRACE
	...
	example:
		...
		\code
		AAX_TRACE(kAAX_Trace_Priority_Normal, "My f32: %f, My C-string: %s", myFloat, myCString);
		\endcode
...
/** \def AAX_TRACE_RELEASE
	...
	For example:
		 ...
		 \code
		 AAX_TRACE_RELEASE(kAAX_Trace_Priority_Normal, "%s %s %s;\tMy f32: %f, My C-string: %s",
...
/** \def AAX_ASSERT
	...
	<B>Usage </B>
	Each invocation of this macro takes a single argument, which is interpreted as a <TT>b8</TT>.
 ...
 */
	...
	#define AAX_TRACEORSTACKTRACE_RELEASE( iTracePriority, iStackTracePriority, ... ) \
		...
		#define AAX_ASSERT( condition ) \
			{ \
				if( ! ( condition ) ) { \
					AAX_CHostServices::HandleAssertFailure( __FILE__, __LINE__, #condition, (i32)AAX_eAssertFlags_Log | (i32)AAX_eAssertFlags_Dialog ); \
				} \
			};
		...
		#define AAX_ASSERT( condition ) \
			{ \
				if( ! ( condition ) ) { \
					AAX_CHostServices::HandleAssertFailure( __FILE__, __LINE__, #condition, (i32)AAX_eAssertFlags_Log ); \
				} \
			};
