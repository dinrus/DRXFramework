									   ...
									   i32 inProcIDsSize)
	{
		...
		if ((AAX_SUCCESS == result) && (hasNative || hasAS))
		{
			...
			result = inComponentDescriptor.AddProcessProc_Native(
				reinterpret_cast<AAX_CProcessProc>(const_cast<uk>(processProc)),
				nativeProps,
				reinterpret_cast<AAX_CInstanceInitProc>(const_cast<uk>(initProc)),
				reinterpret_cast<AAX_CBackgroundProc>(const_cast<uk>(backgroundProc)),
				&nativeProcID);
		...
		{
			tukk fileName = NULL;
			if (false == inProperties->GetPointerProperty(AAX_eProperty_TIDLLFileName, reinterpret_cast<ukk*>(&fileName)))
			{
			...
			tukk processProcSymbol = NULL;
			if (false == inProperties->GetPointerProperty(AAX_eProperty_TIProcessProc, reinterpret_cast<ukk*>(&processProcSymbol)))
			{
			...
			tukk initProcSymbol = NULL;
			inProperties->GetPointerProperty(AAX_eProperty_TIInstanceInitProc, reinterpret_cast<ukk*>(&initProcSymbol));
			
			tukk backgroundProcSymbol = NULL;
			inProperties->GetPointerProperty(AAX_eProperty_TIBackgroundProc, reinterpret_cast<ukk*>(&backgroundProcSymbol));
			
			AAX_CSelector tiProcID = 0;
			result = inComponentDescriptor.AddProcessProc_TI(
				reinterpret_cast<tukk>(fileName),
				reinterpret_cast<tukk>(processProcSymbol),
				inProperties,
				reinterpret_cast<tukk>(initProcSymbol),
				reinterpret_cast<tukk>(backgroundProcSymbol),
				&tiProcID);
		...
		if (AAX_SUCCESS == result && NULL != outProcIDs)
		{
			...
			if (numProcIDs < INT32_MAX && inProcIDsSize > (i32)numProcIDs)
			{
	...
	mIACFComponentDescriptorV3( NULL )
{
	if ( mUnkHost )
	{
		...
		if ( pUnkHost->QueryInterface(IID_IACFComponentFactory, (uk *)&pFactory) == ACF_OK )
		{
			...
			pFactory->CreateComponent(AAXCompID_AAXComponentDescriptor, 0, IID_IAAXComponentDescriptorV1, (uk *)&mIACFComponentDescriptor);
            
			// Get the V2 interface
			if (mIACFComponentDescriptor)
                mIACFComponentDescriptor->QueryInterface(IID_IAAXComponentDescriptorV2, (uk*)&mIACFComponentDescriptorV2);
            
			// Get the V3 interface
			if (mIACFComponentDescriptor)
                mIACFComponentDescriptor->QueryInterface(IID_IAAXComponentDescriptorV3, (uk*)&mIACFComponentDescriptorV3);
		}
	}
}
...
// ******************************************************************************************
IACFUnknown*				
AAX_VComponentDescriptor::GetIUnknown(z0) const
{
...
// ******************************************************************************************
AAX_Result AAX_VComponentDescriptor::AddReservedField ( AAX_CFieldIndex inPortID, u32 inFieldType )
{
	if ( mIACFComponentDescriptor ) 
...
// ******************************************************************************************
AAX_Result AAX_VComponentDescriptor::AddDataInPort ( AAX_CFieldIndex inPortID, u32 inPacketSize, AAX_EDataInPortType inPortType )
{
	if ( mIACFComponentDescriptor ) 
...
// ******************************************************************************************
AAX_Result AAX_VComponentDescriptor::AddAuxOutputStem ( AAX_CFieldIndex inPortID, i32 inStemFormat, const t8 inNameUTF8[] )
{
	if ( mIACFComponentDescriptor ) 
...
// ******************************************************************************************
AAX_Result AAX_VComponentDescriptor::AddPrivateData ( AAX_CFieldIndex inPortID, i32 inDataSize, u32 /* AAX_EPrivateDataOptions */ inOptions /* = AAX_ePrivateDataOptions_DefaultOptions */ )
{
	if ( mIACFComponentDescriptor ) 
...
AAX_IPropertyMap * AAX_VComponentDescriptor::DuplicatePropertyMap (AAX_IPropertyMap* inPropertyMap) const
{
	...
	for (AAX_EProperty curPropertyID = AAX_eProperty_MinProp;
		 ...
		 curPropertyID = (AAX_EProperty((i32)curPropertyID + 1)))
	{
		AAX_CPropertyValue curPropertyValue = 0;
		ukk curPointerPropertyValue = 0;
...
AAX_Result AAX_VComponentDescriptor::AddProcessProc_TI( 
	...
	AAX_IPropertyMap * inProperties /*= NULL*/, 
	const t8 inInstanceInitProcSymbol [] /*= NULL*/, 
	const t8 inBackgroundProcSymbol [] /*= NULL*/, 
	AAX_CSelector * outProcID /*= NULL */ )
{
	if ( mIACFComponentDescriptor ) 
	...
	i32 inProcIDsSize /*= 0*/)
{
	...
	{
		// If the full AddProcessProc routine не поддерживается by the host then
...
// METHOD:	AddMIDINode
// ******************************************************************************************
AAX_Result AAX_VComponentDescriptor::AddMIDINode ( AAX_CFieldIndex inPortID, AAX_EMIDINodeType inNodeType, const t8 inNodeName[], u32 channelMask )
{
	if ( mIACFComponentDescriptor ) 
...
// ******************************************************************************************
AAX_Result AAX_VComponentDescriptor::AddTemporaryData( AAX_CFieldIndex inFieldIndex, u32 inDataElementSize)
{
