...
AAX_VAutomationDelegate::AAX_VAutomationDelegate( IACFUnknown * pUnknown )
{
	if ( pUnknown )
	{
		pUnknown->QueryInterface(IID_IAAXAutomationDelegateV1, (uk *)&mIAutomationDelegate);
		pUnknown->QueryInterface(IID_IAAXControllerV2, (uk*)&mIController);
	}
}
...
// ******************************************************************************************
AAX_Result AAX_VAutomationDelegate::PostSetValueRequest ( AAX_CParamID iParameterID, f64 iNormalizedValue ) const
{
	if ( mIAutomationDelegate )
...
// ******************************************************************************************
AAX_Result AAX_VAutomationDelegate::PostCurrentValue ( AAX_CParamID iParameterID, f64 iNormalizedValue ) const
{
	if ( mIAutomationDelegate )
...
AAX_Result AAX_VAutomationDelegate::ParameterNameChanged ( AAX_CParamID iParameterID )
{
	if ( mIController ) {
		...
		if (end != found) {
			...
			if (len > 0) {
				return mIController->SendNotification (AAX_eNotificationEvent_ParameterNameChanged, iParameterID, sizeof(t8) * (static_cast<u32>(len)+1));
			}
		}
