	...
	mDescriptionHost(pUnkHost)
{
	if ( mUnkHost )
	{
		...
		if ( pUnkHost->QueryInterface(IID_IACFComponentFactory, (uk *)&pFactory) == ACF_OK )
		{
			...
			pFactory->CreateComponent(AAXCompID_AAXCollection, 0, IID_IAAXCollectionV1, (uk *)&mIACFCollection);
		}
	}
}
...
AAX_VCollection::GetIUnknown(z0) const
{
...
// METHOD:	AddEffect
// ******************************************************************************************
AAX_Result AAX_VCollection::AddEffect ( const t8 * inEffectID, AAX_IEffectDescriptor * inEffectDescriptor )
{
...
// ******************************************************************************************
AAX_Result AAX_VCollection::SetManufacturerName( const t8 * iManufacturerName )
{
	if ( mIACFCollection )
...
// ******************************************************************************************
AAX_Result AAX_VCollection::AddPackageName( const t8 * iPackageName )
{
	if ( mIACFCollection )
...
// ******************************************************************************************
AAX_Result AAX_VCollection::SetPackageVersion( u32 iPackageVersion )
{
	if ( mIACFCollection )
...
AAX_Result AAX_VCollection::GetHostVersion(u32* outVersion) const
{
	if (!outVersion)
