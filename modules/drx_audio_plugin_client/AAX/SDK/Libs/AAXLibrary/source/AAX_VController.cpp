...
AAX_VController::AAX_VController( IACFUnknown* pUnknown )
{
	if ( pUnknown )
	{
		pUnknown->QueryInterface(IID_IAAXControllerV1, (uk *)&mIController);
		pUnknown->QueryInterface(IID_IAAXControllerV2, (uk *)&mIControllerV2);
		pUnknown->QueryInterface(IID_IAAXControllerV3, (uk *)&mIControllerV3);
		pUnknown->QueryInterface(IID_IAAXPageTableController, (uk *)&mIPageTableController);
		pUnknown->QueryInterface(IID_IAAXPageTableControllerV2, (uk *)&mIPageTableControllerV2);
...
// METHOD:	PostPacket
// ******************************************************************************************
AAX_Result AAX_VController::PostPacket ( AAX_CFieldIndex inFieldIndex, ukk  inPayloadP, u32 inPayloadSize )
{
	if ( mIController )
...
// ******************************************************************************************
AAX_Result AAX_VController::SendNotification (AAX_CTypeID inNotificationType, ukk inNotificationData, u32 inNotificationDataSize)
{
	if (mIControllerV2)
...
// ******************************************************************************************
AAX_Result	AAX_VController::GetSignalLatency( i32* outSamples) const
{
	if (mIController )
...
// ******************************************************************************************
AAX_Result AAX_VController::GetHybridSignalLatency(i32* outSamples) const
{
	if (mIControllerV2 )
...
// ******************************************************************************************
AAX_Result	AAX_VController::SetSignalLatency(i32 numSamples)
{
	if (mIController )
...
// ******************************************************************************************
AAX_Result	AAX_VController::SetCycleCount( AAX_EProperty* inWhichCycleCounts, AAX_CPropertyValue* inValues, i32 inNumValues)
{
	if (mIController )
...
// ******************************************************************************************
AAX_Result	AAX_VController::GetCurrentMeterValue( AAX_CTypeID inMeterID, f32 * outMeterValue ) const
{
	if ( mIController )
...
// ******************************************************************************************
AAX_Result	AAX_VController::GetMeterPeakValue( AAX_CTypeID inMeterID, f32 * outMeterPeakValue ) const
{
	if ( mIController )
...
// ******************************************************************************************
AAX_Result	AAX_VController::GetMeterCount( u32 * outMeterCount ) const
{
	if ( mIController )
...
// ******************************************************************************************
AAX_IPageTable* AAX_VController::CreateTableCopyForLayout(const t8 * inEffectID,
														  const t8 * inLayoutName,
														  u32 inTableType,
														  i32 inTablePageSize) const
{
	if (!mIPageTableController) { return NULL; }
...
// ******************************************************************************************
AAX_IPageTable* AAX_VController::CreateTableCopyForEffectFromFile(tukk inPageTableFilePath,
...
// ******************************************************************************************
AAX_IPageTable* AAX_VController::CreateTableCopyForLayoutFromFile(tukk inPageTableFilePath,
																AAX_ETextEncoding inFilePathEncoding,
																tukk inLayoutName,
																u32 inTableType,
																i32 inTablePageSize) const
...
ACFPtr<AAX_IACFPageTable_V2> AAX_VController::CreatePageTable() const
{
	...
	if ( mComponentFactory )
	{
		...
		const AAX_Result result = static_cast<AAX_Result>(const_cast<IACFComponentFactory&>(*mComponentFactory).CreateComponent(AAXCompID_PageTable, 0, IID_IAAXPageTableV2, (uk *)&mIPageTable));
		if (ACF_OK != result)
		{
			mIPageTable = NULL;
		}
