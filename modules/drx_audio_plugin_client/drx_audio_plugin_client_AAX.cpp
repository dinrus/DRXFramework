...
DRX_BEGIN_IGNORE_WARNINGS_MSVC (4127 4512 4996 5272)
...
static_assert (AAX_SDK_CURRENT_REVISION >= AAX_SDK_2p6p1_REVISION, "DRX requires AAX SDK version 2.6.1 or higher");
...
#ifndef DrxPlugin_AAX_Chunk_Identifier
 #define DrxPlugin_AAX_Chunk_Identifier     'drx'
#endif

constexpr i32 juceChunkType = DrxPlugin_AAX_Chunk_Identifier;
...
{
    static i32 getAAXParamHash (AAX_CParamID paramID) noexcept
    {
        i32 result = 0;
    ...
    static z0 check ([[maybe_unused]] AAX_Result result)
    {
        jassert (result == AAX_SUCCESS);
    }

    // maps a channel index of an AAX format to an index of a drx format
    ...
    static AAX_EStemFormat stemFormatForAmbisonicOrder (i32 order)
    {
        switch (order)
        {
    ...
    static AAX_EStemFormat getFormatForAudioChannelSet (const AudioChannelSet& set, b8 ignoreLayout) noexcept
    {
    ...
    static inline AudioChannelSet channelSetFromStemFormat (AAX_EStemFormat format, b8 ignoreLayout) noexcept
    {
        if (! ignoreLayout)
        {
            switch (format)
        ...
        return AudioChannelSet::discreteChannels (jmax (0, static_cast<i32> (AAX_STEM_FORMAT_CHANNEL_COUNT (format))));
    }
    ...
    static Color getColorFromHighlightEnum (AAX_EHighlightColor colour) noexcept
    {
        switch (colour)
        {
    ...
    static i32 juceChannelIndexToAax (i32 juceIndex, const AudioChannelSet& channelSet)
    {
        ...
        auto channelType = channelSet.getTypeOfChannel (static_cast<i32> (juceIndex));
        ...
        if (it != std::cend (speakerOrder))
            return (i32) std::distance (std::cbegin (speakerOrder), it);

        return juceIndex;
    }

    //==============================================================================
    class DrxAAX_Processor;

    struct PluginInstanceInfo
    {
        PluginInstanceInfo (DrxAAX_Processor& p)  : parameters (p) {}

        DrxAAX_Processor& parameters;

        DRX_DECLARE_NON_COPYABLE (PluginInstanceInfo)
    };

    //==============================================================================
    struct DRXAlgorithmContext
    {
        f32** inputChannels;
        f32** outputChannels;
        i32* bufferSize;
        ...
        i32* isPrepared;
        f32* const* meterTapBuffers;
        i32* sideChainBuffers;
    };

    struct DRXAlgorithmIDs
    {
        enum
        {
            inputChannels     = AAX_FIELD_INDEX (DRXAlgorithmContext, inputChannels),
            outputChannels    = AAX_FIELD_INDEX (DRXAlgorithmContext, outputChannels),
            bufferSize        = AAX_FIELD_INDEX (DRXAlgorithmContext, bufferSize),
            bypass            = AAX_FIELD_INDEX (DRXAlgorithmContext, bypass),
            midiNodeIn        = AAX_FIELD_INDEX (DRXAlgorithmContext, midiNodeIn),
            midiNodeOut       = AAX_FIELD_INDEX (DRXAlgorithmContext, midiNodeOut),
            pluginInstance    = AAX_FIELD_INDEX (DRXAlgorithmContext, pluginInstance),
            preparedFlag      = AAX_FIELD_INDEX (DRXAlgorithmContext, isPrepared),

            meterTapBuffers   = AAX_FIELD_INDEX (DRXAlgorithmContext, meterTapBuffers),

            sideChainBuffers  = AAX_FIELD_INDEX (DRXAlgorithmContext, sideChainBuffers)
        };
    };
    ...
    class DrxAAX_GUI final : public AAX_CEffectGUI,
                              public ModifierKeyProvider
    {
    public:
        DrxAAX_GUI() = default;
        ~DrxAAX_GUI() override { DeleteViewContainer(); }

        static AAX_IEffectGUI* AAX_CALLBACK Create()   { return new DrxAAX_GUI(); }

        z0 CreateViewContents() override;

        z0 CreateViewContainer() override
        {
            CreateViewContents();

            if (uk nativeViewToAttachTo = GetViewContainerPtr())
            {
               #if DRX_MAC
        ...
        z0 DeleteViewContainer() override
        {
        ...
        AAX_Result GetViewSize (AAX_Point* viewSize) const override
        {
            if (component != nullptr)
            {
                *viewSize = convertToHostBounds ({ (f32) component->getHeight(),
                                                   (f32) component->getWidth() });
        ...
        AAX_Result SetControlHighlightInfo (AAX_CParamID paramID, AAX_CBoolean isHighlighted, AAX_EHighlightColor colour) override
        {
            if (component != nullptr && component->pluginEditor != nullptr)
            {
                ...
                if (index >= 0)
                {
                    ...
                    info.suggestedColor = getColorFromHighlightEnum (colour);
        ...
        i32 getWin32Modifiers() const override
        {
            i32 modifierFlags = 0;
    ...
    private:
        //==============================================================================
        i32 getParamIndexFromID (AAX_CParamID paramID) const noexcept;
        AAX_CParamID getAAXParamIDFromDrxIndex (i32 index) const noexcept;
        ...
        {
            ContentWrapperComponent (DrxAAX_GUI& gui, AudioProcessor& plugin)
                : owner (gui)
            {
                setOpaque (true);
            ...
            z0 callMouseMethod (const MouseEvent& e, MethodType method)
            {
                if (auto* vc = owner.GetViewContainer())
                {
                    auto parameterIndex = pluginEditor->getControlParameterIndex (*e.eventComponent);

                    if (auto aaxParamID = owner.getAAXParamIDFromDrxIndex (parameterIndex))
                    {
                        u32 mods = 0;
            ...
            z0 mouseDown (const MouseEvent& e) override  { callMouseMethod (e, &AAX_IViewContainer::HandleParameterMouseDown); }
            z0 mouseUp   (const MouseEvent& e) override  { callMouseMethod (e, &AAX_IViewContainer::HandleParameterMouseUp); }
            z0 mouseDrag (const MouseEvent& e) override  { callMouseMethod (e, &AAX_IViewContainer::HandleParameterMouseDrag); }
            ...
            b8 resizeHostWindow()
            {
                if (pluginEditor != nullptr)
                {
                    auto newSize = convertToHostBounds ({ (f32) pluginEditor->getHeight(),
                                                          (f32) pluginEditor->getWidth() });
           ...
           #endif
            drx::Rectangle<i32> lastValidSize;
        ...
        DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (DrxAAX_GUI)
    };

    // Copied here, because not all versions of the AAX SDK define all of these values
    enum DRX_AAX_EFrameRate : std::underlying_type_t<AAX_EFrameRate>
    {
        DRX_AAX_eFrameRate_Undeclared = 0,
        DRX_AAX_eFrameRate_24Frame = 1,
        DRX_AAX_eFrameRate_25Frame = 2,
        DRX_AAX_eFrameRate_2997NonDrop = 3,
        DRX_AAX_eFrameRate_2997DropFrame = 4,
        DRX_AAX_eFrameRate_30NonDrop = 5,
        DRX_AAX_eFrameRate_30DropFrame = 6,
        DRX_AAX_eFrameRate_23976 = 7,
        DRX_AAX_eFrameRate_47952 = 8,
        DRX_AAX_eFrameRate_48Frame = 9,
        DRX_AAX_eFrameRate_50Frame = 10,
        DRX_AAX_eFrameRate_5994NonDrop = 11,
        DRX_AAX_eFrameRate_5994DropFrame = 12,
        DRX_AAX_eFrameRate_60NonDrop = 13,
        DRX_AAX_eFrameRate_60DropFrame = 14,
        DRX_AAX_eFrameRate_100Frame = 15,
        DRX_AAX_eFrameRate_11988NonDrop = 16,
        DRX_AAX_eFrameRate_11988DropFrame = 17,
        DRX_AAX_eFrameRate_120NonDrop = 18,
        DRX_AAX_eFrameRate_120DropFrame = 19
    };

    static z0 AAX_CALLBACK algorithmProcessCallback (DRXAlgorithmContext* const instancesBegin[], ukk const instancesEnd);

    static Array<DrxAAX_Processor*> activeProcessors;

    //==============================================================================
    class DrxAAX_Processor final : public AAX_CEffectParameters,
                                    public drx::AudioPlayHead,
    ...
    public:
        ...
        AAX_Result GetNumberOfChunks (i32* numChunks) const override
        {
        ...
        AAX_Result GetChunkIDFromIndex (i32 index, AAX_CTypeID* chunkID) const override
        {
        ...
        AAX_Result GetChunkSize (AAX_CTypeID chunkID, u32* oSize) const override
        {
            ...
            *oSize = (u32) chunkMemoryBlock.data.getSize();
            return AAX_SUCCESS;
        }
        ...
        AAX_Result GetChunk (AAX_CTypeID chunkID, AAX_SPlugInChunk* oChunk) const override
        {
            ...
            oChunk->fSize = (i32) chunkMemoryBlock.data.getSize();
        ...
        AAX_Result SetChunk (AAX_CTypeID chunkID, const AAX_SPlugInChunk* chunk) override
        {
            ...
            pluginInstance->setStateInformation ((uk) chunk->fData, chunk->fSize);
            ...
            for (i32 i = 0; i < numParameters; ++i)
                if (auto* juceParam = juceParameters.getParamForIndex (i))
                    if (juceParam != ownedBypassParameter.get())
                        if (auto paramID = getAAXParamIDFromDrxIndex (i))
                            SetParameterNormalizedValue (paramID, juceParam->getValue());

            return AAX_SUCCESS;
        }

        AAX_Result ResetFieldData (AAX_CFieldIndex fieldIndex, uk data, u32 dataSize) const override
        {
            switch (fieldIndex)
            {
                case DRXAlgorithmIDs::pluginInstance:
                {
                    ...
                    for (size_t i = 0; i < numObjects; ++i)
                        new (objects + i) PluginInstanceInfo (const_cast<DrxAAX_Processor&> (*this));

                    break;
                }

                case DRXAlgorithmIDs::preparedFlag:
                {
                    const_cast<DrxAAX_Processor*> (this)->preparePlugin();

                    auto numObjects = dataSize / sizeof (u32);
                    auto* objects = static_cast<u32*> (data);
                ...
                case DRXAlgorithmIDs::meterTapBuffers:
                {
                    // this is a dummy field only when there are no aaxMeters
                    jassert (aaxMeters.size() == 0);

                    {
                        auto numObjects = dataSize / sizeof (f32*);
                        auto* objects = static_cast<f32**> (data);
        ...
        z0 setAudioProcessorParameter (AAX_CParamID paramID, f64 value)
        {
            if (auto* param = getParameterFromID (paramID))
            {
                auto newValue = static_cast<f32> (value);
        ...
        AAX_Result GetNumberOfChanges (i32* numChanges) const override
        {
        ...
        AAX_Result UpdateParameterNormalizedValue (AAX_CParamID paramID, f64 value, AAX_EUpdateSource source) override
        {
        ...
        AAX_Result GetParameterValueFromString (AAX_CParamID paramID, f64* result, const AAX_IString& text) const override
        {
            if (auto* param = getParameterFromID (paramID))
            {
        ...
        AAX_Result GetParameterStringFromValue (AAX_CParamID paramID, f64 value, AAX_IString* result, i32 maxLen) const override
        {
            if (auto* param = getParameterFromID (paramID))
                result->Set (param->getText ((f32) value, maxLen).toRawUTF8());

            return AAX_SUCCESS;
        }

        AAX_Result GetParameterNumberOfSteps (AAX_CParamID paramID, i32* result) const override
        {
            if (auto* param = getParameterFromID (paramID))
        ...
        AAX_Result GetParameterNormalizedValue (AAX_CParamID paramID, f64* result) const override
        {
            if (auto* param = getParameterFromID (paramID))
                *result = (f64) param->getValue();
            else
                *result = 0.0;
        ...
        AAX_Result SetParameterNormalizedValue (AAX_CParamID paramID, f64 newValue) override
        {
            if (auto* p = mParameterManager.GetParameterByID (paramID))
                p->SetValueWithFloat ((f32) newValue);

            setAudioProcessorParameter (paramID, (f32) newValue);

            return AAX_SUCCESS;
        }

        AAX_Result SetParameterNormalizedRelative (AAX_CParamID paramID, f64 newDeltaValue) override
        {
            if (auto* param = getParameterFromID (paramID))
            {
                auto newValue = param->getValue() + (f32) newDeltaValue;
        ...
        AAX_Result GetParameterNameOfLength (AAX_CParamID paramID, AAX_IString* result, i32 maxLen) const override
        {
            if (auto* param = getParameterFromID (paramID))
        ...
        AAX_Result GetParameterDefaultNormalizedValue (AAX_CParamID paramID, f64* result) const override
        {
            if (auto* param = getParameterFromID (paramID))
                *result = (f64) param->getDefaultValue();
            else
                *result = 0.0;
        ...
        Optional<PositionInfo> getPosition() const override
        {
            ...
            const auto signature = [&]
            {
                ...
                return transport.GetCurrentMeter (&num, &den) == AAX_SUCCESS
                     ? makeOptional (TimeSignature { (i32) num, (i32) den })
                     : nullopt;
            }();
            ...
            info.setTimeInSeconds ((f32) optionalTimeInSamples.orFallback (0) / sampleRate);
            ...
            info.setPpqPosition (tickPosition.hasValue() ? makeOptional (static_cast<f64> (*tickPosition) / 960'000.0) : nullopt);

            b8 isLooping = false;
            z64 loopStartTick = 0, loopEndTick = 0;

            if (transport.GetCurrentLoopPosition (&isLooping, &loopStartTick, &loopEndTick) == AAX_SUCCESS)
            {
                info.setIsLooping (isLooping);
                info.setLoopPoints (LoopPoints { (f64) loopStartTick / 960000.0, (f64) loopEndTick / 960000.0 });
            }
            ...
            if (timeCodeIfAvailable.has_value())
            {
                ...
                {
                    switch ((DRX_AAX_EFrameRate) timeCodeIfAvailable->frameRate)
                    {
                        case DRX_AAX_eFrameRate_24Frame:         return FrameRate().withBaseRate (24);
                        case DRX_AAX_eFrameRate_23976:           return FrameRate().withBaseRate (24).withPullDown();

                        case DRX_AAX_eFrameRate_25Frame:         return FrameRate().withBaseRate (25);

                        case DRX_AAX_eFrameRate_30NonDrop:       return FrameRate().withBaseRate (30);
                        case DRX_AAX_eFrameRate_30DropFrame:     return FrameRate().withBaseRate (30).withDrop();
                        case DRX_AAX_eFrameRate_2997NonDrop:     return FrameRate().withBaseRate (30).withPullDown();
                        case DRX_AAX_eFrameRate_2997DropFrame:   return FrameRate().withBaseRate (30).withPullDown().withDrop();

                        case DRX_AAX_eFrameRate_48Frame:         return FrameRate().withBaseRate (48);
                        case DRX_AAX_eFrameRate_47952:           return FrameRate().withBaseRate (48).withPullDown();

                        case DRX_AAX_eFrameRate_50Frame:         return FrameRate().withBaseRate (50);

                        case DRX_AAX_eFrameRate_60NonDrop:       return FrameRate().withBaseRate (60);
                        case DRX_AAX_eFrameRate_60DropFrame:     return FrameRate().withBaseRate (60).withDrop();
                        case DRX_AAX_eFrameRate_5994NonDrop:     return FrameRate().withBaseRate (60).withPullDown();
                        case DRX_AAX_eFrameRate_5994DropFrame:   return FrameRate().withBaseRate (60).withPullDown().withDrop();

                        case DRX_AAX_eFrameRate_100Frame:        return FrameRate().withBaseRate (100);

                        case DRX_AAX_eFrameRate_120NonDrop:      return FrameRate().withBaseRate (120);
                        case DRX_AAX_eFrameRate_120DropFrame:    return FrameRate().withBaseRate (120).withDrop();
                        case DRX_AAX_eFrameRate_11988NonDrop:    return FrameRate().withBaseRate (120).withPullDown();
                        case DRX_AAX_eFrameRate_11988DropFrame:  return FrameRate().withBaseRate (120).withPullDown().withDrop();

                        case DRX_AAX_eFrameRate_Undeclared:      break;
                    }
            ...
            const auto offset = timeCodeIfAvailable.has_value() ? (f64) timeCodeIfAvailable->offset : 0.0;
...

            {
                    ...
                    && transport.GetBarBeatPosition (&bars, &beats, &displayTicks, *optionalTimeInSamples) == AAX_SUCCESS)
                {
                    ...
                    {
                        const auto ticksSinceBar = static_cast<z64> (((beats - 1) * 4 * 960'000) / signature->denominator) + displayTicks;

                        if (tickPosition.hasValue() && ticksSinceBar <= tickPosition)
                        {
                            const auto barStartInTicks = static_cast<f64> (*tickPosition - ticksSinceBar);
        ...
        z0 audioProcessorParameterChanged (AudioProcessor* /*processor*/, i32 parameterIndex, f32 newValue) override
        {
            ...
            if (auto paramID = getAAXParamIDFromDrxIndex (parameterIndex))
                SetParameterNormalizedValue (paramID, (f64) newValue);
        }

        z0 audioProcessorChanged (AudioProcessor* processor, const ChangeDetails& details) override
        {
            ++mNumPlugInChanges;
            ...
            if (details.parameterInfoChanged)
            {
                for (const auto* param : juceParameters)
                    if (auto* aaxParam = mParameterManager.GetParameterByID (getAAXParamIDFromDrxIndex (param->getParameterIndex())))
        ...
        AAX_Result NotificationReceived (AAX_CTypeID type, ukk data, u32 size) override
        {
            switch (type)
            {
                ...
                case AAX_eNotificationEvent_TrackNameChanged:
                    if (data != nullptr)
                    {
                        ...
                        props.name = std::make_optional (Txt::fromUTF8 (static_cast<const AAX_IString*> (data)->Get()));
        ...
        const f32* getAudioBufferForInput (const f32* const* inputs, i32 sidechain, i32 mainNumIns, i32 idx) const noexcept
        {
        ...
        z0 process (const f32* const* inputs, f32* const* outputs, i32k sideChainBufferIdx,
                      i32k bufferSize, const b8 bypass,
                      AAX_IMIDINode* midiNodeIn, AAX_IMIDINode* midiNodesOut,
                      f32* const meterBuffers)
        {
            ...
            b8 isSuspended = [this, sideChainBufferIdx]
            {
                if (processingSidechainChange)
                    return true;

                b8 processWantsSidechain = (sideChainBufferIdx != -1);
            ...
            else
            {
                ...
                f32** channels = channelList.getRawDataPointer();
                ...
                if (numOuts >= numIns)
                {
                    ...
                    for (i32 i = 0; i < numIns; ++i)
                        memcpy (channels[i], getAudioBufferForInput (inputs, sidechain, mainNumIns, i), (size_t) bufferSize * sizeof (f32));

                    for (i32 i = numIns; i < numOuts; ++i)
                        zeromem (channels[i], (size_t) bufferSize * sizeof (f32));
                ...
                else
                {
                    ...
                    for (i32 i = numOuts; i < numIns; ++i)
                        channels[i] = const_cast<f32*> (getAudioBufferForInput (inputs, sidechain, mainNumIns, i));

                    process (channels, numIns, bufferSize, bypass, midiNodeIn, midiNodesOut);
                }

                if (meterBuffers != nullptr)
                    for (i32 i = 0; i < numMeters; ++i)
        ...
        // Returns false if the in/out main layout не поддерживается
        static b8 fullBusesLayoutFromMainLayout (const AudioProcessor& p,
                                                   ...
                                                   AudioProcessor::BusesLayout& fullLayout)
        {
            ...
            [[maybe_unused]] b8 success = p.checkBusesLayoutSupported (currentLayout);
            jassert (success);
            ...
            const b8 hasSidechain = (numInputBuses > 1 && currentLayout.inputBuses.getReference (1) == AudioChannelSet::mono());
            ...
            if (numOutputBuses > 1)
            {
                ...
                i32 maxAuxBuses = jmin (16, numOutputBuses);

                for (i32 i = 1; i < maxAuxBuses; ++i)
                    copy.outputBuses.getReference (i) = mainOutput;

                for (i32 i = maxAuxBuses; i < numOutputBuses; ++i)
    ...
    private:
        friend class DrxAAX_GUI;
        friend z0 AAX_CALLBACK AAXClasses::algorithmProcessCallback (DRXAlgorithmContext* const instancesBegin[], ukk const instancesEnd);

        z0 process (f32* const* channels,
                      i32k numChans,
                      i32k bufferSize,
                      const b8 bypass,
                      AAX_IMIDINode* midiNodeIn,
        ...
        {
            AudioBuffer<f32> buffer (channels, numChans, bufferSize);
            midiBuffer.clear();
            ...
            if (midiNodeIn != nullptr)
            {
                ...
                for (u32 i = 0; i < numMidiEvents; ++i)
                {
                    auto& m = midiStream->mBuffer[i];
                    jassert ((i32) m.mTimestamp < bufferSize);

                    midiBuffer.addEvent (m.mData, (i32) m.mLength,
                                         jlimit (0, (i32) bufferSize - 1, (i32) m.mTimestamp));
                }
            }
            ...
            if (midiNodesOut != nullptr)
            {
                ...
                for (const auto metadata : midiBuffer)
                {
                    ...
                    {
                        packet.mTimestamp   = (u32) metadata.samplePosition;
                        packet.mLength      = (u32) metadata.numBytes;
        ...
        b8 isBypassPartOfRegularParemeters() const
        {
            auto& audioProcessor = getPluginInstance();

            i32 n = juceParameters.getNumParameters();
        ...
        // parameter steps.
        static i32 getSafeNumberOfParameterSteps (const AudioProcessorParameter& param)
        {
            return jmin (param.getNumSteps(), 2048);
        }

        z0 addAudioProcessorParameters()
        {
            auto& audioProcessor = getPluginInstance();
            ...
            for (const auto [parameterIndex, juceParam] : enumerate (juceParameters))
            {
                ...
                auto paramID  = isBypassParameter ? Txt (cDefaultMasterBypassID)
                                                  : juceParameters.getParamID (audioProcessor, (i32) parameterIndex);

                aaxParamIDs.add (paramID);
                auto* aaxParamID = aaxParamIDs.getReference ((i32) parameterIndex).toRawUTF8();
                ...
                auto parameter = new AAX_CParameter<f32> (aaxParamID,
                                                            AAX_CString (juceParam->getName (31).toRawUTF8()),
                                                            juceParam->getDefaultValue(),
                                                            AAX_CLinearTaperDelegate<f32, 0>(),
                                                            AAX_CNumberDisplayDelegate<f32, 3>(),
                ...
                parameter->SetNumberOfSteps ((u32) parameterNumSteps);
                ...
                if (isBypassParameter)
                    mPacketDispatcher.RegisterPacket (aaxParamID, DRXAlgorithmIDs::bypass);
            }
        }

        b8 getMainBusFormats (AudioChannelSet& inputSet, AudioChannelSet& outputSet)
        {
            auto& audioProcessor = getPluginInstance();
        ...
        AAX_Result preparePlugin()
        {
            ...
            if (isInAudioSuite())
            {
                ...
                for (i32 i = 1; i < newLayout.outputBuses.size(); ++i)
            ...
            const b8 layoutChanged = (oldLayout != newLayout);
        ...
        static z0 algorithmCallback (DRXAlgorithmContext* const instancesBegin[], ukk const instancesEnd)
        {
            for (auto iter = instancesBegin; iter < instancesEnd; ++iter)
            {
                auto& i = **iter;

                i32 sideChainBufferIdx = i.pluginInstance->parameters.hasSidechain && i.sideChainBuffers != nullptr
                                             ? static_cast<i32> (*i.sideChainBuffers) : -1;
                ...
                f32* const meterTapBuffers = (i.meterTapBuffers != nullptr && numMeters > 0 ? *i.meterTapBuffers : nullptr);
        ...
        z0 prepareProcessorWithSampleRateAndBufferSize (f64 sr, i32 bs)
        {
        ...
        //==============================================================================
        static AudioProcessor::CurveData::Type aaxCurveTypeToDRX (AAX_CTypeID type) noexcept
        {
            switch (type)
            {
        ...
        u32 getAAXMeterIdForParamId (const Txt& paramID) const noexcept
        {
            i32 idx;
        ...
        //==============================================================================
        AAX_Result GetCurveData (AAX_CTypeID iCurveType, const f32 * iValues, u32 iNumValues, f32 * oValues ) const override
        {
            auto curveType = aaxCurveTypeToDRX (iCurveType);
        ...
        AAX_Result GetCurveDataMeterIds (AAX_CTypeID iCurveType, u32 *oXMeterId, u32 *oYMeterId)  const override
        {
            auto curveType = aaxCurveTypeToDRX (iCurveType);
        ...
        AAX_Result GetCurveDataDisplayRange (AAX_CTypeID iCurveType, f32 *oXMin, f32 *oXMax, f32 *oYMin, f32 *oYMax) const override
        {
            auto curveType = aaxCurveTypeToDRX (iCurveType);
        ...
        //==============================================================================
        inline i32 getParamIndexFromID (AAX_CParamID paramID) const noexcept
        {
            if (auto* param = getParameterFromID (paramID))
        ...
        inline AAX_CParamID getAAXParamIDFromDrxIndex (i32 index) const noexcept
        {
        ...
        //==============================================================================
        static AudioProcessor::BusesLayout getDefaultLayout (const AudioProcessor& p, b8 enableAll)
        {
        ...
        z0 syncParameterAttributes (AAX_IParameter* aaxParam, const AudioProcessorParameter* juceParam)
        {
            ...
            {
                auto newNumSteps = static_cast<u32> (juceParam->getNumSteps());
...

            {
                ...
                if (! approximatelyEqual (static_cast<f32> (aaxParam->GetNormalizedDefaultValue()), defaultValue))
        ...
        Array<f32*> channelList;
        i32 juceChunkIndex = 0, numSetDirtyCalls = 0;
        AAX_CSampleRate sampleRate = 0;
        i32 lastBufferSize = maxSamplesPerBlock, maxBufferSize = maxSamplesPerBlock;
        b8 hasSidechain = false, canDisableSidechain = false, lastSideChainState = false;

        const b8 supportsMidiIn  = supportsMidiInput  (*pluginInstance);
        const b8 supportsMidiOut = supportsMidiOutput (*pluginInstance);
        ...
        /*  Pro Tools 2021 sends TransportStateChanged on the main thread, but we read
            ...
            state in an atomic i32 and convert it to/from an Optional<b8> as necessary.
        */
        class RecordingState
        {
        public:
            /*  This uses Optional rather than std::optional for consistency with get() */
            z0 set (const Optional<b8> newState)
            {
            ...
            /*  PositionInfo::setIsRecording takes an Optional<b8>, so we use that type rather
                than std::optional to avoid having to convert.
            */
            Optional<b8> get() const
            {
        ...
        std::atomic<b8> processingSidechainChange, sidechainDesired;

        std::vector<f32> sideChainBuffer;
        Array<i32> inputLayoutMap, outputLayoutMap;

        Array<Txt> aaxParamIDs;
        HashMap<i32, AudioProcessorParameter*> paramMap;
        ...
        DRX_DECLARE_NON_COPYABLE (DrxAAX_Processor)
    };

    //==============================================================================
    z0 DrxAAX_GUI::CreateViewContents()
    {
        if (component == nullptr)
        {
            if (auto* params = dynamic_cast<DrxAAX_Processor*> (GetEffectParameters()))
    ...
    i32 DrxAAX_GUI::getParamIndexFromID (AAX_CParamID paramID) const noexcept
    {
        if (auto* params = dynamic_cast<const DrxAAX_Processor*> (GetEffectParameters()))
            return params->getParamIndexFromID (paramID);

        return -1;
    }

    AAX_CParamID DrxAAX_GUI::getAAXParamIDFromDrxIndex (i32 index) const noexcept
    {
        if (auto* params = dynamic_cast<const DrxAAX_Processor*> (GetEffectParameters()))
            return params->getAAXParamIDFromDrxIndex (index);
    ...
    struct AAXFormatConfiguration
    {
        ...
        b8 operator== (const AAXFormatConfiguration other) const noexcept
        {
    ...
    //==============================================================================
    static i32 addAAXMeters (AudioProcessor& p, AAX_IEffectDescriptor& descriptor)
    {
    ...
    static z0 createDescriptor (AAX_IComponentDescriptor& desc,
                                  ...
                                  i32k numMeters)
    {
        ...
        check (desc.AddAudioIn  (DRXAlgorithmIDs::inputChannels));
        check (desc.AddAudioOut (DRXAlgorithmIDs::outputChannels));

        check (desc.AddAudioBufferLength (DRXAlgorithmIDs::bufferSize));
        check (desc.AddDataInPort (DRXAlgorithmIDs::bypass, sizeof (i32)));

        if (supportsMidiInput (processor))
            check (desc.AddMIDINode (DRXAlgorithmIDs::midiNodeIn, AAX_eMIDINodeType_LocalInput,
                                     DrxPlugin_Name, 0xffff));

        if (supportsMidiOutput (processor))
            check (desc.AddMIDINode (DRXAlgorithmIDs::midiNodeOut, AAX_eMIDINodeType_LocalOutput,
                                     DrxPlugin_Name " Out", 0xffff));

        check (desc.AddPrivateData (DRXAlgorithmIDs::pluginInstance, sizeof (PluginInstanceInfo)));
        check (desc.AddPrivateData (DRXAlgorithmIDs::preparedFlag, sizeof (i32)));
        ...
        if (numMeters > 0)
        {
            ...
            check (desc.AddMeters (DRXAlgorithmIDs::meterTapBuffers, meterIDs.getData(), static_cast<u32> (numMeters)));
        }
        else
        {
            ...
            check (desc.AddPrivateData (DRXAlgorithmIDs::meterTapBuffers, sizeof (uintptr_t)));
        }
        ...
        properties->AddProperty (AAX_eProperty_ManufacturerID,      DrxPlugin_AAXManufacturerCode);
        properties->AddProperty (AAX_eProperty_ProductID,           DrxPlugin_AAXProductId);

       #if DrxPlugin_AAXDisableBypass
       ...
       #if ! DrxPlugin_AAXDisableAudioSuite
       ...
       #if DrxPlugin_AAXDisableMultiMono
        ...
        {
            check (desc.AddSideChainIn (DRXAlgorithmIDs::sideChainBuffers));
        ...
        else
        {
            ...
            check (desc.AddPrivateData (DRXAlgorithmIDs::sideChainBuffers, sizeof (uintptr_t)));
        }
        ...
        for (i32 busIdx = 1; busIdx < maxAuxBuses + 1; ++busIdx)
        {
            ...
            if (auxFormat != AAX_eStemFormat_INT32_MAX && auxFormat != AAX_eStemFormat_None)
            {
                ...
                check (desc.AddAuxOutputStem (0, static_cast<i32> (auxFormat), name.toRawUTF8()));
            }
        }
    ...
    static z0 getPlugInDescription (AAX_IEffectDescriptor& descriptor, [[maybe_unused]] const AAX_IFeatureInfo* featureInfo)
    {
        ...
        pluginNames.insert (0, DrxPlugin_Name);
        ...
        descriptor.AddCategory (DrxPlugin_AAXCategory);

        i32k numMeters = addAAXMeters (*plugin, descriptor);
       ...
       if (const auto filename = extensions.getPageFileName(); filename.isNotEmpty())
        ...
        check (descriptor.AddProcPtr ((uk) DrxAAX_GUI::Create,        kAAX_ProcPtrID_Create_EffectGUI));
        check (descriptor.AddProcPtr ((uk) DrxAAX_Processor::Create,  kAAX_ProcPtrID_Create_EffectParameters));
        ...
        {
            i32k numIns  = numInputBuses  > 0 ? numElementsInArray (aaxFormats) : 0;
            i32k numOuts = numOutputBuses > 0 ? numElementsInArray (aaxFormats) : 0;

            for (i32 inIdx = 0; inIdx < jmax (numIns, 1); ++inIdx)
            {
                auto aaxInFormat = numIns > 0 ? aaxFormats[inIdx] : AAX_eStemFormat_None;
                auto inLayout = channelSetFromStemFormat (aaxInFormat, false);

                for (i32 outIdx = 0; outIdx < jmax (numOuts, 1); ++outIdx)
                {
                    ...
                    if (! DrxAAX_Processor::fullBusesLayoutFromMainLayout (*plugin, inLayout, outLayout, fullLayout))
                        continue;
...
z0 AAX_CALLBACK AAXClasses::algorithmProcessCallback (DRXAlgorithmContext* const instancesBegin[], ukk const instancesEnd)
{
    AAXClasses::DrxAAX_Processor::algorithmCallback (instancesBegin, instancesEnd);
}

//==============================================================================
AAX_Result DRX_CDECL GetEffectDescriptions (AAX_ICollection*);
AAX_Result DRX_CDECL GetEffectDescriptions (AAX_ICollection* collection)
{
    ScopedDrxInitialiser_GUI libraryInitialiser;
    ...
    if (auto* descriptor = collection->NewDescriptor())
    {
        ...
        collection->AddEffect (DRX_STRINGIFY (DrxPlugin_AAXIdentifier), descriptor);

        collection->SetManufacturerName (DrxPlugin_Manufacturer);
        collection->AddPackageName (DrxPlugin_Desc);
        collection->AddPackageName (DrxPlugin_Name);
        collection->SetPackageVersion (DrxPlugin_VersionCode);
