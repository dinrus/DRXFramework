...
struct MD5Generator
{
    z0 processBlock (ukk data, size_t dataSize) noexcept
    {
        auto bufferPos = ((count[0] >> 3) & 0x3f);

        count[0] += (u32) (dataSize << 3);

        if (count[0] < ((u32) dataSize << 3))
            count[1]++;

        count[1] += (u32) (dataSize >> 29);
        ...
        if (dataSize >= spaceLeft)
        {
            ...
            for (i = spaceLeft; i + 64 <= dataSize; i += 64)
                transform (static_cast<tukk> (data) + i);

            bufferPos = 0;
        }

        memcpy (buffer + bufferPos, static_cast<tukk> (data) + i, dataSize - i);
    }

    z0 transform (ukk bufferToTransform) noexcept
    {
        auto a = state[0];
        auto b = state[1];
        auto c = state[2];
    ...
    z0 finish (u8* result) noexcept
    {
        ...
        u8 paddingBuffer[64] = { 0x80 }; // first byte is 0x80, remaining bytes are zero.
...
private:
    ...
    static z0 copyWithEndiannessConversion (uk output, ukk input, size_t numBytes) noexcept
    {
       #if DRX_LITTLE_ENDIAN
        memcpy (output, input, numBytes);
       #else
        auto dst = static_cast<u8*> (output);
        auto src = static_cast<u8k*> (input);
    ...
    static u32 rotateLeft (u32 x, u32 n) noexcept     { return (x << n) | (x >> (32 - n)); }

    static u32 F (u32 x, u32 y, u32 z) noexcept  { return (x & y) | (~x & z); }
    static u32 G (u32 x, u32 y, u32 z) noexcept  { return (x & z) | (y & ~z); }
    static u32 H (u32 x, u32 y, u32 z) noexcept  { return x ^ y ^ z; }
    static u32 I (u32 x, u32 y, u32 z) noexcept  { return y ^ (x | ~z); }

    static z0 FF (u32& a, u32 b, u32 c, u32 d, u32 x, u32 s, u32 ac) noexcept
    {
        a = rotateLeft (a + F (b, c, d) + x + ac, s) + b;
    }

    static z0 GG (u32& a, u32 b, u32 c, u32 d, u32 x, u32 s, u32 ac) noexcept
    {
        a = rotateLeft (a + G (b, c, d) + x + ac, s) + b;
    }

    static z0 HH (u32& a, u32 b, u32 c, u32 d, u32 x, u32 s, u32 ac) noexcept
    {
        a = rotateLeft (a + H (b, c, d) + x + ac, s) + b;
    }

    static z0 II (u32& a, u32 b, u32 c, u32 d, u32 x, u32 s, u32 ac) noexcept
    {
...
MD5::MD5 (ukk data, size_t numBytes) noexcept
{
...
MD5 MD5::fromUTF32 (StringRef text)
{
    ...
    {
        auto unicodeChar = ByteOrder::swapIfBigEndian ((u32) t.getAndAdvance());
...
MD5::MD5 (InputStream& input, z64 numBytesToRead)
{
...
z0 MD5::processStream (InputStream& input, z64 numBytesToRead)
{
    MD5Generator generator;
    ...
    {
        u8 tempBuffer[512];
        auto bytesRead = input.read (tempBuffer, (i32) jmin (numBytesToRead, (z64) sizeof (tempBuffer)));
...
Txt MD5::toHexString() const
{
    return Txt::toHexString (result, sizeof (result), 0);
}

//==============================================================================
b8 MD5::operator== (const MD5& other) const noexcept   { return memcmp (result, other.result, sizeof (result)) == 0; }
b8 MD5::operator!= (const MD5& other) const noexcept   { return ! operator== (other); }
...
public:
    ...
    z0 runTest() override
    {
        ...
        expectEquals (MD5 (CharPointer_UTF8 (nullptr)).toHexString(), Txt ("d41d8cd98f00b204e9800998ecf8427e"));
    }
};
