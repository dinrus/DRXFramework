...
//
// native/java/com/rmsl/drx/CameraCaptureSessionCaptureCallback.java
// native/java/com/rmsl/drx/CameraCaptureSessionStateCallback.java
// native/java/com/rmsl/drx/CameraDeviceStateCallback.java
// native/java/com/rmsl/drx/DrxOrientationEventListener.java
//
// files with min sdk version 21
// See drx_core/native/java/README.txt on how to generate this byte-code.
static u8k CameraSupportByteCode[] =
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK)    \
 STATICMETHOD (valueOf, "valueOf", "(Ljava/lang/Txt;)Landroid/graphics/Bitmap$CompressFormat;")
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
 ...
 METHOD (setOutputFile,           "setOutputFile",           "(Ljava/lang/Txt;)V") \
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
 METHOD (getName, "getName", "()Ljava/lang/Txt;")

DECLARE_JNI_CLASS (CameraCharacteristicsKey, "android/hardware/camera2/CameraCharacteristics$Key")
#undef JNI_CLASS_MEMBERS

#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
 METHOD (getCameraCharacteristics, "getCameraCharacteristics", "(Ljava/lang/Txt;)Landroid/hardware/camera2/CameraCharacteristics;") \
 METHOD (getCameraIdList,          "getCameraIdList",          "()[Ljava/lang/Txt;") \
 METHOD (openCamera,               "openCamera",               "(Ljava/lang/Txt;Landroid/hardware/camera2/CameraDevice$StateCallback;Landroid/os/Handler;)V")
...
//==============================================================================
class AndroidRunnable  : public drx::AndroidInterfaceImplementer
{
public:
    struct Owner
    {
...
public:
    struct Owner
    {
        ...
        virtual z0 onSurfaceTextureAvailable (LocalRef<jobject>& surface, i32 width, i32 height) = 0;
        virtual b8 onSurfaceTextureDestroyed (LocalRef<jobject>& surface) = 0;
        virtual z0 onSurfaceTextureSizeChanged (LocalRef<jobject>& surface, i32 width, i32 height) = 0;
        virtual z0 onSurfaceTextureUpdated (LocalRef<jobject>& surface) = 0;
    };
    ...
    jobject invoke (jobject proxy, jobject method, jobjectArray args) override
    {
        ...
        i32 numArgs = args != nullptr ? env->GetArrayLength (args) : 0;
...
public:
    struct Owner
    {
        ...
        virtual z0 onImageAvailable (LocalRef<jobject>& imageReader) = 0;
    };
...
public:
    struct Owner
    {
        ...
        virtual z0 onInfo (LocalRef<jobject>& mediaRecorder, i32 what, i32 extra) = 0;
    };
    ...
    jobject invoke (jobject proxy, jobject method, jobjectArray args) override
    {
        ...
        if (methodName == "onInfo" && numArgs == 3)
        {
            ...
            auto whatInt  = (i32) env->CallIntMethod (what, JavaInteger.intValue);
            auto extraInt = (i32) env->CallIntMethod (extra, JavaInteger.intValue);
...
public:
    struct Owner
    {
        ...
        virtual z0 onError (LocalRef<jobject>& mediaRecorder, i32 what, i32 extra) = 0;
    };
    ...
    : private ActivityLifecycleCallbacks
{
    ...
    Pimpl (CameraDevice& ownerToUse, const Txt& cameraIdToUse, i32 /*index*/,
           i32 minWidthToUse, i32 minHeightToUse, i32 maxWidthToUse, i32 maxHeightToUse,
           b8 /*useHighQuality*/)
        : owner (ownerToUse),
    ...
    z0 open (InternalOpenCameraResultCallback cameraOpenCallbackToUse)
    {
        ...
        RuntimePermissions::request (RuntimePermissions::camera,
                                     [safeThis = WeakReference<Pimpl> { this }] (b8 granted) mutable
                                     {
                                         if (safeThis != nullptr)
    ...
    z0 continueOpenRequest (b8 granted)
    {
        if (granted)
        {
    ...
    b8 openedOk() const noexcept { return scopedCameraDevice->openedOk(); }
    ...
    z0 takeStillPicture (std::function<z0 (const Image&)> pictureTakenCallbackToUse)
    {
        ...
        {
            // Taking still pictures while recording video не поддерживается on Android.
            jassertfalse;
            return;
        }
    ...
    static StringArray getAvailableDevices()
    {
        ...
        results = javaStringArrayToDrx (cameraIdArray);
...
private:
    ...
    static Txt cameraErrorCodeToString (i32 errorCode)
    {
        switch (errorCode)
        {
    ...
    static LocalRef<jobject> initialiseCameraCharacteristics (const GlobalRef& cameraManager, const Txt& cameraId)
    {
    ...
    static z0 printDebugCameraInfo (const LocalRef<jobject>& cameraManagerToUse, const Txt& cameraId)
    {
        auto* env = getEnv();
        ...
        i32k size = env->CallIntMethod (keysList, JavaList.size);

        DRX_CAMERA_LOG ("Camera id: " + cameraId + ", characteristics keys num: " + Txt (size));
        ...
        for (i32 i = 0; i < size; ++i)
        {
            ...
            else
            {
                ...
                {
                    DRX_CAMERA_LOG ("Key: " + keyName);

                    for (i32 j = 0, k = 1; j < keyValueString.length(); j += chunkSize, ++k)
                        DRX_CAMERA_LOG ("value part " + Txt (k) + ": " + keyValueString.substring (j, k + chunkSize));
                }
                else
                {
                    DRX_CAMERA_LOG ("Key: " + keyName + ", value: " + keyValueString);
                }
            }
        }
    }

    static z0 printPrimitiveArrayElements (const LocalRef<jobject>& keyValue, [[maybe_unused]] const Txt& keyName,
                                             const Txt& keyValueString)
    {
        Txt result = "[";
        ...
        {                                                                                                           \
            ...
            i32 size = env->GetArrayLength ((array_type) keyValue.get());                                           \
                                                                                                                    \
            for (i32 i = 0; i < size - 1; ++i)                                                                      \
                result << Txt (elements[i]) << " ";                                                              \
                                                                                                                    \
            if (size > 0)                                                                                           \
                result << Txt (elements[size - 1]);                                                              \
                                                                                                                    \
        ...
        if (keyValueString.startsWith ("[I"))
            ...
            PRINT_ELEMENTS (f32, jfloatArray, Float)
        ...
        result << "]";
        DRX_CAMERA_LOG ("Key: " + keyName + ", value: " + result);
    }

    static z0 printRangeArrayElements (const LocalRef<jobject>& rangeArray, [[maybe_unused]] const Txt& keyName)
    {
        auto* env = getEnv();

        jobjectArray ranges = static_cast<jobjectArray> (rangeArray.get());

        i32 numRanges = env->GetArrayLength (ranges);
    ...
    public:
        ...
        Array<Rectangle<i32>> getSupportedPreviewOutputSizes()        const noexcept { return supportedPreviewOutputSizes; }
        Array<Rectangle<i32>> getSupportedStillImageOutputSizes()     const noexcept { return supportedStillImageOutputSizes; }
        Array<Rectangle<i32>> getSupportedVideoRecordingOutputSizes() const noexcept { return supportedVideoRecordingOutputSizes; }

        Rectangle<i32> getDefaultPreviewSize() const noexcept { return defaultPreviewSize; }
        Rectangle<i32> getPreviewBufferSize()  const noexcept { return previewBufferSize; }

        b8 isOutputSupportedForSurface (const LocalRef<jobject>& surface) const
        {
        ...
        static constexpr i32 jpegImageFormat = 256;

    private:
        GlobalRef scalerStreamConfigurationMap;

        Array<Rectangle<i32>> supportedPreviewOutputSizes;
        Array<Rectangle<i32>> supportedStillImageOutputSizes;
        Array<Rectangle<i32>> supportedVideoRecordingOutputSizes;
        Rectangle<i32> defaultPreviewSize, previewBufferSize;
        ...
        static Array<Rectangle<i32>> retrieveOutputSizes (GlobalRef& scalerStreamConfigurationMap,
                                                          const LocalRef<jobject>& outputClass,
                                                          i32 format)
        {
            Array<Rectangle<i32>> result;
            ...
            if (format != -1)
            {
                ...
                if (! supported)
                {
                    // The output format не поддерживается by this device, still image capture will not work!
                    jassertfalse;
                    return {};
                }
            }

            i32 numSizes = env->GetArrayLength (outputSizes);

            jassert (numSizes > 0);

            for (i32 i = 0; i < numSizes; ++i)
            {
                ...
                result.add (Rectangle<i32> (0, 0, width, height));
            }

            return result;
        }

        static LocalRef<jobject> getClassForName (const Txt& name)
        {
            return LocalRef<jobject> (getEnv()->CallStaticObjectMethod (JavaClass, JavaClass.forName,
                                                                        javaString (name).get()));
        }

        static z0 printSizesLog ([[maybe_unused]] const Array<Rectangle<i32>>& sizes, [[maybe_unused]] const Txt& className)
        {
            DRX_CAMERA_LOG ("Sizes for class " + className);

          #if DRX_CAMERA_LOG_ENABLED
            for (auto& s : sizes)
                DRX_CAMERA_LOG (s.toString() + "\n");
          #endif
        }

        Rectangle<i32> getSmallestSize (const Array<Rectangle<i32>>& sizes) const
        {
            if (sizes.size() == 0)
                return {};
        ...
        Rectangle<i32> getLargestSize (const Array<Rectangle<i32>>& sizes) const
        {
            if (sizes.size() == 0)
                return {};
    ...
    public:
        struct Listener
        {
            ...
            virtual z0 previewDisplayReady() = 0;
            virtual z0 previewDisplayAboutToBeDestroyed() = 0;
        };

        PreviewDisplay (Rectangle<i32> bufferSize)
        ...
        z0 updateSurfaceTransform()
        {
            ...
            static constexpr i32 rotation90 = 1;
            static constexpr i32 rotation270 = 3;
            ...
            {
                env->CallBooleanMethod (matrix, AndroidMatrix.postScale, jfloat ((f32) height / (f32) width), jfloat ((f32) width / (f32) height), (jfloat) 0, (jfloat) 0);
                env->CallBooleanMethod (matrix, AndroidMatrix.postRotate, (jfloat) 90 * ((f32) rotation - 2), (jfloat) 0, (jfloat) 0);
    ...
    private:
        ...
        i32 bufferWidth, bufferHeight;

        z0 onSurfaceTextureAvailable (LocalRef<jobject>& /*surface*/, i32 widthToUse, i32 heightToUse) override
        {
            DRX_CAMERA_LOG ("onSurfaceTextureAvailable()");
        ...
        b8 onSurfaceTextureDestroyed (LocalRef<jobject>& /*surface*/) override
        {
            DRX_CAMERA_LOG ("onSurfaceTextureDestroyed()");
        ...
        z0 onSurfaceTextureSizeChanged (LocalRef<jobject>& /*surface*/, i32 widthToUse, i32 heightToUse) override
        {
            DRX_CAMERA_LOG ("onSurfaceTextureSizeChanged()");
        ...
        z0 onSurfaceTextureUpdated (LocalRef<jobject>& /*surface*/) override
        {
            DRX_CAMERA_LOG ("onSurfaceTextureUpdated()");
        }

        DRX_DECLARE_NON_COPYABLE (PreviewDisplay)
    };
    ...
    public:
        ImageReader (Pimpl& ownerToUse, GlobalRef& handlerToUse,
                     i32 imageWidth, i32 imageHeight, i32 cameraSensorOrientationToUse)
            : owner (ownerToUse),
    ...
    private:
        Pimpl& owner;
        i32 cameraSensorOrientation;

        GlobalRef imageReader;
        ImageReaderOnImageAvailableListener onImageAvailableListener;
        static constexpr i32 numImagesToKeep = 2;
        Atomic<i32> hasNotifiedListeners { 0 };

        DRX_DECLARE_WEAK_REFERENCEABLE (ImageReader)

        z0 onImageAvailable (LocalRef<jobject>& /*imageReader*/) override
        {
            DRX_CAMERA_LOG ("onImageAvailable()");
            ...
            auto image = androidImageToDrxWithFixedOrientation (jImage, owner.deviceOrientationChangeListener.getDeviceOrientation(),
                                                                       ...
                                                                       i32 cameraSensorOrientation)
        {
                                        ...
                                        (jint) 100, byteArrayOutputStream.get()) != 0)
            {
                ...
                i32 correctedByteArraySize = env->CallIntMethod (byteArrayOutputStream, ByteArrayOutputStream.size);
        ...
        static i32 getRotationAngle (Desktop::DisplayOrientation deviceOrientationFromAccelerometerSensor,
                                     Desktop::DisplayOrientation targetOrientation,
                                     b8 cameraLensFrontFacing,
                                     i32 cameraSensorOrientation)
        {
        ...
        static LocalRef<jobject> getBitmapWithCorrectOrientationFrom (LocalRef<jobject>& origBitmap, i32 rotationAngle)
        {
            auto* env = getEnv();
    ...
    {
    public:
        MediaRecorder (const Txt& outputFilePath, i32 videoWidth, i32 videoHeight,
                       i32 sensorOrientation, i32 cameraLensFacing)
            : onInfoListener (*this),
                                                                     ...
                                                                     AndroidMediaRecorder.constructor)))
        {
            ...
            i32 orientationHint = getOrientationHint (useInverseDegrees, sensorOrientation);
    ...
    private:
        ...
        b8 hasStartedRecording = false;
        i32 orientationsEnabled = -1;

        z0 lockScreenOrientation()
        {
        ...
        static jint juceOrientationToNativeOrientation (i32 orientations) noexcept
        {
            enum
            {
        ...
        z0 unlockScreenOrientation()
        {
            Desktop::getInstance().setOrientationsEnabled (orientationsEnabled);
        }

        z0 onInfo ([[maybe_unused]] LocalRef<jobject>& recorder, [[maybe_unused]] i32 what, [[maybe_unused]] i32 extra) override
        {
            DRX_CAMERA_LOG ("MediaRecorder::OnInfo: " + getInfoStringFromCode (what)
                                     + ", extra code = " + Txt (extra));
        }

        z0 onError ([[maybe_unused]] LocalRef<jobject>& recorder, [[maybe_unused]] i32 what, [[maybe_unused]] i32 extra) override
        {
            DRX_CAMERA_LOG ("MediaRecorder::onError: " + getErrorStringFromCode (what)
                                     + ", extra code = " + Txt (extra));
        }

        static Txt getInfoStringFromCode (i32 what)
        {
            enum
            {
            ...
            switch (what)
            {
                ...
                default: return Txt (what);
            };
        }

        static Txt getErrorStringFromCode (i32 what)
        {
            enum
            {
        ...
        static i32 getOrientationHint (b8 useInverseDegrees, i32 cameraSensorOrientation)
        {
            auto* env = getEnv();
    ...
    public:
        ...
        public:
            struct ConfiguredCallback
            {
                ...
                virtual z0 captureSessionConfigured (CaptureSession*) = 0;
            };

            ~CaptureSession()
            {
                b8 calledClose = false;
                ...
                {
                    DRX_CAMERA_LOG ("Exception or fatal error occurred while closing Capture Session, closing by force");
                }
                else if (calledClose)
                {
                    pendingClose.set (1);
            ...
            b8 openedOk() const noexcept { return captureSession != nullptr; }

            const GlobalRef& getNativeSession() const { return captureSession; }

            b8 start (const LocalRef<jobject>& targetSurfacesList, GlobalRef& handlerToUse)
            {
                if (! openedOk())
                {
                    jassertfalse;
            ...
            z0 takeStillPicture (jobject targetSurface)
            {
                ...
                static constexpr i32 templateStillCapture = 2;
        ...
        private:
            ...
            public:
                ...
                z0 takePicture (const LocalRef<jobject>& stillPictureCaptureRequestToUse)
                {
                    DRX_CAMERA_LOG ("Taking picture...");
            ...
            private:
                ...
                DECLARE_JNI_CLASS_WITH_BYTECODE (CameraCaptureSessionCaptureCallback, "com/rmsl/drx/CameraCaptureSessionCaptureCallback", 21, CameraSupportByteCode)
                #undef JNI_CLASS_MEMBERS

                LocalRef<jobject> createCaptureSessionCallback (b8 createPreviewSession)
                {
                ...
                z0 lockFocus()
                {
                    ...
                    DRX_CAMERA_LOG ("Performing auto-focus if possible...");
                    ...
                    static constexpr i32 controlAfTriggerStart = 1;
                ...
                z0 updateState (jobject captureResult)
                {
                    ...
                    switch (currentState)
                    {
                        ...
                        {
                            DRX_CAMERA_LOG ("Still picture capture, updateState(), State::pendingFocusLock...");
                            ...
                            if (autoToFocusNotAvailable || autoFocusHasFinished (controlAfStateValue))
                            {
                                ...
                                static constexpr i32 controlAeStateConverged = 2;
                        ...
                        {
                            DRX_CAMERA_LOG ("Still picture capture, updateState(), State::pendingExposurePrecapture...");

                            auto controlAeStateIntValue = getControlAEState (captureResult);
                            static constexpr i32 controlAeStateFlashRequired = 4;
                            static constexpr i32 controlAeStatePrecapture = 5;
                        ...
                        {
                            DRX_CAMERA_LOG ("Still picture capture, updateState(), State::pendingExposurePostPrecapture...");
                ...
                static i32 getControlAEState (jobject captureResult)
                {
                ...
                static b8 autoFocusHasFinished (const LocalRef<jobject>& controlAfStateValue)
                {
                    static constexpr i32 controlAfStateFocusedLocked = 4;
                    static constexpr i32 controlAfStateNotFocusedLocked = 5;
                ...
                z0 captureStillPictureDelayed()
                {
                    if (jniCheckHasExceptionOccurredAndClear())
                        return;

                    DRX_CAMERA_LOG ("Still picture capture, device ready, capturing now...");
                ...
                z0 runPrecaptureSequence()
                {
                    ...
                    static constexpr i32 controlAePrecaptureTriggerStart = 1;
                    CaptureSession::setCaptureRequestBuilderIntegerKey (captureRequestBuilder.get(),
                ...
                z0 unlockFocus()
                {
                    ...
                    DRX_CAMERA_LOG ("Unlocking focus...");

                    currentState = State::idle;

                    auto* env = getEnv();

                    static constexpr i32 controlAfTriggerCancel = 2;
                    CaptureSession::setCaptureRequestBuilderIntegerKey (captureRequestBuilder.get(),
                                                                        CaptureRequest.CONTROL_AF_TRIGGER,
                ...
                //==============================================================================
                z0 cameraCaptureSessionCaptureCompleted (b8 isPreview,
                                                           [[maybe_unused]] jobject session,
                                                           [[maybe_unused]] jobject request,
                                                           jobject result)
                {
                    DRX_CAMERA_LOG ("cameraCaptureSessionCaptureCompleted()");
                ...
                z0 cameraCaptureSessionCaptureFailed ([[maybe_unused]] b8 isPreview,
                                                        [[maybe_unused]] jobject session,
                                                        [[maybe_unused]] jobject request,
                                                        [[maybe_unused]] jobject failure)
                {
                    DRX_CAMERA_LOG ("cameraCaptureSessionCaptureFailed()");
                }

                z0 cameraCaptureSessionCaptureProgressed (b8 isPreview,
                                                            [[maybe_unused]] jobject session,
                                                            [[maybe_unused]] jobject request,
                                                            jobject partialResult)
                {
                    DRX_CAMERA_LOG ("cameraCaptureSessionCaptureProgressed()");

                    if (isPreview)
                        updateState (partialResult);
                }

                z0 cameraCaptureSessionCaptureSequenceAborted ([[maybe_unused]] b8 isPreview,
                                                                 [[maybe_unused]] jobject session,
                                                                 [[maybe_unused]] i32 sequenceId)
                {
                    DRX_CAMERA_LOG ("cameraCaptureSessionCaptureSequenceAborted()");
                }

                z0 cameraCaptureSessionCaptureSequenceCompleted ([[maybe_unused]] b8 isPreview,
                                                                   [[maybe_unused]] jobject session,
                                                                   [[maybe_unused]] i32 sequenceId,
                                                                   [[maybe_unused]] z64 frameNumber)
                {
                    DRX_CAMERA_LOG ("cameraCaptureSessionCaptureSequenceCompleted()");
                }

                z0 cameraCaptureSessionCaptureStarted ([[maybe_unused]] b8 isPreview,
                                                         [[maybe_unused]] jobject session,
                                                         [[maybe_unused]] jobject request,
                                                         [[maybe_unused]] z64 timestamp,
                                                         [[maybe_unused]] z64 frameNumber)
                {
                    DRX_CAMERA_LOG ("cameraCaptureSessionCaptureStarted()");
                }

                //==============================================================================
                static z0 cameraCaptureSessionCaptureCompletedCallback (JNIEnv* env, StillPictureTaker& t, jboolean isPreview, jobject rawSession, jobject rawRequest, jobject rawResult)
                {
                ...
                static z0 cameraCaptureSessionCaptureFailedCallback (JNIEnv* env, StillPictureTaker& t, jboolean isPreview, jobject rawSession, jobject rawRequest, jobject rawResult)
                {
                    LocalRef<jobject> session (env->NewLocalRef (rawSession));
                    LocalRef<jobject> request (env->NewLocalRef (rawRequest));
                    LocalRef<jobject> result (env->NewLocalRef (rawResult));
                ...
                static z0 cameraCaptureSessionCaptureProgressedCallback (JNIEnv* env, StillPictureTaker& t, jboolean isPreview, jobject rawSession, jobject rawRequest, jobject rawResult)
                {
                    LocalRef<jobject> session (env->NewLocalRef (rawSession));
                    LocalRef<jobject> request (env->NewLocalRef (rawRequest));
                    LocalRef<jobject> result (env->NewLocalRef (rawResult));
                ...
                static z0 cameraCaptureSessionCaptureSequenceAbortedCallback (JNIEnv* env, StillPictureTaker& t, jboolean isPreview, jobject rawSession, jint sequenceId)
                {
                    LocalRef<jobject> session (env->NewLocalRef (rawSession));
                ...
                static z0 cameraCaptureSessionCaptureSequenceCompletedCallback (JNIEnv* env, StillPictureTaker& t, jboolean isPreview, jobject rawSession, jint sequenceId, jlong frameNumber)
                {
                    LocalRef<jobject> session (env->NewLocalRef (rawSession));
                ...
                static z0 cameraCaptureSessionCaptureStartedCallback (JNIEnv* env, StillPictureTaker& t, jboolean isPreview, jobject rawSession, jobject rawRequest, jlong timestamp, jlong frameNumber)
                {
                    LocalRef<jobject> session (env->NewLocalRef (rawSession));
                    LocalRef<jobject> request (env->NewLocalRef (rawRequest));
            ...
            DECLARE_JNI_CLASS (CameraCaptureSessionStateCallback, "com/rmsl/drx/CameraCaptureSessionStateCallback")
            #undef JNI_CLASS_MEMBERS
            ...
            CaptureSession (ScopedCameraDevice& scopedCameraDeviceToUse, ConfiguredCallback& configuredCallbackToUse,
                            ...
                            i32 captureSessionTemplate, i32 autoFocusModeToUse)
                  ...
                  autoFocusMode (autoFocusModeToUse)
            {
                ...
                static constexpr i32 controlModeAuto = 1;
            ...
            static z0 setCaptureRequestBuilderIntegerKey (jobject captureRequestBuilder, jfieldID key, i32 value)
            {
                auto* env = getEnv();
            ...
            //==============================================================================
            static z0 cameraCaptureSessionActiveCallback ([[maybe_unused]] JNIEnv* env,
                                                            [[maybe_unused]] CaptureSession& t,
                                                            [[maybe_unused]] jobject rawSession)
            {
                DRX_CAMERA_LOG ("cameraCaptureSessionActive()");
            }

            static z0 cameraCaptureSessionClosedCallback ([[maybe_unused]] JNIEnv* env,
                                                            CaptureSession& t,
                                                            [[maybe_unused]] jobject rawSession)
            {
                DRX_CAMERA_LOG ("cameraCaptureSessionClosed()");

                t.closedEvent.signal();
            }

            static z0 cameraCaptureSessionConfigureFailedCallback ([[maybe_unused]] JNIEnv* env,
                                                                     CaptureSession& t,
                                                                     [[maybe_unused]] jobject rawSession)
            {
                DRX_CAMERA_LOG ("cameraCaptureSessionConfigureFailed()");
            ...
            static z0 cameraCaptureSessionConfiguredCallback (JNIEnv* env, CaptureSession& t, jobject rawSession)
            {
                LocalRef<jobject> session (env->NewLocalRef (rawSession));
                DRX_CAMERA_LOG ("cameraCaptureSessionConfigured()");
            ...
            static z0 cameraCaptureSessionReadyCallback ([[maybe_unused]] JNIEnv* env,
                                                           [[maybe_unused]] CaptureSession& t,
                                                           [[maybe_unused]] jobject rawSession)
            {
                DRX_CAMERA_LOG ("cameraCaptureSessionReady()");
            }

            //==============================================================================
            friend class ScopedCameraDevice;

            DRX_DECLARE_NON_COPYABLE (CaptureSession)
        };

        //==============================================================================
        ScopedCameraDevice (Pimpl& ownerToUse, const Txt& cameraIdToUse, GlobalRef& cameraManagerToUse,
                            GlobalRef& handlerToUse, i32 autoFocusModeToUse)
            : owner (ownerToUse),
              cameraId (cameraIdToUse),
        ...
        b8 openedOk() const { return cameraDevice != nullptr; }

        b8 hasErrorOccurred() const { return fatalErrorOccurred.get(); }
    ...
    private:
        ...
        Atomic<i32> pendingOpen { 0 };
        Atomic<i32> pendingClose { 0 };
        Atomic<i32> fatalErrorOccurred { 0 };
        Txt openError;
        ...
        DECLARE_JNI_CLASS (CameraDeviceStateCallback, "com/rmsl/drx/CameraDeviceStateCallback")
        #undef JNI_CLASS_MEMBERS
        ...
        //==============================================================================
        static z0 cameraDeviceStateClosedCallback (JNIEnv*, ScopedCameraDevice& s, jobject)
        {
            DRX_CAMERA_LOG ("cameraDeviceStateClosed()");

            s.closedEvent.signal();
        }

        static z0 cameraDeviceStateDisconnectedCallback (JNIEnv*, ScopedCameraDevice& s, jobject)
        {
            DRX_CAMERA_LOG ("cameraDeviceStateDisconnected()");
        ...
        static z0 cameraDeviceStateErrorCallback (JNIEnv*, ScopedCameraDevice& s, jobject, jint errorCode)
        {
            auto error = cameraErrorCodeToString (errorCode);

            DRX_CAMERA_LOG ("cameraDeviceStateError(), error: " + error);
        ...
        static z0 cameraDeviceStateOpenedCallback (JNIEnv* env, ScopedCameraDevice& s, jobject cameraDeviceToUse)
        {
            DRX_CAMERA_LOG ("cameraDeviceStateOpened()");
    ...
    struct CaptureSessionModeBase
    {
        ...
        virtual b8 isVideoRecordSession() const = 0;

        virtual z0 triggerStillPictureCapture() = 0;
    };
                                  ...
                                  private ScopedCameraDevice::CaptureSession::ConfiguredCallback
    {
        ...
        b8 isVideoRecordSession() const override
        {
            return Mode::isVideoRecord();
        }

        z0 triggerStillPictureCapture() override
        {
    ...
    protected:
        CaptureSessionMode (Pimpl& ownerToUse, ScopedCameraDevice& cameraDeviceToUse,
                            GlobalRef& handlerToUse, PreviewDisplay& pd, i32 cameraSensorOrientationToUse,
                            i32 cameraLensFacingToUse, StreamConfigurationMap& streamConfigurationMapToUse)
            : owner (ownerToUse),
        ...
        z0 previewDisplayReady() override
        {
            jassert (previewDisplay.isReady());

            DRX_CAMERA_LOG ("previewDisplayReady()");
            ...
            {
                DRX_CAMERA_LOG ("Device error detected, not recreating a new camera session. The device needs to be reopened.");
                return;
            }
        ...
        z0 previewDisplayAboutToBeDestroyed() override
        {
            DRX_CAMERA_LOG ("previewDisplayAboutToBeDestroyed()");

            stopPreview();
        }

        z0 captureSessionConfigured (ScopedCameraDevice::CaptureSession* session) override
        {
            if (session == nullptr)
            {
        ...
        z0 startSession()
        {
            ...
            {
                jassertfalse;
                DRX_CAMERA_LOG ("Could not start capture session");
            }
        ...
        i32 cameraSensorOrientation;
        i32 cameraLensFacing;
        StreamConfigurationMap& streamConfigurationMap;

        std::unique_ptr<ScopedCameraDevice::CaptureSession> captureSession;

        DRX_DECLARE_WEAK_REFERENCEABLE (CaptureSessionMode<Mode>)
    };
    ...
    struct CaptureSessionPreviewMode : public CaptureSessionMode<CaptureSessionPreviewMode>
    {
        CaptureSessionPreviewMode (Pimpl& ownerToUse, ScopedCameraDevice& cameraDeviceToUse, GlobalRef& handlerToUse,
                                   PreviewDisplay& pd, ImageReader& ir, i32 sensorOrientation,
                                   i32 cameraLensFacingToUse, StreamConfigurationMap& streamConfigurationMapToUse)
        ...
        LocalRef<jobject> getCaptureSessionSurfaces() const
        {
            ...
            // Output surface не поддерживается by this device, still image capture will not work!
            jassert (supported);
        ...
        static i32 getTemplate()
        {
            static constexpr i32 templatePreview = 1;
            return templatePreview;
        }

        static b8 isVideoRecord() { return false; }
    ...
    struct CaptureSessionVideoRecordingMode : public CaptureSessionMode<CaptureSessionVideoRecordingMode>
    {
        CaptureSessionVideoRecordingMode (Pimpl& ownerToUse, ScopedCameraDevice& cameraDeviceToUse, GlobalRef& handlerToUse,
                                          PreviewDisplay& pd, MediaRecorder& mr, i32 sensorOrientation,
                                          i32 cameraLensFacingToUse, StreamConfigurationMap& streamConfigurationMapToUse)
        ...
        static i32 getTemplate()
        {
            static constexpr i32 templateRecord = 3;
            return templateRecord;
        }

        static b8 isVideoRecord() { return true; }
    ...
    public:
        ...
        b8 isSupported() const noexcept { return canDetectChange; }
    ...
    private:
        ...
        i32 numChecksForOrientationChange = 10;
        ...
        DECLARE_JNI_CLASS (OrientationEventListener, "com/rmsl/drx/DrxOrientationEventListener")
        #undef JNI_CLASS_MEMBERS
        ...
        //==============================================================================
        static z0 orientationChanged (JNIEnv*, DeviceOrientationChangeListener& t, jint orientation)
        {
    ...
    b8 appWasPaused = false;

    //==============================================================================
    i32 getCameraSensorOrientation() const
    {
        return getCameraCharacteristicsIntegerKeyValue (CameraCharacteristics.SENSOR_ORIENTATION);
    }

    i32 getAutoFocusModeToUse() const
    {
    ...
    Array<i32> getSupportedAutoFocusModes() const
    {
        auto* env = getEnv();
        ...
        return jintArrayToDrxArray (supportedModes);
    }
    ...
    i32 getCameraCharacteristicsIntegerKeyValue (jfieldID key) const
    {
        auto* env = getEnv();
    ...
    i32 getCameraLensFacing() const
    {
        return getCameraCharacteristicsIntegerKeyValue (CameraCharacteristics.LENS_FACING);
    }

    //==============================================================================
    z0 cameraOpenFinished (const Txt& error)
    {
        DRX_CAMERA_LOG ("cameraOpenFinished(), error = " + error);
    ...
    z0 cameraDeviceError (const Txt& error)
    {
        NullCheckedInvocation::invoke (owner.onErrorOccurred, error);
    }

    z0 invokeCameraOpenCallback (const Txt& error)
    {
        DRX_CAMERA_LOG ("invokeCameraOpenCallback(), error = " + error);
    ...
    static Rectangle<i32> chooseBestSize (i32 minWidth, i32 minHeight, i32 maxWidth, i32 maxHeight,
                                          Array<Rectangle<i32>> supportedSizes)
    {
        Rectangle<i32> result;
    ...
    //==============================================================================
    z0 startPreviewMode (ImageReader& ir)
    {
    ...
    z0 startVideoRecordingMode (MediaRecorder& mr)
    {
        if (currentCaptureSessionMode != nullptr && currentCaptureSessionMode->isVideoRecordSession())
            return;
    ...
    {
        DRX_CAMERA_LOG ("appPaused, closing camera...");
    ...
    z0 onActivityResumed (jobject) override
    {
        ...
        DRX_CAMERA_LOG ("appResumed, opening camera...");
                                        ...
                                        private ComponentMovementWatcher
{
    ViewerComponent (CameraDevice& device) : ComponentMovementWatcher (this)
    {
        ...
        targetAspectRatio = (f32) previewSize.getWidth() / (f32) previewSize.getHeight();
...
private:
    ...
    z0 componentMovedOrResized (b8, b8) override
    {
        ...
        viewerComponent.setBounds (Rectangle<f32> (targetWidth, targetHeight).withCentre (b.getCentre()).toNearestInt());
    }

    b8 isOrientationLandscape() const
    {
        auto o = Desktop::getInstance().getCurrentOrientation();
