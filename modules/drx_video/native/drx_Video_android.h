...
//
// native/java/com/rmsl/drx/MediaControllerCallback.java
// native/java/com/rmsl/drx/MediaSessionCallback.java
// native/java/com/rmsl/drx/SystemVolumeObserver.java
//
// files with min sdk version 21
// See drx_core/native/java/README.txt on how to generate this byte-code.
static u8k MediaSessionByteCode[] =
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
 ...
 METHOD (play,            "play",            "()V") \
 METHOD (playFromMediaId, "playFromMediaId", "(Ljava/lang/Txt;Landroid/os/Bundle;)V") \
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
 METHOD (constructor,            "<init>",                 "(Landroid/content/Context;Ljava/lang/Txt;)V") \
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
 ...
 METHOD (constructor, "<init>",  "()V") \
 METHOD (putLong,     "putLong", "(Ljava/lang/Txt;J)Landroid/media/MediaMetadata$Builder;")
...
public:
    struct Owner
    {
        ...
        virtual z0 onPrepared (LocalRef<jobject>& mediaPlayer) = 0;
        virtual z0 onBufferingUpdate (LocalRef<jobject>& mediaPlayer, i32 progress) = 0;
        virtual z0 onSeekComplete (LocalRef<jobject>& mediaPlayer) = 0;
        virtual z0 onCompletion (LocalRef<jobject>& mediaPlayer) = 0;
        virtual b8 onInfo (LocalRef<jobject>& mediaPlayer, i32 what, i32 extra) = 0;
        virtual b8 onError (LocalRef<jobject>& mediaPlayer, i32 what, i32 extra) = 0;
    };
...
private:
    ...
    jobject invoke (jobject proxy, jobject method, jobjectArray args) override
    {
        ...
        if (methodName == "onBufferingUpdate" && numArgs == 2)
        {
            ...
            auto progressInt = (i32) env->CallIntMethod (progress, JavaInteger.intValue);
...
public:
    struct Owner
    {
        ...
        virtual z0 onAudioFocusChange (i32 changeType) = 0;
    };
...
private:
    ...
    jobject invoke (jobject proxy, jobject method, jobjectArray args) override
    {
        ...
        if (methodName == "onAudioFocusChange" && numArgs == 1)
        {
            ...
            auto changeTypeInt = (i32) env->CallIntMethod (changeType, JavaInteger.intValue);
    ...
    : public AndroidViewComponent, private ActivityLifecycleCallbacks, private SurfaceHolderCallback
{
    ...
    b8 isOpen() const noexcept          { return mediaSession.isVideoOpen(); }
    b8 isPlaying() const noexcept       { return mediaSession.isPlaying(); }

    z0 play()                           { mediaSession.play(); }
    z0 stop()                           { mediaSession.stop(); }

    z0 setPosition (f64 newPosition) { mediaSession.setPosition (newPosition); }
    f64 getPosition() const            { return mediaSession.getPosition(); }

    z0 setSpeed (f64 newSpeed)       { mediaSession.setSpeed (newSpeed); }
    f64 getSpeed() const               { return mediaSession.getSpeed(); }

    Rectangle<i32> getNativeSize() const  { return mediaSession.getNativeSize(); }

    f64 getDuration() const            { return mediaSession.getDuration(); }

    z0 setVolume (f32 newVolume)      { mediaSession.setVolume (newVolume); }
    f32 getVolume() const               { return mediaSession.getVolume(); }
...
private:
    ...
    public:
        MediaSession (Pimpl& ownerToUse)
            : owner (ownerToUse),
              ...
              nativeMediaSession (LocalRef<jobject> (getEnv()->NewObject (AndroidMediaSession,
                                                                          ...
                                                                          getAppContext().get(),
                                                                          javaString ("DrxVideoMediaSession").get()))),
        ...
        b8 isVideoOpen() const { return player.isVideoOpen(); }
        b8 isPlaying() const   { return player.isPlaying(); }

        z0 load (const URL& url) { controller.load (url); }
        ...
        z0 setDisplay (const LocalRef<jobject>& surfaceHolder) { player.setDisplay (surfaceHolder); }

        z0 play() { controller.play(); }
        z0 stop() { controller.stop(); }

        z0 setPosition (f64 newPosition) { controller.setPosition (newPosition); }
        f64 getPosition() const            { return controller.getPosition(); }
        ...
        f64 getSpeed() const              { return controller.getPlaySpeed(); }
        Rectangle<i32> getNativeSize() const { return player.getVideoNativeSize(); }
        f64 getDuration() const           { return (f64) player.getVideoDuration() / 1000.0; }
    ...
    private:
        ...
        public:
            ...
            f64 getPosition() const
            {
                ...
                if (playbackState != nullptr)
                    return (f64) env->CallLongMethod (playbackState, AndroidPlaybackState.getPosition) / 1000.0;
            ...
            f64 getPlaySpeed() const
            {
                ...
                if (playbackState != nullptr)
                    return (f64) env->CallFloatMethod (playbackState, AndroidPlaybackState.getPlaybackSpeed);
            ...
            z0 setVolume (f32 newVolume)
            {
                ...
                auto targetVolume = jmin (jint ((f32) maxVolume * newVolume), maxVolume);
            ...
            f32 getVolume() const
            {
                ...
                auto maxVolume = (i32) (env->CallIntMethod (playbackInfo, AndroidMediaControllerPlaybackInfo.getMaxVolume));
                auto curVolume = (i32) (env->CallIntMethod (playbackInfo, AndroidMediaControllerPlaybackInfo.getCurrentVolume));

                return static_cast<f32> (curVolume) / (f32) maxVolume;
            }
        ...
        private:
            ...
            //==============================================================================
            z0 stateChanged (jobject playbackState)
            {
                DRX_VIDEO_LOG ("MediaSessionController::playbackStateChanged()");
            ...
            DECLARE_JNI_CLASS_WITH_BYTECODE (AndroidMediaControllerCallback, "com/rmsl/drx/MediaControllerCallback", 21, MediaSessionByteCode)
           #undef JNI_CLASS_MEMBERS
            ...
            static z0 audioInfoChanged (JNIEnv*, [[maybe_unused]] Controller& t, [[maybe_unused]] jobject playbackInfo)
            {
                DRX_VIDEO_LOG ("MediaSessionController::audioInfoChanged()");
            }

            static z0 metadataChanged (JNIEnv*, [[maybe_unused]] Controller&, [[maybe_unused]] jobject metadata)
            {
                DRX_VIDEO_LOG ("MediaSessionController::metadataChanged()");
            }

            static z0 playbackStateChanged (JNIEnv*, Controller& t, [[maybe_unused]] jobject state)
            {
                t.stateChanged (state);
            }

            static z0 sessionDestroyed (JNIEnv*, [[maybe_unused]] Controller& t)
            {
                DRX_VIDEO_LOG ("MediaSessionController::sessionDestroyed()");
            }
        };
        ...
        public:
            ...
            z0 setDisplay (const LocalRef<jobject>& surfaceHolder)
            {
            ...
            z0 load (const LocalRef<jstring>& mediaId, [[maybe_unused]] const LocalRef<jobject>& extras)
            {
                closeVideo();
            ...
            b8 isVideoOpen() const noexcept
            {
                return currentState == State::prepared || currentState == State::started
                    || currentState == State::paused || currentState == State::complete;
            }

            i32 getPlaybackStateFlag() const noexcept { return getCurrentStateInfo().playbackStateFlag; }
            i32 getAllowedActions()    const noexcept { return getCurrentStateInfo().allowedActions; }
            ...
            Rectangle<i32> getVideoNativeSize() const
            {
                ...
                auto width  = (i32) env->CallIntMethod (nativeMediaPlayer, AndroidMediaPlayer.getVideoWidth);
                auto height = (i32) env->CallIntMethod (nativeMediaPlayer, AndroidMediaPlayer.getVideoHeight);

                return Rectangle<i32> (0, 0, width, height);
            }
            ...
            z0 play()
            {
                ...
                if (! exactlyEqual (lastAudioVolume, std::numeric_limits<f32>::min()))
            ...
            z0 setPlayPosition (jint newPositionMs)
            {
            ...
            f64 getPlaySpeed() const
            {
                ...
                auto playbackParams = LocalRef<jobject> (env->CallObjectMethod (nativeMediaPlayer, AndroidMediaPlayer.getPlaybackParams));
                return (f64) env->CallFloatMethod (playbackParams, AndroidPlaybackParams.getSpeed);
            }
        ...
        private:
            ...
            struct StateInfo
            {
                i32 playbackStateFlag = 0, allowedActions = 0;

                b8 isPlaying, canCallGetCurrentPosition, canCallGetVideoDuration,
            ...
            StateInfo getCurrentStateInfo() const noexcept             { return stateInfos[static_cast<i32> (currentState)]; }
            ...
            f32 lastAudioVolume = std::numeric_limits<f32>::min();
            ...
            //==============================================================================
            z0 onPrepared ([[maybe_unused]] LocalRef<jobject>& mediaPlayer) override
            {
                DRX_VIDEO_LOG ("MediaPlayer::onPrepared()");
            ...
            z0 onBufferingUpdate ([[maybe_unused]] LocalRef<jobject>& mediaPlayer, i32 progress) override
            {
                owner.playerBufferingUpdated (progress);
            }

            z0 onSeekComplete ([[maybe_unused]] LocalRef<jobject>& mediaPlayer) override
            {
                DRX_VIDEO_LOG ("MediaPlayer::onSeekComplete()");

                owner.playerSeekCompleted();
            }

            z0 onCompletion ([[maybe_unused]] LocalRef<jobject>& mediaPlayer) override
            {
                DRX_VIDEO_LOG ("MediaPlayer::onCompletion()");
            ...
            b8 onInfo ([[maybe_unused]] LocalRef<jobject>& mediaPlayer, i32 what, [[maybe_unused]] i32 extra) override
            {
                DRX_VIDEO_LOG ("MediaPlayer::onInfo(), infoCode: " + Txt (what) + " (" + infoCodeToString (what) + ")"
                                + ", extraCode: " + Txt (extra));
            ...
            static Txt infoCodeToString (i32 code)
            {
                switch (code)
                {
            ...
            b8 onError ([[maybe_unused]] LocalRef<jobject>& mediaPlayer, i32 what, i32 extra) override
            {
                ...
                DRX_VIDEO_LOG ("MediaPlayer::onError(), errorCode: " + Txt (what) + " (" + errorMessage + ")"
                                + ", extraCode: " + Txt (extra) + " (" + extraMessage + ")");
            ...
            static Txt errorCodeToString (i32 code)
            {
                enum
                {
        ...
        b8 pendingSeekRequest = false;

        b8 playerBufferingInProgress = false;
        b8 usesBuffering = false;
        SparseSet<i32> bufferedRegions;

        f64 playSpeedMult = 1.0;
        b8 hasAudioFocus = false;
        ...
        #define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
            ...
            CALLBACK (generatedCallback<&MediaSession::playFromMediaIdCallback>, "mediaSessionPlayFromMediaId", "(JLjava/lang/Txt;Landroid/os/Bundle;)V") \
        ...
        DECLARE_JNI_CLASS (AndroidMediaSessionCallback, "com/rmsl/drx/MediaSessionCallback")
        #undef JNI_CLASS_MEMBERS
        ...
        static z0 pauseCallback (JNIEnv*, MediaSession& t)
        {
            DRX_VIDEO_LOG ("MediaSession::pauseCallback()");
            t.player.pause();
            t.updatePlaybackState();
            t.abandonAudioFocus();
        }

        static z0 playCallback (JNIEnv* env, MediaSession& t)
        {
            DRX_VIDEO_LOG ("MediaSession::playCallback()");
        ...
        static z0 playFromMediaIdCallback (JNIEnv* env, MediaSession& t, jstring mediaId, jobject extras)
        {
            DRX_VIDEO_LOG ("MediaSession::playFromMediaIdCallback()");

            t.player.load (LocalRef<jstring> ((jstring) env->NewLocalRef (mediaId)), LocalRef<jobject> (env->NewLocalRef (extras)));
            t.updatePlaybackState();
        }

        static z0 seekToCallback (JNIEnv*, MediaSession& t, jlong pos)
        {
            DRX_VIDEO_LOG ("MediaSession::seekToCallback()");
        ...
        static z0 stopCallback (JNIEnv* env, MediaSession& t)
        {
            DRX_VIDEO_LOG ("MediaSession::stopCallback()");
        ...
        //==============================================================================
        b8 isSeekInProgress() const noexcept
        {
            ...
            auto playPosPercent = (i32) (100.0 * playPos / static_cast<f64> (durationMs));

            // NB: assuming the playback will start roughly when there is 5% of content loaded...
            return ! bufferedRegions.containsRange (Range<i32> (playPosPercent, jmin (101, playPosPercent + 5)));
        }

        z0 updatePlaybackState()
        {
        ...
        {
            static constexpr i32 bufferingState = 6;
        ...
        z0 playerBufferingStarted() { playerBufferingInProgress = true; }
        z0 playerBufferingEnded()   { playerBufferingInProgress = false; }

        z0 playerBufferingUpdated (i32 progress)
        {
            usesBuffering = true;
            ...
            bufferedRegions.addRange (Range<i32> (playPosPercent, progress + 1));
            ...
            DRX_VIDEO_LOG ("Buffering status update, seek pos: " + Txt (playPosPercent) + "%, buffered regions: " + ranges);
        }

        z0 playerSeekCompleted()
        {
            pendingSeekRequest = false;
            updatePlaybackState();
        }

        z0 playerPlaybackCompleted()
        {
            player.pause();
            abandonAudioFocus();
        ...
        z0 errorOccurred (const Txt& errorMessage)
        {
            ...
            // ...also notify DRX side client
            owner.errorOccurred (errorMessage);
        }

        //==============================================================================
        static LocalRef<jobject> createAudioFocusRequestIfNecessary (i32 sdkVersion, const GlobalRef& audioAttributes,
        ...
        z0 onAudioFocusChange (i32 changeType) override
        {
            static constexpr jint audioFocusGain = 1;

            if (changeType == audioFocusGain)
                DRX_VIDEO_LOG ("Audio focus gained");
            else
                DRX_VIDEO_LOG ("Audio focus lost");

            if (changeType != audioFocusGain)
            {
                if (isPlaying())
                {
                    DRX_VIDEO_LOG ("Received a request to abandon audio focus. Stopping playback...");
                    stop();
                }
    ...
    private:
        ...
        DECLARE_JNI_CLASS (SystemVolumeObserver, "com/rmsl/drx/SystemVolumeObserver")
        #undef JNI_CLASS_MEMBERS
    ...
    private:
        //==============================================================================
        static z0 systemVolumeChanged (JNIEnv*, SystemVolumeListener& t)
        {
        ...
        DRX_DECLARE_WEAK_REFERENCEABLE (SystemVolumeListener)
    };
    ...
    //==============================================================================
    z0 surfaceChanged (LocalRef<jobject> holder, i32 /*format*/, i32 /*width*/, i32 /*height*/) override
    {
        mediaSession.setDisplay (holder);
    }

    z0 surfaceDestroyed (LocalRef<jobject> /*holder*/) override
    {
        mediaSession.setDisplay (LocalRef<jobject>());
    }

    z0 surfaceCreated (LocalRef<jobject> /*holder*/) override
    {
    }
    ...
    z0 onActivityPaused (jobject) override
    {
        ...
        DRX_VIDEO_LOG ("App paused, releasing media player...");
    ...
    z0 onActivityResumed (jobject) override
    {
        ...
        DRX_VIDEO_LOG ("App resumed, restoring media player...");
