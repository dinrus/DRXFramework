...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
  METHOD (getProductId,                     "getProductId",                         "()Ljava/lang/Txt;") \
  METHOD (getTitle,                         "getTitle",                             "()Ljava/lang/Txt;") \
  METHOD (getDescription,                   "getDescription",                       "()Ljava/lang/Txt;") \
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
  METHOD (getFormattedPrice,                "getFormattedPrice",                    "()Ljava/lang/Txt;") \
  METHOD (getPriceCurrencyCode,             "getPriceCurrencyCode",                 "()Ljava/lang/Txt;")
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
  METHOD (getOfferToken,                "getOfferToken",                    "()Ljava/lang/Txt;") \
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
  ...
  METHOD (setOldPurchaseToken,            "setOldPurchaseToken",            "(Ljava/lang/Txt;)Lcom/android/billingclient/api/BillingFlowParams$SubscriptionUpdateParams$Builder;") \
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
  ...
  METHOD (setOfferToken,               "setOfferToken",               "(Ljava/lang/Txt;)Lcom/android/billingclient/api/BillingFlowParams$ProductDetailsParams$Builder;") \
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
  METHOD (getOrderId,       "getOrderId",       "()Ljava/lang/Txt;") \
  ...
  METHOD (getPurchaseToken, "getPurchaseToken", "()Ljava/lang/Txt;")

DECLARE_JNI_CLASS (AndroidPurchase, "com/android/billingclient/api/Purchase")
#undef JNI_CLASS_MEMBERS

template <typename Fn>
static z0 callOnMainThread (Fn&& fn)
{
...
inline StringArray javaListOfStringToDrxStringArray (const LocalRef<jobject>& javaArray)
{
...
//==============================================================================
constexpr u8 juceBillingClientCompiled[]
{
...
struct InAppPurchases::Pimpl
{
    Pimpl (InAppPurchases& parent)
        : owner (parent),
          billingClient (LocalRef<jobject> { getEnv()->NewObject (DrxBillingClient,
                                                                  DrxBillingClient.constructor,
                                                                  getAppContext().get(),
                                                                  (jlong) this) })
    {
    }
    ...
    ~Pimpl()
    {
        getEnv()->CallVoidMethod (billingClient, DrxBillingClient.endConnection);
    }

    //==============================================================================
    b8 isInAppPurchasesSupported() const
    {
        return isReady() && getEnv()->CallBooleanMethod (billingClient, DrxBillingClient.isBillingSupported);
    }
    ...
    z0 getProductsInformation (const StringArray& productIdentifiers)
    {
        productDetailsQueryCallbackQueue.emplace ([this] (LocalRef<jobject> productDetailsList)
        {
            if (productDetailsList != nullptr)
            {
                ...
                for (i32 i = 0; i < env->CallIntMethod (productDetailsList, JavaList.size); ++i)
    ...
    z0 purchaseProduct (const Txt& productIdentifier,
                          const Txt& subscriptionIdentifier,
                          b8 creditForUnusedSubscription)
    {
    ...
    z0 restoreProductsBoughtList (b8, const drx::Txt&)
    {
        purchasesListQueryCallbackQueue.emplace ([this] (LocalRef<jobject> purchasesList)
        {
            if (purchasesList != nullptr)
            {
                ...
                for (i32 i = 0; i < env->CallIntMethod (purchasesList, JavaArrayList.size); ++i)
                {
    ...
    //==============================================================================
    z0 startDownloads ([[maybe_unused]] const Array<Download*>& downloads)
    {
        // Not available on this platform.
        jassertfalse;
    }

    z0 pauseDownloads ([[maybe_unused]] const Array<Download*>& downloads)
    {
        // Not available on this platform.
        jassertfalse;
    }

    z0 resumeDownloads ([[maybe_unused]] const Array<Download*>& downloads)
    {
        // Not available on this platform.
        jassertfalse;
    }

    z0 cancelDownloads ([[maybe_unused]] const Array<Download*>& downloads)
    {
        // Not available on this platform.
        jassertfalse;
    }
...
private:
    #define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
      ...
      METHOD (queryProductDetails,           "queryProductDetails",         "([Ljava/lang/Txt;)V")                                                     \
      ...
      METHOD (consumePurchase,               "consumePurchase",             "(Ljava/lang/Txt;Ljava/lang/Txt;)V")                                    \
                                                                                                                                                          \
      ...
      CALLBACK (generatedCallback<&Pimpl::purchaseConsumed>,     "purchaseConsumedCallback",    "(JLjava/lang/Txt;I)V")

    DECLARE_JNI_CLASS_WITH_BYTECODE (DrxBillingClient,
                                     "com/rmsl/drx/DrxBillingClient",
                                     16,
    ...
    b8 isReady() const
    {
        return getEnv()->CallBooleanMethod (billingClient, DrxBillingClient.isReady);
    }

    b8 checkIsReady() const
    {
        for (i32 i = 0; i < 10; ++i)
        {
            if (isReady())
    ...
    z0 queryProductDetailsAsync (const StringArray& productIdentifiers)
    {
        Thread::launch ([=]
        {
            if (! checkIsReady())
            ...
            MessageManager::callAsync ([=]
            {
                getEnv()->CallVoidMethod (billingClient,
                                          DrxBillingClient.queryProductDetails,
                                          juceStringArrayToJava (productIdentifiers).get());
            });
        });
    }

    z0 getProductsBoughtAsync()
    {
        Thread::launch ([=]
        {
            if (! checkIsReady())
            ...
            {
                getEnv()->CallVoidMethod (billingClient,
                                          DrxBillingClient.queryPurchases);
            });
        });
    }

    //==============================================================================
    z0 notifyListenersAboutPurchase (const InAppPurchases::Purchase& purchase, b8 success, const Txt& statusDescription)
    {
        owner.listeners.call ([&] (Listener& l) { l.productPurchaseFinished ({ purchase, {} }, success, statusDescription); });
    }

    z0 notifyListenersAboutConsume (const Txt& productIdentifier, b8 success, const Txt& statusDescription)
    {
        owner.listeners.call ([&] (Listener& l) { l.productConsumed (productIdentifier, success, statusDescription); });
    }

    z0 launchBillingFlowWithParameters (LocalRef<jobject> params)
    {
        const auto activity = []
        {
        ...
        getEnv()->CallVoidMethod (billingClient,
                                  DrxBillingClient.launchBillingFlow,
                                  activity.get(),
                                  params.get());
    }

    z0 changeExistingSubscription (GlobalRef productDetails, const Txt& subscriptionIdentifier, b8 creditForUnusedSubscription)
    {
        if (! isReady())
        {
    ...
    z0 purchaseProductWithProductDetails (GlobalRef productDetails)
    {
        if (! isReady())
        {
            notifyListenersAboutPurchase ({}, false, NEEDS_TRANS ("In-App purchases unavailable"));
    ...
    z0 consumePurchaseWithToken (const Txt& productIdentifier, const Txt& purchaseToken)
    {
        if (! isReady())
        {
            callMemberOnMainThread ([this, productIdentifier]
        ...
        getEnv()->CallVoidMethod (billingClient,
                                  DrxBillingClient.consumePurchase,
    ...
    static InAppPurchases::Purchase buildPurchase (LocalRef<jobject> purchase)
    {
        ...
        return { juceString (LocalRef<jstring> { (jstring) env->CallObjectMethod (purchase, AndroidPurchase.getOrderId) }),
                 javaListOfStringToDrxStringArray (LocalRef<jobject> { env->CallObjectMethod (purchase, AndroidPurchase.getProducts) }),
    ...
    static Txt getStatusDescriptionFromResponseCode (i32 responseCode)
    {
        switch (responseCode)
        {
    ...
    static b8 wasSuccessful (i32 responseCode)
    {
        return responseCode == 0;
    }

    static z0 purchaseCompleted (JNIEnv*, Pimpl& t, jobject purchase, i32 responseCode)
    {
    ...
    static z0 purchaseConsumed (JNIEnv*, Pimpl& t, jstring productIdentifier, i32 responseCode)
    {
    ...
    static z0 updateProductDetails (JNIEnv*, Pimpl& t, jobject productDetailsList)
    {
    ...
    static z0 updatePurchasesList (JNIEnv*, Pimpl& t, jobject purchasesList)
    {
    ...
    std::queue<std::function<z0 (LocalRef<jobject>)>> productDetailsQueryCallbackQueue,
                                                        purchasesListQueryCallbackQueue;

    //==============================================================================
    z0 callMemberOnMainThread (std::function<z0()> callback)
    {
