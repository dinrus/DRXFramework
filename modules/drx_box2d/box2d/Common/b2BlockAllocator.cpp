...
i32 b2BlockAllocator::s_blockSizes[b2_blockSizes] =
{
	16,		// 0
	32,		// 1
	64,		// 2
...
u8 b2BlockAllocator::s_blockSizeLookup[b2_maxBlockSize + 1];
b8 b2BlockAllocator::s_blockSizeLookupInitialized;
...
uk b2BlockAllocator::Allocate(i32 size)
{
	if (size == 0)
		return NULL;
	...
	i32 index = s_blockSizeLookup[size];
...
#endif
		i32 blockSize = s_blockSizes[index];
...
z0 b2BlockAllocator::Free(uk p, i32 size)
{
	if (size == 0)
	{
		return;
	...
	for (i32 i = 0; i < m_chunkCount; ++i)
	{
		...
		{
			b2Assert(	(i8*)p + blockSize <= (i8*)chunk->blocks ||
