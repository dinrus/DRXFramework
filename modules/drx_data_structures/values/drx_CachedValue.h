...
/**
    ...
    a chosen type. So for example a CachedValue<i32> allows you to read or write the
    property as an i32, and a CachedValue<Txt> lets you work with it as a Txt.
    ...
    Common types like Txt, i32, f64 which can be easily converted to a var should work
...
public:
    ...
    */
    ...
    /** Возвращает true, если the current value of the property (or the fallback value)
        is equal to other.
    */
    template <typename OtherType>
    b8 operator== (const OtherType& other) const
    {
     ...
     */
    template <typename OtherType>
    b8 operator!= (const OtherType& other) const   { return ! operator== (other); }

    //==============================================================================
    /** Returns the current property as a Value object. */
    Value getPropertyAsValue();

    /** Возвращает true, если the current property does not exist and the CachedValue is using
        the fallback default value instead.
    */
    b8 isUsingDefault() const;
    ...
    /** Sets the property. This will actually modify the property in the referenced ValueTree. */
    z0 setValue (const Type& newValue, UndoManager* undoManagerToUse);
    ...
    /** Removes the property from the referenced ValueTree and makes the CachedValue
        return the fallback default value instead.
     */
    z0 resetToDefault (UndoManager* undoManagerToUse);

    /** Resets the fallback default value. */
    z0 setDefault (const Type& value)                { defaultValue = value; }

    //==============================================================================
    /** Makes the CachedValue refer to the specified property inside the given ValueTree. */
    z0 referTo (ValueTree& tree, const Identifier& property, UndoManager* um);

    /** Makes the CachedValue refer to the specified property inside the given ValueTree,
        and specifies a fallback value to use if the property does not exist.
    */
    z0 referTo (ValueTree& tree, const Identifier& property, UndoManager* um, const Type& defaultVal);
...
private:
    ...
    //==============================================================================
    z0 referToWithDefault (ValueTree&, const Identifier&, UndoManager*, const Type&);
    Type getTypedValue() const;

    z0 valueTreePropertyChanged (ValueTree& changedTree, const Identifier& changedProperty) override;

    //==============================================================================
    DRX_DECLARE_WEAK_REFERENCEABLE (CachedValue)
    DRX_DECLARE_NON_COPYABLE (CachedValue)
};
...
template <typename Type>
inline b8 CachedValue<Type>::isUsingDefault() const
{
...
template <typename Type>
inline z0 CachedValue<Type>::setValue (const Type& newValue, UndoManager* undoManagerToUse)
{
...
template <typename Type>
inline z0 CachedValue<Type>::resetToDefault()
{
    resetToDefault (undoManager);
}

template <typename Type>
inline z0 CachedValue<Type>::resetToDefault (UndoManager* undoManagerToUse)
{
...
template <typename Type>
inline z0 CachedValue<Type>::referTo (ValueTree& v, const Identifier& i, UndoManager* um)
{
    referToWithDefault (v, i, um, Type());
}

template <typename Type>
inline z0 CachedValue<Type>::referTo (ValueTree& v, const Identifier& i, UndoManager* um, const Type& defaultVal)
{
    referToWithDefault (v, i, um, defaultVal);
}

template <typename Type>
inline z0 CachedValue<Type>::forceUpdateOfCachedValue()
{
    cachedValue = getTypedValue();
}

template <typename Type>
inline z0 CachedValue<Type>::referToWithDefault (ValueTree& v, const Identifier& i, UndoManager* um, const Type& defaultVal)
{
...
template <typename Type>
inline z0 CachedValue<Type>::valueTreePropertyChanged (ValueTree& changedTree, const Identifier& changedProperty)
{
