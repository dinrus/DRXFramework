...
public:
    ...
    z0 callListeners (ValueTree::Listener* listenerToExclude, Function fn) const
    {
        ...
        for (auto [i, v] : enumerate (listenersCopy, i32{}))
        {
    ...
    z0 setProperty (const Identifier& name, const var& newValue, UndoManager* undoManager,
    ...
    b8 hasProperty (const Identifier& name) const noexcept
    {
        return properties.contains (name);
    }

    z0 removeProperty (const Identifier& name, UndoManager* undoManager)
    {
        if (undoManager == nullptr)
        {
    ...
    z0 removeAllProperties (UndoManager* undoManager)
    {
        if (undoManager == nullptr)
        {
    ...
    z0 copyPropertiesFrom (const SharedObject& source, UndoManager* undoManager)
    {
        for (auto i = properties.size(); --i >= 0;)
    ...
    b8 isAChildOf (const SharedObject* possibleParent) const noexcept
    {
    ...
    i32 indexOf (const ValueTree& child) const noexcept
    {
        return children.indexOf (child.object);
    }

    z0 addChild (SharedObject* child, i32 index, UndoManager* undoManager)
    {
    ...
    z0 removeChild (i32 childIndex, UndoManager* undoManager)
    {
    ...
    z0 removeAllChildren (UndoManager* undoManager)
    {
        while (children.size() > 0)
            removeChild (children.size() - 1, undoManager);
    }

    z0 moveChild (i32 currentIndex, i32 newIndex, UndoManager* undoManager)
    {
    ...
    z0 reorderChildren (const OwnedArray<ValueTree>& newOrder, UndoManager* undoManager)
    {
    ...
    b8 isEquivalentTo (const SharedObject& other) const noexcept
    {
        if (type != other.type
    ...
    z0 writeToStream (OutputStream& output) const
    {
        ...
        for (i32 j = 0; j < properties.size(); ++j)
        {
    ...
    static z0 writeObjectToStream (OutputStream& output, const SharedObject* object)
    {
        if (object != nullptr)
        {
    ...
    struct SetPropertyAction final : public UndoableAction
    {
        SetPropertyAction (Ptr targetObject, const Identifier& propertyName,
                           const var& newVal, const var& oldVal, b8 isAdding, b8 isDeleting,
                           ValueTree::Listener* listenerToExclude = nullptr)
        ...
        {
            return (i32) sizeof (*this); //xxx should be more accurate
        }
    ...
    private:
        ...
        const var newValue;
        var oldValue;
        const b8 isAddingNewProperty : 1, isDeletingProperty : 1;
        ValueTree::Listener* excludeListener;

        DRX_DECLARE_NON_COPYABLE (SetPropertyAction)
    };

    //==============================================================================
    struct AddOrRemoveChildAction final : public UndoableAction
    {
        AddOrRemoveChildAction (Ptr parentObject, i32 index, SharedObject* newChild)
    ...
    private:
        ...
        DRX_DECLARE_NON_COPYABLE (AddOrRemoveChildAction)
    };

    //==============================================================================
    struct MoveChildAction final : public UndoableAction
    {
        MoveChildAction (Ptr parentObject, i32 fromIndex, i32 toIndex) noexcept
    ...
    private:
        const Ptr parent;
        i32k startIndex, endIndex;

        DRX_DECLARE_NON_COPYABLE (MoveChildAction)
    };
    ...
    DRX_LEAK_DETECTOR (SharedObject)
};
...
b8 ValueTree::operator== (const ValueTree& other) const noexcept
{
...
b8 ValueTree::isEquivalentTo (const ValueTree& other) const
{
    return object == other.object
...
z0 ValueTree::copyPropertiesFrom (const ValueTree& source, UndoManager* undoManager)
{
...
z0 ValueTree::copyPropertiesAndChildrenFrom (const ValueTree& source, UndoManager* undoManager)
{
...
b8 ValueTree::hasType (const Identifier& typeName) const noexcept
{
...
ValueTree ValueTree::getSibling (i32 delta) const noexcept
{
    if (object != nullptr)
        if (auto* p = object->parent)
...
b8 ValueTree::hasProperty (const Identifier& name) const noexcept
{
    return object != nullptr && object->hasProperty (name);
}

z0 ValueTree::removeProperty (const Identifier& name, UndoManager* undoManager)
{
    if (object != nullptr)
        object->removeProperty (name, undoManager);
}

z0 ValueTree::removeAllProperties (UndoManager* undoManager)
{
    if (object != nullptr)
        object->removeAllProperties (undoManager);
}

i32 ValueTree::getNumProperties() const noexcept
{
    return object == nullptr ? 0 : object->properties.size();
}

Identifier ValueTree::getPropertyName (i32 index) const noexcept
{
    return object == nullptr ? Identifier()
                             : object->properties.getName (index);
}

i32 ValueTree::getReferenceCount() const noexcept
{
...
{
    ValueTreePropertyValueSource (const ValueTree& vt, const Identifier& prop, UndoManager* um, b8 sync)
    ...
    z0 setValue (const var& newValue) override  { tree.setProperty (property, newValue, undoManager); }

private:
    ValueTree tree;
    const Identifier property;
    UndoManager* const undoManager;
    const b8 updateSynchronously;

    z0 valueTreePropertyChanged (ValueTree& changedTree, const Identifier& changedProperty) override
    {
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ValueTreePropertyValueSource)
};

Value ValueTree::getPropertyAsValue (const Identifier& name, UndoManager* undoManager, b8 updateSynchronously)
{
    return Value (new ValueTreePropertyValueSource (*this, name, undoManager, updateSynchronously));
}

//==============================================================================
i32 ValueTree::getNumChildren() const noexcept
{
    return object == nullptr ? 0 : object->children.size();
}

ValueTree ValueTree::getChild (i32 index) const
{
    if (object != nullptr)
...
ValueTree::Iterator::Iterator (const ValueTree& v, b8 isEnd)
...
b8 ValueTree::Iterator::operator== (const Iterator& other) const  { return internal == other.internal; }
b8 ValueTree::Iterator::operator!= (const Iterator& other) const  { return internal != other.internal; }
...
b8 ValueTree::isAChildOf (const ValueTree& possibleParent) const noexcept
{
    return object != nullptr && object->isAChildOf (possibleParent.object.get());
}

i32 ValueTree::indexOf (const ValueTree& child) const noexcept
{
    return object != nullptr ? object->indexOf (child) : -1;
}

z0 ValueTree::addChild (const ValueTree& child, i32 index, UndoManager* undoManager)
{
...
z0 ValueTree::appendChild (const ValueTree& child, UndoManager* undoManager)
{
    addChild (child, -1, undoManager);
}

z0 ValueTree::removeChild (i32 childIndex, UndoManager* undoManager)
{
    if (object != nullptr)
        object->removeChild (childIndex, undoManager);
}

z0 ValueTree::removeChild (const ValueTree& child, UndoManager* undoManager)
{
    if (object != nullptr)
        object->removeChild (object->children.indexOf (child.object), undoManager);
}

z0 ValueTree::removeAllChildren (UndoManager* undoManager)
{
    if (object != nullptr)
        object->removeAllChildren (undoManager);
}

z0 ValueTree::moveChild (i32 currentIndex, i32 newIndex, UndoManager* undoManager)
{
    if (object != nullptr)
...
//==============================================================================
z0 ValueTree::createListOfChildren (OwnedArray<ValueTree>& list) const
{
    if (object != nullptr)
...
z0 ValueTree::reorderChildren (const OwnedArray<ValueTree>& newOrder, UndoManager* undoManager)
{
    if (object != nullptr)
...
//==============================================================================
z0 ValueTree::addListener (Listener* listener)
{
    if (listener != nullptr)
    {
        if (listeners.isEmpty() && object != nullptr)
...
z0 ValueTree::removeListener (Listener* listener)
{
...
z0 ValueTree::sendPropertyChangeMessage (const Identifier& property)
{
    if (object != nullptr)
...
ValueTree ValueTree::fromXml (const Txt& xmlText)
{
...
Txt ValueTree::toXmlString (const XmlElement::TextFormat& format) const
{
...
//==============================================================================
z0 ValueTree::writeToStream (OutputStream& output) const
{
...
ValueTree ValueTree::readFromData (ukk data, size_t numBytes)
{
    MemoryInputStream in (data, numBytes, false);
    return readFromStream (in);
}

ValueTree ValueTree::readFromGZIPData (ukk data, size_t numBytes)
{
    MemoryInputStream in (data, numBytes, false);
...
z0 ValueTree::Listener::valueTreePropertyChanged   (ValueTree&, const Identifier&) {}
z0 ValueTree::Listener::valueTreeChildAdded        (ValueTree&, ValueTree&)        {}
z0 ValueTree::Listener::valueTreeChildRemoved      (ValueTree&, ValueTree&, i32)   {}
z0 ValueTree::Listener::valueTreeChildOrderChanged (ValueTree&, i32, i32)          {}
z0 ValueTree::Listener::valueTreeParentChanged     (ValueTree&)                    {}
z0 ValueTree::Listener::valueTreeRedirected        (ValueTree&)                    {}
...
public:
    ...
    static Txt createRandomIdentifier (Random& r)
    {
        t8 buffer[50] = { 0 };
        const t8 chars[] = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_-:";

        for (i32 i = 1 + r.nextInt (numElementsInArray (buffer) - 2); --i >= 0;)
    ...
    static Txt createRandomWideCharString (Random& r)
    {
        ...
        for (i32 i = r.nextInt (numElementsInArray (buffer) - 1); --i >= 0;)
        {
            if (r.nextBool())
            {
                ...
                buffer[i] = (t32) (1 + r.nextInt (0x7e));
        }

        return CharPointer_UTF32 (buffer);
    }

    static ValueTree createRandomTree (UndoManager* undoManager, i32 depth, Random& r)
    {
        ValueTree v (createRandomIdentifier (r));

        for (i32 i = r.nextInt (10); --i >= 0;)
        {
            switch (r.nextInt (5))
            {
