...
public:
    ...
    /** Connects to a datagram socket and prepares the socket for sending OSC
        ...
        @see send, disconnect.
    */
    b8 connect (const Txt& targetHostName, i32 targetPortNumber);
    ...
    */
    b8 connectToSocket (DatagramSocket& socket, const Txt& targetHostName, i32 targetPortNumber);
    ...
    */
    ...
    /** Sends an OSC message to a specific IP address and port.
        ...
        @returns true if the operation was successful.
    */
    b8 sendToIPAddress (const Txt& targetIPAddress, i32 targetPortNumber,
    ...
    */
    ...
    b8 send (const OSCAddressPattern& address, Args&&... args);
...
private:
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (OSCSender)
};


//==============================================================================
template <typename... Args>
b8 OSCSender::send (const OSCAddressPattern& address, Args&&... args)
{
    return send (OSCMessage (address, std::forward<Args> (args)...));
}

template <typename... Args>
b8 OSCSender::sendToIPAddress (const Txt& targetIPAddress, i32 targetPortNumber,
                                 const OSCAddressPattern& address, Args&&... args)
{
