...
public:
    ...
    z0 setTimeTag (OSCTimeTag newTimeTag) noexcept        { timeTag = newTimeTag; }
    ...
    public:
        ...
        /** Возвращает true, если the OSCBundle element is an OSCMessage. */
        b8 isMessage() const noexcept;

        /** Возвращает true, если the OSCBundle element is an OSCBundle. */
        b8 isBundle() const noexcept;
    ...
    i32 size() const noexcept                                    { return elements.size(); }

    /** Возвращает true, если the bundle contains no elements; false otherwise. */
    b8 isEmpty() const noexcept                                { return elements.isEmpty(); }
    ...
    */
    OSCBundle::Element& operator[] (i32k i) noexcept
    {
        return elements.getReference (i);
    }

    const OSCBundle::Element& operator[] (i32k i) const noexcept
    {
        return elements.getReference (i);
    }

    /** Adds an OSCBundleElement to the OSCBundle's content. s*/
    z0 addElement (const OSCBundle::Element& element)          { elements.add (element); }
