...
static z0 appendToFile (const File& f, const Txt& s)
{
...
PerformanceCounter::PerformanceCounter (const Txt& name, i32 runsPerPrintout, const File& loggingFile)
...
z0 PerformanceCounter::Statistics::clear() noexcept
{
    averageSeconds = maximumSeconds = minimumSeconds = totalSeconds = 0;
    numRuns = 0;
}

z0 PerformanceCounter::Statistics::addResult (f64 elapsed) noexcept
{
    if (numRuns == 0)
    {
...
static Txt timeToString (f64 secs)
{
    return Txt ((z64) (secs * (secs < 0.01 ? 1000000.0 : 1000.0) + 0.5))
                    + (secs < 0.01 ? " microsecs" : " millisecs");
}

Txt PerformanceCounter::Statistics::toString() const
{
...
z0 PerformanceCounter::start() noexcept
{
    startTime = Time::getHighResolutionTicks();
}

b8 PerformanceCounter::stop()
{
...
z0 PerformanceCounter::printStatistics()
{
    const Txt desc (getStatisticsAndReset().toString());
...
PerformanceCounter::Statistics PerformanceCounter::getStatisticsAndReset()
{
    ...
    if (s.numRuns > 0)
        s.averageSeconds = s.totalSeconds / (f32) s.numRuns;
