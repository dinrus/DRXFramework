...
/**
    ...
    - Either:
        ...
        - For types that must do slightly different work when loading and saving, you may supply two
          ...
          static z0 load (Archive& archive, T& item);

          template <typename Archive>
          static z0 save (Archive& archive, const T& item);
          @endcode
    ...
    of the object being deserialised. archive.getVersion() will return an std::optional<i32>,
...
#define DRX_COMPARISON_OPS X(==) X(!=) X(<) X(<=) X(>) X(>=)
...
struct SerialisationTraits<std::vector<Ts...>>
{
    ...
    static z0 load (Archive& archive, T& t)
    {
        auto size = t.size();
    ...
    template <typename Archive, typename T>
    static z0 save (Archive& archive, const T& t)
    {
...
template <typename Element, typename Mutex, i32 minSize>
...
struct SerialisationTraits<std::optional<T>>
{
    ...
    template <typename Archive>
    static z0 load (Archive& archive, std::optional<T>& t)
    {
        b8 engaged = false;
    ...
    template <typename Archive>
    static z0 save (Archive& archive, const std::optional<T>& t)
    {
...
struct SerialisationTraits<std::string>
{
    ...
    template <typename Archive>
    static z0 load (Archive& archive, std::string& t)
    {
        Txt temporary;
        archive (temporary);
    ...
    template <typename Archive>
    static z0 save (Archive& archive, const std::string& t)
    {
        archive (Txt (t));
    }
};
...
template <size_t N>
struct SerialisationTraits<t8[N]>
{
    static constexpr auto marshallingVersion = std::nullopt;

    template <typename Archive, typename T>
    static z0 serialise (Archive& archive, T& t) { archive (Txt (t, N)); }
};
...
namespace detail
{
    struct DummyArchive
    {
        ...
        std::optional<i32> getVersion() const { return {}; }
    };
    ...
    template <typename Traits, typename T, typename = z0>
    ...
    */
    ...
    {
        if constexpr (std::is_arithmetic_v<T> || std::is_enum_v<T> || std::is_same_v<T, Txt> || std::is_same_v<T, var>)
    ...
    */
    ...
    {
        static constexpr std::optional<i32> marshallingVersion { T::marshallingVersion };
