...
namespace WindowsFileHelpers
{
    //==============================================================================
   #if DRX_WINDOWS
    DRX_BEGIN_IGNORE_WARNINGS_GCC_LIKE ("-Wnested-anon-types")
    ...
    //==============================================================================
    static DWORD getAtts (const Txt& path) noexcept
    {
        return GetFileAttributes (path.toWideCharPointer());
    }

    static b8 changeAtts (const Txt& path, DWORD bitsToSet, DWORD bitsToClear) noexcept
    {
    ...
    static z64 fileTimeToTime (const FILETIME* const ft) noexcept
    {
        static_assert (sizeof (ULARGE_INTEGER) == sizeof (FILETIME),
                       "ULARGE_INTEGER is too small to hold FILETIME: please report!");

        return (z64) ((reinterpret_cast<const ULARGE_INTEGER*> (ft)->QuadPart - 116444736000000000LL) / 10000);
    }

    static FILETIME* timeToFileTime (const z64 time, FILETIME* const ft) noexcept
    {
        if (time <= 0)
            return nullptr;
    ...
    static Txt getDriveFromPath (Txt path)
    {
    ...
    static b8 hasFileAccess (const File& file, DWORD accessType)
    {
...
b8 File::moveToTrash() const
{
    ...
    // The string we pass in must be f64 null terminated..
...
//==============================================================================
z64 drx_fileSetPosition (uk handle, z64 pos)
{
    LARGE_INTEGER li;
    li.QuadPart = pos;
...
ssize_t FileOutputStream::writeInternal (ukk bufferToWrite, size_t numBytes)
{
    DWORD actualNum = 0;
...
z64 File::getSize() const
{
    ...
    if (GetFileAttributesEx (fullPath.toWideCharPointer(), GetFileExInfoStandard, &attributes))
        return (((z64) attributes.nFileSizeHigh) << 32) | attributes.nFileSizeLow;
...
zu64 File::getFileIdentifier() const
{
    ...
    if (h != INVALID_HANDLE_VALUE)
    {
        ...
        if (GetFileInformationByHandle (h, &info))
            result = (((zu64) info.nFileIndexHigh) << 32) | info.nFileIndexLow;
...
//==============================================================================
File DRX_CALLTYPE File::getSpecialLocation (const SpecialLocationType type)
{
    i32 csidlType = 0;
...
Txt File::getVersion() const
{
    ...
    if (GetFileVersionInfo (getFullPathName().toWideCharPointer(), 0, bufferSize, buffer))
    {
        ...
        {
            result << (i32) HIWORD (vffi->dwFileVersionMS) << '.'
                   << (i32) LOWORD (vffi->dwFileVersionMS) << '.'
                   << (i32) HIWORD (vffi->dwFileVersionLS) << '.'
                   << (i32) LOWORD (vffi->dwFileVersionLS);
        }
    }
...
b8 File::isShortcut() const
{
    return hasFileExtension (".lnk");
}

static Txt readWindowsLnkFile (File lnkFile, b8 wantsAbsolutePath)
{
    if (! lnkFile.exists())
...
static Txt readWindowsShortcutOrLink (const File& shortcut, b8 wantsAbsolutePath)
{
   #if DRX_WINDOWS
    if (! wantsAbsolutePath)
    {
        ...
        if (h != INVALID_HANDLE_VALUE)
        {
            ...
            b8 success = DeviceIoControl (h, FSCTL_GET_REPARSE_POINT, nullptr, 0,
    ...
    static constexpr tukk prefix = R"(\\?\)";
...
b8 File::createShortcut (const Txt& description, const File& linkFileToCreate) const
{
...
{
public:
    Pimpl (const File& directory, const Txt& wildCardIn)
        : directoryWithWildCard (directory.getFullPathName().isNotEmpty() ? File::addTrailingSeparator (directory.getFullPathName()) + wildCardIn : Txt()),
               ...
               Time* const modTime, Time* const creationTime, b8* const isReadOnly)
    {
        ...
        if (fileSize != nullptr)      *fileSize     = findData.nFileSizeLow + (((z64) findData.nFileSizeHigh) << 32);
...
private:
    const Txt directoryWithWildCard;
    HANDLE handle;

    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (Pimpl)
};

DirectoryIterator::NativeIterator::NativeIterator (const File& directory, const Txt& wildCardIn)
...
//==============================================================================
b8 DRX_CALLTYPE Process::openDocument (const Txt& fileName, const Txt& parameters)
{
...
z0 File::revealToUser() const
{
    ...
    DRX_LOAD_WINAPI_FUNCTION (dll, ILCreateFromPathW, ilCreateFromPathW, ITEMIDLIST*, (LPCWSTR))
    DRX_LOAD_WINAPI_FUNCTION (dll, ILFree, ilFree, z0, (ITEMIDLIST*))
    DRX_LOAD_WINAPI_FUNCTION (dll, SHOpenFolderAndSelectItems, shOpenFolderAndSelectItems, HRESULT, (ITEMIDLIST*, UINT, uk, DWORD))
...
{
public:
    Pimpl (const Txt& pipeName, const b8 createPipe, b8 mustNotExist)
    ...
    i32 read (uk destBuffer, i32k maxBytesToRead, i32k timeOutMilliseconds)
    {
    ...
    b8 connected, ownsPipe;
    std::atomic<b8> shouldStop;
...
private:
    ...
    b8 waitForIO (OverlappedEvent& over, i32 timeOutMilliseconds)
    {
        if (shouldStop)
        {
            CancelIo (pipeH);
...
b8 NamedPipe::openInternal (const Txt& pipeName, const b8 createPipe, b8 mustNotExist)
{
