...
//==============================================================================
// This byte-code is generated from native/java/com/rmsl/drx/DrxHTTPStream.java with min sdk version 16
// See drx_core/native/java/README.txt on how to generate this byte-code.
static u8k javaDrxHttpStream[] =
...
//==============================================================================
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
 STATICMETHOD (createHTTPStream, "createHTTPStream", "(Ljava/lang/Txt;Z[BLjava/lang/Txt;I[ILjava/lang/StringBuffer;ILjava/lang/Txt;)Lcom/rmsl/drx/DrxHTTPStream;") \
...
DECLARE_JNI_CLASS_WITH_BYTECODE (HTTPStream, "com/rmsl/drx/DrxHTTPStream", 16, javaDrxHttpStream)
#undef JNI_CLASS_MEMBERS
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
  METHOD (createMulticastLock, "createMulticastLock", "(Ljava/lang/Txt;)Landroid/net/wifi/WifiManager$MulticastLock;") \
...
static jobject getMulticastLock()
{
    static GlobalRef multicastLock = [&]
    {
        ...
        return GlobalRef (LocalRef<jobject> (env->CallObjectMethod (wifiManager.get(),
                                                                    ...
                                                                    javaString ("DRX_MulticastLock").get())));
    }();

    return multicastLock.get();
}

DRX_API z0 DRX_CALLTYPE acquireMulticastLock();
DRX_API z0 DRX_CALLTYPE acquireMulticastLock()
{
...
DRX_API z0 DRX_CALLTYPE releaseMulticastLock();
DRX_API z0 DRX_CALLTYPE releaseMulticastLock()
{
    auto multicastLock = getMulticastLock();
...
DRX_API b8 DRX_CALLTYPE Process::openEmailWithAttachments (const Txt& /*targetEmailAddress*/,
...
public:
    ...
    Pimpl (WebInputStream&, const URL& urlToCopy, b8 addParametersToBody)
        : url (urlToCopy),
    ...
    b8 connect (WebInputStream::Listener* /*listener*/)
    {
        ...
        {
            Txt address = url.toString (! addParametersToRequestBody);
    ...
    //==============================================================================
    b8 isError() const                         { return stream == nullptr; }
    b8 isExhausted()                           { return (isContentURL ? eofStreamReached : stream != nullptr && stream.callBooleanMethod (HTTPStream.isExhausted)); }
    z64 getTotalLength()                       { return (isContentURL ? -1           : (stream != nullptr ? stream.callLongMethod (HTTPStream.getTotalLength) : 0)); }
    z64 getPosition()                          { return (isContentURL ? readPosition : (stream != nullptr ? stream.callLongMethod (HTTPStream.getPosition)    : 0)); }
    ...
    b8 setPosition (z64 wantedPos)
    {
        if (isContentURL)
        {
            ...
            HeapBlock<t8> buffer (bytesToSkip);

            return (read (buffer.getData(), (i32) bytesToSkip) > 0);
        }
...
private:
    const URL url;
    const b8 isContentURL, addParametersToRequestBody, hasBodyDataToSend;
    b8 eofStreamReached = false;
    i32 numRedirectsToFollow = 5, timeOutMs = 0;
    Txt httpRequest, headers;
...
static Array<InterfaceInfo> findIPAddresses (i32 dummySocket)
{
    ifconf cfg;
    HeapBlock<t8> buffer;
    i32 bufferSize = 1024;
    ...
    } while (bufferSize < cfg.ifc_len + 2 * (i32) (IFNAMSIZ + sizeof (struct sockaddr_in6)));
