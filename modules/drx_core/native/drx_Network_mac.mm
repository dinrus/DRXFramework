    ...
    if (getifaddrs (&addrs) == 0)
    {
        for (const ifaddrs* cursor = addrs; cursor != nullptr; cursor = cursor->ifa_next)
        {
            ...
            if (sto.sa_family == AF_LINK)
            {
                ...
                {
                    MACAddress ma (((u8k*) sadd->sdl_data) + sadd->sdl_nlen);
...
//==============================================================================
b8 DRX_CALLTYPE Process::openEmailWithAttachments ([[maybe_unused]] const Txt& targetEmailAddress,
                                                      [[maybe_unused]] const Txt& emailSubject,
                                                      [[maybe_unused]] const Txt& bodyText,
                                                      [[maybe_unused]] const StringArray& filesToAttach)
{
    ...
    DRX_AUTORELEASEPOOL
    {
        ...
        const b8 ok = [s executeAndReturnError: &error] != nil;
        [s release];
...
struct SessionListener
{
    ...
    virtual z0 didComplete (NSError*) = 0;
    virtual z0 didReceiveResponse (NSURLResponse*, z0 (^) (NSURLSessionResponseDisposition)) = 0;
    virtual z0 didReceiveData (NSData*) = 0;
    virtual z0 didSendBodyData (z64) = 0;
    virtual z0 willPerformHTTPRedirection (NSURLRequest*, z0 (^) (NSURLRequest *)) = 0;
};
...
public:
    ...
    z0 removeTask (NSURLSessionTask* task)
    {
        std::unique_lock lock { mutex };
...
private:
    z0 didBecomeInvalid ([[maybe_unused]] NSError* error)
    {
       #if DRX_DEBUG
        if (error != nullptr)
            DBG (nsStringToDrx ([error description]));
       #endif
    ...
    z0 didComplete (NSURLSessionTask* task, [[maybe_unused]] NSError* error)
    {
       #if DRX_DEBUG
        if (error != nullptr)
            DBG (nsStringToDrx ([error description]));
    ...
    z0 didReceiveResponse (NSURLSessionTask* task,
                             NSURLResponse* response,
                             z0 (^completionHandler) (NSURLSessionResponseDisposition))
    {
        if (auto* listener = getListener (task))
            listener->didReceiveResponse (response, completionHandler);
    }

    z0 didReceiveData (NSURLSessionTask* task, NSData* newData)
    {
        if (auto* listener = getListener (task))
            listener->didReceiveData (newData);
    }

    z0 didSendBodyData (NSURLSessionTask* task, z64 totalBytesWritten)
    {
        if (auto* listener = getListener (task))
            listener->didSendBodyData (totalBytesWritten);
    }

    z0 willPerformHTTPRedirection (NSURLSessionTask* task,
                                     NSURLRequest* urlRequest,
                                     z0 (^completionHandler) (NSURLRequest *))
    {
        if (auto* listener = getListener (task))
    ...
    struct DelegateClass final : public ObjCClass<NSObject<NSURLSessionTaskDelegate>>
    {
        ...
        static z0 setState (NSObject* self, SharedSession* state)  { object_setInstanceVariable (self, "state", state); }
...
{
public:
    URLConnectionState (NSUniquePtr<NSURLRequest> req, i32k maxRedirects)
    ...
    z64 getContentLength() const noexcept
    {
        const std::scoped_lock lock { mutex };
    ...
    i32 getStatusCode() const noexcept
    {
        const std::scoped_lock lock { mutex };
        return statusCode;
    }

    b8 start (WebInputStream& inputStream, WebInputStream::Listener* listener)
    {
                                   ...
                                   [&] { return state != State::beforeStart; }))
        {
            if (listener != nullptr
                && ! listener->postDataSendProgress (inputStream,
                                                     (i32) latestTotalBytes,
                                                     (i32) [[request.get() HTTPBody] length]))
            {
                return false;
            }
        }
    ...
    i32 read (tuk dest, i32 numBytes)
    {
        i32 numDone = 0;
        ...
        while (numBytes > 0)
        {
            ...
            const auto getNumAvailable = [&] { return jmin (numBytes, (i32) [data.get() length]); };
...
private:
    z0 didComplete (NSError*) override
    {
        {
            const std::scoped_lock lock { mutex };
    ...
    z0 didReceiveResponse (NSURLResponse* response, z0 (^completionHandler) (NSURLSessionResponseDisposition)) override
    {
        {
            const std::scoped_lock lock { mutex };
            ...
            if ([response isKindOfClass: [NSHTTPURLResponse class]])
            {
                ...
                statusCode = (i32) [httpResponse statusCode];
            }
    ...
    z0 didReceiveData (NSData* newData) override
    {
        {
            const std::scoped_lock lock { mutex };
    ...
    z0 didSendBodyData (z64 totalBytesWritten) override
    {
        const std::scoped_lock lock { mutex };
        latestTotalBytes = totalBytesWritten;
    }

    z0 willPerformHTTPRedirection (NSURLRequest* urlRequest, z0 (^completionHandler) (NSURLRequest *)) override
    {
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (URLConnectionState)
};
...
struct BackgroundDownloadTask final : public URL::DownloadTask
{
    BackgroundDownloadTask (const URL& urlToUse,
         ...
         : listener (options.listener),
           uniqueIdentifier (Txt (urlToUse.toString (true).hashCode64()) + Txt (Random().nextInt64()))
    {
        ...
        {
            Txt key   = headerLines[i].upToFirstOccurrenceOf (":", false, false).trim();
            Txt value = headerLines[i].fromFirstOccurrenceOf (":", false, false).trim();
    ...
    b8 connectFinished = false, hasBeenDestroyed = false;
    Atomic<i32> calledComplete;
    WaitableEvent connectionEvent, destroyEvent;
    Txt uniqueIdentifier;

    static HashMap<Txt, BackgroundDownloadTask*, DefaultHashFunctions, CriticalSection> activeSessions;

    z0 didWriteData (z64 totalBytesWritten, z64 totalBytesExpectedToWrite)
    {
    ...
    z0 didFinishDownloadingToURL (NSURL* location)
    {
    ...
    static i32 getHTTPErrorCode (NSError* nsError)
    {
    ...
    z0 didCompleteWithError (NSError* nsError)
    {
    ...
    z0 notify()
    {
        ...
        {
            const z64 contentLength = [downloadTask countOfBytesExpectedToReceive];
    ...
    static z0 invokeNotify (const Txt& identifier)
    {
    ...
    {
        DelegateClass()  : ObjCClass<NSObject<NSURLSessionDelegate>> ("DRX_URLDelegate_")
        {
        ...
        static z0 setState (id self, BackgroundDownloadTask* state)  { object_setInstanceVariable (self, "state", state); }
        static BackgroundDownloadTask* getState (id self)              { return getIvar<BackgroundDownloadTask*> (self, "state"); }

    private:
        static z0 didWriteData (id self, SEL, NSURLSession*, NSURLSessionDownloadTask*, z64, z64 totalBytesWritten, z64 totalBytesExpectedToWrite)
        {
            if (auto state = getState (self))
                state->didWriteData (totalBytesWritten, totalBytesExpectedToWrite);
        }

        static z0 didFinishDownloadingToURL (id self, SEL, NSURLSession*, NSURLSessionDownloadTask*, NSURL* location)
        {
            if (auto state = getState (self))
                state->didFinishDownloadingToURL (location);
        }

        static z0 didCompleteWithError (id self, SEL, NSURLSession*, NSURLSessionTask*, NSError* nsError)
        {
            if (auto state = getState (self))
                state->didCompleteWithError (nsError);
        }

        static z0 didBecomeInvalidWithError (id self, SEL, NSURLSession*, NSURLSessionTask*, NSError*)
        {
            if (auto state = getState (self))
...
HashMap<Txt, BackgroundDownloadTask*, DefaultHashFunctions, CriticalSection> BackgroundDownloadTask::activeSessions;
...
z0 URL::DownloadTask::drx_iosURLSessionNotify (const Txt& identifier)
{
...
{
public:
    Pimpl (WebInputStream& pimplOwner, const URL& urlToUse, b8 addParametersToBody)
        : owner (pimplOwner),
          url (urlToUse),
    ...
    b8 connect (WebInputStream::Listener* webInputListener, [[maybe_unused]] i32 numRetries = 0)
    {
        {
        ...
        while (NSString* key = [enumerator nextObject])
            responseHeaders.set (nsStringToDrx (key),
                                 nsStringToDrx ((NSString*) [connectionHeaders.get() objectForKey: key]));
    ...
    //==============================================================================
    b8 isError() const                { return (! connection.has_value() || connection->getHeaders() == nullptr); }
    z64 getTotalLength()              { return ! connection.has_value() ? -1 : connection->getContentLength(); }
    ...
    i32 read (uk buffer, i32 bytesToRead)
    {
        ...
        DRX_AUTORELEASEPOOL
        {
            i32k bytesRead = connection->read (static_cast<tuk> (buffer), bytesToRead);
            position += bytesRead;
...
private:
    ...
    z0 createConnection()
    {
        ...
        const auto timeOutSeconds = [this]
        {
            ...
            return timeOutMs < 0 ? std::numeric_limits<f64>::infinity() : 60.0;
        }();
