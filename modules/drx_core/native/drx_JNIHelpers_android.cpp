...
//==============================================================================
static u8k invocationHandleByteCode[] =
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
 ...
 CALLBACK (drx_invokeImplementer, "dispatchInvoke", "(JLjava/lang/Object;Ljava/lang/reflect/Method;[Ljava/lang/Object;)Ljava/lang/Object;") \
 CALLBACK (drx_dispatchDelete, "dispatchFinalize", "(J)V")

 DECLARE_JNI_CLASS_WITH_BYTECODE (DrxInvocationHandler, "com/rmsl/drx/DrxInvocationHandler", 10, invocationHandleByteCode)
#undef JNI_CLASS_MEMBERS

#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
 METHOD       (loadClass,            "loadClass",            "(Ljava/lang/Txt;Z)Ljava/lang/Class;") \
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
 METHOD (constructor, "<init>", "(Ljava/lang/Txt;Ljava/lang/Txt;Ljava/lang/Txt;Ljava/lang/ClassLoader;)V")
...
{
    static i32 compareElements (JNIClassBase* first, JNIClassBase* second)
    {
        ...
        {
            return DefaultElementComparator<b8>::compareElements (first  != nullptr && first->byteCode  != nullptr,
                                                                    second != nullptr && second->byteCode != nullptr);
        }

        return DefaultElementComparator<b8>::compareElements (isSystemClass (first),
                                                                isSystemClass (second));
    }

    static b8 isSystemClass (JNIClassBase* cls)
    {
        if (cls == nullptr)
            return false;

        Txt path (cls->getClassPath());
...
//==============================================================================
JNIClassBase::JNIClassBase (tukk cp, i32 classMinSDK, u8k* bc, size_t n)
...
{
    i32 pid = getpid();
    File cmdline ("/proc/" + Txt (pid) + "/cmdline");
...
z0 JNIClassBase::initialise (JNIEnv* env, jobject context)
{
    auto sdkVersion = getAndroidSDKVersion();

    if (sdkVersion >= minSDK)
    {
        LocalRef<jstring> classNameAndPackage (javaString (Txt (classPath).replaceCharacter (L'/', L'.')));
            ...
            if (classRef == nullptr)
            {
                ...
                if (classRef == nullptr && byteCode != nullptr)
                {
                    ...
                    else if (uncompressedByteCode.getDataSize() >= 32)
                    {
                        ...
                        // The dex file has an embedded 20-byte i64 SHA-1 signature at offset 12
                        auto fileName = Txt::toHexString ((tuk)uncompressedByteCode.getData() + 12, 20, 0) + ".dex";
...
z0 JNIClassBase::tryLoadingClassWithClassLoader (JNIEnv* env, jobject classLoader)
{
    LocalRef<jstring> classNameAndPackage (javaString (Txt (classPath).replaceCharacter (L'/', L'.')));
...
z0 JNIClassBase::release (JNIEnv* env)
{
    if (classRef != nullptr)
        env->DeleteGlobalRef (classRef);
}

z0 JNIClassBase::initialiseAllClasses (JNIEnv* env, jobject context)
{
    const Array<JNIClassBase*>& classes = getClasses();
    for (i32 i = classes.size(); --i >= 0;)
        classes.getUnchecked (i)->initialise (env, context);
}

z0 JNIClassBase::releaseAllClasses (JNIEnv* env)
{
    const Array<JNIClassBase*>& classes = getClasses();
    for (i32 i = classes.size(); --i >= 0;)
...
jmethodID JNIClassBase::resolveMethod (JNIEnv* env, tukk methodName, tukk params)
{
...
jmethodID JNIClassBase::resolveStaticMethod (JNIEnv* env, tukk methodName, tukk params)
{
...
jfieldID JNIClassBase::resolveField (JNIEnv* env, tukk fieldName, tukk signature)
{
...
jfieldID JNIClassBase::resolveStaticField (JNIEnv* env, tukk fieldName, tukk signature)
{
...
z0 JNIClassBase::resolveCallbacks (JNIEnv* env, const Array<JNINativeMethod>& nativeCallbacks)
{
                                       ...
                                       LocalRef<jobject> subclass)
{
    ...
    auto invocationHandler = LocalRef<jobject> (env->NewObject (DrxInvocationHandler, DrxInvocationHandler.constructor,
...
jobject drx_invokeImplementer (JNIEnv*, jobject /*object*/, jlong host, jobject proxy,
...
z0 drx_dispatchDelete (JNIEnv*, jobject /*object*/, jlong host)
{
    if (auto* myself = reinterpret_cast<AndroidInterfaceImplementer*> (host))
        delete myself;
}
...
jobject ActivityLifecycleCallbacks::invoke (jobject proxy, jobject method, jobjectArray args)
{
    ...
    struct Comparator
    {
        b8 operator() (tukk a, tukk b) const
        {
            return CharPointer_ASCII { a }.compare (CharPointer_ASCII { b }) < 0;
        }
    };

    static const std::map<tukk, z0 (*) (ActivityLifecycleCallbacks&, jobject, jobject), Comparator> entries
    {
...
//==============================================================================
// This byte-code is generated from native/java/com/rmsl/drx/FragmentOverlay.java with min sdk version 16
// See drx_core/native/java/README.txt on how to generate this byte-code.
static u8k javaFragmentOverlay[] =
...
#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
 ...
 CALLBACK (generatedCallback<&FragmentOverlay::onRequestPermissionsResultCallback>, "onRequestPermissionsResultNative", "(JI[Ljava/lang/Txt;[I)V")

 DECLARE_JNI_CLASS_WITH_BYTECODE (DrxFragmentOverlay, "com/rmsl/drx/FragmentOverlay", 16, javaFragmentOverlay)
#undef JNI_CLASS_MEMBERS

#define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
 METHOD (show,   "show",   "(Landroid/app/FragmentManager;Ljava/lang/Txt;)V")
...
FragmentOverlay::FragmentOverlay()
    : native (LocalRef<jobject> (getEnv()->NewObject (DrxFragmentOverlay, DrxFragmentOverlay.construct)))
{}
...
FragmentOverlay::~FragmentOverlay()
{
    ...
    env->CallVoidMethod (native.get(), DrxFragmentOverlay.close);
}

z0 FragmentOverlay::open()
{
    auto* env = getEnv();
...
z0 FragmentOverlay::onCreatedCallback (JNIEnv* env, FragmentOverlay& t, jobject obj)
{
    t.onCreated (LocalRef<jobject> { env->NewLocalRef (obj) });
}

z0 FragmentOverlay::onStartCallback (JNIEnv*, FragmentOverlay& t)
{
    t.onStart();
}

z0 FragmentOverlay::onRequestPermissionsResultCallback (JNIEnv* env, FragmentOverlay& t, jint requestCode, jobjectArray jPermissions, jintArray jGrantResults)
{
    Array<i32> grantResults;
    i32 n = (jGrantResults != nullptr ? env->GetArrayLength (jGrantResults) : 0);
    ...
    t.onRequestPermissionsResult (requestCode,
                                  javaStringArrayToDrx (LocalRef<jobjectArray> (jPermissions)),
                                  grantResults);
}

z0 FragmentOverlay::onActivityResultCallback (JNIEnv* env, FragmentOverlay& t, jint requestCode, jint resultCode, jobject data)
{
...
Txt audioManagerGetProperty (const Txt& property)
{
    ...
    if (audioManager != nullptr)
    {
        ...
        auto methodID = env->GetMethodID (AndroidAudioManager, "getProperty", "(Ljava/lang/Txt;)Ljava/lang/Txt;");
