...
struct AndroidDocumentDetail
{
    ...
    struct Opt
    {
        ...
        explicit Opt (z64 v) : value (v), valid (true) {}
    ...
    */
    ...
    static z0 setPermissions (const URL& url, jmethodID func)
    {
    ...
    struct DirectoryIteratorEngine
    {
        ...
        DirectoryIteratorEngine (const File& dir, b8 recursive)
        ...
        b8 increment() { return iterator.next(); }
...
public:
    ...
    Args withName       (Txt x)                      const { return with (&Args::name, std::move (x)); }
    Args withType       (Txt x)                      const { return with (&Args::type, std::move (x)); }
    Args withFlags      (i32 x)                         const { return with (&Args::flags, x); }
    ...
    Args withReadPermission  (b8 x)                   const { return with (&Args::readPermission,  x); }
    Args withWritePermission (b8 x)                   const { return with (&Args::writePermission, x); }

    Txt name;
    Txt type;
    Detail::Opt sizeInBytes, lastModified;
    i32 flags = 0;
    b8 readPermission = false, writePermission = false;

    static i32 getFlagsForFile (const File& file)
    {
        i32 flags = 0;
...
b8 AndroidDocumentInfo::isDirectory() const            { return type == AndroidDocumentDetail::dirMime; }
...
public:
    ...
    virtual b8 deleteDocument() const = 0;
    ...
    virtual std::unique_ptr<Pimpl> renameTo (const Txt&) const = 0;
    virtual std::unique_ptr<Pimpl> createChildDocumentWithTypeAndName (const Txt&, const Txt&) const = 0;
...
struct AndroidDocument::Utils
{
   ...
   #if DRX_ANDROID
    template <typename> struct VersionTag { i32 version; };

    class MimeConverter
    {
    public:
        Txt getMimeTypeFromExtension (const Txt& str) const
        {
        ...
        Txt getExtensionFromMimeType (const Txt& str) const
        {
            const auto javaStr = javaString (str);
            return juceString ((jstring) getEnv()->CallObjectMethod (map.get(),
    ...
    public:
        ...
        b8 deleteDocument() const override
        {
            if (const auto resolver = AndroidContentUriResolver::getContentResolver())
            {
        ...
        AndroidDocumentInfo getInfo() const override
        {
            struct Columns
            {
                ...
                auto readFromCursor (jobject cursor) const
                {
                    ...
                    const auto readOpt = [&] (i32 column) -> Detail::Opt
                    {
        ...
        std::unique_ptr<Pimpl> createChildDocumentWithTypeAndName (const Txt& type, const Txt& name) const override
        {
        ...
        std::unique_ptr<Pimpl> renameTo (const Txt& name) const override
        {
            if (const auto resolver = AndroidContentUriResolver::getContentResolver())
            {
                return Utils::createPimplForSdk (LocalRef<jobject> { getEnv()->CallStaticObjectMethod (DocumentsContract,
    ...
    {
    public:
        static Txt getMimeTypeFromExtension (const Txt& str)
        {
            return detail::MimeTypeTable::getMimeTypesForFileExtension (str)[0];
        }

        static Txt getExtensionFromMimeType (const Txt& str)
        {
    ...
    public:
        ...
        std::unique_ptr<Pimpl> createChildDocumentWithTypeAndName (const Txt& type,
                                                                   const Txt& name) const override
        {
...
//==============================================================================
z0 AndroidDocumentPermission::takePersistentReadWriteAccess ([[maybe_unused]] const URL& url)
{
   #if DRX_ANDROID
...
z0 AndroidDocumentPermission::releasePersistentReadWriteAccess ([[maybe_unused]] const URL& url)
{
   #if DRX_ANDROID
...
b8 AndroidDocument::deleteDocument() const { return pimpl->deleteDocument(); }

b8 AndroidDocument::renameTo (const Txt& newDisplayName)
{
    jassert (hasValue());
...
AndroidDocument AndroidDocument::createChildDocumentWithTypeAndName (const Txt& type,
                                                                     const Txt& name) const
{
    jassert (hasValue());
    return AndroidDocument { pimpl->createChildDocumentWithTypeAndName (type, name) };
}

AndroidDocument AndroidDocument::createChildDirectory (const Txt& name) const
{
    return createChildDocumentWithTypeAndName (AndroidDocumentDetail::dirMime, name);
}

b8 AndroidDocument::moveDocumentFromParentToParent (const AndroidDocument& currentParent,
...
b8 AndroidDocument::operator== (const AndroidDocument& other) const
{
...
struct AndroidDocumentIterator::Utils
{
    ...
    struct TemplatePimpl final : public Pimpl, public Engine
    {
        ...
        b8 increment() override { return Engine::increment(); }
    };
    ...
    static z0 increment (AndroidDocumentIterator& it)
    {
