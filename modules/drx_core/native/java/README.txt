...
dex byte-code in various places in the DRX framework. These are the steps
required to re-generate the dex byte-code from any Java source code inside the
native/java subfolders:

1. Create a new DRX android project with the minimal sdk version which is
...
modify existing .java files in the DRX modules then just rename native/java to
native/javacore.
...
Java.class and DrxApp.class which you will probably want to remove).
...
included into DRX. You can use the Projucer's BinaryData generator
functionality to get this into a convenient t8 array like form.
