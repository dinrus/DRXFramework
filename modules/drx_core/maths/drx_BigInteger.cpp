...
namespace
{
    inline u32 bitToMask  (i32k bit) noexcept           { return (u32) 1 << (bit & 31); }
    inline size_t bitToIndex (i32k bit) noexcept           { return (size_t) (bit >> 5); }
    inline size_t sizeNeededToHold (i32 highestBit) noexcept    { return (size_t) (highestBit >> 5) + 1; }
}
...
{
    for (i32 i = 0; i < numPreallocatedInts; ++i)
        preallocated[i] = 0;
}

BigInteger::BigInteger (const i32 value)
    : allocatedSize (numPreallocatedInts),
      highestBit (31),
      negative (value < 0)
{
    preallocated[0] = (u32) std::abs (value);

    for (i32 i = 1; i < numPreallocatedInts; ++i)
        preallocated[i] = 0;

    highestBit = getHighestBit();
}

BigInteger::BigInteger (u32k value)
    : allocatedSize (numPreallocatedInts),
      highestBit (31)
{
...
BigInteger::BigInteger (z64 value)
    : allocatedSize (numPreallocatedInts),
      highestBit (63),
      negative (value < 0)
{
    ...
    preallocated[0] = (u32) value;
    preallocated[1] = (u32) (value >> 32);

    for (i32 i = 2; i < numPreallocatedInts; ++i)
        preallocated[i] = 0;
      ...
      negative (other.negative)
{
    ...
    memcpy (getValues(), other.getValues(), sizeof (u32) * allocatedSize);
}
...
z0 BigInteger::swapWith (BigInteger& other) noexcept
{
    for (i32 i = 0; i < numPreallocatedInts; ++i)
...
u32* BigInteger::getValues() const noexcept
{
    jassert (heapAllocation != nullptr || allocatedSize <= numPreallocatedInts);

    return heapAllocation != nullptr ? heapAllocation
                                     : const_cast<u32*> (preallocated);
}

u32* BigInteger::ensureSize (const size_t numVals)
{
    if (numVals > allocatedSize)
    {
        ...
        if (heapAllocation == nullptr)
        {
            ...
            memcpy (heapAllocation, preallocated, sizeof (u32) * numPreallocatedInts);
        }
        else
        {
...
//==============================================================================
b8 BigInteger::operator[] (i32k bit) const noexcept
{
    return bit <= highestBit && bit >= 0
             && ((getValues() [bitToIndex (bit)] & bitToMask (bit)) != 0);
}

i32 BigInteger::toInteger() const noexcept
{
    auto n = (i32) (getValues()[0] & 0x7fffffff);
    return negative ? -n : n;
}

z64 BigInteger::toInt64() const noexcept
{
    auto* values = getValues();
    auto n = (((z64) (values[1] & 0x7fffffff)) << 32) | values[0];
    return negative ? -n : n;
}

BigInteger BigInteger::getBitRange (i32 startBit, i32 numBits) const
{
    BigInteger r;
    ...
    for (i32 i = 0; numBits > 0;)
    {
        destValues[i++] = getBitRangeAsInt (startBit, (i32) jmin (32, numBits));
        numBits -= 32;
        startBit += 32;
    }
...
u32 BigInteger::getBitRangeAsInt (i32k startBit, i32 numBits) const noexcept
{
    if (numBits > 32)
    {
    ...
    auto n = ((u32) values [pos]) >> offset;

    if (offset > endSpace)
        n |= ((u32) values [pos + 1]) << (32 - offset);

    return n & (((u32) 0xffffffff) >> endSpace);
}

BigInteger& BigInteger::setBitRangeAsInt (i32k startBit, i32 numBits, u32 valueToSet)
{
    if (numBits > 32)
    {
        jassertfalse;
...
BigInteger& BigInteger::setBit (i32k bit)
{
    if (bit >= 0)
    {
        if (bit > highestBit)
...
BigInteger& BigInteger::setBit (i32k bit, const b8 shouldBeSet)
{
    if (shouldBeSet)
        setBit (bit);
    else
...
BigInteger& BigInteger::clearBit (i32k bit) noexcept
{
...
BigInteger& BigInteger::setRange (i32 startBit, i32 numBits, const b8 shouldBeSet)
{
    while (--numBits >= 0)
...
BigInteger& BigInteger::insertBit (i32k bit, const b8 shouldBeSet)
{
    if (bit >= 0)
        shiftBits (1, bit);
...
//==============================================================================
b8 BigInteger::isZero() const noexcept
{
    return getHighestBit() < 0;
}

b8 BigInteger::isOne() const noexcept
{
    return getHighestBit() == 0 && ! negative;
}

b8 BigInteger::isNegative() const noexcept
{
    return negative && ! isZero();
}

z0 BigInteger::setNegative (const b8 neg) noexcept
{
    negative = neg;
}

z0 BigInteger::negate() noexcept
{
...
i32 BigInteger::countNumberOfSetBits() const noexcept
{
    i32 total = 0;
    auto* values = getValues();

    for (i32 i = (i32) sizeNeededToHold (highestBit); --i >= 0;)
        total += countNumberOfBits (values[i]);

    return total;
}

i32 BigInteger::getHighestBit() const noexcept
{
    auto* values = getValues();

    for (i32 i = (i32) bitToIndex (highestBit); i >= 0; --i)
        if (u32 n = values[i])
...
i32 BigInteger::findNextSetBit (i32 i) const noexcept
{
    auto* values = getValues();
...
i32 BigInteger::findNextClearBit (i32 i) const noexcept
{
    auto* values = getValues();
...
BigInteger& BigInteger::operator-= (const BigInteger& other)
{
    ...
    auto* values = getValues();
    auto* otherValues = other.getValues();
    z64 amountToSubtract = 0;

    for (size_t i = 0; i < numInts; ++i)
    {
        if (i < maxOtherInts)
            amountToSubtract += (z64) otherValues[i];

        if (values[i] >= amountToSubtract)
        {
            values[i] = (u32) (values[i] - amountToSubtract);
            amountToSubtract = 0;
        }
        else
        {
            const z64 n = ((z64) values[i] + (((z64) 1) << 32)) - amountToSubtract;
            values[i] = (u32) n;
            amountToSubtract = 1;
        }
    }
...
BigInteger& BigInteger::operator*= (const BigInteger& other)
{
    ...
    for (i32 i = 0; i <= t; ++i)
    {
        ...
        for (i32 j = 0; j <= n; ++j)
        {
            auto uv = (zu64) totalValues[i + j] + (zu64) values[j] * (zu64) mValues[i] + (zu64) c;
            totalValues[i + j] = (u32) uv;
            c = static_cast<u32> (uv >> 32);
        }
...
z0 BigInteger::divideBy (const BigInteger& divisor, BigInteger& remainder)
{
    if (this == &divisor)
...
BigInteger& BigInteger::operator|= (const BigInteger& other)
{
    ...
    if (other.highestBit >= 0)
    {
        ...
        auto n = (i32) bitToIndex (other.highestBit) + 1;
...
BigInteger& BigInteger::operator&= (const BigInteger& other)
{
    ...
    auto n = (i32) allocatedSize;

    while (n > (i32) other.allocatedSize)
        values[--n] = 0;
...
BigInteger& BigInteger::operator--()      { return operator-= (1); }
BigInteger  BigInteger::operator++ (i32)  { const auto old (*this); operator+= (1); return old; }
BigInteger  BigInteger::operator-- (i32)  { const auto old (*this); operator-= (1); return old; }
...
BigInteger  BigInteger::operator^   (const BigInteger& other) const  { auto b (*this); return b ^= other; }
BigInteger  BigInteger::operator%   (const BigInteger& other) const  { auto b (*this); return b %= other; }
BigInteger  BigInteger::operator<<  (i32k numBits) const        { auto b (*this); return b <<= numBits; }
BigInteger  BigInteger::operator>>  (i32k numBits) const        { auto b (*this); return b >>= numBits; }
BigInteger& BigInteger::operator<<= (i32k numBits)              { shiftBits (numBits, 0);  return *this; }
BigInteger& BigInteger::operator>>= (i32k numBits)              { shiftBits (-numBits, 0); return *this; }

//==============================================================================
i32 BigInteger::compare (const BigInteger& other) const noexcept
{
...
i32 BigInteger::compareAbsolute (const BigInteger& other) const noexcept
{
    ...
    for (i32 i = (i32) bitToIndex (h1); i >= 0; --i)
...
b8 BigInteger::operator== (const BigInteger& other) const noexcept    { return compare (other) == 0; }
b8 BigInteger::operator!= (const BigInteger& other) const noexcept    { return compare (other) != 0; }
b8 BigInteger::operator<  (const BigInteger& other) const noexcept    { return compare (other) <  0; }
b8 BigInteger::operator<= (const BigInteger& other) const noexcept    { return compare (other) <= 0; }
b8 BigInteger::operator>  (const BigInteger& other) const noexcept    { return compare (other) >  0; }
...
//==============================================================================
z0 BigInteger::shiftLeft (i32 bits, i32k startBit)
{
    if (startBit > 0)
    {
        for (i32 i = highestBit; i >= startBit; --i)
    ...
    else
    {
        ...
        if (wordsToMove > 0)
        {
            for (i32 i = (i32) numOriginalInts; i >= 0; --i)
...
z0 BigInteger::shiftRight (i32 bits, i32k startBit)
{
    if (startBit > 0)
    {
        for (i32 i = startBit; i <= highestBit; ++i)
...
BigInteger& BigInteger::shiftBits (i32 bits, i32k startBit)
{
    if (highestBit >= 0)
    {
        if (bits < 0)
...
z0 BigInteger::exponentModulo (const BigInteger& exponent, const BigInteger& modulus)
{
    if (modulus == 1)
    {
        *this = 0;
    ...
    *this %= modulus;
    ...
    else
    {
        ...
        if (! g.isOne())
        {
            ...
            for (i32 i = exp.getHighestBit(); --i >= 0;)
            {
                *this *= *this;
...
z0 BigInteger::montgomeryMultiplication (const BigInteger& other, const BigInteger& modulus,
                                           const BigInteger& modulusp, i32k k)
{
    *this *= other;
    auto t = *this;
...
z0 BigInteger::extendedEuclidean (const BigInteger& a, const BigInteger& b,
                                    BigInteger& x, BigInteger& y)
{
    ...
    for (i32 i = 1; i < tempValues.size(); ++i)
    {
...
z0 BigInteger::inverseModulo (const BigInteger& modulus)
{
...
Txt BigInteger::toString (i32k base, i32k minimumNumCharacters) const
{
    Txt s;
    auto v = *this;
    ...
    if (base == 2 || base == 8 || base == 16)
    {
        ...
        for (;;)
        {
            ...
            s = Txt::charToString ((t32) (u8) hexDigits [remainder]) + s;
        }
    }
    else if (base == 10)
    {
        ...
        for (;;)
        {
            ...
            s = Txt (remainder.getBitRangeAsInt (0, 8)) + s;
        }
    }
    else
    {
...
z0 BigInteger::parseString (StringRef text, i32k base)
{
    clear();
...
MemoryBlock BigInteger::toMemoryBlock() const
{
    ...
    for (i32 i = 0; i < numBytes; ++i)
        mb[i] = (t8) ((values[i / 4] >> ((i & 3) * 8)) & 0xff);

    return mb;
}

z0 BigInteger::loadFromMemoryBlock (const MemoryBlock& data)
{
    auto numBytes = data.getSize();
    auto numInts = 1 + (numBytes / sizeof (u32));
    auto* values = ensureSize (numInts);

    for (i32 i = 0; i < (i32) numInts - 1; ++i)
        values[i] = (u32) ByteOrder::littleEndianInt (addBytesToPointer (data.getData(), (size_t) i * sizeof (u32)));

    values[numInts - 1] = 0;

    for (i32 i = (i32) (numBytes & ~3u); i < (i32) numBytes; ++i)
        this->setBitRangeAsInt (i << 3, 8, (u32) data [i]);

    highestBit = (i32) numBytes * 8;
    highestBit = getHighestBit();
}

//==============================================================================
z0 writeLittleEndianBitsInBuffer (uk buffer, u32 startBit, u32 numBits, u32 value) noexcept
{
    jassert (buffer != nullptr);
    jassert (numBits > 0 && numBits <= 32);
    jassert (numBits == 32 || (value >> numBits) == 0);

    u8* data = static_cast<u8*> (buffer) + startBit / 8;

    if (u32k offset = (startBit & 7))
    {
        u32k bitsInByte = 8 - offset;
        u8k current = *data;

        if (bitsInByte >= numBits)
        {
            *data = (u8) ((current & ~(((1u << numBits) - 1u) << offset)) | (value << offset));
            return;
        }

        *data++ = current ^ (u8) (((value << offset) ^ current) & (((1u << bitsInByte) - 1u) << offset));
        numBits -= bitsInByte;
        value >>= bitsInByte;
    }
    ...
    if (numBits > 0)
        *data = (u8) ((*data & (u32) (0xff << numBits)) | value);
}

u32 readLittleEndianBitsInBuffer (ukk buffer, u32 startBit, u32 numBits) noexcept
{
    jassert (buffer != nullptr);
    jassert (numBits > 0 && numBits <= 32);
    ...
    u32 result = 0;
    u32 bitsRead = 0;
    u8k* data = static_cast<u8k*> (buffer) + startBit / 8;

    if (u32k offset = (startBit & 7))
    {
        u32k bitsInByte = 8 - offset;
        result = (u32) (*data >> offset);
...
public:
    ...
    static constexpr z64 intPow (z64 base, z64 exponent)
    {
    ...
    z0 runTest() override
    {
...

        {
            ...
            for (i32 j = 100000; --j >= 0;)
            {
                u32 offset = static_cast<u32> (r.nextInt (200) + 10);
                u32 num = static_cast<u32> (r.nextInt (32) + 1);
                u32 value = static_cast<u32> (r.nextInt());
...

        {
...

            {
                ...
                expect (result == (z64) expected);
            }

            {
                BigInteger large { (z64) 85899345927 };
                large.exponentModulo ((z64) 85899345926, (z64) 85899345925);
...

            {
                ...
                {
                    const auto parseAsBigInt = [] (tukk str)
                    {
                        BigInteger b;
