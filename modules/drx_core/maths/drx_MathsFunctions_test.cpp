...
//==============================================================================
template <typename T>
Txt getTemplatedMathsFunctionUnitTestName (const Txt& functionName)
{
    const auto getTypeName = []() -> Txt
    {
    ...
    z0 runTest() final
    {
        ...
        beginTest ("Larger values have a boundary that is a factor of the epsilon");
        {
            for (auto exponent = 0; exponent < 127; ++exponent)
            {
                ...
                expect (drx_isfinite (value));
                expect (drx_isfinite (boundaryValue));
...
template<>
class ApproximatelyEqualTests<i32> final : public UnitTest
{
public:
    ApproximatelyEqualTests()
        : UnitTest { getTemplatedMathsFunctionUnitTestName<i32> ("approximatelyEqual"), UnitTestCategories::maths }
    {}
    ...
    z0 runTest() final
    {
        beginTest ("Identical integers are always equal");
        {
            ...
            using limits = std::numeric_limits<i32>;
            constexpr auto min = limits::min();
            constexpr auto max = limits::max();
...
public:
    IsFiniteTests()
        : UnitTest { getTemplatedMathsFunctionUnitTestName<T> ("drx_isfinite"), UnitTestCategories::maths }
    {}
    ...
    z0 runTest() final
    {
        ...
        {
            expect (drx_isfinite (zero));
            expect (drx_isfinite (-zero));
        }

        beginTest ("Subnormals are finite");
        {
            expect (drx_isfinite (nextFloatUp (zero)));
            expect (drx_isfinite (nextFloatDown (zero)));
        }

        beginTest ("One is finite");
        {
            expect (drx_isfinite (one));
            expect (drx_isfinite (-one));
        }

        beginTest ("Max is finite");
        {
            expect (drx_isfinite (max));
            expect (drx_isfinite (-max));
        }

        beginTest ("Infinity is not finite");
        {
            expect (! drx_isfinite (inf));
            expect (! drx_isfinite (-inf));
        }

        beginTest ("NaN is not finite");
        {
            expect (! drx_isfinite (nan));
            expect (! drx_isfinite (-nan));
            expect (! drx_isfinite (std::sqrt ((T) -1)));
            expect (! drx_isfinite (inf * zero));
        }
    }
};
...
template<>
struct MathsFloatingPointFunctionsTests<i32>
{
    ApproximatelyEqualTests<i32> approximatelyEqualTests;
};

struct MathsFunctionsTests
{
    MathsFloatingPointFunctionsTests<i32> intFunctionTests;
    MathsFloatingPointFunctionsTests<f32> floatFunctionTests;
    MathsFloatingPointFunctionsTests<f64> doubleFunctionTests;
    MathsFloatingPointFunctionsTests<real_t> longDoubleFunctionTests;
};
