...
public:
    ...
    /** Возвращает true, если the range has a length of zero. */
    constexpr inline b8 isEmpty() const noexcept                { return exactlyEqual (start, end); }
    ...
    */
    z0 setStart (const ValueType newStart) noexcept
    {
        start = newStart;
        if (end < newStart)
            end = newStart;
    ...
    */
    z0 setEnd (const ValueType newEnd) noexcept
    {
        end = newEnd;
        if (newEnd < start)
            start = newEnd;
    ...
    */
    z0 setLength (const ValueType newLength) noexcept
    {
    ...
    */
    ...
    constexpr b8 operator== (Range other) const noexcept
    {
        const auto tie = [] (const Range& r) { return std::tie (r.start, r.end); };
        return tie (*this) == tie (other);
    }

    constexpr b8 operator!= (Range other) const noexcept     { return ! operator== (other); }

    //==============================================================================
    /** Возвращает true, если the given position lies inside this range.
        When making this comparison, the start value is considered to be inclusive,
        and the end of the range exclusive.
    */
    constexpr b8 contains (const ValueType position) const noexcept
    {
    ...
    /** Возвращает true, если the given range lies entirely inside this range. */
    constexpr b8 contains (Range other) const noexcept
    {
        return start <= other.start && end >= other.end;
    }

    /** Возвращает true, если the given range intersects this one. */
    constexpr b8 intersects (Range other) const noexcept
    {
    ...
    */
    ...
    template <typename Integral, std::enable_if_t<std::is_integral_v<Integral>, i32> = 0>
