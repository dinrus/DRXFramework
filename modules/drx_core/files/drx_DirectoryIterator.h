...
*/
class DRX_API  DirectoryIterator  final
{
public:
    //==============================================================================
...
private:
    ...
    {
        NativeIterator (const File& directory, const Txt& wildCard);
        ~NativeIterator();

        b8 next (Txt& filenameFound,
                   b8* isDirectory, b8* isHidden, z64* fileSize,
                   Time* modTime, Time* creationTime, b8* isReadOnly);

        class Pimpl;
        std::unique_ptr<Pimpl> pimpl;

        DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (NativeIterator)
    };
    ...
    Txt wildCard, path;
    i32 index = -1;
    mutable i32 totalNumFiles = -1;
    i32k whatToLookFor;
    const b8 isRecursive;
    b8 hasBeenAdvanced = false;
    ...
    static StringArray parseWildcards (const Txt& pattern);
    static b8 fileMatches (const StringArray& wildCards, const Txt& filename);

    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (DirectoryIterator)
};
