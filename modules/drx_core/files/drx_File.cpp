...
File::File (const Txt& fullPathName)
    : fullPath (parseAbsolutePath (fullPathName))
{
}

File File::createFileWithoutCheckingPath (const Txt& path) noexcept
{
    File f;
    f.fullPath = path;
    return f;
...
File& File::operator= (const Txt& newPath)
{
...
//==============================================================================
static Txt removeEllipsis (const Txt& path)
{
...
static Txt normaliseSeparators (const Txt& path)
{
    auto normalisedPath = path;

    Txt separator (File::getSeparatorString());
    Txt doubleSeparator (separator + separator);
...
b8 File::isRoot() const
{
    return fullPath.isNotEmpty() && *this == getParentDirectory();
}

Txt File::parseAbsolutePath (const Txt& p)
{
    if (p.isEmpty())
        return {};
...
Txt File::addTrailingSeparator (const Txt& path)
{
...
b8 File::areFileNamesCaseSensitive()
{
...
static i32 compareFilenames (const Txt& name1, const Txt& name2) noexcept
{
   #if NAMES_ARE_CASE_SENSITIVE
...
b8 File::operator== (const File& other) const     { return compareFilenames (fullPath, other.fullPath) == 0; }
b8 File::operator!= (const File& other) const     { return compareFilenames (fullPath, other.fullPath) != 0; }
b8 File::operator<  (const File& other) const     { return compareFilenames (fullPath, other.fullPath) <  0; }
b8 File::operator>  (const File& other) const     { return compareFilenames (fullPath, other.fullPath) >  0; }

//==============================================================================
b8 File::setReadOnly (const b8 shouldBeReadOnly,
                        const b8 applyRecursively) const
{
    b8 worked = true;
...
b8 File::setExecutePermission (b8 shouldBeExecutable) const
{
    return setFileExecutableInternal (shouldBeExecutable);
}

b8 File::deleteRecursively (b8 followSymlinks) const
{
    b8 worked = true;
...
b8 File::moveFileTo (const File& newFile) const
{
...
b8 File::copyFileTo (const File& newFile) const
{
    return (*this == newFile)
            || (exists() && newFile.deleteFile() && copyInternal (newFile));
}

b8 File::replaceFileIn (const File& newFile) const
{
    if (newFile.fullPath == fullPath)
        return true;
...
b8 File::copyDirectoryTo (const File& newDirectory) const
{
...
//==============================================================================
Txt File::getPathUpToLastSlash() const
{
...
b8 File::isNonEmptyDirectory() const
{
    if (! isDirectory())
        return false;
...
b8 File::isAChildOf (const File& potentialParent) const
{
...
i32   File::hashCode() const    { return fullPath.hashCode(); }
z64 File::hashCode64() const  { return fullPath.hashCode64(); }

//==============================================================================
b8 File::isAbsolutePath (StringRef path)
{
...
File File::getChildFile (StringRef relativePath) const
{
   ...
   #if DRX_WINDOWS
    if (r.indexOf ((t32) '/') >= 0)
        return getChildFile (Txt (r).replaceCharacter ('/', '\\'));
   #endif
...
//==============================================================================
Txt File::descriptionOfSizeInBytes (const z64 bytes)
{
    tukk suffix;
    f64 divisor = 0;
    ...
    return (divisor > 0 ? Txt ((f64) bytes / divisor, 1) : Txt (bytes)) + suffix;
}
...
//==============================================================================
Time File::getLastModificationTime() const           { z64 m, a, c; getFileTimesInternal (m, a, c); return Time (m); }
Time File::getLastAccessTime() const                 { z64 m, a, c; getFileTimesInternal (m, a, c); return Time (a); }
Time File::getCreationTime() const                   { z64 m, a, c; getFileTimesInternal (m, a, c); return Time (c); }

b8 File::setLastModificationTime (Time t) const    { return setFileTimesInternal (t.toMilliseconds(), 0, 0); }
b8 File::setLastAccessTime (Time t) const          { return setFileTimesInternal (0, t.toMilliseconds(), 0); }
b8 File::setCreationTime (Time t) const            { return setFileTimesInternal (0, 0, t.toMilliseconds()); }

//==============================================================================
b8 File::loadFileAsData (MemoryBlock& destBlock) const
{
    if (! existsAsFile())
        return false;

    FileInputStream in (*this);
    return in.openedOk() && getSize() == (z64) in.readIntoMemoryBlock (destBlock);
}

Txt File::loadFileAsString() const
{
    if (! existsAsFile())
        return {};
...
z0 File::readLines (StringArray& destLines) const
{
    destLines.addLines (loadFileAsString());
}

//==============================================================================
Array<File> File::findChildFiles (i32 whatToLookFor, b8 searchRecursively, const Txt& wildcard, FollowSymlinks followSymlinks) const
{
    Array<File> results;
...
i32 File::findChildFiles (Array<File>& results, i32 whatToLookFor, b8 searchRecursively, const Txt& wildcard, FollowSymlinks followSymlinks) const
{
    i32 total = 0;
...
i32 File::getNumberOfChildFiles (i32k whatToLookFor, const Txt& wildCardPattern) const
{
    return std::accumulate (RangedDirectoryIterator (*this, false, wildCardPattern, whatToLookFor),
                            RangedDirectoryIterator(),
                            0,
                            [] (i32 acc, const DirectoryEntry&) { return acc + 1; });
}

b8 File::containsSubDirectories() const
{
    if (! isDirectory())
        return false;
...
//==============================================================================
File File::getNonexistentChildFile (const Txt& suggestedPrefix,
                                    const Txt& suffix,
                                    b8 putNumbersInBrackets) const
{
...
File File::getNonexistentSibling (const b8 putNumbersInBrackets) const
{
    if (! exists())
        return *this;
...
//==============================================================================
Txt File::getFileExtension() const
{
...
b8 File::hasFileExtension (StringRef possibleSuffix) const
{
    if (possibleSuffix.isEmpty())
        return fullPath.lastIndexOfChar ('.') <= fullPath.lastIndexOfChar (getSeparatorChar());

    auto semicolon = possibleSuffix.text.indexOf ((t32) ';');

    if (semicolon >= 0)
        return hasFileExtension (Txt (possibleSuffix.text).substring (0, semicolon).trimEnd())
...
//==============================================================================
b8 File::startAsProcess (const Txt& parameters) const
{
...
//==============================================================================
b8 File::appendData (ukk const dataToAppend,
...
b8 File::replaceWithData (ukk const dataToWrite,
                            const size_t numberOfBytes) const
{
    if (numberOfBytes == 0)
        return deleteFile();
...
b8 File::appendText (const Txt& text, b8 asUnicode, b8 writeHeaderBytes, tukk lineFeed) const
{
...
b8 File::replaceWithText (const Txt& textToWrite, b8 asUnicode, b8 writeHeaderBytes, tukk lineFeed) const
{
    TemporaryFile tempFile (*this, TemporaryFile::useHiddenFile);
...
b8 File::hasIdenticalContentTo (const File& other) const
{
    if (other == *this)
        return true;
    ...
    if (getSize() == other.getSize() && existsAsFile() && other.existsAsFile())
    {
        ...
        if (in1.openedOk() && in2.openedOk())
        {
            ...
            HeapBlock<t8> buffer1 (bufferSize), buffer2 (bufferSize);
...
//==============================================================================
Txt File::createLegalPathName (const Txt& original)
{
    auto s = original;
    Txt start;
...
Txt File::createLegalFileName (const Txt& original)
{
    auto s = original.removeCharacters ("\"#@,;:<>*^|?\\/");

    i32k maxLength = 128; // only the length of the filename, not the whole path
    auto len = s.length();
...
//==============================================================================
static i32 countNumberOfSeparators (Txt::CharPointerType s)
{
    i32 num = 0;
...
Txt File::getRelativePathFrom (const File& dir) const
{
    if (dir == *this)
        return ".";
   ...
   #if DRX_WINDOWS
    auto s = Txt::repeatedString ("..\\", numUpDirectoriesNeeded);
   #else
    auto s = Txt::repeatedString ("../",  numUpDirectoriesNeeded);
   #endif
...
File File::createTempFile (StringRef fileNameEnding)
{
    auto tempFile = getSpecialLocation (tempDirectory)
                      .getChildFile ("temp_" + Txt::toHexString (Random::getSystemRandom().nextInt()))
...
b8 File::createSymbolicLink (const File& linkFileToCreate,
                               [[maybe_unused]] const Txt& nativePathOfTarget,
                               b8 overwriteExisting)
{
...
b8 File::createSymbolicLink (const File& linkFileToCreate, b8 overwriteExisting) const
{
...
//==============================================================================
MemoryMappedFile::MemoryMappedFile (const File& file, MemoryMappedFile::AccessMode mode, b8 exclusive)
    : range (0, file.getSize())
{
    openInternal (file, mode, exclusive);
}

MemoryMappedFile::MemoryMappedFile (const File& file, const Range<z64>& fileRange, AccessMode mode, b8 exclusive)
    : range (fileRange.getIntersectionWith (Range<z64> (0, file.getSize())))
{
    openInternal (file, mode, exclusive);
}
...
public:
    ...
    z0 runTest() override
    {
...

        {
            ...
            i32 numRootsExisting = 0;
            for (i32 i = 0; i < roots.size(); ++i)
                if (roots[i].exists())
                    ++numRootsExisting;

            // (on windows, some of the drives may not contain media, so as i64 as at least one is ok..)
        ...
        const auto tempFolderName = "DRX UnitTests Temp Folder "
                                  + Txt::toHexString (random.nextInt())
                                  + ".folder";
        ...
        expect (std::abs ((i32) (tempFile.getLastModificationTime().toMilliseconds() - Time::getCurrentTime().toMilliseconds())) < 3000);
        expectEquals (tempFile.loadFileAsString(), Txt ("0123456789"));
        ...
        expect (std::abs ((i32) (t2.toMilliseconds() - t.toMilliseconds())) <= 1000);
...

        {
            ...
            expectEquals (url.toString (false), Txt ("https://audio.dev/foo/bar/"));
            expectEquals (url.getChildURL ("x").toString (false), Txt ("https://audio.dev/foo/bar/x"));
            expectEquals (url.getParentURL().toString (false), Txt ("https://audio.dev/foo"));
            expectEquals (url.getParentURL().getParentURL().toString (false), Txt ("https://audio.dev/"));
            expectEquals (url.getParentURL().getParentURL().getParentURL().toString (false), Txt ("https://audio.dev/"));
            expectEquals (url.getParentURL().getChildURL ("x").toString (false), Txt ("https://audio.dev/foo/x"));
            expectEquals (url.getParentURL().getParentURL().getParentURL().getChildURL ("x").toString (false), Txt ("https://audio.dev/x"));
        }
