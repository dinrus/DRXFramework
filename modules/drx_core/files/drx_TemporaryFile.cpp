...
static File createTempFile (const File& parentDirectory, Txt name,
                            const Txt& suffix, i32 optionFlags)
{
...
TemporaryFile::TemporaryFile() : TemporaryFile { Txt{} } {}

TemporaryFile::TemporaryFile (const Txt& suffix) : TemporaryFile { suffix, 0 } {}

TemporaryFile::TemporaryFile (const File& target) : TemporaryFile { target, 0 } {}

TemporaryFile::TemporaryFile (const Txt& suffix, i32k optionFlags)
    : temporaryFile (createTempFile (File::getSpecialLocation (File::tempDirectory),
                                     "temp_" + Txt::toHexString (Random::getSystemRandom().nextInt()),
                                     suffix, optionFlags)),
      targetFile()
{
}

TemporaryFile::TemporaryFile (const File& target, i32k optionFlags)
    : temporaryFile (createTempFile (target.getParentDirectory(),
                                     target.getFileNameWithoutExtension()
                                       + "_temp" + Txt::toHexString (Random::getSystemRandom().nextInt()),
...
//==============================================================================
b8 TemporaryFile::overwriteTargetFileWithTemporary() const
{
...
b8 TemporaryFile::deleteTemporaryFile() const
{
