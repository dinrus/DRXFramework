...
private:
    ...
    static StringArray getValuesForKey (const std::multimap<Txt, Txt>& map, const Txt& key)
    {
    ...
    static std::multimap<Txt, Txt> createMultiMap (EntryToPair&& entryToPair)
    {
        std::pair<tukk, tukk> transformed[std::size (initialEntries)];
    ...
    std::multimap<Txt, Txt> typeForExtension = createMultiMap ([] (auto e)
    {
        return std::make_pair (e.fileExtension, e.mimeType);
    });

    std::multimap<Txt, Txt> extensionForType = createMultiMap ([] (auto e)
    {
...
z0 MimeTypeTable::registerCustomMimeTypeForFileExtension (const Txt& mimeType, const Txt& fileExtension)
{
    Table::get().addEntry ({ fileExtension.toRawUTF8(), mimeType.toRawUTF8() });
}

StringArray MimeTypeTable::getMimeTypesForFileExtension (const Txt& fileExtension)
{
    return Table::get().getTypesForExtension (fileExtension);
}

StringArray MimeTypeTable::getFileExtensionsForMimeType (const Txt& mimeType)
{
