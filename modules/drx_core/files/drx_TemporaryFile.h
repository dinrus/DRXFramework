...
/**
    ...
    @code
    {
        ...
        if (auto out = std::unique_ptr<FileOutputStream> (temp.getFile().createOutputStream()))
        {
            ...
            b8 succeeded = temp.overwriteTargetFileWithTemporary();
        }
...
public:
    ...
    */
    explicit TemporaryFile (const Txt& suffix);
    ...
    */
    TemporaryFile (const Txt& suffix,
                   i32 optionFlags);
...
private:
    ...
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (TemporaryFile)
};
