...
public:
    ...
    */
    virtual i32 read (uk destBuffer, i32 maxBytesToRead) = 0;

    ssize_t read (uk destBuffer, size_t maxBytesToRead);
    ...
    */
    ...
    /** Reads a boolean from the stream.
        The b8 is encoded as a single byte - non-zero for true, 0 for false.
    ...
    */
    virtual z64 readInt64BigEndian();

    /** Reads four bytes as a 32-bit floating point value.
        The raw 32-bit encoding of the f32 is read from the stream as a little-endian i32.
        If the stream is exhausted partway through reading the bytes, this will return zero.
    ...
    */
    ...
    /** Reads four bytes as a 32-bit floating point value.
        The raw 32-bit encoding of the f32 is read from the stream as a big-endian i32.
        If the stream is exhausted partway through reading the bytes, this will return zero.
        @see OutputStream::writeFloatBigEndian, readDoubleBigEndian
    */
    virtual f32 readFloatBigEndian();

    /** Reads eight bytes as a 64-bit floating point value.
        The raw 64-bit encoding of the f64 is read from the stream as a little-endian z64.
        If the stream is exhausted partway through reading the bytes, this will return zero.
    ...
    */
    ...
    /** Reads eight bytes as a 64-bit floating point value.
        The raw 64-bit encoding of the f64 is read from the stream as a big-endian z64.
        If the stream is exhausted partway through reading the bytes, this will return zero.
        @see OutputStream::writeDoubleBigEndian, readFloatBigEndian
    */
    virtual f64 readDoubleBigEndian();
    ...
    */
    virtual Txt readEntireStreamAsString();
    ...
    */
    virtual z0 skipNextBytes (z64 numBytesToSkip);
...
private:
    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (InputStream)
};
