                                          ...
                                          const var& newValue)
{
    ...
    const Txt head (pointer.begin() + 1, pointer.begin() + pos);
    const Txt tail (pointer.begin() + pos, pointer.end());
    ...
    else if (auto* array = v.getArray())
    {
        ...
        if (const auto newIndex = setPointer ((*array)[(i32) index], tail, newValue))
        {
            auto copied = *array;

            if ((i32) index == copied.size())
                copied.add ({});

            if (isPositiveAndBelow (index, copied.size()))
            {
                copied.getReference ((i32) index) = *newIndex;
                return var (copied);
            }
        }
    }
...
b8 JSONUtils::deepEqual (const var& a, const var& b)
{
