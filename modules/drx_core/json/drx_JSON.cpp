...
struct JSONParser
{
    JSONParser (Txt::CharPointerType text) : startLocation (text), currentLocation (text) {}

    Txt::CharPointerType startLocation, currentLocation;
    ...
    struct ErrorException
    {
        ...
        Txt getDescription() const   { return Txt (line) + ":" + Txt (column) + ": error: " + message; }
        Result getResult() const        { return Result::fail (getDescription()); }
    };

    [[noreturn]] z0 throwError (drx::Txt message, Txt::CharPointerType location)
    {
        ErrorException e;
    ...
    z0 skipWhitespace()             { currentLocation = currentLocation.findEndOfWhitespace(); }
    t32 readChar()             { return currentLocation.getAndAdvance(); }
    t32 peekChar() const       { return *currentLocation; }
    b8 matchIf (t8 c)             { if (peekChar() == (t32) c) { ++currentLocation; return true; } return false; }
    b8 isEOF() const                { return peekChar() == 0; }

    b8 matchString (tukk t)
    {
        while (*t != 0)
            if (! matchIf (*t++))
                return false;
    ...
    static constexpr t32 asCodePoint (CharPointer_UTF16::CharType codeUnit)
    {
        return (t32) (u32) (u16) codeUnit;
    }
    ...
    Txt parseString (const t32 quoteChar)
    {
    ...
    var parseNumber (b8 isNegative)
    {
        auto originalPos = currentLocation;

        z64 intValue = readChar() - '0';
    ...
    var parseObject()
    {
        ...
        for (;;)
        {
            ...
            if (c != '"')
                throwError ("Expected a property name in f64-quotes", errorLocation);
...
{
    static z0 writeEscapedChar (OutputStream& out, u16k value)
    {
        out << "\\u" << Txt::toHexString ((i32) value).paddedLeft ('0', 4);
    }

    static z0 writeString (OutputStream& out, Txt::CharPointerType t, JSON::Encoding encoding)
    {
        for (;;)
        {
                            ...
                            case JSON::Encoding::ascii:
                                ...
                                else
                                {
                                    ...
                                    for (auto& codeUnit : codeUnits)
                                        writeEscapedChar (out, (u16) codeUnit);
                                }
                                break;
                        }
                    }
    ...
    static z0 writeSpaces (OutputStream& out, i32 numSpaces)
    {
        out.writeRepeatedByte (' ', (size_t) numSpaces);
    }

    static z0 writeArray (OutputStream& out, const Array<var>& array, const JSON::FormatOptions& format)
    {
        out << '[';
...
z0 JSON::writeToStream (OutputStream& out, const var& v, const FormatOptions& opt)
{
    if (v.isString())
    {
        out << '"';
    ...
    {
        out << (static_cast<b8> (v) ? "true" : "false");
    }
    else if (v.isDouble())
    {
        auto d = static_cast<f64> (v);
...
Txt JSON::toString (const var& v, const FormatOptions& opt)
{
...
Result JSON::parse (const Txt& text, var& result)
{
    try
    {
...
Txt JSON::toString (const var& data, const b8 allOnOneLine, i32 maximumDecimalPlaces)
{
...
z0 JSON::writeToStream (OutputStream& output, const var& data, const b8 allOnOneLine, i32 maximumDecimalPlaces)
{
    writeToStream (output, data, FormatOptions{}.withSpacing (allOnOneLine ? Spacing::singleLine : Spacing::multiLine)
                                                .withMaxDecimalPlaces (maximumDecimalPlaces));
}

Txt JSON::escapeString (StringRef s)
{
...
Result JSON::parseQuotedString (Txt::CharPointerType& t, var& result)
{
    try
    {
        JSONParser parser (t);
...
public:
    ...
    static Txt createRandomIdentifier (Random& r)
    {
        ...
        for (i32 i = 0; i < numElementsInArray (buffer) - 1; ++i)
        {
            static const t8 chars[] = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_-:";
    ...
    // Creates a random f64 that can be easily stringified, to avoid
    ...
    static var createRandomVar (Random& r, i32 depth)
    {
    ...
    z0 expectCharacterEncoding (t32 character, const Txt& expectedOutput, JSON::Encoding encoding)
    {
        const auto input = Txt::charToString (character);
    ...
    z0 expectNoEscapeSequence (t32 input)
    {
        const auto inputString = Txt::charToString (input);
    ...
    z0 expectEscapeSequenceForAllEncodings (t32 input, const Txt& escapeSequence)
    {
    ...
    z0 expectEscapeSequenceForAsciiEncodingOnly (t32 input, const Txt& escapeSequence)
    {
        expectCharacterEncoding (input, escapeSequence, JSON::Encoding::ascii);
        expectCharacterEncoding (input, Txt::charToString (input), JSON::Encoding::utf8);
    }
    ...
    z0 runTest() override
    {
        ...
        {
            for (t32 c = 32; CharacterFunctions::isAscii (c); ++c)
            {
                if (c != '"')
        ...
        beginTest ("Unicode characters are escaped for ASCII encoding only");
        {
            ...
            expectEscapeSequenceForAsciiEncodingOnly ((t32) 0x0080, "\\u0080");
            expectEscapeSequenceForAsciiEncodingOnly ((t32) 0x07FF, "\\u07ff");

            // First and last 3 byte UTF-8 code points
            expectEscapeSequenceForAsciiEncodingOnly ((t32) 0x0800, "\\u0800");
            expectEscapeSequenceForAsciiEncodingOnly ((t32) 0xffff, "\\uffff");

            // Code points at the UTF-16 surrogate boundaries
            expectEscapeSequenceForAsciiEncodingOnly ((t32) 0xd7ff, "\\ud7ff");
            expectEscapeSequenceForAsciiEncodingOnly ((t32) 0xe000, "\\ue000");

            // First and last 4 byte UTF-8 code points (also first and last UTF-16 surrogate pairs)
            expectEscapeSequenceForAsciiEncodingOnly ((t32) 0x010000, "\\ud800\\udc00");
            expectEscapeSequenceForAsciiEncodingOnly ((t32) 0x10ffff, "\\udbff\\udfff");
        }
