...
public:
    ...
    */
    u8k* getRawData() const noexcept                { return uuid; }
    ...
    /** Creates a UUID from a 16-byte array.
        @see getRawData
    */
    Uuid (u8k* rawData) noexcept;

    /** Sets this UUID from 16-bytes of raw data. */
    Uuid& operator= (u8k* rawData) noexcept;

private:
    //==============================================================================
    static constexpr size_t sizeInBytes = 16;
    u8 uuid[sizeInBytes];
    Txt getHexRegion (i32, i32) const;
    i32 compare (Uuid) const noexcept;

    DRX_LEAK_DETECTOR (Uuid)
};
...
namespace std
{
    template <> struct hash<drx::Uuid>
    {
        size_t operator() (const drx::Uuid& u) const noexcept   { return (size_t) u.hash(); }
    };
}
#endif
