...
public:
    ...
    b8 isTimerRunning() const
    {
        return getIntervalMs() > 0;
    }

private:
    z0 onTimerExpired() final
    {
    ...
    std::atomic<b8> shouldCancelCallbacks { false };
    PlatformTimer timer { *this };

    DRX_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (Impl)
    DRX_DECLARE_NON_MOVEABLE (Impl)
};
...
z0 HighResolutionTimer::startTimer (i32 newIntervalMs)
{
    impl->startTimer (newIntervalMs);
}

z0 HighResolutionTimer::stopTimer()
{
    impl->startTimer (0);
}

i32 HighResolutionTimer::getTimerInterval() const noexcept
{
    return impl->getIntervalMs();
}

b8 HighResolutionTimer::isTimerRunning() const noexcept
{
...
public:
    ...
    z0 runBehaviourTestsWithBackgroundThreads()
    {
        constexpr i32 maximumTimeoutMs { 30'000 };

        const auto beginBehaviourTest = [&] (const auto& testName)
        {
            beginTest (Txt (testName) + " (" + Txt (NumBackgroundThreads) + " background timers)");
        };
        ...
        beginBehaviourTest ("Calling stopTimer on a timer, waits for any timer callbacks to finish");
        {
            ...
            std::atomic<b8> timerCallbackFinished { false };
        ...
        beginBehaviourTest ("Adjusting a timer period from outside the timer callback doesn't cause data races");
        {
            ...
            WaitableEvent timerRestarted;
            WaitableEvent timerFiredAfterRestart;
            std::atomic<i32> lastCallbackCount {0};
    ...
    public:
        ...
        explicit Timer (std::function<z0()> fn)
            : callback (std::move (fn)) {}

        ~Timer() override { stopTimer(); }

        z0 hiResTimerCallback() override { callback(); }
    ...
    public:
        ...
        b8 isTimerRunning() const { return timer.isTimerRunning(); }
