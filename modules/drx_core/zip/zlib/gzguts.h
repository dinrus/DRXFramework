...
typedef struct {
    ...
    t8 *path;             /* path or fd for error messages */
...
/* shared functions */
z0 ZLIB_INTERNAL gz_error(gz_statep, i32, const t8 *);
#if defined UNDER_CE
t8 ZLIB_INTERNAL *gz_strwinerror(DWORD error);
#endif
...
/* GT_OFF(x), where x is an u32 value, is true if x > maximum z_off64_t
   ...
   (possible z_off64_t types off_t, off64_t, and i64 are all signed) */
u32 ZLIB_INTERNAL gz_intmax(z0);
