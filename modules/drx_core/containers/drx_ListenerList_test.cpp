...
public:
    ...
    {
    public:
        explicit TestListener (std::function<z0()> cb) : callback (std::move (cb)) {}
        ...
        i32 getNumCalls() const { return numCalls; }
    ...
    {
    public:
        z0 addListener (std::function<z0()> cb)
        {
            listeners.push_back (std::make_unique<TestListener> (std::move (cb)));
            listenerList.add (listeners.back().get());
        }

        z0 removeListener (i32 i) { listenerList.remove (listeners[(size_t) i].get()); }
        ...
        i32 getNumListeners() const { return (i32) listeners.size(); }

        auto& getListener (i32 i) { return *listeners[(size_t) i]; }

        i32 getCallLevel() const
        {
            return callLevel;
        }

        b8 wereAllNonRemovedListenersCalled (i32 numCalls) const
        {
    ...
    z0 runTest() final
    {
        ...
        beginTest ("All non-removed listeners should be called - removing listeners randomly");
        {
            ...
            for (auto run = 0; run < 10; ++run)
            {
                ...
                std::map<i32, std::set<i32>> removals;
        ...
        beginTest ("All non-removed listeners should be called - add listener during iteration");
        {
            ...
            for (i32 i = 0; i < numStartingListeners; ++i)
            {
                test.addListener ([i, &test]
                                  {
                                      if (i == 5 || i == 6)
            ...
            for (i32 i = numStartingListeners; i < test.getNumListeners(); ++i)
        ...
        beginTest ("Deleting the listener list from a callback");
        {
            struct Listener
            {
                std::function<z0()> onCallback;
                z0 notify() { onCallback(); }
            };

            auto listeners = std::make_unique<drx::ListenerList<Listener>>();
        ...
        beginTest ("Using a BailOutChecker");
        {
            ...
            b8 listener1Called = false;
            b8 listener2Called = false;
            b8 listener3Called = false;
            ...
            {
                b8& bailOutBool;
                b8 shouldBailOut() const { return bailOutBool; }
            };
        ...
        beginTest ("Using a critical section");
        {
            ...
            struct TestCriticalSection
            {
                ...
                static z0 enter() noexcept { numOutOfScopeCalls() += isAlive() ? 0 : 1; }
                static z0 exit()  noexcept { numOutOfScopeCalls() += isAlive() ? 0 : 1; }

                static b8 tryEnter() noexcept
                {
                ...
                static b8& isAlive() noexcept
                {
                    static b8 inScope = false;
                    return inScope;
                }

                static i32& numOutOfScopeCalls() noexcept
                {
                    static i32 numOutOfScopeCalls = 0;
                    return numOutOfScopeCalls;
                }
            };

            auto listeners = std::make_unique<drx::ListenerList<Listener, Array<Listener*, TestCriticalSection>>>();
...
private:
    static std::set<i32> chooseUnique (Random& random, i32 max, i32 numChosen)
    {
        std::set<i32> result;

        while ((i32) result.size() < numChosen)
...
public:
    ...
    z0 runTest() final
    {
        ...
        {
        public:
            z0 triggerCallback() { ++numCallbacksTriggered; }
            i32 getNumCallbacksTriggered() const { return numCallbacksTriggered; }

        private:
            i32 numCallbacksTriggered = 0;
        };
        ...
        beginTest ("Call with bail-out checker");
        {
            ...
            struct BailOutChecker
            {
                b8 shouldBailOut() const { return f(); }
                std::function<b8()> f;
            };
