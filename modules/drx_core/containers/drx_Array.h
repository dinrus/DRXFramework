...
/**
    ...
    Examples of arrays are: Array<i32>, Array<Rectangle> or Array<MyClass*>
    ...
    For holding lists of strings, you can use Array\<Txt\>, but it's usually better to use the
...
public:
    ...
    */
    template <typename TypeToCreateFrom>
    Array (const TypeToCreateFrom* data, i32 numValues)
    {
    ...
    */
    ...
    b8 operator== (const OtherArrayType& other) const
    {
        const ScopedLockType lock (getLock());
    ...
    */
    ...
    z0 fill (const ParameterType& newValue) noexcept
    {
        const ScopedLockType lock (getLock());
    ...
    inline b8 isEmpty() const noexcept
    {
        return size() == 0;
    }
    ...
    */
    ElementType operator[] (i32 index) const
    {
        const ScopedLockType lock (getLock());
    ...
    */
    inline ElementType getUnchecked (i32 index) const
    {
        const ScopedLockType lock (getLock());
        return values[index];
    }
    ...
    /** Returns a direct reference to one of the elements in the array, without checking the index passed in.
        ...
        @see operator[], getFirst, getLast
    */
    inline ElementType& getReference (i32 index) noexcept
    {
        const ScopedLockType lock (getLock());
        return values[index];
    }
    ...
    /** Returns a direct reference to one of the elements in the array, without checking the index passed in.
        ...
        @see operator[], getFirst, getLast
    */
    inline const ElementType& getReference (i32 index) const noexcept
    {
        const ScopedLockType lock (getLock());
        return values[index];
    }
    ...
    */
    i32 indexOf (ParameterType elementToLookFor) const
    {
        const ScopedLockType lock (getLock());
        auto e = values.begin();
        ...
        for (; e != endPtr; ++e)
            if (exactlyEqual (elementToLookFor, *e))
                return static_cast<i32> (e - values.begin());

        return -1;
    }

    /** Возвращает true, если the array contains at least one occurrence of an object.

        @param elementToLookFor     the value or object to look for
        @returns                    true if the item is found
    */
    b8 contains (ParameterType elementToLookFor) const
    {
        const ScopedLockType lock (getLock());
        auto e = values.begin();
        auto endPtr = values.end();
    ...
    */
    z0 insert (i32 indexToInsertAt, ParameterType newElement)
    {
        const ScopedLockType lock (getLock());
    ...
    */
    z0 insertMultiple (i32 indexToInsertAt, ParameterType newElement,
                         i32 numberOfTimesToInsertIt)
    {
    ...
    /** Inserts an array of values into this array at a given position.
        ...
        @see insert, add, addSorted, set
    */
    z0 insertArray (i32 indexToInsertAt,
    ...
    /** Appends a new element at the end of the array as i64 as the array doesn't
        already contain it.
    ...
    */
    b8 addIfNotAlreadyThere (ParameterType newElement)
    {
        const ScopedLockType lock (getLock());
    ...
    /** Replaces an element with a new value.
        ...
        @see add, insert
    */
    z0 set (i32 indexToChange, ParameterType newValue)
    {
        if (indexToChange >= 0)
        {
            const ScopedLockType lock (getLock());
    ...
    */
    z0 setUnchecked (i32 indexToChange, ParameterType newValue)
    {
        const ScopedLockType lock (getLock());
    ...
    */
    template <typename Type>
    z0 addNullTerminatedArray (const Type* const* elementsToAdd)
    {
        i32 num = 0;
    ...
    */
    template <class OtherArrayType>
    z0 swapWith (OtherArrayType& otherArray) noexcept
    {
    ...
    */
    template <class OtherArrayType>
    std::enable_if_t<! std::is_pointer_v<OtherArrayType>, z0>
    ...
    */
    ...
    i32 addSorted (ElementComparator& comparator, ParameterType newElement)
    {
        const ScopedLockType lock (getLock());
    ...
    /** Inserts a new element into the array, assuming that the array is sorted.
        ...
        @see addSorted, sort
    */
    z0 addUsingDefaultSort (ParameterType newElement)
    {
    ...
    */
    ...
    i32 indexOfSorted (ElementComparator& comparator, TargetValueType elementToLookFor) const;
    ...
    */
    ElementType removeAndReturn (i32 indexToRemove)
    {
        const ScopedLockType lock (getLock());
    ...
    */
    z0 remove (const ElementType* elementToRemove)
    {
        jassert (elementToRemove != nullptr);
        const ScopedLockType lock (getLock());

        jassert (values.begin() != nullptr);
        auto indexToRemove = (i32) (elementToRemove - values.begin());
    ...
    */
    i32 removeFirstMatchingValue (ParameterType valueToRemove)
    {
        const ScopedLockType lock (getLock());
        auto* e = values.begin();
    ...
    /** Removes items from the array.
        ...
        @see remove, removeRange, removeIf
    */
    i32 removeAllInstancesOf (ParameterType valueToRemove)
    {
        i32 numRemoved = 0;
        const ScopedLockType lock (getLock());

        for (i32 i = values.size(); --i >= 0;)
        {
    ...
    /** Removes items from the array.
        ...
        @param predicate   the condition when to remove an item. Must be a callable
                           type that takes an ElementType and returns a b8

        @return how many objects were removed.
        @see remove, removeRange, removeAllInstancesOf
    */
    template <typename PredicateType>
    i32 removeIf (PredicateType&& predicate)
    {
        i32 numRemoved = 0;
        const ScopedLockType lock (getLock());
    ...
    */
    z0 removeLast (i32 howManyToRemove = 1)
    {
    ...
    */
    template <class OtherArrayType>
    z0 removeValuesIn (const OtherArrayType& otherArray)
    {
    ...
    */
    template <class OtherArrayType>
    z0 removeValuesNotIn (const OtherArrayType& otherArray)
    {
        const typename OtherArrayType::ScopedLockType lock1 (otherArray.getLock());
        const ScopedLockType lock2 (getLock());
    ...
    */
    ...
    /** Sorts the elements in the array.
        ...
        @code
        i32 compareElements (ElementType first, ElementType second);
        @endcode
    ...
    */
    template <class ElementComparator>
    z0 sort (ElementComparator& comparator, b8 retainOrderOfEquivalentItems = false);
    ...
    */
    ...
    z0 swapWithArray (Array& other) noexcept { swapWith (other); }
   #endif
...
private:
    ...
    z0 removeInternal (i32 indexToRemove)
    {
        values.removeElements (indexToRemove, 1);
        minimiseStorageAfterRemoval();
    }

    z0 minimiseStorageAfterRemoval()
    {
        if (values.capacity() > jmax (minimumAllocatedSize, values.size() * 2))
            values.shrinkToNoMoreThan (jmax (values.size(), jmax (minimumAllocatedSize, 64 / (i32) sizeof (ElementType))));
    }
};

//==============================================================================
template <typename ElementType, typename TypeOfCriticalSectionToUse, i32 minimumAllocatedSize>
template <typename ElementComparator, typename TargetValueType>
i32 Array<ElementType, TypeOfCriticalSectionToUse, minimumAllocatedSize>::indexOfSorted (
    [[maybe_unused]] ElementComparator& comparator,
    TargetValueType elementToLookFor) const
{
    const ScopedLockType lock (getLock());

    for (i32 s = 0, e = values.size();;)
    {
        if (s >= e)
            return -1;
...
template <typename ElementType, typename TypeOfCriticalSectionToUse, i32 minimumAllocatedSize>
template <class ElementComparator>
z0 Array<ElementType, TypeOfCriticalSectionToUse, minimumAllocatedSize>::sort (
    [[maybe_unused]] ElementComparator& comparator,
    b8 retainOrderOfEquivalentItems)
{
    const ScopedLockType lock (getLock());
