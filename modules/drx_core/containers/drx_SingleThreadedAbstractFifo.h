...
public:
    ...
    explicit SingleThreadedAbstractFifo (i32 sizeIn)
        : size (sizeIn)
    {
    ...
    i32 getRemainingSpace() const   { return size - numReadable; }

    /** Returns the number of pending elements present in the buffer. */
    i32 getNumReadable() const      { return numReadable; }
    ...
    */
    std::array<Range<i32>, 2> write (i32 num)
    {
    ...
    */
    std::array<Range<i32>, 2> read (i32 num)
    {
...
private:
    i32 size = 0, readPos = 0, numReadable = 0;
};
