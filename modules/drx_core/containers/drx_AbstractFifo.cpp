...
AbstractFifo::AbstractFifo (i32 capacity) noexcept : bufferSize (capacity)
{
    jassert (bufferSize > 0);
}

i32 AbstractFifo::getTotalSize() const noexcept    { return bufferSize; }
i32 AbstractFifo::getFreeSpace() const noexcept    { return bufferSize - getNumReady() - 1; }

i32 AbstractFifo::getNumReady() const noexcept
{
...
z0 AbstractFifo::reset() noexcept
{
    validEnd = 0;
    validStart = 0;
}

z0 AbstractFifo::setTotalSize (i32 newSize) noexcept
{
    jassert (newSize > 0);
    reset();
    bufferSize = newSize;
...
//==============================================================================
z0 AbstractFifo::prepareToWrite (i32 numToWrite, i32& startIndex1, i32& blockSize1,
                                   i32& startIndex2, i32& blockSize2) const noexcept
{
    auto vs = validStart.get();
    auto ve = validEnd.get();
...
z0 AbstractFifo::finishedWrite (i32 numWritten) noexcept
{
...
z0 AbstractFifo::prepareToRead (i32 numWanted, i32& startIndex1, i32& blockSize1,
                                  i32& startIndex2, i32& blockSize2) const noexcept
{
    auto vs = validStart.get();
    auto ve = validEnd.get();
...
z0 AbstractFifo::finishedRead (i32 numRead) noexcept
{
...
template <AbstractFifo::ReadOrWrite mode>
z0 AbstractFifo::ScopedReadWrite<mode>::swap (ScopedReadWrite& other) noexcept
{
...
AbstractFifo::ScopedRead  AbstractFifo::read  (i32 numToRead) noexcept     { return { *this, numToRead }; }
AbstractFifo::ScopedWrite AbstractFifo::write (i32 numToWrite) noexcept    { return { *this, numToWrite }; }
...
public:
    ...
    {
        WriteThread (AbstractFifo& f, i32* b, Random rng)
        ...
        z0 run() override
        {
            ...
            while (! threadShouldExit())
            {
                ...
                writer.forEach ([this, &n] (i32 index)  { this->buffer[index] = n++; });
            }
        }
    ...
    z0 runTest() override
    {
        ...
        for (i32 count = 100000; --count >= 0;)
        {
            i32 num = r.nextInt (6000) + 1;
            ...
            reader.forEach ([&failed, &buffer, &n] (i32 index)
            {
