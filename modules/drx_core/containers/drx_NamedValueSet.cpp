...
b8 NamedValueSet::NamedValue::operator== (const NamedValue& other) const noexcept   { return name == other.name && value == other.value; }
b8 NamedValueSet::NamedValue::operator!= (const NamedValue& other) const noexcept   { return ! operator== (other); }
...
z0 NamedValueSet::clear()
{
    values.clear();
}

b8 NamedValueSet::operator== (const NamedValueSet& other) const noexcept
{
...
b8 NamedValueSet::operator!= (const NamedValueSet& other) const noexcept   { return ! operator== (other); }

i32 NamedValueSet::size() const noexcept        { return values.size(); }
b8 NamedValueSet::isEmpty() const noexcept    { return values.isEmpty(); }
...
b8 NamedValueSet::set (const Identifier& name, var&& newValue)
{
    if (auto* v = getVarPointer (name))
    {
...
b8 NamedValueSet::set (const Identifier& name, const var& newValue)
{
    if (auto* v = getVarPointer (name))
    {
        if (v->equalsWithSameType (newValue))
...
b8 NamedValueSet::contains (const Identifier& name) const noexcept
{
    return getVarPointer (name) != nullptr;
}

i32 NamedValueSet::indexOf (const Identifier& name) const noexcept
{
...
b8 NamedValueSet::remove (const Identifier& name)
{
    auto numValues = values.size();
...
Identifier NamedValueSet::getName (i32k index) const noexcept
{
...
const var& NamedValueSet::getValueAt (i32k index) const noexcept
{
    if (isPositiveAndBelow (index, values.size()))
...
var* NamedValueSet::getVarPointerAt (i32 index) noexcept
{
    if (isPositiveAndBelow (index, values.size()))
        return &(values.getReference (index).value);
...
const var* NamedValueSet::getVarPointerAt (i32 index) const noexcept
{
    if (isPositiveAndBelow (index, values.size()))
        return &(values.getReference (index).value);
...
z0 NamedValueSet::setFromXmlAttributes (const XmlElement& xml)
{
    values.clearQuick();
...
z0 NamedValueSet::copyToXmlAttributes (XmlElement& xml) const
{
    for (auto& i : values)
    {
