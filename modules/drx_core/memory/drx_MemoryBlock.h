...
public:
    ...
    */
    MemoryBlock (ukk dataToInitialiseFrom, size_t sizeInBytes);
    ...
    */
    b8 operator== (const MemoryBlock& other) const noexcept;
    ...
    */
    ...
    /** Возвращает true, если the data in this MemoryBlock matches the raw bytes passed-in. */
    b8 matches (ukk data, size_t dataSize) const noexcept;

    //==============================================================================
    /** Returns a z0 pointer to the data.

        Note that the pointer returned will probably become invalid when the
        block is resized.
    */
    uk getData() noexcept                                        { return data; }
    ...
    */
    ...
    t8& operator[] (const Type offset) noexcept                   { return data [offset]; }

    /** Returns a byte from the memory block. */
    template <typename Type>
    const t8& operator[] (const Type offset) const noexcept       { return data [offset]; }

    /** Returns an iterator for the data. */
    tuk begin() noexcept                                          { return data; }

    /** Returns an iterator for the data. */
    tukk begin() const noexcept                              { return data; }

    /** Returns an end-iterator for the data. */
    tuk end() noexcept                                            { return begin() + getSize(); }

    /** Returns an end-iterator for the data. */
    tukk end() const noexcept                                { return begin() + getSize(); }

    //==============================================================================
    /** Возвращает true, если the memory block has zero size. */
    b8 isEmpty() const noexcept                                   { return getSize() == 0; }
    ...
    /** Resizes the memory block.
        ...
        @see ensureSize
    */
    z0 setSize (size_t newSize,
                  b8 initialiseNewSpaceToZero = false);
    ...
    /** Increases the block's size only if it's smaller than a given size.
        ...
        @see setSize
    */
    z0 ensureSize (size_t minimumSize,
                     b8 initialiseNewSpaceToZero = false);
    ...
    */
    z0 insert (ukk dataToInsert, size_t numBytesToInsert, size_t insertPosition);
    ...
    */
    z0 removeSection (size_t startByte, size_t numBytesToRemove);
    ...
    */
    z0 copyFrom (ukk srcData,
                   i32 destinationOffset,
    ...
    */
    z0 swapWith (MemoryBlock& other) noexcept;
    ...
    /** Parses a string of hexadecimal numbers and writes this data into the memory block.
        ...
        @see Txt::toHexString()
    */
    z0 loadFromHexString (StringRef sourceHexString);

    //==============================================================================
    /** Sets a number of bits in the memory block, treating it as a i64 binary sequence. */
    z0 setBitRange (size_t bitRangeStart,
                      size_t numBits,
                      i32 binaryNumberToApply) noexcept;

    /** Reads a number of bits from the memory block, treating it as one i64 binary sequence */
    i32 getBitRange (size_t bitRangeStart,
                     size_t numBitsToRead) const noexcept;

    //==============================================================================
    /** Returns a string of characters in a DRX-specific text encoding that represents the
        binary contents of this block.

        This uses a DRX-specific (i.e. not standard!) 64-bit encoding system to convert binary
    ...
    */
    ...
    /** Takes a string created by MemoryBlock::toBase64Encoding() and extracts the original data.
        ...
        Note that these methods use a DRX-specific (i.e. not standard!) 64-bit encoding system.
...
private:
    //==============================================================================
    using HeapBlockType = HeapBlock<t8, true>;
    HeapBlockType data;
    size_t size = 0;

    DRX_LEAK_DETECTOR (MemoryBlock)
};
